...
int COpSensCal::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!m_hInst)
	{
		TRACE("DX_OPSENSCAL control load error....\n");
		return -1;
	}
	...
	if (axCreate == NULL)
	{
		TRACE("DX_OPSENSCAL axCreate proc error...\n");
		return -1;
	}
...
void COpSensCal::OnDestroy() 
{
	...
	{
		delete m_COpSensCal;
		m_IOpSensCal->DetachDispatch();	
		m_IOpSensCal = nullptr;
	}
...
BOOL COpSensCal::CreateOpSensCal(CWnd* parent, CString root)
{
	...
	return CWnd::Create(NULL, "OpSensCal", WS_CHILD|WS_VISIBLE, CRect(0,0,0,0), parent, 1001);
}
...
void COpSensCal::SetUserName(LPCTSTR sUserName)
{
	if (m_IOpSensCal == NULL) return;
	m_IOpSensCal->SetUserName(sUserName);
}
