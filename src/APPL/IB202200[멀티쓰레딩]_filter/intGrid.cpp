...
int CalcDrawThreshold(int m_iTime)
{
	constexpr int MIN_TIME = 300;
	constexpr int MAX_TIME = 5970;

	constexpr int MIN_THRESHOLD = 5;    // 거의 실시간
	constexpr int MAX_THRESHOLD = 80;   // 매우 느림 (조절 가능)

	// 범위 보정
	if (m_iTime < MIN_TIME) m_iTime = MIN_TIME;
	if (m_iTime > MAX_TIME) m_iTime = MAX_TIME;

	const double ratio =
		(double)(m_iTime - MIN_TIME) / (MAX_TIME - MIN_TIME);

	return (int)(MIN_THRESHOLD +
		ratio * (MAX_THRESHOLD - MIN_THRESHOLD));
}
