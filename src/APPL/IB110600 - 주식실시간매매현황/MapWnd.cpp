...
void CMapWnd::CreateControl()
{
	...
	cRc.SetRect(panelRc.left + 82, panelRc.top + GAP_TOP + GAP_CTRLT, 
				panelRc.left + 82 + ACC_WIDTH, panelRc.top + GAP_TOP + GAP_CTRLT + CTRL_HEIGHT);
	...
	tmpRC.SetRect(panelRc.left , panelRc.top,
				panelRc.left  + 20, panelRc.top + 20);

	m_pMarketpicker = std::make_unique<CContainerWnd>();
	m_pMarketpicker->Create(NULL, "", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, tmpRC, this, IDC_CTRL_MARKET);
	m_pMarketpicker->SetParent(m_pWizard);

	int major = m_pWizard->SendMessage(WM_USER, MAKEWPARAM(variantDLL, majorCC));
	stmp.Format("/tIB110600/k2/v%d", major);
	m_pMarketpicker->CreateControl(m_strRoot, "CX_MarketPicker", "cxMarket", tmpRC, stmp);
	m_sMarket = m_pMarketpicker->GetCtrlProperty("sMarket");
	m_sMarket.TrimRight();
	
	m_pMarketpicker->GetWindowRect(tmpRC);
	ScreenToClient(tmpRC);
	tmpRC.OffsetRect(5, 6);
	m_pMarketpicker->MoveWindow(tmpRC);

	m_pMarketpicker->ShowWindow(SW_HIDE);
}
...
long CMapWnd::OnMsgMKDLL(WPARAM wParam, LPARAM lParam)
{
	...
	case 100: //내부 마켓컨트롤 클릭시 메시지
	{
		...
		CString strBuffer;
		strBuffer.Format("%s\t%s\t", "MarketTrigger1106", m_sMarket);
...
//	AfxMessageBox(strTrigger);
	while (!strTrigger.IsEmpty())
	{
		...
		else if (!sym.CompareNoCase("cxMarket"))
		{
			CString strBuffer;
			strBuffer.Format("%s\t%s\t%s\t", MAP_PROC, "Market", str);
			Procedure(strBuffer, m_nKey, 0);
		}
		else if (!sym.CompareNoCase("edMarketTrigger"))
		{
			CString sMarket{};
			sMarket.Format("%s", str);
		
			_variant_t var((LPCTSTR)sMarket);
			m_pMarketpicker->SetCtrlProperty("sMarket", var);

			CString strBuffer;
			strBuffer.Format("%s\t%s\t", "MarketTrigger1106",  str);
			Procedure(strBuffer, m_nKey, 0);
		}
	}
}
