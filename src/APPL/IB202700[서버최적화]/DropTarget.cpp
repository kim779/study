...
BOOL CDropTarget::Register(gsl::not_null<CintGrid*> pintGrid)
{
	if (m_registered)
		return FALSE;

	ASSERT(pintGrid->IsKindOf(RUNTIME_CLASS(CintGrid)));
	if (!pintGrid->IsKindOf(RUNTIME_CLASS(CintGrid)))
		return FALSE;
...
{
	ASSERT(m_intGrid);
	if (m_intGrid && pWnd->GetSafeHwnd() == m_intGrid->GetSafeHwnd())
