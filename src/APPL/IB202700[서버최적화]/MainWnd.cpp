...
CMainWnd::CMainWnd(CWnd *pWnd) : _pApp(static_cast<CIB202700App*>(AfxGetApp()))
{
	m_pWnd = pWnd;
	m_pToolWnd = nullptr;
	m_pTreeWnd = nullptr;
...
void CMainWnd::CheckRTSTimer(bool bFirst)
{
	...
	if (icount == 0)
	{
		...
		if (!bFirst)  //관심설정창 닫은후 호출 
		{
			if (iTime != m_iTime)  //실시간 데이터 수신 설정률이 변한 경우만 타이머 다시 세팅
			{
				m_slog.Format("[IB202700][CheckRTSTimer2] KillTimer 고객용 iTime=[%d] m_iTime=[%d]", iTime, m_iTime);
	...
	case TRKEY_INTER_MEMO_SEARCH:
		{
			...
			if (chRet == '1')
			{
				...
				auto& map = ((CIB202700App*)AfxGetApp())->getMemoMap(nullptr, "");
