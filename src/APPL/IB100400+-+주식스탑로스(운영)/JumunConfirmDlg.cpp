...
void CJumunConfirmDlg::SetValue(CString sAccount, CString sAccnName, CString sCodeName, CString sGubn, CString sCrtText, CString sVol, CString sPrice)
{
	...
	m_strValue[5] = SetComma(sVol);
	m_strValue[6] = SetComma(sPrice);
}
...
//Lable1
	const int iboxw = 240;
	const int ilblwidth= 98;
	const int ihgap = 2;
	const int ih= 19;
	int irow = 4;
	if(m_bCrtType) irow = 5;

	const CRect rect = CRect(4, 30, ilblwidth, 115);
	CRect rc1 =rect;
...
//Lable2
	...
	rc1 = rect;
	const int ilblw = rc1.left = rect.right;
	rc1.bottom = rect.top + ih* irow;
	rc1.right = iboxw;
	dc.Rectangle(rc1); 
	const int idivtop = rc1.top = rc1.bottom + ihgap;
	rc1.bottom = rc1.top + ih* 2;
	dc.Rectangle(rc1); 
	dc.SelectObject(pOldBrush); 
	DeleteObject(pOldBrush);
