...
{
	const int icnt = m_LogList.GetItemCount();
	if( icnt > 0)
	{
		if(nItem == 0)
...
void CLogDlg::LoadLogFile()
{
	...
	strTmp = strRead;
	CString sDate = SplitString(strTmp, (char)tabCHAR);
	if(sCurDate != sDate)
	{
	...
	if( !SplitString(strRead, (char)newlineCHAR, arList, 999) ) return;

	for(int i=arList.GetSize()-1; i>=0; i--)
	{
		if( SplitString(arList[i], (char)tabCHAR, arTabList) < 6) continue;
...
void CLogDlg::OnNextbutton() 
{
	...
	const int istart = m_LogList.GetItemCount()+1;
