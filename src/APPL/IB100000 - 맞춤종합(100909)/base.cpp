...
void CBase::ParseTriggerValue(CString sData)
{
	...
	CString slog;
	slog.Format("		[IB100000][%s]<%d> 시작 sData = [%s]      sKey = [%s]       sParse = [%s]\n", __FUNCTION__, __LINE__, sData, sKey, sParse);
	...
	else if (sKey == "1301")
	{
		if (m_strTrigger != sData)			 //같은 트리거 연속처리 방지(종합화면특성상 4번 발생)
		{
			...
			if (m_TriggerKey > 0)
			{
				...
				int iPermission{};
				int m_markettype{};
				if (IsNXTAble(sParse) || sParse.IsEmpty())
				{
					iPermission = 7;
				}
				else
				...
				if (m_sMarket == "KRX")
					...
					m_markettype = 2;
				else if (m_sMarket == "통합")
					m_markettype = 3;
			
				send.Format("%s\t%d\t%d\t%d", "IB100000", m_markettype, iPermission, major);

slog.Format("		[IB100000][%s]<%d> sKey = [%s]       send = [%s] 메인에 타이틀메시지 송신\n", __FUNCTION__, __LINE__,  sKey, send);
OutputDebugString(slog);

				if (m_pMainWnd)
					m_pMainWnd->SendMessage(WM_USER, MMSG_MKMSG_FROM_MAPNMAP, (LPARAM)(LPSTR)(LPCTSTR)send); //SetsMarket 제목타이틀 거래소구분 바꾸기 위한..
			}
		}
	}
	else if (sKey == JPRC)
	...
	else if (sKey == "cxMarket" || sKey == "edMarketTrigger" || sKey == "edMarketTrigger_1000")
	{
		if (sParse.Find("KRX") > -1)
		{
			m_strTrigger.Format("%s\t%s", "edMarketTrigger_1000", "KRX");
			sParse = "KRX";
			m_sMarket = "KRX";
		}
		else if (sParse.Find("NXT") > -1)
		{
			m_strTrigger.Format("%s\t%s", "edMarketTrigger_1000", "NXT");
			sParse = "NXT";
			m_sMarket = "NXT";
		}
		else if (sParse.Find("통합") > -1)
		{
			m_strTrigger.Format("%s\t%s", "edMarketTrigger_1000", "통합");
			sParse = "통합";
			m_sMarket = "통합";
		}

slog.Format("		[IB100000][%s]<%d> sKey=[%s] m_strTrigger = [%s]\n", __FUNCTION__, __LINE__, sKey, m_strTrigger);
OutputDebugString(slog);

		if (sKey == "edMarketTrigger_1000")
			m_parent->SendMessage(WM_USER, MAKEWPARAM(variantDLL, triggerCC), (LPARAM)(LPCSTR)m_strTrigger);
		...
		CString send;
		send.Format("%s%c%s", "IB100000_Market", P_TAB, sParse);
		Variant(pushCC, send);

		send.Format("%s%c%s", "MKTriggerFromMain", P_TAB, sParse);

slog.Format("		[IB100000][%s]<%d> sKey=[%s] MKTriggerFromMain = [%s]\n", __FUNCTION__, __LINE__, sKey, sParse);
OutputDebugString(slog);

		char	buf[256];
		CString file;
		file.Format("%s\\user\\%s\\userconf.ini", m_home, m_user);
		WritePrivateProfileString("IB100000", "MKgubn", sParse, file);
		...
		int iPermission{};
		int m_markettype{};
		if (IsNXTAble(m_strCode) || m_strCode.IsEmpty())
		{
			iPermission = 7;
		}
		else
...
slog.Format("		[IB100000][%s]<%d> [MMSG_MKMSG_FROM_MAPNMAP] send = [%s]\n",__FUNCTION__,  __LINE__,  send);
OutputDebugString(slog);

		if (m_pMainWnd)
			m_pMainWnd->SendMessage(WM_USER, MMSG_MKMSG_FROM_MAPNMAP, (LPARAM)(LPSTR)(LPCTSTR)send);
...
BOOL CBase::CreateMap(CWnd* parent, CRect wRc)
{
	...
	char readB[1024];
	CString userD;
	userD.Format("%s\\%s\\%s\\%s.ini", m_home, "user", m_user, m_user);
	...
	char	buf[16]{};
	file.Format("%s\\user\\%s\\userconf.ini", m_home, m_user);
	GetPrivateProfileString("MKgubn", "Market", "", buf, sizeof(buf), file);
	stmp.Format("%s", buf);
	stmp.TrimRight();
	WritePrivateProfileString("IB100000", "MKgubn", stmp, file);
...
void CBase::OnTimer(UINT nIDEvent) 
{
	...
	if( nIDEvent == 9999)
	{
		...
		if (m_astrSaveList.Lookup(m_strCurKey, sval))
		{
			...
			for (int ii = 0; ii < m_sList.GetSize(); ii++)
			{
				...
				{
					CString send;
					send.Format("%s%c%s, %s", "OnFromDLLMarket", P_TAB, m_strCurKey, sval);

					m_parent->SendMessage(WM_USER, MAKEWPARAM(procDLL, MAKEWORD(major, pane->m_key)), (LPARAM)(LPCTSTR)send);
					m_strCurrValue.Empty();

					//send.Format("%s%c%s", "sendMarketTrigger", P_TAB, sval);
					//m_parent->SendMessage(WM_USER, MAKEWPARAM(procDLL, MAKEWORD(major, m_TriggerKey)), (LPARAM)(LPCTSTR)send);
				}
			}
		}
	...
	if (nIDEvent == DF_TIME_LOAD_AXCODE)
	{
		KillTimer(DF_TIME_LOAD_AXCODE);
...
BOOL CBase::IsNXTAble(CString sCode)
{
...
int CBase::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CWnd::OnCreate(lpCreateStruct) == -1)
		return -1;

	// TODO:  여기에 특수화된 작성 코드를 추가합니다.
	SetTimer(DF_TIME_LOAD_AXCODE, 2000, nullptr);
	return 0;
}
