...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (lpDrawItemStruct != nullptr) {
		pDC = CDC::FromHandle(lpDrawItemStruct->hDC);	// get device context
		r=lpDrawItemStruct->rcItem;					// context rectangle
	}
	...
	if (itemState & ODS_DISABLED){	// DISABLED BUTTON
		...
		if ((itemState & ODS_SELECTED)||m_Checked){
			...
			if( m_nType ){
/*				pDC->SetPixel(r.left,r.top,GetMapDlg()->GetAxColor(64));
				pDC->SetPixel(r.left,r.bottom,GetMapDlg()->GetAxColor(64));
				pDC->SetPixel(r.right,r.top,GetMapDlg()->GetAxColor(64));
				pDC->SetPixel(r.right,r.top-1,GetMapDlg()->GetAxColor(64));
				pDC->SetPixel(r.right-1,r.top,GetMapDlg()->GetAxColor(64));
				pDC->SetPixel(r.right,r.bottom,GetMapDlg()->GetAxColor(64));
*/			}
		} else {											// DEFAULT BUTTON
			if(m_bNormal.m_hObject==NULL)
                ...
                if ((m_tracking)&&(m_bOver.m_hObject!=NULL)){
					...
					if ((itemState & ODS_FOCUS)&&(m_bFocus.m_hObject!=NULL)){
...
*/			}
		...
		if ((itemState & ODS_FOCUS)&&(m_FocusRectMargin>0)){
...
HRGN CxSkinButton::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
	...
	LPRECT	pRects=nullptr;								
	DWORD	cBlocks = 0;			// number of allocated blocks

	INT		i=0, j=0;					// current position in mask image
	...
	std::unique_ptr<BYTE[]> rData = std::make_unique<BYTE[]>(RDHDR + ++cBlocks * MAXBUF * sizeof(RECT));
	
	// allocate memory for region data
	RGNDATAHEADER* pRgnData = (RGNDATAHEADER*)rData.get();//  new BYTE[RDHDR + ++cBlocks * MAXBUF * sizeof(RECT)];
		...
		for (j = 0; j < bm.bmWidth; j++) {
			...
			if (wasfirst && ((ismask && (j == (bm.bmWidth - 1))) || (ismask ^ (j < bm.bmWidth)))) {
				...
				if (pRgnData->nCount >= cBlocks * MAXBUF) {
					std::unique_ptr<BYTE[]> rDataNew = std::make_unique<BYTE[]>(RDHDR + ++cBlocks * MAXBUF * sizeof(RECT));
					//				LPBYTE pRgnDataNew =  new BYTE[ RDHDR + ++cBlocks * MAXBUF * sizeof(RECT) ];
					//				memcpy( pRgnDataNew, pRgnData, RDHDR + (cBlocks - 1) * MAXBUF * sizeof(RECT) );
					memcpy(rDataNew.get(), pRgnData, RDHDR + (cBlocks - 1) * MAXBUF * sizeof(RECT));
					//delete pRgnData;
					pRgnData = nullptr;
//						pRgnData = (RGNDATAHEADER*)pRgnDataNew;
					pRgnData = (RGNDATAHEADER*)rDataNew.get();
				}
				wasfirst = false;
			}
