...
{	
	if(IsWin8())
	{
		...
		m_search.MoveWindow(x+248, y-16, 200, 20);				//종목검색
	...
	else
	{
		...
		m_search.SetWindowPos(nullptr, x+248, y-16, 200, 20, SWP_NOZORDER);				//종목검색
...
void CPage1::initTree()
{
	...
	m_tree.SetItemData(m_tree.InsertItem(_T("코스피200"), hRoot, TVI_LAST), xKOPSI200);
...
bool CPage1::SearchData(int kind, CString sName, bool bAddAll)
{
	...
	_listitem* listitem{};
	CPtrArray arSearch, arSame, arOther, arrExName, arFirst;
	CString name, code;
	int idx = 0;
	if (sName.IsEmpty())
	{
	...
	{	
		...
		for (int ii = 0; ii < m_listitem.GetSize(); ii++)
		{
			...
			const auto& [found, pos] = findFirst(sName, name.Trim());
			if (found && pos == 0)
				arFirst.Add(listitem);
			else if (found)
				arSearch.Add(listitem);	
			else if (0 == result)
			 	arSearch.Add(listitem);
			...
			else if (_mapExCode.find(code.GetString()) != _mapExCode.end())
			{
				...
				{
					ex.Trim();
					if (auto [bflag, xx] = findFirst(sName, ex); bflag)
					{
						arrExName.Add(listitem);	
						break;
					}
			 	...
			 	arOther.Add(listitem);
		};

		if (arFirst.GetSize() > 1)
		{
			std::vector<_listitem*> sortedFirst;
			sortedFirst.reserve(arFirst.GetSize());
			for (INT_PTR i = 0; i < arFirst.GetSize(); ++i)
			sortedFirst.push_back(static_cast<_listitem*>(arFirst.GetAt(i)));

			std::sort(sortedFirst.begin(), sortedFirst.end(),
			[](const _listitem* lhs, const _listitem* rhs)
			{
				return CString(lhs->name).CompareNoCase(rhs->name) < 0;
			});

			arFirst.RemoveAll();
			for (auto* item : sortedFirst)
				arFirst.Add(item);
		}

		arSearch.InsertAt(0, &arFirst);
		arSearch.Append(arSame);
		arSearch.Append(arrExName);
		arSearch.Append(arOther);
