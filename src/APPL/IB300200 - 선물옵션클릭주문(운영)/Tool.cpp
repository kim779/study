...
LRESULT CTool::SendTR(CString strCode, CString strData, int iKey, int iStat)
{
	...
	ZeroMemory(pcDataBuffer, sizeof(pcDataBuffer));
	...
	LRESULT lResult = m_pwndWizard->SendMessage(WM_USER, MAKEWPARAM(invokeTRx, strData.GetLength()), (LPARAM) pcDataBuffer);
...
{
	char* pcDataBuffer = new char [L_userTH + iLength];
	ZeroMemory(pcDataBuffer, sizeof(pcDataBuffer));
	struct	_userTH* puserTH;
	puserTH = (struct _userTH *)pcDataBuffer;
	...
	CopyMemory(&pcDataBuffer[L_userTH], pcData, iLength);

	LRESULT lResult = m_pwndWizard->SendMessage(WM_USER, MAKEWPARAM(invokeTRx, iLength), (LPARAM) pcDataBuffer);
...
{
	int iKey = (int)m_pwndWizard->SendMessage(WM_USER, MAKEWPARAM(attachMAP, NULL), (LPARAM)pWnd);
	return iKey;
}
...
CString CTool::GetEncPswd(CString strPass, CString strAcc)
{
	CString strBuffer;

	strBuffer.Format("%s\t%s", strPass, strAcc);
	strPass = (char*)m_pwndWizard->SendMessage(WM_USER, MAKEWPARAM(encPASSx, 0), (long)strBuffer.operator LPCTSTR());
	return strPass;
}
