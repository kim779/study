...
const int bmp_selL = 0, bmp_selM = 1, bmp_selR = 2, bmp_norL = 3, bmp_norM = 4, bmp_norR = 5, bmp_nor = 6;
...
void CfxTab::SetImage(HBITMAP hselectL, HBITMAP hSelectM, HBITMAP hSelectR, HBITMAP hL, HBITMAP hM, HBITMAP hR, HBITMAP hRBk)
{
...
void CfxTab::SetImage(CString imageDir, CString selectL, CString selectM, CString selectR, CString left, CString middle, CString right, CString rightBk)
{
	ClearImage();
	...
	path.Format("%s\\%s",  imageDir, rightBk);	m_bmpRBk = CreateBmp(path);

	SetImage(m_bmpSelL->operator HBITMAP(), m_bmpSelM->operator HBITMAP(), m_bmpSelR->operator HBITMAP(), 
		m_bmpL->operator HBITMAP(), m_bmpM->operator HBITMAP(), m_bmpR->operator HBITMAP(), m_bmpRBk->operator HBITMAP());	
}

void CfxTab::LoadTabImage(CString rootDir, bool bTopTab)
{
	CString path, prefix;
	CString selL, selM, selR, left, middle, right, rightBk;
	...
	rightBk.Format("b%s%s", prefix, IMG_FILE);

	SetImage(path, selL, selM, selR, left, middle, right, rightBk);
}
...
void CfxTab::ClearImage()
{
	...
	if (m_bmpRBk)	{ m_bmpRBk->DeleteObject(); delete m_bmpRBk; m_bmpRBk = NULL; };
}
...
void CfxTab::drawImage(CDC* pDC)
{
	...
	for (int ii = 0; ii < GetItemCount(); ii++)
	{
		...
		if (bSelect)
		{
			posX = drawBmp(pDC, &memDC, rtItem, bmp_selL, posX -1);
			posX = drawBmp(pDC, &memDC, rtItem, bmp_selM, posX, rtItem.right - getBmpWidth(bmp_selR));
			posX = drawBmp(pDC, &memDC, rtItem, bmp_selR, posX -1);
			posX--;
	...
	rt.right = 1000;
	posX = drawBmp(pDC, &memDC, rt, bmp_nor, posX, rt.right);
