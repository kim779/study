...
void x_StrInsertReplace( MCD_STR& str, int nLeft, int nReplace, const MCD_STR& strInsert )
{
	...
	//
	const int nStrLength = MCD_STRLENGTH(str);
	int nInsLength = MCD_STRLENGTH(strInsert);
	const int nNewLength = nInsLength + nStrLength - nReplace;
	const int nAllocLen = MCD_STRCAPACITY(str);
...
int x_GetEncodingCodePage( MCD_CSTR pszEncoding )
{
	...
	else if ( nEncLen < 100 )
	{
		...
		{
			const MCD_CHAR cEncChar = pszEncoding[nEncChar];
...
#if defined(MARKUP_WINCONV)
constexpr bool x_NoDefaultChar( int nCP )
{
...
int TextEncoding::PerformConversion( void* pTo, MCD_CSTR pszToEncoding/*=NULL*/ )
{
	...
	else
	{
		...
		const int nUTF16Len = MultiByteToWideChar(nFromCP,0,(const char*)m_pFrom,m_nFromLen,pwszUTF16,m_nFromLen);
		...
		else
		{
			...
			if ( nCP == 0 && nMultibyteCharsToTest )
			{
				...
				*/
				...
				const int nUTF16Len = PerformConversion( (void*)pUTF16Buffer );
				const int nOriginalByteLen = m_nFromLen;
				...
				while ( nTruncBeforeBytes < nMaxBytesToTry && ! bSuccess )
				{
					...
					const int nEndUTF16Len = PerformConversion( (void*)wsz16End );
...
{
	const short nWord = 1;
	const char cFirstByte = ((char*)&nWord)[0];
	if ( cFirstByte ) // LE
	{
...
struct ElemPos
{
    ...
    int nFlags = 0; // 16 bits flags, 16 bits level 65536 depth limit
	int iElemParent = 0;
	int iElemChild = 0; // first child
	int iElemNext = 0; // next sibling
	int iElemPrev = 0; // if this is first, iElemPrev points to last
};
...
struct ElemPosTree
{
	ElemPosTree() { Clear(); m_pSegs = NULL; };
	~ElemPosTree() { Release(); };
...
void ElemPosTree::GrowElemPosTree( int nNewSize )
{
	...
	if ( m_nSegs <= nNewSeg )
	{
		const int nNewSegments = 4 + nNewSeg * 2;
	...
	const int nSegSize = m_nSize - (nSeg << PA_SEGBITS);
	const int nNewSegSize = nNewSize - (nNewSeg << PA_SEGBITS);

	// Complete first segment
	const int nFullSegSize = 1 << PA_SEGBITS;
...
bool TokenPos::FindAttrib( MCD_PCSZ pAttrib, int n/*=0*/ )
{
	...
	while ( 1 )
	{
		...
		{
			// Is it an equal sign?
			const MCD_CHAR cChar = m_pDocText[m_nL];
			if ( cChar == '=' )
			{
				bAfterEqual = true;
				continue;
...
{
	const bool bSuccess = ( fread( pBuffer,m_nOpFileByteLen,1,m_fp) == 1 );
	...
	if ( bSuccess )
	{
		...
		const bool bSingleByteChar = m_nFileCharUnitSize == 1;
...
bool FilePos::FileReadText( MCD_STR& strDoc )
{
	...
	const MCD_INTFILEOFFSET nBytesRemaining = m_nFileByteLen - m_nFileByteOffset;
	...
	{
		const int nUTF16Len = m_nOpFileByteLen / 2;
#if defined(MARKUP_WCHAR) // WCHAR
		...
		if ( bSuccess && bCheckRaggedEnd )
			FileCheckRaggedEnd( (void*)pUTF16Buffer );
		TextEncoding textencoding( MCD_T("UTF-16"), (const void*)pUTF16Buffer, m_nOpFileTextLen );
		const int nMBLen = textencoding.PerformConversion( NULL, MCD_ENC );
		const int nBufferSizeForGrow = nMBLen + nMBLen/100; // extra 1%
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		else // load directly into string
		{
			...
			if ( bAssumeUnknownIsNative )
			{
				...
				int nNonASCII{};
				bool bErrorAtEnd{};
				const bool bIsUTF8 = CMarkup::DetectUTF8( pBuffer, m_nOpFileByteLen, &nNonASCII, &bErrorAtEnd ) || (bCheckRaggedEnd && bErrorAtEnd);
...
bool FilePos::FileWrite( void* pBuffer, const void* pConstBuffer /*=NULL*/ )
{
	...
	const bool bSuccess = ( fwrite( pConstBuffer, m_nOpFileByteLen, 1, m_fp ) == 1 );
...
bool FilePos::FileReadNextBuffer()
{
	...
	if ( m_nFileByteOffset < m_nFileByteLen )
	{
		...
		const int nDocLength = MCD_STRLENGTH( str );
		const int nRemove = m_nReadBufferStart;
		m_nReadBufferRemoved = nRemove;

		// Increase capacity if already at the beginning keeping more than half
		const int nKeepLength = nDocLength - nRemove;
		const int nEstimatedKeepBytes = m_nBlockSizeBasis * nKeepLength / nDocLength; // block size times fraction of doc kept
...
int TokenPos::ParseNode( NodePos& node )
{
	...
	while ( 1 )
	{
		...
		if ( ! cD )
		{
			...
			{
				const int nRemovedAlready = m_pReaderFilePos->m_nReadBufferRemoved;
				if ( m_pReaderFilePos->FileReadNextBuffer() ) // more text in file?
				{
					const int nNodeLength = m_nNext - node.nStart;
					const int nRemove = m_pReaderFilePos->m_nReadBufferRemoved - nRemovedAlready;
					if ( nRemove )
					{
						node.nStart -= nRemove;
						if ( nName )
					...
					const int nNewOffset = node.nStart + nNodeLength;
					const MCD_STR& str = *m_pReaderFilePos->m_pstrBuffer;
...
bool CMarkup::FindElem( MCD_CSTR szName )
{
	...
	if ( m_pElemPosTree->GetSize() )
	{
		...
		const int iPos = x_FindElem( m_iPosParent, m_iPos, szName );
		if ( iPos )
		{
			// Assign new position
...
bool CMarkup::FindChildElem( MCD_CSTR szName )
{
	...
	{
		// Assign new position
		const int iPos = ELEM(iPosChild).iElemParent;
			...
			if ( (nFlags&MNF_WITHREFS) && *pFound == '&' )
			{
						...
						|| c=='#' || c=='_' || c==':' || ((unsigned int)c)>0x7f )
				{
					while ( 1 )
					{
						...
						if ( c == ';' )
						{
							const int nEntityLen = (int)(pCheckEntity - pSource) + 1;
...
int CMarkup::UTF16To8( char* pszUTF8, const unsigned short* pwszUTF16, int nUTF8Count )
{
	...
	while ( *pwszUTF16 )
	{
		...
		{
			const int nUTF8LenSoFar = nUTF8Len;
...
int CMarkup::UTF8To16( unsigned short* pwszUTF16, const char* pszUTF8, int nUTF8Count )
{
	...
	int nUChar = 0, nUTF8Len = 0, nUTF16Len = 0;
	...
	while ( nTypeFound >= 0 )
	{
		...
		else if ( nTypeFound == MNT_ELEMENT )
		{
			...
			else if ( token.Match(MCD_T("meta")) )
			{
				...
				const int nAttribOffset = node.nStart + 1;
				...
				if ( token.FindAttrib(MCD_T("http-equiv")) && token.Match(MCD_T("Content-Type")) )
				{
					...
					{
						const int nContentEndOffset = token.m_nNext;
...
bool CMarkup::SavePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
	...
	if ( szPosName )
	{
		...
		SavedPos* pSavedPos = NULL;

		if (pSavedPos != NULL) return false;

		if (pMap->pTable[nSlot] != NULL) {
			pSavedPos = pMap->pTable[nSlot];
		}

		if (!pSavedPos) return false;
	...
	if ( szPosName )
	{
		...
		if ( pSavedPos )
		{
			...
			while ( pSavedPos[nOffset].nSavedPosFlags & SavedPos::SPM_USED )
			{
				if ( pSavedPos[nOffset].strName == (MCD_PCSZ)szPosName )
				{
					const int i = pSavedPos[nOffset].iPos;
...
bool CMarkup::RemoveElem()
{
	...
	if ( m_iPos && m_nNodeType == MNT_ELEMENT )
	{
		const int iPos = x_RemoveElem( m_iPos );
...
bool CMarkup::RemoveChildElem()
{
	...
	if ( m_iPosChild )
	{
		const int iPosChild = x_RemoveElem( m_iPosChild );
...
bool CMarkup::x_ParseDoc()
{
	...
	const int nDocLen = MCD_STRLENGTH(m_strDoc);
	m_iPosFree = 1;
	...
	{
		TokenPos token( m_strDoc, m_nDocFlags );
		const int iPos = x_ParseElem( 0, token );
...
int CMarkup::x_ParseElem( int iPosParent, TokenPos& token )
{
	...
	int iPosRoot = 0;
	int iPos = iPosParent;
	const int iVirtualParent = iPosParent;
	const int nRootDepth = ELEM(iPos).Level();
	int nMatchLevel = 0;
	int iPosMatch = 0;
	int iTag = 0;
	int nTypeFound = 0;
	...
	while ( 1 )
	{
		...
		if ( nMatchLevel || nTypeFound == -2 )
		{
			...
			while ( elemstack.iTop > nMatchLevel )
			{
				...
				const int nOffset2 = (nTypeFound==0)? token.m_nL-1: MCD_STRLENGTH(m_strDoc);
...
MCD_STR CMarkup::x_GetPath( int iPos ) const
{
	...
	while ( iPos )
	{
		...
		if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		{
			const TagPos& tag = m_pFilePos->m_elemstack.GetRefTagPosAt(iPos);
...
bool CMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
	...
	const int nEscapedValueLen = MCD_STRLENGTH( strEscapedValue );
	MCD_STR strInsert;
	...
	else
	{
		...
		const int nAttribNameLen = MCD_PSZLEN( pAttrib );
	...
	const int nAdjust = MCD_STRLENGTH(strInsert) - nReplace;
	if ( m_nDocFlags & MDF_WRITEFILE )
	{
		const int nNewDocLength = MCD_STRLENGTH(m_strDoc) + nAdjust;
		MCD_STRCLEAR( m_strResult );
		if ( nNodeStart && nNewDocLength > m_pFilePos->m_nBlockSizeBasis )
		{
			const int nDocCapacity = MCD_STRCAPACITY(m_strDoc);
...
bool CMarkup::x_SetData( int iPos, MCD_PCSZ szData, int nFlags )
{
	...
	int iPosBefore = 0;
	const int nReplace = x_InsertNew( iPos, iPosBefore, node );
	const int nAdjust = MCD_STRLENGTH(node.strMeta) - nReplace;
	x_Adjust( iPos, nAdjust );
	ELEM(iPos).nLength += nAdjust;
...
MCD_STR CMarkup::x_GetData( int iPos )
{
	...
	if ( iPos && ! ELEM(iPos).IsEmptyElement() )
	{
		...
		const int nStartContent = pElem->StartContent();
		if ( pElem->IsUnparsed() )
		{
		...
		else if ( ! pElem->iElemChild )
		{
			...
			const int nContentLen = pElem->ContentLen();
...
bool CMarkup::x_SetElemContent( MCD_PCSZ szContent )
{
	...
	// Unlink all children
	const int iPos = m_iPos;
	int iPosChild = ELEM(iPos).iElemChild;
	const bool bHadChild = (iPosChild != 0);
	while ( iPosChild )
	...
	const int iPosVirtual = x_GetFreePos();
...
int CMarkup::x_InsertNew( int iPosParent, int& iPosRel, NodePos& node )
{
	...
	const bool bEmptyParentTag = iPosParent && ELEM(iPosParent).IsEmptyElement();
	const bool bNoContentParentTags = iPosParent && ! ELEM(iPosParent).ContentLen();
	...
	if ( m_nDocFlags & MDF_WRITEFILE )
	{
		...
		const int nNewDocLength = MCD_STRLENGTH(m_strDoc) + MCD_STRLENGTH(node.strMeta) - nReplace;
...
bool CMarkup::x_AddElem( MCD_PCSZ pName, MCD_PCSZ pValue, int nFlags )
{
	...
	//
	const int nLenName = MCD_PSZLEN(pName);
	if ( ! pValue || ! pValue[0] )
	{
		// <NAME/> empty element
	...
	else
	{
		...
		const int nLenValue = MCD_STRLENGTH(strValue);
	...
	// Insert
	const int nReplace = x_InsertNew( iPosParent, iPosBefore, node );
	pElem->nStart = node.nStart;
	pElem->iElemChild = 0;
	if ( nFlags & MNF_WITHNOEND )
		pElem->nFlags = MNF_NONENDED;
...
bool CMarkup::x_AddSubDoc( MCD_PCSZ pSubDoc, int nFlags )
{
	...
	const int iPos = x_ParseElem( iPosVirtual, token );
	...
	int nExtractStart = 0;
	const int iPosLast = ELEM(iPos).iElemPrev;
	if ( bWellFormed )
	{
	...
	const int nAdjust = node.nStart - nExtractStart;
	if ( iPos && nAdjust )
	{
		x_Adjust( iPos, nAdjust );
	...
	while ( iPosChild )
	{
		const int iPosNext = ELEM(iPosChild).iElemNext;
...
int CMarkup::x_RemoveElem( int iPos )
{
	...
	TokenPos token( m_strDoc, m_nDocFlags );
	const int nAfterEnd = token.WhitespaceToTag( ELEM(iPos).StartAfter() );

	// Remove from document, adjust affected indexes, and unlink
	const int nLen = nAfterEnd - ELEM(iPos).nStart;
	x_DocChange( ELEM(iPos).nStart, nLen, MCD_STR() );
	x_Adjust( iPos, - nLen, true );
	const int iPosPrev = x_UnlinkElem( iPos );
	x_CheckSavedPos();
...
{
	const int iPosNext = ELEM(iPos).iElemNext;
			...
			for ( int nSlot = 0; nSlot < pMap->nMapSize; ++nSlot )
			{
				...
				if ( pSavedPos )
				{
					...
					while ( 1 )
					{
						...
						{
							const int iPos = pSavedPos[nOffset].iPos;
...
bool CMarkup::x_AddNode( int nNodeType, MCD_PCSZ pText, int nNodeFlags )
{
	...
	const int nReplace = x_InsertNew( m_iPosParent, iPosBefore, node );
...
void CMarkup::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
	...
	if ( nNodeType == MNT_LONE_END_TAG )
	{
		...
		const int nEndOfContent = token.m_nNext + ELEM(iPosParent).ContentLen();
