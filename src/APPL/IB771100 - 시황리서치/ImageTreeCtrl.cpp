...
//=============================================================================
{
	...
	if (pMsg->message == WM_CHAR)
	{
		if ((pMsg->wParam == VK_SPACE) && m_bCheckBoxes && !m_bReadOnly)
		{
			...
			const XHTMLTREEDATA *pXTCD = GetItemDataStruct(hItem);
	...
	*pResult = CDRF_DODEFAULT;
	...
	//=========================================================================
	{
		...
		if (m_bFirstTime)
		{
			...
			const CImageList *pImageList = GetImageList(TVSIL_NORMAL);
			if (!pImageList)	//+++1.5
			{
//				TRACE(_T("WARNING  no image list, setting m_bImages to FALSE\n"));
		...
		const _TREEITEM *hSelected = GetSelectedItem();
...
//=============================================================================
{
	...
	if (m_pToolTip)
	{
		...
		SendRegisteredMessage(WM_XHTMLTREE_INIT_TOOLTIP, 0, (LPARAM)m_pToolTip.get());
	}
}
...
//			TRACE(_T("mouse button is up >>>>>\n"));
			...
			const _TREEITEM * hItemSelected = GetSelectedItem();
	...
	if (hItem && lpszNote)
	{
		...
		if (pXTCD)
		{
			...
			if (pXTCD->pszNote) {
				...
				pXTCD->pszNote = nullptr;
				pXTCD->pszNote = std::make_unique<TCHAR []>(len + 4);
			}
			else {
				pXTCD->pszNote = std::make_unique<TCHAR []>(len + 4);
			}
...
//			pXTCD->pszNote = new TCHAR[len + 4];
			if (pXTCD->pszNote)
			{
				memset(pXTCD->pszNote.get(), 0, len+4);
				_tcsncpy(pXTCD->pszNote.get(), lpszNote, len+1);
...
//=============================================================================
{
//	std::unique_ptr < XHTMLTREEDATA > pXTCD = std::make_unique< XHTMLTREEDATA >();

	std::unique_ptr< XHTMLTREEDATA > pXTCD = std::make_unique< XHTMLTREEDATA>();
	//XHTMLTREEDATA* pXTCD = nullptr;
	//pXTCD = new XHTMLTREEDATA;

//	XHTMLTREEDATA *pXTCD = new XHTMLTREEDATA;
//	std::unique_ptr<XHTMLTREEDATA> pXTCD;
//	pXTCD = std::make_unique<XHTMLTREEDATA>();
...
//	TRACE(_T("inserting <%s>\n"), strText);
	...
	if (hItem)
	{
		XHTMLTREEDATA* adata = pXTCD.release();
		m_DataMap.SetAt(hItem, (XHTMLTREEDATA *&)adata);
...
//			_mbstowcsz(pTTTW->szText, strToolTip, sizeof(pTTTW->szText) / sizeof(TCHAR));
			_mbstowcsz(pTTTW->szText, strToolTip, sizeof(pTTTW->szText) / 2);
#else
		if (pNMHDR->code == TTN_NEEDTEXTA)
...
//	TRACE(_T("calling CTreeCtrl::Expand()\n"));
	...
	BOOL rc = FALSE;
	if (hItem != 0)
		rc= CTreeCtrl::Expand(hItem, nCode);
...
//=============================================================================
{
	const CEdit *pEdit = GetEditControl();
