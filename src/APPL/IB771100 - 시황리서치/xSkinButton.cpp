...
HRGN CxSkinButton::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
	...
	std::unique_ptr<BYTE[]> pData = std::make_unique<BYTE[]>(RDHDR + ++cBlocks * MAXBUF * sizeof(RECT));

	pRgnData = (RGNDATAHEADER *)pData.get();
	...
	for ( j = 0; j < bm.bmWidth; j++ ){
		...
		if (wasfirst && ((ismask && (j==(bm.bmWidth-1)))||(ismask ^ (j<bm.bmWidth)))){
			...
			if ( pRgnData->nCount >= cBlocks * MAXBUF ){
				...
				std::unique_ptr<BYTE[]> pRgnDataNew = std::make_unique<BYTE[]>(RDHDR + ++cBlocks * MAXBUF * sizeof(RECT));
//				LPBYTE pRgnDataNew = new BYTE[ RDHDR + ++cBlocks * MAXBUF * sizeof(RECT) ];
				memcpy( pRgnDataNew.get(), pRgnData, RDHDR + (cBlocks - 1) * MAXBUF * sizeof(RECT) );
				pData = nullptr;
//				pData = 
//				delete pRgnData;
				// escdream check 2022.07.13
				pRgnData = (RGNDATAHEADER*)pRgnDataNew.get();
			}
			wasfirst = false;
...
void CxSkinButton::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_Style){ //track mouse for radio & check buttons
		...
		if (::WindowFromPoint(p2) == m_hWnd) { // mouse is in button
...
void CxSkinButton::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if ((m_button_down) && (::GetCapture() == m_hWnd)) {
        ...
        const bool need_pressed = (::WindowFromPoint(p2) == m_hWnd);
