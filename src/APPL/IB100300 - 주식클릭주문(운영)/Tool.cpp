...
HBITMAP CTool::RequestHBitmap(CString stPath)
{
	CBitmap* pBitmap = RequestBitmap(stPath);
	if(pBitmap)
...
//2012.11.12 KSJ 히스토리에서 종목삭제
void CTool::DelHistory(CString strData)
{
	Variant(codeCCx, strData);
}
//KSJ
...
CString CTool::AddComma(CString strSrc)
{
	...
	CString strReturn;
	int jj = -1;
	for (int ii = strlen(strNatural) - 1; ii >= 0; ii--)
	{
		jj++;
		if (jj == 3)
		{
...
void CTool::WriteFile(CString strData)
{
	...
	GetLocalTime(&st);
	strTime.Format("[%p] [%d년 %d월 %d 일% d시 %d분 %d초]\n" 
