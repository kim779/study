...
CSpinCtrl::~CSpinCtrl()
{
	...
	if ( m_pLocalFont)
	{
		m_pLocalFont->DeleteObject();
		m_pLocalFont = nullptr;
	}
}
...
int CSpinCtrl::Initialize()
{
	if (m_pfont == NULL)
	{
		m_pLocalFont = std::make_unique<CFont>();
		m_pLocalFont->CreatePointFont(90, "굴림체");
		m_pfont = m_pLocalFont.get();
	}
...
{
	CSpinButtonCtrl::SetRange(0, 10);
// 	CSpinButtonCtrl::SetPos(5);	//2013.07.24 KSJ 5로 초기화 해주지 않음.
