...
//	m_patw[D_ORDER_KIND]->SetText(m_strOrderKind);
	...
	CFont* pfont = m_pTool->RequestFont("굴림체", 9, FALSE, NULL);
	m_pBtnOK = new CfxImgButton();
	m_pBtnOK->Create("\n확인", CRect(75, 257, 75 + 55, 257 + 18), this, IDOK);
	m_pBtnOK->SetWindowText("확인");
	m_pBtnOK->SetFont(pfont);
	m_pBtnOK->SetDrawFocus();

	m_pBtnCancel = new CfxImgButton();
	m_pBtnCancel->Create("\n취소", CRect(139, 257, 139 + 55, 257 + 18), this, IDCANCEL);
...
void CDialogConfirm::CreateTextWnd()
{
	...
	{
		strTitle = "매도주문";
		strPathTitle.Format("%s\\image\\매도확인_bg.bmp", m_pTool->GetHome());
		strPathBar.Format("%s\\image\\매도확인_bar.bmp", m_pTool->GetHome());
	...
	{
		strTitle = "매수주문";
		strPathTitle.Format("%s\\image\\매수확인_bg.bmp", m_pTool->GetHome());
		strPathBar.Format("%s\\image\\매수확인_bar.bmp", m_pTool->GetHome());
	...
	{
		strTitle = "정정주문";
		strPathTitle.Format("%s\\image\\정정확인_bg.bmp", m_pTool->GetHome());
		strPathBar.Format("%s\\image\\정정확인_bar.bmp", m_pTool->GetHome());
	...
	{
		strTitle = "취소주문";
		strPathTitle.Format("%s\\image\\취소확인_bg.bmp", m_pTool->GetHome());
		strPathBar.Format("%s\\image\\취소확인_bar.bmp", m_pTool->GetHome());
	...
	CFont* pfontTitle = m_pTool->RequestFont("굴림체", 11, FALSE, FW_BOLD);
	...
	CFont* pfont = m_pTool->RequestFont("굴림체", 9, FALSE, NULL);
	CFont* pfontOrder = m_pTool->RequestFont("굴림체", 9, FALSE, FW_BOLD);
...
void CDialogConfirm::SetData(char cOrderKind, CString strAccount, CString strName, CString strJM, int iOrderCount, CString strOrderPrice, char cTradeKind, CString strOne)
{
	...
	case 'S':
		...
		if (atoi(strOrderPrice) > 0)
			m_strOrderPrice.Format("%s 원", strOrderPrice);
		break;
