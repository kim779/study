...
//		((CButton *)GetDlgItem(PERCENT_RADIO2))->SetCheck(1);
	...
	((CButton *)GetDlgItem(HOLD_AMT_CHK))->SetCheck(m_config.hold_amt);	//2015.03.11 KSJ 종목변경시 수량 유지

	((CButton *)GetDlgItem(HOGACLR_CHK))->SetCheck(m_config.hogaclr);

	((CButton *)GetDlgItem(POPUP_REMAIN))->SetCheck(m_config.popupremain);   

	((CButton *)GetDlgItem(VI_CHK))->SetCheck(m_config.showVI);
...
void CConfigDlg::InitData()
{
	...
	m_config.hold_amt = GetDataInt("HOGAJUMUN", "hold_amt", 0, m_file);	//2015.03.11 KSJ 종목변경시 수량 유지
	m_config.hogaclr = GetDataInt("HOGAJUMUN", "hogaclr", 1, m_file);
	m_config.popupremain = GetDataInt("HOGAJUMUN", "remain",0,m_file); // 수량팝업 유지 여부
	m_config.showVI = GetDataInt("HOGAJUMUN", "showVI", 1, m_file);
}
...
void CConfigDlg::OnOK()
{
	...
	//2015.03.11 KSJ 종목변경시 수량 유지
	tmpS.Format("%d", m_config.hold_amt);
	SetData("HOGAJUMUN", "hold_amt", tmpS, m_file);

	tmpS.Format("%d", m_config.hogaclr);
	SetData("HOGAJUMUN", "hogaclr", tmpS, m_file);

	tmpS.Format("%d", m_config.popupremain);
	SetData("HOGAJUMUN", "remain", tmpS, m_file);

	tmpS.Format("%d", m_config.showVI);
	SetData("HOGAJUMUN", "showVI", tmpS, m_file);
...
//		m_config.percentRadio = 1;
	...
	m_config.hold_amt = ((CButton *)GetDlgItem(HOLD_AMT_CHK))->GetCheck();	//2015.03.11 KSJ 종목변경시 수량 유지

	m_config.hogaclr = ((CButton *)GetDlgItem(HOGACLR_CHK))->GetCheck();

	m_config.popupremain = ((CButton *)GetDlgItem(POPUP_REMAIN))->GetCheck();

	m_config.showVI = ((CButton *)GetDlgItem(VI_CHK))->GetCheck();
}
