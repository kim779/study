...
{ //  gives back the size of the area (do not forget to delete pLayer)
    ...
    // escdream check needed
	const int Length = 2+GetColumnCount()*2;

    //[[gsl::suppress(26409)]]
    //gsl::owner<int*> Layer = new int[Length];
    std::unique_ptr<int[]> Layer = std::make_unique<int[]>(Length);

    //auto Layer = std::make_unique<int[]>(Length);
	Layer[0]= LAYER_SIGNATURE;
	Layer[1]= GetColumnCount();
	memcpy(&Layer.get()[2], &m_arColOrder[0], GetColumnCount()*sizeof(int));
	memcpy(&Layer.get()[2+GetColumnCount()], &m_arColWidths[0], GetColumnCount()*sizeof(int));
	*pLayer = Layer.release();
	return Length;
}
...
//    COleDataSource* pSource = new COleDataSource();
    std::unique_ptr<COleDataSource> pSource = std::make_unique<COleDataSource>();
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    std::unique_ptr<char[]> szBuffer = std::make_unique<char[]>(::GlobalSize(hmem));

    if (!szBuffer)
        return FALSE;

    sf.Read(szBuffer.get(), (UINT)::GlobalSize(hmem));
    ::GlobalUnlock(hmem);
...
void CGridCtrl::OnBeginDrag()
{
    ...
    std::unique_ptr<COleDataSource> pSource;

    if (!m_bAllowDragAndDrop && m_CurCol==-1)
        return;

    if (m_CurCol >= 0)	pSource = std::make_unique< COleDataSource>(); // new COleDataSource();
    if (!pSource && m_bAllowDragAndDrop) pSource.reset( CopyTextFromGrid());
    if (pSource)
    {
                       ...
                       CPoint /* point */)
{
    ...
    if (m_CurCol>=0)
	{
		...
		{
			const int NewI = m_arColOrder[m_CurCol];
			m_arColOrder.erase(m_arColOrder.begin()+m_CurCol);
			m_arColOrder.insert(m_arColOrder.begin()+m_LastDragOverCell.col, NewI);
			m_CurCol=-1;
			Invalidate();
			return TRUE;
		}
