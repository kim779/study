...
BOOL AxGrid::creategrid(CRect rect, CWnd* pWnd, int rowheight, int time, UINT nID)
{
	...
	_font = getAxFont(_T("굴림체"), 90);
	_brSelected = getAxBrush(RGB(200, 200, 200));
	_brColumnSelected = getAxBrush(RGB(191, 191, 255));
	_brBack = getAxBrush(RGB(255, 255, 255));
...
void AxGrid::DrawSubItem(CDC* pDC, LPDRAWITEMSTRUCT lp, const int& nItem, const int& nSubItem)
{
	...
	const auto& ft    = hidden.find(_T("1021")); // 리얼종목코드
	if (ft == hidden.end())
		return;
	if (colmax < nSubItem)
		return;

	const CString rCode = ft->second;
	CRect rc;
	...
	if (stxt.CompareNoCase(_T("0")) != 0)
	{
		...
		// NXT 종목만 처리... 
		//********************************************************************************
		if (rCode.Find(_T("N.")) != -1)
		{	 
			CString sNxt;
			sNxt.Format("%s", rCode);
			sNxt.TrimLeft(_T("N.A"));
			if (rCode.CompareNoCase(_T("N.")) == 0 || sNxt.FindOneOf("FJQ") == 0)
...
{
	auto& item = _mDraw.emplace(row, vCol);		
	if (!item.second)
	{
...
CFont* AxGrid::getAxFont(CString fName, int point, int style)
{
	struct _fontR fontR;
	...
	if (_Wizard->GetSafeHwnd())
		return (CFont*)_Wizard->SendMessage(WM_USER, getFONT, (long)&fontR);
	return nullptr;
}

CBitmap* AxGrid::getAxBitmap(CString path)
{
	if (_Wizard->GetSafeHwnd())
		return (CBitmap*)_Wizard->SendMessage(WM_USER, getBITMAP, (long)path.operator LPCTSTR());
	return nullptr;
}

CBrush* AxGrid::getAxBrush(COLORREF color)
{
	if (_Wizard->GetSafeHwnd())
		return (CBrush*)_Wizard->SendMessage(WM_USER, MAKEWPARAM(getBRUSH, 0), (LPARAM)color);
	return nullptr;
}

CPen* AxGrid::getAxPen(COLORREF clr, int width, int style)
{
	struct _penR pen{};
	pen.clr = clr;
	pen.width = width;
	...
	if (_Wizard->GetSafeHwnd())
		return (CPen*)_Wizard->SendMessage(WM_USER, getPEN, (long)&pen);
	return nullptr;
}