...
CString CRemainFuture::TotalData()
{
	...
	if (m_code.GetLength()==8)
	{
		...
		if (mkgb>=85 && mkgb<99)	// 상품선물
		{
			...
			|| (sMkgb >= "6A" && sMkgb <= "6Z") || (sMkgb >= "7A" && sMkgb <= "7Z") || (sMkgb >= "8A" && sMkgb <= "8Z") || (sMkgb >= "9A" && sMkgb <= "9Z"))
		{
			t_rprice.Format("%+.f", atof(t_rprice));
			t_curr.Format("%+.f", atof(t_curr));

			//CString m_slog;
			//m_slog.Format("[XXXX00][SF][%s]<%d> m_code=[%s]", __FUNCTION__, __LINE__, m_code);
...
void CRemainFuture::getBase()
{
	...
	case JK_SFUTURE:
	case JK_SFSPREAD:
//		m_dBase = GetBaseValue(m_code);  //test  10으로 
		m_dBase = 10;
		break;
	case JK_FUTURE:
	case JK_SPREAD:
...
int CRemainFuture::getJKind(CString m_code)
{
	...
	case 'A':   //파생상품 코드개편
		{
			...
			{
				CString sMkgb = m_code.Mid(1, 2);
				if ((sMkgb > "10" && sMkgb <= "59") || (sMkgb >= "B0" && sMkgb <= "ZZ") || (sMkgb >= "0A" && sMkgb <= "0Z") || (sMkgb >= "1A" && sMkgb <= "1Z")
					|| (sMkgb >= "2A" && sMkgb <= "2Z") || (sMkgb >= "3A" && sMkgb <= "3Z") || (sMkgb >= "4A" && sMkgb <= "4Z") || (sMkgb >= "5A" && sMkgb <= "5Z")
					|| (sMkgb >= "6A" && sMkgb <= "6Z") || (sMkgb >= "7A" && sMkgb <= "7Z") || (sMkgb >= "8A" && sMkgb <= "8Z") || (sMkgb >= "9A" && sMkgb <= "9Z"))
				{
					return JK_SFUTURE;
	...
	case '2':	// call option
		...
		else if (m_code.GetAt(1) == '0' || m_code.GetAt(1) == 'A')  
			return JK_CALLOPT;
		else
			return JK_JCALLOPT;
		break;
