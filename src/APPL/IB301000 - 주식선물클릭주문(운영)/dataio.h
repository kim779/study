...
typedef enum { 
	...
	TK_OPTIONEX	=	253,		//2013.02.07 CX_OptionEx 추가
} TR_KEY;
...
typedef struct _PIBFUTP1_item {
	char accn[11]; /* ACCOUNT */
	char hash[44]; /* HASH */
} PIBFUTP1_item;

typedef struct _PIBFUTP1_mid{
	char usid[8];
	char nrec[4];
	PIBFUTP1_item list[1];
}PIBFUTP1_mid;

typedef struct _PIBFUTP1_mod{
	char ercd[4]; /* 처리코드 */
				  /* 0:성공 */
}PIBFUTP1_mod;
