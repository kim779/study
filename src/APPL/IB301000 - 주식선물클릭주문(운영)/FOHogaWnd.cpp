...
: CAxisExt(pParent), m_pAccounts(pAcntMap), m_pRsvdData(pRsvdData)
{
	...
	SetLogMode( (GetLogConfig("IB301000.HogaWnd")==0) ? FALSE : TRUE );
}
...
int CFOHogaWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pMarkMado->Init(bsCOLOR|bsTEXT|bsGENERAL, "시장가", "시장가", RGB(0xBC, 0xA0, 0xBC));
	m_pMarkMasu->Init(bsCOLOR|bsTEXT|bsGENERAL, "시장가", "시장가", RGB(0xBC, 0xA0, 0xBC));

	m_pMarkMasu->ShowWindow(SW_HIDE);
	m_pMarkMado->ShowWindow(SW_HIDE);
...
void CFOHogaWnd::CalcSize()
{
	...
	//m_pMarkMasu->MoveWindow(crc.left, pos_y, crc2.right-(crc.left + crc.Width()/2)+2, GRID_CELL_HEIGHT, FALSE);
...
LRESULT CFOHogaWnd::OnRefresh( WPARAM wParam, LPARAM lParam )
{
	if (wParam==WP_MICHEG)
	{
// 		OutputDebugString("[KSJ] HOGAWND REFRESH\n");
		PriceMichegMap tmp;
