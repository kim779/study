...
public:
	...
	BOOL Create(const RECT& rect, CWnd* parent, UINT nID,
		DWORD dwStyle = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE, DWORD dwExStyle = 0);

	///////////////////////////////////////////////////////////////////////////////////
	// Attributes
	///////////////////////////////////////////////////////////////////////////////////
public:
	BOOL GetNoScrollBar() const { return m_bNoScrollBar; }
	void SetNoScrollBar(BOOL bNoScroll) { m_bNoScrollBar = bNoScroll; EnableScrollBars(SB_BOTH, FALSE); }
	...
	void     SetGridBorderColor(COLORREF clr) { m_crBorder = clr; }
	COLORREF GetGridBorderColor() const { return m_crBorder; }
	...
	void     SetBkSelColor(COLORREF clr) { m_bkSELCOL = clr; }
...
public:
	...
	BOOL   SetItem(const GVITEM* pItem);
	BOOL   GetItem(GVITEM* pItem);
...
protected:
	...
	int  SetMouseMode(int nMode) { const int nOldMode = m_MouseMode; m_MouseMode = nMode; return nOldMode; }
...
protected:
	...
	// Editing
	virtual bool  OnEditCell(int nRow, int nCol, CPoint point, UINT nChar);
	...
	void	SetRowEmpty(int nRow, bool bPower = false);	//2016.03.30 KSJ 텍스트 초기화	bPower true이면 무조건 지움


// Attributes
protected:
	// add attributes by LKM
	COLORREF	m_crBorder;
	BOOL        m_bNoScrollBar;
	...
	BOOL		m_focusLine;
	BOOL		m_selectLine;
	COLORREF	m_clrKospi, m_clrKosdaq;
	int			m_margin;
	BOOL		m_bEnSelect;

	COLORREF m_clrBkMarkerColor, m_clrMarkerColor;
	// step color
	int		m_stepUnit;
	...
	CCellID     m_idCurrentCell, m_idTopLeftCell, m_idSelectCell;
	INT_PTR     m_nTimerID;
...
public:
	...
	void	HideCode();
	void	CreateInPlaceEdit(CRect& rect, DWORD dwStyle, UINT nID,
		int nRow, int nCol, LPCTSTR szText, int limit, int nChar);

	BOOL	IsCellAttribute(const CCellID& cell, UINT attr) const;
	...
	LPCTSTR	GetItemSymbol(int nRow, int nCol);

	void	SetHistoryStr(CString strHistory) { m_strHistory = strHistory; }
	void	SetHomeStr(CString strHome) { m_strHome = strHome; }
	void	SetFontEx(CFont* pFont) { m_pFont = pFont; }
	void	SetLogFont(LOGFONT logfont) { m_logfont = logfont; }
	void	SetFocusCellEdit(int nRow, int nCol, bool edit = true);
	...
	void	SetGubnColor(COLORREF color1 = 0, COLORREF color2 = 0) { m_clrKospi = color1; m_clrKosdaq = color2; }
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
	...
	if (GetVirtualMode())
	{
		...
		gvdi.item.state = 0;
		gvdi.item.format = pCell->GetFormat();
		gvdi.item.image = pCell->GetImage();
		gvdi.item.bkcol = pCell->GetBackClr();
		gvdi.item.fgcol = pCell->GetTextClr();
		gvdi.item.param = pCell->GetData();
		gvdi.item.attr = pCell->GetAttr();
		gvdi.item.symbol = pCell->GetSymbol();
		memcpy(&gvdi.item.font, pCell->GetFont(), sizeof(LOGFONT));
		gvdi.item.margin = pCell->GetMargin();
		gvdi.item.text.Empty();
		...
		cell.SetFormat(gvdi.item.format);
		cell.SetImage(gvdi.item.image);
		cell.SetBackClr(gvdi.item.bkcol);
		cell.SetTextClr(gvdi.item.fgcol);
		cell.SetData(gvdi.item.param);
		cell.SetFont(&(gvdi.item.font));
		cell.SetMargin(gvdi.item.margin);
		cell.SetText(gvdi.item.text);
		cell.SetAttr(gvdi.item.attr);
		cell.SetSymbol(gvdi.item.symbol);
...
//2016.03.30 KSJ 텍스트초기화
inline void CGridCtrl::SetRowEmpty(int nRow, bool bPower)
{
	if ((nRow < 0 || nRow >= m_nRows) && !bPower)
		return;
	...
	for (int jj = nCol; jj < m_nCols; jj++)
	{
		SetItemText(nRow, jj, _T(""));
		SetItemData(nRow, jj, 0);
	}
