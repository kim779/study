...
const COLORREF eqCOLOR = PALETTERGB(77, 77, 77);
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle, DWORD dwExStyle)
{
	...
	if (!CWnd::CreateEx(dwExStyle, GRIDCTRL_CLASSNAME, nullptr, dwStyle, rect, pParentWnd, nID))
		return FALSE;
...
void CGridCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	const CCellID nextCell = m_idCurrentCell;
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	GVITEM* pgvItem = &pgvDispInfo->item;

	// In case OnEndInPlaceEdit called as window is being destroyed
	if (!IsWindow(GetSafeHwnd()))
		return;

	CString	str = GetItemText(pgvItem->row, 0);
	if (str.IsEmpty())
		return;

	//2016.03.03 KSJ 책갈피 사라지는 현상
	CString strVal = pgvItem->text;
	//int	nval = atoi(pgvItem->text);
	if (strVal.IsEmpty())
	{
		CString name;
		CString isCode = GetItemText(pgvDispInfo->item.row, colCODE);
		if (IsCellAttribute(CCellID(pgvDispInfo->item.row, colNAME), GVAT_MARKER) == FALSE && isCode.IsEmpty() == FALSE)
		{
			CWnd* pOwner = GetOwner();

			name = ((CGridWnd*)(pOwner))->GetCodeName(isCode);
			OnEndEditCell(pgvItem->row, pgvItem->col, name);
		}
		else
		{
			OnEndEditCell(pgvItem->row, pgvItem->col, "");
		}
	}
	else
		OnEndEditCell(pgvItem->row, pgvItem->col, pgvItem->text);

	InvalidateCellRect(CCellID(pgvItem->row, pgvItem->col));

	switch (pgvItem->param)		// for kiwoom
	{
	case VK_TAB:
		//		TRACE("MessageToParent1 row : %d", gvitem->row);
		MessageToParent(pgvItem->row, pgvItem->col, GVNM_ENDTABKEY);
		break;
	case VK_RETURN:
		//		TRACE("MessageToParent2 row : %d", gvitem->row);
		MessageToParent(pgvItem->row, pgvItem->col, GVNM_ENDRETURN);
		break;
	default:
		//		TRACE("MessageToParent3 row : %d", gvitem->row);
		MessageToParent(pgvItem->row, pgvItem->col, GVNM_ENDEDIT);
		break;
	}

	switch (pgvItem->param)
	{
	case VK_TAB:
	case VK_DOWN:
	case VK_UP:
	...
	case VK_HOME:
	case VK_END:
		OnKeyDown((UINT)pgvItem->param, 0, 0);
		OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, CPoint(-1, -1), (UINT)pgvItem->param);
		break;
	}
...
BOOL CGridCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	case GVNM_ENDEDIT:
	{
		...
		if (IsCellAttribute(CCellID(nmuser->item.row, colNAME), GVAT_MARKER))
		{
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	for (row = 0; row < m_nFixedRows; row++)
	{
		...
		{
			if (IsCellAttribute(CCellID(row, col), GVAT_HIDDEN)) continue;
...

	{
		...
		for (row = minVisibleRow; row <= maxVisibleRow; row++)
		{
			...
			if (!IsCellAttribute(CCellID(row, colNAME), GVAT_MARKER))
				continue;
	...
	for (POSITION pos = m_mapMerge.GetStartPosition(); pos != nullptr; )
	{
		...
		pDC->FillSolidRect(fixMerge.rect, fixMerge.bkcol == CLR_DEFAULT ? m_crFixedBkColour : fixMerge.bkcol);
...
BOOL CGridCtrl::DrawFixedCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	if (nRow)
	{
		// 		OutputDebugString("DrawFixedCell::DrawCell\n");
	...
	bEraseBk = TRUE;
	if (gvitem.state & GVIS_DROPHILITED && !pDC->IsPrinting())
	{
		rect.right++;
		rect.bottom++;
	...
	else if (gvitem.state & GVIS_FOCUSED && !pDC->IsPrinting())
	{
		rect.right++; rect.bottom++;	// FillRect doesn't draw RHS or bottom
		if (bEraseBk)
		{
			const COLORREF color = (gvitem.bkcol == CLR_DEFAULT) ? m_crFixedBkColour : gvitem.bkcol;
			CBrush brush(color);
	...
	else if (m_selectLine && gvitem.state & GVIS_SELECTED && !pDC->IsPrinting() && nRow >= m_nFixedRows)
	{
		rect.right++;
	...
	pDC->SetTextColor((gvitem.fgcol == CLR_DEFAULT) ? m_crFixedTextColour : gvitem.fgcol);

	LOGFONT* logfont = GetItemFont(nRow, nCol);
	const CCellID focusCell = GetFocusCell();
	...
	// updateX_20051018
	if ((++rect.top < rect.bottom) && !(nCol == colCODE && 0 == m_arColWidths[nCol]) && (m_arColWidths[nCol] > 0))
	{
...
{
	if (nCol == colCODE && 0 == m_arColWidths[nCol]) return TRUE;
	...
	const BOOL	bExpect = str == "1" ? TRUE : FALSE;
	...
	if (gvitem.state & GVIS_FOCUSED && !pDC->IsPrinting())
	{
		...
		if (bEraseBk)
		{
			// 			if ((nCol == colCURR || nCol == m_nRateCol || nCol == m_nVolCol) && (bExpect || atoi(str)))
			// 			{
			// 				CBrush brush(bkColExpect);
			// 				pDC->FillRect(rect, &brush);
			// 			}
			// 			else if((gvitem.attr & GVAT_CONTRAST) == GVAT_CONTRAST && (bExpect || atoi(str)))
			// 			{
			// 				CBrush brush(bkColExpect);
			// 				pDC->FillRect(rect, &brush);
			// 			}
	...
	else if (m_selectLine && gvitem.state & GVIS_SELECTED && !pDC->IsPrinting())
	{
		rect.right++;
		rect.bottom++;	// FillRect doesn't draw RHS or bottom
		pDC->FillSolidRect(rect, m_bkSELCOL);
	...
	if ((gvitem.attr & GVAT_CONTRAST) == GVAT_CONTRAST && !string.IsEmpty())
	{
		...
		case '1':
			...
			if (rtmCol == -1 && m_selectLine && (gvitem.state & GVIS_SELECTED) && !(gvitem.state & GVIS_FOCUSED))
			{
		...
		case '4':
			...
			rect.InflateRect(0, 0); pDC->FillSolidRect(rect, gvitem.bkcol); rect.DeflateRect(m_margin, 0);
			break;
		case '-':
		case '5':
		...
		if (false && rtmCol == -1 && m_selectLine && (gvitem.state & GVIS_SELECTED) && !(gvitem.state & GVIS_FOCUSED))
		{
			clr = m_fgSELCOL;
	...
	if (rect.top < rect.bottom)
	{
		...
		else
		{
			...
			if (nCol == colCURR && (bExpect || atoi(str)) && strCode.GetLength() > 0/*&& strCode.GetLength() == 6*/)	//2011.12.22 KSJ 코드길이가 6일때만 예상가 그리도록 2013.08.05 KSJ 선옵, 지수, 주식선물일때도 예상가 표시하도록 요청받음.
			{
				...
				pDC->FillRect(rcOrg, &brush);
				brush.DeleteObject();

				DrawText(pDC->m_hDC, string, -1, rect, gvitem.format);

				//				OutputDebugString("[KSJ] EXPECT DRAW\n");
				// 					CString ss;
				// 					ss.Format("[NEXT]bExpect[%s], str[%s], strCode[%s]", bExpect ? "TRUE" :"FALSE", str, strCode);
				// 					OutputDebugString(ss);

				// 					CRect rcMRatio = rect;
				// 					rcMRatio.left -= 1;
				// 					rcMRatio.right = rcMRatio.left + 12;
				// 					rcMRatio.top = rcMRatio.top +2;
				// 					rcMRatio.bottom-=5;
				// // 					
				// // 					rect.left += 10;
				// // 					
				// 					DrawExpect(pDC, rcMRatio);
...
void CGridCtrl::drawContrast(CDC* pDC, CRect cellRc, CString sText, COLORREF clr)
{
	CRect	pRc = cellRc;
...
void CGridCtrl::DrawBong(CDC* pDC, CRect rect, int nRow, int nCol)
{
	// 	OutputDebugString("[KSJ] DrawBong");

	CString	str = GetItemText(nRow, colEXPECT);

	// 	CString strTemp;
	// 	strTemp.Format("[KSJ] str[%s], nRow[%d]", str, nRow);
	// 	OutputDebugString(strTemp);

	if (atoi(str))
		return;

	// 	OutputDebugString("[KSJ] DrawBong1");
	...
	// 	OutputDebugString("[KSJ] DrawBong2");
	...
	{
		// 		OutputDebugString("[KSJ] DrawBong3");
	...
	// 	OutputDebugString("[KSJ] DrawBong4");
	...
	// 	OutputDebugString("[KSJ] DrawBong5");
	...
	// 	OutputDebugString("[KSJ] DrawBong6");
...
COLORREF CGridCtrl::DisplayAttribute(int attr, CString& data)
{
	if (data.IsEmpty())
	{
...
CString CGridCtrl::CommaModify(CString string)
{
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = nullptr */)
{
	...
	const CCellID tlCell = GetTopleftNonFixedCell();
...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
	...
	if (IsValid(idPrev))
	{
		MessageToParent(idPrev.row, idPrev.col, GVNM_SELCHANGING);
	...
	if (IsValid(m_idCurrentCell))
	{
		SetItemState(m_idCurrentCell.row, m_idCurrentCell.col,
			GetItemState(m_idCurrentCell.row, m_idCurrentCell.col) | (m_focusLine ? GVIS_FOCUSED : 0));
		...
		if (m_idCurrentCell.col >= 16)
			MessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVNM_SELCHANGED);
	...
	BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	for (pos = m_mapMerge.GetStartPosition(); pos != nullptr; )
	{
		m_mapMerge.GetNextAssoc(pos, key, (_fixMerge&)fixMerge);
...
void CGridCtrl::SetSelectRow(int row)
{
	const CCellID cell(row, 0);

	ResetSelectedRange();
	//	EnsureVisible(cell);	// (20070806) 장희 : 자동 Scroll(클릭시 왼쪽 첫 번째 셀 Position 이동) 작동 안함

	m_idSelectCell = CCellID(row, 0);
	SelectRows(cell);
	//SetFocusCell(max(m_idSelect.row, m_nFixedRows), max(m_idSelect.col, colSIG + 1));
	SetFocusCell(max(m_idSelectCell.row, m_nFixedRows), 16);
}


void CGridCtrl::SetFocusCellEdit(int nRow, int nCol, bool edit)
{
	...
	m_idSelectCell = CCellID(nRow, 0);
	SelectRows(cell);

	SetFocusCell(cell);

	if (edit) OnEditCell(nRow, nCol, CPoint(0, 0), VK_END);
}
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
	...
	// Cache data
	COleDataSource* pSource = std::make_unique<COleDataSource>().release();
	...
	BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	// to hold this.
	std::string szBuffer;
	szBuffer.resize(::GlobalSize(hmem)); // FIX: Use LPSTR char here
	sf.Read(szBuffer.data(), (UINT)::GlobalSize(hmem));
	::GlobalUnlock(hmem);

	// Now store in generic TCHAR form so we no longer have to deal with
	// ANSI/UNICODE problems
	CString strText(szBuffer.data());
	//	delete szBuffer;
...
void CGridCtrl::OnBeginDrag()
{
	...
	if (m_CurCol >= 0)
		pSource = std::make_unique<COleDataSource>().release();
...
DROPEFFECT CGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
	CPoint point)
{
	//2016.04.08 KSJ 드래그 드랍 구현되어 있지 않아 리턴처리
	return DROPEFFECT_NONE;
...
LRESULT CGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
	...
	m_cellDefault.SetFont(&m_logfont);
	...
	TRY
	{
	    ...
	    else
	    {
		    ...
		    if (addedRows > 0)
		    {
				...
				m_RowData[row] = std::make_unique<GRID_ROW>().release();
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
	...
	TRY
	{
		...
		if (nRow < 0)
		{
			...
			if (!GetVirtualMode())
				m_RowData.Add(std::make_unique<GRID_ROW>().release());
			else
		...
		else
		{
			...
			if (!GetVirtualMode())
				m_RowData.InsertAt(nRow, std::make_unique<GRID_ROW>().release());
			else
				ResetVirtualOrder();
		}
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
	...
	if (nRow > 0 && nCol >= 0 && nCol < m_nCols)
	{
		...
		pCell->SetAttr(GetItemAttr(refRow, nCol));
		pCell->SetFormat(GetItemFormat(refRow, nCol));
		pCell->SetTextClr(GetItemFgColour(refRow, nCol));
		pCell->SetBackClr(GetItemBkColour(refRow, nCol));
		pCell->SetSymbol(GetItemSymbol(refRow, nCol));
		//pCell->misc   = GetItemMisc(refRow, nCol);
	}
	else if (nRow > 0 && nCol >= 0 && nCol == m_nCols)
	{
		refCol = nCol - 1;
		pCell->SetTextClr(GetItemFgColour(nRow, refCol));
		pCell->SetBackClr(GetItemBkColour(nRow, refCol));
	}
...
void CGridCtrl::UpdateRowSelectedRange(int row, bool insert)
{
	DWORD key{}; CCellID idCell;

	m_idSelectCell.row = -1; m_idSelectCell.col = -1;

	m_PrevSelectedCellMap.RemoveAll();

	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != nullptr; )
	{
		m_SelectedCellMap.GetNextAssoc(pos, key, (CCellID&)idCell);
		m_PrevSelectedCellMap.SetAt(key, idCell);
	}

	m_SelectedCellMap.RemoveAll();
	for (POSITION pos = m_PrevSelectedCellMap.GetStartPosition(); pos != nullptr; )
	{
		m_PrevSelectedCellMap.GetNextAssoc(pos, key, (CCellID&)idCell);
		if (idCell.row < row)
		{
			m_SelectedCellMap.SetAt(key, idCell);
			continue;
		}
...
void CGridCtrl::UpdateColSelectedRange(int col, bool insert)
{
	DWORD key{}; CCellID idCell;
...
BOOL CGridCtrl::SetItem(const GVITEM* pItem)
{
	...
	if (pItem->mask & GVMK_TEXT)
		pCell->SetText(pItem->text);
	if (pItem->mask & GVMK_PARAM)
		pCell->SetData(pItem->param);
	if (pItem->mask & GVMK_IMAGE)
		pCell->SetImage(pItem->image);
	if (pItem->mask & GVMK_STATE)
		SetItemState(pItem->row, pItem->col, pItem->state);
	if (pItem->mask & GVMK_FORMAT)
		pCell->SetFormat(pItem->format);
	if (pItem->mask & GVMK_BKCOLOR)
		pCell->SetBackClr(pItem->bkcol);
	if (pItem->mask & GVMK_FGCOLOR)
		pCell->SetTextClr(pItem->fgcol);
	if (pItem->mask & GVMK_FONT)
		pCell->SetFont(&(pItem->font));
	if (pItem->mask & GVMK_MARGIN)
		pCell->SetMargin(pItem->margin);
	if (pItem->mask & GVMK_SYMBOL)	//KSJ
		pCell->SetSymbol(pItem->symbol);
	if (pItem->mask & GVMK_ATTR)	//KSJ
		pCell->SetAttr(pItem->attr);

	// 	if (pCell->GetAttr() & GVAT_EDIT) m_focusLine = TRUE;
	if (pCell->GetAttr() & GVAT_HIDDEN) SetColumnWidth(pItem->col, 0);

	return TRUE;
}

BOOL CGridCtrl::GetItem(GVITEM* pItem)
{
	if (!pItem)
		return FALSE;
	CGridCellBase* pCell = GetCell(pItem->row, pItem->col);
	...
	if (pItem->mask & GVMK_TEXT)
		pItem->text = GetItemText(pItem->row, pItem->col);
	if (pItem->mask & GVMK_PARAM)
		pItem->param = pCell->GetData();;
	if (pItem->mask & GVMK_IMAGE)
		pItem->image = pCell->GetImage();
	if (pItem->mask & GVMK_STATE)
		pItem->state = pCell->GetState();
	if (pItem->mask & GVMK_FORMAT)
		pItem->format = pCell->GetFormat();
	if (pItem->mask & GVMK_BKCOLOR)
		pItem->bkcol = pCell->GetBackClr();
	if (pItem->mask & GVMK_FGCOLOR)
		pItem->fgcol = pCell->GetTextClr();
	if (pItem->mask & GVMK_FONT)
		memcpy(&(pItem->font), pCell->GetFont(), sizeof(LOGFONT));
	if (pItem->mask & GVMK_MARGIN)
		pItem->margin = pCell->GetMargin();
	if (pItem->mask & GVMK_SYMBOL)
		pItem->symbol = pCell->GetSymbol();
	if (pItem->mask & GVMK_ATTR)
		pItem->attr = pCell->GetAttr();
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
	...
	CString strPreText = pCell->GetText();
...
BOOL CGridCtrl::SetItemFgColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
	...
	if (nRow < GetFixedRowCount() && pCell->GetAttr() & GVAT_HDRMERGE)
	{
		_fixMerge fixMerge;
		if (m_mapMerge.Lookup(nCol, fixMerge))
		{
	...
	if (m_bLMouseButtonDown)
	{
		...
		case MOUSE_SELECT_CELLS:
		{
			...
			if (idCurrentCell != GetFocusCell())
			{
				...
				if ((m_focusLine && idCurrentCell.row >= m_nFixedRows &&
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    */
	...
	if (m_LeftClickDownCell == m_idCurrentCell)
	{
		if (IsCellAttribute(m_LeftClickDownCell, GVAT_EDIT))
		{
			m_MouseMode = MOUSE_PREPARE_EDIT;
			MessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVNM_LMOUSEDOWN);
		}
		return;
	}
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else if (m_MouseMode == MOUSE_SIZING_COL)
	{
		...
		if (m_LeftClickDownPoint != point && (point.x != 0 || point.y != 0)) // 0 pt fix by email1@bierling.net
		{
			...
			MessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVNM_CHGCOLSIZE);
		}
	}
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	else
	{
		...
		MessageToParent(FocusCell.row, FocusCell.col, GVNM_RMOUSEDOWN);
	}
}
#endif
...
CImageList* CGridCtrl::CreateDragImage(CPoint* pHotSpot)
{
	...
	*pHotSpot = rect.BottomRight();
	...
	CImageList* pList = std::make_unique<CImageList>().release();
...
// virtual
bool CGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
#ifndef GRIDCONTROL_NO_TITLETIPS
	m_TitleTip.Hide();  // hide any titletips
#endif
	...
	if ((nCol == colNAME) && !IsCellAttribute(cell, GVAT_MARKER))
	{
		...
		bool bValid = false, bCellAttribute = false, bCellEditable = false;
		bValid = !IsValid(cell);
		bCellAttribute = !IsCellAttribute(cell, GVAT_EDIT);
		bCellEditable = !IsCellEditable(nRow, nCol);

		//		if (!IsValid(cell) || !IsCellAttribute(cell, GVAT_EDIT))
		if (bValid || bCellAttribute)
		{
...
//2014.01.21 추가된것.
LRESULT CGridCtrl::MessageToParent(int nRow, int nCol, int nMessage)
{
	if (!IsWindow(m_hWnd))
		return 0;
	...
	CWnd* pOwner = GetOwner();
	if (pOwner && IsWindow(pOwner->m_hWnd))
		return SendMessage(WM_NOTIFY, nmgv.hdr.idFrom, (LPARAM)&nmgv);

	return 0;
}

void CGridCtrl::CreateCode(CRect rect, CString szText)
{
	szText.TrimLeft(), szText.TrimRight();
	if (m_history == nullptr)
	{
		CString	tabPath; tabPath.Format("%s/%s", m_strHome, "TAB");
	...
	m_history->SetHistory(m_strHistory);
	m_history->SetEditData(szText);
	m_history->ShowWindow(SW_SHOW);
	m_history->SetFocus();
}

void CGridCtrl::OnHistorySelEndOk()
{
	CString	code;
...
void CGridCtrl::HideCode()
{
	if (m_history)
		m_history->ShowWindow(SW_HIDE);
}

void CGridCtrl::CreateInPlaceEdit(CRect& rect, DWORD dwStyle, UINT nID,
	int nRow, int nCol, LPCTSTR szText, int limit, int nChar)
{
	LOGFONT* logfont = GetItemFont(nRow, nCol);
	CInPlaceEdit* pEdit = std::make_unique<CInPlaceEdit>(this, rect, dwStyle, nID, nRow, nCol, szText, limit, nChar, logfont).release();
	// MODIFY PSH 20070914
...
BOOL CGridCtrl::IsCellAttribute(const CCellID& cell, UINT attr) const
{
	CCellID hidden = cell;
	...
	UINT nAttr = 0;
	nAttr = GetItemAttr(hidden.row, hidden.col);

	BOOL bAttr = FALSE;
	bAttr = ((GetItemAttr(hidden.row, hidden.col) & attr) == attr);

	return ((GetItemAttr(hidden.row, hidden.col) & attr) == attr);
}

UINT CGridCtrl::GetItemAttr(int nRow, int nCol) const
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return 0;

	return pCell->GetAttr();
}

BOOL CGridCtrl::SetItemAttr(int nRow, int nCol, UINT attr)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return 0;
...
BOOL CGridCtrl::SetItemParam(int nRow, int nCol, LPARAM param)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return FALSE;

	pCell->SetParam(param);
...
LPARAM CGridCtrl::GetItemParam(int nRow, int nCol)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return (LPARAM)-1;

	return pCell->GetParam();
}

BOOL CGridCtrl::SetItemSymbol(int nRow, int nCol, LPCTSTR str)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return FALSE;
...
LPCTSTR CGridCtrl::GetItemSymbol(int nRow, int nCol)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell) return "";

	return pCell->GetSymbol();
}
