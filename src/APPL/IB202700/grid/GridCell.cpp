...
void CGridCell::Reset()
{
 ...
 //   CGridDefaultCell* pDefaultCell = (CGridDefaultCell*)GetDefaultCell();
   // if (pDefaultCell)
    //    CopyMemory(&m_lfFont, pDefaultCell->GetFont(), sizeof(LOGFONT));
...
{
        CopyMemory(&m_lfFont, plf, sizeof(LOGFONT));
}

LOGFONT* CGridCell::GetFont() const
{
        return(LOGFONT*)&m_lfFont;
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		CGridCtrl* pGrid = GetGrid();
                m_pEditWnd = std::make_unique<CInPlaceEdit>(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), 20, nChar).release();
    }
    return TRUE;
}
...
#else
    m_nFormat = DT_RIGHT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX;	//2014.01.17 KSJ 기본 오른쪽 정렬
#endif
