...
public:
	CfxEditEx(CFont *pFont=nullptr) : CfxEdit(pFont) {}
protected:
...
public:
	CfxStaticEx(CFont *pFont=nullptr) : CfxStatic(pFont) {}
protected:
	virtual LRESULT WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
	{
		if (message==WM_NCPAINT)
...
public:
	...
	CFont*	 GetAxFont(CString fName = _T("굴림체"), int point = 9, bool bItalic = false, bool bBold = false);
	...
	BOOL     SendTR(LPCSTR name, std::byte type, LPCSTR data, int dlen, int key);
	...
	CString	 GetCodeName(LPCSTR code);
