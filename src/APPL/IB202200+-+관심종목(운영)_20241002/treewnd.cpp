...
void CTreeWnd::initTree()
{
	...
	SetItemData(InsertItem(_T("KOSPI200"), 0, 1, hRoot, TVI_LAST), MAKE_TREEID(xKOPSI200));
	SetItemData(InsertItem(_T("KRX100"), 0, 1, hRoot, TVI_LAST), MAKE_TREEID(xKRX100));
	SetItemData(InsertItem(_T("거래소업종"), 0, 1, hRoot, TVI_LAST), MAKE_TREEID(xKOSPIUP));
...
m_slog.Format("\r\n[interest][IB202200][%s]", __FUNCTION__, data.Left(22));
OutputDebugString(m_slog);
	CString strtmp;
	CWnd* pWnd = (CWnd*)m_pMainWnd->SendMessage(WM_MANAGE, MAKEWPARAM(MK_GETWND, MO_GROUP));
	pWnd->SendMessage(WM_MANAGE, MK_CLEARSECONDSEND, 0);
...
//AxStd::_Msg("종목개수 [%s]", sCount);
	...
	{
		m_slog.Format("\r\n[interest][IB202200[[%s] [%s]파일이 없음 ", __FUNCTION__, filePath);
		OutputDebugString(m_slog);
	}
	else
	{
		m_slog.Format("\r\n[interest][IB202200[[%s] [%s]파일이 있음 ", __FUNCTION__, filePath);
		OutputDebugString(m_slog);
	}
...
m_slog.Format("\r\n[interest][IB202200] bookCode=[%s] code=[%s] ", bookCode, code);
OutputDebugString(m_slog);
...
m_slog.Format("\r\n[interest][IB202200] ii=[%d] g=[%s] c=[%s] n=[%s] bookmark=[%s] p=[%s] a=[%s] bookrgb=[%.2s]", ii, CString(item.gubn, 1).Trim(), code, name, bookmark, price, amount, bookrgb);
OutputDebugString(m_slog);
