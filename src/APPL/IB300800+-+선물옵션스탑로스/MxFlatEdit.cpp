...
void CMxFlatEdit::OnSize(UINT nType, int cx, int cy) 
{
...
void CMxFlatEdit::ChangeFont(CFont* pFont, int ifontpoint)
{
	
	CRect rc, rc1;
	GetClientRect(&rc);

	rc1 = CRect(1, 1, rc.right-2, rc.bottom -2);
	m_pEidt->SetFont(pFont); 
	m_pEidt->MoveWindow(rc1);
	...
	int igap = (rc.Height() - ih) / 2; 
	rc1.DeflateRect(0,igap,0,igap); 
//	m_pEidt->SetRectNP (rc1);
