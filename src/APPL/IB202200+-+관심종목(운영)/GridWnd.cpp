...
void CGridWnd::setDelayTime(int delay){
	if (m_grid)
		m_grid->setDelayTime(delay);
}
...
void CGridWnd::setBookmark(int row)
{
	...
	for (int ii = 0; ii < m_colC; ii++)
	{
		...
		if (ii == colCODE)
		{
			...
			{
				result = _Wizard->SendMessage(WM_USER, MAKEWPARAM(codeTYPE, 0), (long)(LPCTSTR)string);
...
//OutputDebugString(m_slog);
	...
	for_each(spanInter.begin(), spanInter.end(), [&](auto& jinfo){
		...
		if (!pInters->code.IsEmpty())
		{
			...
			if (pInters->gubn == ROW_BOOKMARK)
			{
				if (pInters->bookrgb.GetLength() == 0 || pInters->bookrgb.IsEmpty())
				{
...
//			m_slog.Format("\r\n[interest][IB202200] ii=[%d] gubn=[%c] code=[%s] name=[%s] rgb=[%.2s]",
//				ii, jinfo.gubn[0], CString(jinfo.code).Trim(), stmp.Trim(), jinfo.bookrgb);
//			OutputDebugString(m_slog);
		}
	});
...
//m_slog.Format("\r\n[interest][IB202200] ------------------------------");
//OutputDebugString(m_slog);
//m_slog.Format("\r\n[interest][IB202200][%s] gno =[%d] strBook=[%s]", __FUNCTION__, gno , strBook);
//OutputDebugString(m_slog);
...
//	else
//	{
//		m_slog.Format("\r\n[interest][IB202200][%s]  북마크파일[%s] 저장 ", __FUNCTION__, strBook);
//		OutputDebugString(m_slog);
//	}
...
void CGridWnd::SeverMemoCheck(CArray<bool, bool>& parr)
{
	int ii = 0;
	for_each(_vInters.begin(), _vInters.end(), [&](auto& pinters) {
		if (pinters->bMemo == "Y")
		{
			parr.SetAt(ii, true);
//			m_slog.Format("[IB202200][%s][memo2022] ii=[%d] name=[%s] bMemo=[%s]", __FUNCTION__, ii, pinters->name, pinters->bMemo);
//			OutputDebugString(m_slog);
		}
		ii++;
	});
...
void CGridWnd::SeverMemoCheck(int xrow, CArray<bool, bool>& parr)
{
	auto& pinters = _vInters.at(xrow);
	pinters->bMemo = "Y";
}
	...
	for (int rowPosition = 0; rowPosition < count; rowPosition++)
	{
		...
		if ((strGubn == "X" || data[111] || nEndOPMarket == 1) && !bLast) // 예상가 적용	2013.08.22 지수예상가는111심볼이 없고 구분값이X로 온다.
		{
			...
			{  //장마감 동시호가 이후 체결(B) 데이터가 들어와서 현재가(예상가로 세팅되있는)를 초기화 해주는데 
				...
				strTime = (char*)data[40];
				if (m_grid->GetItemText(xrow, colEXPECT) == "0" && atoi(strTime) >= atoi(m_strEndTimeEnd))
				{
...
//					OutputDebugString(m_slog);
		...
		else if (data[23])
		{
			...
			if (_typeAuto == 0)
			{
				...
				if (atoi(m_strBeginTime) <= atoi(strTime) && atoi(m_strBeginTimeEnd) >= atoi(strTime) && bMarket)
				{
					if (entry != "0" && entry != "-0" && entry != "0.00" && entry != "+0" && entry != " 0" && entry != " ")
					{
						m_grid->SetItemText(xrow, colEXPECT, "1");
...
void CGridWnd::parsingAlertxx(LPARAM lParam)
{	
	if (m_bSending)
		return;
	...
	CString strCode = (code.GetLength() == 7) ? code.Mid(1) : code;
	const DWORD* data = (const DWORD*)alertR->ptr[0];
	CString strGubn = data[0] ? (char*)data[0] : "";
	...
	if (code.CompareNoCase("S0000") == 0) 
	{
		...
		if (CAST_TREEID(m_kind)->kind == xISSUE) 
		{
			PostMessage(WM_MANAGE, MK_INSERTNEWS);
		} 
		else
		{
			PostMessage(WM_MANAGE, MK_NEWSX);
			//parsingNewsx(data);
		}
		return;
	}
	...
	const auto isZeroEntry = [](const CString& val) {
		return val == "0" || val == "-0" || val == "0.00" || val == "+0" || val == " 0" || val == " ";
	};

	for (int rowPosition = 0; rowPosition < count; ++rowPosition) {
		// 중복처리...
		const int xrow = m_irowCode[rowPosition];
		CString entry, oldEXP = m_grid->GetItemText(xrow, colEXPECT);
		BOOL bTransSymbol = FALSE, bZisu = FALSE;
		const int nEndOPMarket = m_grid->GetItemData(xrow, colEXPECT);

		// 필터링된 마지막 데이터 무시
		CString str90 = (char*)data[90];
		str90.Trim();
		const BOOL bLast = (strGubn == "L" || strGubn == "4" || strGubn == "P" || strGubn == "g") && (str90 == "99" || str90 == "40");

		// 예상가/체결가 데이터 추출
		if ((strGubn == "X" || data[111] || nEndOPMarket == 1) && !bLast) {
		if (data[111])
			entry = (char*)data[111];
		...
		if (!isZeroEntry(entry)) {
			CString strTime = (char*)data[40];
			if (oldEXP == "0" && atoi(strTime) >= atoi(m_strEndTimeEnd)) {
				// 역전 데이터 스킵
			} else {
				m_grid->SetItemText(xrow, colEXPECT, "1");
			}
			...
			entry = m_grid->GetItemText(xrow, colCURR);
			AxStd::_Msg("[%s]예상가가 0이 올때는 현재가를 뿌려준다.", entry);
		}

		bTransSymbol = (_typeAuto == 0 || _typeAuto == 1);
		if (!bTransSymbol && !entry.IsEmpty())
			m_grid->SetItemText(xrow, colEXPECT, "0");
		}
		else if (data[23]) {
			entry = (char*)data[23];
			CString strTime = (char*)data[34];
			if (_typeAuto == 0) {
				if (atoi(m_strBeginTime) <= atoi(strTime) && atoi(m_strBeginTimeEnd) >= atoi(strTime) && bMarket) {
				if (!isZeroEntry(entry))
					m_grid->SetItemText(xrow, colEXPECT, "1");
			...
			else if (_typeAuto == 1 && !entry.IsEmpty()) {
				bTransSymbol = TRUE;
				if (!isZeroEntry(entry))
					m_grid->SetItemText(xrow, colEXPECT, "1");
				else {
		...
		CString newEXP = m_grid->GetItemText(xrow, colEXPECT);
		const BOOL bForceDraw = (newEXP == oldEXP) ? FALSE : TRUE;
		const BOOL bExpect = (BOOL)atoi(newEXP);
		const int countX = m_gridHdrX.GetSize();

		// 컬럼별 데이터 처리
		for (int ii = 2; ii < countX; ++ii) {
			const _gridHdr xgridHdr = m_gridHdrX.GetAt(ii);
			CString hsymbol(xgridHdr.symbol, strlen(xgridHdr.symbol));
			if (hsymbol == "2204") continue;
			//if (symbol.GetLength() >= 3) symbol = symbol.Right(3);
			CString symbol = symbol.Right(3);
			const int nSymbol = atoi(symbol);

			// entry 값 결정
			if (data[atoi(symbol)] && bZisu) // 2013.08.27 KSJ 지수일때는 심볼과 인덱스가 일치함.
			{
				entry = (char*)data[atoi(symbol)];
			}
			else if (!bTransSymbol) {
				switch (nSymbol) {
				case 111: entry = (char*)data[23]; break;
				case 112: entry = (char*)data[27]; break;
				case 115: entry = (char*)data[24]; break;
				case 116: entry = (char*)data[33]; break;
				case 204: entry = (strCode.GetLength() == 6) ? entry : " "; break;
				case 0:   entry = " "; break;
				default:  entry = (char*)data[nSymbol]; break;
				}
			}
			else {
				switch (nSymbol) {
				case 23:  entry = (char*)data[111]; break;
				case 27:  entry = (char*)data[112]; break;
				case 24:  entry = (char*)data[115]; break;
				case 33:  entry = (char*)data[116]; break;
				case 204: entry = (strCode.GetLength() == 6) ? entry : " "; break;
				case 0:   entry = " "; break;
				default:  entry = (char*)data[nSymbol]; break;
				}
			...
			if (symbol.CompareNoCase("051") == 0)		// 매도호가 심볼 변경
				entry = (char*)data[25];
			else if (symbol.CompareNoCase("071") == 0)	// 매수호가 심볼 변경
				entry = (char*)data[26];
			...
			// 실시간 데이터 가공이 필요한 컬럼들
			// 1.차트컬럼
			if (!bExpect && (m_bongField == ii) && m_bongField >= 0)
			{
				CString bongdata;
				if (data[30] && data[31]) {
					bongdata.Format("%s%c%s", (char*)data[30], P_TAB, (char*)data[31]);
					bongdata.Replace("+", "");
					bongdata.Replace("-", "");
					m_grid->SetItemText(xrow, m_bongField, bongdata);
				}
			...
			// 2. 전일거래대비율
			if (!bExpect && hsymbol.CompareNoCase("2321") == 0 && m_yDayVolField >= 0 && data[27])
			{
				CString s2321 = m_grid->GetItemText(xrow, ii);
				if (!isZeroEntry(s2321))
				{	
					const double dval =  atof((char*)data[27]) / atof(s2321) * 100;				
					entry.Format("%.2f", dval);
					m_grid->SetItemText(xrow, m_yDayVolField, entry);
				}
			}
		...
		// 실시간 계산 등 기타 처리
		if (m_ccField) calcInClient(xrow);
		if (m_posField && !bExpect) { /* ... */ }

		// 배분/임의 정보 처리
		if (data[950]) {
		CString str950 = (char*)data[950];
		if (!str950.IsEmpty()) {
			if (str950.GetAt(0) == '1')
			m_grid->SetItemData(xrow, colINFO, 12);
			else {
			CString item = m_grid->GetItemText(xrow, colINFO);
			CString strName = m_grid->GetItemText(xrow, colNAME);
			SetColInfo(strName, xrow, item);
			}
		}
		}
		if (data[951]) {
		CString str951 = (char*)data[951];
		if (!str951.IsEmpty()) {
			if (str951 == "12" || str951 == "14" || str951 == "16")
			m_grid->SetItemData(xrow, colINFO, 13);
			else {
