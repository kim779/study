...
void CintGrid::OnPaint() 
{
	...
	if (dc.GetClipBox(clipRect) != ERROR)
	{
		...
		{
			_bDraw = false;
			SetTimer(TIMEERDRAW, 300, nullptr);
		}
	}
	else
		_bDraw = true;
...
BOOL CintGrid::RedrawCell(int nRow, int nCol, CDC* pDC)
{
	...
	if (IS_WITHIN(minVisibleRow, maxVisibleRow, nRow) && IS_WITHIN(minVisibleCol, maxVisibleCol, nCol))
	{
		...
		if (m_drawHolding)
		{
			...
			const auto [it, inserted] = _timeFilterCode.try_emplace(nRow, tick);
			
			auto& filter = it->second;
			ULONGLONG diff = inserted ? 9999 : (tick - std::exchange(filter, tick));
			...
			else 
			{
				...
				//AxStd::_Msg("필터링됨 [%d][%ld]", nRow, diff);
			}
			return FALSE;
		}
	...
	if (m_memoRGN.PtInRegion(point))
	{
		...
		{	
			if (row >= GetFixedRowCount() && m_memoAry.GetAt(row - 1))
			{
				...
				m_memoWnd->loadMemoUsingTip(code);
			}
		}
...
bool CintGrid::getmemoCheck()
{
	...
	m_pParent->SeverMemoCheck(m_memoAry);
	return false;	
}

bool CintGrid::setmemoCheck(int xrow)
{
	if (m_memoAry.GetSize() < xrow)
	{
		m_pParent->SeverMemoCheck(xrow - 1, m_memoAry);
		return false;
	}
	return false;
}
