...
//OutputDebugString(m_slog);
		...
		m_iTime = optionIni["CUSTOMER"]["TIME"];
		m_iTime -= 300; // 10초 단위로 설정되어있음
		m_iTime = min(m_iTime, 500);

		//AxStd::_Msg("drawTime [%d]", m_iTime);
...
void CMainWnd::init()
{
	...
	m_iTime -= 300; // 10초 단위로 설정되어있음
	m_iTime = min(m_iTime, 500);
	//AxStd::_Msg("draw Time [%d]", m_iTime);
}
