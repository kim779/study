...
//  	}
	...
	char* dta = (char*)_Wizard->SendMessage(WM_USER, MAKEWPARAM(variantDLL, comm), (LPARAM)(LPCTSTR)data);
...
CPen* CBaseWnd::getAxPen(COLORREF clr, int width, int style)
{
	...
	penR.width = width;
	penR.style = style;
	return (CPen*)_Wizard->SendMessage(WM_USER, getPEN, (long)&penR);
}
...
{
	return (CBrush*)_Wizard->SendMessage(WM_USER, getBRUSH, (long)clr);
}
...
COLORREF CBaseWnd::getAxColor(int color)
{
	...
	return _Wizard->SendMessage(WM_USER, MAKEWPARAM(getPALETTE, 0), (LPARAM)color);
}
...
void CBaseWnd::DrawBitmap(CDC *pDC, CRect rc, CString strImage)
{
	...
	CBitmap* pBitmap = getAxBitmap(sImageDir);
...
CBitmap* CBaseWnd::getAxBitmap(CString path)
{
	CBitmap *pBitmap = (CBitmap *)_Wizard->SendMessage(WM_USER, getBITMAP, (LPARAM)path.operator LPCTSTR());
	return pBitmap;
}