...
LPCSTR head_text[] = { "현재가", "대 비", "등락률", "거래량", "시 가", "고 가", "저 가", "미결제" };
...
// 	CString strTemp;
// 	strTemp.Format("[KSJ] strSiga[%d] strKoga[%d] strJega[%d]", psd->siga, psd->koga, psd->jega);
...
void CSiseWnd::SetRealData( RealData *rp )
{
	...
	{
		SiseData sd;
		sd.curr = fabs(Str2Double( rp->find(23)->second ) * 100.0) + DOUBLE_PREFIX;
		sd.rate = fabs(Str2Double( rp->find(33)->second ) * 100.0) + DOUBLE_PREFIX;
		sd.gvol = Str2Int( rp->find(27)->second );
		sd.siga = fabs(Str2Double( rp->find(29)->second ) * 100.0) + DOUBLE_PREFIX;
		sd.koga = fabs(Str2Double( rp->find(30)->second ) * 100.0) + DOUBLE_PREFIX;
		sd.jega = fabs(Str2Double( rp->find(31)->second ) * 100.0) + DOUBLE_PREFIX;
		sd.mgjv = Str2Int( rp->find(201)->second );

		CString &diff = rp->find(24)->second.ToCStr();
		sd.udgb = diff.GetAt(0);
		sd.diff = Str2Double( diff.Mid(1) ) * 100.0;
...
void CSiseWnd::SetRealData( DWORD* data )
{
	if (data[34])	// 체결시간이 있으면~
	{
		...
		sd.curr = fabs(atof( strCurr ) * 100.0) + DOUBLE_PREFIX;

		CString strRate;
		strRate.Format("%s",(char*)data[33]);
		sd.rate = fabs(atof( strRate ) * 100.0) + DOUBLE_PREFIX;
		...
		sd.siga = fabs(atof( strSiga ) * 100.0) + DOUBLE_PREFIX;

		CString strKoga;
		strKoga.Format("%s",(char*)data[30]);
		sd.koga = fabs(atof( strKoga ) * 100.0) + DOUBLE_PREFIX;

		CString strJega;
		strJega.Format("%s",(char*)data[31]);
		sd.jega = fabs(atof( strJega ) * 100.0) + DOUBLE_PREFIX;
