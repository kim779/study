...
void CGridCtrlExt::OnDraw( CDC* pDC )
{
	...
	int offset_y = (cid.row-1) * GRID_CELL_HEIGHT;
...
BOOL CGridCtrlExt::AddArrow( CArrow &ar )
{
	...
	if (pos==m_Arrows.end())
	{
		...
		dx = (-2.0 * cos(degree)) - ( 3.0 * sin(degree)); ar.m_poly[0].x = (dx>0) ? dx+0.5 : dx-0.5;
		dy = (-2.0 * sin(degree)) + ( 3.0 * cos(degree)); ar.m_poly[0].y = (dy>0) ? dy+0.5 : dy-0.5;
		dx = ( 4.0 * cos(degree)) - ( 0.0 * sin(degree)); ar.m_poly[1].x = (dx>0) ? dx+0.5 : dx-0.5;
		dy = ( 4.0 * sin(degree)) + ( 0.0 * cos(degree)); ar.m_poly[1].y = (dy>0) ? dy+0.5 : dy-0.5;
		dx = (-2.0 * cos(degree)) - (-3.0 * sin(degree)); ar.m_poly[2].x = (dx>0) ? dx+0.5 : dx-0.5;
		dy = (-2.0 * sin(degree)) + (-3.0 * cos(degree)); ar.m_poly[2].y = (dy>0) ? dy+0.5 : dy-0.5;
