...
// 		OutputDebugString(strTemp);
		...
		if (m_pSiseData->code.GetAt(0) == '4' || m_pSiseData->code.GetAt(0) == 'D')
		{
			if (psd->gjga == 0)
			{
				if (*st == 0)
					...
					rate = (double)(*st) / 100 / (double)0.01;

				slog.Format("\r\n [3009] prc=[%d] rate = [%f]  ", *st, rate);
				OutputDebugString(slog);
			}
			else
			{
				double d1, d2, gval;
				gval = (double)psd->gjga;
				if (gval < 0)
					gval *= -1;
				d1 = ((double)*st / 100) - gval / 100;
				d2 = (double)psd->gjga / 100;
				rate = (*st - psd->gjga) / gval;

				slog.Format("\r\n [3009] prc=[%d] diff=[%f] gjga=[%f]  rate = [%f]   rate = [%f] ", *st, d1, (double)psd->gjga, rate, d1 / d2);
				OutputDebugString(slog);
			}
		}
		else
		{
			double d1, d2;
			d1 = ((double)*st / 100) - ((double)psd->gjga) / 100;
			d2 = (double)psd->gjga / 100;
			rate = (*st - psd->gjga) / (double)psd->gjga;
			slog.Format("\r\n [3009] prc=[%d]  diff=[%d] gjga=[%f]  rate = [%f]  rate =[%f] ", *st, (*st - psd->gjga), (double)psd->gjga, rate, d1 / d2);
			OutputDebugString(slog);
		}
		rate *= 100;

		slog.Format("\r\n [3009]  rate = [%f]  str =[%s] ", rate, Int2Str((int)rate));
		OutputDebugString(slog);

		CString stmp;
		stmp.Format("%.2f", rate);
		SetItemText(row, IDX_RATE, stmp);
