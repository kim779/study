...
int CMainWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_bMemoFileProcess = CheckMemoFileProcess();
...
void CMainWnd::CheckRTSTimer(bool bFirst)
{
	...
	m_icheckTime = GetPrivateProfileInt("OVERTIME", "TIME", 0, filePath);
	//고객용 HTS
	int icount = 0;
...
LONG CMainWnd::OperDLLOUB(CRecvData& rdata)
{
	...
	case TRKEY_INTER_MEMO_SEARCH:
	{
		...
		if (chRet == '1')
		{
			...
			m_slog.Format("\r\n[memo2022][%-40s]<%d>[%-35s]--> chRet=[%c]  strCode=[%s] memo=[%s] ",
				__FUNCTION__, __LINE__, "onTimer TM_MEMOSERVERSEARCH", chRet, strCode, strtemp);
			OutputDebugString(m_slog);
		}
		else
		{
			strtemp.Format("%s", DF_SERVERNO);
			m_mapMemoCode.SetAt(strCode, strtemp);
...
void CMainWnd::SendOper(class CSendData *sdata)
{
	...
	if (sdata->GetTRCODE().Find("pidomemo") >= 0)
	{
		...
		m_slog.Format("\r\n[memo][%-40s]<%d>[%-35s]--> sdata->GetDataKey()=[%d] len-[%d] gubn=[%s], code=[%s] ",
...
#ifdef DF_NONORDMAP
		if (_mapNoOrdRealData.size() <= 0)
			return;

		for_each(_mapNoOrdRealData.begin(), _mapNoOrdRealData.end(), [&](const auto& item) {
			_Ralertx* pval = item.second.get();
			m_pGroupWnd->RecvRTSx((LPARAM)pval);
			});
		_mapNoOrdRealData.clear();
#else
		if (_mapRealData.size() <= 0)
			return;

		for_each(_mapRealData.begin(), _mapRealData.end(), [&](const auto& item) {
			_Ralert* pval = item.second.get();
			m_pGroupWnd->RecvRTSx((LPARAM)pval);
			});
		_mapRealData.clear();
#endif
...
void CMainWnd::RTS_RecvRTSx(LPARAM lParam)
{
	...
	if (diff.GetTotalSeconds() > m_icheckTime && m_icheckTime)
		bOverChecking = false;
	...
	constexpr array arr = { 41, 51, 61, 71, 101, 104, 106, 107, 109, 146, 181 };
	bool bHoga = any_of(arr.begin(), arr.end(), [&rmap](const int& symbol) {
	...
	if (bHoga == false && sgubn.FindOneOf("DLy") != -1 && !m_pGroupWnd->CheckGetRTSHoga())
		return;

#ifdef DF_NONORDMAP
	std::string scode = CStringA(code);
	auto it = _mapNoOrdRealData.find(scode);

	if (it != _mapNoOrdRealData.end())
	{
		bool bcheck{};
		if ((char*)data[40] && !bcheck && bOverChecking && m_icheckTime)
		{
			char* pvalTime{};
			pvalTime = (char*)data[40];
			sgubn.Format("%s", pvalTime);
			if (atoi(strCurTime) == atoi(sgubn))
			{
				std::string scode = CStringA(code);
				m_slog.Format("[IB202200] check [%s]  [%s] [%s]", code, strCurTime, sgubn);
				OutputDebugString(m_slog);
				return;
			}
		}

		for (auto symarr : rmap)
		{
			const int ii = symarr.first;
			pdata = (char*)data[ii];
			_Ralertx* pval = it->second.get();

			if (pdata == nullptr)
				continue;

			const int len = strlen(pdata) + 1;
			pval->ptr[ii].reset(std::make_unique<char[]>(len).release());
			ZeroMemory(pval->ptr[ii].get(), len);
			memcpy(pval->ptr[ii].get(), (char*)pdata, len - 1);
		}
	}
	else
	{
		const auto mt = _mapNoOrdRealData.emplace(code, std::make_unique<struct _Ralertx>());
...
#else
	auto it = _mapRealData.find(code);
...
BOOL CMainWnd::CheckMemoFileProcess()
{
	CStringArray arrBookFile;
	CString sname, stmp, spath;
	sname = Variant(nameCC, "");
