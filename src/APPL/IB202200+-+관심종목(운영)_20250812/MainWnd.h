...
public:
	...
	int m_icheckTime{};
	CTime m_timeEnd{};   //동시호가 마감시간
	std::map<CString, int>    _mRealtime;
	std::map<CString, std::unique_ptr<struct _Ralert>> _mapRealData;
#ifdef DF_NONORDMAP
	std::unordered_map<std::string, std::unique_ptr<struct _Ralertx>> _mapNoOrdRealData;
#endif
	...
	BOOL m_bMemoFileProcess{};
	BOOL CheckBookFileProcess();
	BOOL CheckMemoFileProcess();
};
