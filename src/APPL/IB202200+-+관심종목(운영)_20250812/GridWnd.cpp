...
#ifdef DF_NONORDMAP
void CGridWnd::RTS_parsingAlertx(struct _Ralertx* palert)
#else
...
#ifdef DF_NONORDMAP
	code.Format("%s", palert->code.c_str());
#else
...
//XMSG(why????);
		...
		{    //L 선물호가 잔량(90심볼존재), 4 선물예상가(90심볼존재), P 옵션 호가 잔량(90심볼존재), g상품선물 호가잔량 (90심볼존재)
			//test real expect 우선 테스트로 예상가 지우는 코드 뺌  여기는 예상가만 처리하는 시간에 들어온다 아래 2줄 주석
			//m_grid->SetItemText(xrow, colEXPECT, "0"); // 2013.09.13 KSJ 옵션필터링때문에 마지막에 예상가를 주는데 무시해야한다.
			//m_grid->SetItemData(xrow, colEXPECT, 0);
			bLast = TRUE;
		}
