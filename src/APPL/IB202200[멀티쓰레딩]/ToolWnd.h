...
public:
	...
	CRect	MoveControl(CRect rcPrev, CSize size, CSize gap, CWnd* pButton);
...
public:	
	void marketTrigger(CString type);
	void addGroup(std::vector<std::pair<CString, CString>>& vGroup);
	void SetWizard(CWnd* pwizard) { _pWizard = pwizard; };
	...
	std::unique_ptr<CfxImgButton>	m_btTEXT;	// 설정/예상/엑셀
	...
	//std::unique_ptr<CButton>	_cxMarket;
	std::unique_ptr<CContainerWnd>	_cxMarket;
	...
	std::unique_ptr<CComboBox>	_cbAuto;
	int 				_typeauto = 0;
