...
BOOL CPopupDlg::OnInitDialog() 
{
	...
	int nSize = m_arrMenuID.GetSize();

	CRect rcWnd;
	GetWindowRect(rcWnd);
	int nHeight = nSize * 20 + GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CXBORDER) * 2;

	SetWindowPos(NULL, m_nX, m_nY, rcWnd.Width(), nHeight, SWP_NOZORDER);
...
void CPopupDlg::OnDestroy() 
{
	...
	CButton* pBtn;
	int nSize = m_arrBtn.GetSize();

	for (int i = 0; i < nSize; i++)
	{
		pBtn = (CButton*)m_arrBtn.GetAt(i);
