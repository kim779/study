...
#else
	m_bExitOnArrows = TRUE;
#endif
    m_Rect = rect;  // For bizarre CE bug.
    
    const DWORD dwEditStyle = WS_BORDER|WS_CHILD|WS_VISIBLE| ES_AUTOHSCROLL//| ES_NUMBER //|ES_MULTILINE
        | dwStyle;
...
*/
	...
	switch(nFirstChar)
	{
        ...
        case VK_NEXT: case VK_PRIOR: case VK_HOME: case VK_SPACE: case VK_END:
			SetSel(0,-1); break;
		default:
			if( nFirstChar < 0x80)	PostMessage(WM_CHAR, nFirstChar);   
			else					PostMessage(WM_IME_CHAR, nFirstChar);
			break;
	}
#endif
}
...
void CInPlaceEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
#if 1
	{
		...
		if (isalpha(nChar)) return;
		if (nChar=='.' && str.Find('.', 0)!=-1) return;
	}
#endif
