...
BEGIN_MESSAGE_MAP(CMapWnd, CWnd)
	...
	ON_BN_CLICKED(IDC_BTN_XLS, GridToXls)
END_MESSAGE_MAP()
...
//2014.06.03 KSJ 전체 계좌 갯수
void CMapWnd::SetTotalAccnt()
{
	int nTotal = m_owns.size();
	CString strTotal;
	
	strTotal.Format(" %d/750(등록계좌/등록가능계좌)", nTotal);
	m_stTotalAccnt.SetWindowText(strTotal);
}
...
void CMapWnd::Init()
{
	...
	Variant(titleCC, _T("[1900] 복수계좌 그룹등록"));
	...
	// Loading images
	{
		...
		m_hbtnXls[0] = GetAxBitmap((LPCSTR)( m_home + _T("\\image\\엑셀.bmp")));
		m_hbtnXls[1] = GetAxBitmap((LPCSTR)( m_home + _T("\\image\\엑셀_dn.bmp")));
		m_hbtnXls[2] = GetAxBitmap((LPCSTR)( m_home + _T("\\image\\엑셀_en.bmp")));
	}
	...
	m_rcGroupAccount[0].SetRect(285, 0, 650, 25);
	m_rcGroupAccount[1].SetRect(285, 25, 650, 390);

	m_rcRemainAccount[0].SetRect(655, 0, 865, 25);
	m_rcRemainAccount[1].SetRect(655, 25, 865, 390);
...
HBRUSH CMapWnd::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if (nID==IDC_STATIC_CONTROL || nID==IDC_STATIC_TOTOLACCNT)
...
void CMapWnd::InitGrid()
{
	...
	if (pGrid->Create(rc, this, IDC_GRID_GROUPLIST))
	{
		...
		pGrid->SetBkColor(GetIndexColor(COLOR_GRIDHEADTXT));
	...
	// GroupAccount grid setting (285, 25, 650, 390);
	...
	// RemainAccount grid setting (690, 25, 900, 390);
...
void CMapWnd::InitControl()
{
	...
	rc.OffsetRect(200+PADDING_VALUE, 0);
	rc.right = rc.left + 60;
	...
	rc.left = rc.right - 26;
	rc.bottom = rc.top + DEF_STATIC_HEIGHT;
	rc.right = rc.left + DEF_STATIC_HEIGHT;
	m_btnXls = CreateImgButton("", IDC_BTN_XLS, m_hbtnXls);
	m_btnXls->MoveWindow(&rc, FALSE);

	//2014.06.03 KSJ 총 계좌 갯수
// 	rc = m_rcRemainAccount[1];
// 	rc.top += PADDING_VALUE;
// 	rc.left += PADDING_VALUE;
// 	rc.bottom = rc.top + DEF_STATIC_HEIGHT;
// 	rc.right = rc.left + 200;
// 	if (m_stTotalAccnt.Create("", WS_CHILD|WS_VISIBLE|SS_LEFT|SS_CENTERIMAGE, rc, this, IDC_STATIC_TOTOLACCNT))
// 	{
// 		m_stTotalAccnt.SetFont(m_pFont, FALSE);
// 	}
}
...
//	m_pass.clear();	//2014.02.19 KSJ 처음에만 지우고 나중에는 계속 관리한다.
	for(size_t idx=0; idx<pai->size(); ++idx)
	{
...
//2014.12.01 KSJ 등록된 그룹을 엑셀데이터로 만든다.
void CMapWnd::GetGroupExcelData()
{
	vector<accn_info> *pai;
	char buff[32];
	CString strTemp;
...
// 			strTemp.Format("[KSJ] gpno[%s] gpnm[%s] acno[%s]", st->second.gpno, st->second.gpnm, strAcno);
...
BOOL CMapWnd::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	else if (wParam==IDC_GRID_REMAINACCOUNT)
	{
		...
		else if (gv->hdr.code==NM_DBLCLK && gv->iRow != 0)
		{
			...
			int row = pi->item.row, rows = m_pGrid[2]->GetRowCount();

			CDC *pDC = m_pGrid[2]->GetDC();
			for(; row<rows; ++row)
			{
				for(int col=0; col<m_pGrid[2]->GetColumnCount(); ++col)
				{
					if ((row+1)>1000)
						m_pGrid[2]->SetItemText(row, col, "");
					else 
						m_pGrid[2]->SetItemText(row, col, m_pGrid[2]->GetItemText(row+1, col));
					m_pGrid[2]->RedrawCell(row, col, pDC);
				}
			}
			m_pGrid[2]->ReleaseDC(pDC);
...
void CMapWnd::AddGroupAccount( accn_info *ai )
{
	...
	CString strPW = m_pass[ai->acno];  strPW.TrimLeft(); strPW.TrimRight();

	//2014.02.19 KSJ
	bool bExist = false;

	if(!strPW.IsEmpty())		bExist = true;

	WritePLog("AddGroupAccount - group[%s][%s] accn[%s][%s][%s]", m_pCurGroup->gpno, m_pCurGroup->gpnm, ai->acno2, ai->acnm, bExist ? "PASS":"NOPASS");
...
void CMapWnd::UpdateGroup()
{
	...
	if (IDYES==MessageBox(imsg, "IBK투자증권", MB_YESNO|MB_ICONWARNING))
	{
		...
		for(size_t n=0; n<updata.size(); ++n)
		{
...
void CMapWnd::DelGroupAccount( accn_info *ai )
{
	...
	CString strPW = m_pass[ai->acno];  strPW.TrimLeft(); strPW.TrimRight();
	WritePLog("DelGroupAccount - group[%s][%s] accn[%s][%s][%s]", m_pCurGroup->gpno, m_pCurGroup->gpnm, ai->acno2, ai->acnm, strPW.IsEmpty() ? "NOPASS":"PASS");
...
void CMapWnd::OnGroupUpdate( LPCSTR data, int dlen )
{
	...
	if(ecod.Compare("1801") == 0 || ecod.Compare("1796") == 0)	//1801 비밀번호틀림, 1796 비밀번호 5회 오류
	{
		...
		for(size_t ii = 0; ii <m_updateAcnn.size(); ii++)
		{
...
//		TRACE("CreateDispatch error....\n");
	...
	Worksheets Wss; _Worksheet Ws, Ws1;
	Range	range;
	long row, col;
	...
	for(; st!=ed; ++st)
	{
		VARIANT tVar;
		tVar.vt = VT_DISPATCH;
		tVar.pdispVal = (LPDISPATCH)Wss.GetItem(COleVariant((short)Wss.GetCount()));
		
		Ws  = Wss.Add(vOpt, tVar, vOpt, vOpt);
		CString strN = CString(st->second.gpnm, sizeof(st->second.gpnm));
		Ws.SetName(strN);
		
		CString	string; string.Format("A1:%c%d", (char)('A' + 10), 400);
		...
		for(int ii = 0; ii < m_pGrid[1]->GetColumnCount(); ii++)
		{
			TRY
			{
				col = ii+1;
				range.Set_Default(COleVariant((long)1), COleVariant((long)col), COleVariant((LPCTSTR)m_pGrid[1]->GetItemText(0, ii)));
			}
		...
		CString strAcno, strAcnm, strAcpw, strRate, strMult;

		for(size_t idx=0; idx<pai->size(); ++idx)
		{
			TRY
			{				
				strAcno = F2S((*pai)[idx].acno2);
				strAcnm = (*pai)[idx].acnm;
				strAcpw = "****";//F2S((*pai)[idx].acpw);
				strRate.Format("%.02f", (*pai)[idx].rate);
				strMult.Format("%d", (*pai)[idx].mult);
								
				row = idx +2;
				col = 1;
				range.Set_Default(COleVariant((long)row), COleVariant((long)col++), COleVariant((LPCTSTR)strAcno));
				range.Set_Default(COleVariant((long)row), COleVariant((long)col++), COleVariant((LPCTSTR)strAcnm));
				range.Set_Default(COleVariant((long)row), COleVariant((long)col++), COleVariant((LPCTSTR)strAcpw));
				range.Set_Default(COleVariant((long)row), COleVariant((long)col++), COleVariant((LPCTSTR)strRate));
				range.Set_Default(COleVariant((long)row), COleVariant((long)col++), COleVariant((LPCTSTR)strMult));
			}
			CATCH(COleDispatchException, e)
			{
				//				TRACE("runexcel error [%s]\n", e->m_strDescription);
		...
		//2014.11.26 KSJ 데이터 다넣고 AutoFit 해준다
	...
	for (int ii = 0; ii < pGrid->GetRowCount(); ii++)
	{			
		for (int jj = 0; jj < pGrid->GetColumnCount(); jj++)
		{
			TRY
			{
				row = ii+1; col = jj+1;
				range.Set_Default(COleVariant(row), COleVariant(col), COleVariant((LPCTSTR)pGrid->GetItemText(ii, jj)));
			}
			CATCH(COleDispatchException, e)
			{
				//				TRACE("runexcel error [%s]\n", e->m_strDescription);
...
void CMapWnd::WriteLog(LPCSTR name, int dlen, LPCSTR data)
{
	...
	//CString logpath = m_home + "\\IB190000.log";
	...
	fprintf(fp, (LPCSTR)time.FormatGmt("[%Y-%m-%d %H:%M:%S] "));
	fprintf(fp, "%-20s : [", name);
	fwrite(data, 1, dlen, fp);
	fwrite("]\n", 1, 2, fp);
