...
{	if(m_matrix.InsertRow(r)==false) return false;
		// 
	const int col = GetCountColumn();
	CELL* p1 = nullptr, * p2 = nullptr, * p3 = nullptr;
	// 
	for(int c=0; c<col; ++c)
	{	p1 = GetCell(r,c);
		p2 = GetCell(r+1,c);
		...
		{	p3 = GetCell(r-1,c);
			const double totalfactor = p3->real.factorHeight + p2->real.factorHeight;
...
{	if(m_matrix.InsertColumn(c)==false) return false;
	...
	{	p1 = GetCell(r,c);
		...
		{	p3 = GetCell(r,c-1);
			const double totalfactor = p3->real.factorWidth + p2->real.factorWidth;
...
{
	const int iSplitterWidth = GetSplitterWidth();
	const int iSplitterHeight = GetSplitterHeight();
		// 
	...
	m_iTotalHeight = max(rc->Height()-(iCountRow-1)*iSplitterHeight, iCountRow*m_iMinHeight);
		// 
	int r=0, c=0, pos=0, tail=0, cx=0, cy=0;
	CELL *p=nullptr;
		// 
...
{	
	...
	m_iTotalHeight = max(rc->Height()-(iCountRow-1)*iSplitterHeight, iCountRow*m_iMinHeight);
		// 
	int iCorrectedTail=0, iTotalOver=0;
	int r = 0,c = 0, pos = 0, cx = 0,cy = 0;
	CELL *p=nullptr;
		// 
...
{	
	const int iShiftX = point.x - m_ptStartDrag.x;
	const int iShiftY = point.y - m_ptStartDrag.y;
	CRect rcdrag;
		// 
	if(m_bDragHorz==true || m_bDragHorzVert==true)
...
{	
	...
	const double totalfactor = state1->factorWidth + state2->factorWidth;
	int cx1=0,cx2=0;
		// 
	if(m_SnapMode==SPLITTERCTRL_SNAP_LEFT_TOP || m_SnapMode==SPLITTERCTRL_SNAP_LEFT_BOTTOM)
...
{	CELL *p1 = GetCell(row,0);
	...
	const double totalfactor = state1->factorHeight + state2->factorHeight;
	int cy1=0, cy2=0;
		// 
	if(m_SnapMode==SPLITTERCTRL_SNAP_LEFT_TOP || m_SnapMode==SPLITTERCTRL_SNAP_RIGHT_TOP)
...
{	
	...
	const double totalfactor = p1->real.factorWidth + p2->real.factorWidth;
	int cx1 = totalcx/2;
	int cx2 = totalcx - cx1;
	if(cx1<m_iMinWidth)
	{	cx1 = m_iMinWidth;
...
{	
	...
	const double totalfactor = p1->real.factorHeight + p2->real.factorHeight;
	int cy1 = totalcy/2;
	int cy2 = totalcy - cy1;
	if(cy1<m_iMinHeight)
	{	cy1 = m_iMinHeight;
...
{	
	...
	_ASSERTE(rowCount>0 && colCount>0);
		// 
	int r=0,c=0;
	const int totalWidth = m_iTotalWidth - GetCell(0,col)->real.width;	// without width of col.
	double factor=0;
		// 
...
{	
	...
	_ASSERTE(rowCount>0 && colCount>0);
		// 
	int r=0,c=0;
	const int totalHeight = m_iTotalHeight - GetCell(row,0)->real.height;	// without height of row.
	double factor=0;
		// 
	for(r=0; r<rowCount; ++r)
...
{	_ASSERTE(m_ResizeMode==SPLITTERCTRL_RESIZE_DYNAMIC);
	...
	for(int c=0; c<col; ++c)
	{
		const double factor = (double)percent[c]/100.0;
			// 
		if(c!=col-1)
...
{	
	...
	for(int r=0; r<row; ++r)
	{
		const double factor = (double)percent[r]/100.0;
			// 
		if(r!=row-1)
...
{	
	...
	_ASSERTE(row>0 && col>0);
		// 
	int r=0,c=0, width=0, tail=0;
	const double factor = 1.0/(double)col;
		// 
	for(c=0; c<col; ++c)
	{	if(c!=col-1)
		{	width = (int)((double)m_iTotalWidth * factor + 0.5);
...
void SplitterCtrlBase::SetEqualHeightRows()
{
	...
	_ASSERTE(row>0 && col>0);
		// 
	int r=0,c=0, height=0, tail=0;
	const double factor = 1.0/(double)row;
		// 
	for(r=0; r<row; ++r)
	{	if(r!=row-1)
		{	height = (int)((double)m_iTotalHeight * factor + 0.5);
...
{	
	...
	{	
		...
		ar.Flush();
			// 
		const UINT uDataSize = (UINT)file.GetLength();

		std::unique_ptr<BYTE[]> pData = std::make_unique<BYTE[]>(uDataSize);


		memcpy(pData.get(), file.Detach(), uDataSize);

//		BYTE *pData = file.Detach();
			// 
		if(app->WriteProfileBinary(section,entry,pData.get(),uDataSize)==0)
		{
			//free(pData);
			return false;
		}
...
{	_ASSERTE(app!=NULL && section!=NULL && entry!=NULL);
	...
	BYTE *pData = NULL;
	UINT uDataSize=0;
	std::unique_ptr< BYTE[]> pData2 = std::make_unique<BYTE[]>(1);
		// 
	try
...
{	
	...
	*ar >> col;
	...
	{	int width=0,height=0;
		double factorWidth=0, factorHeight=0;
			// 
		for(int c=0; c<col; ++c)
		{	*ar >> width;
			*ar >> factorWidth;
...
{	
	CRect rc;
	const SPLITTERCTRL_SNAP snap = GetSnapMode();
	const int row = GetCountRow();
	const int col = GetCountColumn();
		// 
