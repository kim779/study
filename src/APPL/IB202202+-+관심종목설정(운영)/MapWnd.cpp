...
int CMapWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_slog.Format("\r\n[interest][IB202202][%s] m_bBookFileProcess=[%d]", __FUNCTION__, m_bBookFileProcess);
	OutputDebugString(m_slog);
...
void CMapWnd::uploadSequence()
{
	m_slog.Format("\r\n[interest][IB202202] -------------------------------------");
	OutputDebugString(m_slog);
	m_slog.Format("\r\n[interest][IB202202][%s]", __FUNCTION__);
	OutputDebugString(m_slog);
	...
	for(int i=0; i<count ; i++)
	{
		...
		m_slog.Format("\r\n[interest][IB202202]  ngrs=[%s] ogrs=[%s] gnam=[%s]", ngrs, ogrs, gnam);
		OutputDebugString(m_slog);
...
void CMapWnd::uploadEndGroup(int gno, int ipage)
{
	...
	m_slog.Format("\r\n[interest][IB202202][%s] gno=[%d] ogrs=[%s] gnam=[%s]", __FUNCTION__, gno, ogrs, gnam);
	OutputDebugString(m_slog);
