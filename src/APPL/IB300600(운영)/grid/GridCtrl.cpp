...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
    ...
    if (!pCell)
        return FALSE;
	//부하를 줄이기 위해서 같은건 넣지 않는다.
	if(pCell->GetText() == str)
	{
		return TRUE;
	}
...
CString slog;
slog.Format("[3006] OnLButtonDblClk x=[%d] y=[%d] row=[%d] col=[%d]\n", point.x, point.y, cell.row, cell.col);
...
CString slog;
slog.Format("[3006] OnLButtonDown x=[%d] y=[%d] row=[%d] col=[%d]\n", point.x, point.y, m_LeftClickDownCell.row, m_LeftClickDownCell.col);
OutputDebugString(slog);
...
CCellID cell = GetCellFromPt(point);
CString slog;
slog.Format("[3006] OnLButtonUp x=[%d] y=[%d] row=[%d] col=[%d]\n", point.x, point.y, cell.row, cell.col);
OutputDebugString(slog);
    ...
    {
slog.Format("[3006] MOUSE_PREPARE_DRAG \n", m_idCurrentCell.row, m_idCurrentCell.col, point.x, point.y);
OutputDebugString(slog);
    ...
    {
slog.Format("[3006] MOUSE_SIZING_COL \n", m_idCurrentCell.row, m_idCurrentCell.col, point.x, point.y);
OutputDebugString(slog);
        CRect rect;
        GetClientRect(rect);
    ...
    {
slog.Format("[3006] MOUSE_SIZING_ROW \n", m_idCurrentCell.row, m_idCurrentCell.col, point.x, point.y);
OutputDebugString(slog);
        CRect rect;
        GetClientRect(rect);
