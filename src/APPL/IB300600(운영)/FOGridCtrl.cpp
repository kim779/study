...
{	
	...
	CString strVal;
	//strVal.Format("[3006] %s", (char*)data[0]);
...
////////////////////////////////////////////////////////////
			...
			sRjega = (char*)data[31];  //K 저가
			sRjega.Replace("+", "");
			sRjega.Replace("-", "");
			
			sRgoga = (char*)data[30]; //L 고가
			sRgoga.Replace("+", "");
			sRgoga.Replace("-", "");
		
			map<int,int>::iterator pos = m_mapHogaRow.find((int)(fabs(Str2Double(sRjega)*100.0) + DOUBLE_PREFIX));
			int valL = (pos != m_mapHogaRow.end()) ? pos->second : 0;
			//if (pos == m_mapHogaRow.end())
			//	return;
			...
			int valH = (pos != m_mapHogaRow.end()) ? pos->second : 0;
			//if (pos == m_mapHogaRow.end()) 
			//	return;
...
// 					OutputDebugString(strTemp);
	...
	// [수정] data[40]이 없어도 현재가 바뀌었으면 스크롤 중앙 이동
	if (bCurrChange && !data[40])
	{
		SetCurrToCenter();
	}
}
