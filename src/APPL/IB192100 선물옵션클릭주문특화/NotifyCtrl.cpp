...
void CNotifyCtrl::CheckData()
{
	...
	if (*flag=='A')
	{
		...
		for( st=rows.begin(), ed=rows.end(); st!=ed; ++st)
		{
			...
			n= 0; sprintf(j.code, "%s", cols[n].ToCStr().GetString() );
			n= 1; sprintf(j.name, "%s", cols[n].ToCStr().GetString() );
			n= 2; sprintf(j.gubn, "%s", cols[n].ToCStr().GetString() );
			n= 3; j.jqty = Str2Int( cols[n] );
			n= 4; j.xqty = Str2Int( cols[n] );
			n= 5; j.pprc = (int)(Str2Double( cols[n] )*100 + DOUBLE_PREFIX);	// 가격은 정수로 관리(*100)
	...
	else if (*flag=='U' || *flag=='I')
	{
		...
		n= 3; j.jqty = Str2Int( cols[n] );
		n= 4; j.xqty = Str2Int( cols[n] );
		n= 5; j.pprc = (int)(Str2Double( cols[n] )*100.0 + DOUBLE_PREFIX);	// 가격은 정수로 관리(*100)
		n= 8; j.camt = Str2Double( cols[n] );
		n= 9; j.tamt = Str2Double( cols[n] );
		n=10; j.srat = Str2Double( cols[n] );
		n=11; j.mamt = Str2Double( cols[n] );
