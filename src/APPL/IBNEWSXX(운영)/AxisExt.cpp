...
CString CAxisExt::DiffStr( char udgb, int diff, int round /*= 2*/ )
{
	...
	{
	case '1': 
		pos = sprintf_s(buff, "¡è"); break;
	case '2':
	case '+':
		pos = sprintf_s(buff, "¡ã"); break;
	case '5':
	case '-':
		pos = sprintf_s(buff, "¡å"); break;
	case '4':
		pos = sprintf_s(buff, "¡é"); break;
	case '3':
	default:
		pos = sprintf_s(buff, " "); break;
	}
...
void CAxisExt::SetSize( int width, int height )
{
	m_pParent->SendMessage(WM_USER, MAKEWPARAM(sizeDLL, 0), MAKELPARAM(width, height));
}
