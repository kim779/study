...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
...

	{
		CRect rc;
		GetClientRect(&rc);
		if ( (scrollPos+rc.Height()) >= m_nVScrollMax )
			::PostMessage(GetParent()->GetSafeHwnd(), WM_VSCROLL_PS_END, 0, 0);
	}
...
std::shared_ptr<COleDataSource> CGridCtrl::CopyTextFromGrid()
...
//    COleDataSource* pSource = new COleDataSource();
    std::shared_ptr<COleDataSource> pSource = std::make_shared<COleDataSource>();
...
void CGridCtrl::OnBeginDrag()
{
    ...
    std::shared_ptr< COleDataSource > pSource = nullptr;
    if (!m_bAllowDragAndDrop && m_CurCol==-1)
        return;

    if (m_CurCol>=0)	pSource = std::shared_ptr<COleDataSource>();
...
void CGridCtrl::OnEditCut()
{
    ...
    //std::shared_ptr< COleDataSource > pSource = nullptr;
    std::shared_ptr< COleDataSource > pSource = CopyTextFromGrid();
    if (!pSource)
        return;
