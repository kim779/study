...
double IH::CharToDouble(char* data, int len)
{
	...
	const double ret = atof(sztmp);
	return ret;
}

double IH::CharToDouble(CString data)
{
	const double ret = atof(data);
	return ret;
}
...
{
	UINT	ret = value; 
	const CButton*	pButton = (CButton*)pWnd->GetDlgItem(id);
	if (pButton->GetCheck())
		ret |= option;
	else
		ret &= ~option;
...
{
	const COLORREF ocolor = pDC->SetTextColor(color);
	const int	nMode = pDC->SetBkMode(TRANSPARENT);
	const int	opt  = DT_SINGLELINE|DT_VCENTER | option;
...
int IH::ParsingOption(CMapStringToString& fms, CString stream, CString token)
{
	...
	const int	nToken = token.GetLength();
	if (nToken <= 0)
		return 0;
