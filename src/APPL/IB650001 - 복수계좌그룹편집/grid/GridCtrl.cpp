...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	//2013.08.13 KSJ bAscending이면 실제로는 빈값이 더 작은 값이지만 정렬되어 버리면 기존값이 아래로 가버린다.
	//그래서 빈값이면  -1을 곱해줘서 더 큰 것 처럼 인식하도록 수정
	if(lParamSort == 1)
	{
		if(strlen(pCell1->GetText()) == 0 || strlen(pCell2->GetText()) == 0)
			return _tcscmp(pCell1->GetText(), pCell2->GetText()) * -1;
		else
...
// 					if(strlen(GetCell(hi, col)->GetText()) == 0 || strlen(pCell->GetText()) == 0)
// 						continue;
