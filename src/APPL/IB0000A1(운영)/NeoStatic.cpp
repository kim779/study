 ...
 *****************************************************************************/
// NeoStatic.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CNeoStatic, CStatic)
	//{{AFX_MSG_MAP(CNeoStatic)
...
LRESULT CNeoStatic::HandleSetText(WPARAM wParam, LPARAM lParam)
{
	if (lParam)
	{
		m_text = (char*)lParam;
...
long CNeoStatic::HandleGetText(WPARAM wParam, LPARAM lParam)
{
...
long CNeoStatic::OnGetTextLength(WPARAM, LPARAM)
{
	return m_text.GetLength();
}

void CNeoStatic::OnPaint() 
{
	...
	else
	{
		...
		memdc.SetTextColor(m_frClr);
		memdc.DrawText(m_text, rc, dwStyle);
	}
...
void CNeoStatic::setColor(COLORREF bkClr, COLORREF frClr, COLORREF bxClr)
{
	m_bkClr = bkClr;
	m_frClr = frClr;
	m_bxClr = bxClr;
...
void CNeoStatic::setHeadColor(COLORREF bkClr, COLORREF frClr)
{
	m_hbkClr = bkClr;
	m_hfrClr = frClr;
}

CString CNeoStatic::Parser(CString &srcstr, CString substr)
{
...
void CNeoStatic::SetData(int nRow, CString sDat)
{
	if (ROWS <= nRow)
		return;
...
void CNeoStatic::SetHead(int nRow, CString sDat)
{
	if (ROWS <= nRow)
		return;
...
CString CNeoStatic::GetData(int nRow, bool bModified)
{
	if (ROWS <= nRow)
		return "";
...
CString CNeoStatic::GetHead(int nRow)
{
	if (ROWS <= nRow)
		return "";
...
void CNeoStatic::SetAttr(int nRow, DWORD Attr)
{
	if (ROWS <= nRow)
		return;
...
DWORD CNeoStatic::GetAttr(int nRow)
{
	if (ROWS <= nRow)
		return 0;
...
CString CNeoStatic::SetComma(CString src)
{
...
void CNeoStatic::OnTimer(UINT nIDEvent) 
{
	KillTimer(nIDEvent);
...
CRect CNeoStatic::getRect(int nRow)
{
	CRect rc;
	GetClientRect(&rc);
	if (ROWS)
...
void CNeoStatic::BeginBlink(int nRow)
{
	if (ROWS <= nRow)
		return;
	SetAttr(nRow, GetAttr(nRow) | DA_BLINK);
	InvalidateRect(getRect(nRow));
	SetTimer(nRow, 500, NULL);
}

LRESULT CNeoStatic::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	if (!m_bInitialize)
	{
		CString sText;
			...
			(LPARAM)sText.GetBufferSetLength(_cchUsual)) >= _cchUsual - 1)
		{
			...
			int nLen = CStatic::DefWindowProc(WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0) + 1;
...
void CNeoStatic::drawContrast(CDC *pDC, CRect cellRc, char cStyle, COLORREF clr)
{
	CRect	pRc = cellRc;
