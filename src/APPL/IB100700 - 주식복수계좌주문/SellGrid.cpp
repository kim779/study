// SellGrid.cpp: implementation of the CSellGrid class.
//
//////////////////////////////////////////////////////////////////////
...
void CSellGrid::OnSetup()
{
	CMapWnd* pMapWnd = (CMapWnd*)((CSellWnd*)GetParent())->GetParent();

	const CString arrHeader[MAX_SELLGRIDCOL] = { "", "계좌번호", "계좌명", "배수", "가능수량", "주문수량", "주문단가", "주문유형", "조건", "주문번호", "주문상태" };
	const int arrColWidth[MAX_SELLGRIDCOL] = { 20, 90, 60, 40, 60, 60, 60, 95, 45, 60, 400};
	const CString arrData[MAX_SELLGRIDCOL] = { "", "", "", "",	"", "", "", "지정가\n시장가\n조건부지정가\n최유리지정가\n최우선지정가\n장개시전 시간외종가\n시간외종가\n",
										"없음\nFOK\nIOC\n", "", "" };
	...
	SetNumberCols(MAX_SELLGRIDCOL);
	SetNumberRows(DEF_SELLGRIDROW);
	SetSH_Width(0);
	...
	for (int i = 0; i < DEF_SELLGRIDROW; i++)
		SetRowHeight(i, SZ_ROWHEIGHT);
}

void CSellGrid::OnDrawFocusRect(CDC *dc,RECT *rect)
{
...
int CSellGrid::OnDropList(long ID,int col,long row,long msg,long param)
{
...
int CSellGrid::OnCheckbox(long ID,int col,long row,long msg,long param)
{
	CUGCell cell;
...
BOOL CSellGrid::IsChecked(long nRow)
{
	CUGCell cell;
	GetCell(0, nRow, &cell);
	return cell.GetBool();
...
void CSellGrid::SetCheckState(int nRow, BOOL bState)
{
	CUGCell cell;
	GetCell(0, nRow, &cell);
	cell.SetBool(bState);
...
void CSellGrid::UpdateCheckedRowData(int nCol, CString strVal)
{
...
BOOL CSellGrid::IsAccComplete(long nRow)
{
...
int CSellGrid::GetSendData(CStringArray *pArr)
{
	CUGCell cell;
	const int nCount = GetNumberRows();
	CString strVal;
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (cell.GetBool())
		{
			...
			for (int j = 1; j < MAX_SELLGRIDCOL - 2; j++)
			{
				if (j==2 || j==4)
					continue;
...
void CSellGrid::OnDClicked(int col,long row,RECT *rect,POINT *point,BOOL processed)
{
	if (col != 5 && col != 6)
		return;
...
void CSellGrid::OnSetCell(int col,long row,CUGCell *cell)
{
	...
	case 5:
		{
			...
			const float fRate = (float)atof((LPCTSTR)strRate);
...
int CSellGrid::OnCellTypeNotify(long ID,int col,long row,long msg,long param)
{
	if (ID == UGCT_DROPLIST)
...
int CSellGrid::OnEditFinish(int col, long row, CWnd* edit, LPCTSTR string, BOOL cancelFlag)
{
/*	CUGCell cell;
	...
	if(col == 4)
	{
		CString strRate = QuickGetText(col - 1, row);
		if (strRate.IsEmpty())
			return TRUE;

		float fRate = (float)atof((LPCTSTR)strRate);

		int nQty = atoi(string);
		strVal.Format("+%015d", (int)(nQty * fRate));

		CString strTemp = QuickGetText(col, row);
		cell.SetText(strVal);
		SetCell(col, row, &cell);
		QuickSetText(col, row, strVal);
...
int CSellGrid::OnCanSizeCol(int col){
	UNREFERENCED_PARAMETER(col);
	return FALSE;
}

CString CSellGrid::Parser(CString &srcstr, CString substr)
{
...
BOOL CSellGrid::CheckupMistakeOrder(CWnd *pWnd, double strPrice, int strQuantity, CString strCode, CString strListedQuantity)
{
	ASSERT(pWnd != NULL);
...
void CSellGrid::ChangeFont(CFont* pFont, double* lrate)
{
...
void CSellGrid::SetPallete(CMapWnd* pWizard)
{
	const COLORREF colorbg[] = { pWizard->GetIndexColor(CLR_GRID_BACK), pWizard->GetIndexColor(CLR_GRID_BACK2)};
	int ibgrow=0;
	...
	m_pLinePen = std::make_unique<CPen>(PS_SOLID, 1, pWizard->GetIndexColor(CLR_GRID_LINE));
	CUGCell cell;

	for (int i = -1; i < GetNumberRows(); i++)
	{
		for (int j = 0; j < MAX_SELLGRIDCOL; j++)
		{
			GetCell(j, i, &cell);

			if (i == -1)
			{
				cell.SetBackColor(pWizard->GetIndexColor(CLR_GRID_HBACK));
				cell.SetTextColor(pWizard->GetIndexColor(CLR_GRID_HTEXT));
			}
			else
			{
				cell.SetHBackColor(pWizard->GetIndexColor(CLR_GRID_SBACK));
				cell.SetHTextColor(pWizard->GetIndexColor(CLR_GRID_STEXT));

				cell.SetBackColor(colorbg[ibgrow]);
				cell.SetTextColor(pWizard->GetIndexColor(CLR_GRID_TEXT));
			}
