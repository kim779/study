...
inline LPCSTR CCommonWnd::GetDiffGiho(double val)
{
	static LPCSTR giho[3] = { "¡ã", "¡å", " " };
	if (val > 0.0) return giho[0];
	else if (val < 0.0) return giho[1];
	else return giho[2];
}

inline CString CCommonWnd::GetTrimStr(LPCSTR data, int dlen)
{
...
inline double CCommonWnd::Str2Double(LPCSTR data, int dlen)
{
	char buff[32];
...
inline int CCommonWnd::Str2Int(LPCSTR data, int dlen)
{
	char buff[32];
	CopyMemory(buff, data, dlen);
	buff[dlen] = 0;
...
inline CString CCommonWnd::Int2Str(int data, int round)
{
	char buff[32]{};
	if (round == 0)
	{
...
inline CString CCommonWnd::Str2CommaStr(LPSTR src)
{
	char dst[32]{}, * buf{}, * str{};
	int len = strlen(src);
...
inline CString CCommonWnd::Int2CommaStr(int val)
{
...
inline CString CCommonWnd::Double2CommaStr(double val)
{
	char dst[32]{}, src[32]{}, * buf = NULL, * str = NULL;
...
inline void CCommonWnd::ParseString(LPCSTR st, LPCSTR ed, CHAR del, vector<StringProxy>& retval)
{
	retval.clear();
	LPCSTR pos{};
	while (st < ed)
...
inline int CCommonWnd::GetTick(int fprc, int tprc, CODE_TYPE type)
{
	int tick = 0;
	if (fprc < tprc)
	{
...
inline int CCommonWnd::GetPrice(int price, CODE_TYPE type, int tick)
{
...
inline COLORREF CCommonWnd::GetColor(double val)
{
