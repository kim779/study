...
//IMPLEMENT_DYNAMIC(CScrollWindow, CWnd)

BOOL CScrollWindow::m_bGotMouseScrollLines = FALSE;

//마우스 휠 라인을 알아 오려고 하는것이다.
//메시지(MSH_SCROLL_LINES)를 던져서 알아오려고 했는데 안되면
//그냥 uCachedScrollLines =3 으로 하고 적용해버리려는 것이다. 이때 윈도우 버전으로 나눠서 한다는
UINT CScrollWindow::GetMouseScrollLines()
{
   ...
   // return what we've already got
	...
	{
		//윈도우 메시지 하나를 등록해서 이것도 콜백으로 시스템에서 내려받으려는것이다.
		//화면를 이루는 스크롤의 라인수를 받아 오려고 하는 것이다.
	...
	{
		//위도우를 찾으면 위의 메시지를 날려서 알아오려 했지만 윈도우는 없다.
...
CScrollWindow::CScrollWindow()
{
	...
	DWORD m_dwVersion, m_dwVersionMinor;
	BOOL m_bWin95;
	BOOL m_bWin4;
	BOOL m_bMarked4;

	GetWindowsVersion(m_dwVersion, m_dwVersionMinor);
...
BOOL CScrollWindow::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
