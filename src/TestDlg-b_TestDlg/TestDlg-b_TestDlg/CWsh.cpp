...
// CWsh 메시지 처리기
//실제 이CWsh 클래스가 WinThread 상속이다 이 함수는 클래스가 만들어지면 호출되서 클래스 목적이 되는  while를 돈다.
int CWsh::Run()
{
...
void CWsh::DoParse(CMQue* pMQ)
{
	...
	switch (rsmH->resK)  //리소스의 종류가
	{
	case resK_REQ:      // request runtime module information  exe파일이거나
	case resK_REQ2:  // request resource information   //다른 리소스 파일이거나..
		if (!MakeDir(rsmH, path))
			return;
		switch (rsmH->resF)
		{
			case resF_ONLY:  // only chain resource  파일을 패킷을 쪼개서 내리니 체인처럼.. .붙여야 한다.
			case resF_FIR:
		...
		if (rsmH->resF == resF_ONLY || rsmH->resF == resF_LAS)  //내려주는 파일이 연속처리 필요없는 하나거나  마지막 패킷이면..
		{
			switch (rsmH->resK)
			{
				case resK_REQ:
		...
		return;
	case resK_RSP:  // resource end response  리소스 내려주는게 끝났다??
		switch (m_state)
		{
		case stFlag::stAXIS:
			m_state = stFlag::stERR;
			m_pWnd->SendMessage(WFM_EVENT, FEV_ERROR, (LPARAM)tmps.operator LPCSTR());
			break;
		case  stFlag::stRSC:
	...
	case resK_AXIS:  //리소스kind rk  플랫폼 파일이거나
	case resK_RSC:   //RSC 파일이면
		switch (rsmH->resF)   //resource flow 가
		{
		case resF_ONLY:
		case resF_FIR:
			if (m_datb)
	...
	BOOL update = FALSE;
	if (rsmH->resF == resF_LAS || rsmH->resF == resF_ONLY)  //마지막이거나 하나면 되는 패킷은..
	{
		update = true;
...
{
	m_pWnd->SendMessage(WFM_EVENT, FEV_ANM, (LPARAM)pMQ);  //실시간 데이터 처리하라고 보내는..
}
