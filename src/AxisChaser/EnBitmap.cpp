...
BOOL CImageGrayer::ProcessPixels(RGBX* pSrcPixels, CSize sizeSrc, RGBX* pDestPixels, CSize )
{
	...
	for (int nX = 0; nX < sizeSrc.cx; nX++)
	{
		for (int nY = 0; nY < sizeSrc.cy; nY++)
		{
			...
			{
				//----------
				x = btnpos[nX/ICX].left+nX%ICX;
				y = btnpos[nX/ICX].top +nY;
//				if (x < 0 || x > sizeSrc.cx)
//					continue;
...
void	CToolBar24::SetFullColorImage(UINT ID, COLORREF rgbBack, int* num)
{
	...
	if(num!=NULL)
	{
		...
		C32BitImageProcessor::btnpos[0].top=int(3.5);// 7/2=3.5
		...
		for(j=i=0;i<btn;i++)
		{
			if(num[i+1]==0)
			{
				...
				k+=31;//24+7=31
				C32BitImageProcessor::btnpos[j].top=int(3.5);
...
//delete C32BitImageProcessor::btnpos;
	}
	else
	{
	...
	m_imgToolbar.Create(ICX, ICY, ILC_COLOR24|ILC_MASK, 1, 1);
...
/*
	...
	m_imgToolbarDis.Create(ICX, ICY, ILC_COLOR24|ILC_MASK, 1, 1);
	...
	m_imgToolbarNA.Create(ICX, ICY, ILC_COLOR24|ILC_MASK, 1, 1);
...
CToolBar24::~CToolBar24()
{
	if (C32BitImageProcessor::btnpos)
		delete C32BitImageProcessor::btnpos;
}