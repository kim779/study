...
Hinweis: ?1999 by Christian Rodemeyer
         Info ?er HLS Konvertierungsfunktion
...
void CColor::ToHLS() 
{
  if (!m_bIsHLS)
  {
    ...
    { 
      ...
      float rate = 60.0f;
      if (m_color[c_red]   == maxval) m_hue = rate * (6.0f + bnorm - gnorm);
      if (m_color[c_green] == maxval) m_hue = rate * (2.0f + rnorm - bnorm);
      if (m_color[c_blue]  == maxval) m_hue = rate * (4.0f + gnorm - rnorm);
