...
void CMultiTree::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_bDragDrop)
	{
		...
		{
			CWnd *pWnd = GetOwner();
			pWnd->SendMessage(DD_LBDOWN, nFlags, (LPARAM)&point);
		}
	}
}
...
void CMultiTree::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	{
		CWnd *pWnd = GetParent();
		pWnd->SendMessage(DD_LBUP, nFlags, (LPARAM)&point);
	}
}
...
void CMultiTree::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{
		CWnd *pWnd = GetParent();
		pWnd->SendMessage(DD_MOUSEMOVE, nFlags, (LPARAM)&point);
	}
}
...
const UINT RMSG_DBCLK = ::RegisterWindowMessage("RMSG_DBCLK");
void CMultiTree::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	GetParent()->SendMessage(RMSG_DBCLK);
	*pResult = 0;
}
