...
int CSTTreeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CSTExtTree::LoadSTTree()
{
	...
	szDir.Format("%s%s", m_szRootDir , "\\Icss\\STData\\ST");	//@Solomon
	CStringArray arrDir;
	...
	//2. 사용자단위전략을 로딩한다.
	szDir.Format("%s%s", m_szRootDir , "\\Usr\\STData\\ST\\사용자단위전략");
	...
	//3. 사용자조합전략을 로딩한다.
	szDir.Format("%s%s", m_szRootDir , "\\Usr\\STData\\ST\\사용자조합전략");
...
void CSTBasicTree::LoadSTTree()
{
	CString szDir;
	//szDir.Format("%s%s", m_szRootDir , "\\Data\\STData\\STBasic");
	szDir.Format("%s%s", m_szRootDir , "\\Icss\\STData\\STBasic");	//@Solomon
	CStringArray arrDir;
	...
	//2. 사용자단위전략을 로딩한다.
	szDir.Format("%s%s%s", m_szRootDir , "\\Usr\\STData\\STBasic\\", SZUSERFOLDER);
