...
void CXListCtrlJongData::InsertCodeData(LPCTSTR lpCode, LPCTSTR lpName,CString strPrice,CString strChange,CString strChgrate,CString strExtCode,char chSign, int nMarketGubun /*=1*/)//, BOOL bKeeping /*=FALSE*/)
{
...
/////////////
	...
	if(nCnt<0) return;		//Error Handling
...
void CXListCtrlJongData::GetCodeData(int nIndexIn, CString& rstrPriceOut,CString & rstrChangeOut,CString & rstrChgrateOut,CString & rstrExtCodeOut,char& rchSign)
{
	if(nIndexIn!=-1)
	{
//		char szTemp[126];
...
//		rstrChgrateOut = szTemp;
		...
		if(pData)
		{
			...
			rstrExtCodeOut = pData->m_strExtCode;
...
void CXListCtrlJongData::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if( nChar==VK_DOWN || nChar==VK_UP )
	{
		...
		{
			if( m_pLeftBarDlg->GetEnableWindow() )
				((CTabDlg*)GetParent())->m_pChartMng->SendMessage(UMSG_INPUTCODETOCHARTMNG, MODE_DEFAULT, (LPARAM)&stInputCodeData);
			else
			{
				
				((CTabDlg*)GetParent())->m_pChartMng->SendMessage(UMSG_STANDBY_TR_CALL, 0, (LPARAM)(LPCTSTR)stInputCodeData.m_strCode);
			}
		}
	}
...
BOOL CXListCtrlJongData::UpdateCodeData(LPCTSTR lpCode, LPCTSTR lpName, CString strPrice,CString strChange,CString strChgrate,CString strExtCode,char chSign, int nIndex)
{
//	::ZeroMemory(m_bSortIndex,sizeof(m_bSortIndex));
	int nCnt = 0;
	CJongDataInListCtrl *pData = NULL;
...
//	{
		...
		if(nCntAll!=(nItemLastSelected+1))
		{
			...
			for( nPos=(nItemLastSelected+1);nPos<nCntAll&&(nPos-nItemLastSelected-1)<nCnt;nPos++)
			{
				SetCurSel(nPos);
			}
			EnsureVisible(nPos,FALSE);
