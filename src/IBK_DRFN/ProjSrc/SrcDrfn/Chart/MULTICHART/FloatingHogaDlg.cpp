...
{	
	...
	if(PtInRect(trect, point))
	{
		...
		if(m_bIconState && m_strCode.GetLength()>0)
			OnCodeUnAdvise(m_strCode);	// Old Code해제
	}
...
/*	else
	{
		...
		{
			SetHogaData(pData, nLen, TRUE);			// 상한가 하한가등 기본정보만 얻음
			MakeFullHogaGrid();
		}
		m_bUseLimitHoga = TRUE;

		SetFullHogaDataToGrid();					// 전체 호가를 그리드에 세팅					
		SetHogaData(pData, nLen);				// 호가데이터를 m_HogaData에 저장
		SetRsvdOdrInfoAtGrid_FullHoga();			// 그리드 예약 내역 표시
		...
		{
			ResortHogaRow(m_nChartPriceRow);	// 차트가격 Scroll
			m_bMoveChartPriceRow = FALSE;
		}
		else
		{
			ResortHogaRow(m_nCurPriceRow);		// 현재가 Scroll
		}
		// <----------------------------------------
	}*/
}	
...
//	if(m_chSign == '9')
	{
		...
		// 예상 체결수량
		strExpVol = m_Util.GetComma(kb_01230.m_szyevolume) + "주";
...
//	}	
//
//	//{{ 2007.06.26 by LYH 예상가 리얼 적용
//
//	CString strSign;
//	bRet = pReal->GetDataString(15317, strSign);// 등락부호
//	if(bRet)
//	{
//		if(strSign == "69")
//			m_chSign = '9';
...
//		{
//			sTemp1.Format("%10.10s", sTemp);		sTemp1.TrimLeft();		sTemp1.TrimRight();
//			strText = m_Util.GetComma(sTemp1) + "주";
...
// 	if(strncmp(pRealItem->szBcCode, NAME_KXH1, strlen(NAME_KXH1)) == 0)			// 호가
// 	{
// 		SetRealHogaData(pRealItem->szBcData);
// 		return 1L;
// 	}
// 
// 	if(strncmp(pRealItem->szBcCode, NAME_KXYS, strlen(NAME_KXYS)) == 0)			// 예상
// 	{
...
//<< [QC No]090803HMC075

	{
		...
		// 예상 체결수량
		strExpVol = m_Util.GetComma(kbReal_KXYS.m_szyevolume) + "주";
...
//@SM-100104SM001. 예상체결일때 Minus(-)값이 와서 제거 시킴.
