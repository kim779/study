...
HRESULT WINAPI fnInit(STODINIT_IN* pIn, STODINIT_OUT* pOut)
{
	...
	CStdDialog* pDialog = NULL; 

	if (pIn->m_nScreenID == CTradingLogDialog::ScreenNo)
		pDialog = new CTradingLogDialog;	// 매매일지 차트인 경우, CTradingLogDialog 생성
	else
		pDialog = new CStdDialog;
	//<< 240412 홍성식 - 매매일지차트
...
extern "C" __declspec(dllexport) CWnd* CREATE_CHARTSCREEN(int iTradingID, CWnd* pParent, HWND hPlatform)
{
	...
	InitIn.m_nScreenID = iTradingID;	// 화면 번호를 넘김
