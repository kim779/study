...
//#ifdef __MULTI_LANG__
	HWND m_hOcxWnd;		//@Solomon-MultiLang:091117
//#endif
...
public:
	...
	void	ReceiveRealHogaData(DWORD dwKey, LPCTSTR pszCode, long dwData);
	//===============================================================================

	//enum	{STOCK_CHART, UPJONG_CHART, FUTOPT_CHART, FOREIGN_CHART};
//@유진삭제	enum	{STOCK_CHART, UPJONG_CHART, FUTOPT_CHART, ELW_CHART, FOREIGN_CHART, COMMODITY_CHART};	// 상품선물옵션차트 - ojtaso (20071001)
	//===============================================================================
	//차트 환경 연동
...
//@유진추가
	...
	void SetRealYPriceData(void *pRealData);
	...
	int	 m_nSTRunState;		// 전략저장상태(시작/종료)
	int GetSTRunState()						{ return m_nSTRunState; }
	void SetSTRunState(int nSTRunState)		{ m_nSTRunState = nSTRunState; }
...
public:
	...
	void SetChartLayout(int nLayout);
	BOOL LoadDWMChart();
	void ShowAvgLine(CString strCode, CString strPrice);
...
private:
	...
	//차트 전체 초기화
	void ResetChartAll(BOOL bTrRetry=TRUE);
	// 새로운 차트 추가 : 복수종목 - ojtaso (20070108) 
	BOOL m_bAddMode;
	...
	void DoSiteMenuCall(int _nMenuID);	//MultiChart에서 추가한 메뉴에 대한 처리.

	int m_nHeightOfCaption;

	int m_nChartLayout;
	int m_nDWMChartIndex;
	BOOL m_bFutPrevChart;

	int	m_nAllDayViewCount;
...
public:
	...
	void		GoNextChart();
	void		SetInLoop(BOOL bLoop);

	BOOL		GetPrevDateFlag() { return m_bFutPrevChart; }

	void		SetFutPrevChart(BOOL bSet) { m_bFutPrevChart = bSet; }
...
//	HBITMAP			m_hBitmap;
	...
	BOOL IsAvailableForReplay();
	BOOL GetChartExtended() { return m_bChartExtended; }
	
// vntsorl:[0001250] [5001] 종합차트 - 일주월분초틱 주기별로 지표 설정 및 설정값을 다르게 할 수 있도록 개선요청
	void Synchro_jipyo(BOOL bChangePeriod=TRUE, LPCTSTR lpszCodeForRequest=NULL);
	BOOL GetBTypeLink()							{ return m_bTypeLink;}

	void SendPrevDateData();

	void GetCurStatusInfo(char& cType, CString& strNumber, CString& strDate, CString& strTime, long& lCount);
	long GetPacketCount(LPCTSTR lpszPacketName);
	short GetPacketIndex(LPCTSTR lpszPacketName, double dData);
	short GetPacketNearIndex(LPCTSTR lpszPacketName, double dData);
	CString GetPacketData(LPCTSTR lpszPacketName, short nDataIndex);

	void AddConcPacketGraph();
	void AddConcPacketData(LPCTSTR lpszConcTimePacketData, int nConcCountLen,
...
//>> [QC No]090731HMC070
	BOOL	m_bNoInsertChart;
//<< [QC No]090731HMC070

	void ChartItemUnAdvise(CChartItem *pItem, BOOL bAddMode);

// (2008/8/10 - Seung-Won, Bae) Support UpdateIndex in Maxized Chart Item.
protected:
	BOOL	m_bUpdateIndexAfterExtended;

	int		m_nLeftbarReplayState;

	CString m_szCommodityUnit;
...
protected:
	...
	LRESULT OnSetQueryOpy( WPARAM wParam, LPARAM lParam);
//	LRESULT OnTestTR(WPARAM wParam, LPARAM lParam);
	LRESULT OnChangeVertScaleType( WPARAM wParam, LPARAM lParam);
	LRESULT OnRmgGetInfo(WPARAM wParam, LPARAM lParam);
	LRESULT OnRmgSetInfo(WPARAM wParam, LPARAM lParam);
// vntsorl:[0001250] [5001] 종합차트 - 일주월분초틱 주기별로 지표 설정 및 설정값을 다르게 할 수 있도록 개선요청
	LRESULT OnGetCurrentMode( WPARAM wParam, LPARAM lParam);
	DECLARE_MESSAGE_MAP()
		
//	CUIntArray	m_menuMain, menuChart;
	void _DoMoveToSaveDir(LPCSTR szSelInfo);
	void _DoSavedData_To_LastCode(LPCSTR strFileName);
public:
	BOOL  IsLastEnvUpdated() { return m_bLastEnvUpdated; }	
	BOOL  GetRelayPossible();
};
