...
#define TOOL_BUTTON_BASE_ID		500
...
BOOL g_bToggle_Bar[NUM_BUTTONS] = {
	...
	TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, TRUE, TRUE,			//40~
...
BOOL CDlgToolbar::OnInitDialog() 
{
	...
	if(m_pMultiChartWnd)
	{
		...
		pMainDlg->SetPermanentHandle(this);
...
BOOL CDlgToolbar::CreateToolButton()
{
	...
	if(!m_BtnFont.CreateFontIndirect(&lfont))
	{
		TRACE("%s[%d] : Tool Button용 font 생성 실패!", __FILE__, __LINE__);
		return FALSE;
	}
	m_ToolTip.SetFont(&m_BtnFont);
...
void CDlgToolbar::ReadIniFile(BOOL bReload /*=FALSE*/)
{
	...
	int nID = 21;
	IMainInfoManager* pMainInfoManager = (IMainInfoManager*)m_pGateManager->GetInterface(nID);

	//# Data Folder
	strPath = pMainInfoManager->GetDataDir();
	strDataPath.Format("%s\\%s", strPath, "ChartToolBarSetting.dat");
	//# User Folder
	//strPath = pMainInfoManager->GetEncUserDir();
	strPath = ((CStdDialog*)m_pMultiChartWnd)->m_strUserDir;
...
//-----------------------------------------------------------------------------
// Author		: Hui-Syn, Lee	Date :2006.9.11
// Modifier		: 
...
//	case 30: return CToolOptionInfo::TOOLOPTION::T_TOOL_PERIOD_SUMMARY;		// 기간요약
	...
	case 61: return CToolOptionInfo::TOOLOPTION::T_TOOL_SELECTION;			// 차트이동
//	case 62: return CToolOptionInfo::TOOLOPTION::T_TOOL_SELECTION;			// 부분확대
	...
	case 74: return CToolOptionInfo::TOOLOPTION::T_TOOL_BALANCE_BASIC;		// 일목수치			// 2014.09.24 박성경 : 0010862: [7111] 삼성증권 차트 툴바의 [일목수치] 기능 IBK차트에도 동일하게 추가 요청
...
void CDlgToolbar::ResizeControl(int cx, int cy)
{
	...
	if(m_nSelTool == 0)
	{
		...
		m_ToolTip.AddTool(m_pToolBtn[0], "사용자설정");//KHD : 툴팁변경 
	...
	else if(m_nSelTool == 1)
	{
		...
		m_pToolBtn[0]->SetValueBtn(GetToolOption(TOOL_BUTTON_BASE_ID + 0), 
							   g_bToggle_Bar[0], m_pIL_W28H22, nKeyInx*3, nKeyInx*3+1, nKeyInx*3+2);
		m_ToolTip.AddTool(m_pToolBtn[0], "분석도구");//KHD : 툴팁변경 
	...
	else if(m_nSelTool == 2)
	{
		...
		m_pToolBtn[0]->SetValueBtn(GetToolOption(TOOL_BUTTON_BASE_ID + 0), 
							   g_bToggle_Bar[0], m_pIL_W28H22, nKeyInx*3, nKeyInx*3+1, nKeyInx*3+2);
		m_ToolTip.AddTool(m_pToolBtn[0], "부가기능");//KHD : 툴팁변경 
...
{
//@Solomon--> 솔로몬에서는 PreTranslateMessage가 불리지 않기때문에 WindowProc에서 처리함.
...
{
	//@Solomon:091111SM018--> 솔로몬에서는 PreTranslateMessage가 불리지 않기때문에 WindowProc에서 처리함.
...
// 	}
	...
	if(message == UDM_CHARTBUTTON_CLICK)
	{
		if(wParam == 3)	//Not Toggle Button Up
		{
			...
			if(nIdx > 3)
			{
				...
				{
					m_bCheckDWMMChart = !m_bCheckDWMMChart;
					if(m_pChartMng->GetSafeHwnd())
						m_pChartMng->SendMessage(UMSG_DWMM_CHART, NULL, (LPARAM)m_bCheckDWMMChart);
					if(!m_bCheckDWMMChart)
						SelectToolSelection();
				}
...
//	rcDraw.bottom = Y_OFFSET - 7;
	...
	if(m_pBtnDown && m_pBtnDown->IsWindowVisible())
	{
		memDC.MoveTo(rcBtns.left, rcBtns.bottom);
		memDC.LineTo(rcBtns.right, rcBtns.bottom);
	}
...
BOOL CDlgToolbar::DrawLowMenu(int nLowMenuCount, int nHighMenuCount, int nSelTool)
{
	...
	rect.right = 0;
	rect.bottom = 0;
	for( i = 3; i < m_arrayTotalIdx.GetSize()+5; i++)
	{
		if(m_pToolBtn[i])
			m_pToolBtn[i]->MoveWindow(&rect);
	}
...
void CDlgToolbar::SetToggleButton(int nSelToolNo, BOOL bSelect)
{
	...
	else
	{
		...
		case CToolOptionInfo::TOOLOPTION::T_DRAG_Y_ZOOM				:nID=60; break;	// Y축확대
			//	case CToolOptionInfo::TOOLOPTION::T_TOOL_SELECTION			:nID=61; break;	// 차트이동
		...
		case CToolOptionInfo::TOOLOPTION::T_TOOL_BALANCE_BASIC		:nID=74; break;	// 일목수치			// 2014.09.24 박성경 : 0010862: [7111] 삼성증권 차트 툴바의 [일목수치] 기능 IBK차트에도 동일하게 추가 요청
		}
	}
	if(nID<0)
		return;
