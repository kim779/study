...
// --> [Edit]  BT.Jung 2006/06/14
#define BCMENU_PAD_XSIZE  0	// 4	
#define BCMENU_PAD_YSIZE  6	// 4	
// <-- [Edit]  BT.Jung 2006/06/14
...
// --> [Edit]  BT.Jung 2006/06/14
BOOL BCMenu::xp_draw_3D_bitmaps=FALSE;//TRUE;
// <-- [Edit]  BT.Jung 2006/06/14
...
int BCMenu::m_iconX = 12;//16;
int BCMenu::m_iconY = 12;//15;
...
#define  SELECT_BACKCOLOR  RGB(77,149,249)		  
#define  SELECT_TEXTCOLOR  RGB(255,255,255)		  
#define  LIGHT_TEXTCOLOR   CLR_FONT_DEFAULT		  
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CRect rect,rect2;
	UINT state = (((BCMenuData*)(lpDIS->itemData))->nFlags);
	COLORREF m_newclrBack=GetSysColor(COLOR_3DFACE);				// 메뉴 좌측 백그라운드
	COLORREF m_clrBack=GetSysColor(COLOR_WINDOW);					// 메뉴 우쯕 백그라운드
	...
	int BCMENU_PAD= BCMENU_PAD_XSIZE;
...
//		COLORREF crSelect = GetSysColor(COLOR_HIGHLIGHT);
		COLORREF crSelect = SELECT_BACKCOLOR;				
...
// --> [Edit]  BT.Jung 2006/06/02
			crText = SELECT_TEXTCOLOR;				
// <-- [Edit]  BT.Jung 2006/06/02
//			pDC->FillRect (rect,&m_brSelect);
			pDC->FillSolidRect (rect, crSelect);
		...
		x0=rect.left;y0=rect.top;
// --> [Edit]  BT.Jung 2006/06/15
		rect.left = rect.left + barwidth + 6;//8; 
// <-- [Edit]  BT.Jung 2006/06/15
...
//		lpMIS->itemHeight = temp>m_iconY+BCMENU_PAD ? temp : m_iconY+BCMENU_PAD;
		lpMIS->itemHeight = temp < m_iconY+BCMENU_PAD_YSIZE ? temp : m_iconY+BCMENU_PAD_YSIZE;
// <-- [Edit]  BT.Jung 2006/06/15
		m_fontMenu.DeleteObject();
	}
}
