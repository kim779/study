...
UINT NEAR WM_XLISTCTRL_EDIT_RETURN = ::RegisterWindowMessage(_T("WM_XLISTCTRL_EDIT_RETURN"));
/////////////////////////////////////////////////////////////////////////////
// CXListCtrl
...
//		m_NewHeaderFont.CreatePointFont(110,"MS Serif");
		...
		lf.lfHeight = 12;
		m_NewHeaderFont.CreateFontIndirect(&lf);
...
void CXListCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	else if (pLVCD->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
	{
		...
		XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) GetItemListCtrlData(nItem);
		//ASSERT(pXLCD);
...
void CXListCtrl::OnPaint()
{
	...
	if( (GetStyle() & LVS_TYPEMASK) == LVS_REPORT )
	{
		...
		if( !GetItemRect( 0, &rectHoriz, LVIR_BOUNDS ))
		{
		...
		int height = rectHoriz.bottom - rectHoriz.top;
		...
		dc.LineTo(rect.left, height*(GetItemCount()+1) -1);
		...
		for( int i = 0; i < nColumnCount; i++ )
		{
			...
			dc.LineTo(borderx-1, height*(GetItemCount()+1) -1);
		}

		// Draw the horizontal grid lines
		dc.MoveTo(0, top + height*GetItemCount());
		dc.LineTo( borderx-1, top + height*GetItemCount() );
						   ...
						   COLORREF crBackground)
{
	...
	if (!pXLCD)
		return -1;
	CString strKey;
	strKey.Format("%d", nItem);

	pXLCD[0].crText       = crText;
	pXLCD[0].crBackground = crBackground;
	pXLCD[0].nImage       = -1;

	m_mapListData.InsertAt(nItem, pXLCD);
	//CListCtrl::SetItemData(index, (DWORD) pXLCD);
...
int CXListCtrl::SetItem(const LVITEM* pItem)
{
	...
	XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) GetItemListCtrlData(pItem->iItem);
	if (pXLCD)
	{
...
//
BOOL CXListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText,
					COLORREF crText, COLORREF crBackground, BOOL bDraw /*=TRUE*/)
{
	ASSERT(nItem >= 0);
//	ASSERT(nItem < GetItemCount());
//	if ((nItem < 0) || nItem >= GetItemCount())
...
//	m_bShowEmptyMessage = FALSE;
	...
	int nSize =m_mapListData.GetSize();
	for(int i=0; i< nSize; i++)
	{
		pXLCD = (XLISTCTRLDATA*)m_mapListData.GetAt(i);
		if(pXLCD)
		{
			delete [] pXLCD;	
		}
...
{
	if(m_NewHeaderFont.GetSafeHandle())
...
///////////////////////////////////////////////////////////////////////////////
// SetCheckbox
BOOL CXListCtrl::SetCheckbox(int nItem, int nSubItem, int nCheckedState, BOOL bDraw /*=TRUE*/)
{
	ASSERT(nItem >= 0);
//	ASSERT(nItem < GetItemCount());
//	if ((nItem < 0) || nItem >= GetItemCount())
...
///////////////////////////////////////////////////////////////////////////////
// SetCurSel
BOOL CXListCtrl::SetCurSel(int nItem, BOOL bSelect /*= TRUE*/)
{
	if(!bSelect)
	{
...
DWORD CXListCtrl::GetItemData(int nItem)
{
	ASSERT(nItem >= 0);
	//ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount() || nItem >= m_mapListData.GetSize())	//Error handling
		return 0;

	XLISTCTRLDATA *pXLCD = NULL;
	pXLCD = (XLISTCTRLDATA*)m_mapListData.GetAt(nItem);
	if (!pXLCD)
	{
		return 0;
	}
...
XLISTCTRLDATA* CXListCtrl::GetItemListCtrlData(int nItem)
{
	ASSERT(nItem >= 0);
//	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= m_mapListData.GetSize())
		return 0;
...
//		return FALSE;
	...
	XLISTCTRLDATA *pXLCD = GetItemListCtrlData(nItem);
...
BOOL CXListCtrl::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
//@유진추가	
	...
	XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) GetItemListCtrlData(pnmlv->iItem);
	if (!pXLCD)
	{
		*pResult = 0;
		return FALSE;
...
LRESULT CXListCtrl::OnEditReturn( WPARAM wParam, LPARAM lParam )
{	
...
void CXListCtrl::SetSortImage(int nCol,int nType)
{
	if(nType==2)
	{
...
//	리스트의 행의 높이를 적용한다.
void CXListCtrl::SetRowHeight(short nHeight)
{
