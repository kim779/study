...
//                    allow the user to switch between the three aligment 
...
LRESULT CTextProgressCtrl::OnGetText(UINT wParam, LPARAM lParam)
{
...
void CTextProgressCtrl::OnPaint() 
{
        ...
        )
    {
		...
		// -------------------------------------------------------
        // m_dwTextStyle = DT_CENTER | DT_VCENTER | DT_SINGLELINE;
		// -------------------------------------------------------
		// Removed by Kriz, 2004 - We don't need this line anymore
		// when using the AlignText() method
        ...
        dc.DrawText(str, ClientRect, m_dwTextStyle);
...
LRESULT CTextProgressCtrl::OnSetRange32(WPARAM Lower, LPARAM Upper)
{
    ...
    // PBM_SETRANGE32 returns a DWORD value that holds the previous 16-bit 
    // low limit in its low word and the previous 16-bit high limit in its 
    // high word. If the previous ranges were 32-bit values, the return value 
    // consists of the low words of both 32-bit limits. 
    LRESULT result = (DWORD) (LOWORD(m_nMax) << 16) & DWORD(LOWORD(m_nMin));
    return result;
}
...
/* Added by Kriz, 2004
   ---------------------------------------------------------------------
   This method sets the text alignment WITHOUT invalidating the control!
   If you need invalidation just call the AlignTextInvalidate() method.
   ---------------------------------------------------------------------
   The method retrieves a DWORD parameter which is equal to this flags:

   DT_LEFT   - Left aligned..........
   DT_CENTER - ...Centered aligned...
   DT_RIGHT  - .........Right aligned

   Example: AlignText(DT_RIGHT);

   The method returns TRUE if everything went right, otherwise it returns
   FALSE (when you've entered anything else than the three flags above)
   and nothing changes at all.
*/
BOOL CTextProgressCtrl::AlignText(DWORD aligment)
{
	switch(aligment)
	{
	case DT_LEFT: 
	case DT_CENTER: 
	case DT_RIGHT:
		m_dwTextStyle = DT_VCENTER | DT_SINGLELINE | aligment;
		break;
	default:
		return FALSE;
	}
...
/* Added by Kriz, 2004
   ---------------------------------------------------------------------
   This method sets the text alignment AND invalidates the control!
   If you don't need invalidation just call the AlignText() method.
   ---------------------------------------------------------------------
   The method retrieves a DWORD parameter which is equal to this flags:
   ...
   Example: AlignTextInvalidate(DT_RIGHT);

   The method returns TRUE if everything went right, otherwise it returns
   FALSE (when you've entered anything else than the three flags above)
   and nothing changes at all.
*/
BOOL CTextProgressCtrl::AlignTextInvalidate(DWORD aligment)
{
	switch(aligment)
	{
	case DT_LEFT: 
