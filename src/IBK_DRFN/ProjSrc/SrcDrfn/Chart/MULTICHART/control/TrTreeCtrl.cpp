...
void CTrTreeCtrl::ClickDefault() 
{
	HTREEITEM hSelectItem = GetRootItem();
	if(hSelectItem == NULL)
		return;
	
	HTREEITEM hChildItem = NULL;
	if(ItemHasChildren(hSelectItem))
	{
		hChildItem = GetChildItem(hSelectItem);
		while (hChildItem != NULL && ItemHasChildren(hChildItem))
		{
			hSelectItem = hChildItem;
			hChildItem = GetChildItem(hSelectItem);
		}
		
		if(hChildItem != NULL && ItemHasChildren(hChildItem) == FALSE)
			hSelectItem = hChildItem;
	}
	
	DWORD dwSelectItemData = GetItemData(hSelectItem);
	if(dwSelectItemData)
	{
		//<<솔로몬. 저장화면들 동시에 로드하다가 죽는 현상 발생하여 수정
		//GetParent()->GetParent()->SendMessage(UDM_TREE_CLICK, 0, (LPARAM)dwSelectItemData);
		CWnd* pWnd = GetParent();
		if( pWnd && ::IsWindow(pWnd->GetSafeHwnd()) )
		{
			pWnd = pWnd->GetParent();
			if( pWnd && ::IsWindow(pWnd->GetSafeHwnd()) )
				pWnd->SendMessage(UDM_TREE_CLICK, 0, (LPARAM)dwSelectItemData);
		}
		//>>
	}
...
int CTrTreeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
