...
LRESULT CTabIndices::OnRmsgWMSize(WPARAM wp, LPARAM lp)
{
	RecalcLayout();
	return 0L;
}
...
void CTabIndices::SetConfig(BOOL bLoad /*= TRUE*/)
{
	...
	if(bLoad)
	{
		::GetPrivateProfileString("TabIndices","HeightOfIndices","80",szTemp,sizeof(szTemp),strCfgFile);
		m_nHeightOfIndices = atoi(szTemp);
		::GetPrivateProfileString("TabIndices","IndexSelected","0",szTemp,sizeof(szTemp),strCfgFile);
		m_nIndexSelected = atoi(szTemp);
	}
	else
	{
		strTemp.Format("%d",m_nHeightOfIndices);
		::WritePrivateProfileString("TabIndices","HeightOfIndices",strTemp,strCfgFile);
		strTemp.Format("%d",m_nIndexSelected);
		::WritePrivateProfileString("TabIndices","IndexSelected",strTemp,strCfgFile);
	}
}
...
//		rect.top = rect.top + GAP_TABDLG;
	...
	case 10102 : 
		...
		if(m_bShowJipyoItem)			
			rect.bottom = rect.top + m_nHeightOfIndices - 2;			
		else
			rect.bottom = rect.bottom - HEIGHTOFBUTTON - GAP_TABDLG - 2;
		break;
	case IDC_TREEITEMS:
		rect.top = rect.top + GAP_TABDLG;
		if(m_bShowJipyoItem)			
	...
	case IDC_STATIC_TITLE2:
		rect.top = rect.bottom - HEIGHTOFBUTTON + 2 - 2;
...
//	m_listFind.AddString(strResult[i]);
	...
	{
		nPos = strResult[i].Find(":");
		str = strResult[i].Left(nPos);
		m_listFind.AddString(strResult[i].Mid(nPos + 1));
		m_listFind.SetItemData(i, atol(str));
	}
//<< [QC No]090728HMC039
}
...
//>> [QC No]090728HMC039
				case TFI_JIPYOITEM_ID:			// 선택된 데이터를 Select시킴
					nPos = strFindData.Find(":");
					str = strFindData.Mid(nPos + 1);
					if(strTemp.Find(str) != -1)
					{
						str = strFindData.Left(nPos);
						if(hItem == (HTREEITEM)atol(str))
						{
				...
				case TFI_JIPYOITEM_TITLE:		// 해당데이터를 찾는다
					...
					{
						str.Format("%d:%s", hItem, strIndicator);
						strResult.Add(str);
					} 
					break;
//<< [QC No]090728HMC039
...
//	GetFindItemList(TFI_JIPYOITEM_ID, strData, strResult);
	CString str;
	str.Format("%d:%s", m_listFind.GetItemData(nSel), strData);
	CStringArray strResult;
	GetFindItemList(TFI_JIPYOITEM_ID, str, strResult);
//<< [QC No]090728HMC039
}

//@Solomon	091202SM110	-->
void CTabIndices::OnChangeEditFind()
{
	OnBnClickedBtnFind();
}
//@Solomon	091202SM110	<--
