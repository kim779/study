...
protected:
	...
	void	_DoReplyResourceCheck();	//복기 Resource체크.
	void	Init_Size();
...
public:
	...
	//@Sol-100104SM005 통합버전에서 실시간처리시마다 ZoomValue를 보내고 있어서 수정하기 위해 추가함.
	CString m_szOldZoomValue;

	BOOL GetOldFututureCodeName(LPCSTR szCode, CString& szCodeName);
	BOOL m_bInitChartData;	//ZoomValue가 조회에 의한 셋팅인지를 확인하기위해 추가.

	int GetComboMarket(); //콤보에 셋팅된 market Type.
