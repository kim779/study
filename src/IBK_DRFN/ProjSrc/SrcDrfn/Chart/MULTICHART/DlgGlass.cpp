...
void CDlgGlass::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bClicked = false;
	ReleaseCapture();
	if(m_DrawID>=0) KillTimer(m_DrawID);m_DrawID=-1;
...
void CDlgGlass::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_DrawID<0)
		m_DrawID = SetTimer(ID_DRAWTID, 10, NULL);
...
//	if(m_bClicked == false)
//		return;
	//if( nFlags & MK_LBUTTON) DoMouseMove(point)
	
	CDialog::OnMouseMove(nFlags, point);
}

///////////////////////////////////////////////////////////////////////////////
//	작업자	:	배승원(Ssyng-Won, Bae)
...
void CDlgGlass::OnTimer(UINT nIDEvent) 
{
	if(nIDEvent==ID_DRAWTID)	
	{
		CPoint point;
		GetCursorPos(&point);
...
void CDlgGlass::DoMouseMove(CPoint point)
{
