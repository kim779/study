...
#include "../CommonTR/KXH1.h"
#include "../CommonTR/KXYS.h"
...
//>> [QC No]090803HMC075
	CString		m_szkshcode;	
//	CString		m_szpreychange;	
	CString		m_szjnilychange;
	CString		m_szyeprice;	
	CString		m_szyevolume;	
//	CString		m_szpreysign;	
	CString		m_szjnilysign;
	CString		m_szjnilydrate;
//<< [QC No]090803HMC075
...
public:
	...
	virtual void  Convert(LPKXYS_OutBlock pInput)
	{
		CString sTemp1 ;
		char sTmp[20+1];

//>> [QC No]090803HMC075
		DATA_MEMCOPY(sTmp, pInput, kshcode);	
//		DATA_MEMCOPY(sTmp, pInput, preychange);	
		DATA_MEMCOPY(sTmp, pInput, jnilychange);	
		DATA_MEMCOPY(sTmp, pInput, yeprice);	
		DATA_MEMCOPY(sTmp, pInput, yevolume);	
//		DATA_MEMCOPY(sTmp, pInput, preysign);	
		DATA_MEMCOPY(sTmp, pInput, jnilysign);	
		DATA_MEMCOPY(sTmp, pInput, jnilydrate);	
//<< [QC No]090803HMC075
	}
};
...
//	void			SetRealHogaData(LPCSTR pRealStruct);
	void			SetRealHogaData(CKBREAL_KXH1 &kbReal_KXH1);
	//void			SetRealHogaData(DWORD dwKey, CString strCode, CMapStringToString *pMapRealData);
	void			SetRealHogaData(DWORD dwMethodKey, CString strCode, LONG _pMapRealData);
	void			SetRealYeShangData(LPCSTR pRealStruct);
...
public:
	...
	void RemoveExtra(CKBREAL_KXH1 &kbReal_KXH1);	//@SM-100104SM001
};
