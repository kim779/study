...
//======================================================================================
	...
	#include "../CompareChart/StdDialog.h"
#endif
...
// (2005/8/16 - Ssyng-Won, Bae) Updated for Hanwha Platform
...
#ifdef	_MULTICHART	
		return ((CStdDialog*	)(pThis->GetParent()->GetParent()))->GetMainPath();
#endif
...
//-----------------------------------------------------------------------------
// Author		: Ssyng-Won, Bae	Date :2005/7/31
// Modifier		: 
...
*/
// (2005/8/10 - Ssyng-Won, Bae) Shared Data Manager Interface
...
//}
//
//// (2005/8/12 - Ssyng-Won, Bae) Real Table Manager Interface
...
//-----------------------------------------------------------------------------
// Author		: Ssyng-Won, Bae	Date :2005/8/10
// Modifier		: 
// Comments		: Advise Shared Data Routine for SkcChart OCX
//-----------------------------------------------------------------------------
BOOL CChartItem::AdviseSharedData( const char *p_szShareName, int p_nShareType, int p_nShareArea)
{
//	if( !m_pDrMLib_DBMgr || !p_szShareName || !*p_szShareName) return FALSE;
//	return m_pDrMLib_DBMgr->AdviseSharedInfo( p_szShareName, this, p_nShareType, p_nShareArea,0);
	return FALSE;
}
//
////-----------------------------------------------------------------------------
//// Author		: Ssyng-Won, Bae	Date :2005/8/10
//// Modifier		: 
//// Comments		: Unadvise Shared Data Routine for SkcChart OCX
////-----------------------------------------------------------------------------
BOOL CChartItem::UnadviseSharedData( const char *p_szShareName)
{
//	if( !m_pDrMLib_DBMgr || !p_szShareName || !*p_szShareName) return FALSE;
//	return m_pDrMLib_DBMgr->UnAdviseSharedInfo( p_szShareName, this);
	return FALSE;
}
...
// Author		: Ssyng-Won, Bae	Date :2005/8/10
// Modifier		: 
// Comments		: Get Shared Data Routine for SkcChart OCX
//-----------------------------------------------------------------------------
...
//
//	// m_pParentWnd ==>
//	if( !m_pDrMLib_DBMgr /*|| !m_pParentWnd*/ || !p_szShareName || !*p_szShareName) return FALSE;
//	if( /*!m_pParentWnd->SendMessage( WMU_GET_SHARE_STATE,0,0) || */m_bSharedOn) return FALSE;
//
//	// 1. Get Shared Data and Check Code Filter
//	int nLen, nStock;
//	char *pData;
//	pData = ( char *)m_pDrMLib_DBMgr->GetSharedData( p_szShareName, nLen, nStock);
...
// Author		: Ssyng-Won, Bae	Date :2005/8/10
// Modifier		: 
// Comments		: Notify Shared Data Routine to SkcChart OCX
//-----------------------------------------------------------------------------
long CChartItem::ChangeSharedData( CString sSharedName, BYTE* pData, int nLen, int nStock)
{
//	// 0. Check Base Info
//	if( !m_pDrMLib_DBMgr /*|| !m_pParentWnd*/) return FALSE;
...
// Author		: Ssyng-Won, Bae	Date :2005/8/10
// Modifier		: 
// Comments		: Fire Shared Data Routine from SkcChart OCX
//-----------------------------------------------------------------------------
...
//
//
//	GetParent()->SendMessage(UMSG_FIREGSHAREDATA,(WPARAM)p_szShareName,(LPARAM)p_szSharedData);
//
//	// 1. Fire Shared Data
//	return m_pDrMLib_DBMgr->SetSharedData( GetSafeHwnd(), p_szShareName, ( unsigned char *)p_szSharedData, strlen( p_szSharedData), p_lFilterCode);
	return FALSE;
}
...
//-----------------------------------------------------------------------------
// Author		: Ssyng-Won, Bae	Date :2005/8/12
// Modifier		: 
...
//-----------------------------------------------------------------------------
// Author		: Ssyng-Won, Bae	Date :2005/8/13
// Modifier		: 
...
/*	// 0. Check NULL Pointers
	if( !m_pDrMLib_DBMgr || !p_szRecordName || !p_szItemName) return FALSE;

	// 1. Get Real Table Info
	CMapWordToPtr *pRealTable = m_pDrMLib_DBMgr->GetRealTable();
...
//-----------------------------------------------------------------------------
// Author		: Ssyng-Won, Bae	Date :2005/8/16
// Modifier		: 
...
{
//	// 0. Check NULL Pointers
//	if( !m_pDrMLib_DBMgr || !p_szRealKeyCode || !*p_szRealKeyCode) return FALSE;
//
//	CString strRealKey;
//	strRealKey.Format("%d,%s", p_wRealRecordID, p_szRealKeyCode);
//	m_pDrMLib_DBMgr->AddRealUpdate( strRealKey, GetSafeHwnd());
...
//	CString strRealKey;
//	strRealKey.Format("%d,%s", p_wRealRecordID, p_szRealKeyCode);
//	m_pDrMLib_DBMgr->ResetRealUpdate( strRealKey, GetSafeHwnd());
...
BOOL CChartItem::GetData( long p_lRealDataAddress, WORD p_wRealItemID, double &p_dData)
{
	...
	BOOL bResult = ( m_fnGetData)( p_lRealDataAddress, p_wRealItemID, p_dData);
//	if(p_wRealItemID == 2) //time
//	{
//		TRACE( "%f\r\n", p_dData);
//	}
	return bResult;
