...
void CTabSystemTradingEx::OnBtnStanal()
{
	...
	{
		AfxMessageBox("분할/중첩일 경우에는 보고서 버튼을 이용해주세요.");
...
CString CTabSystemTradingEx::CSTAnalysisInput::GetCodeName()
{
	...
	IMasterDataManager* pMasterMng = (IMasterDataManager*)AfxGetPctrInterface(UUID_IMasterDataManager);
	CString szCodeName;
...
int CTabSystemTradingEx::CSTAnalysisInput::GetCount()
{
	...
	int nTmp = ((CChartMng*)pThis->m_pChartMng)->GetpInputBarDlg()->GetDlgItemInt(IDC_EDITZOOMVALUE);
...
/*
	...
	envHelper.LoadInfo(szRunFile, szRunFile, STConfigEnv_Jumun);
	if (envHelper.m_InfoJumun.m_nJMethod > 0)
	{
		// Send2Server 호가.
		// Send2SErver 리얼데이터.
	}
...
void CTabSystemTradingEx::DataMonitorCallbackProc(LPCSTR szKey, long pThisPoint, long dwKey, long dwSubKey, long dwNotifyData)
{
	...
	//		// 현재보고 있는 멀티차트에만 적용한다.
	//		long pMainDlg = (long)pThis->m_pStdDialog;
...
//	InData.szSTName  = m_xSTAnalysisInput.m_Info.szSTName;//KHD :2006.11.16
	...
	InData.szSTName = strSTName;
	{
		...
		int nFind = InData.szSTName.Find(gSTGubunKeys);	//090902-solomon @->^
...
BOOL CTabSystemTradingEx::CheckRegister(LPCSTR p_szProgID, LPCSTR p_szFileName)
{
	...
	if(pMng)
	{
		...
		CString szOcxName;
		if(strcmp(p_szFileName, "TeeChart7.ocx")==0)
			szOcxName.Format("%s\\exe\\%s", m_pParentDlg->m_strRootPath, p_szFileName);
		else
	...
	else if(p_nSTRunType==STRun_Type_BullBear)
	{
		...
		else
		{
			...
			// MakeInput
			{
				...
				// SFG경로, 내부적으로 ^기준으로 잘라야 함.
				szTmp.Format("SFGPATH=%s\\%s", strSDPath, p_pArg->szSTName);	//090902-solomon @->^
...
LONG CTabSystemTradingEx::OnRmsgIndexAddinNotifyBuySell(WPARAM wp, LPARAM lp)
{
	...
	// HTS만 주문가능 0(HTS), 1(FO)
	if (m_nUserType)
		return 1L;

/*
	// 계좌체크.
	CString szAcctCode, szAcctType, szAcctSerial;
	BOOL bFound = FALSE;
	int nMarket = lpOrderData->nMarket;
	szAcctCode = lpOrderData->aAcctNo;

	ICompanyManager_SS* pMng = (ICompanyManager_SS*)AfxGetPctrInterface(UUID_ICompanyManager);
	if(pMng)
	{
		szAcctType = szAcctCode.Mid(3, 2);

		if(szAcctType.Compare("22")==0 && nMarket!=1)
		{
			AfxMessageBox("전략에 설정된 계좌는 선물옵션계좌입니다. 주문가능한 계좌를 확인바랍니다.");
			return 0L;
		}
		if(szAcctType.Compare("01")==0 && nMarket!=0)
		{
			AfxMessageBox("전략에 설정된 계좌는 선물옵션계좌가 아닙니다. 주문가능한 계좌를 확인바랍니다.");
			return 0L;
		}
	}
*/
	CSTOrderRData_EG inData;
	inData.m_nMarket = lpOrderData->nMarket;				// 0:주식, 1:선물/옵션, 2:업종
	inData.m_nOrderType = lpConfigEnv->m_nOrderType;		// 0:시스템트레이딩 매수진입, 1:매수정산, 2:매도청산, 3:매도진입
	inData.m_szGeja = lpOrderData->aAcctNo;
	inData.m_szGejaname = lpOrderData->aAcctName;
	inData.m_szCode = lpConfigEnv->m_szCode;				// 종목코드
	inData.m_szCodeName = lpConfigEnv->m_szCodeName;		// 종목코드명
	inData.m_szSTPath = lpConfigEnv->m_szSTFileName;		// 전략의 경로
															// 이 부분은 수정되어야 할 듯. 전략실행시 정보를 알아야한다.
															// 실행이후 전략파일이 변경될 수 있다.
	inData.m_nQty = lpConfigEnv->m_InfoJumun.m_nJAmount;
	inData.m_nAutoOrder = lpConfigEnv->m_InfoJumun.m_nJMethod;		// 주문방법.

	inData.m_bSound_Buy_Use = lpConfigEnv->m_InfoJumun.m_bSound_Buy_Use;
	inData.m_szSound_Buy_File = lpConfigEnv->m_InfoJumun.m_szSound_Buy_File;
	inData.m_bSound_Sell_Use = lpConfigEnv->m_InfoJumun.m_bSound_Sell_Use;
	inData.m_szSound_Sell_File = lpConfigEnv->m_InfoJumun.m_szSound_Sell_File;

	if(inData.m_nOrderType & FORCEDEXIT)
	{
		inData.m_nRealOrderType = FORCEDEXIT;
		inData.m_nOrderMarketClear = lpConfigEnv->m_InfoJumun.m_nMkprc_Clr;
		inData.m_nOrderPriceType = lpConfigEnv->m_InfoJumun.m_nSellPrice;
		CallJunmunDlg(&inData);
	}
	if(inData.m_nOrderType & VALENTRYBUY)
	{
		inData.m_nRealOrderType = VALENTRYBUY;
		inData.m_nOrderMarketEntry = lpConfigEnv->m_InfoJumun.m_nMkprc_Ent;
		inData.m_nOrderPriceType = lpConfigEnv->m_InfoJumun.m_nBuyPrice;
		CallJunmunDlg(&inData);
	}
	if(inData.m_nOrderType & VALEXITSHORT)
	{
		inData.m_nRealOrderType = VALEXITSHORT;
		inData.m_nOrderMarketClear = lpConfigEnv->m_InfoJumun.m_nMkprc_Clr;
		inData.m_nOrderPriceType = lpConfigEnv->m_InfoJumun.m_nBuyPrice;
		CallJunmunDlg(&inData);
	}
	if(inData.m_nOrderType & VALENTRYSELL)
	{
		inData.m_nRealOrderType = VALENTRYSELL;
		inData.m_nOrderMarketEntry = lpConfigEnv->m_InfoJumun.m_nMkprc_Ent;
		inData.m_nOrderPriceType = lpConfigEnv->m_InfoJumun.m_nSellPrice;
		CallJunmunDlg(&inData);
	}
	if(inData.m_nOrderType & VALEXITLONG)
	{
		inData.m_nRealOrderType = VALEXITLONG;
		inData.m_nOrderMarketClear = lpConfigEnv->m_InfoJumun.m_nMkprc_Clr;
		inData.m_nOrderPriceType = lpConfigEnv->m_InfoJumun.m_nSellPrice;
		CallJunmunDlg(&inData);
	}
...
LONG CTabSystemTradingEx::OnRmsgIndexAddinNotifySignal(WPARAM wp, LPARAM lp)
{
	...
	else if(gCURRENT_COMPANY_ID==nPCTRComID_KB)
	{
		PlayOrderSound(	pEnv->m_nOrderType,
						pEnv->m_InfoJumun.m_bSound_Buy_Use,
						pEnv->m_InfoJumun.m_szSound_Buy_File,
...
void CTabSystemTradingEx::CallJunmunDlg(CSTOrderRData_EG *pInData)
{
	...
	HWND hConfirmWnd = pConfirmWnd->GetSafeHwnd();
	if (hConfirmWnd == NULL)
		return;

	CString strParam;
	CString strScrNo;
	CString strItemCode = pConfirmWnd->m_InData.m_szCode;
	int nRealMarketType;
	int nEnumMarketType = ((CStdDialog*)m_pMultiChartWnd)->GetMarketTypeFromCode(strItemCode, nRealMarketType);

	FID_SENDDATA *pstSendData = NULL;
	//	주식, 선옵일때만
	if ((nRealMarketType == CDRCodeTypeInfo::CODE_KOSPI_STOCK) ||
		(nRealMarketType == CDRCodeTypeInfo::CODE_KOSDAQ_STOCK))
	{
		pstSendData = new FID_SENDDATA;

		pstSendData->szTrCode = QUERY_STOCK_HOGA;
		int nSize = sizeof(k01230In1);
		char *pData = new char[nSize+1];
		memset(pData, 0x00, nSize+1);
		k01230In1 *pIn = (k01230In1 *)pData;
		memcpy(pIn->shcode, (LPTSTR)(LPCTSTR)strItemCode, min(sizeof(pIn->shcode), strItemCode.GetLength()));
		pstSendData->pData = pData;
		pstSendData->nLen = nSize;
	}
	else if ((nRealMarketType == CDRCodeTypeInfo::CODE_KFUTURE) ||
			(nRealMarketType == CDRCodeTypeInfo::CODE_KOPTION))
	{
		pstSendData = new FID_SENDDATA;

		pstSendData->szTrCode = QUERY_FUTOPT_HOGA;
		int nSize = sizeof(KB_f0201_InRec1);
		char *pData = new char[nSize+1];
		memset(pData, 0x00, nSize+1);
		KB_f0201_InRec1 *pIn = (KB_f0201_InRec1 *)pData;
		memcpy(pIn->futcode, (LPTSTR)(LPCTSTR)strItemCode, min(sizeof(pIn->futcode), strItemCode.GetLength()));
		pstSendData->pData = pData;
		pstSendData->nLen = nSize;
	}
	else
	...
	pstSendData->hWnd = hConfirmWnd;

	IPartnerManager* pPTManager = ((CStdDialog*)m_pMultiChartWnd)->m_pPTManager;
	pPTManager->directSendFID((long)pstSendData);

	if (pstSendData)
	{
		if (pstSendData->pData)
		{
			delete []pstSendData->pData;
			pstSendData->pData = NULL;
		}
...
HWND CTabSystemTradingEx::CSTDlgConfirmSendJumun::GetHwndScreenView()
{
	...
	return ((CStdDialog*)pThis->m_pMultiChartWnd)->m_hViewInMain;
}
