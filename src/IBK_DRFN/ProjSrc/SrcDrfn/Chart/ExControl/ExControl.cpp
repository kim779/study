// ExControl.cpp : 해당 DLL의 초기화 루틴을 정의합니다.
//
...
static AFX_EXTENSION_MODULE ExControlDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ExControl.DLL 초기화!\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(ExControlDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ExControlDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ExControl.DLL 종료!\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(ExControlDLL);
	}
	return 1;   // 확인
}
