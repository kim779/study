...
//
// CGridDropTarget is an OLE drop target for CExGridCtrl. All it does
...
//
// For use with CExGridCtrl v2.10+
//
/////////////////////////////////////////////////////////////////////////////
...
BOOL CGridDropTarget::Register(CExGridCtrl *pGridCtrl)
{
    if (m_bRegistered)
        return FALSE;
    ...
    ASSERT(pGridCtrl->IsKindOf(RUNTIME_CLASS(CExGridCtrl)));
    ASSERT(pGridCtrl);

    if (!pGridCtrl || !pGridCtrl->IsKindOf(RUNTIME_CLASS(CExGridCtrl)))
    {
        bInProcedure = FALSE;
        return FALSE;
    }
