...
{	
	...
	int nSysEdu = -1;
	//<<20100308_JS.Kim 솔로몬
	//ICompanyManager_SS* pMng = (ICompanyManager_SS*)AfxGetPctrInterface(UUID_ICompanyManager, 2);
	...
	else if(m_nMarketType == FX_CHART || m_nMarketType == FUTOPT_CHART || m_nMarketType == UPJONG_CHART)
	{
...
void CXTabCtrl::ShowTab(int nIndex)
{
	...
	else
	{
		...
		if(m_nMarketType == UPJONG_CHART&&nIndex==TABNO_CODESEARCH)
			m_pTabDlgFocused = m_tabDlgCreator.CreateTabDlg(TABNO_CODESEARCHUP);
		else if(m_nMarketType == FUTOPT_CHART&&nIndex==TABNO_CODESEARCH)
			m_pTabDlgFocused = m_tabDlgCreator.CreateTabDlg(TABNO_CODESEARCHFO);
		else if(m_nMarketType == FOREIGN_CHART&&nIndex==TABNO_CODESEARCH)
...
/*
	...
	if(m_pTabDlgCodeSearch)		m_pTabDlgCodeSearch->ChangeSkinColor(clrBkgrnd);
	if(m_pTabIndices)			m_pTabIndices->ChangeSkinColor(clrBkgrnd);
	if(m_pTabDlgSystemTabEx)	m_pTabDlgSystemTabEx->ChangeSkinColor(clrBkgrnd);
	...
	if(pKSLibMng)
	{
		...
		clrTabLine = pKSLibMng->Lib_KSLink_GetColor(99);		// 탭 라인 
	}
	SetTabColor(clrBkgrnd, clrTabActive, clrTabInactive, clrTabLine);
*/
	STTabColor tabColor;
