...
BOOL CTabSystemTradingEx::OnInitDialog() 
{
	...
	//<<20100308_JS.Kim 솔로몬
	//IMainInfoManager* pMng = (IMainInfoManager*)AfxGetPctrInterface(UUID_IMainInfoManager, 2);
	...
	if(pMng)
	{
		...
		m_szEncUserDir = pMng->GetEncUserDir();
		m_nUserType = atoi(pMng->GetLoginInfo("ISHTS")); //"1":HTS  "0":FRONT(지점)
	}
...
void CTabSystemTradingEx::RunWizard(BOOL bSelectedDefault)
{
	...
	CDllDlg dlg(szDllName, "전략작성(Wizard)", &m_xSTRunManager, hOld);
...
//#endif
	...
	CDllDlg dlg(szDllName, stTitle, &m_xSTInterface, hOld);
...
void CTabSystemTradingEx::LoadInfo()
{
	...
	//<<20100308_JS.Kim 솔로몬
	//m_pIAUDataMonitorManager = (IAUDataMonitorManager*)AfxGetPctrInterface(nID, 2);
...
CWnd *CTabSystemTradingEx::LoadTradeAup(CString strFile, CWnd *pParent)
{
	...
	CreateTradingAupEx *pCreateTrading = (CreateTradingAupEx *)GetProcAddress(hTradeInst,_T("CREATE_TRADING_FX"));
...
CString CTabSystemTradingEx::CSTAnalysisInput::GetCodeName()
{
	...
	//<<20100308_JS.Kim 솔로몬
	//IMasterDataManager* pMasterMng = (IMasterDataManager*)AfxGetPctrInterface(UUID_IMasterDataManager, 2);
...
BOOL CTabSystemTradingEx::CSTAnalysisInput::GetSTInfo(STMNG_ITEMINFO* pArg)
{
	...
	pArg->nSTType	= m_Info.nSTType	;
	pArg->szSTName	= m_Info.szSTName;
	pArg->szSTPath	= m_Info.szSTPath;
	pArg->szSTRData = m_Info.szSTRData;
													...
													LONG& dwDataVolume, LONG& dwDataExt)
{
	...
	CChartItem* pCItem = ((CChartMng*)pThis->m_pChartMng)->GetpChartItemSelected();
	//<<20100308_JS.Kim 솔로몬
	//IMainExInfoManager2* pMng = (IMainExInfoManager2*)AfxGetPctrInterface(UUID_IMainExInfoManager2, 2);
...
int CTabSystemTradingEx::CSTRunManager::RunST(BOOL bImsiRun, CString &szRunFile, LONG _pInData)
{
	...
	MYMETHOD_PROLOGUE(CTabSystemTradingEx, STRunManager)
		...
		if(pThis->m_pDlgOption) 
		{
			...
			{
				STMNG_ITEMINFO_EX1* pInDataEx = (STMNG_ITEMINFO_EX1*)pInData;
				CTreeCtrl* pTreeCtrl;
				if(pInDataEx->nSTLevel==STLevel_Basic)		pTreeCtrl = &pThis->m_STBasicTree;	// 초급전략이면
				else	pTreeCtrl = &pThis->m_STTree;

				HTREEITEM hFolder = CAZUtilCtrl::SearchFolderItem(pTreeCtrl, pInData->szSTRData);
				if(hFolder)
				{
					szSTName = pInData->szSTName;
					// 실제 전략이름은 @앞까지
					int nFind = szSTName.Find("@");
					if(nFind>0) szSTName = szSTName.Left(nFind);
					HTREEITEM hItem = CAZUtilCtrl::SearchItem(pTreeCtrl, hFolder, szSTName, TRUE);
					if(hItem)
					{
					...
					// 최종 선택된 전략 정보 셋팅
					pThis->m_xSTAnalysisInput.m_Info.nSTType		= pInDataEx->nSTType;
					pThis->m_xSTAnalysisInput.m_Info.szSTName		= pInDataEx->szSTName;
					pThis->m_xSTAnalysisInput.m_Info.szSTPath		= pInDataEx->szSTPath;
					pThis->m_xSTAnalysisInput.m_Info.szSTRData		= pInDataEx->szSTRData;	
					pThis->m_xSTAnalysisInput.m_Info.nFolderIndx	= pInDataEx->nFolderIndx;
					pThis->m_xSTAnalysisInput.m_Info.nItemIndx		= pInDataEx->nItemIndx;
...
//	rc2.top += 3;
	...
	CString szImageChartDir = szImageDir + "\\Chart";
...
//	CString szUserDir = (LPCSTR)AfxGetPctrData(17);	// 17 : User Directory
	...
	CString szSTData_STPath = m_szDataDir + "\\" + STFOLDER_ST;
	CString szSTUser_STPath = m_szEncUserDir + "\\" + STFOLDER_ST;
...

	{
		...
		if(m_pSTLoadMnger)
		{
			...
			int p_LoadOption = 0;
			//<<20100308_JS.Kim 솔로몬
			//m_pSTLoadMnger->LoadST2(m_pWndOutBar->GetCoutBarCtrl(), &m_arSTPath, 0, 0, p_LoadOption, 2);
...
BOOL CTabSystemTradingEx::CheckRegister(LPCSTR p_szProgID, LPCSTR p_szFileName)
{
	CLSID clsid;
	//<<20100308_JS.Kim 솔로몬
	//ISystemUtilManager2* pMng = (ISystemUtilManager2*)AfxGetPctrInterface(UUID_ISystemUtilManager, 2);
...
//		strSDPath = m_pParentDlg->m_strRootPath  +  "\\" + STSD_D;
//	else
//		strSDPath = m_szEncUserDir + "\\STData\\SDD";
//#else
//	if(p_pArg->nSTType==0)	// 제공전략(0), 사용자 전략(1)
//		strSDPath = m_pParentDlg->m_strRootPath + "\\" + STSD;
//	else
...
LONG CTabSystemTradingEx::OnRmsgIndexAddinNotifyBuySell(WPARAM wp, LPARAM lp)
{
	...
	if(gCURRENT_COMPANY_ID==nPCTRComID_EUGENE)
	{
		...
		{
			case 0 : strOrdertype = "1";		break;
			case 1 : strOrdertype = "1";		break;
			case 2 : strOrdertype = "0";		break;
			case 3 : strOrdertype = "0";		break;
		}
		...
		if (strOrdertype == "0")
		{
			strOut.Format ("계좌번호 : [%s] \n종목코드 : [%s] \n주문유형 : [시장가] \n주문구분 : [%s]\n시스템트레이딩 주문은 \n추후에 반영될 예정입니다.",
						  strAccount.Mid(8), strCode, "매도");
		}
		else
		{
			strOut.Format ("계좌번호 : [%s] \n종목코드 : [%s] \n주문유형 : [시장가] \n주문구분 : [%s]\n시스템트레이딩 주문은 \n추후에 반영될 예정입니다.",
						  strAccount.Mid(8), strCode, "매수");
		}
		...
		//<<20100308_JS.Kim 솔로몬
		/*
		//<<< 20091008 by kmlee 현재 메시지박스로만 출력을 하는데..
		// 향후 미니주문으로 연결해야 함...
		//>>>
		 CStdDialog	*pMainDlg = (CStdDialog *)this->m_pMultiChartWnd;
		 pMainDlg->DoOrderFromSTSignal (wp, lp);
		 */
		//>>
	}
...
//	if(m_bSignCheckLogin==false) return FALSE;
	...
	//3. 복기모드인지 체크
	{
		...
		if(pInputBarDlg->m_nMarketType != FX_CHART
			&& pInputBarDlg->m_nMarketType != FUTOPT_CHART)
			return FALSE;
