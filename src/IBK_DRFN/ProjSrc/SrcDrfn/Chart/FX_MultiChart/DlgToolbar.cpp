...
#define Y_OFFSET	3		// Top margin from top dlg's border to button's top face
#define Y_OFFSET2	5		// Top margin from top dlg's border to button's top face
...
//<<20100315 솔로몬
//	//# Data Folder
//	strPath = pMainDlg->m_strRootPath +  "\\data";
...
//	strUserPath.Format("%s\\%s", strPath, "FX_ChartToolBarSetting.dat");	
	IGateManager*	pGateManager;
	IMainInfoManager* pMainInfoManager;

	AfxGetIGateManager(pGateManager);
	if( pGateManager )
		pMainInfoManager = (IMainInfoManager*)pGateManager->GetInterface(UUID_IMainInfoManager);
	if( !pMainInfoManager )		return;

	//# Data Folder
	strPath = pMainInfoManager->GetDataDir();
	strDataPath.Format("%s\\%s", strPath, "FX_ChartToolBarSetting.dat");
	//# User Folder
	strPath = pMainDlg->m_strUserDir;
	strUserPath.Format("%s\\%s", strPath, "FX_ChartToolBarSetting.dat");	
//>>
...
//	rcDraw.bottom = Y_OFFSET - 7;
	...
	rcBtns.bottom = rcClient.bottom - Y_OFFSET-1;
...
BOOL CDlgToolbar::DrawLowMenu(int nLowMenuCount, int nHighMenuCount, int nSelTool)
{
	...
	for(i = 0; i < nLoop ; i++)
	{
		...
		else if ( i < nLoop)
		{
			...
			rect.left = X_OFFSET;
			rect.top = toolrect.bottom - TOOL_BUTTON_HEIGHT - Y_OFFSET-1;
			rect.right = rect.left + TOOL_BUTTON_WIDTH;
			rect.bottom = toolrect.bottom - (TOOL_BUTTON_HEIGHT / 2) - Y_OFFSET-1;

			m_pBtnUp->MoveWindow(&rect);

			rect.left = X_OFFSET;
			rect.top = toolrect.bottom - (TOOL_BUTTON_HEIGHT / 2) - Y_OFFSET-1;
			rect.right = rect.left + TOOL_BUTTON_WIDTH;
			rect.bottom = toolrect.bottom - Y_OFFSET-1;
