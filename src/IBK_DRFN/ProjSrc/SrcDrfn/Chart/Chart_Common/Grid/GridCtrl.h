...
#define GRIDCTRL_CLASSNAME    _T("StockOrderCtrl")	// Window class name
...
//-----------------------------------------------------------------------------
// start
#define GVN_VSCROLLEND			WM_USER+1001		// JSJ_ADD_041028
// end

// UserMessage
#define	UDM_DROPORDER_CANCEL	WM_USER+1300
#define	UDM_CREATE_CANCEL_DLG	WM_USER+1310
#define	UDM_DESTROY_CANCEL_DLG	WM_USER+1320
#define	UDM_DROPORDER_CHANGE	WM_USER+1400
#define	UDM_TRACE_MOUSEMOVE		WM_USER+1600
#define WM_RSVDGRID_LCLICK		WM_USER+1610
...
public:
	...
	//-----------------------------------------------------------------------------
	// Author		: Dae-Sung, Byun	Date :2005/5/10
	// Reason		: 체결 다이얼로그 추가 함에 따라 추가함
	//-----------------------------------------------------------------------------
	// start
	DWORD		m_dwGridStyle;					// Default Window Style		JSJ_ADD
	void		SetWndStyle_PreCreate(DWORD dwGridStyle);

	BOOL		Get_IsVisibleVScroll() { return ( (m_nBarState & GVL_VERT) > 0); } 
	// end
...
///////////////////////////////////////////////////////////////////////////////////
public:
	CCellID GetLeftClickDownCell() { return m_LeftClickDownCell; }
	CCellID	GetLastDragOverCell()	{ return m_LastDragOverCell; }
	...
	BOOL SetFixedBtmRowCount(int nFixedBtmRowCnt = 0);	// FixedBtmRow__JSJ_ADD_040828
    BOOL SetFixedColumnCount(int nFixedCols = 1);

    int  GetRowHeight(int nRow) const;
    BOOL SetRowHeight(int row, int height);
	BOOL SetRowHeight_NoReset(int row, int height);

    int  GetColumnWidth(int nCol) const;
    BOOL SetColumnWidth(int col, int width);
	BOOL SetColumnWidth_NoReset(int nCol, int width);
	...
	int	 GetFixedBtmRowHeight() const;	// FixedBtmRow__JSJ_ADD_040828
...
// JSJ_ADD -------------------------------------------------------------------------------------->>>>
public:
	CString		m_strPriceAtDragStart;			// LButtonDown 드래그 시작시 호가

	CCellID		GetLastCellMousePoint();		// 현재 마우스포인트가 위치해 있는 셀

	void		Set_DropTargetGridHwnd();		// Set DropTarget Grid Hwnd
	void		Rel_DropTargetGridHwnd();		// Release DropTarget Grid Hwnd
	
	BOOL		InsideStaticLeft(int nRow, int nCol);
	BOOL		InsideStaticRight(int nRow, int nCol);

	void		SetBongGraphEnable(BOOL bEnable);
	BOOL		GetIsBongGraphEnable();

	void		SetSelectableFixedRow(BOOL bSelectableFixedRow, int nNotSelectableRowCnt=1);// 고정열이라도 셀선택 가능하게
	BOOL		GetIsSelectableFixedRow();
	int			GetNotSelectableRowCnt();

	void		SetTenHogaGrid(BOOL bIsTenHoga);											// 10호가
	void		SetCurPriceRow(int nCurPriceRow);											// 현재가 열
	void		SetBottomInfoRow(int nLowLimitRow, int n5HogaSumRow, int nTotalSumRow);		// 하단 정보표시열
	int			Get5HogaSumRow();
	int			GetTotalSumRow();

	void		SendMessageToCancelDlg(int nRow, int nCol, int nMessage);					// 취소타이틀 Dlg Show, Hide 메시지
	void		ReleaseAllSelectedCell();													// 선택된 모든셀 해제

	void		DrawCandleChart(void* pChartData, CMapStringToPtr* pGridMap, int* nHogaData);	// 봉그래프
	void		DrawMiniCandleChart(void* pChartData);											// 미니 봉그래프

	void		SetVScrollPosForReOrder(int nRow);								// 특정 Row가 그리드 가운데오게 스크롤 조정
	void		Call_ResetScrollBars();											// 스크롤바 조정함수 호출
	
	void		SetBtmCancelArea(BOOL bExpandRight);							// 하단 취소영역
	BOOL		GetBtmCancelArea();

	void		SetShowInfoStatic(BOOL bShowInfoStatic);	// 가격정보가 있는 경우 Redraw, Cell Select 막음

	
protected:
	void		DrawCandle(CDC* pDC);														// 봉그래프
	void		GradientFill(CDC* pDC, CRect rect, COLORREF color);
	void		PaintGradientRect(CDC* pDC, int x, int y, int w, int h, COLORREF color);

	BOOL		m_bTracking;				// 마우스이벤트를 Tracking 할지를 결정하는 플래그
	BOOL		m_bIsDragNDrop;				// OnDrag에서 OnDrop 함수로 이벤트가 발생한 경우만 주문되게
	BOOL		m_bBongGraph;				// 봉그래프
	BOOL		m_bSelectableFixedRow;		// 고정열이라도 셀선택 가능하게
	int			m_nNotSelectableRowCnt;		// 고정열 중 선택 불가한 셀수
	BOOL		m_bIsTenHoga;				// 10호가

	int			m_nCurPriceRow;				// 현재가열
	int			m_nPrePriceRow;				// 이전현재가열
	int			m_nLowLimitRow;				// 하한가 열
	int			m_n5HogaSumRow;				// 5단게합 표시열
	int			m_nTotalSumRow;				// 전체합 표시열
	int			m_nSiKoJeoJongRow[4];		// 시고저종 Row
	int			m_nMiniBong[4];				// 시고저종(미니봉)
	BOOL		m_bShowInfoStatic;			// 가격정보 Show, Hide
	BOOL		m_bBtmCancelArea;			// 하단 취소영역

	PCHARTDATA							m_pChartData;		// 봉그래프 데이터
	CMapStringToPtr*					m_pGridMap;			// 전체 호가데이터 맵
	
	CUtil		m_Util;
	UINT		m_Message;
	CCellID		m_cellLastFocus;			// 현재셀 바로 이전의 셀값
// <<<<-----------------------------------------------------------------------------------------------
...
protected:
	...
	// Mouse Moving Auto Selecte row
	virtual void  OnMouseMoveAutoSelectRow(CCellID& cell);
...
protected:
    ...
    // Cell size details
    int         m_nRows, m_nFixedRows, m_nCols, m_nFixedCols, m_nFixedBtmRows;
