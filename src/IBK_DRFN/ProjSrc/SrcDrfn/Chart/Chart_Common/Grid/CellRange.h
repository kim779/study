...
public:
    ...
    CCellRange(int nMinRow = -1, int nMinCol = -1, int nMaxRow = -1, int nMaxCol = -1)
    {
		...
		SetFixedBtmRow(-1, -1);				// FixedBtmRow__JSJ_ADD_040828 
    }

    void Set(int nMinRow = -1, int nMinCol = -1, int nMaxRow = -1, int nMaxCol = -1);
	void SetFixedBtmRow(int nMinFixedBtmRow = -1, int nMaxFixedBtmRow = -1 );	// FixedBtmRow__JSJ_ADD_040828 
...
protected:
	...
	int m_nMinFixedBtmRow;	// FixedBtmRow__JSJ_ADD_040828 
	int m_nMaxFixedBtmRow;	// FixedBtmRow__JSJ_ADD_040828 
};
...
inline void CCellRange::SetFixedBtmRow(int nMinFixedBtmRow, int nMaxFixedBtmRow)
{
     m_nMinFixedBtmRow = nMinFixedBtmRow;
     m_nMaxFixedBtmRow = nMaxFixedBtmRow;
}
//----------------------------------------------------------------------------
...
inline int CCellRange::InRange(int row, int col) const
{
	//return (row >= m_nMinRow && row <= m_nMaxRow && col >= m_nMinCol && col <= m_nMaxCol);
	 return (	((row >= m_nMinRow && row <= m_nMaxRow) || (row >= m_nMinFixedBtmRow && row <= m_nMaxFixedBtmRow))
			 &&  (col >= m_nMinCol && col <= m_nMaxCol)	);
}
