...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_nCols               = 0;
    m_nFixedRows          = 0;
	m_nFixedBtmRows		  = 0;			// FixedBtmRow__JSJ_ADD_040828
    m_nFixedCols          = 0;
	...
	// JSJ_Add_N_Modify -------------------------------------------------------------------------------->
	m_bTracking			 	= FALSE;
	m_bIsDragNDrop			= FALSE;				// OnDrag에서 OnDrop 함수로 이벤트가 발생한 경우만 주문되게	JSJ_Add
	m_bBongGraph			= FALSE;				// 봉그래프

	m_bSelectableFixedRow	= FALSE;				// 고정열이라도 셀선택 가능하게
	m_nNotSelectableRowCnt	= 1;					// 고정열 중 선택 불가한 셀수
	m_bIsTenHoga			= TRUE;					// 10호가

	m_nCurPriceRow			= -1;					// 현재가열
	m_nPrePriceRow			= -1;					// 이전 현재가열
	m_nLowLimitRow			= LOW_ROW;				// 하한가 열
	m_n5HogaSumRow			= TENHOGA_ROW_5HOSUM;	// 5단게합 표시열
	m_nTotalSumRow			= TENHOGA_ROW_TOTSUM;	// 전체합 표시열
	
	m_bBtmCancelArea		= FALSE;				// 하단 취소영역

	m_pChartData			= NULL;					// 봉그래프 데이터
	m_pGridMap				= NULL;					// 호가 데이터 

	m_strPriceAtDragStart	= _T("");				// LButtonDown 드래그 시작시 호가
	m_LeftClickDownCell		= CCellID(0,0);			// 최초 그리드 포커스 문제로 이벤트없는 Header로 잡아준다  JSJ_Add
	m_cellLastFocus			= CCellID(-1,-1);		// 현재셀 바로 이전의 셀값
	// <-------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------
	// Author		: Dae-Sung, Byun	Date :2005/4/29
	// Reason		: 한화 관련 초기화
	//-----------------------------------------------------------------------------
	// start
	m_dwGridStyle			= WS_EX_CLIENTEDGE;		// Default Window Style
	m_LastMousePoint		= CPoint(0,0);
	m_bShowInfoStatic		= TRUE;					// 가격정보 Show, Hide(쾌속주문기준)
	// end
}
...
//    m_TitleTip.SetParentWnd(this);	
    ...
    if (::IsWindow(m_hWnd))
        ModifyStyleEx(0, m_dwGridStyle, 0);
   
    bInProcedure = FALSE;
    return TRUE;
}

void CGridCtrl::SetWndStyle_PreCreate(DWORD dwGridStyle)
{
	m_dwGridStyle = dwGridStyle;
}
...
{
	// 수 정 자 : 김용식 (2002년2월6일   14시47분)
	// 수정사항 : 셀 선택 방지
	return;
	// 수정완료 :(2002년2월6일 14시47분)	
...
{
	// 수 정 자 : 김용식 (2002년3월5일   17시49분)
	// 수정사항 : 키 다운 방지
	if( m_idCurrentCell.col != HOGA_COL)
		return;

	if( !( m_idCurrentCell.row == ETC_HOGAUP_ROW || m_idCurrentCell.row == ETC_HOGADOWN_ROW ) )
		return;
	// 수정완료 :(2002년3월5일 17시49분)
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	// 수 정 자 : 김용식 (2002년3월4일   10시32분)
	// 수정사항 : 에디팅이 끝나면 포커스를 없애준다.
	//OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, CPoint( -1, -1), pgvItem->lParam);
	SetFocusCell(-1, -1);		
	SelectCells(m_LeftClickDownCell, FALSE, FALSE);
	// 수정완료 :(2002년3월4일 10시32분)
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	// FixedBtmRow__JSJ_ADD_040828 -------------------------
	//rect.bottom -= GetFixedBtmRowHeight();		// Invalidate Area
	//------------------------------------------------------
    ...
    case SB_LINEDOWN:
        if (scrollPos < m_nVScrollMax)
        {
            ...
            //if (GetScrollPos32(SB_VERT) == scrollPos)
			//{
			//	// SendMessage VScrollEnd	JSJ_ADD_041028
			//	SendMessageToParent(0, 0, GVN_VSCROLLEND);
            //    break;          // didn't work
			//}
			// end
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	// FixedBtmRow__JSJ_ADD_040828 -------------------------------
	VisRect.bottom += GetFixedBtmRowHeight();
	//------------------------------------------------------------


	// 수 정 자 : 김용식 (2002년2월7일   10시18분)
	// 수정사항 : 사용하지 않는 코드 제거
	//*
	...
	// 하단 고정열 Draw
	int nHeightTemp = 0;
    for (row = minVisibleRow; row <= maxVisibleRow/*-m_nFixedBtmRows*/; row++)
    {
        nHeightTemp += GetRowHeight(row);
	}

	rect.bottom = nFixedRowHeight + nHeightTemp-1;
    for (row = GetRowCount()-m_nFixedBtmRows; row < GetRowCount(); row++)
    {
        rect.top = rect.bottom+1;
        rect.bottom = rect.top + GetRowHeight(row)-1;
	...
	// 봉칼럼의 수평라인은 그리지 않는다
	if (m_bBongGraph == TRUE)
	{
		//minVisibleRow = m_nNotSelectableRowCnt;
		VisRect.right -= BONG_COL_WIDTH;
		...
		if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)
		{
			...
			for (row = minVisibleRow; row <= maxVisibleRow; row++)
			{
				...
				// ***예외***
				// 봉칼럼의 5호가잔량 열의 수평라인
				if (m_bIsTenHoga == TRUE || m_nFixedBtmRows < 1)
				{
					if (row == m_nLowLimitRow)	// 10호가
					{
						VisRect.right += BONG_COL_WIDTH;
						pDC->MoveTo(nFixedColWidth, y-1);
						pDC->LineTo(VisRect.right,  y-1);
				...
				{
					if (row == maxVisibleRow-m_nFixedBtmRows+1)	// Full호가(하한가열)
					{
						VisRect.right += BONG_COL_WIDTH;
						pDC->MoveTo(nFixedColWidth, y-1);
						pDC->LineTo(VisRect.right,  y-1);
	...
	// 현재가 라인 그리기 -------------------------------------->>
	CRect	rectFirst, rectEnd;
	int		nWidth;
	
	// 이전 현재가의 라인 지움
	if ( m_nCurPriceRow != m_nPrePriceRow )
	{
		if ( IsCellVisible(m_nPrePriceRow, MAEDO_JAN_COL) && IsCellVisible(m_nPrePriceRow, MAESU_JAN_COL) )
		{
			//-----------------------------------------------------------------------------
			// Author		: Dae-Sung, Byun	Date :2005/5/12
			// Reason		: 체결리스트 화면 추가로 인해 외곽선 그리는 부분 수정 
			//-----------------------------------------------------------------------------
			// start
			if(m_nPrePriceRow >= CHEGYUL_DLG_ROW_FIRST && m_nPrePriceRow <= CHEGYUL_DLG_ROW_END)
			{
				GetCellRect(m_nPrePriceRow, HOGA_COL, &rectFirst);
			}
			else
			{
				GetCellRect(m_nPrePriceRow, MAEDO_JAN_COL, &rectFirst);
			}

			if(m_nPrePriceRow >= INFO_ROW_FIRST && m_nPrePriceRow <= INFO_ROW_END)
			{
				GetCellRect(m_nPrePriceRow, DAEBI_COL, &rectEnd);
			}
			else
				GetCellRect(m_nPrePriceRow, MAESU_JAN_COL, &rectEnd);

			// end
			
			nWidth  = rectEnd.right-rectFirst.left;
			pDC->Draw3dRect(rectFirst.left-1, rectFirst.top-1, nWidth+2, rectFirst.Height()+2, COLOR_GRIDLINE, COLOR_GRIDLINE);
		}
		m_nPrePriceRow = m_nCurPriceRow;
	}
	
	if ( m_nCurPriceRow != -1 && IsCellVisible(m_nCurPriceRow, MAEDO_JAN_COL) && IsCellVisible(m_nCurPriceRow, MAESU_JAN_COL) )
	{
		//-----------------------------------------------------------------------------
		// Author		: Dae-Sung, Byun	Date :2005/5/12
		// Reason		: 체결리스트 화면 추가로 인해 외곽선 그리는 부분 수정 
		//-----------------------------------------------------------------------------
		// start
		if(m_nCurPriceRow >= CHEGYUL_DLG_ROW_FIRST && m_nCurPriceRow <= CHEGYUL_DLG_ROW_END)
			GetCellRect(m_nCurPriceRow, HOGA_COL, &rectFirst);
		else
			GetCellRect(m_nCurPriceRow, MAEDO_JAN_COL, &rectFirst);

		if(m_nCurPriceRow >= INFO_ROW_FIRST && m_nCurPriceRow <= INFO_ROW_END)
		{
			GetCellRect(m_nCurPriceRow, DAEBI_COL, &rectEnd);
		}
		else
		{
			GetCellRect(m_nCurPriceRow, MAESU_JAN_COL, &rectEnd);
		}
		// end
		
		nWidth  = rectEnd.right-rectFirst.left;
		pDC->Draw3dRect(rectFirst.left-1, rectFirst.top-1, nWidth+2, rectFirst.Height()+2, COLOR_BLACK, COLOR_BLACK);
		pDC->Draw3dRect(rectFirst.left, rectFirst.top-1, nWidth, rectFirst.Height()+2, COLOR_BLACK, COLOR_BLACK);
	}
	// <<--------------------------------------------------------
	...
	if (m_bBongGraph == TRUE)
	{
		...
		{
			CPen pen3;
			pen3.CreatePen(PS_SOLID, 0, COLOR_GRIDLINE);
			
			CRect rectFirst, rectEnd;
			GetCellRect(m_nNotSelectableRowCnt, BONG_COL, &rectFirst);
			GetCellRect(GetFixedRowCount()-1,	BONG_COL, &rectEnd);
			
			poldPen = pDC->SelectObject(&pen3);
			pDC->MoveTo(rectFirst.right, rectFirst.top-1);        
			pDC->LineTo(rectEnd.right, rectEnd.bottom+1);
...
{
	// Redraw Cell Skip ------------------------------------------------------------>
	if( InsideStaticLeft(nRow, nCol) )									// 좌측 취소 스태틱
		return FALSE;

	if( InsideStaticRight(nRow, nCol) )									// 우측 취소 스태틱
		return FALSE;


	//-----------------------------------------------------------------------------
	// Author		: Dae-Sung, Byun	Date :2005/5/13
	// Reason		: 
	//-----------------------------------------------------------------------------
	// start
	if (m_bShowInfoStatic)
	{
		if( nCol == MAESU_JAN_COL )		// 정보스태틱
			if( MAEDO_ROW<= nRow && nRow < MAESU_ROW )	
				return FALSE;
	}
	// end


	if (m_bBtmCancelArea)												// 하단 취소영역
	{
		if( nRow >= m_n5HogaSumRow && nCol == HOGA_COL )
			return FALSE;
	}
	// <--------------------------------------------------------------------------------
        ...
        else
        {
			...
			if ( m_nCurPriceRow != m_nPrePriceRow )
			{
				if ( IsCellVisible(m_nPrePriceRow, MAEDO_JAN_COL) && IsCellVisible(m_nPrePriceRow, MAESU_JAN_COL) )
				{
					...
					// 이전 현재가 Cell ReDraw(Cell내에 이전 현재가라인 잔상 때문)
					//-----------------------------------------------------------------------------
					// Author		: Dae-Sung, Byun	Date :2005/5/12
					// Reason		: 체결리스트 화면 추가로 인해 외곽선 그리는 부분 수정 
					//-----------------------------------------------------------------------------
					...
					if(m_nPrePriceRow >= CHEGYUL_DLG_ROW_FIRST && m_nPrePriceRow <= CHEGYUL_DLG_ROW_END)
					{
						pCell = GetCell(m_nPrePriceRow, HOGA_COL);
						if (pCell)		pCell->Draw(pDC, m_nPrePriceRow, HOGA_COL, rectFirst, TRUE);	// TRUE:bEraseBkgnd
					}
					else
					{
						pCell = GetCell(m_nPrePriceRow, MAEDO_JAN_COL);
						if (pCell)		pCell->Draw(pDC, m_nPrePriceRow, MAEDO_JAN_COL, rectFirst, TRUE);	// TRUE:bEraseBkgnd
					}
					
					if(m_nPrePriceRow >= INFO_ROW_FIRST && m_nPrePriceRow <= INFO_ROW_END)
					{
						pCell = GetCell(m_nPrePriceRow, DAEBI_COL);
						if (pCell)		pCell->Draw(pDC, m_nPrePriceRow, MAESU_JAN_COL, rectEnd, TRUE);
					}
					else
					{
						pCell = GetCell(m_nPrePriceRow, MAESU_JAN_COL);
						if (pCell)		pCell->Draw(pDC, m_nPrePriceRow, MAESU_JAN_COL, rectEnd, TRUE);
					}
					// end
                                 ...
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	// JSJ_ADD_040812 COMMENT: 고정열이라도 선택할 필요가 있을 경우
	if (m_bSelectableFixedRow == TRUE)
	{
		VisCellRange.SetMinRow(m_nNotSelectableRowCnt);

		if(nMinRow >= 0 && nMinRow < m_nNotSelectableRowCnt)	// 고정열 중 선택 불가한 셀수 m_nNotSelectableRowCnt
			nMinRow = m_nNotSelectableRowCnt;
		if(nMaxRow >= 0 && nMaxRow < m_nNotSelectableRowCnt)
			nMaxRow = m_nNotSelectableRowCnt;
	...
	// FixedBtmRow__JSJ_ADD_040828 -------------------------------------
	// 하단 고정열이 있을경우
	if ( m_nFixedBtmRows > 0 )
	{
		VisCellRange.SetFixedBtmRow(m_nLowLimitRow, m_nLowLimitRow);
	}
	//------------------------------------------------------------------
    ...
    if (bSelectCells)
    {
		...
		// 수 정 자 : 김용식 (2002년 3월 19일 13시 25분)
		// 수정사항 : 
        ...
        nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++)
            {
                ...
                if (bSelectCells)
				{
					// JSJ_Add_041115 COMMENT: 취소칼럼인 경우 셀선택 제외
					if (col == LEFT_CAN_COL || col == RIGHT_CAN_COL)
						continue;

					if (m_bShowInfoStatic)
					{
						// JSJ_Modify_040517	체결리스트 다이아로그 깜빡임 문제
						if(row >= CHEGYUL_DLG_ROW_FIRST && row <= CHEGYUL_DLG_ROW_END && col > MAEDO_COL && col < HOGA_COL)
							continue;
						
						// JSJ_Modify_040517	시고저종 스태틱 깜빡임 문제
						if(row >= INFO_ROW_FIRST && row <= INFO_ROW_END && col > DAEBI_COL && col < MAESU_COL)
							continue;
					}
                           ...
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	if (m_bSelectableFixedRow == TRUE)
	{
		if (currentCell.row < m_nNotSelectableRowCnt)
			return;
	}
	else
	{
	...
	// 수 정 자 : 김용식 (2002년 3월 19일 13시 15분)
	// 수정사항 : 
	//if (GetSingleRowSelection())
    ...
    //                     GetColumnCount()-1,
    //                     bForceRedraw, bSelectCells);
	// 수정완료 :(2002년 3월 19일 13시 15분)

	// JSJ_Modify_040520 -----------------------------------------------------> 
	CCellID Cell;
	Cell = GetCellFromPt(m_LastMousePoint);
	
	// 취소영역에 포커스가 올경우 미체결정보가 있을경우 포커스 JSJ_Modify_040528
	if ( currentCell.row >= m_n5HogaSumRow )
	{
		// 열선택 해제
		ResetSelectedRange();

		/*if (Cell.col == LEFT_CAN_COL)
		{
			// 좌측 취소영역 마지막열 셀만 선택
			SetSelectedRange(m_n5HogaSumRow, LEFT_CAN_COL, m_n5HogaSumRow, LEFT_CAN_COL, bForceRedraw, bSelectCells);
		}
		else if ( Cell.col == RIGHT_CAN_COL)
		{
			// 우측 취소영역 마지막열 셀만 선택
			SetSelectedRange(m_n5HogaSumRow, RIGHT_CAN_COL, m_n5HogaSumRow, RIGHT_CAN_COL, bForceRedraw, bSelectCells);
		}*/
	}
	else if( HOGA_COL<=Cell.col && Cell.col <= DAEBI_COL )
	{
		// 호가,대비 칼럼
		SetSelectedRange(currentCell.row, HOGA_COL, currentCell.row, DAEBI_COL, bForceRedraw, bSelectCells);
	}
	else if( MAESU_JAN_COL<=Cell.col && Cell.col <= MAESU_RESERVE_COL )
	{
		// 매수 영역
		SetSelectedRange(currentCell.row, HOGA_COL, currentCell.row, MAESU_RESERVE_COL, bForceRedraw, bSelectCells);
	}
	//else if ( LEFT_CAN_COL <= Cell.col && Cell.col < HOGA_COL )
	else if ( MAEDO_RESERVE_COL <= Cell.col && Cell.col < HOGA_COL )
	{
		// 매도 영역
		SetSelectedRange(currentCell.row, MAEDO_RESERVE_COL, currentCell.row, HOGA_COL, bForceRedraw, bSelectCells);
	}
	else
	{
		ResetSelectedRange();
	...
	if ( GetColumnCount() < 4 )
	{
		SetSelectedRange(currentCell.row, RSVDGRID_MAEDO_COL, currentCell.row, RSVDGRID_MAESU_COL, bForceRedraw, bSelectCells);
	}
	// <<---------------------------------------

	// 마우스 Move Message ------------------------------------------------>>
	if ( m_cellLastFocus != currentCell)
	{
		m_cellLastFocus = currentCell;
		SendMessageToParent(currentCell.row, currentCell.col, UDM_TRACE_MOUSEMOVE);
	}
	// <<--------------------------------------------------------------------
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
	...
	// Mouse Mode (Drag Prepare or Drag Not Prepare) --------------------->>
	if (currentCell.row <= m_nTotalSumRow)
	{
		CString sTemp = GetItemText(currentCell.row, currentCell.col);
		if( sTemp == "" || sTemp == " " )
			m_MouseMode = MOUSE_NOTHING;
		else
		{
			if( currentCell.col == MAEDO_COL || currentCell.col == MAESU_COL || 
				currentCell.col == MAEDO_RESERVE_COL || currentCell.col == MAESU_RESERVE_COL )
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
	...
	TRACE("[CGridCtrl::CopyTextFromGrid] End\n");
...
/*
    ...
    do
    {
		...
		strNext = (nIndex < 0)? _T(strLine)  : strLine.Mid(nIndex + 1);

		if( cell.col == MAEDO_COL )
		{
			strNext = strLine.Left(nIndex);
			break;
		}
		strLine = strNext;
	} while (nIndex >= 0);
	...
	// 드랍한 셀의 텍스트 JSJ_Modify ------------------------------------>>
	//if( m_Message == UDM_DROPORDER_CHANGE )
	ValidateAndModifyCellContents(cell.row, cell.col, _T(""));
	//else
	//	ValidateAndModifyCellContents(cell.row, cell.col, strNext);
	// <<------------------------------------------------------------------
...
{
	// 매도, 매수칼럼이 아니거나 값이 없으면 리턴 JSJ_Modify_040520 -------->
	if( !(	   m_idCurrentCell.col == MAEDO_COL 
			|| m_idCurrentCell.col == MAESU_COL 
			|| m_idCurrentCell.col == MAEDO_RESERVE_COL 
			|| m_idCurrentCell.col == MAESU_RESERVE_COL		))
	{
		return;
	}
	
	CString strText = GetItemText(m_idCurrentCell.row, m_idCurrentCell.col);
	if( strText == "" || strText == " ")
		return;
	// <---------------------------------------------------------------------
    ...
    if (pSource)
    {
		// Set DropTarget Grid Hwnd
		Set_DropTargetGridHwnd();
		
		// 드래그 시작시 호가 저장
		m_strPriceAtDragStart = GetItemText(m_LeftClickDownCell.row, HOGA_COL);
		...
		// 취소 다이아로그에 포커스가 갔을경우 DROPEFFECT_MOVE 로 인한 문제		JSJ_Add
		if (m_bIsDragNDrop == FALSE)
		{
			if (pSource)
				delete pSource;    // Did not pass source to clipboard, so must delete

			// Release DropTarget Grid Hwnd
			Rel_DropTargetGridHwnd();
		...
		{
		    CutSelectedText();			
			LRESULT lRet = SendMessageToParent(m_LastDragOverCell.row, m_LastDragOverCell.col, m_Message);
		}
		// DragNDrop 실패시 값 복원
		else
		{
			SetItemText(m_LeftClickDownCell.row, m_LeftClickDownCell.col, strText);	
			RedrawCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
			SetItemText(m_LastDragOverCell.row, m_LastDragOverCell.col, _T(""));	
...
{
	//TRACE1("OnDragOver :: MOUSE MODE : %d \r\n", m_MouseMode);
	
	m_bIsDragNDrop = FALSE;			// 마우스 드래그가 시작		JSJ_Add 

    // Any text data available for us?
    if (!m_bAllowDragAndDrop || !IsEditable() || !pDataObject->IsDataAvailable(CF_TEXT))
        return DROPEFFECT_NONE;

	// Find which cell we are over and drop-highlight it (JSJ_Modify_040902)
	//CCellID cell = GetCellFromPt(point, FALSE);
	CCellID cell;
	if ( m_bSelectableFixedRow )
	{
		cell = GetCellFromPt(point);
		if (cell.row < m_nNotSelectableRowCnt)
			cell.row = -1;
	}
	else
	{
		cell = GetCellFromPt(point, FALSE);
	}
	...
	// JSJ_Modify_040511	취소표시 스태틱때문에 조건 추가
	if (!IsCellEditable(cell) && cell.col != HOGA_COL)
	{
		return DROPEFFECT_NONE;
	}
...
/*		
		// 수 정 자 : 김용식 (2002년2월7일   13시24분)
		// 수정사항 : 커서 변경
		SetCursor(AfxGetApp()->LoadCursor(IDC_MOVING));
		m_MouseMode = MOUSE_DRAGGING;
		// 수정완료 :(2002년2월7일 13시24분)
*/
    }

	//TRACE("[CGridCtrl::OnDragOver] End\n");

	// Return an appropraite value of DROPEFFECT so mouse cursor is set properly
	// 수 정 자 : 김용식 (2002년2월14일   17시04분)
	// 수정사항 : 카피 방지
	/*
...
DROPEFFECT CGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
                                  CPoint point)
{	
	//TRACE1("OnDragEnter :: MOUSE MODE : %d \r\n", m_MouseMode);
...
{
	//TRACE1("OnDragLeave :: MOUSE MODE : %d \r\n", m_MouseMode);
	...
	// Drag & Drop후 기존 셀의 포커스 및 셀렉트를 풀어준다 ---------->>
	if (IsValid(m_LeftClickDownCell))
	{
		if( IsCellSelected(m_LeftClickDownCell) )
		{
			...
			// Text가 업데이트 안되서 강제로 업데이트 해줌 <<-- JSJ_BLOCK_041029
			//SetItemText(m_LeftClickDownCell.row, m_LeftClickDownCell.col, "");
...
{
	m_bIsDragNDrop = TRUE;		// 마우스 이벤트가 OnDrag에서 OnDrop 함수로 끝났을경우	JSJ_Add

//	TRACE("[CGridCtrl::OnDrop] m_LastDragOverCell.row = %d, m_LastDragOverCell.col = %d\n",
//	m_LastDragOverCell.row, m_LastDragOverCell.col);

	// 수 정 자 : 김용식 (2002년2월6일   20시59분)
	// 수정사항 : 커서 변경을 위해서 마우스 모드 변경	
	if( m_LastDragOverCell.col == LEFT_CAN_COL )		// 좌측취소 스태틱	
	{
		if( InsideStaticLeft(m_LastDragOverCell.row, m_LastDragOverCell.col) )
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
			m_Message =  UDM_DROPORDER_CANCEL;
			OnDragLeave();
	...
	else if( m_LastDragOverCell.col == RIGHT_CAN_COL )	// 우측취소 스태틱
	{
		if( InsideStaticRight(m_LastDragOverCell.row, m_LastDragOverCell.col) )
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
			m_Message =  UDM_DROPORDER_CANCEL;
			OnDragLeave();
	...
	else if( m_LastDragOverCell.col == HOGA_COL )
	{
		m_MouseMode = MOUSE_PREPARE_DRAG;
		m_Message =  UDM_DROPORDER_CHANGE;
		OnDragLeave();
	...
	{
		CString sTemp;
		sTemp = GetItemText(m_LastDragOverCell.row, HOGA_COL);
		if( sTemp == "" || sTemp == " " )
		{
			m_MouseMode = MOUSE_NOTHING;
		}
...
CCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
    ...
    if (point.y < 0 || (!bAllowFixedCellCheck && point.y < fixedRowHeight)) // not in window
        cellID.row = -1;
    else if (point.y < fixedRowHeight) // in fixed row
    {
        int ypos = 0;
        int row = 0;
    ...
    else
    {
		...
		if (m_nFixedBtmRows < 1)		// Non Fixed Bottom row
		{
			ypos = fixedRowHeight;
			row = idTopLeft.row; //m_nFixedRows;
			while ( row < GetRowCount() )
			{
				ypos += GetRowHeight(row);
				if (ypos > point.y)
		...
		else		// Exist Fixed Bottom row
		{
			...
			// 하단 고정열 전까지의 높이를 구한다
			ypostemp = fixedRowHeight;
			for ( row = minVisibleRow; row <= maxVisibleRow; row++)
			{
				ypostemp += GetRowHeight(row);
			}
			...
			if (ypostemp > point.y)
			{
				ypos = fixedRowHeight;
				row = idTopLeft.row; //m_nFixedRows;
				while ( row < (GetRowCount()-m_nFixedBtmRows))
				{
					ypos += GetRowHeight(row);
					if (ypos > point.y)
						break;
			...
			{
				row = GetRowCount()-m_nFixedBtmRows;
				while (row < GetRowCount() )
				{
					ypostemp += GetRowHeight(row);
					if (ypostemp > point.y)
                                                  ...
                                                  BOOL bForceRecalculation /*=FALSE*/)
{
	...
	// FixedBtmRow__JSJ_ADD_040828 ---------------
	rect.bottom -= GetFixedBtmRowHeight();
	//--------------------------------------------
...
void CGridCtrl::ResetSelectedRange()
{
    ...
    //SetFocusCell(-1,-1);		// JSJ_Modify_040702 COMMENT: Cell Redraw 문제로 막자
}
...
void CGridCtrl::ResetScrollBars()
{
	...
	// FixedBtmRow__JSJ_ADD_040828 ----------------
	rect.bottom -= GetFixedBtmRowHeight();
	//VirtualRect.bottom -= GetFixedBtmRowHeight();
	//---------------------------------------------
	...
	// FixedBtmRow__JSJ_ADD_040828 -------------------------
	//VisibleRect.bottom -= ROW_HEIGHT *3;
	VirtualRect.bottom -= GetFixedBtmRowHeight()+1;
	//------------------------------------------------------
...
BOOL CGridCtrl::GetCellOrigin(int nRow, int nCol, LPPOINT p)
{
	...
	// FixedBtmRow__JSJ_ADD_040828 -------------------------------------
	if (nRow > (GetRowCount()-m_nFixedBtmRows-1) )
	{
		CCellRange VisCellRange = GetVisibleNonFixedCellRange();
		int maxVisibleRow = VisCellRange.GetMaxRow();
		...
		{                                        // is a scrollable data row
			for (i = 0; i < m_nFixedRows; i++)
				p->y += GetRowHeight(i);
			for (i = idTopLeft.row; i <= maxVisibleRow; i++)
				p->y += GetRowHeight(i);
			for (i = GetRowCount()-m_nFixedBtmRows; i < nRow; i++)
				p->y += GetRowHeight(i);
		}
		return TRUE;
	}
...
BOOL CGridCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
    ...
    {
		//TRACE("[ OnSetCursor ] Mouse Mode = %d\n", m_MouseMode);
        switch (m_MouseMode)
        {
        ...
        case MOUSE_OVER_ROW_DIVIDE:
            ...
            break;
#ifndef GRIDCONTROL_NO_DRAGDROP
		// 수 정 자 : 김용식 (2002년2월6일   20시52분)
		// 수정사항 : Drag & Drop 커서 변경 			
...
// FixedBtmRow__JSJ_ADD_040828 ----------------------------------------------------------
BOOL CGridCtrl::SetFixedBtmRowCount(int nFixedBtmRowCnt)
{
    if (m_nFixedBtmRows == nFixedBtmRowCnt)
        return TRUE;

    ASSERT(nFixedBtmRowCnt >= 0);
	...
	int nFixedBtmRowPos = GetRowCount() - nFixedBtmRowCnt;
	
    if (nFixedBtmRowCnt+nFixedRowCnt > GetRowCount())
        if (!SetRowCount(nFixedBtmRowCnt+nFixedRowCnt))
            return FALSE;
        
        if (m_idCurrentCell.row > nFixedBtmRowPos)
            SetFocusCell(-1, - 1);
        
        if (!GetVirtualMode())
        {
            if (nFixedBtmRowCnt > m_nFixedBtmRows)
            {
                for (int i = nFixedBtmRowPos; i < GetRowCount()-m_nFixedBtmRows; i++)
                    for (int j = 0; j < GetColumnCount(); j++)
                    {
						//OutputDebugString("CGridCtrl:: 하단고정열 세팅");
            ...
            {
                for (int i = GetRowCount()-m_nFixedBtmRows; i < nFixedBtmRowPos; i++)
                {
                    int j;
                    for (j = 0; j < GetFixedColumnCount(); j++)
                        SetItemState(i, j, GetItemState(i, j) & ~GVIS_FIXEDROW );
        ...
        m_nFixedBtmRows = nFixedBtmRowCnt;
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
    ...
    if (nRow > (nRowCnt-m_nFixedBtmRows-1))
        pCell->SetState(pCell->GetState() | GVIS_FIXED | GVIS_FIXEDROW);
	//--------------------------------------------------------------------
...
BOOL CGridCtrl::DeleteAllItems()
{
	...
	if(m_PrinterFont.GetSafeHandle())
	{
		m_PrinterFont.DeleteObject();//KHD
	}	
    return TRUE;
}
...
int CGridCtrl::GetRowHeight(int nRow) const
{
	...
	// Reason		: 한화 디버그 적용시 에러 땜시 막는다..ㅜㅜ
	//-----------------------------------------------------------------------------
	// start
...
BOOL CGridCtrl::SetRowHeight_NoReset(int nRow, int height)
{
    ASSERT(nRow >= 0 && nRow < m_nRows && height >= 0);
    if (nRow < 0 || nRow >= m_nRows || height < 0)
        return FALSE;
...
BOOL CGridCtrl::SetColumnWidth_NoReset(int nCol, int width)
{
    ASSERT(nCol >= 0 && nCol < m_nCols && width >= 0);
    if (nCol < 0 || nCol >= m_nCols || width < 0)
        return FALSE;
...
// FixedBtmRow__JSJ_ADD_040828 -------------------------
int CGridCtrl::GetFixedBtmRowHeight() const
{
    int nHeight = 0;
	if (m_nFixedBtmRows < 1)
		return 0;

	for (int i = GetRowCount()-m_nFixedBtmRows; i < GetRowCount(); i++)
        nHeight += GetRowHeight(i);
...
{
	BOOL bCellEditable;
	bCellEditable = IsEditable() && (	((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY) ||
										(GetItemState(nRow, nCol) & GVIS_CELLEDITABLE) );
...
BOOL CGridCtrl::IsCellVisible(int nRow, int nCol)
{
	...
	if (nRow > (GetRowCount()-m_nFixedBtmRows-1))
	{
		int		rowCnt = m_nFixedBtmRows - (GetRowCount()-nRow);
		//int		rowCnt = 1;
		int		fixedRowHeight = GetFixedRowHeight();
		int		ypostemp;
		
		ypostemp = fixedRowHeight;
		for (x = nRow; x <= nRow+rowCnt; x++)
		{
			ypostemp += GetRowHeight(x);
		}
		
		if (ypostemp >= rect.bottom )
			return FALSE;
		else
			return TRUE;
	}

	// 하단 고정열위의 Scroll Row Draw 영역
	rect.bottom -= GetFixedBtmRowHeight();
	//------------------------------------------------------------------------
...
LRESULT CGridCtrl::OnMouseLeave(WPARAM, LPARAM)
{
    m_bTracking = FALSE;
	ResetSelectedRange();
	return 0;
...
{	
	...
	//TRACE1("OnMouseMove :: MOUSE MODE : %d \r\n", m_MouseMode);
...
//		TRACE("[CGridCtrl::OnMouseMove0] rect.top = %d, rect.left = %d, rect.bottom = %d, rect.right = %d,\
//			point.x = %d, point.y = %d\n", rect.top, rect.left, rect.bottom, rect.right, point.x, point.y);
		//CCellID ce = GetCellFromPt(point);		
		//TRACE("[CGridCtrl::OnMouseMove0] ce.row = %d, ce.col = %d, rect.PtInRect(point) = %d\n", ce.row, ce.col, 
		//	rect.PtInRect(point));

#ifndef GRIDCONTROL_NO_DRAGDROP
    // If outside client area, return (unless we are drag n dropping)
	
	// 수 정 자 : 김용식 (2002년2월14일   10시33분)
	// 수정사항 : 밑의 조건으로 들어가지 않는다. !rect.PtInRect(point)가 항상 0이기 때문
	//			  나중에 고쳐줘야 함
	if (m_MouseMode != MOUSE_DRAGGING && !rect.PtInRect(point))
	{
		// 수 정 자 : 김용식 (2002년2월14일   10시00분)
		// 수정사항 : 포커스 및 셀렉트 해제
		SetFocusCell(-1, -1);		
		SelectCells(m_LeftClickDownCell, FALSE, FALSE);
		m_MouseMode = MOUSE_NOTHING;
		// 수정완료 :(2002년2월14일 10시00분)
		return;
	}     
	// 수정완료 :(2002년2월14일 10시33분)
	...
	*/
	// 수정완료 :(2002년2월6일   16시42분)


	// 커서 변경을 위해서 마우스 모드 변경(Mouse Drag Check)-------------------->>>
	CCellID cell = GetCellFromPt(point);
	CGridCell *pCell = (CGridCell *)GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
	if( pCell == NULL )
		return;
	...
	if( !(cell.col == MAEDO_COL || cell.col == MAESU_COL || cell.col == MAEDO_RESERVE_COL || cell.col == MAESU_RESERVE_COL) )
	{
		// 포커스 및 셀렉트 해제
		//SetFocusCell(-1, -1);		// JSJ_Modify_040702 COMMENT: Cell Redraw 문제로 막자	
		SelectCells(cell, FALSE, FALSE);
	...
	{
		//CGridCell *pCell = (CGridCell *)GetCell(cell.row, cell.col);
		//if( pCell == NULL )
		//	return;
		//CString sTemp = pCell->GetText();
		//if( sTemp == "" )
		if( GetItemText(cell.row, cell.col) == "" || GetItemText(cell.row, cell.col) == " ")
		{
			m_MouseMode = MOUSE_NOTHING;		
			//m_MouseMode = MOUSE_PREPARE_DRAG;	// JSJ_Test  공백이어도 드래그 준비가 되었다고 세팅
		}
		//else if( m_bDragging )
		//{
		//	m_MouseMode = MOUSE_DRAGGING;
		...
		else
		{
			int nFixedRow;
			if ( m_bSelectableFixedRow )
				nFixedRow = m_nNotSelectableRowCnt;
			else
				nFixedRow = GetFixedRowCount();
				
			if( cell.row == TENHOGA_ROW_FIRST || m_LeftClickDownCell.row < nFixedRow)
				m_MouseMode = MOUSE_NOTHING;
			else
				m_MouseMode = MOUSE_PREPARE_DRAG;
		}
	...
	// 수 정 자 : 김용식 (2002년2월14일   11시15분)
	// 수정사항 : 마우스 클릭하지 않았을때도 Drag & Drop 모드로 
	/*
	if (!IsValid(m_LeftClickDownCell))
    {
		TRACE("IsValid Error\n");
	...
	*/
	// 수정완료 :(2002년2월14일 11시15분)
	
	
	// 수 정 자 : 김용식 (2002년2월7일   10시24분)
	// 수정사항 : 마우스 움직일때 포커스 주기 위해서 추가
	if( !m_bLMouseButtonDown )
	{
		CCellID FocusCell;
		FocusCell = GetCellFromPt(point);
		if( TENHOGA_ROW_FIRST < FocusCell.row && FocusCell.row < GetRowCount() )	// JSJ_Modify_040528 COMMENT: 취소영역 마지막열 Move 포커스줌
		{
			// 마우스 포커스가 있는열 색상표시 JSJ_Modify ---------------->>
			//SetFocusCell(-1,-1);
			OnMouseMoveAutoSelectRow(FocusCell);
			//SetFocusCell(FocusCell);			
			// <<-----------------------------------------------------------
		}
		
		// Drag and Drop이 False이면 커서를 디폴트 값으로...
		if( ! m_bAllowDragAndDrop )
		{
#ifndef _WIN32_WCE_NO_CURSOR
			SetCursor(AfxGetApp()->LoadStandardCursor(IDC_ARROW));			
    ...
    //if (m_bLMouseButtonDown)
	// 수정완료 :(2002년2월7일 10시24분)
    {
		//TRACE("[ OnMouseMove ] Mouse Mode = %d\n", m_MouseMode);
        switch (m_MouseMode)
        {
		case MOUSE_SELECT_ALL:
            break;
	...
	//TRACE("m_LastMousePoint.x = %d, m_LastMousePoint.y = %d\n", m_LastMousePoint.x, m_LastMousePoint.y);
    m_LastMousePoint = point;
}
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        {
			// 수 정 자 : 김용식 (2002년2월6일   16시48분)
			// 수정사항 : 에디팅 속성 있을 경우에만 가능하게 추가
			//if( !(pCell->GetState() & GVIS_READONLY) )
				OnEditCell(cell.row, cell.col, pointClickedRel, VK_LBUTTON);
			//OnEditCell(cell.row, cell.col, pointClickedRel, VK_LBUTTON);
			// 수정완료 :(2002년2월6일 16시48분)			
        }
        else if (m_bListMode)
        {
            if (!IsValid(cell))
	...
	*/
	...
	// 수 정 자 : 김용식 (2002년2월14일   19시46분)
	// 수정사항 : 주문에서 더블 클릭 했을 경우 주문 창 화면에 표시
	if (m_MouseMode == MOUSE_NOTHING || m_MouseMode == MOUSE_PREPARE_DRAG)
	{
		CPoint pointClickedRel;
        pointClickedRel = GetPointClicked( cell.row, cell.col, point);
		...
		*/
		...
		// 그리드 취소행의 추가로 인함	JSJ_Modify_040422  
		// 미체결이 있는 경우 취소영역 더블클릭으로 주문취소 JSJ_Add_040528
		if( cell.row > TENHOGA_ROW_FIRST && cell.row <= m_nTotalSumRow )				// 주문 더블 클릭
		{
			if ( cell.row < m_n5HogaSumRow )
			{
				if (cell.col == MAEDO_COL || cell.col == MAESU_COL || cell.col == HOGA_COL)		
				{
					if( IsValid(m_LeftClickDownCell) )
					{				
						CString strData;
						...
						strData = GetItemText(cell.row, HOGA_COL);
						strData.TrimLeft();	strData.TrimRight();
						if ( strData != "" || cell.row == MARKETPRICE_ROW)		// 호가가 있거나 시장가 주문인 경우만 메시지보냄
							SendMessageToParent(cell.row, cell.col, NM_DBLCLK);
					}
				}
			}
			...
			if(cell.col == LEFT_CAN_COL )
			{
				...
				{	
					...
					strData = GetItemText(cell.row, MAEDO_COL);
					strData.TrimLeft();	strData.TrimRight();
					if ( strData != "")
						SendMessageToParent(cell.row, cell.col, NM_DBLCLK);
				}
			...
			else if ( cell.col == RIGHT_CAN_COL )	
			{
				...
				{	
					...
					strData = GetItemText(cell.row, MAESU_COL);
					strData.TrimLeft();	strData.TrimRight();
					if ( strData != "")
						SendMessageToParent(cell.row, cell.col, NM_DBLCLK);
				}
			}
			else if ( cell.col == MAEDO_RESERVE_COL || cell.col == MAESU_RESERVE_COL )
			{
				if( IsValid(m_LeftClickDownCell) )
				{	
					CString strData;
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	// 수 정 자 : 김용식 (2002년3월4일   10시22분)
	// 수정사항 : 에디팅 중이면 포커스를 없앤다
	CGridCellBase* pCell0 = GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
	if( pCell0->IsEditing() )
	{
		SetFocusCell(-1, -1);		
		SelectCells(m_LeftClickDownCell, FALSE, FALSE);
	}
	// 수정완료 :(2002년3월4일 10시22분)

//==========================================================================
// 	CString strOut;
//	strOut.Format("%d", m_LeftClickDownCell.row);
//	OutputDebugString("\nCGridCtrl::OnLButtonDown():" + strOut);
//=============================================================================

	// 수 정 자 : 김용식 (2002년2월6일   14시29분)
	// 수정사항 : Shift 셀 선택 방지
	/*	
	...
	*/
	m_SelectionStartCell = m_LeftClickDownCell;	
	m_idCurrentCell		 = m_LeftClickDownCell;	
	m_strPriceAtDragStart= _T("");				// 드래그 시작시 호가 초기화
	// 수정완료 :(2002년2월6일 14시29분)	

	// 수 정 자 : 김용식 (2002년2월14일   9시46분)
	// 수정사항 : 셀이 편집 가능하면 편집 종료  --> 나중에 고쳐줘야 함
	//EndEditing();
	// 수정완료 :(2002년2월14일 9시46분)	
	...
	// 단위셀만 편집가능하게 수정 JSJ_ADD_050126 ----------------------------------------------->>
	// JSJ_ADD_050126__for editable cell check
	BOOL bCellEditable = (GetItemState(m_LeftClickDownCell.row, m_LeftClickDownCell.col) & GVIS_CELLEDITABLE);
    ...
    if (m_LeftClickDownCell == m_idCurrentCell && 
        ...
        IsCellEditable(m_LeftClickDownCell)	&& bCellEditable)
    {
	...
	*/
	if( pCell->GetText() != _T("") )
	{
		//if( IsCellSelected(m_LeftClickDownCell) && m_bAllowDragAndDrop)
		if( !pCell->IsReadOnly() && m_bAllowDragAndDrop)
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
		}
		else
...
#endif
	*/
	// 수정완료 :(2002년2월6일 15시56분)    
    {
		// JSJ_Block_040630	COMMENT: 클릭이벤트는 LButtonUp에서 발생하게 막자 ---------------->
	    //SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_SELCHANGING);
		// <----------------------------------------------------------------------------------------

		// 수 정 자 : 김용식 (2002년2월6일   14시37분)
		// 수정사항 : 셀 선택 방지 및 사용하지 않는 코드 제거
		/*
		...
		*/
		// 수정완료 :(2002년2월6일 14시37분)                
		
		m_MouseMode = m_bListMode? MOUSE_SELECT_ROW : MOUSE_SELECT_CELLS;

		// 전체합열, 5호가합 열인 경우 Cell Select
		if( m_LeftClickDownCell.row >= m_n5HogaSumRow )
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	// LButtonUp시 m_idCurrentCell에 현재 셀값	JSJ_Add
    m_idCurrentCell = GetCellFromPt(point);
    ...
    else if (m_MouseMode == MOUSE_PREPARE_DRAG) 
    {
		...
		// 마우스 LButton Message->MainDlg		JSJ_Modify ------------------------------->>
        // 버튼 클릭 메시지를 메인에 보냄 (GVN_SELCHANGING, GVN_SELCHANGED는 현재 사용하지 않는 기능)
		// LButtonDown = LButtonUp Cell 이 같은 경우에만 메시지 발생
		if (m_LeftClickDownCell == m_idCurrentCell)
		{
			SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, NM_CLICK);
			GetParent()->SendMessage(WM_RSVDGRID_LCLICK, 0, (LPARAM)&m_idCurrentCell);
		}
		
		//SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK);
	
		//SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_SELCHANGING);
		//ResetSelectedRange();			// 선택된셀 모두 해제
		//SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_SELCHANGED);
		// <<----------------------------------------------------------------------------------
    }
#endif
	// 수 정 자 : 김용식 (2002년2월6일   16시28분)
	// 수정사항 : 사용하지 않는 코드 제거			
	/*
	...
	*/
    ...
    else
    {
		...
		/* 이미지의 범위를 클릭한 경우 메시지 발생
		CRect rect0;
		GetCellRect(m_idCurrentCell.row, m_idCurrentCell.col, &rect0);
		rect0.left = rect0.right-13-2;//+pCell->GetMargin();
		rect0.right = rect0.left+13;
		if( rect0.PtInRect(point) && pCell->GetImage()>=2 )
		{
			SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK);
		}
		*/
	...
	//  커서 변경을 위해서 마우스 모드 변경 JSJ_Modify ------------------------------->>
	if ( m_idCurrentCell.row > GetFixedRowCount()-1)		// 고정 Row가 아닌 경우에
	{
		if( !( m_idCurrentCell.col == MAEDO_COL || m_idCurrentCell.col == MAESU_COL 
			|| m_idCurrentCell.col == MAEDO_RESERVE_COL || m_idCurrentCell.col == MAESU_RESERVE_COL) )
			m_MouseMode = MOUSE_NOTHING;
		else 
		{
			if( GetItemText(m_idCurrentCell.row, m_idCurrentCell.col) == "" || GetItemText(m_idCurrentCell.row, m_idCurrentCell.col) == " ")
				m_MouseMode = MOUSE_NOTHING;
			else
				m_MouseMode = MOUSE_PREPARE_DRAG;
		}
...
#endif
	// 수정완료 :(2002년2월6일 21시08분)    
    //TRACE("[OnLbuttonUp] row = %d, col = %d\n", m_LeftClickDownCell.row, m_LeftClickDownCell.col);
    if (!IsValid(m_LeftClickDownCell))
        return;
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	// JSJ_Modify_040616 ----------------------------------------->
	// COMMENT: m_n5HogaSumRow 열을 클릭했을경우 -> RButton Down 명령  
	SetFocus();

	m_bRMouseButtonDown = TRUE;
    //m_bLMouseButtonDown	= TRUE;			// LButton Down 효과 막음
    m_LeftClickDownPoint= point;
    m_LeftClickDownCell	= GetCellFromPt(point);
	m_idCurrentCell		= m_LeftClickDownCell;
...
{
	// JSJ_Modify_040616 --------------------------------------------------->>
	//ReleaseCapture();						// LButton Down 효과 막음
	
	//CWnd::OnRButtonUp(nFlags, point);		// CWnd로 마우스 이벤트 가는것 막음	JSJ_Modify_040628

	m_bRMouseButtonDown = FALSE;
	m_bLMouseButtonDown = FALSE;

	// RButtonUp시 m_idCurrentCell에 현재 셀값	JSJ_Add
	m_idCurrentCell = GetCellFromPt(point);
    ...
    else
    {
		...
		// RButtonDown = RButtonUp Cell 이 같고 고정열이 아닌경우 메시지 발생
		if (m_LeftClickDownCell == m_idCurrentCell && m_idCurrentCell.row > GetFixedRowCount()-1)
		{
...
void CGridCtrl::OnMouseMoveAutoSelectRow(CCellID& cell)
{
    if (!IsValid(cell))
        return;
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
    ...
    {
		// 수 정 자 : 김용식 (2002년3월17일   15시53분)
		// 수정사항 : ','를 붙인다.
		CString strTemp;
		strTemp = str;
		str = m_Util.GetComma(strTemp);
		// 수정완료 :(2002년3월17일 15시53분)
...
// JSJ_ADD ***********************************************************************************
// 현재 마우스포인트가 위치해 있는 셀
CCellID CGridCtrl::GetLastCellMousePoint()
{
	return GetCellFromPt(m_LastMousePoint);
}

// Set DropTarget Grid Hwnd
void CGridCtrl::Set_DropTargetGridHwnd()
{
#   ifndef GRIDCONTROL_NO_DRAGDROP
	m_DropTarget.SetGridHwnd_DragBegin();	// Drag N' Drop Start -> Set Grid hwnd
#	endif
}

// Release DropTarget Grid Hwnd
void CGridCtrl::Rel_DropTargetGridHwnd()
{
#   ifndef GRIDCONTROL_NO_DRAGDROP
	m_DropTarget.RelGridHwnd_DragEnd();		// Drag N' Drop End	  -> Release Grid hwnd
#	endif
}


// JSJ_ITCastle_Modify_040426
BOOL CGridCtrl::InsideStaticLeft(int nRow, int nCol)
{
	if( nCol == LEFT_CAN_COL )
	{
		if( TENHOGA_ROW_FIRST < nRow )		// 좌측 취소
			return TRUE;
	}
...
// JSJ_ITCastle_Modify_040426
BOOL CGridCtrl::InsideStaticRight(int nRow, int nCol)
{
	if( nCol == RIGHT_CAN_COL )
	{
		if( TENHOGA_ROW_FIRST < nRow )		// 우측 취소
			return TRUE;
	}
...
// 봉그래프 Enable(마지막칼럼)
void CGridCtrl::SetBongGraphEnable(BOOL bEnable)
{
	m_bBongGraph = bEnable;
}

BOOL CGridCtrl::GetIsBongGraphEnable()
{
	return m_bBongGraph;
}

// 고정열이라도 셀선택 가능하게			JSJ_Add_040812
void CGridCtrl::SetSelectableFixedRow(BOOL bSelectableFixedRow, int nNotSelectableRowCnt/*=1*/)
{
	if (nNotSelectableRowCnt > GetFixedRowCount())
		return;

	m_bSelectableFixedRow	= bSelectableFixedRow;
	m_nNotSelectableRowCnt	= nNotSelectableRowCnt;
}

BOOL CGridCtrl::GetIsSelectableFixedRow()
{
	return m_bSelectableFixedRow;
}

int CGridCtrl::GetNotSelectableRowCnt()
{
	return m_nNotSelectableRowCnt;
}


// 취소타이틀 Dlg Show, Hide 메시지		JSJ_Add_040510
void CGridCtrl::SendMessageToCancelDlg(int nRow, int nCol, int nMessage)
{
	SendMessageToParent(nRow, nCol, nMessage);
}

// 열선택 해제		JSJ_Add_040716
void CGridCtrl::ReleaseAllSelectedCell()
{
	ResetSelectedRange();
}

// 10호가인지 전체호가 인지
void CGridCtrl::SetTenHogaGrid(BOOL bIsTenHoga)
{
	m_bIsTenHoga = bIsTenHoga;
}

// 현재가열
void CGridCtrl::SetCurPriceRow(int nCurPriceRow)
{
	m_nCurPriceRow = nCurPriceRow;

	RedrawCell(m_nCurPriceRow, HOGA_COL);
}

// 하한가열, 5호가합계, 전체합계열
void CGridCtrl::SetBottomInfoRow(int nLowLimitRow, int n5HogaSumRow, int nTotalSumRow)
{
	m_nLowLimitRow = nLowLimitRow;
	m_n5HogaSumRow = n5HogaSumRow;
	m_nTotalSumRow = nTotalSumRow;
}

int CGridCtrl::Get5HogaSumRow()
{
	return m_n5HogaSumRow;
}


int CGridCtrl::GetTotalSumRow()
{
	return m_nTotalSumRow;
}

// 봉그래프
void CGridCtrl::DrawCandleChart(void* pChartData, CMapStringToPtr* pGridMap, int* nHogaData)
{
	m_pChartData = (PCHARTDATA)pChartData;
	if(m_pChartData == NULL) return;	
	
	m_pGridMap	= pGridMap;

	int	nCnt;
	CString strPrice;
	PTGRID_1ROWINFO pGridRowInfo = NULL;

	// 시고저 값이 0인 경우 return
	if ( m_pChartData->nOpen == 0 && m_pChartData->nHigh == 0 && m_pChartData->nLow == 0 )
	{
		return;
	}

	// 시고저종 초기화
	m_nSiKoJeoJongRow[0] = m_nSiKoJeoJongRow[1] = m_nSiKoJeoJongRow[2] = m_nSiKoJeoJongRow[3] = -1;


	// ***** 10호가 시고저종 구하기 *****
	// 상한가와 하한가 열은 체크하지 않는다
	if (m_bIsTenHoga == TRUE)
	{
		// 시가
		if (m_pChartData->nClose < m_pChartData->nOpen)
		{
			for(nCnt = 1; nCnt < TENHOGA_SAVED_CNT-1; nCnt++)
			{
				if (nHogaData[nCnt] <= m_pChartData->nOpen && nHogaData[nCnt] > 0)
				{
					m_nSiKoJeoJongRow[0] = nCnt+HIGH_ROW;
					break;
				}
			}
		}
		...
		{
			for(nCnt = TENHOGA_SAVED_CNT-2; nCnt >= 1 ; nCnt--)
			{
				if (nHogaData[nCnt] >= m_pChartData->nOpen && nHogaData[nCnt] > 0)
				{
					m_nSiKoJeoJongRow[0] = nCnt+HIGH_ROW;
		...
		for(nCnt = 1; nCnt < TENHOGA_SAVED_CNT-1; nCnt++)
		{
			if (nHogaData[nCnt] <= m_pChartData->nHigh && nHogaData[nCnt] > 0)
			{
				m_nSiKoJeoJongRow[1] = nCnt+HIGH_ROW;
				break;
			}
		}
		...
		for(nCnt = TENHOGA_SAVED_CNT-2; nCnt >= 1 ; nCnt--)
		{
			if (nHogaData[nCnt] >= m_pChartData->nLow && nHogaData[nCnt] > 0)
			{
				m_nSiKoJeoJongRow[2] = nCnt+HIGH_ROW;
				break;
			}
		}
		...
		if (m_pChartData->nClose >= m_pChartData->nOpen)
		{
			for(nCnt = 1; nCnt < TENHOGA_SAVED_CNT-1; nCnt++)
			{
				if (nHogaData[nCnt] <= m_pChartData->nClose && nHogaData[nCnt] > 0)
				{
					m_nSiKoJeoJongRow[3] = nCnt+HIGH_ROW;
					break;
				}
			}
		}
	...
	{
		// 시가
		strPrice.Format("%d", m_pChartData->nOpen);
		pGridRowInfo = NULL;
		m_pGridMap->Lookup(strPrice, (void*&)pGridRowInfo);
		if (pGridRowInfo == NULL)	return;
		m_nSiKoJeoJongRow[0] = pGridRowInfo->nRow;

		// 고가
		strPrice.Format("%d", m_pChartData->nHigh);
		pGridRowInfo = NULL;
		m_pGridMap->Lookup(strPrice, (void*&)pGridRowInfo);
		if (pGridRowInfo == NULL)	return;
		m_nSiKoJeoJongRow[1] = pGridRowInfo->nRow;

		// 저가
		strPrice.Format("%d", m_pChartData->nLow);
		pGridRowInfo = NULL;
		m_pGridMap->Lookup(strPrice, (void*&)pGridRowInfo);
		if (pGridRowInfo == NULL)	return;
		m_nSiKoJeoJongRow[2] = pGridRowInfo->nRow;

		// 종가
		strPrice.Format("%d", m_pChartData->nClose);
		pGridRowInfo = NULL;
		m_pGridMap->Lookup(strPrice, (void*&)pGridRowInfo);
		if (pGridRowInfo == NULL)	return;
		m_nSiKoJeoJongRow[3] = pGridRowInfo->nRow;
	}

	// 미니봉 --------------------------------------------->>>
	CRect rect1, rect2;
	GetCellRect(m_n5HogaSumRow, BONG_COL, &rect1);
	GetCellRect(m_nTotalSumRow+1, BONG_COL, &rect2);
	
	double fY	= rect2.bottom - rect1.top;
	
	// 상하한가 기준
	double fY2	= m_pChartData->nHighLimit - m_pChartData->nLowLimit;
	m_nMiniBong[0]	= rect1.top + int(fY * (m_pChartData->nHighLimit - m_pChartData->nOpen)  / fY2 + 0.5);
	m_nMiniBong[1]	= rect1.top + int(fY * (m_pChartData->nHighLimit - m_pChartData->nHigh)  / fY2 + 0.5);
	m_nMiniBong[2]	= rect1.top + int(fY * (m_pChartData->nHighLimit - m_pChartData->nLow)	 / fY2 + 0.5);
	m_nMiniBong[3]	= rect1.top + int(fY * (m_pChartData->nHighLimit - m_pChartData->nClose) / fY2 + 0.5);

/*
	// 고가저가 기준
	double fY2	= m_pChartData->nHigh - m_pChartData->nLow;
	m_nMiniBong[0]	= rect1.top + int(fY * (m_pChartData->nHigh - m_pChartData->nOpen)  / fY2 + 0.5);
	m_nMiniBong[1]	= rect1.top + int(fY * (m_pChartData->nHigh - m_pChartData->nHigh)  / fY2 + 0.5);
	m_nMiniBong[2]	= rect1.top + int(fY * (m_pChartData->nHigh - m_pChartData->nLow)	/ fY2 + 0.5);
	m_nMiniBong[3]	= rect1.top + int(fY * (m_pChartData->nHigh - m_pChartData->nClose) / fY2 + 0.5);*/

	// 시가와 종가 포인트가 같은 경우
	if(m_nMiniBong[0] == m_nMiniBong[3])
		m_nMiniBong[0] += 1;
	// <<<----------------------------------------------------
	...
	CRect rcMiniBong;
	GetCellRect(TENHOGA_ROW_FIRST	, BONG_COL, &rect1);
	GetCellRect(TENHOGA_ROW_EXTRAVOL, BONG_COL, &rect2);
	rcMiniBong			= rect1;
	rcMiniBong.bottom	= rect2.bottom;
	InvalidateRect(&rcMiniBong);
	// ----------------------------------------------------------------------<<

}

// 미니 봉그래프
void CGridCtrl::DrawMiniCandleChart(void* pChartData)
{
	PCHARTDATA pChartDataTmp = (PCHARTDATA)pChartData;
	...
	// 상하한가 기준
	double fY2	= pChartDataTmp->nHighLimit - pChartDataTmp->nLowLimit;
	m_nMiniBong[0]	= rect1.top + int(fY * (pChartDataTmp->nHighLimit - pChartDataTmp->nOpen)  / fY2 + 0.5);
	m_nMiniBong[1]	= rect1.top + int(fY * (pChartDataTmp->nHighLimit - pChartDataTmp->nHigh)  / fY2 + 0.5);
	m_nMiniBong[2]	= rect1.top + int(fY * (pChartDataTmp->nHighLimit - pChartDataTmp->nLow)   / fY2 + 0.5);
	m_nMiniBong[3]	= rect1.top + int(fY * (pChartDataTmp->nHighLimit - pChartDataTmp->nClose) / fY2 + 0.5);
	// <<<----------------------------------------------------
...
// 봉그래프 그리기
void CGridCtrl::DrawCandle(CDC* pDC)	
{
	if(m_pChartData == NULL) return;

	// 시고저종 열 값이 -1인 경우 return
	if ( m_nSiKoJeoJongRow[0] == -1 || m_nSiKoJeoJongRow[1] == -1 ||
		 m_nSiKoJeoJongRow[2] == -1 || m_nSiKoJeoJongRow[3] == -1 )
	{
		return;
	}
	...
	int	nMinusXPos	 = 0;
	int nCol		 = BONG_COL;
	int nHRow, nLRow, nORow, nCRow;

	CRect ValueRect;	
	CRect first_rect;

	CPoint ptOpen, ptHigh, ptLow, ptClose;

	GetCellRect(0, 0, &first_rect);

	nMinusXPos	 = BONGGRAPH_LEFT_OFFSET;

	// 시고저종
	nORow = m_nSiKoJeoJongRow[0];
	nHRow = m_nSiKoJeoJongRow[1];
	nLRow = m_nSiKoJeoJongRow[2];
	nCRow = m_nSiKoJeoJongRow[3];

	// 시가
	ValueRect.left = ValueRect.right = ValueRect.top  = ValueRect.bottom = -1;
	GetCellRect(nORow, nCol, ValueRect);
	ptOpen.x = ValueRect.left + nMinusXPos;
	ptOpen.y = ValueRect.top+ValueRect.Height()/2;

	// 고가
	ValueRect.left = ValueRect.right = ValueRect.top  = ValueRect.bottom = -1;
	GetCellRect(nHRow, nCol, ValueRect);
	ptHigh.x = ValueRect.left + nMinusXPos;
	ptHigh.y = ValueRect.top+ValueRect.Height()/2;
	
	// 저가
	ValueRect.left = ValueRect.right = ValueRect.top  = ValueRect.bottom = -1;
	GetCellRect(nLRow, nCol, ValueRect);
	ptLow.x = ValueRect.left + nMinusXPos;
	ptLow.y = ValueRect.top+ValueRect.Height()/2;
	
	// 종가
	ValueRect.left = ValueRect.right = ValueRect.top  = ValueRect.bottom = -1;
	GetCellRect(nCRow, nCol, ValueRect);
	ptClose.x = ValueRect.left + nMinusXPos;
	ptClose.y = ValueRect.top+ValueRect.Height()/2;

	
	BOOL	bHRowVisible, bLRowVisible, bORowVisible, bCRowVisible;
	bHRowVisible = bLRowVisible = bORowVisible = bCRowVisible = FALSE;

	// Visible Check
	bORowVisible = IsCellVisible(nORow, nCol);
	bHRowVisible = IsCellVisible(nHRow, nCol);
	bLRowVisible = IsCellVisible(nLRow, nCol);
	bCRowVisible = IsCellVisible(nCRow, nCol);


	// color
	COLORREF colorBongGraph;
	if( m_pChartData->nClose < m_pChartData->nOpen)
		colorBongGraph = COLOR_BLUE;
	else if( m_pChartData->nClose > m_pChartData->nOpen)
		colorBongGraph = COLOR_RED;
	else
		colorBongGraph = COLOR_BLACK;

	
	CRect grid_rect;
	GetCellRect(m_nLowLimitRow, nCol, grid_rect);
	grid_rect.bottom -= grid_rect.Height()+1;
//	GetWindowRect(&grid_rect);
//	grid_rect.bottom -= ROW_HEIGHT*3;//GetFixedRowBtmHeight()

	CRect rectFixedRow;			
	GetCellRect(GetFixedRowCount()-1, nCol, rectFixedRow);
	rectFixedRow.bottom += 1;		// 상단고정열 라인 안보이게
	
	
	//상하 가느다란 선이 해당 컬럼의 left보다 좌측으로 치우친 좌표면
	if(ptHigh.x < rectFixedRow.left )
	{				
		ptHigh.x = rectFixedRow.left + BONGGRAPH_LEFT_OFFSET;
	}	

	// ************** 고가, 저가 (Line) ***************************************************************
	//if(ptHigh.x > first_rect.right )	// 미사용
	BOOL bHighLowDraw = FALSE;
	if ( bHRowVisible && bLRowVisible)
	{
		bHighLowDraw = TRUE;
	}
	else if( !bHRowVisible && bLRowVisible)
	{
		if (nLRow == m_nLowLimitRow && ptHigh.y >= ptLow.y)
			ptHigh.y = ptLow.y-GetRowHeight(nLRow)/2;
		else
			ptHigh.y = rectFixedRow.bottom;
		bHighLowDraw = TRUE;
	}
	else if( bHRowVisible && !bLRowVisible)
	{
		if (nHRow == HIGH_ROW && ptHigh.y >= ptLow.y)
			ptLow.y = ptHigh.y+GetRowHeight(nHRow)/2;
		else
			ptLow.y = grid_rect.bottom;
		bHighLowDraw = TRUE;
	}
	else
	{
		for(int nFRow = GetFixedRowCount(); nFRow < GetRowCount(); nFRow++)
		{
			if(IsCellVisible(nFRow, nCol)) //첫번째로 보이는 놈을 찾았을 때 
			{
				if(nFRow > nHRow && nFRow <= nLRow) // 고가Row < nFRow <= 저가Row
				{
					ptHigh.y = rectFixedRow.bottom;
					ptLow.y = grid_rect.bottom;
					bHighLowDraw = TRUE;
	...
	if (bHighLowDraw)
	{	
		ptHigh.x += BONGGRAPH_WIDTH / 2;
		pDC->FillSolidRect( ptHigh.x, ptHigh.y,	BONGGRAPH_PEN_WIDTH, ptLow.y-ptHigh.y ,	colorBongGraph);
		ptHigh.x -= BONGGRAPH_WIDTH / 2;
	}

	// 미니봉
	ptHigh.x += BONGGRAPH_WIDTH / 2;
	pDC->FillSolidRect( ptHigh.x, m_nMiniBong[1], BONGGRAPH_PEN_WIDTH, m_nMiniBong[2]-m_nMiniBong[1], colorBongGraph);	//Mini
	ptHigh.x -= BONGGRAPH_WIDTH / 2;
	// **************************************************************************************************
	...
	// ************** 시가, 종가 (Bar) ******************************************************************
	BOOL bOpenCloseDraw;
	if( m_pChartData->nClose < m_pChartData->nOpen)			// 시가 > 종가
	{
		bOpenCloseDraw = FALSE;
		if ( bORowVisible && bCRowVisible)
		{
			bOpenCloseDraw = TRUE;
		}
		else if( !bORowVisible && bCRowVisible)
		{
			if (nCRow == m_nLowLimitRow && ptOpen.y >= ptClose.y)
				ptOpen.y = ptClose.y-GetRowHeight(nCRow)/2;
			else
				ptOpen.y = rectFixedRow.bottom;
			bOpenCloseDraw = TRUE;
			
		}
		else if( bORowVisible && !bCRowVisible)
		{
			if (nORow == HIGH_ROW && ptOpen.y >= ptClose.y)
				ptClose.y = ptOpen.y+GetRowHeight(nORow)/2;
			else
				ptClose.y = grid_rect.bottom;
			bOpenCloseDraw = TRUE;
		}
		else
		{
			for(int nFRow = GetFixedRowCount(); nFRow < GetRowCount()-2; nFRow++)
			{
				if(IsCellVisible(nFRow, nCol)) //첫번째로 보이는 놈을 찾았을 때 
				{
					if(nFRow > nORow && nFRow <= nCRow) // 시가Row < nFRow <= 종가Row
					{
						ptOpen.y = rectFixedRow.bottom;
						ptClose.y = grid_rect.bottom;
						bOpenCloseDraw = TRUE;
		...
		if ( bOpenCloseDraw )
		{
			if(ptOpen.y == ptClose.y)	// 시가와 종가 포인트가 같은 경우
				pDC->FillSolidRect( ptHigh.x, ptOpen.y, BONGGRAPH_WIDTH, 1, colorBongGraph);
			else
				pDC->FillSolidRect( ptHigh.x, ptOpen.y, BONGGRAPH_WIDTH, ptClose.y-ptOpen.y, colorBongGraph);
			//CRect rect(ptHigh.x, ptOpen.y, ptHigh.x+BONGGRAPH_WIDTH, ptClose.y);	
			//GradientFill(pDC, rect, colorBongGraph);
			//pDC->Draw3dRect(ptHigh.x+1, ptOpen.y, BONGGRAPH_WIDTH-1, ptClose.y-ptOpen.y+1, colorBongGraph, colorBongGraph);
		}
		
		// 미니봉
		pDC->FillSolidRect( ptHigh.x, m_nMiniBong[0], BONGGRAPH_WIDTH, m_nMiniBong[3]-m_nMiniBong[0], colorBongGraph);	// Mini
		//CRect rect2(ptHigh.x, m_nMiniBong[0], ptHigh.x+BONGGRAPH_WIDTH, m_nMiniBong[3]);
		//GradientFill(pDC, rect2, colorBongGraph);
		//pDC->Draw3dRect(ptHigh.x+1, m_nMiniBong[0], BONGGRAPH_WIDTH-1, m_nMiniBong[3]-m_nMiniBong[0]+1, colorBongGraph, colorBongGraph);
	}
	else if( m_pChartData->nClose > m_pChartData->nOpen)	// 시가 < 종가
	{
		bOpenCloseDraw = FALSE;
		if ( bORowVisible && bCRowVisible)
		{
		...
		{
			if (nCRow == HIGH_ROW && ptOpen.y <= ptClose.y)
				ptOpen.y = ptClose.y+GetRowHeight(nCRow)/2;
			else
				ptOpen.y = grid_rect.bottom;
			bOpenCloseDraw = TRUE;
			
		}
		else if( bORowVisible && !bCRowVisible)
		{
			if (nORow == m_nLowLimitRow && ptOpen.y <= ptClose.y)
				ptClose.y = ptOpen.y-GetRowHeight(nORow)/2;
			else
				ptClose.y = rectFixedRow.bottom;
			bOpenCloseDraw = TRUE;
		}
		else
		{
			for(int nFRow = GetFixedRowCount(); nFRow < GetRowCount(); nFRow++)
			{
				if(IsCellVisible(nFRow, nCol)) //첫번째로 보이는 놈을 찾았을 때 
				{
					if(nFRow > nCRow && nFRow <= nORow) // 종가Row < nFRow <= 시가Row
					{
						ptOpen.y = grid_rect.bottom;
						ptClose.y = rectFixedRow.bottom;
						bOpenCloseDraw = TRUE;
		...
		{
			if(ptOpen.y == ptClose.y)	// 시가와 종가 포인트가 같은 경우
				pDC->FillSolidRect( ptHigh.x, ptClose.y, BONGGRAPH_WIDTH, 1, colorBongGraph);
			else
				pDC->FillSolidRect( ptHigh.x, ptClose.y, BONGGRAPH_WIDTH, ptOpen.y-ptClose.y, colorBongGraph);
			//CRect rect(ptHigh.x, ptClose.y, ptHigh.x+BONGGRAPH_WIDTH, ptOpen.y);	
			//GradientFill(pDC, rect, colorBongGraph);
			//pDC->Draw3dRect(ptHigh.x+1, ptClose.y, BONGGRAPH_WIDTH-1, ptOpen.y-ptClose.y+1,	colorBongGraph, colorBongGraph);
		}

		// 미니봉
		pDC->FillSolidRect( ptHigh.x, m_nMiniBong[3], BONGGRAPH_WIDTH, m_nMiniBong[0]-m_nMiniBong[3], colorBongGraph);	// Mini
		//CRect rect2(ptHigh.x, m_nMiniBong[3], ptHigh.x+BONGGRAPH_WIDTH, m_nMiniBong[0]);
		//GradientFill(pDC, rect2, colorBongGraph);
		//pDC->Draw3dRect(ptHigh.x+1, m_nMiniBong[3], BONGGRAPH_WIDTH-1, m_nMiniBong[0]-m_nMiniBong[3]+1, colorBongGraph, colorBongGraph);
	}
	else
	{
		if ( bCRowVisible)
		{
			pDC->FillSolidRect( ptHigh.x, ptClose.y-1, BONGGRAPH_WIDTH, 2, colorBongGraph);
		}
		
		// 미니봉
		pDC->FillSolidRect( ptHigh.x, m_nMiniBong[3]-1,	BONGGRAPH_WIDTH, 2, colorBongGraph);	// Mini
	}
	// **************************************************************************************************
}


void CGridCtrl::GradientFill(CDC* pDC, CRect rect, COLORREF color)
{
	...
	nCurBlock=rect.Width();
	nWidth=rect.Width();
	nWidth_x_2=nWidth*nWidth;
	nDelta=1;
	...
	while(nCurBlock>0) 
	{
		...
		//	빨녹파 -> 흰색
		PaintGradientRect(	pDC, rect.left + nCurBlock, rect.top, nDelta, rect.Height()+1,	
							RGB(red+((255-red)*nRest_x_2)/nWidth_x_2, 
							green+((255-green)*nRest_x_2)/nWidth_x_2, 
							blue+((255-blue)*nRest_x_2)/nWidth_x_2) );
...
void CGridCtrl::PaintGradientRect(CDC* pDC, int x, int y, int w, int h, COLORREF color)
{
	CBrush brush(color);
...
//m_idTopLeftCell의 Row값을 강제로 변경시킨다.
void CGridCtrl::SetVScrollPosForReOrder(int nRow)
{
	//m_idTopLeftCell.row = nRow;
	if(nRow > 0)
	{
		//EnsureVisible(CCellID(nRow, 0));

		int nHeight = 0;
		for ( int i = 0; i < nRow-GetFixedRowCount(); i++)
			nHeight += GetRowHeight(i);

		SetScrollPos32(SB_VERT, nHeight);
	}

	//SetScrollPos32(SB_VERT, 37, TRUE);
	//SetScrollPos(SB_VERT);
}

void CGridCtrl::Call_ResetScrollBars()
{
	ResetScrollBars();
}

void CGridCtrl::SetBtmCancelArea(BOOL bBtmCancelArea)
{
	m_bBtmCancelArea = bBtmCancelArea;
}

BOOL CGridCtrl::GetBtmCancelArea()
{
	 return m_bBtmCancelArea;
}

void CGridCtrl::SetShowInfoStatic(BOOL bShowInfoStatic)
{
	m_bShowInfoStatic = bShowInfoStatic;
}
// *******************************************************************************************	