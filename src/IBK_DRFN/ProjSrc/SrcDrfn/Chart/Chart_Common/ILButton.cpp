...
void CILButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
  POINT mouse_position;
	...
	pDC->FillSolidRect(rc, pDC->GetBkColor()); //KHD :배경 찌꺼기 없애기
	if(m_pImageList == NULL || m_pImageList->GetSafeHandle() == NULL)
		return;
	...
	else
	{
		...
		else if(m_bHover && m_nOver != m_nNormal)
		{
			m_pImageList->Draw(pDC,m_nOver, CPoint(0,0), ILD_TRANSPARENT);
		}
		else																// 보통상태
		{
...
//		CFont *pFont = CFont::FromHandle((HFONT)::GetStockObject(DEFAULT_GUI_FONT));
		CFont* pOldFont=pDC->SelectObject(&m_Font);			// get text font
...
void CILButton::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if(m_nOver != m_nNormal)
	{
		if(!m_bHover)
		{
			m_bHover = TRUE;
			Invalidate();
			SetTimer(UM_TIMER, 100, NULL);
		}
...
void CILButton::OnTimer(UINT nIDEvent)
{
	CPoint pt;
	CRect rcClient;
    ...
    if (!rcClient.PtInRect(pt) || (WindowFromPoint(pt) != this))
	{ 
        KillTimer(UM_TIMER); 
