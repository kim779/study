...
public:
	...
	bool MoveAllVertScaleGroupsInHeadBlock(const bool bDoesMaintainOldScale = true);
	bool MoveAllVertScaleGroupsInPartition(const CBlockIndex& targetBlockIndex, const bool bDoesMaintainOldScale = true);
	...
	bool ChangeAllFunctionNameAndSubGraphNames(const CString& targetFunctionName, const CString& sourceFunctionName, const bool bIsSubGraphCheck = true);
	...
	bool ChangeFunctionNameAndSubGraphNames(const CBlockIndex& blockIndex, const CString& targetFunctionName, const CString& sourceFunctionName, const bool bIsSubGraphCheck = true);
	// graph packetName 변경.
	bool ChangePacketNames(const CString& targetFunctionName, const CString& sourcePacketNames, const bool bIsChangeIndicatorInfo = true);
	// graph Data 변경.
	...
	bool ChangeGraphDataDrawingData(const CString& targetFunctionName, 
		const bool bLowHigh, const bool bLowCur, const bool bHighCur, const bool bPrevCur, const int nCurData);
	// ViewMinMaxDrawingData 변경.
	bool ChangeViewMinMaxDrawingData(const CString& targetFunctionName, 
		const bool bIsMinDrawing, const bool bIsMaxDrawing, const bool bIsSignDrawing, const int nTextColorType);
	// graph type 변경.
	...
	bool ChangeSubGraphTitleFromIndicator(const CString& strFunctionName, const int nSubGraphIndex);
	// subGraph color 변경.
	bool ChangeAllHeadSubGraphColor(const CList<CGraphColor, CGraphColor&>& graphColorList) const;
	bool ChangeAllHeadSubGraphColor(const CBlockIndex& blockIndex, const CList<CGraphColor, CGraphColor&>& graphColorList) const;
	...
	// -> 해당 graph를 Map Default IndicatorInfo로 변경(bIsAutoMake = true : MapDefault가 없는 경우 생성).	
	bool ChangeAllMapDefaultIndicatorInfo(const bool bIsAutoMake = true);
	bool ChangeMapDefaultIndicatorInfo(const CString& strFunctionName, const bool bIsAutoMake = true);
	...
	bool SetSubGraphHiding(const CString& subGraphTitle, const bool bIsHiding = true);
	bool SetSubGraphHiding(const CString& strIndicatorName, const int nSubGraphIndex, const bool bIsHiding = true);
	...
	bool GetAllHeadSubGraphColor(const CBlockIndex& blockIndex, CList<CGraphColor, CGraphColor&>& graphColorList) const;
	...
	bool GetGraphDataDrawingData(const CString& strFunctionName, 
		bool& bLowHigh, bool& bLowCur, bool& bHighCur, bool& bPrevCur, int& nCurData);
	// ViewMinMaxDrawingData 변경.
	bool GetViewMinMaxDrawingData(const CString& strFunctionName, 
		bool& bIsMinDrawing, bool& bIsMaxDrawing, bool& bIsSignDrawing, bool& nTextColorType);
