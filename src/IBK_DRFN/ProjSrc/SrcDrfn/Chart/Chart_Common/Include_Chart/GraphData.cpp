...
CString CGraphData::GetData() const
{
	...
	strGraphData.Format("%d;%s;%s;%d;%s%s\r\n", m_nFunctionIndex, m_strFunctionName, m_strPacketName, (int)m_bSignal, 
		m_graphDataDrawingData.GetData(), m_viewMinMaxDrawingData.GetData());
	return strGraphData;
}
...
void CGraphData::SetData(const CString& strData)
{
	...
	//m_graphDataDrawingData = CGraphDataDrawingData(strRealData);
