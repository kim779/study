// IWChartManager050531.cpp: implementation of the CIWChartManager050531 class.
//
//////////////////////////////////////////////////////////////////////
...
#include "stdafx.h"
#include "IWChartManager050531.h"
...
CIWChartManager050531::CIWChartManager050531( CMainBlock *p_pMainBlock)
{
	// (2004.10.11, 배승원) MainBlock Object의 Pointer를 관리한다.
	m_pMainBlock = p_pMainBlock;
	if( !m_pMainBlock)
	{
		AfxMessageBox( "CIWChartManager050531 Null Wrapping Error!");
		return;
	}
}

CIWChartManager050531::~CIWChartManager050531()
{
...
int CIWChartManager050531::GetRowCount( void)
{
	if( m_pMainBlock) return m_pMainBlock->GetRowCount();
	return 0;
}
int CIWChartManager050531::GetColumnCount( void)
{
...
// Block Region
CRect CIWChartManager050531::GetBlockRegion( int p_nRowIndex, int p_nColumnIndex)
{
...
CRect CIWChartManager050531::GetNoFrameGraphRegion( int p_nRowIndex, int p_nColumnIndex)	// Block Region
{
	if( !m_pMainBlock) return CRect( 0, 0, 0, 0);
	CBlockIndex blockIndex( p_nRowIndex, p_nColumnIndex);
	return m_pMainBlock->GetNoFrameGraphRegion( blockIndex);
}
CRect CIWChartManager050531::GetNoFrameGraphRegion( CPoint p_pPoint)
{
	if( m_pMainBlock) return m_pMainBlock->GetNoFrameGraphRegion( p_pPoint);
	return CRect( 0, 0, 0, 0);
}
// View의 Graph Drawing영역이다. (실제 Graph의 좌표 영역이다.)
CRect CIWChartManager050531::GetDrawingGraphRegion( int p_nRowIndex, int p_nColumnIndex)	// Graph Region
{
	CBlockIndex blockIndex( p_nRowIndex, p_nColumnIndex);
	if( m_pMainBlock) return m_pMainBlock->GetDrawingGraphRegion( blockIndex);
	return CRect( 0, 0, 0, 0);
}
CRect CIWChartManager050531::GetDrawingGraphRegion( CPoint p_pPoint)
{
...
BOOL CIWChartManager050531::CheckGraphInBlock( int p_nRowIndex, int p_nColumnIndex, const char *p_szGraphName)
{
...
BOOL CIWChartManager050531::GetPacketNameOfGraph( const char *p_szGraphName, CString &p_strPacketNames)
{
	if( !m_pMainBlock) return FALSE;
...
BOOL CIWChartManager050531::GetBlockIndexOfGraph( const char *p_szGraphName, int &p_nR, int &p_nC)
{
...
BOOL CIWChartManager050531::GetMinMaxDataInViewWithGraph( const char *p_szGraphName, double &p_dMin, double &p_dMax)
{
...
BOOL CIWChartManager050531::GetVerticalScaleConversionType( int p_nR, int p_nC, int &p_bLog, int &p_bReverse)
{
	// 0. 먼저 MainBlock DLL을 확인한다.
	if( !m_pMainBlock) return FALSE;
	...
	BOOL bResult = m_pMainBlock->GetVertScaleLogAndInvertType( blockIndex, bLog, bReverse);
	p_bReverse	= bReverse;
	p_bLog		= bLog;
