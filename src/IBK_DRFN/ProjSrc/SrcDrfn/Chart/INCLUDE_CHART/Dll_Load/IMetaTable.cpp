...
#include "../../../Inc/ProgIDInfo.h"		// for CProgIDInfo
#include "../MapStringToWord.h"				// for CMapStringToWord
#include "../../../ForSite/FS_Env.h"		// For Directory
...
void IMetaTable::LoadMetaTable( void)
{
	if( !m_szMetaTables)
	{
		...
		{
			GetDir_DrfnBin(strOcxPath);
			strOcxPath += "\\";
		}
		if(m_strHtsFolderPath.IsEmpty()== FALSE)
		{
			LoadDll( m_strHtsFolderPath + "ChartLanguageTable.dll");
		}
		else
		...
		{
			GetDir_DrfnData(m_strDataFolderPath);
			m_strDataFolderPath += "\\";
		}
...
BOOL IMetaTable::IsIndicatorInThisMode( const char *p_szIndicatorName, CChartInfo::CHART_MODE p_eChartMode)
{
	...
	switch( p_eChartMode)
	{
		case CChartInfo::FX: switch( GetIndicatorID( p_szIndicatorName))
			{
				case EDDC_OCX_VOLUMEFORSALE:
				case EDDC_ST_ADLINE:
				case EDDC_VOLUMECHART:
				...
				case EDDC_OCX_MARKETPROFILECHART:		return FALSE;
			}
			break;
		case CChartInfo::WORLD_ON: switch( GetIndicatorID( p_szIndicatorName))
			{
				case EDDC_ORGAN_PUREBUY_ACCUMULATION:
				case EDDC_ORGAN_PUREBUY:
				case EDDC_SUSPENSE_CONTRACT:
				...
				case EDDC_PROGRAM_TRADE_PUREBUY_KOSPI:
				case EDDC_PROGRAM_TRADE_PUREBUY_KOSDAQ:
				case EDDC_VOLUMEPRICE:						return FALSE;
			}
			break;
	}
	return TRUE;
