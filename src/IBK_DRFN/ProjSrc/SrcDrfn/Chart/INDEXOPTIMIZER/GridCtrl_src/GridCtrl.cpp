...
BOOL CGridCtrl::RegisterWindowClass()
{
	...
	HINSTANCE hInst = AfxGetInstanceHandle();		//=== 2007.06.04. SSH : Class등록 오류로 수정 (무슨 이유로 Resource Handle로 등록하려 했을까?)
...
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	//KHD: 수정 : Sort ,로 인해 잘 못 연산하는 것 수정
	CString str1,str2;
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
            ...
            if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
            {
				for (int col = 0; col < GetColumnCount(); col++)
                {
					//만약 Sort 구분을 하려면.... 여기서 추가 해줘야한다.KHD
...
//포맷 추가 : KHD
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str,DWORD nFormat)
{
    if (GetVirtualMode())
        return FALSE;
