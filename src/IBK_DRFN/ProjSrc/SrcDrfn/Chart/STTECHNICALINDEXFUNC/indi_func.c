...
*/
	...
	// 0024976: [7111] 종합차트 화면 - 특정 종목에서 'Bollinger Bands' 지표 추가 시 오류 현상 발생
	if (lIndex + 1 < lPeriod || lPeriod < 1)
	{
		return ldRetVal;
	}
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	double *ldpFator;
	double *ldpLastIndex;
	double *ldpSetLastEMA;

    ldpTemp = GET_TEMP_DBL(TEMP_VAR);
    if (ldpTemp == NULL) return ldRetVal;
	ldpFator = &ldpTemp[0];
	ldpLastIndex = &ldpTemp[1];
	ldpSetLastEMA = &ldpTemp[2];
...
*/
	...
	if (*ldpCurrentIndex >= lPeriod && lIndex >= lPeriod)
	{
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	double dStdVal = yesStd(lIsUpdateData, lIndex, lPeriod, SRC_VAR, 
		GET_SUB_TEMP(TEMP_VAR, 2));
	if(dStdVal == NONAVAILABLE || dStdVal == -NONAVAILABLE)
		dStdVal = 0.;
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if(lIndex == 1)
	{
		SET_AT(OPT_VAR2, lIndex, ldValue);
		return ldValue;
	}
...
*/
	...
	if(ldMa == NONAVAILABLE || ldMa == -NONAVAILABLE)
	{
		return NONAVAILABLE;
	}
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	double ldPMF = yesAccumN(lIsUpdateData, lIndex, lPeriod, OPT_VAR1, 
		GET_SUB_TEMP(TEMP_VAR, 1));
	double ldNMF = yesAccumN(lIsUpdateData, lIndex, lPeriod, OPT_VAR2, 
		GET_SUB_TEMP(TEMP_VAR, 2));

	if(ldPMF == NONAVAILABLE || ldNMF == NONAVAILABLE)
		return NONAVAILABLE;
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if (lIndex < 1)
	{
		SET_AT(OPT_VAR, lIndex, ldVolume);
	...
	{
		ldPVTOld = GET_AT(OPT_VAR, lIndex - 1);
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	{
		//ldRetVal = ldMa1 / ldMa2 * 100.;
		ldRetVal = 100. - (100. / (1. + (ldMa1 / ldMa2)));
	}
	else
	{
		ldRetVal = 0.;
...
//	if (lIndex + 1 < lPeriod || lPeriod < 1 || lIndex + 1 < mPeriod || mPeriod < 1)
	...
	ldVal = yesEMA(lIsUpdateData, lIndex, lPeriod, SRC_VAR, OPT_VAR1,
					GET_SUB_TEMP(TEMP_VAR, 1));
	SET_AT(OPT_VAR2, lIndex, ldVal);
...
//	if (lIndex + 1 < lPeriod + mPeriod)
	...
	ldPrevVal = GET_AT(OPT_VAR2, lIndex - mPeriod);
	ldRetVal = ldVal - ldPrevVal;
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	ldRetVal = (yesAccumN(lIsUpdateData, lIndex, lPeriod, OPT_VAR, 
				GET_SUB_TEMP(TEMP_VAR, 1)) / lPeriod) * 100.;
...
//>> TRIX 수정(리얼처리시 버그) - vntsorl(20081209)
	//{{ Prev
		...
		TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if (ldVal1 - ldVal2)
	{
		ldRetVal = ((ldClose - ldVal1) / (ldVal1 - ldVal2)) * 100.;
	}
	else
	{
		ldRetVal = 0.;
					...
					TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if(dATR == NONAVAILABLE || dATR == -NONAVAILABLE)
	{
		return NONAVAILABLE;
	}
					...
					TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if(dAD == NONAVAILABLE || dAddVolume == NONAVAILABLE)
		return NONAVAILABLE;
				...
				TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if(ldBBUpper == NONAVAILABLE || ldBBLower == NONAVAILABLE ||
		ldBBUpper == ldBBLower)
		return NONAVAILABLE;
				...
				TEMP_DATA_TYPE TEMP_VAR)
{
	...
	if(ldBBUpper == NONAVAILABLE || ldBBCenter == NONAVAILABLE ||ldBBLower == NONAVAILABLE)
		return NONAVAILABLE;
...
//	ldTR = (dValue1 > dValue3? ((dValue1 > dValue2)? dValue1 : dValue2) : dValue3);
	// 20121112 이상신 : 한투와 동일하게 수정.
	ldTR = (dValue1 > dValue3? ((dValue1 > dValue2)? dValue1 : dValue2) : ((dValue3 > dValue2)? dValue3 : dValue2));

	SET_AT(OPT_VAR2, lIndex, ldTR);

	if(lIndex == 1)
	{
		SET_AT(OPT_VAR4, lIndex, ldTR);
		ldTRn = ldTR;
	}
	else
