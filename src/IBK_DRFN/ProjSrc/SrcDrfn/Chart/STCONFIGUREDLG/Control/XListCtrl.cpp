...
#endif
#ifndef DO_NOT_INCLUDE_XEDITBOX	
	ON_REGISTERED_MESSAGE( WM_XEDIT_VK_RETURN, OnEditReturn )
	ON_REGISTERED_MESSAGE( WM_XEDIT_VK_ESCAPE, OnEditEscape )	
#endif
END_MESSAGE_MAP()
...
BOOL CXListCtrl::PreTranslateMessage(MSG* pMsg) 
{
	...
	if(pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN)   
	{
		...
		{
			m_pEdit->GetWindowText( clstrText );
			OnEditReturn((WPARAM)(LPCTSTR)clstrText, (LPARAM)0);
		}
		return TRUE;
	}
	...
	{
		if(m_pEdit && m_pEdit->GetSafeHwnd())
			OnEditEscape((WPARAM)0, (LPARAM)0);
		return FALSE;
	}
...
BOOL CXListCtrl::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(pXLCD[pnmlv->iSubItem].bUseEditBox)
	{
		...
		{
			CRect rect;
			m_pEdit->GetWindowText(m_strEdit);
			SetItemText( m_nEditItem, m_nEditSubItem, m_strEdit );	
			m_nEditItem = pnmlv->iItem;
			m_nEditSubItem = pnmlv->iSubItem;
			m_strEdit = GetItemText( m_nEditItem, m_nEditSubItem );
			GetSubItemRect(m_nEditItem, m_nEditSubItem , LVIR_LABEL, rect );
...
LRESULT CXListCtrl::OnEditEscape( WPARAM wParam, LPARAM lParam )
{	
	LRESULT lResult = NULL;
	CString strEditNow;
	m_pEdit->GetWindowText(strEditNow);
	m_pEdit->DestroyWindow();
	delete m_pEdit;
	m_pEdit = NULL;
	if(m_strEdit==strEditNow)
	{
		return lResult;
	}
	SetItemText( m_nEditItem, m_nEditSubItem, strEditNow );		
	return lResult;
}

void CXListCtrl::UnpressEditBox()
{
	if(m_pEdit) 
	{
		CRect rect;
