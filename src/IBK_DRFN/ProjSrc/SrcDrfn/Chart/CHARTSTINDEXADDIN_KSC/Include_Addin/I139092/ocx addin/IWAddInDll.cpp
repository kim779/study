...
IAddInDllBase::EAddInEventType	CIWAddInDll::GetGlobalAddInEventType( const long p_eAddInEventType)
{
	...
	switch( ( EAddInEventType)p_eAddInEventType)
	{
		...
		case IChartAddIn::ADDINEVENT( OnAddInToolCommand_EAICommandType):	eAddInEventType = IAddInDllBase::EAddInEventType::ADDINEVENT( OnAddInToolCommand_EAICommandType);
																			...
																			AddAdditionalRequestEvent( IAddInDllBase::EAddInEventType::ADDINEVENT( OnPacketData));			// for EAI_ALL_RESET_CURRENT_TOOL
		...
		case IChartAddIn::ADDINEVENT( OnUpdatePacketData):	AddAdditionalRequestEvent( IAddInDllBase::EAddInEventType::ADDINEVENT( OnUserIndiatorConditionLoaded));
															AddAdditionalRequestEvent( IAddInDllBase::EAddInEventType::ADDINEVENT( OnPacketData));
															...
															AddAdditionalRequestEvent( IAddInDllBase::EAddInEventType::ADDINEVENT( OnPacketTruncated));				// for EAI_ST_REMOVED_DATA_COUNT //JS.Kim_20100908 데이타건수 초과시 삭제할때 지표 반영안되는 문제 수정
															break;
		case IChartAddIn::ADDINEVENT( OnAddInToolCommand_EAICommandType_char_char):	eAddInEventType = IAddInDllBase::EAddInEventType::ADDINEVENT( OnAddInToolCommand_EAICommandType_char_char);
		...
		CASE_ADDINEVENT( OnUpdatePacketDataMultiItem)
	}
...
void CIWAddInDll::OnPacketData( const char *p_szPacketNames, BOOL p_bWithReset)
{
...
{
	OnAddInToolCommand( EAI_ALL_ON_DELETE_INDICATOR, p_szIndicatorName, p_szGraphName);
}
...
void CIWAddInDll::OnRealPacketEnd( void)
{
	OnUpdatePacketData();
}

// 복수종목별 Real반영 : 복수종목 - ojtaso (20070131)
void CIWAddInDll::OnRealPacketEndMultiItem( LPCTSTR lpszRQ)
{
	OnUpdatePacketDataMultiItem(lpszRQ);
}
