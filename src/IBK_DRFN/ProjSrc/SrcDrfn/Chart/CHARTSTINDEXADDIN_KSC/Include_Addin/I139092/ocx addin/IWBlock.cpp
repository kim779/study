...
{
	if( !m_pBlock) return FALSE;
	return m_pBlock->GetLogAndReverseScaleFlag( p_bLogScale, p_bReverseScale);
}

BOOL CIWBlock::SetLogAndReverseScaleFlag( BOOL p_bLogScale, BOOL p_bReverseScale)
{
	if( !m_pBlock) return FALSE;
	return m_pBlock->SetLogAndReverseScaleFlag( p_bLogScale, p_bReverseScale);
}
...
//////////////////////////////////////////////////////////////////////
IPacket *CIWBlock::GetPacket( const char *p_szPacketName)
{
	if( !m_pBlock) return NULL;

	IPacket *pIPacket = NULL;
	CPacket *pPacket = m_pBlock->GetPacket( p_szPacketName);
	if( !pPacket) return NULL;
	return CIWPacket::CreateObject( pPacket);
}
