...
protected:
	...
	virtual BOOL	SetScrollData( const int nType = SETSCROLL_TR, const BOOL bIsHorzScaleDataCheck = TRUE, const BOOL bIsRedraw = FALSE);
...
protected:	// Get
	...
	virtual	int			GetStyleValueGridHLineType(		void);
	virtual	int			GetStyleValueGridVLineType(		void);
...
public:	// SetAllHorzScaleGridType
	virtual	BOOL		SetStyleValueGridHLineType(		int			p_nGridHLineType,	BOOL p_bWithUpdate = FALSE);
public:	// SetAllVertScaleGridType
	virtual	BOOL		SetStyleValueGridVLineType(		int			p_nGridVLineType,	BOOL p_bWithUpdate = FALSE);
