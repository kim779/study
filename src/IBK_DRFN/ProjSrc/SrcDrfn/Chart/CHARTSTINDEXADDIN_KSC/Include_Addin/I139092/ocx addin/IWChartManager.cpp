...
{
	if( !m_pMainBlock) return FALSE;
	return m_pMainBlock->GetIndexOfGraph( p_szRQ, p_szGraphName, p_nR, p_nC, p_nV, p_nG);
}
...
BOOL CIWChartManager::FindFirstIndicator( const char *p_szIndicatorName, int &p_nR, int &p_nC, int &p_nV, int &p_nG)
{
	...
	return m_pMainBlock->FindFirstIndicator( p_szIndicatorName, p_nR, p_nC, p_nV, p_nG, strGraphName);
}
...
POSITION CIWChartManager::GetSubGraphViewData_First( void)
{
	...
	if( !m_pMainBlock->GetViewDataListInAllBlock( *sgvdIterator.m_pViewData)) return NULL;
