...
CMChartInterface *CIWChartOCX::GetMChartInterface( void)
{
	if( !m_pIChartCtrl) return NULL;
...
{
	if( !m_pIChartCtrl) return FALSE;
	return m_pIChartCtrl->GetRedrawAfterUpdatedAllPacketFlag();
}
...
{
	if( m_pIChartCtrl) return m_pIChartCtrl->SetDataStartEnd( p_nStartIndex, p_nEndIndex);
	return FALSE;
}
...
{
	if( !m_pIChartCtrl) return FALSE;
	return m_pIChartCtrl->SetScrollData( nType, bIsHorzScaleDataCheck, bIsRedraw);
}
...
{
	if( !m_pIChartCtrl) return 0;
	return m_pIChartCtrl->GetRightMargin();
}

BOOL CIWChartOCX::SetRightMargin( const int p_nCount)
{
	if( !m_pIChartCtrl) return 0;
	m_pIChartCtrl->SetRightMargin( p_nCount);
	return TRUE;
}
...
bool CIWChartOCX::SetStartIndexOfPacket(const char* pcszPacketName, int nStartDataIndex, BOOL bAddSpace)
{
	if(!m_pIChartCtrl)
		return false;
	return m_pIChartCtrl->SetStartIndexOfPacket(pcszPacketName, nStartDataIndex, bAddSpace);
}
...
void CIWChartOCX::SetRealData(const char* pcszPacketName, double dData, short nOption)
{
	if(!m_pIChartCtrl)
		return;
	m_pIChartCtrl->SetRealDataAdapter(pcszPacketName, dData, nOption);
}
...
int CIWChartOCX::GetStyleValueGridHLineType( void)
{
	if( !m_pIChartCtrl) return 0;
	return m_pIChartCtrl->GetStyleValueGridHLineType();
}
int CIWChartOCX::GetStyleValueGridVLineType( void)
{
	if( !m_pIChartCtrl) return 0;
	return m_pIChartCtrl->GetStyleValueGridVLineType();
}
...
BOOL CIWChartOCX::SetStyleValueGridHLineType( int p_nGridHLineType, BOOL p_bWithUpdate)
{
	if( !m_pIChartCtrl) return FALSE;
	return m_pIChartCtrl->SetStyleValueGridHLineType( p_nGridHLineType, p_bWithUpdate);
}
// SetAllVertScaleGridType
BOOL CIWChartOCX::SetStyleValueGridVLineType( int p_nGridVLineType, BOOL p_bWithUpdate)
{
	if( !m_pIChartCtrl) return FALSE;
	return m_pIChartCtrl->SetStyleValueGridVLineType( p_nGridVLineType, p_bWithUpdate);
}
// Updated in OnDraw
