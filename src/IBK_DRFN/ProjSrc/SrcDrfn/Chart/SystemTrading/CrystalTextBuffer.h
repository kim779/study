...
#if !defined(AFX_CRYSTALTEXTBUFFER_H__3B5010CA_2B3D_11D5_975A_00D0B71DFCCD__INCLUDED_)
#define AFX_CRYSTALTEXTBUFFER_H__3B5010CA_2B3D_11D5_975A_00D0B71DFCCD__INCLUDED_
...
class CCrystalTextBuffer : public CCmdTarget
{
	...
	CCrystalTextBuffer();           // protected constructor used by dynamic creation
...
#pragma pack(push, 1)
	...
	public:
		...
		{
			if(HIWORD((DWORD) m_pszText) != 0) return m_pszText;
			return m_szText;
		};
	};
...
public:
	...
	// LeeYong, 2001.6.29
	BOOL	InsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar, int nAction = CE_ACTION_UNKNOWN, BOOL bEnableUndo = TRUE);
	BOOL	DeleteText(CCrystalTextView *pSource, int nStartLine, int nStartPos, int nEndLine, int nEndPos, int nAction = CE_ACTION_UNKNOWN, BOOL bEnableUndo = TRUE);
	...
	void	UpdateViews(CCrystalTextView *pSource, CUpdateContext *pContext, DWORD dwUpdateFlags, int nLineIndex = -1);
...
protected:
	...
	// Support For Descriptions On Undo/Redo Actions
	void	AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText, int nActionType = CE_ACTION_UNKNOWN);
...
#endif // !defined(AFX_CRYSTALTEXTBUFFER_H__3B5010CA_2B3D_11D5_975A_00D0B71DFCCD__INCLUDED_)
