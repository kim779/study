...
{
	if(HIWORD((DWORD) m_pszText) != 0) delete m_pszText;
}
...
void CCrystalTextBuffer::CInsertContext::RecalcPoint(CPoint &ptPoint)
{
	...
	if(ptPoint.y < m_ptStart.y) return;
	if (ptPoint.y > m_ptStart.y)
	{
		ptPoint.y += (m_ptEnd.y - m_ptStart.y);
		return;
	}
	if(ptPoint.x <= m_ptStart.x) return;
	ptPoint.y += (m_ptEnd.y - m_ptStart.y);
...
void CCrystalTextBuffer::CDeleteContext::RecalcPoint(CPoint &ptPoint)
{
	...
	if(ptPoint.y == m_ptStart.y)
	{
		if(ptPoint.x > m_ptStart.x) ptPoint.x = m_ptStart.x;
		return;
	}
	ptPoint = m_ptStart;
}
...
/////////////////////////////////////////////////////////////////////////////
// CCrystalTextBuffer User Function

void CCrystalTextBuffer::InsertLine(LPCTSTR pszLine, int nLength, int nPosition)
{
	if(nLength == -1)
	{
		if(pszLine == NULL) nLength = 0;
		else nLength = lstrlen(pszLine);
	}
	...
	if(li.m_nMax > 0) li.m_pcLine = new TCHAR[li.m_nMax];
	if(li.m_nLength > 0) memcpy(li.m_pcLine, pszLine, sizeof(TCHAR) * li.m_nLength);

	if(nPosition == -1) m_aLines.Add(li);
	else m_aLines.InsertAt(nPosition, li);

#ifdef _DEBUG
	int nLines = m_aLines.GetSize();
	if(nLines % 5000 == 0) TRACE1("%d lines loaded!\n", nLines);
#endif
}
...
void CCrystalTextBuffer::AppendLine(int nLineIndex, LPCTSTR pszChars, int nLength)
{
	...
	if(nBufNeeded > li.m_nMax)
	{
		...
		if(li.m_nLength > 0) memcpy(pcNewBuf, li.m_pcLine, sizeof(TCHAR) * li.m_nLength);
		delete li.m_pcLine;
...
void CCrystalTextBuffer::FreeAll()
{
	...
	for( I = 0; I < nCount; I++)
	{
		if(m_aLines[I].m_nMax > 0) delete m_aLines[I].m_pcLine;
	}
	m_aLines.RemoveAll();

	// Free undo buffer
	int nBufSize = m_aUndoBuf.GetSize();
	for(I = 0; I < nBufSize; I++) m_aUndoBuf[I].FreeText();
	m_aUndoBuf.RemoveAll();

	m_bInit = FALSE;
}

BOOL CCrystalTextBuffer::InitNew(int nCrlfStyle)
{
	ASSERT(!m_bInit);
...
BOOL CCrystalTextBuffer::LoadFromFile(LPCTSTR pszFileName, int nCrlfStyle)
{
	ASSERT(!m_bInit);
	ASSERT(m_aLines.GetSize() == 0);
	...
	__try
	{
		...
		if(dwFileAttributes == (DWORD)-1) __leave;
		...
		if(!::ReadFile(hFile, pcBuf, dwBufSize, &dwCurSize, NULL)) __leave;
		...
		if(nCrlfStyle == CRLF_STYLE_AUTOMATIC)
		{
			...
			else
			{
				...
				if(I > 0 && pcBuf[I-1] == _T('\x0d')) nCrlfStyle = CRLF_STYLE_DOS;
				else
				{
					if(I < dwCurSize-1 && pcBuf[I+1] == _T('\x0d')) nCrlfStyle = CRLF_STYLE_UNIX;
					else nCrlfStyle = CRLF_STYLE_MAC;
				}
			}
		}
	...
	__finally
	{
		if(pcLineBuf) delete pcLineBuf;
		if(hFile) ::CloseHandle(hFile);
	}
	return bSuccess;
}

BOOL CCrystalTextBuffer::SaveToFile(LPCTSTR pszFileName, int nCrlfStyle, BOOL bClearModifiedFlag)
{
	ASSERT(nCrlfStyle == CRLF_STYLE_AUTOMATIC || nCrlfStyle == CRLF_STYLE_DOS|| nCrlfStyle == CRLF_STYLE_UNIX || nCrlfStyle == CRLF_STYLE_MAC);
	ASSERT(m_bInit);
	...
	__try
	{
		...
		if(::GetTempFileName(szTempFileDir, _T("CRE"), 0, szTempFileName) == 0) __leave;

		hTempFile = ::CreateFile(szTempFileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if(hTempFile == INVALID_HANDLE_VALUE) __leave;

		if(nCrlfStyle == CRLF_STYLE_AUTOMATIC) nCrlfStyle = m_nCRLFMode;
		...
		for(int nLine = 0; nLine < nLineCount; nLine ++)
		{
			...
			if(nLength > 0)
			{
				if(!::WriteFile(hTempFile, T2A(m_aLines[nLine].m_pcLine), nLength, &dwWrittenBytes, NULL)) __leave;
				if(nLength != (int) dwWrittenBytes) __leave;
			}
			if(nLine < nLineCount - 1)	// Last line must not end with CRLF
			{
				if(!::WriteFile(hTempFile, pszCRLF, nCRLFLength, &dwWrittenBytes, NULL)) __leave;
				if(nCRLFLength != (int) dwWrittenBytes) __leave;
			}
		}
		...
		if(m_bCreateBackupFile)
		{
			...
			if(hSearch != INVALID_HANDLE_VALUE)
			{
				...
				if(!::MoveFile(pszFileName, szBackupFileName)) __leave;
				::FindClose(hSearch);
		...
		if(!::MoveFile(szTempFileName, pszFileName)) __leave;
...
int CCrystalTextBuffer::FindLineWithFlag(DWORD dwFlag)
{
	...
	{
		if((m_aLines[L].m_dwFlags & dwFlag) != 0) return L;
	}
	return -1;
}
...
void CCrystalTextBuffer::SetLineFlag(int nLine, DWORD dwFlag, BOOL bSet, BOOL bRemoveFromPreviousLine)
{
	ASSERT(m_bInit);	// Text buffer not yet initialized.
						// You must call InitNew() or LoadFromFile() first!
	int nFlagIndex = ::FlagToIndex(dwFlag);
	...
	if(bSet) dwNewFlags = dwNewFlags | dwFlag;
	else dwNewFlags = dwNewFlags & ~dwFlag;
...
void CCrystalTextBuffer::GetText(int nStartLine, int nStartChar, int nEndLine, int nEndChar, CString &text, LPCTSTR pszCRLF)
{
	ASSERT(m_bInit);	// Text buffer not yet initialized.
						// You must call InitNew() or LoadFromFile() first!
...
BOOL CCrystalTextBuffer::InternalDeleteText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
	...
	else
	{
		...
		for(int L = nStartLine+1; L <= nEndLine; L++) delete m_aLines[L].m_pcLine;
	...
	if(!m_bModified) SetModified(TRUE);

	// LeeYong, 2001.6.29 for script error
	if(pSource)
		pSource->ResetScriptError();
...
BOOL CCrystalTextBuffer::InternalInsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
	...
	{
		nTextPos = 0;
		while(pszText[nTextPos] != 0 && pszText[nTextPos] != _T('\r')) nTextPos ++;

		if(nCurrentLine == nLine) AppendLine(nLine, pszText, nTextPos);
		else
		{
		...
		else ASSERT(FALSE);			// Invalid line-end format passed

		pszText += nTextPos;
	}

	if(pszRestChars) delete pszRestChars;

	context.m_ptEnd.x = nEndChar;
	context.m_ptEnd.y = nEndLine;

	if(bNewLines) UpdateViews(pSource, &context, UPDATE_HORZRANGE | UPDATE_VERTRANGE, nLine);
	else UpdateViews(pSource, &context, UPDATE_SINGLELINE | UPDATE_HORZRANGE, nLine);
...
POSITION CCrystalTextBuffer::GetUndoDescription(CString &desc, POSITION pos)
{
	ASSERT(CanUndo());		// Please call CanUndo() first
	ASSERT((m_aUndoBuf[0].m_dwFlags & UNDO_BEGINGROUP) != 0);

	int nPosition;
	if(pos == NULL) nPosition = m_nUndoPosition;	// Start from beginning
	else
	{
		nPosition = (int) pos;
	...
	nPosition--;
	while((m_aUndoBuf[nPosition].m_dwFlags & UNDO_BEGINGROUP) == 0) nPosition--;

	// Read description
	if(!GetActionDescription(m_aUndoBuf[nPosition].m_nAction, desc)) desc.Empty();	// Use empty string as description

	// Now, if we stop at zero position, this will be the last action, since we return (POSITION) nPosition
	return (POSITION) nPosition;
}

POSITION CCrystalTextBuffer::GetRedoDescription(CString &desc, POSITION pos)
{
	...
	// Advance to next undo group
	nPosition++;
	while(nPosition < m_aUndoBuf.GetSize() && (m_aUndoBuf[nPosition].m_dwFlags & UNDO_BEGINGROUP) == 0) nPosition--;

	if(nPosition >= m_aUndoBuf.GetSize()) return NULL;	// No more redo actions!
	return (POSITION) nPosition;
}
...
//
// NOTES:
//		예약어 처리에서 기존에 입력된 문자와 치환된 문자가 차이가 발생하여
//		아래 코드에서 ASSERT가 발생된다. 이를 해결하기 위해 다음 코드를 막았음.
//			ASSERT(lstrcmp(text, ur.GetText()) == 0);
		...
		if(ur.m_dwFlags & UNDO_BEGINGROUP) break;
	}
	if(m_bModified && m_nSyncPosition == m_nUndoPosition) SetModified(FALSE);
	if(!m_bModified && m_nSyncPosition != m_nUndoPosition) SetModified(TRUE);
	return TRUE;
}
...
BOOL CCrystalTextBuffer::Redo(CPoint &ptCursorPos)
{
	...
	for(;;)
	{
		...
		m_nUndoPosition ++;
		if(m_nUndoPosition == m_aUndoBuf.GetSize()) break;
		if((m_aUndoBuf[m_nUndoPosition].m_dwFlags & UNDO_BEGINGROUP) != 0) break;
	}
	if(m_bModified && m_nSyncPosition == m_nUndoPosition) SetModified(FALSE);
	if(!m_bModified && m_nSyncPosition != m_nUndoPosition) SetModified(TRUE);
	return TRUE;
...
void CCrystalTextBuffer::AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText, int nActionType)
{
	...
	{
		for(int I = m_nUndoPosition; I < nBufSize; I++) m_aUndoBuf[I].FreeText();
	...
	if(nBufSize >= m_nUndoBufSize)
	{
		...
		for(;;)
		{
			...
			nIndex ++;
			if(nIndex == nBufSize || (m_aUndoBuf[nIndex].m_dwFlags & UNDO_BEGINGROUP) != 0) break;
		}
...
// LeeYong, 2001.6.29
BOOL CCrystalTextBuffer::InsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar, int nAction, BOOL bEnableUndo)
{
	if(!InternalInsertText(pSource, nLine, nPos, pszText, nEndLine, nEndChar)) return FALSE;
	...
	if(bGroupFlag) FlushUndoGroup(pSource);
	return TRUE;
}

BOOL CCrystalTextBuffer::DeleteText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar, int nAction, BOOL bEnableUndo)
{
	CString sTextToDelete;
	GetText(nStartLine, nStartChar, nEndLine, nEndChar, sTextToDelete);

	if(!InternalDeleteText(pSource, nStartLine, nStartChar, nEndLine, nEndChar)) return FALSE;
...
#else
	if(CCrystalTextView::s_hResourceInst) AfxSetResourceHandle(CCrystalTextView::s_hResourceInst);
#endif
	BOOL bSuccess = FALSE;
	switch(nAction)
	{
		case CE_ACTION_UNKNOWN    : bSuccess = desc.LoadString(IDS_EDITOP_UNKNOWN);		  break;
		case CE_ACTION_PASTE      : bSuccess = desc.LoadString(IDS_EDITOP_PASTE);		  break;
		case CE_ACTION_DELSEL     : bSuccess = desc.LoadString(IDS_EDITOP_DELSELECTION);  break;
		case CE_ACTION_CUT        : bSuccess = desc.LoadString(IDS_EDITOP_CUT);			  break;
		case CE_ACTION_TYPING     : bSuccess = desc.LoadString(IDS_EDITOP_TYPING);		  break;
		case CE_ACTION_BACKSPACE  : bSuccess = desc.LoadString(IDS_EDITOP_BACKSPACE);	  break;
		case CE_ACTION_INDENT     : bSuccess = desc.LoadString(IDS_EDITOP_INDENT);		  break;
		case CE_ACTION_DRAGDROP   : bSuccess = desc.LoadString(IDS_EDITOP_DRAGDROP);	  break;
		case CE_ACTION_REPLACE    : bSuccess = desc.LoadString(IDS_EDITOP_REPLACE);		  break;
		case CE_ACTION_DELETE     : bSuccess = desc.LoadString(IDS_EDITOP_DELETE);		  break;
		case CE_ACTION_AUTOINDENT : bSuccess = desc.LoadString(IDS_EDITOP_AUTOINDENT);	  break;
	}
...
int CCrystalTextBuffer::FindNextBookmarkLine(int nCurrentLine)
{
	...
	if((dwFlags & LF_BOOKMARKS) != 0) nCurrentLine++;
	...
	{ 
		...
		{
			if((m_aLines[nCurrentLine].m_dwFlags & LF_BOOKMARKS) != 0) return nCurrentLine;
			nCurrentLine++;		// Keep going
		}
		// End of text reached
		if(!bWrapIt) return -1;
...
BOOL CCrystalTextBuffer::IsMBSLead(int nLine, int nCol)
{
	ASSERT(m_bInit);	// Text buffer not yet initialized.
						// You must call InitNew() or LoadFromFile() first!
	const unsigned char *string = (const unsigned char *)GetLineChars(nLine);
	const unsigned char *current = string + nCol;
	if(_ismbslead(string, current) < 0) return TRUE;
	return FALSE;
}

BOOL CCrystalTextBuffer::IsMBSTrail(int nLine, int nCol)
{
	ASSERT(m_bInit);	// Text buffer not yet initialized.
						// You must call InitNew() or LoadFromFile() first!
	const unsigned char *string = (const unsigned char *)GetLineChars(nLine);
	const unsigned char *current = string + nCol;
	if(_ismbstrail(string, current) < 0) return TRUE;
	return FALSE;
}
