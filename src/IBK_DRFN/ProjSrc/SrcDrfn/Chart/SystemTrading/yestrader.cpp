...
static BOOL IsStsKeyword(LPCTSTR pszChars, int nLength)
{
	...
	for (int L = 0; s_apszStsKeywordList_YES[L] != NULL; L ++)
	{
		if (strncmp(s_apszStsKeywordList_YES[L], szUpperChar, nLength) == 0
				&& s_apszStsKeywordList_YES[L][nLength] == 0)
		{
			::free(szUpperChar);
			return TRUE;
		}
...
DWORD CSystemTradingView::ParseLineYES(DWORD dwCookie, int nLineIndex, TEXTBLOCK *pBuf, int &nActualItems)
{
