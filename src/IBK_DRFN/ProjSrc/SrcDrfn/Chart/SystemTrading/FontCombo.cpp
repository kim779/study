...
{
	// ojtaso 수정
	if(!(lpntm->ntmTm.ntmFlags & NTM_REGULAR) || !(lpntm->ntmTm.tmPitchAndFamily == 54 || lpntm->ntmTm.tmPitchAndFamily == 22))
		return 1;
	...
	NEWTEXTMETRICEX* /* lpntm */, int FontType, LPVOID pThis)
{
	...
	// ojtaso 수정
void CFontCombo::SetDefalutFont()
{
	SetCurSel(FindStringExact(0, m_strDefault));
