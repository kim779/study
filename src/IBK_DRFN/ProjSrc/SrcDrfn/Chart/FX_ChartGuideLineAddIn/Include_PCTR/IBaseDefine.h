...
#define		HANARO_PCTRMNG_FX				"FX_PcTrMng.dll"
...
#define	AfxGetHanaroMainWndFX(hMainHwnd)								\
	{																\
		hMainHwnd=NULL;												\
		char aBufWnd[8+1]={0,};										\
		CString szClipFormat;										\
		szClipFormat.Format("%s%08X", HANARO_CLIPKEY_PCTRMAIN, ::GetModuleHandle(HANARO_PCTRMNG_FX));	\
		::GetEnvironmentVariable(szClipFormat, aBufWnd, 9);			\
		sscanf(aBufWnd,"%X", &hMainHwnd);							\
	}
// szClipFormat.Format("%s%08X", HANARO_CLIPKEY_MAINFRAME, ::GetModuleHandle(HANARO_PCTRMNG));
...
#define	AfxGetIGateManager(pGateMng)								\
	{																\
		...
		::GetEnvironmentVariable(szClipFormat, aBufWnd, 9);			\
		sscanf(aBufWnd,"%X", &pGateMng);							\
	}

#define	AfxGetIGateManagerFX(pGateMng)								\
	{																\
		pGateMng=NULL;												\
		char aBufWnd[8+1]={0,};										\
		CString szClipFormat;										\
		szClipFormat.Format("%s%08X", HANARO_CLIPKEY_GATEMANAGER, ::GetModuleHandle(HANARO_PCTRMNG_FX));	\
		::GetEnvironmentVariable(szClipFormat, aBufWnd, 9);			\
		sscanf(aBufWnd,"%X", &pGateMng);							\
	}
...
// 2 : FX
// 3 : GNF
static HWND AFXAPI AfxGetPctrMainHandle(int nChartMode)
{
	HWND hMain=NULL;
	if(nChartMode == 2)
		AfxGetHanaroMainWndFX(hMain);
	else
...
static CWnd* AFXAPI AfxGetPctrMainWnd(int nChartMode)
{
	HWND hMain = AfxGetPctrMainHandle(nChartMode);
...
static LRESULT AFXAPI AfxGetPctrInterface(int nID, int nChartMode)
{
	HWND hMainWnd;
	if(nChartMode == 2)
		AfxGetHanaroMainWndFX(hMain);
...
static LRESULT AFXAPI AfxGetPctrData(int nID, int nChartMode)
{
	HWND hMainWnd;
	if(nChartMode == 2)
		AfxGetHanaroMainWndFX(hMain);
...
static LRESULT AFXAPI AfxGetPctrDataEx(int nID, int nChartMode)
{
	HWND hMainWnd;
	if(nChartMode == 2)
		AfxGetHanaroMainWndFX(hMain);
...
static LRESULT AFXAPI AfxSetPctrData(int nID, CString strData, int nChartMode)
{
...
static LRESULT AFXAPI AfxSetPctrDataEx(int nID, COleVariant varData, int nChartMode)
{
