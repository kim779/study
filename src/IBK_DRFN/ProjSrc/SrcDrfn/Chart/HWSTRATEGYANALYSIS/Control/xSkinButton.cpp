...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetSkin(CString strnormal, CString strdown, 
						 CString strover, CString strdisabled, 
						 CString strmask, CString strfocus,
						 short drawmode, short border, short margin)
{
	...
	if (!strnormal.IsEmpty()) m_bNormal.Attach((HBITMAP)LoadImage(NULL,strnormal,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
	if (!strdown.IsEmpty())	  m_bDown.Attach((HBITMAP)LoadImage(NULL,strdown,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
	if (!strover.IsEmpty())	  m_bOver.Attach((HBITMAP)LoadImage(NULL,strover,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
	if (!strfocus.IsEmpty())  m_bFocus.Attach((HBITMAP)LoadImage(NULL,strfocus,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));

	if (!strdisabled.IsEmpty()) m_bDisabled.Attach((HBITMAP)LoadImage(NULL,strdisabled,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
	else if (!strnormal.IsEmpty()) m_bDisabled.Attach((HBITMAP)LoadImage(NULL,strnormal,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
	...
	if (!strmask.IsEmpty()){
		m_bMask.Attach((HBITMAP)LoadImage(NULL,strmask,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION|LR_DEFAULTSIZE));
