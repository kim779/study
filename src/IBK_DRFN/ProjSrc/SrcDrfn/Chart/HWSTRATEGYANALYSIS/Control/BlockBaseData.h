...
public:
	...
	void OffSet(const int rowOffSet, const int columnOffSet) { m_nRow += rowOffSet; m_nColumn += columnOffSet; };

	int GetRow() const { return m_nRow; };
	int GetColumn() const { return m_nColumn; };
	int& GetRow() { return m_nRow; };
	int& GetColumn() { return m_nColumn; };
	bool IsAllValueBiggerThanZero() const { return (m_nRow >= 0 && m_nColumn >= 0); };
