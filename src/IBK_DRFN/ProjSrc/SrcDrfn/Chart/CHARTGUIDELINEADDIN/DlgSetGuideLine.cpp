...
void CDlgSetGuideLine::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_NAME					 ,      m_bChkName);
	DDX_Check(pDX, IDC_CHECK_VALUE					 ,      m_bChkValue);
	//}}AFX_DATA_MAP
}
...
void CDlgSetGuideLine::Initial_Option()
{
	m_bChkName = FALSE;
	m_bChkValue = FALSE;

	m_cmbAlign.ResetContent();
	m_cmbAlign.AddString( _T("왼쪽"));
	m_cmbAlign.AddString( _T("오른쪽"));
	m_cmbAlign.AddString( _T("양쪽"));
	m_cmbAlign.SetCurSel( 0);
	
	m_cmbAlign.Invalidate();
...
//			OutputDebugString(sTTT);
			...
			{				
				...
				case 58: { m_clConfigSet.m_nLineYSprice		= nValue;	m_cmbYSprice		.SetCurSel((const int)nValue - 1);  } break;	//전일가격 시가				라인 크기
				case 59: { m_clConfigSet.m_nLineYHprice		= nValue;	m_cmbYHprice		.SetCurSel((const int)nValue - 1);  } break;	//전일가격 고가				라인 크기
				case 60: { m_clConfigSet.m_nLineYLprice		= nValue;	m_cmbYLprice		.SetCurSel((const int)nValue - 1);  } break;	//전일가격 저가				라인 크기
				case 61: { m_clConfigSet.m_nLineYEprice		= nValue;	m_cmbYEprice		.SetCurSel((const int)nValue - 1);  } break;	//전일가격 종가				라인 크기
				case 62: { m_clConfigSet.m_nLineYSHL		= nValue;	m_cmbYSHL			.SetCurSel((const int)nValue - 1);  } break;	//전일가격 (시+고+저)/3		라인 크기
				case 63: { m_clConfigSet.m_nLineTSprice		= nValue;	m_cmbTSprice		.SetCurSel((const int)nValue - 1);  } break;	//당일가격 시가				라인 크기
				case 64: { m_clConfigSet.m_nLineYSprice		= nValue;	m_cmbTHprice		.SetCurSel((const int)nValue - 1);  } break;	//당일가격 고가				라인 크기
				case 65: { m_clConfigSet.m_nLineYHprice		= nValue;	m_cmbTLprice		.SetCurSel((const int)nValue - 1);  } break;	//당일가격 저가				라인 크기
				case 66: { m_clConfigSet.m_nLineYLprice		= nValue;	m_cmbTEprice		.SetCurSel((const int)nValue - 1);  } break;	//당일가격 종가				라인 크기
				case 67: { m_clConfigSet.m_nLineTHL			= nValue;	m_cmbTHL			.SetCurSel((const int)nValue - 1);  } break;	//당일가격 (고+저)/2		라인 크기
				case 68: { m_clConfigSet.m_nLineTSHL		= nValue;	m_cmbTSHL			.SetCurSel((const int)nValue - 1);  } break;	//당일가격 (시+고+저)/3		라인 크기
				case 69: { m_clConfigSet.m_nLineTHLE		= nValue;	m_cmbTHLE			.SetCurSel((const int)nValue - 1);  } break;	//당일가격 (고+저+(종*2))/4 라인 크기		
				case 70: { m_clConfigSet.m_nLineHighLimit	= nValue;	m_cmbHighLimit		.SetCurSel((const int)nValue - 1);  } break;	//상한가					라인 크기
				case 71: { m_clConfigSet.m_nLineLowLimit	= nValue;	m_cmbLowLimit		.SetCurSel((const int)nValue - 1);  } break;	//하한가					라인 크기		
				case 72: { m_clConfigSet.m_nLinePivot2Rst	= nValue;	m_cmbPivot2Rst		.SetCurSel((const int)nValue - 1);  } break;	// 피봇 2차 저항 라인 크기
				case 73: { m_clConfigSet.m_nLinePivot1Rst	= nValue;	m_cmbPivot1Rst		.SetCurSel((const int)nValue - 1);  } break;	// 피봇 1차 저항 라인 크기
				case 74: { m_clConfigSet.m_nLinePivotGLine	= nValue;	m_cmbPivotGLine		.SetCurSel((const int)nValue - 1);  } break;	// 피봇 기준선	 라인 크기
				case 75: { m_clConfigSet.m_nLinePivot1GG	= nValue;	m_cmbPivot1GG		.SetCurSel((const int)nValue - 1);  } break;	// 피봇 1차 지지 라인 크기
				case 76: { m_clConfigSet.m_nLinePivot2GG	= nValue;	m_cmbPivot2GG		.SetCurSel((const int)nValue - 1);  } break;	// 피봇 2차 지지 라인 크기		
				case 77: { m_clConfigSet.m_nLineDMKPreHPrice= nValue;	m_cmbDMKPreHPrice	.SetCurSel((const int)nValue - 1);  } break;	// Demark 예상고가  라인 크기
				case 78: { m_clConfigSet.m_nLineDMKGLine	= nValue;	m_cmbDMKGLine		.SetCurSel((const int)nValue - 1);  } break;	// Demark 기준선	라인 크기
				case 79: { m_clConfigSet.m_nLineDMKPreLPrice= nValue;	m_cmbDMKPreLPrice	.SetCurSel((const int)nValue - 1);  } break;	// Demark 예상저가  라인 크기	
				case 80: { m_clConfigSet.m_nLine2Pyung1		= nValue;	m_cmb2Pyung1		.SetCurSel((const int)nValue - 1);  } break;	// 이동평균 첫번째 입력값 라인 크기
				case 81: { m_clConfigSet.m_nLine2Pyung2		= nValue;	m_cmb2Pyung2		.SetCurSel((const int)nValue - 1);  } break;	// 이동평균 두번째 입력값 라인 크기
				case 82: { m_clConfigSet.m_nLine2Pyung3		= nValue;	m_cmb2Pyung3		.SetCurSel((const int)nValue - 1);  } break;	// 이동평균 세번째 입력값 라인 크기
				case 83: { m_clConfigSet.m_nLine2Pyung4		= nValue;	m_cmb2Pyung4		.SetCurSel((const int)nValue - 1);  } break;	// 이동평균 네번째 입력값 라인 크기				
				case 84: { m_clConfigSet.m_nLinePmmDanga	= nValue;	m_cmbPmmDanga		.SetCurSel((const int)nValue - 1);  } break;	// 평균매입 단가 라인 크기
				case 85: { m_clConfigSet.m_nLinePmmPDanga	= nValue;	m_cmbPmmPDanga		.SetCurSel((const int)nValue - 1);  } break;	// 평균매입 단가 +n% 라인 크기
				case 86: { m_clConfigSet.m_nLinePmmMDanga	= nValue;	m_cmbPmmMDanga		.SetCurSel((const int)nValue - 1);  } break;	// 평균매입 단가 -n% 라인 크기			
				...
				case 94: { m_clConfigSet.m_bNotDrawLine		= m_bNotDrawLine = nValue;	}
					break;
				case 95: { m_clConfigSet.m_bChkName			= m_bChkName		= nValue; }
					break;	// 정보표시 : 이름	체크 버튼
				case 96: { m_clConfigSet.m_bChkValue		= m_bChkValue		= nValue; }
					break;	// 정보표시 : 값		체크 버튼
				case 97: { m_clConfigSet.m_nAlign			= nValue;	m_cmbAlign.SetCurSel((const int)nValue);  }
					break;	// 정보표시 (왼쪽/오른쪽/양쪽)
			}
...
BOOL CDlgSetGuideLine::SaveCfgFile()
{
	...
	if( pfile.Open(m_strCfgFile, CFile::modeCreate | CFile::modeWrite) )
	{
		 ...
		 strCfg += "[OPTION]\n";
		 m_clConfigSet.m_bChkName		= m_bChkName				;	sTemp.Format("CHKN=%d", m_bChkName		);		strCfg += (sTemp + strSep);
		 m_clConfigSet.m_bChkValue		= m_bChkValue				;	sTemp.Format("CHKV=%d", m_bChkValue	);			strCfg += (sTemp + strSep);
		 m_clConfigSet.m_nAlign			= m_cmbAlign.GetCurSel()	;	sTemp.Format("ALN=%d", m_clConfigSet.m_nAlign);	strCfg += (sTemp + strSep);				
