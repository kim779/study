...
bool CxSplitterWnd::CreateInWnd(CWnd *parent, DWORD ctrlId, LPRECT rc)
{
	...
	if (!Create(_T("STATIC"), "Hi", WS_CHILD | WS_VISIBLE | SS_NOTIFY | WS_EX_CLIENTEDGE | SS_GRAYRECT,
			*rc, parent, ctrlId))
		return FALSE;
...
{ 
	...
	for (int i=0; i<m_rightCtrls.GetSize(); i++) 
	{ 
...
bool CxSplitterWnd::GetMouseClipRect(LPRECT rectClip, CPoint pointInSplitter)
{
    ...
    for ( i=0; i<m_leftCtrls.GetSize(); i++) 
	{
        cInfo = m_leftCtrls.GetAt(i);
...
int CxSplitterWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_MovingSplitter.Create(_T(""), WS_CHILD|WS_VISIBLE|SS_CENTER,CRect(0,0,0,0), m_pParent);
	m_MovingSplitter.ShowWindow(SW_HIDE);
...
void CxSplitterWnd::OnDestroy() 
{
	...
	m_MovingSplitter.DestroyWindow( );
}
...
//	ASSERT_VALID(pDrawDC);
 ...
 //   pDrawDC->DrawDragRect(&rectSplitter, sizeBar, NULL, sizeBar);
	m_MovingSplitter.SetWindowPos(&CWnd::wndTop,rectSplitter.left,rectSplitter.top,rectSplitter.Width(),rectSplitter.Height(),SWP_SHOWWINDOW);
	...
	/*		GetParent()->GetWindowRect(pRect);
			if( rectOld.right - pRect.left < 52		)
			{	
				break;
			}

			if( rectOld.right - pRect.left > 260	)
			{	
				break;
			}
	*/
			...
			m_MovingSplitter.SetWindowPos(&CWnd::wndTop,rectCur.left,rectCur.top,rectSplitter.Width(),rectSplitter.Height(),SWP_SHOWWINDOW);
			m_MovingSplitter.Invalidate();		
...
void CxSplitterWnd::OnPaint()
{
	...
	pDC->FillSolidRect(clientRc, (COLORREF)GetParent()->SendMessage(RMSG_GET_SKINCOLOR, (WPARAM)0));
...
{ 
	...
	ControlInfo cInfo;
	int i = 0;
	for ( i=0; i<m_aboveCtrls.GetSize(); i++) 
	{ 
