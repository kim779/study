...
/*	m_arrCode.RemoveAll();
	...
	{	//모두
		for( int i = 0; i < m_StockArray.GetSize(); i++)
		{
			MASTERCODE *pData = m_StockArray.GetAt(i);
			m_arrCode.Add(pData->strCode);
			m_arrKor.Add(pData->strKor);
			m_arrEng.Add(pData->strEng);
		}
		if( m_nOption == 0)
		{	//코스닥 종목도 추가.
			for( int i = 0; i < m_KosdaqArray.GetSize(); i++)
			{
				MASTERCODE *pData = m_KosdaqArray.GetAt(i);
				m_arrCode.Add(pData->strCode);
				m_arrKor.Add(pData->strKor);
				m_arrEng.Add(pData->strEng);
			}
