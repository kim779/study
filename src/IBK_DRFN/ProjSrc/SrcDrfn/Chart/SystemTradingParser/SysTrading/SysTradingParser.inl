...
inline void SysTradingParser::SetEndSentence(BOOL bEnd)
{
	if(m_nKindofFile == PARSE_CPLUSPLUS)
		SetEndSentence_CPP(bEnd);
}

inline int SysTradingParser::IsEndSentence(LPSTR p_lpszName)
{
	int nReturn = 0;

	if(m_nKindofFile == PARSE_CPLUSPLUS)
		nReturn = IsEndSentence_CPP(p_lpszName);
