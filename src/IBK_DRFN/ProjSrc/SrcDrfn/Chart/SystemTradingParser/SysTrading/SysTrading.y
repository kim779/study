...
|	expr stmt_suffix 
	{
		...
		nError = g_pSysTradingParser->WriteGeneralSentence($1.sStr);
		if(nError)
			return nError;
	}
|	assign_expr stmt_suffix
	{
		...
		nError = g_pSysTradingParser->WriteAssignSentence($1.sStr);
		if(nError)
			return nError;
	}
...
|	LPAREN expr2 RPAREN
	{ 
		...
		::wsprintf($$.sStrOrg, _T("(%s)"), $2.sStrOrg);
	}
|	expr2 PLUS expr3 
	{ 
		CString strExpr2 = ::lstrlen($1.sStrOrg) ? $1.sStrOrg : $1.sStr;
		CString strExpr3 = ::lstrlen($3.sStrOrg) ? $3.sStrOrg : $3.sStr;

		::wsprintf($$.sStrOrg, _T("%s + %s"), strExpr2, strExpr3);
