...
public:
	...
	static void TrimLeft(CString& strSource, LPCTSTR lpszTargets)
	{
		CXTPString& str = (CXTPString&) strSource;

		// if we're not trimming anything, we're not doing any work
		if (!lpszTargets || lpszTargets[0] == 0)
			return;

		str.CopyBeforeWrite();
		LPCTSTR lpsz = str.m_pchData;
		...
		if (lpsz != str.m_pchData)
		{
			// fix up data and length
			int nDataLength = str.GetData()->nDataLength - (lpsz - str.m_pchData);
			memmove(str.m_pchData, lpsz, (nDataLength+1)*sizeof(TCHAR));
			str.GetData()->nDataLength = nDataLength;
		}
	}
};
...
AFX_INLINE void TRIMLEFT_S(CString& str, LPCTSTR lpszTargetList) {
#if (_MSC_VER <= 1100) // Using Visual C++ 5.0
	CXTPStringHelper::TrimLeft(str, lpszTargetList);
#else
	str.TrimLeft(lpszTargetList);
#endif
}
