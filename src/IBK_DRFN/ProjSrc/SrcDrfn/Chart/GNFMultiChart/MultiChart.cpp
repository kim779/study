...
//		pDialog->m_hViewInMain = pIn->m_hParent;
		pDialog->m_nWidthOfDlg = 1022;
		pDialog->m_nHeightOfDlg = 585;

		if(pIn->m_nID == 1)
			pDialog->SetSystemChart(TRUE);
		else if(pIn->m_nID == 2)
			pDialog->SetParameters1("구분=주식");
		else if(pIn->m_nID == 3)
			pDialog->SetParameters1("구분=선물옵션");
		else if(pIn->m_nID == 4)
			pDialog->SetParameters1("구분=지수");
		else if(pIn->m_nID == 5)
			pDialog->SetParameters1("구분=비교차트");
		else if(pIn->m_nID == 6)	// 상품선물옵션차트 - ojtaso (20071001)
			pDialog->SetParameters1("구분=상품선물옵션");
