...
//@유진추가
	...
	void SetRealJPriceData(void *pRealData, int pRealType);
...
// --> [Edit]  강지원 2008/10/21		( 시간대 설정 )
	CString		m_strTimeSet;
	void		SetTimeGab(int nIndex, BOOL	bFlag = TRUE);
	CString		GetTimeGab() { return m_strTimeSet; };
	BOOL		CheHoga_Save(int nIndex, BOOL bSaveFlag = TRUE, BOOL bLoadFlag = TRUE);
	CString		GetCalcTimeGab(CString strTime, int nTimeGab);
	CString		GetSelectTime();
	CString		GetCtlTime();
	int			GetCheHogaType();
...
// --> [Edit]  강지원 2008/11/04
	...
	CChartItem * m_pChartItemForRequest;
// <-- [Edit]  강지원 2008/11/04
	CStringArray*	m_saListData;	//	수치조회 데이터
// <-- [Edit]  강지원 2008/10/21
...
// --> [Edit]  강지원 2008/11/05	( 수치조회 메시지 )
	LRESULT OnInquiryDataExport( WPARAM wParam, LPARAM lParam);
// <-- [Edit]  강지원 2008/11/05
