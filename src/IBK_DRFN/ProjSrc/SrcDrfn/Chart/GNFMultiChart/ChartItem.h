...
//#include "../Chart_common/WRFuChart_ocx/WRFuChart.h"
//#include "../Chart_common/KTBChart_ocx/ktbchart.h"//"ktbchart.h"
...
// 
	...
	CString m_strNextKey;			//	넥스트 키를 저장하는 변수
// <-- [Edit]  이도영 2008/09/11

// --> [Edit]  강지원 2008/10/15
	int		m_nTimeIndex;
	ST_CHARTITEM_REALDATA	m_ChartItemRealData;
	ST_CHARTITEM_REALDATA*	GetChartItemRealData() { return &m_ChartItemRealData; };

	short	m_nCheHoga;				//	체결(0), 호가(1)
	void	SetCheHogaChange(short nType );
	short	GetCheHogaChange();

	char	GetDecimalRate(CString strDecimalGB, int &nPcnt, double &dRate, int &nFcnt, int &nHcnt, int &nBase);
	CString GetCodeConvInfo( CString strCode, int &nLog, int &nLogDispSize, int &nPrecision, int &nValidPoint, int &nMask, int &nTickSize);
	CString GetCodePrecisionFormat( CString strCode );
	CString	Gl_Format32String(long nTBValue, int scale, int outdec, int indec, UINT uMask);
	CString Gl_Format32String(double dTBValue, int scale, int outdec, int indec);
	long	FtoTbondPrcL( double inprice, int scale, int outdec );
	BOOL	TbondPrcLToF(char cType, int nPcnt, double dRate, CString &strData);
	long	DataSeparation(CString strData, int nMask);
	CString	GetCalcTimeGab(CString& strDate, CString strTime, int nTimeGab, BOOL bCalcType = TRUE );
	CString	GetCtlTime();
	int		GetExInputBarDlg();
	void	SetTypeExInputBarDlg(char chType);

	void	SetTimeIndex(int nIndex) { m_nTimeIndex = nIndex; };

	void	SetRealTimeSignal(void *pData, long lDataKind, int nRealType);		//	타임 시그널을 받기위한 함수

	BOOL	m_bCheHogaChange;

	void	AllChartItemCodeRequest();
// <-- [Edit]  강지원 2008/10/15
...
//	CDrMLib_CDBMgr				*m_pDrMLib_DBMgr;
	...
	void SetRealPriceData(void *pData, long lDataKind, int nRealType = -1);
...
// --> [Edit]  강지원 2008/11/02
	void GetRowNCol(long& lRowIndex, long& lColIndex);
// <-- [Edit]  강지원 2008/11/02
...
//2006.12.11 LYH 로드 및 저장파일명 읽어오기
	// 분석툴 종목별 저장 - ojtaso (20080728)
	CString GetOpenAndSaveFileName(BOOL bWithTypeValue = FALSE, BOOL bSaveFlag = FALSE);

	// 2011.01.31 by SYS >> 분차트 YYYY(년) 추가 및 분석툴 공유 기능 추가
	// 분석툴용 저장파일
	CString GetOpenAndSaveAnalFileName(BOOL bWithTypeValue = FALSE, BOOL bSaveFlag = FALSE);
	// 2011.01.31 by SYS <<
