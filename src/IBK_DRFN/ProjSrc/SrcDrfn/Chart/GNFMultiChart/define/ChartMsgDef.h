...
#define UMSG_SET_NORMALIZEDMINUTETIMERULE_AND_GAPREVISION	\
										WM_USER+72	// 허수봉 및 갭보정을 재설정한다. (분 데이터 재조회 필수)
...
#define UMSG_FORMGETPAGESAVE				WM_USER+216 //08.04.29	FormInitAfter event 처리
#define UMSG_FORMSETPAGESAVE				WM_USER+217 //08.04.29	FormInitAfter event 처리

// --> [Edit]  이도영 2008/09/13
// 
#define UMSG_SETORDERLINK					WM_USER+218	// 주문연동세팅을 한다.
// <-- [Edit]  이도영 2008/09/13

// 20080914 이문수 >>
#define	UMSG_CHART_CONTAINER_PRINT			WM_USER+219
#define	UMSG_CHART_IMG_SAVE					WM_USER+220
// 20080914 이문수 <<

// --> [Edit]  강지원 2008/10/15
#define UMSG_CHE_HOGA_CHANGE				WM_USER+221	// 체결/호가 버튼 클릭
#define UMSG_CHE_HOGA_VALUE					WM_USER+222	// 체결/호가 버튼 값 얻기
#define UMSG_CREATESCREEN_LINK				WM_USER+223	// 호가테이블에서 주문화면을 띄워준다.
// <-- [Edit]  강지원 2008/10/15
#define	UMSG_CHART_MIGYULCHE				WM_USER+224

// --> [Edit]  강지원 2008/11/05	( 수치조회창 관련 )
#define	UMSG_CHART_INQUIRY_SHOW				WM_USER+225
#define	UMSG_CHART_TOOLBAR_BTN_TOGGLE		WM_USER+226
#define	UMSG_CHART_INQUIRY_DELETE			WM_USER+227
// <-- [Edit]  강지원 2008/11/05

// --> [Edit]  강지원 2008/11/28	( 연속월물 체크 )
#define	UMSG_CHART_TICKCHART				WM_USER+228
// <-- [Edit]  강지원 2008/11/28

// --> [Edit]  강지원 2008/10/28	( 차트 내부에서도 이 번호로 사용하기 때문에 번호를 바꾸면 안된다고 함... )
//	왼쪽 탭의 인덱스 설정값을 변경하거나 세팅할때 사용하는 메시지
//	차트로 시간대 설정하는 메시지
#define UDM_CHANGE_TIMEDIFF					WM_USER+618	// wParam - Time Index, lParam - 0	//20081021 JS.Kim.. 사용자시간대선택
//	차트에서 메뉴 선택시 넘어오는 메시지
#define UDM_NOTIFY_TIMEDIFF					WM_USER+619	// wParam - Time Index, lParam - Not Used
// <-- [Edit]  강지원 2008/10/28

// 2011.01.12 by SYS >> 분차트 YYYY(년) 추가 및 분석툴 공유 기능 추가
#define UMSG_SET_ALALTOOL_SHAREFILE		WM_USER+701	// 11.01.12 일/주/월/분/틱 구분없이 분석툴 공유 기능
#define UMSG_GET_ALALTOOL_SHAREFILE		WM_USER+702	// 11.01.12 일/주/월/분/틱 구분없이 분석툴 공유 기능
//
#define UMSG_ALALTOOL_FILE_CHANGE		WM_USER+703	// 추세선저장 <=> 추세선공유 기능이 전환된 경우 
// 2011.01.12 by SYS <<
...
#define TAB_CODESEARCH	"시간"
#define TAB_ORDER		"주문"
#define TAB_MYFAVORITE	"사용자"
#define TAB_SYSTEM		"시스템"		// 2005. 05. 16 add by sy, nam
//#define TAB_SYSTEMTRADING	"시스템트레이딩" // 2005.10.25 gm0604
#define TAB_SYSTEMTRADING	"시스템" // 2005.10.25 gm0604
...
// --> [Edit]  강지원 2008/11/05		//	지표/시스템/시간대의 탭 순서를 바꿈
#define TABNO_ASSISTINDEX		(0)
#define TABNO_CODESEARCH		(1)
...
#define UPJONG_TABTOTALCNT		(2)		// 2005. 05. 16 add by sy, nam
#define FUTOPT_TABTOTALCNT		(2)		// 2005. 05. 16 add by sy, nam
...
// --> [Edit]  강지원 2008/10/20
//#define DOWN_LIMIT				'▼'		//하한 2005. 05. 17 add by sy, nam	( '-' )		( 사용안함 )
//#define DOWN					'-'		//하락 2005. 05. 17 add by sy, nam	( '-' )
//#define EQUAL					' '		//보합 2005. 05. 17 add by sy, nam	( 공백 )
//#define UP_LIMIT				'▲'		//상한 2005. 05. 17 add by sy, nam	( '+' )		( 사용안함 )
//#define UP						'+'		//상승 2005. 05. 17 add by sy, nam	( '+' )
...
#define	SEC30_STRING_FORMAT		"½"	//	30초 스트링
#define	SEC30_STRING			"999"
#define	SEC30_VALUE				(999)
// <-- [Edit]  강지원 2008/10/20

//20080913 이문수 >>
//fx에서는 장구분이 없으므로 기본적으로 주식과 같은 처리를 하게 놓았다.
//장구분이 없고 종목코드는 무조건 7자리로 판단하므로 기존에 길이가 겹치는 elw의 길이7을 
//임의로 동작하지 않도록 999로 셋팅한다.
//#define CODE_STOCK_LENGTH		6
#define CODE_STOCK_LENGTH		7
#define SIMBOL_CODE_MIN_LENGTH	5		//	해외선물 품목코드 길이는 가변이라서 최소 단위로 체크한다.
//20080913 이문수 <<
...
//20080913 이문수 >>
//fx에서는 장구분이 없고 종목코드는 무조건 7자리로 판단하므로 기존에 길이가 겹치는 elw의 길이7을 
//임의로 동작하지 않도록 999로 셋팅한다.
//#define ELW_LENGTH			7		// ELW 삼성: 길이가 7
#define ELW_LENGTH			999		// ELW 삼성: 길이가 7
//20080913 이문수 <<

#define COMMODITY_LENGTH			8	// 상품선물옵션차트 - ojtaso (20071001)

// --> [Edit]  강지원 2008/11/02	( 최대 종목 추가 갯수 )
#define MAX_ADD_CODE		(10)
// <-- [Edit]  강지원 2008/11/02
...
//#define TICK_DATA_CHART_HOGA	'9'		// 호가	차트	2008. 10. 30 우리선물 호가차트
...
#define QUERY_STRJONGMOK			"o44003$"     //"HCQ03005"
#define QUERY_STRJONGMOK_SISE		"o44000$" 
#define QUERY_STRJONGMOK_HOGA	"o56501$"    //"HCQ03012"
#define QUERY_STRUPJONG			"p0603"
#define QUERY_STRFUTOPT			"p0604"
#define QUERY_STRSTOCKFUTOPT	"p0605"
#define QUERY_STRFOREIGN		"p0606"
#define QUERY_STRMARKET			"p0607"
#define QUERY_STRDOLLAR			"p0610"
#define QUERY_STOCK_HOGA		"o44010$" //"HCQ01112"
#define QUERY_MEDOSU_HOGA		"HCQ03011"
...
#define DEFAULT_BKGRND_COLOR		RGB(229,229,229)
...
#define GRDMX					(3000)
#define CHTMX					(5200)
#define CHTMXREQUEST			(2000)
#define BOJO_MSG_LENGTH			(500)
#define BOJO_MSG_LENGTH_SIZE	(4)
#define CHARTDATA_MAX_CNT		(4000)			//	데이터 최대 카운트
...
typedef struct  {
	...
	CString m_strOrderType;	// 매도 매수 구분	
	short	m_nCheHoga;		// 체결/호가 버튼 구분
}stChartInitData;
...
typedef struct 
{
	bool bImaginaryCandle;	// TRUE 실봉 + 허봉, FALSE 실봉
	bool bUseGapRevision;	// 갭보정	// (2008/11/2 - Seung-Won, Bae) for Gap Revision.
...
typedef struct _ST_SELCHART {
	...
	CString			m_strTimeName;
	int				m_nTimeIndex;
	int				m_nOrgTimeIndex;
	CString			m_strTimeGab;
