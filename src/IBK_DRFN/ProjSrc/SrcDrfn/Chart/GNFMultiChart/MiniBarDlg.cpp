...
void CMiniBarDlg::Init()
{
	...
	m_strImagePath = ((CStdDialog*)GetParent())->m_strRootPath + "\\Image\\Chart_Img";
...
//	SetTimer(ID_SHARE_CODE_WAIT, 200, NULL);
	...
	{
		strCode = GetLastHistoryCode(CMasterTypeInfo::STOCKFUTOPT_CHART);
		if(strCode.GetLength() < 1)
		{
			return;
		}
...
void CMiniBarDlg::SetAllPosition(int nType)
{
	if(nType == 0)
		return;
//	m_chkFirst.ShowWindow(SW_SHOW);
//	m_chkSecond.ShowWindow(SW_SHOW);
//	m_chkThird.ShowWindow(SW_SHOW);
//	m_chkFourth.ShowWindow(SW_SHOW);
...
void CMiniBarDlg::MoveToLeftRight(UINT nMoveDirection)
{
	...
	case 2:
		...
		if(m_cbMarket.GetSafeHwnd())//@sini && m_ctrlCodeInput.GetSafeHwnd())
		{
			CRect rcMarketCombo, rcCodeInput;
			m_cbMarket.GetWindowRect(&rcMarketCombo);
//@sini		m_ctrlCodeInput.GetWindowRect(&rcCodeInput);
			int cx = 0;
...
//@sini		m_ctrlCodeInput.ShowWindow(SW_HIDE);
...
//@sini			m_ctrlCodeInput.ShowWindow(SW_SHOW);
		}
		break;
	}
}
