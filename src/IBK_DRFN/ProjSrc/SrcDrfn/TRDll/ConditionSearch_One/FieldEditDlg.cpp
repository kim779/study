...
BOOL CFieldEditDlg::OnInitDialog() 
{
	...
	m_nIndustry = pwndMainFrm->m_nIndustry;

	if(m_nIndustry == 0)
		this->SetWindowText((CString)PROGRAM_TITLE + " [조회항목 편집]");
	else if(m_nIndustry == 1)
		this->SetWindowText((CString)PROGRAM_TITLE_INDUSTRY + " [조회항목 편집]");
	else
		this->SetWindowText((CString)PROGRAM_TITLE_REAL + " [조회항목 편집]");
...
void CFieldEditDlg::OnBtnUp()
{
	...
	if (nRow == 0)
	{
		...
		if(m_nIndustry == 0)
			...
			SetMessageBox(strErrorMsg, (CString)PROGRAM_TITLE_INDUSTRY, MB_OK);
		else
			SetMessageBox(strErrorMsg, (CString)PROGRAM_TITLE_REAL, MB_OK);
				...
				else
				{
					...
					if(saSourceData.GetAt(1) == "유사도")
					{
						...
						if(m_nIndustry == 0)
							...
							SetMessageBox("유사도 필드는 제거할 수 없습니다", (CString)PROGRAM_TITLE_INDUSTRY, MB_OK);
						else
							SetMessageBox("유사도 필드는 제거할 수 없습니다", (CString)PROGRAM_TITLE_REAL, MB_OK);
...
// 			SetMessageBox(strMsg, (CString)PROGRAM_TITLE_REAL, MB_OK);
		...
		if(m_nIndustry == 0)
			...
			strTitle = PROGRAM_TITLE_INDUSTRY;
		else
			strTitle = PROGRAM_TITLE_REAL;
