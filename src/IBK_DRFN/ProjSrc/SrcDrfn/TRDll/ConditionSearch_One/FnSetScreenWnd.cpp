...
LONG CFnSetScreenWnd::ReceiveTRx(int nKey, char* pBytes, int nLen)
{
	...
	case TRKEY_GWANSIMITEM://tour2k item
		pScreenWnd->ReceiveTR(TRNO_FAVORITE, (LPVOID)pBytes, (long)nLen);
		break;
	}
...
void CFnSetScreenWnd::ShareCode(CString strCode)
{
	...
	if(strCode.GetLength() >= 6)
		strShareCode.Format("%s\t%s", "1301", strCode);	//주식
	else
	{
		CString strTemp;
		strTemp.Format("%03d", atoi(strCode));
		strShareCode.Format("UPCD\t%s", strTemp); //업종
	}
