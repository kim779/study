...
void CListImageWnd::SetUseHelpMenu(BOOL bUse)
{
	m_bUseHelpMenu = bUse;
}
	...
	if(m_bUseDeleteMenu)
	{
		...
		for(int i = 0; i < nImageCount; i++)
		{
			...
			if(rcImage.PtInRect(ptClick))
			{
				...
				menu.AppendMenu(MF_STRING, MENU_RESULTCTRL_INDEX[0], "캔들 삭제"); 
	...
	else if(m_bUseHelpMenu)
	{
		...
		for(int i = 0; i < nImageCount; i++)
		{
			...
			if(rcImage.PtInRect(ptClick))
			{
				CMenu	menu;
				menu.CreatePopupMenu();
				menu.AppendMenu(MF_STRING, MENU_RESULTCTRL_INDEX[0], "캔들패턴설명"); 
				...
				if( nRetValue>0 )
				{
					m_pParent->PostMessage(RMSG_HELPHTML_ITEM, i, 0);
				}
				break;
			}
		}
