...
const		int			HEADWIDTH_RESULT[] = { 100, 65, 39, 40, 80, 24,28 };


const		LPCTSTR		HEADNAME_BASE[] = {_T("분"), _T("신"), _T("∨종목코드"), _T("종목명") };
const		int			HEADWIDTH_BASE[] = { 0, 0, 63, 90 };
...
void CGrid_Comm::OnRClicked(int col, long row, int updn, RECT *rect, POINT *point, BOOL processed)
{
	if ((row != -1) && (updn == 1))
	{
		...
		CString strLPARM;
		strLPARM.Format("%s\n%d\t%d\n1301\t%s\n", "IB713600", point->x, point->y, strCode);
		// mapN \n pt.x \t pt.y \n SYM \t DATA \n
...
void CGrid_Comm::SetAddRow(int nRow, int lOutFieldCnt, CANDLE_OUT_LIST stCANDLE_OUT_LIST, int arCandleData[5])
{
	...
	GetCell(2, nRow, &cellItemCode);
	GetCell(3, nRow, &cellItemName);
...
void CGrid_Comm::SetAddRow(int nRow, char chSign, int ntotCol, char chMarkType,
						   CString strItemCode, CString strItemName, double* dcolData, bool bRedraw)
{
	...
	GetCell((int)COL_BUN, nRow, &cellItemBun);
	GetCell((int)COL_SIN, nRow, &cellItemSin);
	GetCell((int)COL_CODE, nRow, &cellItemCode);
	GetCell((int)COL_NAME, nRow, &cellItemName);
	...
	for (nCol = 0; nCol < ntotCol; nCol++)
	{
		...
		else
		{
			...
			if (TempHeadType.strDataType == "debi")    //## 대비 ##
			{
				GetSignInfo(nRow, chSign, clrContrastText);

				cellData[nCol + nCol_Offset].SetTextColor(clrContrastText);
				cellData[nCol + nCol_Offset].SetSign(atoi(&chSign));
			}
			else if (TempHeadType.strDataType == "debiuel")   //## 대비율 ##
			{
				COLORREF clrNumber;

				if (dcolData[nRealDataIndex] > 0)
					clrNumber = RED;
				else if (dcolData[nRealDataIndex] < 0)
					clrNumber = BLUE;
				else if (dcolData[nRealDataIndex] == 0)
					clrNumber = BLACK;

				cellData[nCol + nCol_Offset].SetTextColor(clrNumber);
			}
			nRealDataIndex++;
		}
	...
	cellItemCode.SetBorderColor(&m_penGridLine);
	cellItemCode.SetBorder(UG_BDR_RTHIN);
	SetCell((int)COL_CODE, nRow, &cellItemCode);

	cellItemName.SetBackColor(clrBk);
	cellItemName.SetBorderColor(&m_penGridLine);
	cellItemName.SetBorder(UG_BDR_RTHIN);
	SetCell((int)COL_NAME, nRow, &cellItemName);
