...
void CFnSetScreenWnd::ReceiveReal(WPARAM wParam, LPARAM lParam)
{
	...
	if(strKey == "CVOL")
	{
		...
		for (int i=alertR->size-1; i>=0; i--)
		{
			...
			pScreenWnd->ReceiveReal(wParam, (LPARAM)(LPCTSTR)m_strSendData);
			m_strSendData = "CVOL";
		}
	}
}
...
BOOL CFnSetScreenWnd::GetStockMasterData(CStringArray &arrCode, CStringArray &arrName, UINT Jgub, UINT Upgub)
{
			...
			if(pmapMarket->Lookup(strMarketCode, (void*&)plist))
			{
				for(int i = 0; i < plist->GetCount(); i++)
				{
					...
					case 115: // KOSPI200 동일가중
						if ((int)stJCode.kpgb == 3 || (int)stJCode.kpgb == 2 || (int)stJCode.kpgb == 1)
							append = true;
						break;
					case 107: // 건설기계
						if ((int)stJCode.uc10 == 1)
							append = true;
						break;
					case 108: // 조선운송
						if ((int)stJCode.uc10 == 2)
							append = true;
						break;
					case 109: // 철강소재
						if ((int)stJCode.uc10 == 3)
							append = true;
						break;
					case 110: // 에너지화학
						if ((int)stJCode.uc10 == 4)
							append = true;
						break;
					case 111: // 정보통신
						if ((int)stJCode.uc10 == 5)
							append = true;
						break;
					case 112: // 금융
						if ((int)stJCode.uc10 == 6)
							append = true;
						break;
					case 113: // 필수소비재
						if ((int)stJCode.uc10 == 7)
							append = true;
						break;
					case 114: // 자유소비재
						if ((int)stJCode.uc10 == 8)
							append = true;
						break;
					case 401: // KOSPI100
					case 403: // KOSPI100 동일가중
						if ((int)stJCode.kpgb == 3 || (int)stJCode.kpgb == 2)
							append = true;
						break;
					case 402: // KOSPI50
					case 404: // KOSPI50 동일가중
						if ((int)stJCode.kpgb == 3)
							append = true;
						break;
					}
	...
	case GETMARKET_KRX		:	
		{
			...
			if(pmapMarket->Lookup(strMarketCode, (void*&)plist))
			{
				for(int i = 0; i < plist->GetCount(); i++)
				{
					...
					{
					case 601: // KRX 자동차
						if ((int)stJCode.sec1 & SEC1_AT)
							append = true;
						break;
					case 602: // KRX 반도체
						if ((int)stJCode.sec1 & SEC1_SM)
							append = true;
						break;
					case 603: // KRX 건강
						if ((int)stJCode.sec1 & SEC1_HC)
							append = true;
						break;
					case 604: // KRX 은행
						if ((int)stJCode.sec1 & SEC1_BK)
							append = true;
						break;
					case 605: // KRX 정보통신
						if ((int)stJCode.sec2 & SEC2_IT)
							append = true;
						break;
					case 606: // KRX 화학에너지
						if ((int)stJCode.sec2 & SEC2_EC)
							append = true;
						break;
					case 607: // KRX 철강
						if ((int)stJCode.sec2 & SEC2_ST)
							append = true;
						break;
					case 608: // KRX 필수소비재
						if ((int)stJCode.sec2 & SEC2_CS)
							append = true;
						break;
					case 609: // KRX 미디어통신
						if ((int)stJCode.sec3 & SEC3_MT)
							append = true;
						break;
					case 610: // KRX 건설
						if ((int)stJCode.sec3 & SEC3_CO)
							append = true;
						break;
					case 611: // KRX 금융서비스
						if ((int)stJCode.sec3 & SEC3_FB)
							append = true;
						break;
					case 612: // KRX 증권
						if ((int)stJCode.sec3 & SEC3_SI)
							append = true;
						break;
					case 613: // KRX 조선
						if ((int)stJCode.sec4 & SEC4_SC)
							append = true;
						break;
					case 614: // KRX SRI
						if ((int)stJCode.sec4 & SEC4_SR)
							append = true;
						break;
					case 615: // KRX 보험
						if ((int)stJCode.sec4 & SEC4_IS)
							append = true;
						break;
					case 616: // KRX 운송
						if ((int)stJCode.sec4 & SEC4_TP)
							append = true;
						break;
					case 617: // KRX 소비자유통
						if ((int)stJCode.sec5 & SEC5_RT)
							append = true;
						break;
					case 618: // KRX 레저엔터테인먼트
						if ((int)stJCode.sec5 & SEC5_LS)
							append = true;
						break;
					case 619: // KRX100 동일가중
						if ((int)stJCode.sec5 & SEC5_KX)
							append = true;
						break;
					case 999: // KRX통합지수
						if (stJCode.unio == 1)
	...
	case GETMARKET_KOSTAR:
		{
			POSITION pos;
			bool append = false;
			CList<hjcodex, hjcodex&>* plist = NULL;
			
			CString strMarketCode = "201";
			pmapMarket = &m_mapKosdaqStockMarketGroup;
			if(pmapMarket->Lookup(strMarketCode, (void*&)plist))
			{
				for(int i = 0; i < plist->GetCount(); i++)
