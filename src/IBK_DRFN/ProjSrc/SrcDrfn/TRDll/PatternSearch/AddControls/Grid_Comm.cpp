...
const		int			HEADWIDTH_RESULT[] = { 100, 65, 39, 40, 80, 24,38 };


const		LPCTSTR		HEADNAME_BASE[] = {_T("분"), _T("신"), _T("∨종목코드"), _T("종목명"), _T("유사도") };
const		int			HEADWIDTH_BASE[] = {0, 0, 63, 90, 50};
...
void CGrid_Comm::OnRClicked(int col, long row, int updn, RECT *rect, POINT *point, BOOL processed)
{
	if ((row != -1) && (updn == 1))
	{
		...
		CString strLPARM;
		strLPARM.Format("%s\n%d\t%d\n1301\t%s\n", "IB713500", point->x, point->y, strCode);
		// mapN \n pt.x \t pt.y \n SYM \t DATA \n
...
void CGrid_Comm::SetAddRow(int nRow, int lOutFieldCnt, DRAW_OUT_LIST stDRAW_OUT_LIST, int arCandleData[5])
{
	...
	cellItemCode.SetReadOnly(FALSE);		//ReadOnly속성을 빼줘야 체크를 사용할 수 있다.
	//<<0000135
