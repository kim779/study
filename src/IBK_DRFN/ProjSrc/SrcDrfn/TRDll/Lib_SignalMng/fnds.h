///////////////////////////////////////////////////////////////////////////////////////////////////
// 종목검색 신호설정(FNDS) ( FNDS,attr,key=4,group=1 )
#pragma pack( push, 1 )

#define NAME_FNDS     "FNDS"

// 입력
typedef struct _FNDS_InBlock
{
    char    alcode              [   4];    char    _alcode              ;    // [string,    4] KEY패킷구분코드
} FNDS_InBlock, *LPFNDS_InBlock;
#define NAME_FNDS_InBlock     "InBlock"

// 출력
typedef struct _FNDS_OutBlock
{
    char    alcode              [   4];    char    _alcode              ;    // [string,    4] KEY패킷구분코드
    char    gsalertgubun        [   1];    char    _gsalertgubun        ;    // [string,    1] System User Gb
    char    gsdate              [   8];    char    _gsdate              ;    // [string,    8] 신호발생일자
    char    gstime              [   6];    char    _gstime              ;    // [string,    6] 신호발생시간
    char    gstruecnt           [   4];    char    _gstruecnt           ;    // [string,    4] 현재만족종목수
    char    gsdatacnt           [   4];    char    _gsdatacnt           ;    // [long  ,    4] 전송종목수
    char    gsstatus            [   1];    char    _gsstatus            ;    // [string,    1] 신호발생
    char    gsjobflag           [   1];    char    _gsjobflag           ;    // [string,    1] 신규 재진입 탈퇴
    ...
    char    gsvolumerate        [   6];    char    _gsvolumerate        ;    // [float ,  6.2] 거래량 전일대비율
    char    gsamount            [   9];    char    _gsamount            ;    // [long  ,    9] 거래대금
} FNDS_OutBlock, *LPFNDS_OutBlock;
#define NAME_FNDS_OutBlock     "OutBlock"
