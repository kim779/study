...
BOOL CFnSetScreenWnd::SetInit(CWnd* pWndModuleMain, CWnd* pWndScreenWnd)
{
	...
	CString	strGubn = _T("");
	strGubn = Variant(systemCC);
	if (!strGubn.IsEmpty())
		m_cGubun = strGubn.GetAt(0);
	else
		m_cGubun = ' ';
...
LONG CFnSetScreenWnd::ReceiveTR(WPARAM wParam, LPARAM lParam)
{
	...
	return ReceiveTRx(nKind, (char*)lParam, (int)HIWORD(wParam));
}

LONG CFnSetScreenWnd::ReceiveTRx(int nKey, char* pBytes, int nLen)
{
	LONG lRet = 0;
	CScreenWnd* pScreenWnd = (CScreenWnd*)m_pWndScreen;
	switch(nKey)
	{
	case TRKEY_MSTCODE :
		DoPreReceiveData(TRNO_MSTCODE, (LPVOID)pBytes, (long)nLen);
		pScreenWnd->ReceiveTR(TRNO_MSTCODE, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_WORKDATE :
		DoPreReceiveData(TRNO_WORKDATE, (LPVOID)pBytes, (long)nLen);
		pScreenWnd->ReceiveTR(TRNO_WORKDATE, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_CONDITION :
		pScreenWnd->ReceiveTR(TRNO_CONDITION, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_LANGUAGE :
		pScreenWnd->ReceiveTR(TRNO_LANGUAGE, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_CONDIONECLK_RESULT :
		pScreenWnd->ReceiveTR(TRNO_CONDIONECLK_RESULT, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_ALERT_REALREG :
		pScreenWnd->ReceiveTR(TRNO_ALERT_REALREG, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_DRAW_RESULT :
		pScreenWnd->ReceiveTR(TRNO_DRAW_RESULT, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_GROUPTOBASE_UP :
		pScreenWnd->ReceiveTR(TRNO_GROUPTOBASE_UP, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_GROUPTOBASE :
		pScreenWnd->ReceiveTR(TRNO_GROUPTOBASE, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_SET_REAL :
		pScreenWnd->ReceiveTR(TRNO_SET_REAL, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_VALUE_PREDICTION :
		pScreenWnd->ReceiveTR(TRNO_VALUE_PREDICTION, (LPVOID)pBytes, (long)nLen);
		break;
	case TRKEY_ACCOUNTINFO:
		RevData_AccountInfo(pBytes, nLen);
		break;
	case TRKEY_GWANSIMGROUP://tour2k 관심종목 그룹
	case TRKEY_GWANSIMITEM://tour2k item
		pScreenWnd->ReceiveTR(TRNO_FAVORITE, (LPVOID)pBytes, (long)nLen, nKey);
		break;
	}
...
void CFnSetScreenWnd::OpenScreen(CString strScreenNo, CStringArray* pSaCode, CStringArray* pSaName, CString strOtherData)
{
	...
	else if(strScreenNo == "7129")
	{
		...
		SetUserMaster(usrMasterCondition, pSaCode->GetSize(), pSaCode, pSaName);

		CString strTrigger;
		strTrigger.Format("%s\t%s", "대상변경", "성과분석");
	...
	else if(strScreenNo == "돌려보기")
	{
		OnLook(pSaCode, pSaName);
		return;
	}
...
{
// 	CScreenWnd* pScreenWnd = (CScreenWnd*)m_pWndScreen;
// 	if(!pScreenWnd->GetRegState())
// 		return;
// 	
// 	CString strData = (char*)lParam;
// 	CString strKey = PeekCode(strData);
// 	
// 	if(strKey != "FNDA")
// 		return;
// 	
// 	pScreenWnd->ReceiveReal(wParam, lParam);
}
									  ...
									  CList <hjcodex, hjcodex&>* plistKosdaqStock)
{
	...
	for(pos = plistKospiStock->GetHeadPosition(); pos != NULL; )
	//20120119 이문수 <<
	{
		//20120119 이문수 >>
		//pos = plistKospiStock->FindIndex(i);
		//JCode = plistKospiStock->GetAt(pos);
		JCode = plistKospiStock->GetNext(pos);
		//20120119 이문수 <<
...
// 계좌
void CFnSetScreenWnd::setAccount()
{
	m_saAccNo.RemoveAll();
	m_saAccNo.Add("");
	...
	for(int i=0 ; i<parrCode->GetSize() ; i++)
	{
		...
		if( strT != "5" && strT != "0" )
		{
			m_saAccNo.Add(strAccn);
			m_saAccNo.Add(strText);
		}
	}
}

BOOL CFnSetScreenWnd::IsNumber(CString str)
{
    BOOL bRet = TRUE;
...
void CFnSetScreenWnd::SendTr_AccountInfo(CString strAccNo)
{
	...
	{
		pswd = m_accn.loadPass(acctdata,strAccNo);
		
		if(pswd == "")
		{
			CInputPswd dlg(NULL, strAccNo);
	...
	if(strAccNo.Mid(3, 1) == "2")
	{
		//queryFutureRemain2(strAccount, pswd);
		AfxMessageBox("선물계좌는 사용할수 없습니다.");
	}
	else
	{
		...
		m_pWndModuleMain->SendMessage(WM_USER, ledgerDLL, (LPARAM)&ledger);
		
		CopyMemory(&ledger.svcd, "SONAQ052", sizeof(ledger.svcd));
		CopyMemory(&ledger.usid, (LPCTSTR)strMember, strMember.GetLength());
		
		CString strBrno = (char*)m_pWndModuleMain->SendMessage(WM_USER, MAKEWPARAM(variantDLL, deptCC), 0L);
		...
		CopyMemory(&mid.accn, strAccNo, sizeof(mid.accn));
		...
		SendTR(TRNO_ACCOUNTINFO, (LPSTR)(LPCTSTR)strSendData, strSendData.GetLength(), TRKEY_ACCOUNTINFO, US_KEY);
	}
}

void CFnSetScreenWnd::RevData_AccountInfo(char* buf, int len)
{
 	m_saCode_Target.RemoveAll();
	...
	else
	{
		...
		{
			SetDataStockAccount(buf);
		}
	}
}

void CFnSetScreenWnd::SetDataStockAccount(char* buf)
{
	struct s_mod	mod;	
	...
	for (int i = 0 ; i<nRec ; i++)
	{
		...
		if (!strCode.IsEmpty())
		{
			...
			m_saCode_Target.Add(strCode);
		}
	}

	CScreenWnd* pScreenWnd = (CScreenWnd*)m_pWndScreen;
	pScreenWnd->SendHaveItem(&m_saCode_Target);
}

void CFnSetScreenWnd::GetAccNo(CStringArray* psaAccNo)
{
	psaAccNo->Copy(m_saAccNo);
}

void CFnSetScreenWnd::OnLook(CStringArray* psaItemCode, CStringArray* psaItemName)
{
	int nTotCnt = psaItemCode->GetSize();
	CString strItemCode;
	CString strItemName;
	CString data = "";
	CString txt;
	...
	{
		strItemCode = psaItemCode->GetAt(nIndex);
		strItemName = psaItemName->GetAt(nIndex);
	...
	if(nTotCnt > 0)
		m_pWndModuleMain->SendMessage(WM_USER, MAKEWPARAM(autoDLL, nTotCnt), (LPARAM)(LPCTSTR)data);
}

BOOL CFnSetScreenWnd::GetMemberCompany(CStringArray &arrCode, CStringArray &arrName)
{
	arrCode.RemoveAll();
	arrName.RemoveAll();
	...
	CFile	fileH;
	CString	filePath;
	filePath.Format("%s/%s/%s", Variant(homeCC), "tab", "TRAD3.CBS");
	if (!fileH.Open(filePath, CFile::modeRead|CFile::typeBinary))
	{
		AfxMessageBox("회원사 정보를 읽어올수 없습니다.");
		return FALSE;
	}

	long nDataLen = fileH.GetLength();
	if(nDataLen > 0)
	{
		CString strData, strCode, strName, strEngName;
		strData.GetBufferSetLength(nDataLen);
		fileH.Read((char*)(LPCSTR)strData, nDataLen);
		fileH.Close();			
		...
		while(TRUE)
		{
			...
			strEngName = strData.Mid(0, nPos + 1);
			strEngName.Remove('\t');
			strData = strData.Mid(nPos + 1, strData.GetLength());
		}
	}
