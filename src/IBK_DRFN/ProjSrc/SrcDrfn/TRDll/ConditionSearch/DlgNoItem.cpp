...
int CDlgNoItem::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CRect reTemp;
	reTemp.left = reClinet.left + x - 100;
	reTemp.right = reTemp.left + 200;
	reTemp.top = reClinet.top + y - 10;
	reTemp.bottom = reTemp.top + 20;

	CString strMsg = _T("검색된 종목이 없습니다.");
	if(m_nType == COND_INVALID)
	{
		reTemp.left = reClinet.left + x - 200;
		reTemp.right = reTemp.left + 400;

		strMsg = _T("유효한 조건이 아닙니다. 설정된 조건을 확인 하시기 바랍니다.");
	}
	
	m_staticText.Create(strMsg, WS_VISIBLE | WS_CHILD | SS_CENTERIMAGE | SS_CENTER,
		reTemp, this, 5000);
