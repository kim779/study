					 ...
					 LPCTSTR strSubTRCode, IDispatch * FormDisp, LPCTSTR strMapFileName, HWND hFormMng, CMapStringToOb * ArrCtrlName, long lBkColor, int nAddView)
{
	...
	*ppScrWnd = pWrapWnd->m_pView;
	...
	((CTrMainView*)pWrapWnd->m_pView)->m_hFormMng = hFormMng;
	((CTrMainView*)pWrapWnd->m_pView)->m_lBkColor = lBkColor;
	((CTrMainView*)pWrapWnd->m_pView)->m_MapControlName = ArrCtrlName;
	((CTrMainView*)pWrapWnd->m_pView)->m_pFormDisp = FormDisp;
...
int CViewWrap::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if( !CreateView( RUNTIME_CLASS( CTrMainView ) ) )
		return -1;
