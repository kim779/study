...
LONG CFnSetScreenWnd::ReceiveTRx(int nKey, char* pBytes, int nLen)
{
	...
	case TRKEY_DRAW_RESULT :
		...
		break;
	case TRKEY_CANDLE_RESULT :
		pScreenWnd->ReceiveTR(TRNO_CANDLE_RESULT, (LPVOID)pBytes, (long)nLen);
		break;
...
{
	CString strDataFile;
	strDataFile.Format("%s%s\\%s\\%s", Variant(homeCC), FOLDER_FIND, gszCacheDir, "MstCode.dat");
...
{
	CString strDataFile;
	strDataFile.Format("%s%s\\%s\\%s", Variant(homeCC), FOLDER_FIND, gszCacheDir, "\\WorkDate.dat");
	{
		CFileFind find;
		if(find.FindFile(strDataFile) == FALSE)
			return 0;
