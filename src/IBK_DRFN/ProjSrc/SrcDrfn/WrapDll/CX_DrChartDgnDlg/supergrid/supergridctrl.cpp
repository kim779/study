...
CSuperGridCtrl::CSuperGridCtrl()
{
	...
	// kanam add
	m_clrSelBackColor = GetSysColor(RGB(171,195,211));
	m_clrSelTextColor = GetSysColor(COLOR_HIGHLIGHTTEXT);
	m_pCopyBufItem = NULL;
}
...
CSuperGridCtrl::~CSuperGridCtrl()
{
	...
	if (m_pCopyBufItem != NULL)
	{
		delete m_pCopyBufItem;
		m_pCopyBufItem = NULL;
	}
}
...
//			crText = GetSysColor (COLOR_HIGHLIGHTTEXT);
			// kanam add
			crBackground = m_clrSelBackColor;
			crText = m_clrSelTextColor;
...
void CSuperGridCtrl::DrawComboBox(CDC* pDC, CTreeItem *pSelItem, int nItem, int nColumn,int iSubIconOffset)
{
	...
	if(pInfo->GetControlType(nColumn, ctrlType))
	{
...
//			MakeColumnVisible(Header_OrderToIndex(pHeader->m_hWnd, m_CurSubItem));
			OnControlLButtonDown(0, pNMListView->ptAction, ht);
...
BOOL CSuperGridCtrl::DoDragDrop(CTreeItem* pTarget, CTreeItem* pSource)
{
	...
	if (ItemHasChildren(pTarget)) {
		...
		CTreeItem* pNewParent = InsertItemBefore(pTarget, lp, bUpdate); 
		if(ItemHasChildren(pSource))
			CopyChildren(pNewParent, pSource);
	}
...
//									DWORD dwStyle = WS_BORDER | WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL|ES_LEFT;
				...
				case 'C':
					if (GetKeyState(VK_CONTROL) & 0x8000)
					{
						...
						CTreeItem *pSelItem = (CTreeItem *)GetItemData(nIndex);

						CItemInfo *lpSrc = GetData(pSelItem);
						if (m_pCopyBufItem != NULL)
						{
							delete m_pCopyBufItem;
							m_pCopyBufItem = NULL;
				...
				case 'V':
					if (GetKeyState(VK_CONTROL) & 0x8000)
					{
						if (m_pCopyBufItem != NULL)
						{
							...
							CTreeItem *pSelItem = (CTreeItem *)GetItemData(nIndex);
							CItemInfo *lpDes = CopyData(m_pCopyBufItem);

							if (ItemHasChildren(pSelItem))
								CTreeItem* pNewParent = InsertItem(pSelItem, lpDes, TRUE);
							else
								CTreeItem* pNewParent = InsertItemBefore(pSelItem, lpDes, TRUE); 
						}
					}
					break;
...
//insert item and return new parent pointer.
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertItemBefore(CTreeItem *pTarget, CItemInfo* lpInfo,  BOOL bUpdate)
{
	CTreeItem *pParent = GetParentItem(pTarget);
	...
	while (pos) {
		if (pParent->m_listChild.GetPrev(pos) == pTarget) {
			if (pos) pParent->m_listChild.InsertAfter(pos, pItem);
			else pParent->m_listChild.AddHead(pItem);
			break;
		}
	}
...
//insert item and return new parent pointer.
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertItemAfter(CTreeItem *pTarget, CItemInfo* lpInfo,  BOOL bUpdate)
{
	CTreeItem *pParent = GetParentItem(pTarget);
	...
	while (pos) {
		if (pParent->m_listChild.GetNext(pos) == pTarget) {
			if (pos) pParent->m_listChild.InsertBefore(pos, pItem);
			else pParent->m_listChild.AddTail(pItem);
			break;
		}
	}
...
void CSuperGridCtrl::OnControlLButtonDown(UINT nFlags, CPoint point, LVHITTESTINFO& ht)
{
	...
	{	
		CItemInfo* pInfo = GetData(pSelItem);
		CItemInfo::CONTROLTYPE ctrlType;
		if(pInfo->GetControlType(ht.iSubItem, ctrlType)) {	
			if(ctrlType==pInfo->CONTROLTYPE::combobox) {
				CStringArray* list=NULL; CDWordArray *ids=NULL;
				pInfo->GetListData(ht.iSubItem, list);
				pInfo->GetListItemData(ht.iSubItem, ids);
				CComboBox * pList = ShowList(ht.iItem, ht.iSubItem, list, ids);
			}
			else if (ctrlType==pInfo->CONTROLTYPE::edit) {
				if(ht.iSubItem==0)
				{
					CRect rcItem;
...
BOOL CSuperGridCtrl::OnVkReturn(void)
{
	...
	if( GetCurSubItem() != -1 && iItem != -1)
	{
		...
		{	
			...
			CItemInfo* pInfo = GetData(pSelItem);
			CItemInfo::CONTROLTYPE ctrlType;
			if(pInfo->GetControlType(iSubItem, ctrlType))
			{	
				switch(ctrlType)
				{
					...
					case pInfo->CONTROLTYPE::combobox: 
						{
							...
							CStringArray* list=NULL;  CDWordArray *ids=NULL;
							pInfo->GetListData(iSubItem, list);
							pInfo->GetListItemData(iSubItem, ids);
							CComboBox * pList = ShowList(iItem, iSubItem, list, ids);
...
BOOL CSuperGridCtrl::IsRoot(int nIndex)
{
	CTreeItem *pItem = (CTreeItem *)GetItemData(nIndex);
	return IsRoot(pItem);
}

BOOL CSuperGridCtrl::IsFolder(int nIndex)
{
	CTreeItem *pItem = (CTreeItem *)GetItemData(nIndex);
...
void CSuperGridCtrl::DeleteSelectItem()
{
	int nIndex = GetSelectedItem();
	if (nIndex == -1) return;
	CTreeItem *pSelItem = (CTreeItem *)GetItemData(nIndex);
...
CComboBox* CSuperGridCtrl::ShowList(int nItem, int nCol, CStringArray *lstItems, CDWordArray *ids)
{
...
//	DWORD dwStyle =  WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL|CBS_DROPDOWNLIST | CBS_DISABLENOSCROLL;
	...
	CComboBox *pList = new CComboInListView(nItem, nCol, lstItems, ids);
...
int CSuperGridCtrl::CalcHorzExtent(CWnd* pWnd, CStringArray *pList)
{
	int nExtent=0;
	if(pWnd!=NULL)
	{
