...
#ifdef _SUPER_GRID_DLL
#define SUPER_GRID_DLL	__declspec( dllexport )
#else
#define SUPER_GRID_DLL	__declspec( dllimport )
#endif

#define IDC_COMBOBOXINLISTVIEW 0x1235

class SUPER_GRID_DLL CItemInfo //should have called this LV_INSERTITEM ..what ever
{
public:
	...
	void SetListData(int iSubItem, CStringArray *strInitArr, CDWordArray *parrIDs=NULL)
	{
		CStringArray *list;
		list = new CStringArray;//will be deleted in destructor
		list->Copy(*strInitArr);
		m_listdata.SetAt(iSubItem, list);
		if (parrIDs != NULL) {
			CDWordArray *ids = new CDWordArray;//will be deleted in destructor
			ids->Copy(*parrIDs);
			m_listitemdata.SetAt(iSubItem, ids);
		}
	}	

	BOOL GetListData(int iSubItem, CStringArray*& pList) const
	{	
		return m_listdata.Lookup(iSubItem, pList);
	}

	BOOL GetListItemData(int iSubItem, CDWordArray*& pIDs) const
	{	
		return m_listitemdata.Lookup(iSubItem, pIDs);
	}
	...
	~CItemInfo()
	{
		...
		pos = m_listitemdata.GetStartPosition();
		while(pos != NULL)
		{
			int nKey;
			CDWordArray* b; 
			m_listitemdata.GetNextAssoc(pos, nKey, b);
			if(b!=NULL)
			{
				b->RemoveAll();
				delete b;
...
private:
	...
	CMap<int,int, CStringArray*, CStringArray*> m_listdata;//listbox
	CMap<int,int, CDWordArray*, CDWordArray*> m_listitemdata;//listbox의 itemdata
...
//
/////////////////////////////////////////////////////////////////////////////
class SUPER_GRID_DLL CSuperGridCtrl : public CListCtrl
{
// Construction
public:
	CSuperGridCtrl();
...
public:
	...
	CTreeItem* InsertItemBefore(CTreeItem *pTarget, CItemInfo* lpInfo,  BOOL bUpdate=FALSE);
//  이상신	20080924	[Insert function]	>>
	CTreeItem* InsertItemAfter(CTreeItem *pTarget, CItemInfo* lpInfo,  BOOL bUpdate=FALSE);
//======================[Insert function]	<<
	...
	BOOL IsRoot(int nIndex);
	BOOL IsFolder(int nIndex);
...
protected:
	...
	// kanam add
	void	SetSelBackColor(COLORREF clr) { m_clrSelBackColor = clr; }
	void	SetSelTextColor(COLORREF clr) { m_clrSelTextColor = clr; }
...
private:
	...
	CComboBox* ShowList(int nItem, int nCol, CStringArray *lstItems, CDWordArray *ids=NULL);
	int CalcHorzExtent(CWnd* pWnd, CStringArray *pList);
...
//  이상신	20081204	[Copy Data Buffer]	>>
	CItemInfo	*m_pCopyBufItem;
//======================[Copy Data Buffer]	<<
...
//the nested class 
class SUPER_GRID_DLL CSuperGridCtrl::CTreeItem : public CObject
{
