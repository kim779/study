...
public:
	...
	// Return		: 계좌인풋 초기화(생성) 성공여부
	// Paramter	
	//		CDialog* pParent : 화면dll의 Dialog(CExOCXDLL가 Parent인 Dialog)
	//		CRect& rcControl : 계좌 컨트롤의 Rect
	//					   ResourceEditor에서 Rect값을 구하기 위해 컨트롤을 사용했다면 
	//					   pRect값을 입력할 필요없습니다.
	//		UINT nID : 계좌컨트롤 리소스 ID 
	//				   리소스 에디터에서 Rect값을 구하기 위해 컨트롤을 사용했다면 그 컨트롤의 
	//				   ID를 입력하십시오. 컨트롤의 Rect를 구해서 계좌컨트롤을 생성합니다.
	//		UINT nProperties : 일반적으로 사용되는 속성 구분값
	// Comments		: ocx기반 컨트롤인 계좌인풋을 생성합니다.
	//				  CBaseDesk2* 를 넘겨 데스크의 정보를 계좌인풋에게 세팅하고
	//				  계좌인풋으로부터 이벤트를 받을 수 있도록 합니다.
	//				  계좌인풋을 사용하기 위해서는 반드시 호출하애 하는 함수입니다.
	...
	// Return		: 계좌인풋 초기화(생성) 성공여부
	// Paramter	
	//		CExOCXDLL* pParent : 화면dll의 Dialog(CExOCXDLL)
	//		CRect& rcControl : 계좌 컨트롤의 Rect
	//					   ResourceEditor에서 Rect값을 구하기 위해 컨트롤을 사용했다면 
	//					   pRect값을 입력할 필요없습니다.
	//		UINT nID : 계좌컨트롤 리소스 ID 
	//				   리소스 에디터에서 Rect값을 구하기 위해 컨트롤을 사용했다면 그 컨트롤의 
	//				   ID를 입력하십시오. 컨트롤의 Rect를 구해서 계좌컨트롤을 생성합니다.
	//		CStringList* pListProperty : 계좌인풋에 필요한 속성문자열을 가지고 있는 리스트 포인터
	// Comments		: ocx기반 컨트롤인 계좌인풋을 생성합니다.
	//				  CBaseDesk2* 를 넘겨 데스크의 정보를 계좌인풋에게 세팅하고
	//				  계좌인풋으로부터 이벤트를 받을 수 있도록 합니다.
	//				  계좌인풋을 사용하기 위해서는 반드시 호출하애 하는 함수입니다.
	...
	// Paramter
	//		LPCSTR lpszEventName : 계좌인풋으로부터 시작된 이벤트의 이름입니다.
	//		LPCSTR lpszParam : 이벤트로부터 넘겨지는 파라메터 입니다.
	// Comments		: 계좌인풋에서 발생한 이벤트를 넘겨받는 함수입니다.
	//				  virtual로 선언되어있으므로 CExAcctInput를 상속받아 사용하는 class에서
	//				  다시 선언하여 사용할 수 있습니다.
	...
	//				  이벤트는 계좌인풋ocx가 추가/삭제됨에 따라 화면 dll 처리도 수정될 수 있습니다.
	//
	//				  화면 Dialog에서 이벤트를 받을때에는 RMSG_EVENT_ACCTINPUT 메세지로 받습니다.
	//				  RMSG_EVENT_ACCTINPUT는 _INCLUDE/ExCommonHeader.h에 정의되어있습니다.
