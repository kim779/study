...
//
// For use with CExGridCtrl v2.22+
//
// History:
...
//            TextClr = ::GetSysColor(COLOR_HIGHLIGHTTEXT);
			// 수정 LYH
            TextBkClr = pGrid->GetHighlightColor();
			if(pGrid->GetUseHighlightText() != FALSE) // 추가 LYH
				TextClr = pGrid->GetHighlightTextColor();
            bEraseBkgnd = TRUE;
        }
        ...
        if (pGrid->GetFrameFocusCell() && pGrid->GetUseFocusRect())
        {
    ...
    {
        rect.right++; rect.bottom++;    // FillRect doesn't draw RHS or bottom
//        pDC->FillSolidRect(rect, ::GetSysColor(COLOR_HIGHLIGHT));// 수정 LYH
		pDC->FillSolidRect(rect, pGrid->GetHighlightColor());// 수정 LYH
        rect.right--; rect.bottom--;
//        pDC->SetTextColor(::GetSysColor(COLOR_HIGHLIGHTTEXT));// 수정 LYH
		if(pGrid->GetUseHighlightText() != FALSE) // 추가 LYH
			TextClr = pGrid->GetHighlightTextColor();
		pDC->SetTextColor(TextClr);// 수정 LYH
    }
    else
    {
        if(bEraseBkgnd)
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        {
            CPen lightpen(PS_SOLID, 1,  pGrid->GetFixedGridLineColor_Light()),
                darkpen(PS_SOLID,  1, pGrid->GetFixedGridLineColor_Dark());
//			// 수정 LYH : 그냥 까만색은 안이쁘잖아. 옵션화한다.
...
//                darkpen(PS_SOLID,  1, ::GetSysColor(COLOR_3DDKSHADOW));
    ...
    rect.DeflateRect(GetMargin(), 1); // - changed by Yogurt
...
//    DrawText(pDC->m_hDC, GetText(), -1, rect, GetFormat() | DT_NOPREFIX);
	// 수정 20050322 : DrawText -> TextOut 로 전환하는 작업
	CString strCellText = GetText();
	UINT nCellFormat = GetFormat();
	int	nTextLeft, nTextTop, nTextLen = strCellText.GetLength();
	GetDrawTextRect(pDC, rect, strCellText, nTextLen, nTextLeft, nTextTop);
	pDC->ExtTextOut(nTextLeft, nTextTop, ETO_CLIPPED, (LPCRECT)rect, strCellText, nTextLen, NULL); //FIND:DRAW_TUNING //by LEJ, 2005.02.07
...
// 함수설명 : format 을 대체하도록 text 의 위치를 계산한다.
// N O T E : 추가 20050322 : DrawText -> TextOut 로 전환하는 작업
void CGridCellBase::GetDrawTextRect(CDC* pDC, CRect rect, CString text, int nLen, int& left, int& top)
{
	DWORD dwFormat = GetFormat();
	SIZE size;

	GetTextExtentPoint(pDC->m_hDC,text,nLen,&size);

	/* default : DT_LEFT (==0) */	left = rect.left; 
	if(dwFormat & DT_RIGHT)			left = rect.right - size.cx;	
	else if(dwFormat & DT_CENTER)	left = rect.left + (rect.right - rect.left - size.cx) /2;

	/* default : DT_TOP (==0) */	top = rect.top+1;
	if(dwFormat & DT_VCENTER)		top = int(rect.top + double(rect.bottom - rect.top - size.cy)/2.0);
	else if(dwFormat & DT_BOTTOM)	top = rect.bottom - size.cy - 1;
}
