...
DECLARE_INTERFACE(ITLMCallback)
{
	...
	STDMETHOD_(void, ReceiveData)(int nRtnState, char aDataType, long dwReserved, int nLength, void* pData) PURE;
...
DECLARE_INTERFACE(ITreeListManager)
{
	...
	STDMETHOD_(LONG, LoadTreeList)(CInputCodeList *pInData, LPCSTR szSection="TREELIST") PURE;
	STDMETHOD_(void, UnLoadTreeList)(LONG dwKey) PURE;
//	STDMETHOD_(BOOL, LoadFile)(CInputCodeList *pInData, LPCSTR szSection="LIST") PURE;
...
===================================================================== */
#define		TREELIST_DEFAULT_TR			"88803"
...
{
public:
	CBaseLongData() { m_nType=BaseLongData_Item; }
...
public:
	...
	virtual void  Convert(char* pData) 
	{
		...
		char sTmp[20+1];
		m_cGb = pData[0];   pData=&pData[1];
		nL= 6; memcpy(sTmp, pData, nL); sTmp[nL]=0; pData=&pData[nL]; m_szcode=sTmp;	m_szcode.TrimRight();
		...
		nL= 9; memcpy(sTmp, pData, nL); sTmp[nL]=0; pData=&pData[nL]; m_szchange=sTmp;	m_szchange.TrimRight();
		nL= 9; memcpy(sTmp, pData, nL); sTmp[nL]=0; pData=&pData[nL]; m_szprice=sTmp;	m_szprice.TrimRight();
		nL= 5; memcpy(sTmp, pData, nL); sTmp[nL]=0; pData=&pData[nL]; m_szchrate=sTmp;	m_szchrate.TrimRight();
