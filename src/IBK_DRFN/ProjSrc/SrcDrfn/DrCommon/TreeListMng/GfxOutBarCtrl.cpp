...
CGfxOutBarCtrl::CGfxOutBarCtrl()
{
	...
	crDkShadowBorder = GetSysColor(COLOR_3DDKSHADOW); //선택시 란인색
...
void CGfxOutBarCtrl::OnSize(UINT nType, int cx, int cy) 
{
	...
	for (t = 0; t < max; t++)
	{
		...
		if (pc) 
			pc->SetWindowPos(0, rc.left+5, rc.top, rc.Width(), rc.Height(), SWP_NOZORDER);
	}
}
