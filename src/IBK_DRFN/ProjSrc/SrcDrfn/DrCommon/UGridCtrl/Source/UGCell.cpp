...
int CUGCell::AddCellInfo(CUGCell *src,CUGCell *dest){
	...
	//## Modify #########
	if (src->m_propSetFlags & UGCELL_OTHER_SET)
	{
		dest->m_lOtherData = src->m_lOtherData;
		dest->m_nOtherData = src->m_nOtherData;

		dest->m_strJongCode = src->m_strJongCode;
		dest->m_strCellText = src->m_strCellText;
	}

  if(src->m_propSetFlags & UGCELL_IMAGELIST_SET)
	{
		dest->m_pImageList = src->m_pImageList;
		dest->m_nImgIndex = src->m_nImgIndex;
	}

  if(src->m_propSetFlags & UGCELL_SIGN_SET)
		dest->m_nSign = src->m_nSign;
	else if (src->m_nSign >= 0 && src->m_nSign <= 5)
		dest->m_nSign = src->m_nSign;

  if(src->GetCellTypeEx() & UGCELL_CANDLE_SET)
  {
	  dest->m_strCandleData = src->m_strCandleData;
  }

  if(src->GetCellTypeEx() & UGCELL_LHCANDLE_SET)
  {
	  dest->m_strLHCandleData = src->m_strLHCandleData;
  }
...
int	CUGCell::SetOtherDataCellText(LPCTSTR text)
{
	m_strCellText = text;
	m_dataType	= UGCELLDATA_OTHER;
	m_propSetFlags |= (UGCELL_OTHER_SET + UGCELL_DATATYPE_SET);

	return UG_SUCCESS;
}

CString	CUGCell::GetOtherDataCellText()
{
	return m_strCellText;
}
   

int	CUGCell::SetOtherDataInt(int nOtherData)
{
	m_nOtherData = nOtherData;
...
int	CUGCell::GetOtherDataInt()
{
	//if (m_propSetFlags & UGCELL_OTHER_SET)
		return m_nOtherData;

	//return -1;
}

int	CUGCell::SetOtherDataLong(long lOtherData)
{
	m_lOtherData = lOtherData;
...
int	CUGCell::SetOtherDataDouble(double dOtherData)
{
	m_dOtherData = dOtherData;
...
double CUGCell::GetOtherDataDouble()
{
	//if (m_propSetFlags & UGCELL_OTHER_SET)
		return m_dOtherData;
...
int	CUGCell::SetOtherJongCode(CString strJongCode)
{
	m_strJongCode = strJongCode;
...
CString	CUGCell::GetOtherJongCode()
{
	//if (m_propSetFlags & UGCELL_OTHER_SET)
		return m_strJongCode;
...
void CUGCell::SetImageList(int nImageSel, CImageList* pImgList)
{
	if (!pImgList)
	{
		if (m_propSetFlags & UGCELL_IMAGELIST_SET)
			m_propSetFlags &= ~UGCELL_IMAGELIST_SET;
	...
	m_nImgIndex = nImageSel;
	m_propSetFlags |= UGCELL_IMAGELIST_SET;
}

BOOL CUGCell::SetSign(int nSign)
{
	if (nSign > (int)SIGN_DOWN || nSign < (int)SIGN_MAX)
		return UG_ERROR;

	m_nSign = nSign;
	m_propSetFlags |= UGCELL_SIGN_SET;
	return UG_SUCCESS;
}

BOOL CUGCell::SetCandleData(CString strCandleData)
{
	m_strCandleData = strCandleData;
	
	return UG_SUCCESS;
}

BOOL CUGCell::SetLHCandleData(CString strLHCandleData)
{
	m_strLHCandleData = strLHCandleData;

	return UG_SUCCESS;
}

CString CUGCell::GetCandleData() 
{
	return m_strCandleData;
}

CString CUGCell::GetLHCandleData() 
{
	return m_strLHCandleData;
}