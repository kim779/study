		...
		while(1){
			...
			if(point.x < 0){
				...
				if(m_GI->m_ballisticMode){
					//int increment = (int)pow((point.x * -1)/ m_GI->m_defColWidth +1, m_GI->m_ballisticMode); 
					int increment = (int)pow(((long double)(point.x * -1))/ ((int)m_GI->m_defColWidth +1), 
			...
			//if the mouse is off the right side
				...
				//if ballistic mode
				if(m_GI->m_ballisticMode){
					//int increment = (int)pow((point.x - m_GI->m_gridWidth) / m_GI->m_defColWidth +1, m_GI->m_ballisticMode); 
					int increment = (int)pow(((long double)(point.x - m_GI->m_gridWidth)) / 
						((int)m_GI->m_defColWidth +1), m_GI->m_ballisticMode); 
					m_ctrl->GotoCol(m_GI->m_dragCol + increment);
					if(increment == 1)
						Sleep(m_GI->m_ballisticDelay);
				}
			...
			if(point.y < 0){
				//if ballistic mode
				if(m_GI->m_ballisticMode){
					//long increment = (long)pow((point.y* -1) / m_GI->m_defRowHeight +1, m_GI->m_ballisticMode); 
					long increment = (long)pow(((long double)(point.y* -1)) / ((int)m_GI->m_defRowHeight +1), 
						m_GI->m_ballisticMode);
					m_ctrl->GotoRow(m_GI->m_dragRow - increment);
					if(increment == 1)
						Sleep(m_GI->m_ballisticDelay);
			...
			//if the mouse is below the bottom
				...
				//if ballistic mode
				if(m_GI->m_ballisticMode){
					//long increment = (long)pow((point.y-m_GI->m_gridHeight) / m_GI->m_defRowHeight +1, m_GI->m_ballisticMode); 
					long increment = (long)pow(((long double)(point.y-m_GI->m_gridHeight)) / 
						((int)m_GI->m_defRowHeight +1), m_GI->m_ballisticMode);
					m_ctrl->GotoRow(m_GI->m_dragRow + increment);
					if(increment == 1)
						Sleep(m_GI->m_ballisticDelay);
				}
