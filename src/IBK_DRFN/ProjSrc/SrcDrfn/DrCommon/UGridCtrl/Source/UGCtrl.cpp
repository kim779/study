...
void CUGCtrl::AdjustComponentSizes(){
	...
	OnAdjustComponentSizes(&m_GI->m_gridRect, &m_GI->m_topHdgRect,
													&m_GI->m_sideHdgRect, &m_GI->m_cnrBtnRect, 
													&m_GI->m_vScrollRect, &m_GI->m_hScrollRect, 
													&m_GI->m_tabRect);
...
int CUGCtrl::GetCellFromPoint(int px,int py,int *ptcol,long *ptrow,RECT *rect){
	...
	if(px < 0 || px > m_GI->m_gridWidth)
		return 1;
	if(py < 0 || py > m_GI->m_gridHeight+GetTH_Height())
		return 1;
	...
	for(y=0;y<=m_GI->m_numberRows;y++){
		...
		if(m_GI->m_numberRows==y)	height += GetRowHeight(y-1);
		else		height += GetRowHeight(y);
...
/***************************************************
	...
	Return
***************************************************/
int	CUGCtrl::AppendRow(BOOL bRedraw){
	
	int rt = m_GI->m_defDataSource->AppendRow();
	if(rt == UG_SUCCESS){
		SetNumberRows(m_GI->m_numberRows + 1, bRedraw);
	}
	return rt;
}
/***************************************************
