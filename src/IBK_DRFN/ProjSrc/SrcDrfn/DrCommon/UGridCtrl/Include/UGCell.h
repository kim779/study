...
//# Danny Modify!
#define UGCELL_OTHER_SET	      671088642
#define UGCELL_IMAGELIST_SET  BIT30
#define UGCELL_SIGN_SET       BIT31
//
#define UGCELL_CANDLE_SET	  BIT29
#define UGCELL_LHCANDLE_SET	  BIT28

#define SIGN_MAX          1
#define SIGN_UP           2
#define SIGN_STEADY       3
#define SIGN_DOWN         5
#define SIGN_MIN          4
...
#define UGCELLDATA_OTHER			6


class CUGCtrl;
class AFX_EXT_CLASS CUGCell: public CObject{
protected:
...
public:
	...
	//## Modify ########################
	int	SetOtherDataInt(int nOtherData);
	int	GetOtherDataInt();

	int	SetOtherDataLong(long lOtherData);
	long GetOtherDataLong();

	int	SetOtherDataDouble(double dOtherData);
	double GetOtherDataDouble();

	int	SetOtherJongCode(CString strJongCode);
	CString	GetOtherJongCode();

	int	SetOtherDataCellText(LPCTSTR text);
	CString	GetOtherDataCellText();
	//##################################
  ...
  //# Danny Modify : 2006.03.07
	CImageList* m_pImageList;
	int	m_nImgIndex;
	void SetImageList(int nImgIndex, CImageList* pImgList);

	int m_nSign;
	BOOL SetSign(int nSign);
	int GetSign() {return m_nSign;}

	CString m_strCandleData;
	BOOL SetCandleData(CString strCandleData);
	CString GetCandleData();

	CString m_strLHCandleData;
	BOOL SetLHCandleData(CString strLHCandleData);
	CString GetLHCandleData();
};
