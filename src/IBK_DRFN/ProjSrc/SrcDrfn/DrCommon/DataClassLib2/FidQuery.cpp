...
void CFidPack::SetGridSymbol(CString strSymbol)
{
	...
	for (int i=0; i<nSize; i++)
	{
		pBlock = m_arrBlock.GetAt(i);
		if (pBlock->m_bArray)
			pBlock->m_strCntItemID = strSymbol;
	}
}
...
CString CFidQuery::GetCodeFID(int nMarket)
{
	...
	else if (nMarket == MARKET_FUTOPT)	return _T("20301");
	else if (nMarket == MARKET_UPJONG)	return _T("30301");
...
{
	if (nType == 410)		return _T("101208");		//	틱
	else if (nType == 510)	return _T("101308");		//	분
	else if (nType == 610)	return _T("NONE");			//	일
//	else if (nType == 610)	bozoInput.m_sTerm = "2";	//	주
//	else if (nType == 610)	bozoInput.m_sTerm = "2";	//	월
//	else if (nType == 610)	bozoInput.m_sTerm = "2";	//	초
