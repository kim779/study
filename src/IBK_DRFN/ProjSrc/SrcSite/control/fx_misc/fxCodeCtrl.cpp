...
#include "../../include/jmcode.h"
#include "../../include/axisvar.h"
#include <imm.h>
...
void CCodeCombo::OnSelchange() 
{
	...
	if (index >= 0)
	{
		if (IsCodeListMode())
		{
			...
			m_pParent->m_pEdit->ListCode(CString(jCode.Code).Mid(1));
			m_pParent->m_pEdit->SetFocus();
		}
		else if (IsHistoryMode())
		{
			jCode = m_pHCode.GetAt(index);
			m_pParent->m_pEdit->ListCode(CString(jCode.Code));
			m_pParent->m_pEdit->SetFocus();
		}
	}
}
...
//		info.Name.FreeExtra();
	...
	for( i = 0; i < m_pHCode.GetSize(); i++ )
	{
//		_JCode info = m_pHCode.GetAt( i );
...
void CCodeCombo::SearchCode(CString code)
{
	...
	if (IsCodeListMode())
	{
		...
		for (int ii = 0; ii < size; ii++)
		{
			...
			if (strlen(jCode.Name) < len)
				continue;

			char buf[40];
			memset( buf, 0, sizeof(buf) );
			strncpy( buf, jCode.Name, len );
						
			if (strcmp(buf,/*CString(jCode.Name).Left(len)*/code ) == 0)
			{
				SetTopIndex(ii);
				SetCurSel(ii);
				return;
...
CString CCodeCombo::GetSelectCode()
{
	...
	if (IsCodeListMode())
	{
		...
		m_sCode = CString(jCode.Code).Mid(1);
	}
	else if (IsHistoryMode())
	{
...
//			jCode.Name = Str.Mid(index + 1);
			...
			/*CopyMemory(jCode.Code, Str.Left(index), sizeof(jCode.Code));
			CopyMemory(jCode.Name, Str.Mid(index + 1), sizeof(jCode.Code));*/
			sprintf(jCode.Code, "%s", Str.Left(index));
			sprintf(jCode.Name, "%s", Str.Mid(index + 1));
			m_pHCode.Add(jCode);
		}
	}
...
//		jCode.Code.Empty();
	//	ZeroMemory(jCode.Code, sizeof(jCode.Code));
	//	ZeroMemory(jCode.Name, sizeof(jCode.Name));
		memset( &jCode, 0, sizeof(struct _JCode) );
	
//		jCode.Name = CString(pHJCode->hnam, HNameLen);
		memcpy(jCode.Name, pHJCode->hnam, 40);			

//		CString(jCode.Name).MakeUpper();
		CopyMemory(jCode.Name, strupr(jCode.Name), sizeof(jCode.Name));
		strtrim( jCode.Name, sizeof(jCode.Name) );	
 
		//if (!CString(jCode.Name).IsEmpty())
		if( strlen(jCode.Name) != 0 )
		{
			fname = jCode.Name[0];
			if (pHJCode->ssgb != jmELW)
				memcpy(jCode.Name, jCode.Name+1/*CString(jCode.Name).Mid(1)*/, sizeof(jCode.Name)-1);
		}
		...
		default:
// updateX_20060209
			if (pHJCode->ssgb == jmELW || (pHJCode->ssgb != jmSINJU && pHJCode->ssgb != jmSINJS))
			{
				//jCode.Code = CString(pHJCode->code, HCodeLen);
				CopyMemory(jCode.Code, pHJCode->code, sizeof(jCode.Code));
				//CString(jCode.Code).TrimRight();
				strtrim( jCode.Code, sizeof(jCode.Code) );
			}
			break;
		}
		if (!CString(jCode.Code).IsEmpty())
			m_pJCode.Add(jCode);
	}
  	CloseHandle(fh);
...
//		return true;
	...
	for (int ii = 0; ;ii++)
	{
		...
		if (pHJCode->ssgb == jmELW)
		{
			//CString(jCode.Code).Empty();
			memset(jCode.Code, 0, sizeof(jCode.Code) );
//			jCode.Code = CString(pHJCode->code, HCodeLen);
			CopyMemory(jCode.Code, pHJCode->code, sizeof(jCode.Code));
			//CString(jCode.Code).TrimRight();
			strtrim( jCode.Code, sizeof(jCode.Code) );
...
//		jCode.Name = CString(pHJCode->hnam, HNameLen);
		CopyMemory(jCode.Name, pHJCode->hnam, sizeof(jCode.Name));
//		CString(jCode.Name).MakeUpper();
		CopyMemory(jCode.Name, strupr(jCode.Name), sizeof(jCode.Name));
		//CString(jCode.Name).TrimRight();
		strtrim( jCode.Name, sizeof(jCode.Name) );

		fname = jCode.Name[0];
//		jCode.Name = jCode.Name.Mid(1);
		//CopyMemory(jCode.Name, jCode.Name+1, sizeof(jCode.Name)+1);
		sprintf(jCode.Name, "%s", jCode.Name+1 );

		if (!CString(jCode.Code).IsEmpty())
		{
//			jCode.Code = jCode.Code.Mid(1);
			//CopyMemory(jCode.Code, jCode.Code+1, sizeof(jCode.Code)+1);
			sprintf(jCode.Code, "%s", jCode.Code+1 );
...
//		fCode.Code = CString(pFjCode->cod2, FCodeLen);
		CopyMemory(fCode.Code, pFjCode->cod2, sizeof(fCode.Code));
		//CString(fCode.Code).TrimRight();
		strtrim( fCode.Code, sizeof(fCode.Code) );
//		fCode.Name = CString(pFjCode->hnam, FNameLen);
		CopyMemory(fCode.Name, pFjCode->hnam, sizeof(fCode.Name));
		//CString(fCode.Name).TrimRight();
		strtrim( fCode.Name, sizeof(fCode.Name) );
		m_pJCode.Add(fCode);
	}
...
/*	char	buf[1024];
	...
	for (int ii = 0; ; ii++)
	{
		...
		for (int jj = 0; jj < 4; jj++)
		{
			...
			{
				CopyMemory(tOCode.Code, oCode.Call[jj].Code, sizeof(tOCode.Code));
				CopyMemory(tOCode.Name, oCode.Call[jj].Name, sizeof(tOCode.Name));
				m_pJCode.InsertAt(0, tOCode);
			}
			if (oCode.Put[jj].Exist == '1')
			{
				CopyMemory(tOCode.Code, oCode.Put[jj].Code, sizeof(tOCode.Code));
				CopyMemory(tOCode.Name, oCode.Put[jj].Name, sizeof(tOCode.Name));
				m_pJCode.InsertAt(0, tOCode);
			}
		}
	}
...
{
	int iSize = m_pJCode.GetSize();

	strCode.TrimRight();

	for( int i = 0; i < iSize; i++ )
	{
		_JCode jcode = m_pJCode.GetAt( i );
		if( jcode.Code == strCode )
			return jcode.Name;
	}
