...
int CDataConversion::RelativeToAbsoluteOutDisplayIndex_X(const CRect& region, const double& realWidth, 
		const int startIndex, const int endIndex, const currentIndex)
{
...
/*! Comment       : 
//******************************************************************/
long  CDataConversion::FtoTbondPrcL( double inprice, int scale, int outdec )
{
	int		i_intnum;
...
BOOL CDataConversion::GetDataToDec(CString strInput, CString strPacketType, CString & strOutput )
{
	...
	//소수점 자리 오버하는 부분은 제거..
	if(strPoint.GetLength() > nDecLen2) strPoint = strPoint.Left(nDecLen2);

	//소수점 자리 부족한 것은 오른쪽으로 0값 채워주기..
	while(nDecLen2 - strPoint.GetLength() > 0) strPoint += "0";

	strPoint	= strMid + strPoint;
	long lPoint = atol(strPoint);
	long double dPoint=0;

	if(nBase == 8)
	{
		dPoint = lPoint / (long double)nBase;
	}
	else if(nBase == 32)
	{
		if(nDecLen1 + nDecLen2 == 4)
			dPoint = lPoint / (long double)(nBase * 100.);
		else
			dPoint = lPoint / (long double)(nBase * 10.);
	}
	else
		dPoint = lPoint / (long double)(nBase * 10.);


	long double dInput  = (long double)fabs(atof(strInt));
	long double dOutput =  (bSign) ? - (dInput + dPoint) : dInput + dPoint;
