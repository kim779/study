...
// log 값
//>>[A00000494]종가가 1.00 보다 작을때 발생하며, 십자선,추세선,툴팁의 Y축 값 오류
/*
...
double CMath::Log(const double& data)
{
	...
	if( data > 0) return log( data + _PSEUDO_ZERO) - log( _PSEUDO_ZERO);
	return -log( -data + _PSEUDO_ZERO) + log( _PSEUDO_ZERO);
}
...
double CMath::Exp(const double& data)
{
	...
	if( data > 0) return exp( data + log( _PSEUDO_ZERO)) - _PSEUDO_ZERO;
	return -exp( -data + log( _PSEUDO_ZERO)) + _PSEUDO_ZERO;
}
//<<[A00000494]
