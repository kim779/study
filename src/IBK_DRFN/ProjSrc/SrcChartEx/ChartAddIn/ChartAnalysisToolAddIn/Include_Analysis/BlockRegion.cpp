...
BOOL CBlockRegionList::GetBlockRegionList( const CPoint& point, IChartManager* pMainBlock)
{
	if( !pMainBlock) return FALSE;
	int nRow, nColumn;
	if( !pMainBlock->GetBlockIndex( point, nRow, nColumn)) return FALSE;
	return GetBlockRegionList( nColumn, pMainBlock);
}

BOOL CBlockRegionList::GetBlockRegionList( const int nColumn, IChartManager* pMainBlock)
{
	if( !pMainBlock) return FALSE;
	...
	CRect rt1,rt2;
	int nRowCount = pMainBlock->GetRowCountInColumn( nColumn);
	for( int i = 0; i < nRowCount; i++)
	{
		IBlock *pIBlock = pMainBlock->GetBlock( i, nColumn);
		if( !pIBlock) return FALSE;

		pIBlock->GetGraphRegions( rt1,rt2);
		CGraphPartRegion graphPartRegion( rt1, rt2);

		pIBlock->GetHorzScaleRegion( rt1, rt2);
		CHorzScaleRegion horzScaleRegion;
		horzScaleRegion.SetTopScaleRegion( rt1);
		horzScaleRegion.SetBottomScaleRegion( rt2);

		pIBlock->GetVertScaleRegion( rt1, rt2);
		CVertScaleRegion vertScaleRegion;
		vertScaleRegion.SetLeftScaleRegion( rt1);
		vertScaleRegion.SetRightScaleRegion( rt2);

		AddTail( CBlockRegion( graphPartRegion, horzScaleRegion, vertScaleRegion));
