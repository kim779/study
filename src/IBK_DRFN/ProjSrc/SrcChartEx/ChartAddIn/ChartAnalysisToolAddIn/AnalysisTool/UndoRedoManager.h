...
public:
	CUndo_Redo_Info();
	CUndo_Redo_Info(int nUndoRedoType, long lElementAddress, CString strElementData, CString strElementData_Prev);
	~CUndo_Redo_Info();
...
public:
	...
	long GetElementAddress();
	void SetElementAddress(long lNewElementAddress);

	CString GetElementData();
	CString GetElementData_Prev();

private:
	int m_nUndoRedoType;
	CBlockIndex m_blockIndex;
	long m_lElementAddress;

	CString m_strElementData;
	CString m_strElementData_Prev;
};
...
public:
	...
	void RegisterElementToUnRedoManager(const int nUndoRedoType, const long lElementAddress, LPCTSTR lpszElementData_Prev = NULL, LPCTSTR lpszElementData = NULL);
	...
	void GetUndoRedoInfoListOfBlock(const CBlockIndex& blockIndex, CDequePointerList<CUndo_Redo_Info>& undoredoInfoList);
...
private:
	CDequePointerList<CUndo_Redo_Info> m_UndoList;
	CDequePointerList<CUndo_Redo_Info> m_RedoList;
};
