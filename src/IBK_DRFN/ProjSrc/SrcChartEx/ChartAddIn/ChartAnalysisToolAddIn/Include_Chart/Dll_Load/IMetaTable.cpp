...
void IMetaTable::LoadMetaTable( void)
{
	if( !m_szMetaTables)
	{
		...
		if(strOcxPath.IsEmpty())
		{
			...
			if( _loc_ > 0)
				_strRoot_ = _strRoot_.Left(_loc_);
			strOcxPath = _strRoot_;
			strOcxPath += "\\Dev\\";

			if(m_strDataFolderPath.IsEmpty())
			{
				m_strDataFolderPath = _strRoot_;
				m_strDataFolderPath += "\\Icss\\";
			}
		}
...
bool IMetaTable::IsDataTitle(const CString& IndicatorName)
{
	...
	switch( GetIndicatorID( IndicatorName))
	{
		...
		case EDDC_OCX_BOXCHARTCHART:
		case EDDC_OCX_VOLUMEFORSALE:
		case EDDC_OCX_VOLUMESELLBUYCHART:
												return false;
	}
...
bool IMetaTable::IsNoneZoomChart( const char *p_szIndicatorName)
{
	switch( GetIndicatorID( p_szIndicatorName))
	{
		case EDDC_OCX_INVERSLINECHART:
...
// 		case EDDC_ST_VOLUMEMACHART:
		...
		case EDDC_ST_PSYCHCHART:
		case EDDC_ST_NEW_PSYCHOLOGY_LINE:
		case EDDC_ST_DISPARITYCHART_SMA:
		case EDDC_ST_DISPARITYCHART_EMA:
...
BOOL IMetaTable::CanNotMultipleAddedOnMultipleSameNameIndicator( const char *p_szIndicatorName)
{
	...
	switch( GetIndicatorID( p_szIndicatorName))
	{
		...
		case EDDC_VOLUMECHART:
		case EDDC_ST_DEMACHART:
		case EDDC_ST_PIVOTCHART:			return TRUE;
	}
	return FALSE;
}

// (2009/9/16 - Seung-Won, Bae) for Market Index and Server Data
BOOL IMetaTable::IsMarketOrServerIndex( const char *p_szIndicatorName)
{
	if( !p_szIndicatorName) return FALSE;
	if( !*p_szIndicatorName) return FALSE;
	...
	switch( GetIndicatorID( p_szIndicatorName))
	{
		case EDDC_ORGAN_PUREBUY_ACCUMULATION:
		case EDDC_ORGAN_PUREBUY:
		case EDDC_SUSPENSE_CONTRACT:
		case EDDC_TOTAL_AMOUNT:
		case EDDC_FOREIGN_HOLDING:
		case EDDC_FOREIGN_HOLDING_RATE:
		case EDDC_FOREIGN_PUREBUY:
		case EDDC_KOSPI_PER_INVESTER:
		case EDDC_KOSDAQ_PER_INVESTER:
		case EDDC_K200_PER_INVESTER:
		case EDDC_FUTURE_PER_INVESTER:
		case EDDC_CALL_OPTION_PER_INVESTER:
		case EDDC_PUT_OPTION_PER_INVESTER:
		case EDDC_PROGRAM_TRADE_PUREBUY_KOSPI:
		case EDDC_PROGRAM_TRADE_PUREBUY_KOSDAQ:
		case EDDC_VOLUMEPRICE:
		case EDDC_PERSON_PUREBUY://844
		case EDDC_PERSON_PUREBUYACCUM: //845
		case EDDC_FOREIGN_PUREBUYACCM://846
		case EDDC_ORGAN_FINANCE_PUREBUYACCUM:		// 기관(금융투자) 순매수누적
		case EDDC_ORGAN_FINANCE_PUREBUY:			// 기관(금융투자) 순매수량
		case EDDC_ORGAN_INSURE_PUREBUYACCUM:		// 기관(보험) 순매수누적
		case EDDC_ORGAN_INSURE_PUREBUY:				// 기관(보험) 순매수량
		case EDDC_ORGAN_INVESTMENT_PUREBUYACCUM:	// 기관(투신) 순매수누적
		case EDDC_ORGAN_INVESTMENT_PUREBUY:			// 기관(투신) 순매수량
		case EDDC_ORGAN_PRIVATEEQUITY_PUREBUYACCUM:	// 기관(사모) 순매수누적
		case EDDC_ORGAN_PRIVATEEQUITY_PUREBUY:		// 기관(사모) 순매수량
		case EDDC_ORGAN_BANK_PUREBUYACCUM:			// 기관(은행) 순매수누적
		case EDDC_ORGAN_BANK_PUREBUY:				// 기관(은행) 순매수량
		case EDDC_ORGAN_ETCFINANCE_PUREBUYACCUM:	// 기관(기타금융) 순매수누적
		case EDDC_ORGAN_ETCFINANCE_PUREBUY:		// 기관(기타금융) 순매수량
		case EDDC_ORGAN_PENSIONFUND_PUREBUYACCUM:	// 기관(연기금) 순매수누적
		case EDDC_ORGAN_PENSIONFUND_PUREBUY:		// 기관(연기금) 순매수량
		case EDDC_ORGAN_COUNTRY_PUREBUYACCUM:		// 기관(국가) 순매수누적
		case EDDC_ORGAN_COUNTRY_PUREBUY:			// 기관(국가) 순매수량
		case EDDC_ORGAN_ETCCORP_PUREBUYACCUM:		// 기관(기타법인) 순매수누적
		case EDDC_ORGAN_ETCCORP_PUREBUY:			// 기관(기타법인) 순매수량
		case EDDC_ADL:			// ADL 업종 시장지표
		case EDDC_ADR:			// ADR 업종 시장지표
		case EDDC_ORGAN_CREDIT_BALANCE_RATE:			// 신용잔고율
		case EDDC_ORGAN_CREDIT_GIVING_RATE:				// 신용공여율
		case EDDC_ORGAN_CREDIT_BALANCE_QTY:				// 신용잔고수량
		case EDDC_ORGAN_CREDIT_BALANCE_VARIATION:		// 신용잔고증감
		case EDDC_FOREIGN_ORGAN_PUREBUY:				// 외국인+기관 순매수
		case EDDC_FOREIGN_ORGAN_PUREBUYACCUM:			// 외국인+기관 순매수누적
...
BOOL IMetaTable::IsIndexNeedVolume( const char *p_szIndicatorName)
{
	if( !p_szIndicatorName) return FALSE;
	if( !*p_szIndicatorName) return FALSE;
...
// 		case EDDC_ST_VOLUMEMACHART:
		...
		case EDDC_OCX_INVERSLINECHART:
		case EDDC_OCX_RENKOCHART:
		case EDDC_DISPERSIONCHART:
		case EDDC_OCX_MARKETPROFILECHART:		return TRUE;
	}
	return FALSE;
}
BOOL IMetaTable::IsIndicatorInThisMode( const char *p_szIndicatorName, CChartInfo::CHART_MODE p_eChartMode)
{
	if( !p_szIndicatorName) return FALSE;
	if( !*p_szIndicatorName) return FALSE;
	...
	{
		case CChartInfo::FX:
				return !IsMarketOrServerIndex( p_szIndicatorName) && !IsIndexNeedVolume( p_szIndicatorName);
		case CChartInfo::WORLD_ON: 
			return !IsMarketOrServerIndex( p_szIndicatorName);
	}
	return TRUE;
}
...
BOOL IMetaTable::IsSpecialTime( int p_nTime)
{
	static CTypedPtrMap < CMapPtrToPtr, void *, void *> mapSpecialTimes;
	if( mapSpecialTimes.GetCount() <= 0)
	{
		// (2009/10/22 - Seung-Won, Bae) 특수코드 시간값을 관리한다.
		// -> 장전종료(666666) 장마감종료(777777) 장종료(888888) 시간외종료(999999)
		mapSpecialTimes.SetAt( ( void *)atoi( "666666"), 0);
		mapSpecialTimes.SetAt( ( void *)atoi( "777777"), 0);
		mapSpecialTimes.SetAt( ( void *)atoi( "888888"), 0);
		// (2006/11/19 - Seung-Won, Bae) Support samsung code. 장마감 009991, 시간외마감 009999
		mapSpecialTimes.SetAt( ( void *)atoi( "009991"), 0);
		mapSpecialTimes.SetAt( ( void *)atoi( "009999"), 0);
		// (2006/12/29 - Seung-Won, Bae) Support koscom code. 장마감 310000
		mapSpecialTimes.SetAt( ( void *)atoi( "310000"), 0);
		// 시간외마감 추가 - ojtaso (20071130)
		mapSpecialTimes.SetAt( ( void *)atoi( "410000"), 0);
		// (2009/3/19 - Seung-Won, Bae) for After Closing.
		mapSpecialTimes.SetAt( ( void *)atoi( "710000"), 0);
		mapSpecialTimes.SetAt( ( void *)atoi( "810000"), 0);

		// (2013/03/14 - 이상신) Support IBK code. 장마감 8888, 장종료 9999
		mapSpecialTimes.SetAt( ( void *)atoi( "8888"), 0);
		mapSpecialTimes.SetAt( ( void *)atoi( "9999"), 0);
	}

	void *pTemp = NULL;
	return mapSpecialTimes.Lookup( ( void *)p_nTime, pTemp);
}
