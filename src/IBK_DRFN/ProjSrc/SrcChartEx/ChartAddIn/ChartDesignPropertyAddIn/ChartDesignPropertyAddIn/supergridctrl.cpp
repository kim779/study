...
/*
	...
	4 dec. 1998.
				...
				- Combobox support in CItemInfo				
				  see the ShowList in PacketSuperGrid.h/cpp, PreTranslateMessage functions
				...
				- Added some examples on how to print preview selected item(or at least to get some information from of the grid)
				  the sample is in the CPacketSuperGrid.h/cpp which is derived from CSuperGridCtrl it shows you
   ...
   22.june 1999 - Added GetCount()
				...
				- added GetTreeItem(.....) 
				- GetSelectedItem(.....)
				- Howto disable Drag/drop in CPacketSuperGrid
				- level 4 compiled
	...
	9. juli 99  - CRAIG SCHMIDT "the beachboy":)
				  ...
				  see the file CPacketSuperGrid.cpp for usage of the InsertRootItem.
	...
	14. juli 99	 - Fixed a memory leak in CPacketSuperGrid::HowToInsertItemsAfterTheGridHasBeenInitialized....
		...
		case VK_DELETE: 
			{
				...
				//부모 일때는 지우지 않는다.
				if(item.CompareNoCase("Packet") == 0 || item.CompareNoCase( _MTEXT( C6_USER_DEFINE)) == 0 || item.CompareNoCase("Item") == 0 || item.CompareNoCase( _MTEXT( C6_INPUT_ITEM)) == 0)
					return;
				if(nItem!=-1)
				{
...
BOOL CSuperGridCtrl::HitTestOnSign(CPoint point, LVHITTESTINFO& ht)
{
	...
	if(ht.iItem!=-1)
	{
		...
		BOOL bHit = FALSE;
//12.12
//		if(GetExtendedStyle()) //체크 박스 없는 리스트 때문에..// & LVS_EX_CHECKBOXES)
//		if(GetExtendedStyle() & LVS_EX_CHECKBOXES)
//		{
...
void CSuperGridCtrl::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(strItem != _MTEXT( C6_INPUT_ITEM)) {
		LVHITTESTINFO ht;
...
void CSuperGridCtrl::CopyChildren(CTreeItem* pDest, CTreeItem* pSrc)
{
	if (ItemHasChildren(pSrc))
	{
		...
		while (pos != NULL)
		{
			...
			InsertItemToListCtrl(pDest, pNewItem, lp);
...
{
	//false일때는 리스트에 AddTail, true일때는 중간에 Insert
	if(m_bDragandDrop == false)
		m_bDragandDrop = true;
	...
	CTreeItem* pNewParent = InsertItem(pTarget, lp, bUpdate); // pTarget;
	InsertItemToListCtrl(pTarget, pNewParent, lp, bUpdate);
...
*/
								if(iSubItem == 1 || iSubItem == 2 || iSubItem == 3)
...
CEdit* CSuperGridCtrl::EditLabelEx(int nItem, int nCol)
{
	...
	//edit 할 필요가 없음. 부모의 경우..
	CString strItem = GetItemText(nItem, 0);
	if(strItem.CompareNoCase( _MTEXT( C6_USER_DEFINE)) == 0 || strItem.CompareNoCase("Packet") == 0 )
		return NULL;
	...
	if (plvItem->pszText != NULL)//valid text
	{
		if(plvItem->iItem != -1) //valid item
		{
			...
			if(pSelItem != NULL) {
				...
				//타입에서 설정한 길이를 길이 Item에 세팅.
				//자료일자일경우에만 행하는 행동.
				CItemInfo *lp = GetData(pSelItem);
				if(plvItem->iSubItem == 3 && plvItem->iItem == 1 && (lp->GetItemText()).Compare( _MTEXT( C0_DATE_TIME)) == 0) {
					plvItem->iSubItem = 2;
					CString strText = plvItem->pszText;
					CString strInText = "";
					strInText.Format("%d", strText.GetLength());
					plvItem->pszText = (LPSTR)(LPCSTR)strInText;
					OnUpdateListViewItem(pSelItem, plvItem);
				}
			}
		}
...
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertItem(CTreeItem *pParent, CItemInfo* lpInfo,  BOOL bUpdate)
{
	...
	if(m_bDragandDrop == true) //Drag Drop일때 이 루틴을 타야함. 
	{
		CTreeItem *pSelItem = GetTreeItem(0);
		UpdateData(pItem, lpInfo);
		SetIndent(pItem, GetIndent(pSelItem)+1);        //pParent

		SetParentItem(pItem, pSelItem);  //pParent;
		//add as the last child 

		CString ParentText = pParent->m_lpNodeInfo->GetItemText(); 
		POSITION pos = pSelItem->m_listChild.GetHeadPosition();
		while (pos)
		{
			POSITION currentpos = pos;
			CTreeItem *pChild = (CTreeItem*)pSelItem->m_listChild.GetNext(pos);
			CString ChildText = pChild->m_lpNodeInfo->GetItemText();
			if(ParentText.Compare(ChildText) == 0)
				pSelItem->m_listChild.InsertBefore(currentpos, pItem);  //pParent			
		}
		m_bDragandDrop = false;
	}
	else
	{
		UpdateData(pItem, lpInfo);
		SetIndent(pItem, GetIndent(pParent)+1);        //pParent

		SetParentItem(pItem, pParent);  //pParent;
		//add as the last child 

		pParent->m_listChild.AddTail(pItem);  //pParent
	}
	return pItem;
}	
...
void CSuperGridCtrl::InsertRootItemToListCtrl(CTreeItem *pItem)
{
...
void CSuperGridCtrl::InsertItemToListCtrl(CTreeItem *pParent, CTreeItem *pItem, CItemInfo* lpInfo,  BOOL bUpdate)
{
	if(!bUpdate)
		 Hide(pParent, TRUE);	
	else
...
void CSuperGridCtrl::InsertOneItemToListCtrl(CTreeItem *pParent, CTreeItem *pItem, CItemInfo *lpInfo, BOOL bUpdate)
{
	if(!bUpdate)
		 Hide(pParent, TRUE);	
	else
