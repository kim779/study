...
public:
    CColourPopup();
    CColourPopup( HWND p_hOcxWnd, CPoint p, COLORREF crColour, CWnd* pParentWnd,
                 LPCTSTR szDefaultText = NULL, LPCTSTR szCustomText = NULL);
    void Initialise();

	CColorTableMgr m_colorTableMgr;
...
protected:
    ...
    CString GetColourName(int nIndex);          // { return m_crColours[nIndex].szName; }
...
protected:
//    static 
//	ColourTableEntry m_crColours[256];
    int            m_nNumColours;
...
protected:
	...
	afx_msg void OnActivateApp(BOOL bActive, DWORD/*HTASK*/ hTask);
	//}}AFX_MSG
    DECLARE_MESSAGE_MAP()
