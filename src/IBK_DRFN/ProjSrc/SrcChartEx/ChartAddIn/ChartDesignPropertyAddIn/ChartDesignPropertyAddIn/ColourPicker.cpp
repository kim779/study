...
/*
//AFXAPI
void  DDX_ColourPicker(CDataExchange *pDX, int nIDC, COLORREF& crColour)
{
...
BOOL CColourPicker::OnClicked()
{
    ...
    CRect rect;
    GetWindowRect(rect);
    new CColourPopup( m_hOcxWnd, CPoint(rect.left, rect.bottom),    // Point to display popup
...
//            pDC->SetTextColor((m_crColourText == CLR_DEFAULT)? 0 : m_crColourText);
			pDC->SetTextColor(0xFFFFFF - m_crColourBk);
            pDC->DrawText(m_strText, rect, DT_CENTER|DT_SINGLELINE|DT_VCENTER);
        }
    }
...
void CColourPicker::SetWindowSize()
{
    ...
    //MoveWindow(rect.left, rect.top, nWidth, rect.Height(),TRUE);//ArrowSize.cy+2*MarginSize.cy, TRUE);
	
	MoveWindow(rect.left, rect.top, rect.Width(), rect.Height(),TRUE);//ArrowSize.cy+2*MarginSize.cy, TRUE);
...
void CColourPicker::InitColorPicker(COLORREF color)
{
	SetSelectionMode(CP_MODE_BK);
	SetDefaultText(CString(""));
	SetCustomText(CString("ETC..."));
	SetColour(color);	
}