...
CColourPopup::CColourPopup( HWND p_hOcxWnd, CPoint p, COLORREF crColour, CWnd* pParentWnd,
...
{
	return m_colorTableMgr.GetColor(nIndex);
}

CString CColourPopup::GetColourName(int nIndex)          // { return m_crColours[nIndex].szName; }
{
	return m_colorTableMgr.GetColorText(nIndex);
}
...
void CColourPopup::Initialise()
{
    ...
    for (int i = 0; i < m_nNumColours; i++)
    {
		...
		COLORREF color = m_colorTableMgr.GetColor(i);
		pLogPalette->palPalEntry[i].peRed   = GetRValue(color);
        pLogPalette->palPalEntry[i].peGreen = GetGValue(color);
        pLogPalette->palPalEntry[i].peBlue  = GetBValue(color);
...
void CColourPopup::CreateToolTips()
{
    ...
    for (int i = 0; i < m_nNumColours; i++)
    {
		...
		LPCTSTR szColorName = (LPCTSTR) GetColourName(i);
        m_ToolTip.AddTool(this, szColorName, rect, 1);
    }
}
...
void CColourPopup::OnActivateApp(BOOL bActive, DWORD/*HTASK*/ hTask)
{
