...
//	POINT ptTemp = point; 
	...
	while(pos)
	{
		...
		{
			CRgn rgn;
			rgn.CreateRectRgn((int)rctGraphRegion.left, (int)rctGraphRegion.top, (int)rctGraphRegion.right, (int)rctGraphRegion.bottom);
...
void CChartNewsAddInImp::OnDrawAfterGraph(HDC p_hDC)
{
	...
	// IPacket *pIPacketForLow = m_pIPacketManager->GetPacket( "저가");
 	IPacket *pIPacketForLow = m_pIPacketManager->GetPacket( _MTEXT( C0_LOW ));
...
int CChartNewsAddInImp::GetValueFromIndex(int ix, int iy, long &lx, double &dy)
{
	...
	IPacket *pIPacket = m_pIPacketManager->GetPacket( _MTEXT( C0_DATE_TIME ));
	...
	pIPacket = m_pIPacketManager->GetPacket( _MTEXT( C0_CLOSE ));
	if( !pIPacket) return FALSE;
