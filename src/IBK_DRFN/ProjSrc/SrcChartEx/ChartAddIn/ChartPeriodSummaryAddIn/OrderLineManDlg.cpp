...
BOOL COrderLineManDlg::OnInitDialog() 
{
	...
	strTemp.LoadString(IDS_ISSUE_CODE);
	m_ctrlOLList.InsertColumn(1, strTemp);
	// m_ctrlOLList.InsertColumn(2, "종목명");
	strTemp.LoadString(IDS_ISSUE_NAME);
	m_ctrlOLList.InsertColumn(2, strTemp);
	// m_ctrlOLList.InsertColumn(3, "기준봉");
	strTemp.LoadString(IDS_BASIC_BONG);
	m_ctrlOLList.InsertColumn(3, strTemp);
	// m_ctrlOLList.InsertColumn(4, "개수");
	strTemp.LoadString(IDS_NUMBER);
	m_ctrlOLList.InsertColumn(4, strTemp);
...
//	CChartOrderLineAddInImp::m_slSavedOrderLines.Serialize(ar);
	// 리스트에 삽입
	POSITION pos = CChartOrderLineAddInImp::m_slSavedOrderLines.GetHeadPosition();
	while (pos)
	{
		int iStart = 0;
		CString s;
		strLineInfo = CChartOrderLineAddInImp::m_slSavedOrderLines.GetNext(pos);
		strLineInfo += ",";
...
void COrderLineManDlg::OnBtnDel() 
{
	...
	while(pos)
	{
		...
		POSITION pos2 = CChartOrderLineAddInImp::m_slSavedOrderLines.GetTailPosition();
		while(pos2)
		{
			POSITION posForDel = pos2;
			CString strOrderLine = CChartOrderLineAddInImp::m_slSavedOrderLines.GetPrev(pos2);
			if(strOrderLine.Find(strCodeAndDateType) >= 0)
			{
				CChartOrderLineAddInImp::m_slSavedOrderLines.RemoveAt(posForDel);
...
void COrderLineManDlg::RemoveLIneFromObList(CString strSerial)
{
	...
	POSITION psnAddIn = CChartOrderLineAddInImp::m_alOrderLineAddInManager.GetHeadPosition();
	while( psnAddIn)
	{
		CChartOrderLineAddInImp *pAddIn = CChartOrderLineAddInImp::m_alOrderLineAddInManager.GetNext( psnAddIn);
		if( !pAddIn) return;
		COLList *pLineList = pAddIn->GetLineList();						// 주문선 리스트
		POSITION pos = pLineList->GetHeadPosition();
		while (pos)
		{
			POSITION pos2 = pos;	// 삭제용
			CLineObj* pLine = pLineList->GetNext(pos);
			if(pLine->m_strSerial.Find(strSerial) >= 0)
			{
				delete pLine;
				pLine = NULL;
				pLineList->RemoveAt(pos2);
				break;
			}
		}

		pAddIn->InvalidateControl();
	}
}
