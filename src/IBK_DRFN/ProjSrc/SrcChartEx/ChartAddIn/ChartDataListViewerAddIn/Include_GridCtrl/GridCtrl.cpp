...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_bEditSelected		  = TRUE;		// 에디트 활성화시 전체선택된 상태로 시작하기.
...
//메세지와 함께 부가 정보를 보낼 때 사용
LRESULT CGridCtrl::SendEndEditDataToParent(int nRow, int nCol, CString strData) const
{
    if (!IsWindow(m_hWnd))
        return 0;

    GV_DISPINFO Info;
	Info.item.strText	= strData;
    Info.item.row		= nRow;
    Info.item.col		= nCol;
    Info.hdr.hwndFrom	= m_hWnd;
    Info.hdr.idFrom		= GetDlgCtrlID();
    Info.hdr.code		= GVN_ENDLABELEDIT;

    CWnd *pOwner = GetOwner();
    if (pOwner && IsWindow(pOwner->m_hWnd))
        return pOwner->SendMessage(WM_NOTIFY, Info.hdr.idFrom, (LPARAM)&Info);
    else
        return 0;
}
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    else if (nChar == VK_F2 || nChar == VK_RETURN)
    {
...
void CGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
    ...
    if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0)
    {
        ...
        CGridCellBase* pCell = GetCell(nRow, nCol);
        if (pCell)
            pCell->Edit(nRow, nCol, rect, point, IDC_INPLACE_CONTROL, nChar, GetEditSelected());
    }
}
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	...
	//20081017 이문수 <<
    {
        ...
        if (ValidateEdit(nRow, nCol, str) && 
            SendEndEditDataToParent(nRow, nCol, str) >= 0)
        {
            SetModified(TRUE, nRow, nCol);
            RedrawCell(nRow, nCol);
        }
