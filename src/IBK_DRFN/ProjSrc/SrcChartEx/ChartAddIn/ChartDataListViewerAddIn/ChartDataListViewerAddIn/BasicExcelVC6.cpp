...
long BasicExcelCell::GetDate()
{
	...
	{
		serialDate = val;
		lDate = SerialDateToDMY(serialDate);		
	}
	return lDate;
}

long BasicExcelCell::GetTime()
{
	double val;
	long lTime = 0;
	...
	if(Get(val))
	{
		lTime = SerialDateToDMY(val);		
	}
	return lTime;
}

long BasicExcelCell::SerialDateToDMY(int nSerialDate)
{
    ...
    if (nSerialDate == 60)
    {
        ...
        lDate = nYear*10000 + nMonth*100 + nDay;
		return lDate;
    }
...
long BasicExcelCell::SerialTimeToHMS(double nSerialTime)
{
	int nHour = 0,  nMin = 0,  nSec = 0;
	long lTime = 0;

	// 86400 -> 24 ½Ã
	nSec = nSerialTime * 86400;
	nHour = nSec/3600;
	nSec = nSec % 3600;
	nMin = nSec / 60;
	nSec = nSec % 60;
	
	lTime = nHour*10000 + nMin*100 + nSec;
	return lTime;
}
