...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (/* IsFocused() ||*/ IsDropHighlighted() )	// 2005/05/20 - SeoJeong - Block Focused cell color
	{
	...
	if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            {
				if( GetImage() == 0 || GetImage() == 1 )		// 가운데 정열 이미지
				{
					CPoint pt;
					pt.x = rect.left;
					pt.y = rect.top+3;
	...
	// JSJ_Modify_040601		COMMENT: 그리드내 텍스트를 최대한 안짤리게 하자 ---------->>
	if ( pGrid->GetPatternReportType() == WAVE_REPORTTYPE1)
	{
		if ( nRow == ROW_FIRST )			// 그리드 헤더인 경우
		{
			CRect rectTemp;
			
			// 1줄 타이틀 ----------------------------------->>
			if ( nCol == COL_WAV1_NUM || nCol == COL_WAV1_TIME || nCol == COL_WAV1_TYPE || nCol == COL_WAV1_NAME || nCol == COL_WAV1_CANDLECNT)
			{
				pGrid->GetCellRect(ROW_FIRST,	nCol, rect);
				pGrid->GetCellRect(ROW_FIRST+1,	nCol, rectTemp);
				rect.bottom = rectTemp.bottom;
			}
			// <<-------------------------------------------------
			else if ( nCol >=  COL_WAV1_EXPUPDOWN_PER && nCol <=  COL_WAV1_ACHIEVE_PER)
			{
				pGrid->GetCellRect(ROW_FIRST, COL_WAV1_EXPUPDOWN_PER, rect);
				pGrid->GetCellRect(ROW_FIRST, COL_WAV1_ACHIEVE_PER, rectTemp);
		...
		else if ( nRow == ROW_FIRST+1)			// 그리드 헤더인 경우
		{
			CRect rectTemp;
		...
		else
		{
			...
			UINT nFlag = pGrid->GetItemFormat(nRow, nCol);
			if (nFlag & DT_LEFT)
				rect.left += 3;
			else if (nFlag & DT_RIGHT)
				rect.right -= 3;
	...
	else if ( pGrid->GetPatternReportType() == CANDLE_REPORTTYPE1)
	{
		if ( nRow == ROW_FIRST )			// 그리드 헤더인 경우
		{
			CRect rectTemp;
			...
			// 1줄 타이틀 ----------------------------------->>
			if ( nCol == COL_CND1_NUM || nCol == COL_CND1_TIME || nCol == COL_CND1_TYPE || nCol == COL_CND1_NAME)
			{
				pGrid->GetCellRect(ROW_FIRST,	nCol, rect);
				pGrid->GetCellRect(ROW_FIRST+1,	nCol, rectTemp);
				rect.bottom = rectTemp.bottom;
			...
			// <<-------------------------------------------------		
			else if ( nCol >=  COL_CND1_DAY1 && nCol <=  COL_CND1_DAY5)
			{
				pGrid->GetCellRect(ROW_FIRST, COL_CND1_DAY1, rect);
				pGrid->GetCellRect(ROW_FIRST, COL_CND1_DAY5, rectTemp);
				rect.right = rectTemp.right;
			}
			else																// 그외 타이틀
			{
