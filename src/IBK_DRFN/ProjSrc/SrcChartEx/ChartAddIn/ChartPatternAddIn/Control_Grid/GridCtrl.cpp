...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	// JSJ_Add_N_Modify -------------------------------------------------------------------------------->
	m_bTracking				= FALSE;
	m_bIsDragNDrop			= FALSE;				// OnDrag에서 OnDrop 함수로 이벤트가 발생한 경우만 주문되게
	m_nPatternReportType	= WAVE_REPORTTYPE1;		// Pattern Report Type

	m_LeftClickDownCell = CCellID(1,1);			// 최초 그리드 포커스 문제로 이벤트없는 Header로 잡아준다
	// <-------------------------------------------------------------------------------------------------
}
...
//	return;
    ...
    else if (nChar == VK_DOWN)
    {
		...
		// 2005/05/20 - SeoJeong - Add for Mouse Click Event
		SendMessageToParent(next.row, next.col, NM_CLICK);
    }
    else if (nChar == VK_UP)
    {
        // don't let user go to a hidden row
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	// JSJ_Modify_040722 COMMENT: 셀에디팅이 끝난후 포커스만 없앰 -------------------------->
	//OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, CPoint( -1, -1), pgvItem->lParam);
	SetFocusCell(-1, -1);		
...
void CGridCtrl::SetStaticSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
									   BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
    ...
    if (bSelectCells)
    {
        ...
        // Unselect all previously selected cells
        for (pos = m_StaticCellMap.GetStartPosition(); pos != NULL; )
        {
            DWORD key;
            CCellID cell;
            m_StaticCellMap.GetNextAssoc(pos, key, (CCellID&)cell);

            // Reset the selection flag on the cell
            if (IsValid(cell))
            {
				// This will remove the cell from the m_StaticCellMap map
                SetStaticItemState(cell.row, cell.col, GetItemState(cell.row, cell.col) & ~GVIS_STATIC);
            ...
            {
                m_StaticCellMap.RemoveKey( key);  // if it's not valid, get rid of it!
            }
        }
    }
        ...
        nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++)
            {
                ...
                // Why is there no XOR operator in C??? We have bitwise XOR ^, but what about ^^??
                if ( (bSelectCells && (nState & GVIS_STATIC)) ||
                     (!bSelectCells && !(nState & GVIS_STATIC)) )
                    continue;    // Already selected or deselected - ignore

				// Set the selected state. This will add/remove the cell to m_StaticCellMap
                if (bSelectCells)
				{
                    SetStaticItemState(row, col, nState | GVIS_STATIC);
				}
                else
                    SetStaticItemState(row, col, GetItemState(row, col) & ~GVIS_STATIC);
                           ...
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	if (GetSingleRowSelection())
        SetSelectedRange(currentCell.row, GetFixedColumnCount(), currentCell.row, GetColumnCount()-1, 
                         bForceRedraw, bSelectCells);
    else
	...
	// 마우스 Move Message ------------------------------------------------>>
	SendMessageToParent(currentCell.row, currentCell.col, UDM_TRACEMOUSEMOVE);
	// <<--------------------------------------------------------------------
}
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
	...
	// JSJ_Modify_040603	COMMENT: 스탑로스에선 드래그앤드랍을 사용하지 않으므로 막음
	m_MouseMode = MOUSE_NOTHING;
//	// 수 정 자 : 김용식 (2002년 3월 19일 13시 53분)
//	// 수정사항 : 
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject)
{
	...
	// 2002.12.26 KYS EDIT {{
	// 정정 확인 후 셀에 표시
	if( m_Message == UDM_DROPORDERCHANGE )
...
void CGridCtrl::OnBeginDrag()
{
	...
	// 수정사항 : 특정 위치의 Drag & Drop을 막음, 데이터가 없어도 막음
	//TRACE("row = %d, col = %d, Data = %s\n", m_idCurrentCell.row, m_idCurrentCell.col, GetCell(m_idCurrentCell.row, m_idCurrentCell.col)->GetText());
	//if( m_idCurrentCell.row == ROW_FIRST )
	//	return;
...
//			if( nRet == 0 )
//			{
//				//TRACE("row = %d, col = %d, Data = %s\n", m_LeftClickDownCell.row, m_LeftClickDownCell.col, GetItemText(m_LeftClickDownCell.row, m_LeftClickDownCell.col));
//				SetItemText(m_LastDragOverCell.row, m_LastDragOverCell.col, " ");	
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
	...
	if (!IsCellEditable(cell) )//&& !(cell.row > ROW_FIRST && cell.col > MAEDO_COL && cell.col < MAESU_COL) )
	{
		return DROPEFFECT_NONE;
	}
...
void CGridCtrl::OnDragLeave()
{
	...
	// 수 정 자 : 김용식 (2002년2월7일   10시36분)
	// 수정사항 : Drag & Drop후 기존 셀의 포커스 및 셀렉트를 풀어준다.		
...
/*
	...
	if( m_LastDragOverCell.col == COL_FIRST )		// 좌측취소 스태틱	
	{
		if( InsideStaticLeft(m_LastDragOverCell.row, m_LastDragOverCell.col) )
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
			//::AfxMessageBox("좌측취소주문");
			//SendMessageToParent(m_LastDragOverCell.row, m_LastDragOverCell.col, UDM_DROPORDERCANCEL);
			m_Message =  UDM_DROPORDERCANCEL;
			OnDragLeave();
			TRACE("[CGridCtrl::OnDrop]\n");
	...
	else if( m_LastDragOverCell.col == COL_CENTER )
	{
		m_MouseMode = MOUSE_NOTHING;
	}
	else if( m_LastDragOverCell.col == COL_END )	// 우측취소 스태틱
	{
		if( InsideStaticRight(m_LastDragOverCell.row, m_LastDragOverCell.col) )
		{
			m_MouseMode = MOUSE_PREPARE_DRAG;
			//::AfxMessageBox("우측취소주문");
			//SendMessageToParent(m_LastDragOverCell.row, m_LastDragOverCell.col, UDM_DROPORDERCANCEL);
			m_Message =  UDM_DROPORDERCANCEL;
			OnDragLeave();
			TRACE("[CGridCtrl::OnDrop]\n");
	...
	{
		CString sTemp;
		//sTemp = GetItemText(m_idCurrentCell.row, m_idCurrentCell.col);
		sTemp = GetItemText(m_LastDragOverCell.row, COL_CENTER);
		if( sTemp == "" || sTemp == " " )
		{
			m_MouseMode = MOUSE_NOTHING;
		}
...
BOOL CGridCtrl::SetStaticItemState(int nRow, int nCol, UINT state)
{
	// Static cell Map	JSJ_Add_040804 ----------------------->
	BOOL bStatic;
    ...
    {   
        ...
        bStatic = m_StaticCellMap.Lookup(key, (CCellID&)cell);       
    }
    else
        bStatic =  ((GetItemState(nRow, nCol) & GVIS_STATIC) == GVIS_STATIC);


	if ( bStatic && !(state & GVIS_STATIC) )
	{
        CCellID cell;
        DWORD key = MAKELONG(nRow, nCol);

        if (m_StaticCellMap.Lookup(key, (CCellID&)cell))
            m_StaticCellMap.RemoveKey(key);
	}
	else if(!bStatic && (state & GVIS_STATIC) )
	{
        CCellID cell(nRow, nCol);
        m_StaticCellMap.SetAt(MAKELONG(nRow, nCol), cell);
	}
	// <-------------------------------------------------------
...
{
    m_bTracking = FALSE;
	//ResetSelectedRange();		// JSJ_Block_040722 COMMENT: 마우스가 그리드를 벗어났을 때에도 Selected 상태 유지
	return 0;
}
...
//			point.x = %d, point.y = %d\n", rect.top, rect.left, rect.bottom, rect.right, point.x, point.y);
	...
	*/
	...
	m_MouseMode = MOUSE_NOTHING;	// JSJ_Temp
/* // JSJ_Blick
	if( !(cell.col == MAEDO_COL || cell.col == MAESU_COL))
	{
//		TRACE("[OnMouseMove] cell.row = %d, cell.col = %d\n", cell.row, cell.col);
...
*/
	...
	*/
	...
	if( !m_bLMouseButtonDown )
	{
	/*	// JSJ_Block  마우스 움직일때는 열 색상 표시 안함
		CCellID FocusCell;
		FocusCell = GetCellFromPt(point);
		if( ROW_DATA_START <= FocusCell.row )
		{
			// 마우스 포커스가 있는열 색상표시 JSJ_Modify ---------------->>
			//SetFocusCell(-1,-1);
			OnFixedColumnClick(FocusCell);		
	...
	*/
	...
	if (m_MouseMode == MOUSE_NOTHING || m_MouseMode == MOUSE_PREPARE_DRAG)
	{
		...
		*/
		...
		// 그리드 더블클릭시 종목코드 공유  JSJ_Modify_040718	-------------------->>
/*
		if( cell.row >= ROW_DATA_START )				// 주문 더블 클릭
		{
			if (cell.col >= S_ACCT_COL && cell.col <= S_ERATE_COL)		
			{
				if( IsValid(m_LeftClickDownCell) )
				{				
					CString strData;
					...
					strData = GetItemText(cell.row, S_ITEMNO_COL);		// 종목코드
					strData.TrimLeft();	strData.TrimRight();
					if ( strData != "" )								// 종목코드가 있는 경우
						SendMessageToParent(cell.row, cell.col, NM_DBLCLK);
				}
			}
		}*/
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	*/
	...
	// JSJ_Modify COMMENT:셀 편집시 선택된 컬럼 유지 ------------------------------>>>
		...
		IsCellEditable(m_LeftClickDownCell))
	{
		// 셀 에디트 칼럼에서도 열 선택
		m_MouseMode = m_bListMode? MOUSE_SELECT_ROW : MOUSE_SELECT_CELLS;
		if( !IsCellSelected(m_LeftClickDownCell.row, m_LeftClickDownCell.col) )
    ...
    // (If the user moves the mouse, then dragging occurs)
	// JSJ_Modify_040607	COMMENT: 열 드래그앤 드랍은 없으므로
   /*
	...
	*/
	// 수정완료 :(2002년2월6일 15시56분)    
    {
		...
		*/
		...
		// 이미 선택된 셀이 아닌경우 셀선택 표시		JSJ_Modify_040602		
		if( !IsCellSelected(m_LeftClickDownCell.row, m_LeftClickDownCell.col) )
			OnSelecting(m_LeftClickDownCell);
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	*/
	...
	m_MouseMode = MOUSE_NOTHING;		// JSJ_Modify_040608	COMMENT: 마우스 버튼업시 드래그모양 포인트 없음
...
//	else 
//	{
//		//CGridCell *pCell = (CGridCell *)GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
//		//CString sTemp;
//		//sTemp = pCell->GetText();
//		//if( sTemp == "" )
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	// JSJ_Modify_040616 ----------------------------------------->
	// COMMENT: ROW_END 열을 클릭했을경우 -> RButton Down 명령  
	SetFocus();
...
{
	CString strCurrentText = GetItemText(nRow, nCol);
	SetEditingData( strCurrentText);		// Old 데이터
...
// JSJ_ADD ***********************************************************************************
// JSJ_Add_040716	COMMENT: 열선택 해제
void CGridCtrl::ReleaseAllSelectedCell()
{
	ResetSelectedRange();
}

void CGridCtrl::ReleaseAllStaticSelectedCell()
{
    SetStaticSelectedRange(-1,-1,-1,-1);
	//m_StaticCellMap.RemoveAll();
}

void CGridCtrl::SelectOneRows(CCellID currentCell)
{
    if (!IsValid(currentCell))
        return;

	SetStaticSelectedRange(	currentCell.row, GetFixedColumnCount(),
							currentCell.row, GetColumnCount()-1 );
}

// 편집중인 데이터 Set
void CGridCtrl::SetEditingData(CString strEditingData)
{
	m_strEditingData = strEditingData;
}

// 편집중인 데이터 Get
CString	CGridCtrl::GetEditingData()
{
	return m_strEditingData;
}


void CGridCtrl::SetPatternReportType(int nPatternReportType)
{
	m_nPatternReportType = nPatternReportType;
}

BOOL CGridCtrl::GetPatternReportType()
{
	return m_nPatternReportType;
}
