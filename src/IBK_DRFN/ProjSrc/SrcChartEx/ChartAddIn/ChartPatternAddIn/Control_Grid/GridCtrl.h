...
#define	UDM_DROPORDERCANCEL		WM_USER+1300
#define	UDM_TITLECANCELSHOW		WM_USER+1310
#define	UDM_TITLECANCELHIDE		WM_USER+1320
#define	UDM_DROPORDERCHANGE		WM_USER+1400
#define	UDM_TRACEMOUSEMOVE		WM_USER+1600
...
public:
	...
	BOOL   SetStaticItemState(int nRow, int nCol, UINT state);
...
///////////////////////////////////////////////////////////////////////////////////
 public:
    ...
    void SetStaticSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
								BOOL bForceRepaint = FALSE, BOOL bSelectCells = TRUE);
...
public:
	...
	void		ReleaseAllStaticSelectedCell();				// 모든 스태틱셀 해제
	void		SelectOneRows(CCellID currentCell);			// 열 선택

	void		SetEditingData(CString strEditingData);		// 편집중인 데이터 Set
	CString		GetEditingData();							// 편집중인 데이터 Get

protected:
	BOOL		m_bTracking;			// 마우스이벤트를 Tracking 할지를 결정하는 플래그
	BOOL		m_bIsDragNDrop;			// OnDrag에서 OnDrop 함수로 이벤트가 발생한 경우만 주문되게

	CString		m_strEditingData;		// Edit 하고 있는 데이터	JSJ_Add_040607
...
protected:
	int			m_nPatternReportType;							// 패턴 보고서 타입

public:
	void		SetPatternReportType(int nPtnAnalysisType);		
	BOOL		GetPatternReportType();	

	void		Call_ResetScrollBars();							// Call ResetScrollBars Func.
...
protected:
    ...
    CMap<DWORD,DWORD, CCellID, CCellID&> m_SelectedCellMap, m_PrevSelectedCellMap, m_StaticCellMap;
