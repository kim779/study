...
BOOL CIWAddInToolMgr::OnPreRealData( const char *p_szPacketName, double &p_dData, const char *p_szRQ)
{
	if( m_pAddInManager) return m_pAddInManager->OnPreRealData( p_szPacketName, p_dData, p_szRQ);
	return TRUE;
}
BOOL CIWAddInToolMgr::OnPreIndexedRealData( const char *p_szPacketName, const int p_nIndex, double &p_dData)
{
	if( m_pAddInManager) return m_pAddInManager->OnPreIndexedRealData( p_szPacketName, p_nIndex, p_dData);
	return TRUE;
}
BOOL CIWAddInToolMgr::OnPreRealPacketEnd( const char *p_szRQ)
{
	if( m_pAddInManager) return m_pAddInManager->OnPreRealPacketEnd( p_szRQ);
	return TRUE;
}
