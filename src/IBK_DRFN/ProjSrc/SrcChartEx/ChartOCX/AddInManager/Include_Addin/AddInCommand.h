...
{
	EAI_ALL_RESET_CURRENT_TOOL_NOT_USED,
	...
	EAI_ALL_ENVIRONMENT_CHANGED_NOT_USED,
	...
	EAI_ALL_BLOCK_BEHAVIOR_NOT_USED,
	...
	EAI_OOA_ON_TOOL_FUNCTION_NOT_USED,
	...
	EAI_ALL_ON_DELETE_INDICATOR_NOT_USED,
	...
	EAI_ST_REMOVED_DATA_COUNT_NOT_USED,
	...
	EAI_GLANCEBALANCE_CURSOR_SET_STATE,		// 일목균형표 설정 커서 적용 - ojtaso (20071109) 
	// (2006/10/17 - Seung-Won, Bae) GlanceBalance Cursor AddIn Command to get the state of cursor.
	//		OnAddInToolCommand(	const EAICommandType p_eCommandID)

	EAI_USER_CHANGE_TIMEDIFF,
	// (2008/10/07 - JS.Kim)
	//		OnAddInToolCommand(	long &p_lResult, const EAICommandType p_eCommandID, const long p_lParam1, const long p_lParam2)
	//			p_lResult	: Out, Result, bool
	//			p_lParam1	: Block Index
	//			p_lParam2	: Selected Time Difference Value
	
	EAI_DATA_SIMULATOR,
	//20081011 이문수 데이터시뮬레이션 >>
	//	BOOL OnAddInToolCommand( const EAICommandType p_eCommandID)
	
	EAI_ENVSETUP_LHLCHC_CHANGED,
	// (2008/10/26 - Seung-Won, Bae) for O/H/L/C (not LH/LC/HC)
	//	BOOL OnAddInToolCommand( const EAICommandType p_eCommandID)

	//>> 갭보정 설정변경 - vntsorl(20090217)
	EAI_ENVSETUP_GAPREVISION_CHANGED,
	//	BOOL OnAddInToolCommand( const EAICommandType p_eCommandID, const long p_lParam1, const long p_lParam2)
	//			p_lParam1	: bUseGapRevision
	//			p_lParam2	: bWithNewData

	EAI_CUSTOM_GET_GAPREVISION,
	//		OnAddInToolCommand(	long &p_lResult, const EAICommandType p_eCommandID)
	//			p_lResult	: bUseGapRevision
	//<< 갭보정 설정변경 - vntsorl(20090217)

	//<< 20130530 이상신 : 월물간 갭보정 추가(IBK) (Issue No : 0006572)
	EAI_ENVSETUP_MONTHGAPREVISION_CHANGED,
	//	BOOL OnAddInToolCommand( const EAICommandType p_eCommandID, const long p_lParam1)
	//			p_lParam1	: bUseMonthGapRevision

	EAI_ENVSETUP_SHOWMONTHGAPREVISION_CHANGED,
	//	BOOL OnAddInToolCommand( const EAICommandType p_eCommandID, const long p_lParam1)
	//			p_lParam1	: bShowMonthGapRevision

	EAI_CUSTOM_GET_MONTHGAPREVISION
	//		OnAddInToolCommand(	long &p_lResult, const EAICommandType p_eCommandID)
	//			p_lResult	: bUseMonthGapRevision
	//>> (Issue No : 0006572)
};
