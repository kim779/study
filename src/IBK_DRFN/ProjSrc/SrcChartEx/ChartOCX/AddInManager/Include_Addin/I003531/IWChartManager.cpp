...
BOOL CIWChartManager::IsEmptyData( void)
{
	if( !m_pXScaleManager) return TRUE;
	return m_pXScaleManager->IsEmptyData();
}

// (2009/6/5 - Seung-Won, Bae) Reset partition info.
void CIWChartManager::SetBlockCount( int p_nRowCount, int p_nColumnCount)
{
	if( m_pMainBlock) m_pMainBlock->SetBlockCount( CBlockIndex( p_nRowCount, p_nColumnCount));
}
...
CScaleBaseData::HORZSCALEDRAWERTYPE	CIWChartManager::GetMasterValueType( const int nRow, const int nCol)
{
	if( !m_pMainBlock) return CScaleBaseData::HORZ_NOTYPE;
	return m_pMainBlock->GetMasterValueType( nRow, nCol);
}

void CIWChartManager::GetHorzScaleCompart( const int p_nR, const int p_nC, ILPCSTR &p_szDateCompart, ILPCSTR &p_szTimeCompart)
{
	if( !m_pMainBlock) return;
	CString strDateCompart, strTimeCompart;
	m_pMainBlock->GetHorzScaleCompart( CBlockIndex( p_nR, p_nC), strDateCompart, strTimeCompart);
	p_szDateCompart = CIWString::CreateObject( strDateCompart);
	p_szTimeCompart = CIWString::CreateObject( strTimeCompart);
}
...
IString *CIWChartManager::GetSubGraphPacketNameString( const char *p_szGraphName, const char *p_szSubGraphName, const int p_nPacketIndex)
{
	if( !m_pMainBlock) return NULL;
...
IString *CIWChartManager::GetRQofGraph( const char *p_szGraphName)
{
	if( !m_pMainBlock) return NULL;
	return CIWString::CreateObject( m_pMainBlock->GetRQofGraph( p_szGraphName));
}
...
IString *CIWChartManager::GetSubGraphViewData_PacketNameString( POSITION p_psnViewData)
{
	if( !p_psnViewData) return NULL;
	CSubGraphViewDataIterator *pSGVDIterator = m_lSubGraphViewData.GetAt( p_psnViewData);
	if( !pSGVDIterator)								return NULL;
	...
	return CIWString::CreateObject( pSGVDIterator->m_pSubGraphViewData->GetPacketNames());
}
IString *CIWChartManager::GetSubGraphViewData_RQString( POSITION p_psnViewData)
{
	if( !p_psnViewData) return NULL;
	CSubGraphViewDataIterator *pSGVDIterator = m_lSubGraphViewData.GetAt( p_psnViewData);
	if( !pSGVDIterator)								return NULL;
	...
	return CIWString::CreateObject( pSGVDIterator->m_pSubGraphViewData->GetRQ());
}
void CIWChartManager::GetSubGraphViewData_TitleAndPacketNameAndRQString( POSITION p_psnViewData, ILPCSTR &p_pszTitle, ILPCSTR &p_pszPacketNames, ILPCSTR &p_pszRQ)
{
	p_pszTitle			= NULL;
	...
	p_pszRQ				= CIWString::CreateObject( pSGVDIterator->m_pSubGraphViewData->GetRQ());
}
...
void CIWChartManager::RealDataReceived( const char *p_szRQ, BOOL p_bIndexedReal)
{
	if( m_pXScaleManager) m_pXScaleManager->RealDataReceived( p_szRQ, p_bIndexedReal);
}
...
double CIWChartManager::GetDoubleFromX(const int nRow, const int nCol, int nCursor_x, int* pnBlockColumn, int& nCenter_x, const char *p_szRQ)
{
	if( !m_pXScaleManager) return 0L;
	CXScaleManager::PAREA pparea = NULL;
	double ret = m_pXScaleManager->GetDoubleFromX(nRow, nCol, nCursor_x, pnBlockColumn, &pparea, p_szRQ);
	if( !pparea) return 0L;
	nCenter_x = pparea->center;
	return ret;
}
...
void CIWChartManager::SetCandleTypeOnResize(int nType)					// 우측 여백 동작 방식 설정
{
	CXScaleManager::RSA_TYPE rsaType = (CXScaleManager::RSA_TYPE)nType;
	m_pXScaleManager->SetCandleTypeOnResize( rsaType);
}

int CIWChartManager::GetCandleTypeOnResize()
{
	int nType = (int)m_pXScaleManager->GetCandleTypeOnResize();
	return nType;
}

// 봉간격 일정 : xScaleManager - ojtaso (20070528)
void CIWChartManager::SetFixedInterval(BOOL bFixedInterval, BOOL bCalc)
{
	m_pXScaleManager->SetFixedInterval(bFixedInterval, bCalc);
}

BOOL CIWChartManager::IsFixedInterval()
{
	return m_pXScaleManager->IsFixedInterval();
}

// (2009/5/10 - Seung-Won, Bae) Scroll with added and trucated by Real
void CIWChartManager::SetFlagForCalcDRIandScrollByReal( BOOL p_bWithTrucated)
{
	if( m_pXScaleManager) m_pXScaleManager->SetFlagForCalcDRIandScrollByReal( p_bWithTrucated);
}

// (2008/10/20 - Seung-Won, Bae) Retreive RQ
IString *CIWChartManager::GetMainRQofBlock( int p_nRow, int p_nCol)
{
	if( !m_pMainBlock) return NULL;

	IString *pIString = CIWString::CreateObject( m_pMainBlock->GetMasterValueRQ( p_nRow, p_nCol));
	return pIString;
}
IString *CIWChartManager::GetCurrentRQ( void)
{
	if( !m_pIChartOCX) return NULL;
	return m_pIChartOCX->GetCurrentRQ();
}
