...
public:
	...
	virtual int			GetDateUnitType( void);
...
public:
	...
	// 분차트 근접시간 적용 - ojtaso (20080811)
	virtual int		GetIndexFromData( double p_dData, const char *p_szType);
	...
	virtual BOOL	IsUseStandardValue(	const int p_nType = 0);		// 0:Base,	1:Upper Base,	2:Lower Base
	virtual double	GetStandardValue(	const int p_nType = 0);		// 0:Base,	1:Upper Base,	2:Lower Base
...
public:
	...
	virtual void OffsetPacketData(double dData);

// 20081007 JS.Kim	사용자시간대 선택
public:
	virtual int		GetBaseTimeDifference( void);
	virtual int		GetOrgBaseTimeDifference( void);
	virtual double	CalcTimeDiff(double dData, int nTimeDiff);
};
