...
// (2009/10/23 - Seung-Won, Bae) for Base Price
// 0:Base,	1:Upper Base,	2:Lower Base
BOOL CIWPacket::IsUseStandardValue(	const int p_nType)
{
	if( !m_pPacket) return FALSE;
	return m_pPacket->IsUseStandardValue( p_nType);
}
double CIWPacket::GetStandardValue(	const int p_nType)
{
	if( !m_pPacket) return -1;
	return m_pPacket->GetStandardValue( p_nType);
}
...
int CIWPacket::GetBaseTimeDifference( void)
{
	if( !m_pPacket) return 0;
	CPacketList *pParentList = m_pPacket->GetPacketList();
	if( !pParentList) return 0;
	return pParentList->GetBaseTimeDifference();
}
int	CIWPacket::GetOrgBaseTimeDifference()
{
	if( !m_pPacket) return 0;
	CPacketList *pParentList = m_pPacket->GetPacketList();
	if( !pParentList) return 0;
	return pParentList->GetOrgBaseTimeDifference();
}
double CIWPacket::CalcTimeDiff(double dData, int nTimeDiff)
{
	if( !m_pPacket) return dData;
	return m_pPacket->CalcTimeDiff( dData, nTimeDiff);
}
