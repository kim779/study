...
protected:
	virtual IChartCtrl *				GetIChartCtrl( void);
...
protected:
	...
	virtual IPacketManager *		GetIDefaultPacketManager( void);
...
protected:
	...
	virtual void		OnCmdMsg( HMENU p_hMenu, UINT p_nItemID, int p_nItemPos, int p_nItemCount, int p_nCmdCode);
...
protected:
	...
	virtual IString *	GetProgID( void);
	...
	virtual IString *	GetTestEnvValue( const char *p_szEnvKey);
	// (2009/9/2 - Seung-Won, Bae) for Image Path
	virtual IString *	GetImagePathString( void);
...
protected:
	...
	virtual BOOL		IsOnNoZoomChart( void);
	...
	virtual BOOL		IsMultipleSameNameIndicator( void);
	virtual void		SetMultipleSameNameIndicator( BOOL p_bEnable);
	...
	virtual BOOL		IsTRIndicator( const char *p_szGroupName, const char *p_szIndicatorName);
	// (2009/6/5 - Seung-Won, Bae) Check mode for moving and deleting the graph.
	virtual BOOL		CanGraphMoveAndDelete( void);
...
protected:
	...
	virtual int		GetBongCountMargin( void);
	virtual BOOL	SetBongCountMargin( const int p_nCount);
...
protected:
	virtual	IString	*	AddChartBlock( const char *p_szIndicatorName);
	...
	virtual IString *	GetChartList( void);
...
protected:
	virtual void	SetOnRealProcessing( BOOL p_bOn);

// (2009/5/13 - Seung-Won, Bae) for Right Margin
protected:
	virtual void	SetRightMarginWidth( int nRightMarginWidth);
	virtual int		GetRightMarginWidth( void);

// (2009/5/17 - Seung-Won, Bae) for Future Time
protected:
	virtual void	SetFutureTimeCount( int nFutureTimeCount);
	virtual int		GetFutureTimeCount( void);
...
protected:
	virtual CChartInfo::CHART_MODE		GetChartMode( void);
...
public:
	//	시간구분선
	virtual	int			GetFirstBlockHScaleGridType_TimeGubun(int );
	virtual BOOL        GetFirstBlockHScaleType_TimeGubun(int );
	virtual	COLORREF	GetStyleValueHScaleGridColor_TimeGubun(	int );
	virtual	int			GetStyleValueScaleLineThickness_TimeGubun(int	);

	//KHD : 시간구분
	virtual	BOOL		SetAllHScaleGridType_TimeGubun(int p_nHScaleGridType, BOOL p_bCheckTime = FALSE, int p_nHScaleTimeType = 0);
	virtual	BOOL		SetStyleValueHScaleGridColor_TimeGubun(COLORREF p_clrHScaleGridColor, COLORREF p_clrHScaleGrid_WeekColor, COLORREF p_clrHScaleGrid_MonColor, COLORREF p_clrHScaleGrid_YearColor, BOOL p_bWithUpdate = FALSE);
