...
BOOL CIWPacketListManager::SetRealData( LPCTSTR lpszRQ, const char *p_szPacketName, double p_dData, int &p_nRemovePacketDataCount)
{
	if( !m_pPacketListManager) return FALSE;
	return m_pPacketListManager->SetRealData( lpszRQ, p_szPacketName, p_dData, p_nRemovePacketDataCount);
}
...
void CIWPacketListManager::OnNMTRClock( const __int64 &p_nClock, IRealReceiver *p_pIRealReceiver)
{
	if( !m_pPacketListManager) return;
	m_pPacketListManager->OnNMTRClock( p_nClock, p_pIRealReceiver);
}


// (2009/11/3 - Seung-Won, Bae) Make Market Date to Real Date of WorldOn Real Data.
void CIWPacketListManager::GetRealData_RealDateTime( LPCTSTR lpszRQ, double &p_dData)
{
	CPacketList* pPacketList = m_pPacketListManager->GetPacketList( lpszRQ);
	if( !pPacketList) return;
	pPacketList->GetRealData_RealDateTime( p_dData);
}
