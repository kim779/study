...
BOOL CIWBlock::GetIndexOfGraphWithIndicatorName( const char *p_szIndicatorName, int &p_nV, int &p_nG)
{
	p_nV = p_nG = -1;
	if( !m_pBlock) return FALSE;
	CString strGraphName;
	return m_pBlock->FindFirstIndicator( p_szIndicatorName, p_nV, p_nG, strGraphName, NULL);
}
//<< 호가차트시 분석도구 종가따라가기 버그 수정 - vntsorl(20090211)
