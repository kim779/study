...
BOOL CIWChartManager::FindFirstIndicator( const char *p_szIndicatorName, int &p_nR, int &p_nC, int &p_nV, int &p_nG)
{
	if( !m_pMainBlock) return FALSE;
	CString strGraphName;
	return m_pMainBlock->FindFirstIndicator( p_szIndicatorName, p_nR, p_nC, p_nV, p_nG, strGraphName, "DEFAULT");
}
...
// XScale : 관련 Interface로 사용 - vntsorl(20070412)
// 자료 인덱스로 x위치를 얻어온다.
//int CIWChartManager::GetXPosition( int nIdx, int nCol)
//{
//	if( !m_pXScaleManager) return FALSE;
//	CXScaleManager::AREA area;
//	m_pXScaleManager->GetAREAfRQnDTI("DEFAULT", nIdx, nCol, &area);
//
...
// XScale : 관련 Interface로 사용 - vntsorl(20070412)
//int CIWChartManager::FindViewDataIndex_Pt(int nXPoint)
//{
//	if( !m_pXScaleManager) return -1;
//	int nRet = 0;
//	int nCol = 0;
//	m_pXScaleManager->GetDTInAREAfRQnPt("DEFAULT", nXPoint, nCol, &nRet, NULL);
//	return nRet;
//}
