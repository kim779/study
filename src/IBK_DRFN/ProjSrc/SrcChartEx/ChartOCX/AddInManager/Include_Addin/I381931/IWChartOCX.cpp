...
BOOL CIWChartOCX::GetRedrawAfterUpdatedAllPacketFlag( void)
{
	return FALSE;
}
...
// (2007/2/12 - Seung-Won, Bae) Get Active RQ
IString *CIWChartOCX::GetActiveRQ( void)
{
	if( m_pIChartCtrl) return CIWString::CreateObject( m_pIChartCtrl->GetCurrentRQ());
	return NULL;
}
...
int	CIWChartOCX::GetRightMargin( void)
{
	if( !m_pIChartCtrl) return 0;
	return m_pIChartCtrl->GetRightMarginWidth();
}

BOOL CIWChartOCX::SetRightMargin( const int p_nCount)
{
	if( !m_pIChartCtrl) return 0;
	m_pIChartCtrl->SetRightMarginWidth( p_nCount);
	return TRUE;
}
