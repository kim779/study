...
BOOL CIWPacketManager::SetRealData( IPacket *p_pIPacket, double p_dData, int &p_nRemovePacketDataCount)
{
	ASSERT( FALSE);
	return FALSE;		// (2009/10/19 - Seung-Won, Bae) Use IPacketListManager.
}

BOOL CIWPacketManager::SetRealData( IPacket *p_pIPacket, const char *p_szField, const char *p_szKey, double p_dData, int &p_nRemovePacketDataCount)
{
	ASSERT( FALSE);
	return FALSE;		// (2009/10/19 - Seung-Won, Bae) Use IPacketListManager.
}
...
// 2008.08.07 김진순 종목간 Ratio chart 
//		모든 RQ의 이름을 구해온다		<<
void CIWPacketManager::GetAllRQ(CStringArray& arRQ)
{
...
void CIWPacketManager::GetItemCode(LPCTSTR lpszRQ, LPCTSTR& lpszItemCode, LPCTSTR& lpszItemName)
{
	if(m_pPacketListManager)
