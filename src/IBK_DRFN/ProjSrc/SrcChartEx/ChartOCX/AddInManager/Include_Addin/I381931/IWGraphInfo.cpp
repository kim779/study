...
BOOL CIWGraphInfo::GetSubGraphShowFlag( const int p_nSubGraphIndex, BOOL &p_bShow)
{
	...
	if( m_pIndicatorInfo)
	{
		...
		if( pGraph) if( pGraph->IsGraphHide()) p_bShow = FALSE;

		if( p_bShow) p_bShow = !m_pIndicatorInfo->GetSubGraphHiding( p_nSubGraphIndex);
	}
	return TRUE;
}
...
COLORREF CIWGraphInfo::GetSubGraphColorWithZBI( const int p_nSubGraphIndex, const int p_nColorIndex)
{
	CSubGraphData* pSubGraphData = GetSubGraphData( p_nSubGraphIndex);
	...
	{
		case 0: return clrGraph.GetColor1();
		case 1: return clrGraph.GetColor2();
		case 2: return clrGraph.GetColor3();
		case 3: return clrGraph.GetColor4();
		case 4: return clrGraph.GetColor5();
		case 5: return clrGraph.GetColor6();
	}
	return RGB( 0, 0, 0);
}
BOOL CIWGraphInfo::SetSubGraphColorWithZBI( const int p_nSubGraphIndex, const int p_nColorIndex, const COLORREF p_clrColor)
{
	CSubGraphData* pSubGraphData = GetSubGraphData( p_nSubGraphIndex);
	if(pSubGraphData == NULL) return FALSE;
	...
	switch( p_nColorIndex)
	{
		case 0: clrGraph.SetColor1( p_clrColor);
				break;
		case 1: clrGraph.SetColor2( p_clrColor);
				break;
		case 2: clrGraph.SetColor3( p_clrColor);
				break;
		case 3: clrGraph.SetColor4( p_clrColor);
				break;
		case 4: clrGraph.SetColor5( p_clrColor);
				break;
...
int CIWGraphInfo::GetSubGraphPenThicknessWithZBI( const int p_nSubGraphIndex, const int p_nPenIndex)
{
	CSubGraphData* pSubGraphData = GetSubGraphData( p_nSubGraphIndex);
	if(pSubGraphData == NULL) return -1;
	...
	switch( p_nPenIndex)
	{
		case 0: return penThickness.GetThickness1();
		case 1: return penThickness.GetThickness2();
		case 2: return penThickness.GetThickness3();
	}
	return -1;
}
...
void CIWGraphInfo::SetGraphDataDrawingData( BOOL p_bLowHigh, BOOL p_bLowCur, BOOL p_bHighCur, BOOL p_bPrevCur, int p_nCurData)
{
	if( !m_pIndicatorInfo) return;
	CGraphDataDrawingData *pGraphDataDrawingData = m_pIndicatorInfo->GetGraphDataDrawingData();
	if( !pGraphDataDrawingData) return;
	pGraphDataDrawingData->SetDoesCalcRiseFallRate_LowHigh( p_bLowHigh ? true : false);
	pGraphDataDrawingData->SetDoesCalcRiseFallRate_LowCur( p_bLowCur ? true : false);
	pGraphDataDrawingData->SetDoesCalcRiseFallRate_HighCur( p_bHighCur ? true : false);
	pGraphDataDrawingData->SetDoesCalcRate_PrevCur( p_bPrevCur ? true : false);
	pGraphDataDrawingData->SetCalcRiseFallRate_CurData( p_nCurData);
}
