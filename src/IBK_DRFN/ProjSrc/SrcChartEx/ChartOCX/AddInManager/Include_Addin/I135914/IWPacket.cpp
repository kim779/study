...
// (2007/12/17 - Seung-Won, Bae) Retreive the Prev Value (0:Base, 1:Upper Base,	2:Lower Base)
BOOL CIWPacket::GetStandardValue( double &p_dValue, int p_nType)
{
	p_dValue = -HUGE_VAL;
	if( !m_pPacket) return FALSE;
	if( !m_pPacket->IsUseStandardValue( p_nType)) return FALSE;
	p_dValue = m_pPacket->GetStandardValue( p_nType);
	return TRUE;
}
