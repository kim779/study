...
#include "_IChartAddIn.h"		// IChartAddIn
...
protected:
	...
	virtual void	OnDrawBeforeGraph( HDC p_hDC)
					{	m_pIChartAddIn->OnDrawBeforeGraph( CDC::FromHandle( p_hDC));	}
	// [04/12/25] Draw Event를 AddIn DLL에 알려 처리될 수 있도록 한다.
	virtual void	OnDrawAfterGraph( HDC p_hDC)
					{	m_pIChartAddIn->OnDrawAfterGraph( CDC::FromHandle( p_hDC));	}
	...
	virtual void	OnUpdatePacketDataMultiItem( LPCTSTR lpszRQ, BOOL p_bTrUpdate = FALSE)
					{	m_pIChartAddIn->OnUpdatePacketDataMultiItem(lpszRQ, p_bTrUpdate);	}
	...
	virtual void	OnDrdsData( WPARAM p_wParam, LPARAM p_lParam)
					{	m_pIChartAddIn->OnDrdsData( p_wParam, p_lParam);	}
...
//					{	m_pIChartAddIn->OnRealData( p_wRealRecordID, p_wRealItemID, p_szRealKeyCode, p_lPacketAddress, dData);	}
...
//					{	m_pIChartAddIn->OnRealRedcordEnd( p_wRealRecordID, p_szRealKeyCode);	}
	...
	virtual BOOL	OnPreRealData( const char *p_szPacketName, double &p_dData, const char *p_szRQ = NULL, BOOL p_bRealData = TRUE)	{	return TRUE;	}
	virtual BOOL	OnPreIndexedRealData( const char *p_szPacketName, const int p_nIndex, double &p_dData, BOOL p_bRealData = TRUE)	{	return TRUE;	}
	virtual BOOL	OnPreRealPacketEnd( const char *p_szRQ = NULL, BOOL p_bRealData = TRUE)											{	return TRUE;	}
	virtual BOOL	OnPreNMTRClock( __int64 &p_nRealClockSecs)																		{	return TRUE;	}
...
protected:
	virtual BOOL	OnCmdMsg(			UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
					{	return m_pIChartAddIn->OnCmdMsg(			nID, nCode, pExtra, pHandlerInfo);	}
	...
	virtual BOOL	OnAddInToolCommand(	const EAICommandType p_eCommandID, HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnAddInToolCommand(	p_eCommandID, CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	...
	virtual BOOL	OnAddInToolCommand(	const EAICommandType p_eCommandID, const CBlockBehaviorData *p_pParam)
					{	return m_pIChartAddIn->OnAddInToolCommand(	p_eCommandID, p_pParam);	}
	virtual BOOL	OnAddInToolCommand(	const EAICommandType p_eCommandID, const CStringList *p_pParam)
					{	return m_pIChartAddIn->OnAddInToolCommand(	p_eCommandID, p_pParam);	}
	virtual BOOL	OnAddInToolCommand(	const EAICommandType p_eCommandID, const CStyleData *p_pParam)
					{	return m_pIChartAddIn->OnAddInToolCommand(	p_eCommandID, p_pParam);	}
	...
	virtual BOOL	OnAddInToolCommand(	const EAICommandType p_eCommandID, HWND p_hWnd, POINT &point)
					{	return FALSE;	}
...
protected:
	...
	virtual BOOL	OnMouseMove(		HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID, const HBITMAP p_hbmpMainOSB)	// (2006/11/5 - Seung-Won, Bae) Support MainOSB
					{	return m_pIChartAddIn->OnMouseMove(		CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnLButtonDown(		HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnLButtonDown(		CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnLButtonUp(		HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnLButtonUp(			CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnLButtonDblClk(	HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnLButtonDblClk(		CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnRButtonDown(		HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnRButtonDown(		CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnRButtonUp(		HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnRButtonUp(			CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
	virtual BOOL	OnRButtonDblClk(	HDC p_hDC, UINT nFlags, POINT &point, const DWORD p_dwMouseActionID)
					{	return m_pIChartAddIn->OnRButtonDblClk(		CDC::FromHandle( p_hDC), nFlags, CPoint( point), p_dwMouseActionID);	}
};
