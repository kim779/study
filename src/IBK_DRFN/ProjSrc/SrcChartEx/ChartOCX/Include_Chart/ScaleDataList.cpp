...
#include <assert.h>
#include "DLL_Load/MainBlock.h"		// for CMainBlock
...
CHorizontalScalePtrDataList& CHorizontalScalePtrDataList::operator=(const CHorizontalScalePtrDataList& horzScalePtrDataList)
{
	...
	m_pMainBlock = horzScalePtrDataList.m_pMainBlock;

	return *this;
}

CHorizontalScalePtrDataList::CHorizontalScalePtrDataList( CMainBlock *p_pMainBlock):
	m_nFirstIndex(0),
	m_nMarginCount(0),
...
void CHorizontalScalePtrDataList::SetHorzScale(const int nIndex, const CHorizontalScale& horzScale)
{
	...
	// (2009/9/6 - Seung-Won, Bae) Do not support CHorizontalScale::SetHorzScalePosition() as public.
	//		To rearrange the row positions. Use CMainBlockImp::SetHorzScalePosition().
	if( m_pMainBlock) m_pMainBlock->SetHorzScalePosition( *pHorzScale, horzScale.GetHorzScalePosition());
...
CVerticalScalePtrData& CVerticalScalePtrData::operator=(const CVerticalScalePtrData& vertScalePtrData)
{
	...
	m_strPacketType	= vertScalePtrData.GetVertScalePacketType();
	return *this;
}
...
CVerticalScalePtrData::CVerticalScalePtrData(CVerticalScale* pVertScale, const CString& vertScaleTitle, const CString& vertPacketType):
	m_pVertScale( pVertScale),
	m_strVertScaleTitle( vertScaleTitle),
	// (2008/12/12 - Seung-Won, Bae) for octal number.
	m_strPacketType( vertPacketType)
{
}
...
void CVerticalScalePtrDataList::SetLogType(bool* pIsLog)
{
	m_pbLogType = pIsLog;
}

void CVerticalScalePtrDataList::SetLogType(const bool bIsLog)
{
	if( m_pbLogType != NULL) *m_pbLogType = bIsLog;
}
...
void CVerticalScalePtrDataList::SetVertScale(const int nIndex, const CVerticalScale& vertScale)
{
	...
	pVertScale->SetVertScaleMinMaxLock(	vertScale.IsVertScaleMinMaxLock());
	pVertScale->SetVertScaleMinMaxType(	vertScale.GetVertScaleMinMaxType());
	pVertScale->SetVertScaleMin(		vertScale.GetVertScaleMin());
	pVertScale->SetVertScaleMax(		vertScale.GetVertScaleMax());
	//2007.01.22 Scale사용자 설정,호가단위별
	pVertScale->SetVertScaleHoGaNumber(vertScale.GetVertScaleHoGaNumber());
	pVertScale->SetVertScaleUserNumber(vertScale.GetVertScaleUserNumber());
	pVertScale->SetVertUserHogaScaleType(vertScale.GetVertUserHogaScaleType());

	// 지표 뒤집기 - ojtaso (20071023)
	pVertScale->SetVertScaleInvertType( vertScale.GetVertScaleInvertType());
}
