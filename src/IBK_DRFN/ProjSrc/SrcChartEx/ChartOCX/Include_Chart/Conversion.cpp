...
// 2011.01.25 by SYS <<
	...
	if( 0 < length)
	{
		if( length < strData.GetLength())
			return strData.Left( length);
		else if( strData.GetLength() < length)
		{
			CString temp( '0', length - strData.GetLength());
			return temp + strData;
		}
	}
	else if(length < 0)
...
int CDataConversion::GetStringFormatTransCutoff(const double& dData, const CString& strFormat, CString& strData, CString& strFormatTrans, BOOL p_bWithAutoScale)
{
	...
	if (szFormat.Find(".") < 0)
	{
		...
		if (0 <= nStartIndex)
		{
			...
			if (0 < nTruncCount)
				szData = szData.Left( szData.GetLength() - nTruncCount);
		}
	}
...
CString CDataConversion::GetStringFormatTrans2(const CString& strData, const int& nDataUnit/*, const bool bIsRound*/)
{
...
//  
//  	// 2011.01.06 by SYS >> 분차트 YYYY(년) 추가
//  	if(szFormat == "YYYYMMDDHHMMSS" && strData.GetLength() == 14)
//  	{
//  		strValue = strData.Mid(4,8);
//  		szFormat = "MMDDHHMM";
//  	}
//  	// 2011.01.06 by SYS <<
//  
											  ...
											  const CString& strCompartDate, const CString& strCompartTime)
{
	...
	if (szFormat == "YYYYMMDDHHMMSS" || strData.GetLength() == 14)
	{
		// 날짜시간 data가 14Byte(YYYYMMDDhhmmss)인 경우.. 포맷에 맞게 Data를 짜른다.
		if (strFormat == "YYYYMMDD")
			strRealData = strData.Left(8);		// 일/주 경우...
		else if (strFormat == "YYYYMM")
			strRealData = strData.Left(6);		// 월간 경우...
		else if (strFormat == "MMDDHHMM")
			strRealData = strData.Mid(4,8);		// 분간 경우...
		else if (strFormat == "DDHHMMSS")
			strRealData = strData.Mid(6,8);		// 틱인 경우...
	}
	
	//CString strValue = IntToString(atoi(strData), szFormat.GetLength());
	CString strValue = IntToString(_atoi64(strRealData), szFormat.GetLength());
	// 2011.01.25 by SYS <<
	
	
	//sy 2004.04.16
	CString strSpecialTime;
	if(GetSpecialTime(strFormat, strRealData, strSpecialTime))
		return strSpecialTime;
	//end
...
// 2011.01.20 by SYS >> 분차트 YYYY(년) 추가 : 날짜/시간 14Byte형태 변경 (YYYYMMDDhhmmss)
CString CDataConversion::GetStringFormatTrans2(const CString& strData, const int& nDataUnit,
											  const CString& strCompartDate, const CString& strCompartTime)
{
	CString szData = "", szCompartDate = strCompartDate, szCompartTime = strCompartTime ;
...
double CDataConversion::CalcTimeDiff(double dData, int nTimeDiff, int nCurrTimeDiff, CString strType)
{
	...
	time_t timeResult;
	tm tmResult;
	tmResult.tm_isdst = -1;		//[A00000554]alzioyes:20110530 시간셋팅 초기값변경.
	/*
	time_t timeCur;
	tm* ptmCur;
	time(&timeCur);
...
// 20081014 박동관 해당 진법의 최소값을 리턴해준다. >>
CString CDataConversion::GetMinimumValue(CString strPacketType)
{
	CString strMinimumValue="";
	int nBase, nDecLen1, nDecLen2;

	BOOL bResult = GetBaseFormatInfo(strPacketType, nBase, nDecLen1, nDecLen2);
	...
	{
		// 소수부가 없다면... (예... 1.0, 10.0)
		for(int i=0; i<nDecLen1-1; i++)
			strMinimumValue += "0";
		strMinimumValue += "1";
	}
	else
	{
		// 소수부가 있다면... (예... 0.1, 0.001)
		for(int i=0; i<nDecLen1; i++)
			strMinimumValue += "0";
		strMinimumValue += ".";

		for(int i=0; i<nDecLen2-1; i++)
			strMinimumValue += "0";
		strMinimumValue += "1";
	}
...
#define _DAYS_NOV		30
#define _DAYS_DEC		31
#define _DAYS_BF_JAN	0
#define _DAYS_BF_FEB	_DAYS_BF_JAN + _DAYS_JAN
#define _DAYS_BF_MAR	_DAYS_BF_FEB + _DAYS_FEB
#define _DAYS_BF_APR	_DAYS_BF_MAR + _DAYS_MAR
#define _DAYS_BF_MAY	_DAYS_BF_APR + _DAYS_APR
#define _DAYS_BF_JUN	_DAYS_BF_MAY + _DAYS_MAY
#define _DAYS_BF_JUL	_DAYS_BF_JUN + _DAYS_JUN
#define _DAYS_BF_AUG	_DAYS_BF_JUL + _DAYS_JUL
#define _DAYS_BF_SEP	_DAYS_BF_AUG + _DAYS_AUG
#define _DAYS_BF_OCT	_DAYS_BF_SEP + _DAYS_SEP
#define _DAYS_BF_NOV	_DAYS_BF_OCT + _DAYS_OCT
#define _DAYS_BF_DEC	_DAYS_BF_NOV + _DAYS_NOV
const int g_nDaysBeforeMonth[ 12] = 
{
	_DAYS_BF_JAN, _DAYS_BF_FEB, _DAYS_BF_MAR, _DAYS_BF_APR, _DAYS_BF_MAY, _DAYS_BF_JUN,
	_DAYS_BF_JUL, _DAYS_BF_AUG, _DAYS_BF_SEP, _DAYS_BF_OCT, _DAYS_BF_NOV, _DAYS_BF_DEC
};

void CDataConversion::CalcTimeSum_YYYYMM( tm &tmSum, __int64 p_nTime2)
{
	ASSERT( 0 == p_nTime2 % 100000000);
	p_nTime2 /= 100000000;
	if( 0 == p_nTime2) return;

	tmSum.tm_mon +=  int( p_nTime2 % 100);
	p_nTime2 /= 100;
	tmSum.tm_year += int( p_nTime2);

	int nFullMonth = tmSum.tm_mon + tmSum.tm_year * 12;
	int nLeftDay = tmSum.tm_mday;
	mktime( &tmSum);

	// Check Day Rouding.
	if( tmSum.tm_mon + tmSum.tm_year * 12 != nFullMonth)
	{
		tmSum.tm_mon--;
		tmSum.tm_mday = nLeftDay - tmSum.tm_mday;
		mktime( &tmSum);
	}
}
void CDataConversion::CalcTimeSum_DDHHMMSS( tm &tmSum, __int64 p_nTime2)
{
	ASSERT( 0 == p_nTime2 / 100000000);

	tmSum.tm_sec += int( p_nTime2 % 100);
	p_nTime2 /= 100;
	tmSum.tm_min += int( p_nTime2 % 100);
	p_nTime2 /= 100;
	tmSum.tm_hour += int( p_nTime2 % 100);
	p_nTime2 /= 100;
	tmSum.tm_mday += int( p_nTime2 % 100);

	mktime( &tmSum);
}
__int64 CDataConversion::CalcTimeSum( __int64 p_nTime1, __int64 p_nTime2, BOOL p_bTime2IsSecond)
{
	if( 0 != p_nTime2 && p_bTime2IsSecond)
	{
		BOOL bMinus = FALSE;
		if( p_nTime2 < 0)
		{
		...
		int nSec = int( p_nTime2 % 60);
		p_nTime2 -= nSec;
		p_nTime2 /= 60;

		int nMin = 0, nHour = 0, nDay = 0, nMonth = 0;
		if( 0 < p_nTime2)
		{
			nMin = int( p_nTime2 % 60);
			p_nTime2 -= nMin;
			p_nTime2 /= 60;

			if( 0 < p_nTime2)
			{
				nHour = int( p_nTime2 % 24);
				p_nTime2 -= nHour;
				p_nTime2 /= 24;

				if( 0 < p_nTime2)
				{
					nDay = int( p_nTime2 % 365);
					p_nTime2 -= nDay;
					p_nTime2 /= 365;

					if( 0 < nDay) for( nMonth = 11; 0 <= nMonth; nMonth--)
						if( g_nDaysBeforeMonth[ nMonth] < nDay)
						{
							nDay -= g_nDaysBeforeMonth[ nMonth];
							break;
						}
				}
			}
		...
		if( bMinus) p_nTime2 = -p_nTime2;
	}

	tm tmSum;
	tmSum.tm_isdst = -1;		//[A00000554]alzioyes:20110530 시간셋팅 초기값변경.

	tmSum.tm_sec = int( p_nTime1 % 100);
	p_nTime1 /= 100;
	tmSum.tm_min = int( p_nTime1 % 100);
	p_nTime1 /= 100;
	tmSum.tm_hour = int( p_nTime1 % 100);
	p_nTime1 /= 100;
	tmSum.tm_mday = int( p_nTime1 % 100);
	int nOverDay = 0;
	if( 0 == tmSum.tm_mday)
	{
		nOverDay = 1;
	...
	p_nTime1 /= 100;
	tmSum.tm_mon = int( p_nTime1 % 100) - 1;
	int nOverMonth = 0;
	if( tmSum.tm_mon < 0)
	{
		nOverMonth = -tmSum.tm_mon;
		tmSum.tm_mon = 0;
	}
	p_nTime1 /= 100;
	tmSum.tm_year = int( p_nTime1) - 1900;
	int nOverYear = 0;
	if( tmSum.tm_year < 71)
	{
		nOverYear = 71 - tmSum.tm_year;
		tmSum.tm_year = 71;
	}
	tmSum.tm_wday = 0;
	tmSum.tm_yday = 0;
	...
	if( p_nTime2 > 0)
	{
		CalcTimeSum_YYYYMM( tmSum, p_nTime2 - p_nTime2 % 100000000);
		CalcTimeSum_DDHHMMSS( tmSum, p_nTime2 % 100000000);
	}
	else
	{
		CalcTimeSum_DDHHMMSS( tmSum, p_nTime2 % 100000000);
	...
	__int64 nTime = tmSum.tm_year + 1900 - nOverYear;
	nTime *= 100;
	nTime += tmSum.tm_mon + 1 - nOverMonth;
	nTime *= 100;
	nTime += tmSum.tm_mday - nOverDay;
	nTime *= 100;
	nTime += tmSum.tm_hour;
	nTime *= 100;
	nTime += tmSum.tm_min;
...
__int64 CDataConversion::GetSecond( __int64 p_nTime, BOOL p_bAfter19710101000000)
{
	int nSec = int( p_nTime % 100);
	p_nTime -= nSec;
	p_nTime /= 100;

	int nMin = int( p_nTime % 100);
	p_nTime -= nMin;
	p_nTime /= 100;
	...
	// p_nTime has year only.
	
	p_nTime *= 365;
	if( p_bAfter19710101000000)
	{
		p_nTime += ( nMonth / 12) * 365;
		p_nTime += g_nDaysBeforeMonth[ nMonth % 12];
	}
	else 				p_nTime += __int64( nMonth * 30.41666666666666666666667);
	p_nTime += nDay;
	p_nTime *= 24;
	p_nTime += nHour;
	p_nTime *= 60;
