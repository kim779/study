...
public:
	CHorizontalScalePtrDataList( CMainBlock *p_pMainBlock);
...
public:
	...
	CVerticalScalePtrData(CVerticalScale* pVertScale, const CString& vertScaleTitle, const CString& vertPacketType);

	CVerticalScale* GetVertScale();
	CVerticalScale* GetVertScale() const;
	CString GetVertScaleTitle() const;
	CString GetVertScalePacketType( void) const		{	return m_strPacketType;		}	// (2008/12/12 - Seung-Won, Bae) for octal number.
...
public:
	...
	void SetLogType(bool* pIsLog);
	void SetLogType(const bool bIsLog);
