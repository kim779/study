...
bool CScaleCalculator::GetPacketDataInTimeOut(const CString& dataType, CString& realPacketData)
{
	...
	if(CDataConversion( m_hOcxWnd, m_eChartMode).GetSpecialTime(dataType, realPacketData, strSpecialTime))
	{
...
CString CScaleCalculator::GetCompartScaleDate(const CString& packetData, const CString& dataType, const CString& compartType)
{
	...
	// < 년월일 >인 경우
		...
		if(dataType.Find('Y') <= 0)	{	//년도가 앞에 있는 경우 
			ChangeData(changePacketData, _LTEXT6( C6_DAY), dataType.ReverseFind('D'));
			ChangeData(changePacketData, _LTEXT6( C6_MONTH), dataType.ReverseFind('M'));
			ChangeData(changePacketData, _LTEXT6( C6_YEAR), dataType.ReverseFind('Y'));
		}
		else{	// 년도가 뒤에 있는 경우 - 태국
			ChangeData(changePacketData, _LTEXT6( C6_YEAR), dataType.ReverseFind('Y'));
			ChangeData(changePacketData, _LTEXT6( C6_MONTH), dataType.ReverseFind('M'));
...
CString CScaleCalculator::GetCompartScaleTime(const CString& packetData, const CString& dataType, const CString& compartType)
{
	...
	// < 시분초 >인 경우
		...
		ChangeData(changePacketData, _LTEXT6( C6_SECOND), dataType.ReverseFind('S'));
		ChangeData(changePacketData, _LTEXT6( C6_MINUTE), dataType.ReverseFind('M'));
		ChangeData(changePacketData, _LTEXT6( C6_HOUR), dataType.ReverseFind('H'));
	}
