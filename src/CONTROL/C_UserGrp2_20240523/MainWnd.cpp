...
CMainWnd::CMainWnd(CWnd *pParent, void *pInfo)
{
	...
	if (pInfo)
	{
		...
		m_param.key = pParam->key;  //test

		CString strtmp;
		strtmp = m_param.options;
		m_strMapOption = OptionParser(strtmp, "/f");
...
int CMainWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pGrpWnd->m_strMapOption = m_strMapOption;  //test 2009
	m_pGrpWnd->m_key = m_param.key;
	m_pGrpWnd->m_strparamName = m_param.name;
