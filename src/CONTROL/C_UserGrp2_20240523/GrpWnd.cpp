...
LONG CGrpWnd::OnUser(WPARAM wParam, LPARAM lParam)
{
	...
	case DLL_INB:		// get data from out
		...
		break;
	case DLL_ALERTx:
		if(m_strMapOption.Find("60interval") < 0)  //test
...
LONG CGrpWnd::OnGrp(WPARAM wParam, LPARAM lParam)
{
	...
	case GRP_Control:
		...
		case ChgDisplay:	// change display, lParam:display cnt
			ipos = (int)lParam;
m_slog.Format("\r\n [2009[%s] !!!!!!!  여기 여기 ChgDisplay ]ipos=[%d] ", __FUNCTION__, ipos);
...
void CGrpWnd::OnTimer(UINT nIDEvent) 
{
	...
	switch (nIDEvent)
	{
	case TIMER_60INTERVAL:
	{
		KillTimer(TIMER_60INTERVAL);
		if (m_bDestory)
			return;
		m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(0x97, 0), 0);
		m_bInterValSearch = TRUE;
		m_pView->SendMessage(WM_USER, MAKEWPARAM(eventDLL, MAKEWORD(m_key, evOnChange)), (LPARAM)m_strparamName.GetString());
	}
	break;
	case TIMER_STRID:
		KillTimer(nIDEvent);
...
m_slog.Format("\r\n [2009[%s] ]!!!!!!-----DateFromServer!!!! nData=[%d] iSize=[%d] dSize=[%d] nHead=[%d] ", __FUNCTION__, nData, iSize, dSize, nHead);
OutputDebugString(m_slog);
...
//-----------------------------------
	...
	if (m_strCode != m_pDataInfo->m_pInfo[1]->RtsCode)
	{
		m_slog.Format("\r\n [2009] 1111  m_strCode=[%s]",  m_strCode);
		OutputDebugString(m_slog);

		m_strCode = m_pDataInfo->m_pInfo[1]->RtsCode;
		m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(CTRL_Enable, TRUE), 0);
		m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(CTRL_UpdateScr, 0), 0);
		...
		if (m_strMapOption.Find("60interval") >= 0)  //test
			SetTimer(TIMER_60INTERVAL, 60000, nullptr);
	}
	else
	{
		if (m_bDestory)
		...
		if (m_strMapOption.Find("60interval") >= 0)  //test
		{
			int nSlider = m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(0x99, FALSE), 0);
			m_slog.Format("\r\n [2009] 22222 nSlider =[%d] m_strCode=[%s]", nSlider, m_strCode);
			OutputDebugString(m_slog);
			if (m_bInterValSearch && nSlider != 0)
			{
				m_bInterValSearch = false;
				m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(CTRL_Enable, TRUE), 0);
				m_pParent->SendMessage(CM_CTRL, MAKEWPARAM(0x98, nSlider), 0);
				ChangeDisplay(nSlider);
				RecalculateMinMax();
				Invalidate();
			}
			else
...
void CGrpWnd::RealTimeData(struct _alertR* alert)
{
	...
	//m_slog.Format("\r\n [2009][%s] sCode=[%s] ", __FUNCTION__, sCode);
	//OutputDebugString(m_slog);

	if (m_strMapOption.Find("2009") < 0)
	{
		if (sCode != m_pDataInfo->m_pInfo[0]->RtsCode)   //test  여기 주석이다. 첫번째만 실시간 받으면 쌍으로 그리는 차트는 어쩌란건지..
			return;
	}
	...
	for (int ii = alert->size - 1; ii >= 0; ii--)
	{
		...
		{
			if (!m_strMapOption.IsEmpty() && m_strMapOption.Find("2009") >= 0)
			{
				if (RealTime2009Basic(sCode, (DWORD*)alert->ptr[ii]))
					bChange = true;
			}
			else
			{
	...
	if (bChange)
	{
		int ival = m_pDataInfo->GetDataCount();
m_slog.Format("\r\n[2009]  !!! GetDataCount = true count=[%d]", m_pDataInfo->GetDataCount());
OutputDebugString(m_slog);
...
bool CGrpWnd::RealTime2009Basic(CString sCode, DWORD* data)
{
	CString	sVal, sTime = "";
	if (data[34])		// RS_CTIM
	{
	...
	for (int ii = 0; ii < m_GrpCnt; ii++)
	{
		if (sCode == m_pDataInfo->m_pInfo[ii]->RtsCode)
		{
			...
			{	// 각각의 그래프는 코드와 심볼 두개를 가지고 있다
				...
				//m_slog.Format("\r\n [2009][%s] sCode=[%s]  sVal=[%s] sKey=[%s] 0symbol= [%s]", __FUNCTION__, sCode, sVal, sKey, (char*)data[0]);
				//OutputDebugString(m_slog);

				m_slog.Format("\r\n [2009][%s]ii=[%d] sCode=[%s]  sVal=[%s] sKey=[%s] 0symbol= [%s]", __FUNCTION__, ii, sCode, sVal, sKey, (char*)data[0]);
				OutputDebugString(m_slog);

				iResult = m_pDataInfo->RealTime2009Data(ii, sTime, sVal, (char*)data[32], bInsert);
				if (iResult == RTS_INSERT)
				{
					bChange = TRUE;
					bInsert = false;
...
void CGrpWnd::ParseParam(char *param)
{
	...
	if ((pGrpInfo->option3 & ICF_ITEMRTS) == ICF_ITEMRTS)	
		m_bItemRts = true;
...
m_slog.Format("\r\n [2009[%s] ]!!!!!!!!!! m_dIndex=[%d] m_bItemRts=[%d] m_CfgRgnInfo=[%s] m_CfgRgnCnt=[%d] gCnt=[%d]", __FUNCTION__,
	m_dIndex, m_bItemRts, m_CfgRgnInfo, m_CfgRgnCnt, gCnt) ;
OutputDebugString(m_slog);
}
...
{
	//m_slog.Format("\r\n [2009[%s]  ChgDisplay ]dispDay=[%d] ", __FUNCTION__, dispDay);
	//OutputDebugString(m_slog);
