...
CfxTab::CfxTab()
{
	...
	m_bmpSelL = m_bmpSelM = m_bmpSelR = m_bmpL = m_bmpM = m_bmpR = nullptr;
...
void CfxTab::SetBorderColor(COLORREF bcolor)
{
	m_bLine = TRUE;
	m_linecolor = bcolor;
}
...
void CfxTab::SetImage(HBITMAP hselectL, HBITMAP hSelectM, HBITMAP hSelectR, HBITMAP hL, HBITMAP hM, HBITMAP hR)
{
...
void CfxTab::SetImage(CString imageDir, CString selectL, CString selectM, CString selectR, CString left, CString middle, CString right)
{		
	if (m_bmpSelL != nullptr)
		return;
...
std::shared_ptr<CBitmap> CfxTab::CreateBmp( LPCTSTR sBMPFile/*, CPalette *pPal */)
{
	CFile file;
	...
	const DWORD nPackedDIBLen = gsl::narrow_cast<const int>(file.GetLength() - sizeof(BITMAPFILEHEADER));
	...
	std::shared_ptr<CBitmap> bitmap = std::make_shared<CBitmap>();
	bitmap->Attach( hBmp );
...
int CfxTab::getBmpHeight(int bmpidx)
{
	BITMAP bm;
	GetObject(m_pics[bmpidx], sizeof(bm), &bm);
	return bm.bmHeight;
