...
CfxColorPopup::CfxColorPopup(CFont* pFont/*=nullptr*/)
{
	m_pFont = pFont;
...
{
	const int width = xBTN * nCOL + szGAP2*5 + szGAP4*2 +1;
	...
	if (!CreateEx(0, AfxRegisterWndClass(CS_DBLCLKS, ::LoadCursor(nullptr, IDC_ARROW)),
		_T("Color popup"), WS_POPUP|WS_BORDER,
			0, 0, width, height, parent->GetSafeHwnd(), (HMENU) 0, nullptr))
	{
...
void CfxColorPopup::DisplayColorPopup(CPoint point, bool show, WPARAM wp)
{
	...
	if (show)
		...
		SetWindowPos(nullptr, 0, 0, 0, 0, SWP_HIDEWINDOW|SWP_NOSIZE|SWP_NOMOVE);
...
{
	CColorBtn* oldcurBtn = nullptr, *curBtn = nullptr;
...
void CfxColorPopup::drawCaption(CDC* dc)
{
	...
	CGdiObject* sFont{};
	if (m_pFont != nullptr) sFont = dc->SelectObject(m_pFont);
...
void CfxColorPopup::drawCustom(CDC* dc, bool hasColor)
{
	...
	CGdiObject* oldFont{};
	if (m_pFont != nullptr) oldFont = dc->SelectObject(m_pFont);
...
{
	const CColorBtn* selBtn = HitTest(point);
	if (selBtn == nullptr && m_customRC.PtInRect(point))
	{
		m_customSEL = true;
...
{
	CColorBtn *selBtn = nullptr, *hitBtn = nullptr;
