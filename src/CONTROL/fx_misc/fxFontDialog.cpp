...
CfxFontDialog::CfxFontDialog(CString home, CWnd* pParent /*=nullptr*/)
...
{
	const int	entry = m_sizeCB.GetCurSel();
	if (entry != CB_ERR)
	{
...
{
	const int	entry = m_styleCB.GetCurSel();
	m_actual = m_styleCB.GetItemData(entry);
	GetParent()->PostMessage(WM_USER+112, 0, 0);
}
...
BOOL CfxFontDialog::Create(CWnd* parent)
{
	...
	const BOOL rc = CDialog::Create(IDD_FONTDLG, parent);
	SetOwner(parent);
...
void CfxFontDialog::SetFont(CString faces, int point, bool italic, bool bold, DWORD fgColor, DWORD bgColor, bool applyNow)
{
	...
	if (applyNow)
	{
		const int	entry = m_nameCB.FindStringExact(-1, m_faces);
		if (entry != CB_ERR)
		{
			m_nameCB.SetCurSel(entry);
			m_nameCB.SetEditSel(-1, -1);
...
int CALLBACK EnumFontFamiliesCallBack(ENUMLOGFONTEX* lpelf, NEWTEXTMETRICEX* metric, int fontType, LPARAM lParam)
{
	...
	const gsl::not_null<CfxFontDialog*> dlg = (CfxFontDialog *)lParam;
	//ASSERT(dlg);
...
//	ASSERT(dlg);
	...
	else
	{
		...
		{
			CDC*	dc = dlg->GetDC();
			const int height = metric->ntmTm.tmHeight - metric->ntmTm.tmInternalLeading;
...
void CfxFontDialog::fillFontSizes()
{
	...
	const BOOL	bRegular    = (BOOL)(m_style & NTM_REGULAR);
	...
	const int	count  = m_styleCB.GetCount();
