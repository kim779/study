...
CSize CMBongWnd::CalcuWndSize(CString strExam)
{
	...
	sFont = pDC->SelectObject(m_pFont);
...
{
	POINT	pt{};
	CRect	rect, rcMy, rcTip, rcParent;
...
COLORREF CMBongWnd::GetGradColor(COLORREF color, bool bDark)
{
	...
	r = (rColor + rRate > 255)? 255:rColor + rRate;
	g = (gColor + gRate > 255)? 255:gColor + gRate;
	b = (bColor + bRate > 255)? 255:bColor + bRate;
	return RGB(r, g, b);
}
