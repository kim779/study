...
CUGGridInfo::CUGGridInfo()
{
	...
	m_WEResizseCursor	= LoadCursor(nullptr,IDC_SIZEWE);
	m_NSResizseCursor	= LoadCursor(nullptr,IDC_SIZENS);
...
CUGGridInfo::~CUGGridInfo()
{
	if(m_colInfo != nullptr)
	{
		...
		{
			if(m_colInfo[loop].colDefault != nullptr)
				delete m_colInfo[loop].colDefault;
		}
		delete[] m_colInfo;
	}

	if(m_rowHeights	!= nullptr)
		delete[] m_rowHeights;
	if(m_topHdgHeights != nullptr)
		delete[] m_topHdgHeights;
	if(m_sideHdgWidths != nullptr)
		delete[] m_sideHdgWidths;
	if(m_gridDefaults != nullptr)
		delete m_gridDefaults;
	if(m_hdgDefaults != nullptr)
		delete m_hdgDefaults;
	if(m_multiSelect != nullptr)
		delete m_multiSelect;
	if(m_CUGOverlay != nullptr)
		delete m_CUGOverlay;
	if(m_CUGMem != nullptr)
		delete m_CUGMem;

	if(m_defBorderPen != nullptr)
