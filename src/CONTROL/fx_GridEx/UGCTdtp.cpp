								 ...
								 int selected,int current)
{
	...
	{
		const int nOldTextColor = cell->GetTextColor();
		if(cell->GetParam())
	...
	//draw the down arrow
	const int x= ((m_btnWidth-5)/2) + rect->left;
...
[[gsl::suppress(6387)]]
	...
	while(nState==0)
	{
		...
		{
			const POINTS points=MAKEPOINTS(msg.lParam);	
			const CPoint pt(points.x,points.y);
...
BOOL CUGCTDateTimePicker::SetMonthCalDimension(CMonthCalCtrl* pMonthCal, 
	...
	const CPoint ptTopLeft=rect.TopLeft();
