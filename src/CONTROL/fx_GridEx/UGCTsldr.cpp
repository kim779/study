...
BOOL CUGSliderType::OnLClicked(int col,long row,int updn,RECT *rect,POINT *point){
	...
	const long width = rect->right - rect->left - 6;
	const int xPos = rect->left + 3 + ((width * percent) / 100);
...
void CUGSliderType::OnDraw(CDC *dc,RECT *rect,int col,long row,
	...
	const int width = rect->right - rect->left - 6;
	if(width <= 6){
		return;
	}

	//find the slider x position
	const int xPos = rect->left + ((width * percent) / 100) +3;

	const int yPosMid = rect->top + (rect->bottom - rect->top)/2;
