...
int CUGDropListType::StartDropList()
{
	...
	m_listBox->Create(WS_CHILD|WS_BORDER|WS_VSCROLL,CRect(0, 0, 0, 0),m_ctrl->m_CUGGrid,123);
	...
	while(pos < len){
		...
		GetTextExtentPoint(pDC->GetSafeHdc(), strTemp, strTemp.GetLength(), &sz);
		if (nWidth < sz.cx)
			nWidth = sz.cx;
	}
