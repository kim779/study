...
{
	CUGPrintList * item = nullptr;	//new print list item

	//if the list is already empty then just return
	if(m_printList == nullptr)
	{
						...
						int endCol,long endRow)
{
	...
	CDC	sDC;
	sDC.CreateDC(_T("DISPLAY"), nullptr, nullptr, nullptr);
	...
	const float paperWidthPX		= (float)pDC->GetDeviceCaps(HORZRES);
	const float paperHeightPX		= (float)pDC->GetDeviceCaps(VERTRES);
	const float paperWidthMM		= (float)pDC->GetDeviceCaps(HORZSIZE);
	const float paperHeightMM		= (float)pDC->GetDeviceCaps(VERTSIZE);
	const int virtualWidthPX = (int)( 3.79 * (paperWidthMM - m_printLeftMargin - m_printRightMargin));
	const int virtualHeightPX = (int)( 3.78 * (paperHeightMM - m_printTopMargin - m_printBottomMargin));
...
int CUGPrint::PrintPage(CDC * pDC, int pageNum)
{
	...
	RECT origCellRect{};
	const int leftMargin = m_printLeftMarginPX;
	const int topMargin =  m_printTopMarginPX;
