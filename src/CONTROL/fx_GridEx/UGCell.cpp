...
int CUGCell::AddCellInfo(CUGCell *src,CUGCell *dest){
	...
	if(src->m_propSetFlags&UGCELL_EXTRAMEMORY_SET){
		if(dest->m_extraMem)
			dest->m_extraMem.reset();
		if(src->m_extraMem != nullptr){
			TRY{
				dest->m_extraMem = std::make_unique<std::byte[]>(src->m_extraMemSize);
				memcpy(dest->m_extraMem.get(), src->m_extraMem.get(), (size_t)src->m_extraMemSize);
				dest->m_extraMemSize = src->m_extraMemSize;
			}
			CATCH( CException, e ){
				dest->m_extraMem = nullptr;
...
{
	if (!( m_propSetFlags&UGCELL_STRING_SET ))
	{	// if a value is not set to a cell than return nullptr
		return nullptr;
	}
	else if ( m_dataType == UGCELLDATA_NUMBER )
	{
		...
		if (!( m_propSetFlags&UGCELL_DONOT_LOCALIZE ))
		{
			...
			{
				const int iDotIndex = strDouble.ReverseFind('.');
				if (iDotIndex == -1)
					nformat.NumDigits = 0;
				else
			...
			// Format the buffer
			const int iRes = ::GetNumberFormat(LOCALE_USER_DEFAULT, 0, strDouble, &nformat, szBuffer, 256); 
			if (iRes == 0)
			{
...
GetBorderColor
	...
	Params
		none
	Return
		Pointer to a CPen object, or nullptr if
...
void *	CUGCell::AllocExtraMem(long len){
	...
	try{
		m_extraMem = std::make_unique<std::byte[]>(len);
		if(m_extraMem == nullptr)
			return nullptr;
	}
	catch( CException* e ){
		e->Delete();
...
int CUGCell::StringToNumber(CString *string,double *number)
{
	...
	*number = ::_tcstod(tempString, nullptr);
...
int CUGCell::StringToTime(CString* string,int* second,int* minute,int* hour,int* day, int* month,int* year)
{
	...
	const COleDateTime ovTime(ovStr);
