...
int CUGMem::InsertRow(long row)
{
	...
	if(GotoRow(row)==UG_SUCCESS)
	{
		...
		newrow->cellLookup = nullptr;

		if(newrow->prev != nullptr)
...
int CUGMem::InsertCol(int col)
{
	...
	const int numCols = m_ctrl->GetNumberCols();
	if(numCols > 0)
	{
...
int CUGMem::DeleteRow(long row)
{
	...
	if(GotoRow(row)==UG_SUCCESS)
	{
		...
		while(m_colInfo != nullptr)       //delete the cols to the end
		{
			next = m_colInfo->next;
				
			//delete the colinfo
			if(m_colInfo->cell != nullptr)
		...
		else if(m_rowInfo->prev != nullptr)
		{
...
int CUGMem::SetCell(int col,long row,CUGCell *cell)
{
	...
	if(m_rowInfo == nullptr)
	{
		...
		m_rowInfo->col	= nullptr;
		m_rowInfo->next	= nullptr;
		m_rowInfo->prev	= nullptr;
		m_rowInfo->cellLookup = nullptr;
	...
	if(m_colInfo == nullptr)
	{
		...
		m_colInfo->cell	= nullptr;
		m_colInfo->next	= nullptr;
		m_colInfo->prev	= nullptr;
		m_currentCol	= 0; 
...
int CUGMem::FindNext(CString *string,int *col,long *row,int flags)
{
	...
	const long lastRow = m_ctrl->GetNumberRows()-1;
	const int  lastCol = m_ctrl->GetNumberCols()-1;
	const long searchLimit = lastRow+2;		// search with wrap
	long rowCount = 0;
	BOOL colsDone = FALSE;
...
int CUGMem::SortBy(long startRow, long endRow, int *cols,int numCols,int flags)
{
	...
	for(index =0; index < numCols;index++){
		if(GotoCol(cols[index])==UG_SUCCESS)
			...
			NewListRowInfo->cellLookup[index] = nullptr;
	}

	
	//main row sort loop - while there are items in the unsorted list
	while(OldListRowInfo != nullptr && (rowCount < endRow) ){
		...
		for(index =0; index < numCols;index++){
			if(GotoCol(cols[index])==UG_SUCCESS)
				...
				OldListRowInfo->cellLookup[index] = nullptr;
		}

		//check the half point 
		//================================================
		//get a cell from the unsorted list
		if(OldListRowInfo->cellLookup[0] != nullptr)
		...
		if(NewListRowInfo_HalfPt->cellLookup[0] != nullptr)
		...
		if(halfPtOffset == -2){
			halfPtOffset = 0;
			if(NewListRowInfo_HalfPt->prev != nullptr)
				if(NewListRowInfo_HalfPt->prev->cellLookup != nullptr)
					NewListRowInfo_HalfPt = NewListRowInfo_HalfPt->prev;
		}
		if(halfPtOffset == 2){
			halfPtOffset = 0;
			if(NewListRowInfo_HalfPt->next != nullptr)
		...
		//NewListRowInfo = NewListStartRowInfo;
		bInserted = FALSE;
		while(NewListRowInfo != nullptr){

			//sort by the columns in their given order
			for(index = 0; index < numCols; index++){

				//get a cell from the unsorted list
				if(OldListRowInfo->cellLookup[index] != nullptr)
				...
				//get a cell from the sorted list
				if(NewListRowInfo->cellLookup[index] != nullptr)
				...
				if(rt < 0){					
					...
					if(OldListRowInfo->prev !=nullptr)
	...
	if(rowCount == endRow && OldListRowInfo != nullptr){
		GotoRow(endRow);
	...
	GotoRow(0);
	do{
		if(m_rowInfo->cellLookup != nullptr)
...
int CUGMem::GetPrevNonBlankCol(int *col,long row)
{
	...
	while(tempCol >= 0)
	{	
		if(tempCI->cell != nullptr && *col != tempCol){
