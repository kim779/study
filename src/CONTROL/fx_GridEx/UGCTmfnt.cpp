...
void CUGMultiFontType::OnDraw(CDC *dc,RECT *rect,int col,long row,
	...
	for(x=0;x<len;x++){
		...
		if(once){
			once = FALSE;
			dc->ExtTextOut(left,top,ETO_CLIPPED,rect,string.Mid(x,1),1,nullptr);
		}
		else
			dc->ExtTextOut(left,top,ETO_CLIPPED,rect,string.Mid(x,1),1,nullptr);
...
int CUGMultiFontType::ParseCommand(CDC *dc,CString *command)
{
	int x{};
	int pos[30]{};
	const int len = command->GetLength();
	int count =0;
	CString fontName;
	CString string;
	int r{}, g{}, b{}, p{}, w{};
	...
	if(command->GetAt(0)==_T('F')){
		...
		m_font = std::unique_ptr<CFont>();
		m_font->CreateFont(p,0,0,0,w,0,0,0,0,0,0,0,0,fontName);
		m_oldFont = dc->SelectObject(m_font.get());
	}
...
void CUGMultiFontType::GetBestSize(CDC *dc,CSize *size,CUGCell *cell)
{
	...
	for( int inStrPos = 0; inStrPos < len; inStrPos++ )
	{
		...
		{
			//get the format option string
			const int tempStrPos = inStrPos + 2;
