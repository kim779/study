...
{
	m_pDropGridStruct = std::unique_ptr<CreateDropDownGrid>();
	m_pGridCtrl = nullptr;
	...
	m_popupWnd.CreateEx( WS_EX_TOOLWINDOW|WS_EX_TOPMOST, className, _T(""), WS_POPUP, CRect(0,0,0,0), 0, 0 );
...
CUGCTDropGrid::~CUGCTDropGrid()
{
	if (m_pGridCtrl && m_pGridCtrl->GetSafeHwnd() != nullptr )
	...
	if( updn )
	{
		...
		{			
			if( col == m_btnCol && row == m_btnRow )
			{
				m_btnCol = -1;
				m_btnRow = -1;		
				if ( m_pDropGridStruct->pGridCtrl->GetSafeHwnd() != nullptr )
...
void CUGCTDropGrid::OnDraw(CDC *dc,RECT *rect,int col,long row,CUGCell *cell,int selected,int current)
{
	...
	{
		const int x= (int) ((fScale * m_btnWidth-5)/2) + rect->left;
		const int y = ((rect->bottom - rect->top -3)/2) + rect->top;

		// create a pen object that will be used to draw the arrow on the button
		CPen *arrowPen = (CPen*)CPen::FromHandle((HPEN)GetStockObject(BLACK_PEN));;
		std::unique_ptr<CPen> uPen;
		if ( cell->GetReadOnly() == TRUE )
		{
			uPen = std::make_unique<CPen>();
			uPen->CreatePen( PS_SOLID, 1, RGB(128,128,128));
			arrowPen = uPen.get();
		}
...
int CUGCTDropGrid::ShowPopupGridWnd()
{
	...
	if (!( cell.GetCellTypeEx()&UGCT_SHOWASPOPUPWND ))
	{
		...
		{
			const int dif = m_pDropGridStruct->popUpWndRect.bottom - clientRect.bottom;
		...
		{
			const int dif = m_pDropGridStruct->popUpWndRect.right - clientRect.right;
	...
	if ( m_pGridCtrl != nullptr )
	{
		if ( m_pGridCtrl->GetSafeHwnd() != nullptr )
			m_pGridCtrl->DestroyWindow();

		m_pGridCtrl.reset();;
	}
	// create the default grid control
	m_pGridCtrl = std::make_unique<CDropDownCug>();
	m_pGridCtrl->CreateGrid( WS_CHILD|WS_BORDER, CRect(0,0,0,0), &m_popupWnd, 3551512 );

	m_pDropGridStruct->pPopupWnd = &m_popupWnd;
	m_pDropGridStruct->pGridCtrl = m_pGridCtrl.get();

	// notify the user of the celltype that the popup window is about to be displayed
	if ( OnCellTypeNotify(m_ID,m_btnCol,m_btnRow,UGCT_SHOWDROPGRID,(long)m_pDropGridStruct.get()) == FALSE )
...
Return:
	<none>
***************************************************/
int CUGCTDropGrid::OnSelectedGridItem( SelectedGridItem *pSelectedInfo /*= nullptr*/ )
{
	CUGCell cell;
