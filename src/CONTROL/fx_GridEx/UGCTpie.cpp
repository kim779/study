							...
							CUGCell *cell,int selected,int current)
{
	...
	if(m_AspectRatio == TRUE){
		...
		if(width != height){
			if(width >height){
				const int dif = (width - height)/2;
				rect->left += dif;
				rect->right -= dif;
			}
			else{
				const int dif = (height - width)/2;
				rect->top += dif;
				rect->bottom -= dif;
			}
		}
	...
	const int len = pString.GetLength();
	int spos =0;
	int index = 0;
	for(int t=0;t<len;t++){
	...
	CUGCell		c;	
	double		totalValue =0;
	std::unique_ptr <double[]>    Values = std::make_unique<double[]>((range[2]-range[0]+1) * (range[3]-range[1]+1));
	std::unique_ptr <COLORREF[]>  Colors = std::make_unique<COLORREF[]>((range[2]-range[0]+1) * (range[3]-range[1]+1));
	long		totalSlices =0;
...
int CUGPieChartType::DrawPieSlice(CDC *pDC,RECT *rect,int startAngle,int endAngle,int separation,COLORREF color){
	...
	const double sa = (6.28318 / 360 ) * (double)startAngle*-1;	//start angle
	const double ea = (6.28318 / 360 ) * (double)endAngle*-1;		//end angle
	const double ma = (6.28318 / 360 ) *
	...
	const int width	  = tRect.right - tRect.left;
	const int height  = tRect.bottom - tRect.top;
	const int yCenter = tRect.top + (height/2)  ;
	const int xCenter = tRect.left + (width/2) ;
