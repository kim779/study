...
CUGAdvProgressType::CUGAdvProgressType(){
	...
	m_numberSections =	0;
	m_gradlinePercent = 5;
	m_sectionPercent =	nullptr;
	m_sectionColor =	nullptr;
	m_canAdjust =		TRUE;
								...
								CUGCell *cell, int selected, int current)
{
	...
	for (i=0;i<m_numberSections;i++){
		...
		dc->ExtTextOut(left,top,ETO_CLIPPED,&fillrect,string,nullptr);
...
int CUGAdvProgressType::SetNumberSections(int sections){
	...
	if (m_sectionPercent != nullptr){
		m_sectionPercent.reset();
		m_sectionColor.reset();
	}

	// create new lists for section data
	m_sectionPercent = std::make_unique<int[]>(sections);
	m_sectionColor   = std::make_unique<COLORREF[]>(sections);
