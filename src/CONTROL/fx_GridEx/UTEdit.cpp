...
void CUTEdit::PreSubclassWindow()
{
	...
	if(pThreadState->m_pWndInit==nullptr && !m_bInitialized)
	{
		if(!InitControl())
...
{
	const BOOL bResult=CEdit::Create(dwStyle, rect, pParentWnd, nID);
	if(bResult)
	{
...
void CUTEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	const BOOL bIsShiftKeyDown=::GetAsyncKeyState(VK_SHIFT)<0;
	const BOOL bIsCtrlKeyDown=::GetAsyncKeyState(VK_CONTROL)<0;
	if(nChar==VK_DELETE)
	{
...
LONG CUTEdit::OnCut(UINT wParam, LONG lParam)
{
	...
	const int nDeleteCount=DeleteRange(nSelectionStart,nSelectionEnd);
...
LONG CUTEdit::OnPaste(UINT wParam, LONG lParam)
{
	...
	if(nSelectionStart!=nSelectionEnd)
	{
		...
		const int nPosition=sNewString.Find(sRightLiterals);
		if(nPosition!=-1 && 
	...
	const int nNewInsertionPoint=GetNextInputLocation(nSelectionStart);
...
LONG CUTEdit::OnSetText(UINT wParam, LONG lParam)
{
	...
	{
		const LONG result=CEdit::Default();
...
BOOL CUTEdit::SetMask(LPCTSTR lpszMask)
{
	...
	for(int nIndex=0; nIndex<sMask.GetLength(); nIndex++)
	{
		...
		const TCHAR chSymbol=sMask[nIndex];
		const TCHAR chSpecialSymbol=OXEDITABLETEXT_SYMBOL;
...
{
	const int nNewInsertionPoint=GetPreviousInputLocation(nSelectionStart);
	SetSel(nNewInsertionPoint,nNewInsertionPoint);
}
...
BOOL CUTNumericEdit::GetLocaleSettings()
{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SDECIMAL,nullptr,0);
	if(nLength)
	{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_STHOUSAND,nullptr,0);
	if(nLength)
	{
	...
	TCHAR* buffer{};
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_ILZERO,nullptr,0);
	if(nLength)
	{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SGROUPING,nullptr,0);
	if(nLength)
	{
		...
		const int nPosition=sGrouping.Find(_T(";0"));
		if(nPosition!=-1)
		{
			ASSERT(nPosition>0);
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_IDIGITS,nullptr,0);
	if(nLength)
	{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_INEGNUMBER,nullptr,0);
	if(nLength)
	{
		...
		const int nNegativeNumberFormatType=_ttoi(buffer);
...
{
	const int nSymbolsBefore=m_arrInputData.GetSize();
	...
	const int nFractionalDigitsInserted=GetFractionalDigitsInserted();
	...
	const int nFractionalOvertypedCount=
	...
	if(m_arrInputData.GetSize()>0 && GetDecimalDigitCount()!=-1)
	{
		...
		{
			const int nDecimalOvertypedCount=nDecimalDigitsInserted-GetDecimalDigitCount();
	...
	const int nSymbolsAfter=m_arrInputData.GetSize();
...
int CUTNumericEdit::ChangeSign(BOOL bNegative)
{
	...
	const int nPosInputData=sMask.Find(OXEDITABLETEXT_SYMBOL);
	...
	const int nLeftLiteralsBefore=m_arrLeftLiterals.GetSize();
	...
	if(m_bNegativeValue)
	{
		const int nPosInNegative=m_sFormatNegative.Find(OXNUMERICVALUE_PLACEHOLDER);
	...
	{
		const int nPosInPositive=m_sFormatPositive.Find(OXNUMERICVALUE_PLACEHOLDER);
...
{
	ASSERT(lpszFormatNegative!=nullptr);
...
{
	ASSERT(lpszFormatPositive!=nullptr);
...
BOOL CUTNumericEdit::SetValue(double dValue)
{
	...
	const BOOL bSetNegative=m_bNegativeValue;
									...
									TCHAR chNewChar, BOOL bReplaceExisting)
{
		...
		(m_bForceToUsePeriod && chNewChar==_T('.')))
	{
			...
			(m_bForceToUsePeriod && chNewChar==_T('.')))
		{
			if(GetFractionalDigitCount()==0)
				return -1;
			const int nPos=FindSymbol(m_chDecimalSeparator);
			if(nPos==-1)
			{
...
{
	// be sure that the control is a CUTNumericEdit
	CUTNumericEdit* pEdit = (CUTNumericEdit*)pDX->m_pDlgWnd->GetDlgItem(nIDC);
	ASSERT(pEdit->IsKindOf(RUNTIME_CLASS(CUTNumericEdit)));
	ASSERT_VALID(pEdit);
...
BOOL CUTCurrencyEdit::GetLocaleSettings()
{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SMONDECIMALSEP,nullptr,0);
	if(nLength)
	{
		CString sExisting(m_chDecimalSeparator);
		CString sDefault(m_chDecimalSeparatorDefault);
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SMONTHOUSANDSEP,nullptr,0);
	if(nLength)
	{
		CString sExisting(m_chGroupSeparator);
		CString sDefault(m_chGroupSeparatorDefault);
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SMONGROUPING,nullptr,0);
	if(nLength)
	{
		bSetNewValue=(m_nGroupLength==m_nGroupLengthDefault);
		buffer=new TCHAR[nLength];
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_ICURRDIGITS,nullptr,0);
	if(nLength)
	{
		bSetNewValue=(m_nFractionalDigitCount==m_nFractionalDigitCountDefault);
		buffer=new TCHAR[nLength];
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_SCURRENCY,nullptr,0);
	if(nLength)
	{
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_ICURRENCY,nullptr,0);
	if(nLength)
	{
		...
		const int nPositiveNumberFormatType=_ttoi(buffer);
	...
	nLength=::GetLocaleInfo(LOCALE_USER_DEFAULT,LOCALE_INEGCURR,nullptr,0);
	if(nLength)
	{
		bSetNewValue=(m_sFormatNegative.CompareNoCase(m_sFormatNegativeDefault)==0);
		buffer=new TCHAR[nLength];
...
void CUTCurrencyEdit::UpdateMask()
{
	...
	const int nPosValue=sFormat.Find(OXNUMERICVALUE_PLACEHOLDER);
	ASSERT(nPosValue!=-1);
...
{
	// be sure that the control is a CUTNumericEdit
	CUTCurrencyEdit* pEdit = (CUTCurrencyEdit*)pDX->m_pDlgWnd->GetDlgItem(nIDC);
	ASSERT(pEdit->IsKindOf(RUNTIME_CLASS(CUTCurrencyEdit)));
	ASSERT_VALID(pEdit);
	if (pDX->m_bSaveAndValidate)
	{
		// retreive data
