...
int CUGTab::AdjustScrollBars()
{
	...
	CRect rect;
	GetClientRect( rect );
	const int width = rect.right - m_scrollWidth;
	const int dif = totalTabWidths - width;
...
void CUGTab::OnPaint() 
{
	...
	{	
		...
		if ( nTabIndex != m_currentTab )
		{
			...
			rect.left += 10;
			rect.top ++;
			dc.ExtTextOut( rect.left, rect.top, 0, rect, m_tabStrings[nTabIndex], nullptr );	
			rect.left -= 10;
			rect.top --;
		}
		else
	...
	rect.left += 10;
	rect.top ++;
	dc.ExtTextOut( rect.left, rect.top, 0, rect, m_tabStrings[m_currentTab], nullptr );	
...
void CUGTab::OnRButtonDown( UINT nFlags, CPoint point ) 
{
	...
	if ( m_GI->m_enablePopupMenu )
	{
		...
		{	
			const int right = left + m_tabWidths[tabIndex];
