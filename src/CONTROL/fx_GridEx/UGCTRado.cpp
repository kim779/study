...
BOOL CUGRadioType::OnLClicked(int col,long row,int updn,RECT *rect,POINT *point)
{
	if(updn)
	{
		...
		if(m_cell.IsPropertySet(UGCELL_FONT_SET) == FALSE)
			if(m_ctrl->m_GI->m_defFont != nullptr)
			dc->SelectObject(m_ctrl->m_GI->m_defFont);
		else
			dc->SelectObject(m_cell.GetFont());

		const CSize size = dc->GetTextExtent(_T("X"),1);
		m_ctrl->ReleaseDC(dc);
		itemHeight = size.cy;
		...
		const int select = y / itemHeight;

		//get the old selection
		const int oldSelect = (int)m_cell.GetNumber();

		//get the number of items
		CString label = m_cell.GetLabelText();
		int itemCount = 0;
		int t{};
		const int len = label.GetLength();
		for(t = 0;t<len;t++){
			if(label.GetAt(t) == _T('\n')){
				itemCount++;
			}
...
void CUGRadioType::OnDraw(CDC *dc,RECT *rect,int col,long row,
	...
	std::unique_ptr<int[]> positions = std::make_unique<int[]>(itemCount+1);
	positions[0] = 0;
	int count =1;
	for(t = 0;t<len;t++){
		if(label.GetAt(t) == _T('\n')){
	...
	const int selectNum = (int)cell->GetNumber();
...
void CUGRadioType::GetBestSize(CDC *dc,CSize *size,CUGCell *cell)
{
	...
	const CSize itemSize = dc->GetTextExtent(_T("X"),1);
