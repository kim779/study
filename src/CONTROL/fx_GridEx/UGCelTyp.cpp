...
BOOL CUGCellType::CanOverLap(CUGCell* cell)
{
	...
	{
		const long alignment = cell->GetAlignment();
	...
	int selected, int current)
{
	...
	CRect   rectTmp(rect);
	int     nHeadColCount = 0;
	const float   fScale = m_ctrl->GetUGPrint()->GetPrintVScale(dc);
	const int     nColCount = m_ctrl->GetUGPrint()->m_ColRects.GetSize();
	...
	if ((size.cx > (int)(m_ctrl->GetColWidth(col) * fScale)) &&
		...
		for (int i = col + 1; i + nHeadColCount < nColCount; i++) {
			...
			if (nullptr != cellRight.GetText()) break;
...
void CUGCellType::DrawText(CDC* dc, RECT* rect, int offset, int col, long row, CUGCell* cell, int selected, int current)
{
	...
	if (cellTypeEx & (UGCT_NORMALMULTILINE | UGCT_NORMALELLIPSIS)) {
		...
		if (cellTypeEx & UGCT_NORMALMULTILINE) { // multiline
			...
			// 		dc->ExtTextOut(rect->left + 3,top,ETO_OPAQUE|ETO_CLIPPED,rect,string,stringLen,nullptr);
			// 	rect->left = rect->right;
			// 	if(rect->left <= oldleft)
			// 		rect->left = oldleft + 1;
			// 	rect->right += size.cx + 3;
			// 	dc->ExtTextOut(rect->left+1,top,ETO_OPAQUE|ETO_CLIPPED,rect,_T("..."),3,nullptr);
			// }
			// else{
			// 	dc->ExtTextOut(left,top,ETO_OPAQUE|ETO_CLIPPED,rect,string,stringLen,nullptr);
			// }
		}
	}
	else {
		...
		DrawBackground(dc, rect, backcolor);
		dc->ExtTextOut(left, top, ETO_CLIPPED, rect, string, stringLen, nullptr);
	}
...
int CUGCellType::DrawBitmap(CDC* dc, CBitmap* bitmap, RECT* rect, COLORREF backcolor)
{
	...
	if (dc->IsPrinting())	// Print the bitmap	
	{
		...
		hDib = BitmapToDIB((HBITMAP)(bitmap->m_hObject), nullptr);
...
{
	const long props = cell->GetPropertyFlags();
	...
	const int style = cell->GetBorder();
	CPen* pen = cell->GetBorderColor();
	CPen* origPen = nullptr;
...
void CUGCellType::GetBestSize(CDC* dc, CSize* size, CUGCell* cell)
{
	...
	else if (m_ctrl->m_GI->m_defFont != nullptr)
