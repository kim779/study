...
void CUGMarqueeType::OnDraw(CDC *dc,RECT *rect,int col,long row,
	...
	int length{};
	int position{};
	const int cellWidth = rect->right - rect->left;

	//get the length of the text
	if(cell->IsPropertySet(UGCELL_FONT_SET) )
		dc->SelectObject(cell->GetFont());
	const CSize size = dc->GetOutputTextExtent(cell->GetText());
	...
	dc->ExtTextOut(rect->left + (cellWidth - position),top,ETO_CLIPPED,rect,cell->GetText(),nullptr);
}
			...
			)
{
	...
	const int leftCol = ctrl->GetLeftCol();
	const int rightCol = ctrl->GetRightCol();
	const long topRow = ctrl->GetTopRow();
	const long bottomRow = ctrl->GetBottomRow();
