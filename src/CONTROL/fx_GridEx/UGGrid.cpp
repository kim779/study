...
void CUGGrid::OnPaint() 
{
	...
	{
		db_dc = new CDC;
		db_dc->CreateCompatibleDC(nullptr);
...
void CUGGrid::DrawCellsIntern(CDC *dc,CDC *db_dc)
{
	...
	for(y = 0; y < m_GI->m_numberRows;y++)
	{
		...
		if(y >= (m_GI->m_numberRows-1)){
			const long origNumRows = m_GI->m_numberRows;
			long newRow = y+1;
		...
		if((m_hasFocus || m_ctrl->m_findDialogRunning) && !m_ctrl->m_editInProgress)
		{
			if(m_GI->m_highlightRowFlag)
			{
				...
				else
				{
					...
					{
						const int iStartCol = m_ctrl->GetLeftCol(), iEndCol = m_ctrl->GetNumberCols() - 1;
						int iWidth = 0;
...
void CUGGrid::Moved()
{
	...
	int x{};
	long y{};
	CDC * pDC{}, *screenDC{},*db_DC = nullptr;
	...
	for (hPrevWnd = m_hWnd; (hNextWnd = ::GetWindow(hPrevWnd, GW_HWNDPREV)) != nullptr; hPrevWnd = hNextWnd){
	...
	if ( (nullptr != hActiveWnd) && (m_hWnd != hActiveWnd) ) {
	...
	if ( (nullptr != hDtWnd) && (m_hWnd != hDtWnd) ) {
	...
	if(xScroll != 0 || yScroll != 0)
	{
		...
		pDC->ScrollDC(-xScroll,-yScroll,&scrollRect,&scrollRect,nullptr,nullptr);
...
void CUGGrid::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if(m_GI->m_ballisticKeyMode > 0)
	{
		...
		const int value = (m_keyRepeatCount / m_GI->m_ballisticKeyMode);
	...
	const BOOL processed = m_ctrl->GetCellType(m_GI->m_currentCol,m_GI->m_currentRow)->
		OnKeyDown(m_GI->m_currentCol,m_GI->m_currentRow,&nChar);

	//send a keydown notify message
	m_ctrl->OnKeyDown(&nChar,processed);

	const int  curCol = m_GI->m_currentCol;
	const long curRow = m_GI->m_currentRow;
...
void CUGGrid::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(m_cellTypeCapture)
	{
		...
		//send a notification to the cell type
		const BOOL processed = m_ctrl->GetCellType(col,row)->OnMouseMove(col,row,&point,nFlags);
	...
	if(nFlags & MK_LBUTTON)
	{
		...
		while(1)
		{
			...
			if(point.x < 0)
			{
				...
				{
					const int increment = (int)pow((point.x * -1)/ m_GI->m_defColWidth +1,
			...
			else if(point.x > m_GI->m_gridWidth || horsViewMove)
			{
				...
				if(m_GI->m_ballisticMode)
				{
					const int increment = (int)pow((point.x - m_GI->m_gridWidth) /
			...
			if(point.y < 0)
			{
				...
				if(m_GI->m_ballisticMode)
				{
					const long increment = (long)pow((point.y* -1) / m_GI->m_defRowHeight +1,
						m_GI->m_ballisticMode); 
					m_ctrl->GotoRow(m_GI->m_dragRow - increment);
					if(increment == 1)
						Sleep(m_GI->m_ballisticDelay);
			...
			else if(point.y > m_GI->m_gridHeight || vertViewMove)
			{
				...
				if(m_GI->m_ballisticMode)
				{
					const long increment = (long)pow((point.y-m_GI->m_gridHeight) /
...
void CUGGrid::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	m_ctrl->SendMessage(WM_VSCROLL,MAKEWPARAM(nSBCode,nPos),
		(LPARAM)(pScrollBar!=nullptr ? pScrollBar->GetSafeHwnd() : nullptr));
}
...
BOOL CUGGrid::OnHelpInfo(HELPINFO* pHelpInfo) 
{
	if (pHelpInfo->iContextType == HELPINFO_WINDOW)
	{
		...
		const DWORD dwTopicID = m_ctrl->OnGetContextHelpID( col, row, UG_GRID );
		if ( dwTopicID == 0 )
			return FALSE;
