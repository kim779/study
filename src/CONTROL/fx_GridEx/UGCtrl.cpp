...
CUGCtrl::CUGCtrl()
{
	...
	*********************************************/
	...
	//add a nullptr datasource to the beginning of the
	//data source list, since index 0 is not used
	AddDataSource(nullptr);
...
CUGCtrl::~CUGCtrl()
{
	...
	if(m_bitmapList != nullptr)
		delete m_bitmapList;

	if(m_cellTypeList != nullptr)
		delete m_cellTypeList;
	
	if(m_cellStylesList != nullptr)
		delete m_cellStylesList;
	
	if(m_validateList != nullptr)
		delete m_validateList;
	
	if(m_displayFormatList != nullptr)
		delete m_displayFormatList;
	
	if(m_menu != nullptr)
		delete m_menu;

	if(m_CUGGrid != nullptr)
		delete m_CUGGrid;
	
	if(m_CUGTopHdg != nullptr)
		delete m_CUGTopHdg;
	
	if(m_CUGSideHdg != nullptr)
		delete m_CUGSideHdg;
	
	if(m_CUGVScroll != nullptr)
		delete m_CUGVScroll;
	
	if(m_CUGHScroll != nullptr)
		delete m_CUGHScroll;
	
	if(m_CUGCnrBtn != nullptr)
		delete m_CUGCnrBtn;

	//scroll hint window
	#ifdef UG_ENABLE_SCROLLHINTS
		if(m_CUGHint != nullptr)
			delete m_CUGHint;
	#endif

	#ifdef UG_ENABLE_PRINTING
		if(m_CUGPrint != nullptr)
			delete m_CUGPrint;
	#endif
...
void CUGCtrl::OnSysColorChange() 
{
	...
	int loop{};
	const int count = m_cellTypeList->GetCount();

	for(loop=0;loop <count;loop++){
		if(m_cellTypeList->GetPointer(loop) != nullptr){
...
void CUGCtrl::AdjustComponentSizes()
{
	int loop{};
	RECT rect{};
	const int vScrollWidth = m_GI->m_vScrollWidth;
	const int hScrollHeight = m_GI->m_hScrollHeight;
	const int origTabWidth = m_GI->m_tabWidth;
...
BOOL CUGCtrl::CreateChildWindows(){
	...
	m_CUGGrid->Create(nullptr,_T(""),WS_CHILD|WS_VISIBLE,rect,this,1,nullptr);
	m_CUGTopHdg->Create(nullptr,_T(""),WS_CHILD|WS_VISIBLE,rect,this,2,nullptr);
	m_CUGSideHdg->Create(nullptr,_T(""),WS_CHILD|WS_VISIBLE,rect,this,3,nullptr);
	m_CUGCnrBtn->Create(nullptr,_T(""),WS_CHILD|WS_VISIBLE,rect,this,4,nullptr);
	m_CUGTab->Create(nullptr,_T(""),WS_CHILD|WS_VISIBLE,rect,this,5,nullptr);
...
BOOL CUGCtrl::CreateGrid(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID){
	...
	const BOOL rt = Create((LPCTSTR)nullptr,(LPCTSTR)_T(""),(DWORD) dwStyle, (const RECT&) rect,
		(CWnd*) pParentWnd,(UINT) nID, (CCreateContext*)nullptr);
...
void CUGCtrl::CalcTopRow(){
	...
	const long oldMaxRow = m_GI->m_maxTopRow;
...
void CUGCtrl::AdjustTopRow(){
	...
	if(m_GI->m_uniformRowHeightFlag == FALSE){
		...
		const int height = m_GI->m_gridHeight - m_GI->m_lockRowHeight;
		const long row = m_GI->m_dragRow - height / m_GI->m_defRowHeight + 1;
...
void CUGCtrl::Moved()
{
	...
	BOOL scrolled = FALSE;
	const int nOldRightCol = GetRightCol();
	const long nOldBottomRow = GetBottomRow();
	...
	int nIndex{};
	const int nNewRightCol = GetRightCol();
	const long nNewBottomRow = GetBottomRow();
...
int CUGCtrl::GetAbsoluteCellFromPoint(int x,int y,int *ptcol,long *ptrow){
	...
	const int tempJoinFlag = m_GI->m_enableJoins;

	RECT rect{};
	const int rt = GetCellFromPoint(x,y,ptcol,ptrow,&rect);
...
int	CUGCtrl::StartEdit(int col,long row,int key)
{
	...
	if(m_GI->m_defDataSource->OnEditStart(m_editCol,m_editRow,&m_editCtrl) != FALSE)
	{
		...
		{	
			...
			else if(m_GI->m_defFont != nullptr)
...
int	CUGCtrl::GetCellIndirect(int col,long row,CUGCell *cell){
	...
	if(col >= 0 && row >= 0 && col < m_GI->m_numberCols){
		if(m_GI->m_colInfo[col].colDefault != nullptr)
...
int	CUGCtrl::SetNumberRows(long rows,BOOL redraw){
	...
	const long oldNumRows = m_GI->m_numberRows;
	...
	const int oldTop = m_GI->m_topRow;
	CalcTopRow();
...
int CUGCtrl::SetNumberCols(int cols,BOOL redraw){
	...
	const int oldNumCols = m_GI->m_numberCols;
...
int CUGCtrl::FitToWindow(int startCol,int endCol){
	...
	const BOOL bOldPaintMode = m_GI->m_paintMode;
	m_GI->m_paintMode = TRUE;
...
int	CUGCtrl::InsertCol(int col){
	...
	const int rt = ds->InsertCol(m_GI->m_colInfo[col].colTranslation);
...
int	CUGCtrl::DeleteCol(int col){
	...
	const int rt = ds->DeleteCol(m_GI->m_colInfo[col].colTranslation);
...
int	CUGCtrl::AppendRow(){
	...
	const int rt = m_GI->m_defDataSource->AppendRow();
	if(rt == UG_SUCCESS){
		SetNumberRows(m_GI->m_numberRows + 1);
	}
	return rt;
...
int	CUGCtrl::DeleteRow(long row){
	...
	const int rt = m_GI->m_defDataSource->DeleteRow(row);
	if(rt == UG_SUCCESS){
		SetNumberRows(m_GI->m_numberRows - 1);
	}
	return rt;
...
int CUGCtrl::FindDialog(){
	...
	m_findReplaceDialog->Create(TRUE,_T(""),nullptr,FR_DOWN,this);
...
int CUGCtrl::ReplaceDialog(){
	...
	m_findDialogStarted = TRUE;
	m_findReplaceDialog = new CFindReplaceDialog;
	m_findReplaceDialog->Create(FALSE,_T(""),nullptr,FR_DOWN,this);
...
long CUGCtrl::ProcessFindDialog(UINT,long)
{
	...
	if(m_findReplaceDialog == nullptr)
		return UG_ERROR;
	if(m_findReplaceDialog->GetSafeHwnd() == nullptr)
		return UG_ERROR;
...
long CUGCtrl::AddCellType(CUGCellType *celltype){
	...
	const int index = m_cellTypeList->AddPointer(celltype);
...
int CUGCtrl::RemoveCellType(int index){
	...
	if (nullptr != pCellType) delete pCellType;
...
#	ifdef _UNICODE
	...
	res = WideCharToMultiByte(CP_ACP,0,(LPCTSTR)*string,len,tempStr,len,nullptr,nullptr);
...
int CUGCtrl::RemoveDataSource(int index){
	...
	if(m_dataSrcList[index] != nullptr){
		m_dataSrcList[index] = nullptr;
		return UG_SUCCESS;
	}
...
int CUGCtrl::SetGridUsingDataSource(int index){
	...
	const int cols = ds->GetNumCols();
	if(cols <1)
		return 2;

	SetNumberCols(cols,FALSE);

	const long rows = ds->GetNumRows();
	if(rows >=0)
...
int CUGCtrl::SetSheetNumber(int index,BOOL update){
	...
	const BOOL origValue = m_enableUpdate;
	if(!update){
...
int	CUGCtrl::SetTH_NumberRows(int rows){
	...
	if(m_GI->m_topHdgHeights != nullptr){
...
int	CUGCtrl::SetSH_NumberCols(int cols){
	...
	if(m_GI->m_sideHdgWidths != nullptr){
...
#	ifdef _UNICODE
	...
	// convert to text for copy...
	res = WideCharToMultiByte(CP_ACP,0,SelectString,len,string,len,nullptr,nullptr);
...
////////////////////////////////////////////////////
	...
	GlobalUnlock(hglobal);
	if(res)	// conversion ok
		m_dataSource.CacheGlobalData(CF_TEXT,hglobal,nullptr);
...
#	ifdef _UNICODE
	...
	GlobalUnlock(hglobalu);
	m_dataSource.CacheGlobalData(CF_UNICODETEXT,hglobalu,nullptr);
#	endif


////////////////////////////////////////////////////

	m_dataSource.DoDragDrop(DROPEFFECT_COPY,nullptr,nullptr);
...
int CUGCtrl::SetGridLayout( int layoutMode )
{
	...
	// as current layout
	const LONG lExStyles = GetWindowLong( GetSafeHwnd(), GWL_EXSTYLE );
...
int CUGCtrl::StartMenu(int col,long row,POINT *point,int section){
	...
	if( m_menu->TrackPopupMenu(TPM_LEFTALIGN|TPM_RIGHTBUTTON,
		point->x,point->y,this,nullptr) == FALSE){
...
{
	const int lfBaseSize = sizeof(LOGFONT) - LF_FACESIZE;
	int count{}, loop{};
	LOGFONT lf2{};
...
int CUGCtrl::RemoveBitmap(int index){
	...
	if (nullptr != bitmap) delete bitmap;
...
int CUGCtrl::MoveColPosition(int fromCol,int toCol,BOOL insertBefore){
	...
	const int fTransCol				= m_GI->m_colInfo[fromCol].colTranslation;
	const int fWidth					= m_GI->m_colInfo[fromCol].width;
...
void CUGCtrl::MoveTrackingWindow(){
	...
	if(m_GI->m_trackingWndMode == 1){  //stay close mode
		const int tWidth = tRect.right - tRect.left;
		const int tHeight = tRect.bottom - tRect.top;
