...
{	
	...
	const int oldEndCol = m_currentItem->endCol;
	const int oldEndRow = m_currentItem->endRow;
...
{	
	...
	if(!found)
	{
		...
		const int selected = IsSelected(col,row);
		//add the cell
		StartBlock(col,row);
		if(selected)
...
int CUGMultiSelect::EnumNextBlock(int *startCol,long *startRow,int *endCol,long *endRow)
{
	...
	while(next != nullptr && count < m_enumBlockNumber)
	{
		next = next->next;
		count++;
	}
...
Returns:
	UG_ERROR if the multiselection is disabled, or no
		cells are selected (m_list is nullptr)
