...
int CUGMaskedEdit::MaskKeyStrokes(UINT *vKey){
	...
	const int l = m_literals.GetAt(endPos);
	if(l != 8){
	...
	//check the key against the mask
	const int c = m_mask.GetAt(endPos);
	switch(c){
		case '0':{
			if(isdigit(key))
				return TRUE;
...
int CUGMaskedEdit::SetMask(LPCTSTR string){
	...
	int loop{}, loop2{}, pos{};
	const int len = m_origMask.GetLength();
...
int CUGMaskedEdit::MaskString(CString *string){
	...
	const int len = m_mask.GetLength();
	TCHAR *buf = new TCHAR[len+1];
	...
	for(loop =0;loop <len;loop++){
		...
		//check to see if the string matches the mask
			...
			const int c = m_mask.GetAt(loop);
			if(pos == string->GetLength())
				break;
			const int d = string->GetAt(pos);
			int valid = FALSE;
