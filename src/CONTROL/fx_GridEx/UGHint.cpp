...
BOOL CUGHint::Create(CWnd* pParentWnd, HBRUSH hbrBackground)
{
    ...
    wndClass.hIcon=::LoadCursor(nullptr,IDC_ARROW); 
    wndClass.hCursor=0; 
    ...
    if(!CWnd::CreateEx(WS_EX_TOOLWINDOW|WS_EX_TOPMOST, wndClass.lpszClassName, _T(""), 
		WS_BORDER|WS_POPUP, rect, nullptr, 0, nullptr))
	{
        return FALSE;
	}
	...
	m_hFont		= nullptr;				//font handle
	
	//get the font height
	CDC * dc = GetDC();
	const CSize fontSize = dc->GetTextExtent(_T("X"),1);
...
int CUGHint::SetFont(CFont * font)
{
	...
	if(m_hFont != nullptr)
		dc->SelectObject(m_hFont);
	const CSize s = dc->GetTextExtent(_T("Xy"),2);
	m_fontHeight = s.cy + 3;
	ReleaseDC(dc);
...
int CUGHint::MoveHintWindow(int x,int y,int width)
{
	...
	const CSize textSize = dc->GetTextExtent( m_text, m_text.GetLength());
