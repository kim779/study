...
void WebLinkCtrl::GetInformation()
{
	...
	m_sPopMap = ProfileStr(m_menu, "POPMAP", "", path).value;
	m_sPopMap.TrimRight();
}
...
LRESULT WebLinkCtrl::OnUser(WPARAM wParam, LPARAM lParam)
{
	...
	// 			}
				...
				if (url.Find("ibks.com") > -1 && m_description.Find("NOHTTPS") < 0 )
					url.Replace("http://", "https://");

				m_finalurl = url;

				//if (GetOSversion() == 11 || IsHTML())
				//{
				//	openhtml();
				//	//openhtml(m_bCertLogin);
				//}
				//else
					PostMessage(WM_BROWSER, 0, (LPARAM)url.GetString());

	
				if (!m_sPopMap.IsEmpty() && m_sPopMap.Left(2) == "IB")
				{
					m_parent->SendMessage(WM_USER, MAKEWPARAM(viewDLL, typeVIEW), (LPARAM)m_sPopMap.operator LPCSTR());
				}

				//m_parent->GetParent()->ShowWindow(SW_HIDE);
...
#define DF_TESTBROWSER1
void WebLinkCtrl::testbrowser(WPARAM wParam, LPARAM lParam)
{
	CString stmp;

	stmp.Format("\r\n[CX_WEBLINK][%s]<%d> 1  pBrowser2=[%x]  m_bCertLogin=[%d]\r\n",  __FUNCTION__, __LINE__, pBrowser2, m_bCertLogin);
	OutputDebugString(stmp);

	if (pBrowser2 == nullptr)
	{
		openhtml();
		return;
	}
	...
	stmp.Format("\r\n[CX_WEBLINK][%s]<%d> 2  hr=[%d]  m_pIEApp=[%x] \r\n", __FUNCTION__, __LINE__, hr, m_pIEApp);
	OutputDebugString(stmp);
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			BSTR bstrFDS = GetFSDValue().AllocSysString();  //test
			...
			hr = pBrowser2->Navigate(bstrURLFDS, &vEmpty, &vEmpty, &vEmpty, &vHeader);
		}
	}
	else
		openhtml();
...
{
#ifdef DF_TESTBROWSER
	testbrowser(wParam, lParam);
	return 0;
#endif
	...
	// 				surl += m_sUrl;
					...
					//	sParam.Format("url=%s&cflag=1&certType=%s&auth=%s", m_sUrl,"cloude", m_sCert);
						if (bCloude)
							sParam.Format("url=%s&cflag=1&certType=%s&auth=%s", m_sUrl, "cloud", m_sCert);
						else
							sParam.Format("url=%s&cflag=1&certType=%s&auth=%s", m_sUrl, "", m_sCert);

						sParam += GetFSDValue();  //onBrowser m_bCertLogin
					}
				...
				else
				{
					...
					if (ival)
					{
						BSTR bstrFDS = GetFSDValue().AllocSysString();   //onBrowser m_bCertLogin not
					...
					/*else if (ival == 2)
					{
						...
						BSTR bstrFDS = GetFSDValue().AllocSysString(); //주석
						...
						CString sGubn;
						sGubn = _T("&auth=");
						int ifind = stmp.Find(sGubn);
						ifind += sGubn.GetLength();
						CString sParam;
						sParam.Format("url=%s&cflag=1&certType=%s&auth=%s", m_sUrl, "", stmp.Mid(ifind));
						sParam += GetFSDValue();  //주석
...
int WebLinkCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_slog.Empty();
	if (m_pMainFrame != nullptr && m_sMapName == "IB596400")  //IB596400 미국시세 신청 화면 팝업시 메인에 저장되있는 고객권한을 초기환다다
	{
		if (1)
		{
			int readL = 0;
			...
			m_pMainFrame->SendMessage(WM_USER, MAKEWPARAM(MMSG_SET_USER_ABORAD_AUTH, 0), (LPARAM)(LPSTR)(LPCTSTR)m_slog);
	}
	return 0;
}
...
CString WebLinkCtrl::GetFSDValue()
{
...
BOOL WebLinkCtrl::IsHTML()
{
	CString sFDS{}, sRes{};
	CString filename{}, sPath{}, stmp{};
	filename.Format("%s", m_sRoot);
	filename.Format("%s\\%s\\%s", m_sRoot, "tab", "WEBINFO.INI");
	CFileFind   finder;

	int readL = 0;
	char readB[8]{};
	readL = GetPrivateProfileString("cx_weblink", "htmlview", "0", readB, sizeof(readB), filename);
	stmp.Format("%s", readB);
	stmp.TrimRight();
...
void WebLinkCtrl::openhtml(bool bCertLogin)
{
	CString url;
	url = m_finalurl;
