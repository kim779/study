...
BEGIN_DISPATCH_MAP(CControlWnd, CAxWnd)
	...
	DISP_FUNCTION(CControlWnd, "IsForeign", IsForeign, VT_I2, VTS_BSTR)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
long CControlWnd::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case DLL_OUB:	// NOT USED. Use DLL_OUBx!!
		...
		if (m_bDomino && length < 13 && length > 2)
		{
			...
			m_pCode->SetValidCheck(false);
			m_pCode->SetEditData((char*)lParam);
			m_bFocus = false;
		}
		else if (length > 100 && !m_bInit)
		{
...
// 	if (hcode != "" && m_pCode->GetEditData() == "") 
// 	{
// 		//m_pCode->SetEditData(hcode);//SetCode(hcode);
// 	}
// 	
...
CString CControlWnd::Variant(int iCommand, CString strData)
{	
	if (iCommand == triggerCC && !m_bTrigger)
		return "";

	return CAxWnd::Variant(iCommand, strData);
}
...
void CControlWnd::SetData(LPCTSTR code) 
{
	...
	if (!m_pCode || m_rtnStr.CompareNoCase(strCode) == 0)
		return;

	Msg("SetData(%s)", strCode);
	m_pCode->SetEditData(strCode);
	m_pCode->m_bFirst = true;

// 	m_pCode->HistoryMode();
// 	CString hcode = m_pCode->GetHistCode(0);

	//m_pCode->SetEditData(hcode);
...
void CControlWnd::SetCode(LPCTSTR code) 
{
	...
	Msg("SetCode(%s)", strCode);
	m_bModify = true;
	m_pCode->SetEditData(strCode);
	m_pCode->m_bFirst = true;
// 
// 	//m_pCode->m_pEdit->_commandEntries//
// 	m_pCode->m_pEdit->GetParent()->SetFocus();

	//m_pCode->m_pEdit->SendMessage(WM_LBUTTONDOWN,0,0);
}
...
void CControlWnd::SetTabStop(BOOL bNewValue) 
{
	...
	m_pWizard->SendMessage(WM_USER, MAKEWPARAM(tabsDLL, MAKEWORD(m_Param.key, m_bTabStop ? 1 : 0)), 
...
void CControlWnd::OnTimer(UINT nIDEvent) 
{
	...
	else if (nIDEvent == stockDELAY)
	{
		KillTimer(stockDELAY);
		m_pCode->LoadData();
...
// 	m_pCode->m_pEdit->GetParent()->SetFocus();
// 	m_pCode->m_pEdit->PostMessage(WM_KEYDOWN, VK_RIGHT, 0);
}
...
void CControlWnd::SetUnitData(short nNewValue) 
{
	...
	if (m_nGubn == GU_FOREIGN || m_nGubn == GU_FCODE)
	{
 		m_pCode->HistoryMode();
 		CString hcode = m_pCode->GetHistCode(0);
	...
	//SetTimer(stockDELAY,100,NULL);
	m_pCode->LoadData();
}

short CControlWnd::IsForeign(LPCTSTR code) 
{
	CString strCode(code);

	return m_pCode->IsForeign(strCode);
}
