...
int CExListCtrl::hitTestEx(const CPoint& Point, int* pColumn)
{
	...
	int	colNum = 0;
	int	colWidth = 0;
	const	int	nColCount = m_Header.GetItemCount();
...
{
	const	int	nColCnt = m_Header.GetItemCount();
	if (col >= nColCnt || GetColumnWidth(col) < 5)
		return (CEdit *)nullptr;
	...
	SetEditState(true);
	m_pEdit->SetLimitText((col == 1) ? 12 : 9);
...
{
	std::shared_ptr<ListData> pData = std::make_shared<ListData>(str);
	m_Ptr.Add(pData);
	SetItemData(row, (DWORD)pData.get());
}
