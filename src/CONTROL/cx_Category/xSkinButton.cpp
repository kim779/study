...
void CxSkinButton::PreSubclassWindow() 
{
	...
	if ((m_Style & BS_AUTOCHECKBOX)==BS_AUTOCHECKBOX)	// ||((m_Style & BS_CHECKBOX)==BS_CHECKBOX))
		m_Style=BS_CHECKBOX;
	else if ((m_Style & BS_AUTORADIOBUTTON)==BS_AUTORADIOBUTTON) // ||((m_Style & BS_RADIOBUTTON)==BS_RADIOBUTTON))
...
void CxSkinButton::FillWithBitmap(CDC* dc, HBITMAP hbmp, RECT r)
{
	...
	for (int y = r.top, z = 0; y < h ; y += by)
	{
		if (y + by > h)
			by = h - y;
		z=bx;
		for (int x = r.left ; x < w ; x += z)
		{
			if (x + z >w)
				z = w - x;
...
void CxSkinButton::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{ //track mouse for radio & check buttons
		...
		const	HWND	mouse_wnd = ::WindowFromPoint(p2);
		if (mouse_wnd == m_hWnd)
		{ // mouse is in button
...
LRESULT CxSkinButton::OnBMSetCheck(WPARAM wparam, LPARAM)
{
	...
	case BS_RADIOBUTTON:
		...
		{ //uncheck the other radio buttons (in the same group)
			...
			HWND	hthis = GetSafeHwnd();			//get this button handle
			HWND	hwnd2 = hthis;
			if (hthis && hpwnd)
			{				//consistency check
				for (;;)
