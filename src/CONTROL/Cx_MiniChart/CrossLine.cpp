...
CCrossLine::CCrossLine(CWnd *pView, CWnd *pParent, CWnd *pMainWnd, COLORREF bg, COLORREF fg)
{
	m_pView = pView;
	m_pParent = pParent;
	m_fgColor = fg;
...
void CCrossLine::DrawCrossLine(CDC *pDC)
{
	...
	CPen	*cPen = (CPen*)((CMainWnd*)m_pMainWnd)->GetPen(PS_SOLID, 1, m_fgColor^m_bgColor);
	CPen	*sPen = pDC->SelectObject(cPen);
	CBrush	*cBrush = (CBrush*)((CMainWnd*)m_pMainWnd)->GetBrush(m_fgColor^m_bgColor);
