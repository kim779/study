...
LRESULT CCtrlWnd::OnCtrl(WPARAM wParam, LPARAM lParam)
{
	...
	case 0x97:
	{
		m_nSlider = m_pSlider->GetPos();
	}
	break;
	case 0x98:
	{
		
		if (m_pSlider && m_pSlider->IsWindowEnabled() 	&& m_pSlider->GetRangeMax() >= 20)
		{
			int ipos = HIWORD(wParam);
			m_pSlider->SetPos(ipos);
...
{
	CString m_slog;
	m_slog.Format("\r\n [2009] 여기가 시스템콜백말고 함수 호출로 와야 한다 m_AutoScroll =[%d] nSlider =[%d] nPos=[%d]", m_AutoScroll, m_nSlider, nPos);
