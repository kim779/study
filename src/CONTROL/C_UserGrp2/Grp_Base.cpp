...
//DWORD WINAPI ErrorWritting(void* lpParam)
//{
//	CGrp_Base* pObj = (CGrp_Base*)lpParam;
//	int cnt{};
//
//	while (1)
//	{
//		FileLog(pObj->m_strLog);
//		cnt++;
//		Sleep(2000);
//		if (cnt >= 1)
//			break;
//	}
//	pObj->m_bLogThread = FALSE;
//
//	return 0;
//}
...
void CGrp_Base::DrawDateBottom(CDC *pDC)
{
	...
	for (int ii = 0; ii < dispEnd - dispPos; ii++)
	{
		...
		{
			continue;
		  /*m_strLog.Format("UserGrp] Error code=[%s] dispEnd=[%d] dispPos=[%d] dispDay=[%d] ",  m_pGrpWnd->m_pDataInfo->m_pInfo[1]->RtsCode, dispEnd, dispPos, dispDay);
			if (!m_bLogThread)
			{
				m_bLogThread = TRUE;
				_beginthreadex(0, 0, (_beginthreadex_proc_type)ErrorWritting, (void*)this, 0, 0);
			}*/
		}
