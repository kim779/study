...
CAppROC::CAppROC(class COrgData *pOrgData, int dKey, int gKind, struct _chartinfo *pCInfo, int cOption)
...
bool CAppROC::CalcuData(bool bShift, bool bLast)
{
	if (m_iDataSize <= 0)
		return false;
		...
		if (nCount >= nMim)
		{
			...
			for ( ii = nStart ; ii < nDataSize ; ii++ )
			{
				...
				if (m_iGKind == GK_PROC)				
					...
					pVal = (double)gBasic->m_dVolume;
				
				if (pVal == 0.0)
					m_apdVal[0][ii + nPos] = val;
				else
					m_apdVal[0][ii + nPos] = ((val - pVal) * 10000.0) / pVal;
			}			
