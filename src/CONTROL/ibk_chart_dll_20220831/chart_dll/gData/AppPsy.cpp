...
CAppPsy::CAppPsy(class COrgData *pOrgData, int dKey, int gKind, struct _chartinfo *pCInfo, int cOption)
...
bool CAppPsy::CalcuData(bool bShift, bool bLast)
{
	if (m_iDataSize <= 0)
		return false;
	...
	if (nStart >= 0)
	{
		...
		if (nCount >= nMim)
		{
			...
			for ( ii = nStart ; ii < nDataSize ; ii++ )
			{
				...
				m_apdVal[0][ii + nPos] = ((double)nCnt / (double)nday) * 100.0;
			}			
