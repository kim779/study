...
CAppSona::CAppSona(class COrgData *pOrgData, int dKey, int gKind, struct _chartinfo *pCInfo, int cOption)
...
bool CAppSona::CalcuData(bool bShift, bool bLast)
{
	if (m_iDataSize <= 0)
		return false;
	...
	if (nStart >= 0)
	{
		...
		if (nCount >= nMim)
		{
			...
			for ( ii = nStart ; ii < nDataSize ; ii++ )
			{
				...
				if (pVal != 0)
					CCI = (val - pVal) / pVal * 100.0;
				
				m_apdVal[0][ii + nPos] = CCI * 100.0;
				pVal = val;
