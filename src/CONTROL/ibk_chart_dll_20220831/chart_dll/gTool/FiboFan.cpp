// FiboFan.cpp: implementation of the CFiboFan class.
//
//////////////////////////////////////////////////////////////////////
...
double	fiboFAN[] = {38.2, 50.0, 61.8};
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CFiboFan::CFiboFan(CWnd *pView, CWnd *pParent, CRect toolRC, CRect drawRC, int bgColor, CIndcBase* pIndcBase, int tkname)
...
void CFiboFan::DrawTool(CDC* pDC)
{
...
//		pDC->Polyline(m_BOXfibo, 5);
		...
		for (int ii = 0; ii < maxFIBOFAN-1; ii++)
		{
...
void CFiboFan::HideTool()
{
	CClientDC ClientDC(m_pwndParent);
	DrawTool(&ClientDC);
}

void CFiboFan::CreateToolLBDN(CPoint pt, bool &bMouseCapture)
{
	m_pointStart = pt;
	m_pointEnd = pt;
	m_orgSPoint = pt;
...
void CFiboFan::CreateToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CFiboFan::CreateToolLBUP(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CFiboFan::ChangeToolLBDN(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CFiboFan::ChangeToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CFiboFan::ChangeToolLBUP(CPoint pt, bool &bMouseCapture)
{
	MoveToolLBUP(pt, bMouseCapture);
}

void CFiboFan::MoveToolLBDN(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
	m_pointCur = pt;
...
void CFiboFan::MoveToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CFiboFan::MoveToolLBUP(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
bool CFiboFan::PtInToolRegion(CPoint pt)
{
	if (!m_rectGrp.PtInRect(pt))	return false;

	CPoint	pt1, pt2;
	const	int arrayCOUNT = 7;//maxFIBOFAN * 5 +1;
	int	aIDX = 0;

	SetPointArray(aIDX++, m_pointStart, m_pointEnd);
/*	for (int ii = 0; ii < maxFIBOFAN; ii++)
	{
...
bool CFiboFan::CalculateDrawingPosition()
{
	...
	for (int ii = 0; ii < maxFIBOFAN; ii++)
	{
		...
		case firstAREA:
			...
			m_Yfibo[ii].y = m_pointStart.y - int(Height * fiboFAN[ii] / 100.);
			// 가로축
			m_Xfibo[ii].x = m_pointStart.x + int(Width * fiboFAN[ii] / 100.);
	...
	for (int ii = 0; ii < maxFIBOFAN; ii++)
	{
		...
		case firstAREA:
			...
			else
			{
				...
				m_YfiboEXT[ii].y = m_rectChart.top;
			}
			break;
		case secondAREA:
			if (xChkRATE > baseRATE)
			...
			else
			{
				...
				m_XfiboEXT[ii].x = m_rectChart.left;
				m_XfiboEXT[ii].y = m_pointStart.y - int(Width * xChkRATE);
			...
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
				m_YfiboEXT[ii].x = m_rectChart.left;
				m_YfiboEXT[ii].y = m_pointStart.y - int(Width * yChkRATE);
			}
			else
			{
		...
		case thirdAREA:
			if (xChkRATE > baseRATE)
			{
				...
				m_XfiboEXT[ii].x = m_pointStart.x - int(Height / xChkRATE);
				m_XfiboEXT[ii].y = m_rectChart.bottom;
			}
			else
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
			...
			if (yChkRATE > baseRATE)
			{
				Height = abs(m_pointStart.y - m_rectChart.bottom);
				m_YfiboEXT[ii].x = m_pointStart.x -  int(Height / yChkRATE);
				m_YfiboEXT[ii].y = m_rectChart.bottom;
			}
			else
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
...
void CFiboFan::SetPointArray(int idx, CPoint pt1, CPoint pt2)
{
	if (pt1.x == pt2.x)
	{
