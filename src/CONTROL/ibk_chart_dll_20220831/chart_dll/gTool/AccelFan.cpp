// AccelFan.cpp: implementation of the CAccelFan class.
//
//////////////////////////////////////////////////////////////////////
...
double	accelFAN[] = {33.3, 66.6};

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CAccelFan::CAccelFan(CWnd *pView, CWnd *pParent, CRect toolRC, CRect drawRC, int bgColor, CIndcBase* pIndcBase, int tkname)
...
void CAccelFan::DrawTool(CDC* pDC)
{
...
//		pDC->Polyline(m_BOXaccel, 5);
	
		for (int ii = 0; ii < maxACCELFAN; ii++)
		{
//			pDC->MoveTo(m_Yaccel[ii]);
//			pDC->LineTo(m_pointStart.x, m_Yaccel[ii].y);

//			pDC->MoveTo(m_Xaccel[ii]);
//			pDC->LineTo(m_Xaccel[ii].x, m_pointStart.y);

			pDC->MoveTo(m_pointStart);
			pDC->LineTo(m_XaccelEXT[ii]);

			pDC->MoveTo(m_pointStart);
			pDC->LineTo(m_YaccelEXT[ii]);
		}
	}
	pDC->SetROP2(sROP);
...
void CAccelFan::HideTool()
{
	CClientDC ClientDC(m_pwndParent);
	DrawTool(&ClientDC);
}

void CAccelFan::CreateToolLBDN(CPoint pt, bool &bMouseCapture)
{
	m_pointStart = pt;
	m_pointEnd = pt;
...
void CAccelFan::CreateToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CAccelFan::CreateToolLBUP(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
	...
	m_pIndcBase->GetPtToPosValue(pt, m_dPosEnd, m_dValueEnd);
}

void CAccelFan::ChangeToolLBDN(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CAccelFan::ChangeToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CAccelFan::ChangeToolLBUP(CPoint pt, bool &bMouseCapture)
{
...
void CAccelFan::MoveToolLBDN(CPoint pt, bool &bMouseCapture)
{
	m_pointCur = pt;
...
void CAccelFan::MoveToolLBMV(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
void CAccelFan::MoveToolLBUP(CPoint pt, bool &bMouseCapture)
{
	CClientDC ClientDC(m_pwndParent);
...
bool CAccelFan::PtInToolRegion(CPoint pt)
{
...
/*	for (int ii = 0; ii < 4; ii++)
	{
		SetPointArray(aIDX, m_BOXaccel[ii], m_BOXaccel[ii+1]);
		aIDX++;
	}
*/
	for (int ii = 0; ii < maxACCELFAN; ii++)
...
/*		pt1 = m_Yaccel[ii];
		...
		aIDX++;
*/
		SetPointArray(aIDX, m_pointStart, m_YaccelEXT[ii]);
		aIDX++;
		SetPointArray(aIDX, m_pointStart, m_XaccelEXT[ii]);
		aIDX++;
	}
	...
	CRgn AccelRGN;
	if (!AccelRGN.CreatePolyPolygonRgn(&m_RgnPoint[0][0], count, arrayCOUNT, WINDING))
		return false;

	BOOL bInRgn = AccelRGN.PtInRegion(pt); 		
	AccelRGN.DeleteObject();	
...
bool CAccelFan::CalculateDrawingPosition()
{
	...
	m_BOXaccel[0].x = m_pointStart.x; m_BOXaccel[0].y = m_pointStart.y;
	m_BOXaccel[1].x = m_pointEnd.x;   m_BOXaccel[1].y = m_pointStart.y;
	m_BOXaccel[2].x = m_pointEnd.x;   m_BOXaccel[2].y = m_pointEnd.y;
	m_BOXaccel[3].x = m_pointStart.x; m_BOXaccel[3].y = m_pointEnd.y;
	m_BOXaccel[4].x = m_pointStart.x; m_BOXaccel[4].y = m_pointStart.y;
	...
	for (int ii = 0; ii < maxACCELFAN; ii++)
	{
		...
		case firstAREA:
			// 세로축
			m_Yaccel[ii].x = m_pointEnd.x;
			m_Yaccel[ii].y = m_pointStart.y - int(Height * accelFAN[ii] / 100.);
			// 가로축
			m_Xaccel[ii].x = m_pointStart.x + int(Width * accelFAN[ii] / 100.);
			m_Xaccel[ii].y = m_pointEnd.y;
			break;
		case secondAREA:
			// 세로축
			m_Yaccel[ii].x = m_pointEnd.x;
	...
	for (int ii = 0; ii < maxACCELFAN; ii++)
	{
		Width  = abs(m_pointStart.x - m_Xaccel[ii].x);
		Height = abs(m_pointStart.y - m_Xaccel[ii].y);
		if (Width < 1)	Width = 1;
		xChkRATE = double(Height) / double(Width);
		if (xChkRATE == 0)	xChkRATE = 1;

		Width  = abs(m_pointStart.x - m_Yaccel[ii].x);
		Height = abs(m_pointStart.y - m_Yaccel[ii].y);
		if (Width < 1)	Width = 1;
		...
		case firstAREA:
			if (xChkRATE > baseRATE)
			{
				...
				m_XaccelEXT[ii].x = m_pointStart.x + int(Height / xChkRATE);
				m_XaccelEXT[ii].y = m_rectChart.top;
			}
			else
			{
				Width = abs(m_pointStart.x - m_rectChart.right);
				m_XaccelEXT[ii].x = m_rectChart.right;
				m_XaccelEXT[ii].y = m_pointStart.y - int(Width * xChkRATE);
			}
			if (yChkRATE <= baseRATE)
			{
				Width = abs(m_pointStart.x - m_rectChart.right);
				m_YaccelEXT[ii].x = m_rectChart.right;
				m_YaccelEXT[ii].y = m_pointStart.y - int(Width * yChkRATE);
			}
			else
			{
				Height = abs(m_pointStart.y - m_rectChart.top);
				m_YaccelEXT[ii].x = m_pointStart.x + int(Height / yChkRATE);
				m_YaccelEXT[ii].y = m_rectChart.top;
			}
			break;
		case secondAREA:
			if (xChkRATE > baseRATE)
			...
			else
			{
				...
				m_XaccelEXT[ii].x = m_rectChart.left;
				m_XaccelEXT[ii].y = m_pointStart.y - int(Width * xChkRATE);
			...
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
				m_YaccelEXT[ii].x = m_rectChart.left;
				m_YaccelEXT[ii].y = m_pointStart.y - int(Width * yChkRATE);
			}
			else
			{
		...
		case thirdAREA:
			if (xChkRATE > baseRATE)
			{
				...
				m_XaccelEXT[ii].x = m_pointStart.x - int(Height / xChkRATE);
				m_XaccelEXT[ii].y = m_rectChart.bottom;
			}
			else
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
			...
			if (yChkRATE > baseRATE)
			{
				Height = abs(m_pointStart.y - m_rectChart.bottom);
				m_YaccelEXT[ii].x = m_pointStart.x -  int(Height / yChkRATE);
				m_YaccelEXT[ii].y = m_rectChart.bottom;
			}
			else
			{
				Width = abs(m_pointStart.x - m_rectChart.left);
...
void CAccelFan::SetPointArray(int idx, CPoint pt1, CPoint pt2)
{
	if (pt1.x == pt2.x)
	{
