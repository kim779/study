...
CBPivot::CBPivot(CWnd *pView, CWnd *pParent, class CDataMgr *pDataMgr, char *pInfo, char *graph, int dispDay)
...
void CBPivot::DrawGraph(CDC* pDC)
{
	m_RgnInfo.iCount = 0;
...
void CBPivot::drawChart(CDC* pDC)
{
	class CAppData *pAppData;
	if (!m_pDataMgr->GetData(m_iDataKey, pAppData))
		return;
	...
	for ( int ii = 0 ; ii < nCount ; ii++)
	{
		...
		if (ii == 0)
			dispText.Format("Pivot ");
		else if (ii == 1)
			dispText.Format("1차저항 ");
		else if (ii == 2)
			dispText.Format("1차지지 ");
		else if (ii == 3)
			dispText.Format("2차저항 ");
		else
			dispText.Format("2차지지");
...
CString CBPivot::GetDisplayPosData(CPoint pt, bool bGrid)
{
	...
	double	val = 0.0;
	char*	title[] = {"Pivot", "1차저항", "1차지지", "2차저항", "2차지지"};
...
CString CBPivot::ExcelData(int idx)
{
	CString	dispStr = _T("");
	CString	tmpS = _T("");
	char*	title[] = {"Pivot", "1차저항", "1차지지", "2차저항", "2차지지"};	
