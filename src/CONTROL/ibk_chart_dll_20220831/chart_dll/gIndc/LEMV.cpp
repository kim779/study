...
CLEMV::CLEMV(CWnd *pView, CWnd *pParent, class CDataMgr *pDataMgr, char *pInfo, char *graph, int dispDay)
...
CString CLEMV::MakeLegend(int nIndex)
{
	CString	ret;

	if (nIndex == 0)
		ret.Format("EOM[%d]", (int)m_pgraph->cInfo.awValue[0]);
	else if (nIndex == 1)
...
void CLEMV::MakeTitle(CStringArray& arTitle)
{
	arTitle.RemoveAll();

	CString	tmpstr;

	tmpstr.Format("EOM[%d]", (int)m_pgraph->cInfo.awValue[0]);
	arTitle.Add(tmpstr);
	tmpstr.Format("Signal[%d]", (int)m_pgraph->cInfo.awValue[1]);
	arTitle.Add(tmpstr);
}

void CLEMV::FormatVariableComma(CString &dispTxt, double dVal, double Max, int maxDigit)
{
	if (GetDataUnit() == GU_CODE || GetDataUnit() == GU_COFUT || GetDataUnit() == GU_COOPT)
	{
		CLineBase::FormatVariableComma(dispTxt, dVal, Max, maxDigit);
		return;
	}
	CString	edit1, edit2;
...
void CLEMV::DrawTickLeft(CDC* pDC, double Max, double Min)
{
	if (GetDataUnit() == GU_CODE || GetDataUnit() == GU_COFUT || GetDataUnit() == GU_COOPT)
	{
		CLineBase::DrawTickLeft(pDC, Max, Min);
		return;
	}
...
void CLEMV::DrawTickRight(CDC* pDC, double Max, double Min)
{
	if (GetDataUnit() == GU_CODE || GetDataUnit() == GU_COFUT || GetDataUnit() == GU_COOPT)
	{
		CLineBase::DrawTickRight(pDC, Max, Min);
		return;
	}
	...
	for ( int ii = 0; ii < dispCount; ii++ )		// 가격별가로 (DOT)
	{
		...
		if (ii == 0)
		{
			...
			{
				size = pDC->GetOutputTextExtent(dispStr);
				textRC.SetRect(m_rectOrgGrp.right + lenTICK, tickRC.bottom - size.cy,
						tickRC.right, tickRC.bottom);
				textRC.IntersectRect(&textRC, &orgTickRC);
...
CString CLEMV::GetDisplayPosData(CPoint pt, bool bGrid)
{
	if (GetDataUnit() == GU_CODE || GetDataUnit() == GU_COFUT || GetDataUnit() == GU_COOPT)
	{		
		return CLineBase::GetDisplayPosData(pt, bGrid);
	}
...
CString CLEMV::ExcelData(int idx)
{
	if (GetDataUnit() == GU_CODE || GetDataUnit() == GU_COFUT || GetDataUnit() == GU_COOPT)
	{
		return CLineBase::ExcelData(idx);
	}
