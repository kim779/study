...
CBDemark::CBDemark(CWnd *pView, CWnd *pParent, class CDataMgr *pDataMgr, char *pInfo, char *graph, int dispDay)
...
void CBDemark::DrawGraph(CDC* pDC)
{
	m_RgnInfo.iCount = 0;
...
void CBDemark::drawChart(CDC* pDC)
{
	class CAppData *pAppData;
	if (!m_pDataMgr->GetData(m_iDataKey, pAppData))
		return;
...
CString CBDemark::GetDisplayPosData(CPoint pt, bool bGrid)
{
	...
	double	val = 0.0;
	char*	title[] = {"Demark 고가", "Demark 저가"};
...
CString CBDemark::ExcelData(int idx)
{
	CString	dispStr = _T("");
	CString	tmpS = _T("");
	char*	title[] = {"Demark 고가", "Demark 저가"};	
