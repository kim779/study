...
CBBollinger::CBBollinger(CWnd *pView, CWnd *pParent, class CDataMgr *pDataMgr, char *pInfo, char *graph, int dispDay)
...
CBBollinger::~CBBollinger()
{
	if (m_iDataKey > 0)
		m_pDataMgr->DetachData(m_iDataKey);
}

void CBBollinger::DrawGraph(CDC* pDC)
{
	m_RgnInfo.iCount = 0;
...
void CBBollinger::drawChart(CDC* pDC)
{
	class CAppData *pAppData;
	if (!m_pDataMgr->GetData(m_iDataKey, pAppData))
		return;
	...
	for ( int ii = 0 ; ii < nCount ; ii++)
	{
		...
		if (ii == 0)
			dispText.Format("Bollinger[기간:%d,승수:%d] 상한선 ", (int)m_pgraph->cInfo.awValue[0], (int)m_pgraph->cInfo.awValue[1]);			
		else if (ii == 1)
			dispText.Format("중심선 ");
		else
			dispText.Format("하한선");			
...
CString CBBollinger::GetDisplayPosData(CPoint pt, bool bGrid)
{
	...
	double	val = 0.0;
	char*	title[] = {"Bollinger상한", "Bollinger중심", "Bollinger하한"};
...
CString CBBollinger::ExcelData(int idx)
{
	CString	dispStr = _T("");
	CString	tmpS = _T("");
	char*	title[] = {"Bollinger상한", "Bollinger중심", "Bollinger하한"};
