...
void CAxMShade::DrawShade(CDC* dc, CRect rect, UINT shadeID, BYTE granularity, 
						  BYTE highlight, BYTE coloring, COLORREF color, BOOL down, BOOL over)
{
	...
	m_h.Create(max(1,sXSize /*-2*m_FocusRectMargin-1*/ ),1,8);	//create the horizontal focus bitmap
	m_v.Create(1,max(1,sYSize /*-2*m_FocusRectMargin*/),8);	//create the vertical focus bitmap

	m_normal.Create(sXSize,sYSize,8);					//create the default bitmap
	...
	for(i=0;i<129;i++){
		...
		m_normal.SetPaletteIndex((BYTE)i,(BYTE)r,(BYTE)g,(BYTE)b);
		m_h.SetPaletteIndex((BYTE)i,(BYTE)r,(BYTE)g,(BYTE)b);
		m_v.SetPaletteIndex((BYTE)i,(BYTE)r,(BYTE)g,(BYTE)b);
	}
	for(i=1;i<129;i++){
		...
		m_normal.SetPaletteIndex((BYTE)(i+127),(BYTE)r,(BYTE)g,(BYTE)b);
		m_h.SetPaletteIndex((BYTE)(i+127),(BYTE)r,(BYTE)g,(BYTE)b);
		m_v.SetPaletteIndex((BYTE)(i+127),(BYTE)r,(BYTE)g,(BYTE)b);
	}

	m_normal.BlendPalette(color,coloring);	//color the palette

	iDst=m_h.GetBits();		//build the horiz. dotted focus bitmap
	j=(long)m_h.GetWidth();
	for(i=0;i<j;i++){
//		iDst[i]=64+127*(i%2);	//soft
	...
	iDst=m_v.GetBits();		//build the vert. dotted focus bitmap
	j=(long)m_v.GetHeight();
	for(i=0;i<j;i++){
//		*iDst=64+127*(i%2);		//soft
	...
	bytes = m_normal.GetLineWidth();
	iDst = m_normal.GetBits();
	posDst =iDst;
...
//----------------------------------------------------
	case 8:	//SHS_METAL
		...
		for(a=0;a<200;a++){
			...
			for(i=0;i<xs;i++){
				if (((x-i)>0)&&((y+i)<sYSize))
					m_normal.SetPixelIndex(x-i,y+i,(BYTE)d);
				if (((x+i)<sXSize)&&((y-i)>0))
					m_normal.SetPixelIndex(sXSize-x+i,y-i,(BYTE)d);
			}
		}
...
//----------------------------------------------------
///**
	m_disabled.Clone(&m_normal);	//build the other bitmaps
	m_over.Clone(&m_normal);
	m_over.BlendPalette(hicr,highlight);
	m_down.Clone(&m_over);
//**/
	...
	{
		if (down)
			m_over.Draw(dc->GetSafeHdc(),1,1);
		else
			m_over.Draw(dc->GetSafeHdc(),0,0);
	}
	else
		m_normal.Draw(dc->GetSafeHdc(),0,0);
} 
