	...
	: CDialog(CTargetItemDlg::IDD, pParent)
{
	...
	m_bar = (CAxMConfigBar*)pParent;
	m_selectKind.Empty();

	//** ZeroMemory(&m_lastKind, sizeof(KIND));
	CAxTargetItem::KeyToKind(DEF_TARGETGROUP, m_key);
}
...
BEGIN_MESSAGE_MAP(CTargetItemDlg, CDialog)
	...
	ON_WM_DESTROY()
	ON_NOTIFY(NM_DBLCLK, IDC_LIST1, OnDblclkListSource)
	ON_NOTIFY(NM_DBLCLK, IDC_LIST2, OnDblclkListSelect)
	//}}AFX_MSG_MAP
...
BOOL CTargetItemDlg::OnInitDialog() 
{
	...
	InitControls();	
	m_items = new CAxTargetItem(m_bar->GetTargetItemFileName());
...
{
	m_bar->ResizeListColumn(&m_listSource);
	m_bar->ResizeListColumn(&m_listSelect);
}

void CTargetItemDlg::AddItem()
{
	//**ExchangeItem(m_listSource, m_listSelect, TRUE);
	AddItem(m_listSource, m_listSelect, TRUE);
}

void CTargetItemDlg::AddAllItem()
{
	//**ExchangeItem(m_listSource, m_listSelect, FALSE);
	AddItem(m_listSource, m_listSelect, FALSE);
}
...
void CTargetItemDlg::ExchangeItem(CAxMListCtrl& src, CAxMListCtrl& dest, BOOL selectOnly)
{
	...
	for (int i = 0; i < src.GetItemCount(); i++) {
		if (!selectOnly || src.GetItemState(i, LVIS_SELECTED) & LVIS_SELECTED) {
			...
			dest.SetItemText(dest.InsertItem(dest.GetItemCount(), item), 1, src.GetItemText(i, 1));
		}
	}
	if (!selectOnly) {
		src.DeleteAllItems();
...
void CTargetItemDlg::AddItem(CAxMListCtrl& src, CAxMListCtrl& dest, BOOL selectOnly)
{
	if (!src.GetItemCount()) return;
	if (selectOnly && !src.GetSelectedCount()) return;
...
void CTargetItemDlg::RemoveItem(CAxMListCtrl& list, BOOL selectOnly)
{
	if (!list.GetItemCount()) return;
	if (selectOnly && !list.GetSelectedCount()) return;
	...
	if (!selectOnly) {
		...
		for (int i = list.GetItemCount() - 1; i >= 0; i--) {
			if (!selectOnly || list.GetItemState(i, LVIS_SELECTED) & LVIS_SELECTED) {
				list.DeleteItem(i);
			}
		}
	}
...
void CTargetItemDlg::MoveListItem(CAxMListCtrl& list, int distance)
{
	...
	for (int i = 0; i < list.GetItemCount(); i++) {
		if (list.GetItemState(i, LVIS_SELECTED) & LVIS_SELECTED) {
			...
			name = list.GetItemText(i, 1);
			list.DeleteItem(i);
			list.SetItemText(list.InsertItem(i + distance, item), 1, name);
...
void CTargetItemDlg::Modified()
{
	...
	m_items->SetItem(m_key, m_listSelect);
}
...
void CTargetItemDlg::DisplaySelectedItem()
{
	m_listSelect.DeleteAllItems();

	int find;
	CString code, name, items = m_items->GetItem(m_key);
	...
	while (TRUE) {
		...
		if (!name.IsEmpty()) {
			name.TrimRight();
			m_listSelect.SetItemText(m_listSelect.InsertItem(m_listSelect.GetItemCount(), code), 1, name);
		}
	}
}
...
void CTargetItemDlg::OnDblclkListSource(NMHDR* pNMHDR, LRESULT* pResult) 
{
	AddItem(m_listSource, m_listSelect, TRUE);
...
void CTargetItemDlg::OnDblclkListSelect(NMHDR* pNMHDR, LRESULT* pResult) 
{
	RemoveItem(m_listSelect, TRUE);
