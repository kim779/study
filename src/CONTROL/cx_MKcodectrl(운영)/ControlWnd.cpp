...
CControlWnd::CControlWnd(CWnd* pWnd) : CAxWnd(pWnd)
{
	...
	m_bTimer = true;
	m_bMarketpicker = true; //moid 우선 테스트
	m_pMarketPicker = std::make_unique< CMaketPickerWnd>();
}
...
BEGIN_DISPATCH_MAP(CControlWnd, CAxWnd)
	...
	DISP_PROPERTY_EX(CControlWnd, "sMKtype", GetsMKtype, SetsMKtype, VT_BSTR)
	...
	//}}AFX_DISPATCH_MAP
	DISP_FUNCTION_ID(CControlWnd, "WriteLog", dispidWriteLog, WriteLog, VT_EMPTY, VTS_I2 VTS_BSTR)
END_DISPATCH_MAP()
...
CString CControlWnd::GetOnMsgData(WPARAM wParam, LPARAM lParam)
{
	switch (LOBYTE(LOWORD(wParam)))
	{
	case DLL_INB: return "DLL_INB";
	case DLL_OUBx: return "DLL_OUBx";
	case DLL_OUB: return "DLL_OUB";
	case DLL_ALERT:  return "DLL_ALERT";
	case DLL_TRIGGER:  return "DLL_TRIGGER";
	case DLL_DOMINO:  return "DLL_DOMINO";
	case DLL_NOTICE: return "DLL_NOTICE";
	case DLL_SETPAL:  return "DLL_SETPAL";
	case DLL_SETFONT:  return "DLL_SETFONT";
	case formDLL: return "formDLL";
...
long CControlWnd::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	CString tmp;
	if (LOBYTE(LOWORD(wParam)) == DLL_OUBx)
	{
		struct	_extTHx* exth;
	...
	m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d> m_sid=[%s] OnMessage [%0x][%s]  length=[%d]   ",
		__FUNCTION__, __LINE__, m_sid, LOBYTE(LOWORD(wParam)), GetOnMsgData(wParam, lParam), length);
	...
	case DLL_INB:
		...
		return (long)m_rtnStr.operator LPCTSTR();  //ControlWnd::onMessage DLL_INB
	...
	case DLL_OUBx:
		...
		if(length < 5)   //modi NXT   시장구분 피커 클릭시 트리거 연동 메시지 오는부분
		{
			CString str = CString(exth->data);
			str.TrimRight();

m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d>  [length < 5]  m_sid=[%s] length=[%d]val=[%s] ",
	__FUNCTION__, __LINE__, m_sid, length, str);
OutputDebugString(m_slog);


			if (m_pMarketPicker && m_bMkTrigger)
			{
				m_pMarketPicker->SetMarketType(str);
				m_sMarketType = m_pMarketPicker->m_strMarket;
			}
		}
...
m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d>  [length < 13 && length > 2]  m_sid=[%s] length=[%d]val=[%s] ",
	__FUNCTION__, __LINE__, m_sid, length, str);
OutputDebugString(m_slog);
			...
			if (length == 8)
			{
				if (str.Left(2) == "T.")
				{
					...
					if (m_pMarketPicker)
					{
						m_pMarketPicker->SetMarketType("통합");
						m_pMarketPicker->Invalidate();
						m_sMarketType = "통합";
					}
				}
				else if (str.Left(2) == "N.")
				{
					str = str.Mid(2);
					length = str.GetLength();
					if (m_pMarketPicker)
					{
						m_pMarketPicker->SetMarketType("NXT");
						m_pMarketPicker->Invalidate();
						m_sMarketType = "NXT";
					}
				}
			...
			else if (length == 6)
			{
				if (m_pMarketPicker)
				{
					m_pMarketPicker->SetMarketType("KRX");
					m_pMarketPicker->Invalidate();
					m_sMarketType = "KRX";
				}
			}
...
void CControlWnd::SetProperties(LPCTSTR sParam) 
{
	...
	for (int ii = 0; ii < keys.GetLength(); ii++)
	{
		...
		case 'x':  //modi nxt 이미지 크기 타입
			...
			break;
		case 'y':   //modi nxt 화면번호 
			m_sid = text;
			break;
		case 'z':   //modi nxt 거래소 변동 트리거 수신 여부
			m_bMkTrigger = (bool)atoi(text);
			break;
		}
	}
...
//	m_pCode->SetEditData(Parser(str, " "));
	...
	if (m_bMarketpicker)
	{
		m_pMarketPicker->Create(NULL, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN, CRect(0, 0, 0, 0), this, 0x99);
		if (m_pMarketPicker)
		{
			m_pMarketPicker->m_sRoot = m_sRoot;
			m_pMarketPicker->m_pWizard = m_pWizard;
			m_pMarketPicker->m_sizetype = m_sizetype;
			m_pMarketPicker->Init();
			m_sMarketType = m_pMarketPicker->m_strMarket;
...
bool CControlWnd::Resize()
{
	...
	if (m_nGubn == GU_CODE)
	{
		...
		//width = (m_bCate ? BTN + gap: 0) + (m_bInter ? BTN + gap: 0);    //modi m_bMarketpicker
		width = (m_bCate ? BTN + gap : 0) + (m_bInter ? BTN + gap : 0) + (m_bMarketpicker ?(BTN * m_sizetype) + gap : 0);
		nSymbolWidth = rc.Width() - width - CODEWIDTH;

//		m_pCode->SetWindowPos(NULL, 0, 0, rc.Width()/*CODEWIDTH*/ , rc.Height(), SWP_SHOWWINDOW);
		m_pCode->SetWindowPos(NULL, m_bMarketpicker ? (BTN * m_sizetype) + gap : 0,
																0, 
			                                                     rc.Width() - (m_bMarketpicker ? (BTN * m_sizetype) + gap : 0),
		...
		if (m_bMarketpicker)  //modi NXT
		{
			CRect codeRec{}, CateRec{}, interRec{};
			m_pCode->GetClientRect(codeRec);
			if(m_bCate)
				m_pCategory->GetClientRect(CateRec);
			if (m_bInter)
				m_pBtnInter->GetClientRect(interRec);

			int iwidth = rc.Width() - codeRec.Width() - CateRec.Width() - interRec.Width();
			m_pMarketPicker->SetWindowPos(NULL, 0, 0, iwidth, (int)(BTN * rHeight), SWP_SHOWWINDOW);
		}
...
{
	CString strResult = GetHNam(m_rtnStr);  //맵에서코드컨트롤 내부 종목코드의 이름을 반환받기위해
	return strResult.AllocSysString();
}
...
void CControlWnd::SendEventDLL(int itype)
{
	CString str;
	str.Format("%s\t%s\r%d", m_Param.name, m_sMarketType, m_Param.key); //맵에서 코드컨트롤 내용으로 트리거를 내부로

m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d> m_sid=[%s]  m_sMarketType=[%s]  ",
	__FUNCTION__, __LINE__, m_sid, m_sMarketType);
OutputDebugString(m_slog);

	//Variant(triggerCC, str);
	m_pWizard->SendMessage(WM_USER, MAKEWPARAM(eventDLL, MAKEWORD(m_Param.key, itype)), (LPARAM)m_Param.name.GetString());
}
...
void CControlWnd::Trigger(LPCTSTR name, BOOL all) 
{
	...
	CString sMkTriggerCode{};
	if (m_sMarketType == "통합")
		sMkTriggerCode = "T." + m_rtnStr;
	else if (m_sMarketType == "NXT")
		sMkTriggerCode = "N." + m_rtnStr;
	else
		sMkTriggerCode = m_rtnStr;
	
	if (all == TRUE)	str.Format("%s\t%s", CString(name), sMkTriggerCode);	   //맵에서 코드컨트롤 내용으로 트리거를 외부로
	else			str.Format("%s\t%s\r%d", CString(name), sMkTriggerCode, m_Param.key); //맵에서 코드컨트롤 내용으로 트리거를 내부로

	m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d> m_sid=[[%s]]  m_sMarketType=[%s]  sMkTriggerCode=[%s]",
		__FUNCTION__, __LINE__, m_sid, m_sMarketType, sMkTriggerCode);
	OutputDebugString(m_slog);
...
{
	return m_rtnStr.AllocSysString();  //맵에서 컨트롤.Data 로 컨트롤의 데이터를 반환받을때
}

BSTR CControlWnd::GetsMKtype()
{
	return m_sMarketType.AllocSysString();
}

void CControlWnd::SetsMKtype(LPCTSTR code)
{
	m_sMarketType.Format("%s", code);
	m_sMarketType.Trim();
	if (m_pMarketPicker)
	{
		m_pMarketPicker->SetMarketType(m_sMarketType);
		m_pMarketPicker->Invalidate();
	}
}
...
{
	if (!m_pCode || m_rtnStr.CompareNoCase(code) == 0)  //맵에서 컨트롤의 값을 SetData로 세팅
		return;
...
{
	if (!m_pCode || m_rtnStr.CompareNoCase(code) == 0)  //맵에서 컨트롤의 값을 SetCode로 세팅
		return;
	Msg("SetCode(%s)", code);
	m_bModify = true;
...
void CControlWnd::Clear() 
{
	...
	m_rtnStr = _T("");  //맵에서 clear를 호출해서 컨트롤 에디트를 지우고 값도 초기화
}
...
void CControlWnd::WriteLog(SHORT bAddFile, LPCTSTR lpszNewValue)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	m_slog.Format("[TRIGGER][cx_mkcodectrl][%s]<%d> --CTRL LOG --- m_sid=[[%s]]                   log=[%s]   ",
		__FUNCTION__, __LINE__, m_sid, lpszNewValue);
	OutputDebugString(m_slog);
