...
// Attributes
public:  
	  CString		m_sData, m_sUserID, m_sRoot, m_sMarketType{};
	...
	enum {
		dispidWriteLog = 20L,
		TYPE_SMALL = 1, TYPE_NORMAL };
	int	m_sizetype;  //1 small type(18X20) ,  2 normal type(36X20)
	
	bool	m_bMkTrigger{};  //거래소 변동 트리거
protected:
...
protected:
	...
	afx_msg BSTR GetsMKtype();
	afx_msg void SetsMKtype(LPCTSTR lpszNewValue);
	//}}AFX_DISPATCH
...
private:
	...
	CString m_slog{};
	CString GetOnMsgData(WPARAM wParam, LPARAM lParam);
protected:
	void WriteLog(SHORT bAddFile, LPCTSTR lpszNewValue);
};