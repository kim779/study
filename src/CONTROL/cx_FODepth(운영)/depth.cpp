...
void Cdepth::drawBong(CDC* dc)
{
	...
	if (rc.top + rc.Height() < rc.top + sigaY)
	{
		tmpS.Format("\r\n [cx_depth] 양봉 리턴 rc.top = [%d] rc height =[%d] sigaY =[%d]", rc.top, rc.Height(), sigaY);
		OutputDebugString(tmpS);
		return;
	}

	if (rc.top + rc.Height() < rc.top + jjgaY)
	{
		tmpS.Format("\r\n [cx_depth] 음봉 리턴 rc.top = [%d] rc height =[%d] jjgaY =[%d]", rc.top, rc.Height(), jjgaY);
		OutputDebugString(tmpS);
		return;
	}
...
void Cdepth::calculatePercent()
{
	...
	{
		if (m_code.GetAt(0) == '4' || m_code.GetAt(0) == 'D')	// 20230125 파생상품 코드 개편	'4', 'D' : 스프레드
			clos = m_clos;
...
{
	string.Remove(',');
	if (string.Left(2) == "0+" || string.Left(2) == "0-" || string.Left(2) == "00" ||
		string.Left(2) == "++" || string.Left(2) == "+-" || string.Left(2) == "+0" ||
		string.Left(2) == "--" || string.Left(2) == "-+" || string.Left(2) == "-0")
		string = string.Mid(1);
