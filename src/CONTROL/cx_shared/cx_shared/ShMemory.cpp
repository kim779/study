...
{
	CString slog;
	slog.Format("\r\n[cx_shared.dll][%s]<%d> cnt=[%d]\r\n", __FUNCTION__, __LINE__, (long)*m_pKeyView);
...
//		TRACE("\tSendTrigger - %x\n", _hwnd);
	...
	for (int ii = 0/*0:Master*/; ii < GetHandleCount(); ii++)
	{
...
//slog.Format("\r\n[cx_shared.dll][NXT_TIME][%s]<%d> Trigger=[%s]\r\n", __FUNCTION__, __LINE__, Trigger);
//OutputDebugString(slog);
