...
BEGIN_MESSAGE_MAP(CMainWnd, CWnd)
	...
	ON_WM_RBUTTONDOWN()
	ON_WM_MBUTTONDOWN()
	ON_MESSAGE(WM_APP + 1, OnAppWriteLog)
END_MESSAGE_MAP()
...
long CMainWnd::OnAppWriteLog(WPARAM wParam, LPARAM lParam)
{
	m_slog.Format("				[cxMarketPicker][MARKETPICKER][%s]<%d>[%s] m_Param.key=[%d]   m_Param.options=[%s] m_strMarket=[%s] ", __FUNCTION__, __LINE__,
		m_sCtrlName, m_Param.key, m_Param.options, m_strMarket);
	OutputDebugString(m_slog);
	return 0;
}
...
void CMainWnd::Init()
{
	m_mapPermission.emplace("IB303000", 3);
	m_mapPermission.emplace("IB100800", 3);
	m_mapPermission.emplace("IB101900", 3);
	m_mapPermission.emplace("IB105000", 1);
	m_mapPermission.emplace("IB500000", 1);
	m_mapPermission.emplace("IB610100", 1);
	m_mapPermission.emplace("IB680100", 1);


	m_mapMapInfo.emplace("IB711100", "1");
	m_mapMapInfo.emplace("IB711200", "1");
	m_mapMapInfo.emplace("IB711700", "1");
	m_mapMapInfo.emplace("IB711800", "1");
	m_mapMapInfo.emplace("IB711900", "1");
	m_mapMapInfo.emplace("IB712500", "1");
	m_mapMapInfo.emplace("IB712600", "1");
	m_mapMapInfo.emplace("IB714100", "1");
...
//	if (m_strMarket.IsEmpty())
	{
		CString sfile;
		char readB[32]{};
		sfile.Format("%s\\%s\\%s\\%s", m_sRoot, "user", m_sName, "userconf.ini");
	...
	char readB[1024];
	CString userD;
	userD.Format("%s\\%s\\%s\\%s.ini", m_sRoot, "user", m_sName, m_sName);
	...
	//	m_pMainWnd = GetTopLevelParent();

	if (shareddll.IsLoaded())
	{
		if (!shareddll.Function<const bool WINAPI(int, CWnd**)>("axGetData")(1, &m_pMainWnd))
		{
			m_pMainWnd = GetTopLevelParent();

			m_slog.Format("				[cxMarketPicker][mainWnd]  GetTopLevelParent m_pMainWnd=[%x]", m_pMainWnd);
			OutputDebugString(m_slog);
		}
		else
		{
			m_slog.Format("				[cxMarketPicker][mainWnd]  !!!!not GetTopLevelParent[%x] shared!!! m_pMainWnd=[%x] ", GetTopLevelParent(), m_pMainWnd );
			OutputDebugString(m_slog);
		}
	}
	//m_pMainWnd = CWnd::FromHandle(hWnd);
...
void CMainWnd::ReadMarketFile()
{
	...
	while (file.ReadString(line))
	{
		...
		if (pos > 0)
		{
			...
			if (!key.IsEmpty())
				m_mapPermission.emplace(key, atoi(value));
...
void CMainWnd::SetsCode(BSTR newVal)
{
	...
	{
		auto it = m_mapPermission.find(m_sID);    //SetsCode Nxt 가능
		if (it != m_mapPermission.end())
			m_iPermission = it->second;
		else
			m_iPermission = 7;
	}
	else
	{//트리거등으로 입력된 종목코드가 NXT 불가 종목인경우 
		...
		auto it = m_mapPermission.find(m_sID);  //SetsCode Nxt 불가능
		if (it != m_mapPermission.end())
		{
			m_iPermission = it->second;
			m_iPermission &= ~2;
	...
	CString slog;
	slog.Format("				[cxMarketPicker][MARKET]<SetsCode>[%s]  m_sID=[%s] m_markettype=[%s] m_iPermission=[%d] m_major=[%d] m_sCode=[%s] m_minor = [%d] newVal =[%s]", 
		m_sCtrlName,m_sID, MarketToString(m_markettype), m_iPermission, m_major, m_sCode, m_minor, newVal);
...
void CMainWnd::SetsMarket(BSTR newVal)
{
	...
	if (m_bAutoMarketChange)  //test
	//if (0)
	{
		if (IsNXTCode(m_sCode) || m_sCode.IsEmpty())
		{
			auto it = m_mapPermission.find(m_sID);  //SetsMarkset  Nxt 가능
			if (it != m_mapPermission.end())
				m_iPermission = it->second;
			else
				m_iPermission = 7;
		...
		{//트리거등으로 입력된 종목코드가 NXT 불가 종목인경우 
			...
			auto it = m_mapPermission.find(m_sID);   //SetsMarkset Nxt 불가능
			if (it != m_mapPermission.end())
			{
				m_iPermission = it->second;
				m_iPermission &= ~2;
	...
	slog.Format("				[cxMarketPicker][MARKET]<SetsMarket>[%s] m_sID=[%s] m_markettype=[%s] m_iPermission=[%d] m_major=[%d] m_sCode=[%s] m_minor = [%d] newVal=[%s]",
		m_sCtrlName, m_sID, MarketToString(m_markettype), m_iPermission, m_major, m_sCode, m_minor, newVal);
	OutputDebugString(slog);
...
void CMainWnd::SaveMapInfo()
{
	char	buf[64]{};
	CString file;
	file.Format("%s\\user\\%s\\userconf.ini", m_sRoot, m_sName);

	CString sVal;
	auto& it = m_mapMapInfo.find(m_sID.GetString());
	if (it != m_mapMapInfo.end())
	{
		sVal = it->second;
		WritePrivateProfileString(m_sID, "MKgubn", m_strMarket, file);

	/*	m_slog.Format("				[cxMarketPicker][MARKETPICKER][%s]<%d>[%s] m_sID=[%d]   m_strMarket=[%s] ", __FUNCTION__, __LINE__,
			m_sCtrlName, m_sID, m_strMarket);
		OutputDebugString(m_slog);*/
	}

}

void CMainWnd::LoadMapInfo()
{
	char	buf[64]{};
	CString file;
	file.Format("%s\\user\\%s\\userconf.ini", m_sRoot, m_sName);
	...
	if (it != m_mapMapInfo.end())
	{
		int readL = GetPrivateProfileString(m_sID, "MKgubn", "", buf, sizeof(buf), file);
		if (readL > 0)
		{
			sVal.Format("%s", buf);
			sVal.TrimRight();
...
void CMainWnd::OnTimer(UINT_PTR nIDEvent)
{
	...
	switch (nIDEvent)
	{
		...
		case TM_SENDMKMSG_TOMAIN:
		{
			...
			if ((IsNXTCode(m_sCode) || m_sCode.IsEmpty()) && m_bAutoMarketChange)
			{
				auto it = m_mapPermission.find(m_sID);  //OnTimer Nxt 가능
				if (it != m_mapPermission.end())
					m_iPermission = it->second;
				else
					m_iPermission = 7;
			...
			{//트리거등으로 입력된 종목코드가 NXT 불가 종목인경우 
				...
				auto it = m_mapPermission.find(m_sID);  //OnTimer Nxt 불가능
				if (it != m_mapPermission.end())
				{
					m_iPermission = it->second;
					m_iPermission &= ~2;
...
{
	m_slog.Format("			[cxMarketPicker][MARKET]<TraceLog 시작>[%s] m_sID=[%s] m_markettype=[%s] code=[%s] m_iPermission=[%d] m_major=[%d] m_minor = [%d]", m_sCtrlName,
		m_sID, MarketToString(m_markettype),m_sCode, m_iPermission, m_major, m_minor);
	OutputDebugString(m_slog);

	m_slog.Format("			[cxMarketPicker][MARKET]<TraceLog >slog=[%s] sKey=[%s] ", slog, sKey);
	OutputDebugString(m_slog);
	...
	if (stmp == "ctrlinfo")
	{
		int minor = (int)m_pWizard->SendMessage(WM_USER, MAKEWPARAM(variantDLL, minorCC), NULL);
		int major = (int)m_pWizard->SendMessage(WM_USER, MAKEWPARAM(variantDLL, majorCC), NULL);
		m_slog.Format("				[cxMarketPicker][MARKET]m_sCtrlName =[%s] m_sID=[%s] m_markettype=[%s] m_iPermission=[%d] m_major=[%d] major=[%d] m_minor=[%d] minor=[%d]",
