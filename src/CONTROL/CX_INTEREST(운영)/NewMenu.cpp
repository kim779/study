		...
		if (!bIsMenuBar)
		{
			...
			if (pMenuData->m_nMenuIconOffset < 0 /*&& state&ODS_CHECKED */ && !checkflag)
			{
				...
				if (state & ODS_CHECKED || info.hbmpUnchecked)
				{
					...
					if (state & ODS_CHECKED)
					{
						CRect rectcheck(RectIcon);
						rectcheck.InflateRect(2, ((m_iconY - RectIcon.Height()) >> 1) + 2);

						if (!info.hbmpChecked)
						{ // Checkmark
							DrawSpecialCharStyle(pDC, rectcheck, 98, state);
						}
						else if (!info.hbmpUnchecked)
						{ // Bullet
							DrawSpecialCharStyle(pDC, rectcheck, 105, state);
						}
						else
						{ // Draw Bitmap
							BITMAP myInfo = {0};
