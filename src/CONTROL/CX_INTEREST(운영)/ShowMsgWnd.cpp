...
BOOL CShowMsgWnd::Create(CWnd* pParentWnd)
{
	...
	const DWORD dwExStyle = WS_EX_STATICEDGE;
	...
	return CWnd::CreateEx(dwExStyle, nullptr, nullptr, dwStyle, CRect(0,0,0,0), pParentWnd, 0);
}
...
CWnd* CShowMsgWnd::ShowMessage(CWnd* pParentWnd, LPCTSTR cszMessage, UINT nShowTime /* = 1000 */, BOOL bProgress /* = FALSE */)
{
	...
	//const CSize size = GetTextExtent(pParentWnd, cszMessage);
	...
	//if (0 != nShowTime)
	//{
	//	pShowMsgWnd->SetWindowPos(nullptr, rcThis.left, rcThis.top, rcThis.Width(), rcThis.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
	//	pShowMsgWnd->CenterWindow();
	//	pShowMsgWnd->ShowWindow(SW_SHOW);
	//	pShowMsgWnd->SetTimer(FBSM_TIMERID, nShowTime, nullptr);
	//}
	//else
	//{
