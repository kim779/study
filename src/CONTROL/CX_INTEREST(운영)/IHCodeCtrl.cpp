...
const bool CIHCodeCombo::IsHistoryMode() const 
{ 
	return (m_DataMode == DM_HISTORY)? true:false; 
}

const bool CIHCodeCombo::IsCodeListMode() const
{
	return (m_DataMode == DM_CODELIST)? true:false; 
}

const bool CIHCodeCombo::JCodeLoad(CString tabPath)
{
...
const bool CIHCodeCombo::ELWCodeLoad(CString tabPath)
{
	DllInvoker dll("AxisCode");
	if (!dll.IsLoaded())
		return false;
...
//		return true;
	...
	for (int ii = 0; ; ii++)
	{
		const int res = ReadFile(fh, buf, bufsize, &dw, nullptr);
...
void CIHCodeEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	case VK_RETURN:
		...
		if (m_Unit == GU_CODE || m_Unit == GU_ELWCODE)
		{
			...
			{
				CString ss = m_pParent->m_pCombo->GetSelectCode();
...
CString CIHCodeCtrl::GetEditData() const
{
	CString	code;
...
void CIHCodeCtrl::SetEditData(CString sData) const
{
...
void CIHCodeCtrl::HistoryMode() const
{
	CString	symbol;
	switch (m_Unit)
	{
...
void CIHCodeCtrl::CodeListMode() const
{
