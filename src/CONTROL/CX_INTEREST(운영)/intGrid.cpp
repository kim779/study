...
constexpr int IDC_INPLACE_EDIT  = 8;
constexpr int IDC_INPLACE_COMBO = 9;
constexpr int IDC_INPLACE_DATE = 10;
constexpr int IDC_INPLACE_TIME = 11;


constexpr int IDC_HISTORY = 1000;
constexpr int indicatorW = 11;
...
CWnd* CintGrid::Create(const RECT& rect, CWnd* parent, UINT nID, int NOscroll, int dragdrop, DWORD dwStyle)
{
	...
	m_pBmpExpect = m_pParent->getAxBitmap(fileIMG);
	return this;
}
...
void CintGrid::OnTimer(UINT nIDEvent)
{
	...
	{
		KillTimer(TM_MEMOREFRESH);

		// m_slog.Format("\r\n[memo_cx_interest][%-40s]<%d>[%-35s]-->",
		// 	__FUNCTION__, __LINE__, "TM_MEMOREFRESH");
		// OutputDebugString(m_slog);

		m_pMainWnd->SendMessage(WM_MANAGE, MK_REFLESH);
		return;
	}
...
//			TRACE("OnDraw::DrawCell1\n");
	...
	CPen* pen = m_pParent->GetAxPen(m_gridCOL, 0, m_gridLineType);	
...
//			y += GetRowHeight(row) * m_yRate;
	...
	if ( m_nAlmRow > 0 )
	{
		CPen* pena = m_pParent->GetAxPen(RGB(0,0,255), 2);
...
//			rect.bottom = rect.top + GetRowHeight(row) * m_yRate -1;
			...
			if (_vBmpInfo.at(bmpInfo::imgMarker))
			{
				...
				CBitmap* pOldBitmapImage = (CBitmap*)dcImage.SelectObject(_vBmpInfo.at(bmpInfo::imgMarker));
...
// 		TRACE("DrawFixedCell::DrawCell\n");
	...
	if (nCol == colSIG)
	{
		...
		*/
		...
		CPen* pen = m_pParent->GetAxPen(RGB(200, 200, 200));
...
// updateX_20090109
	...
	if (!string.IsEmpty() && (nCol == colNAME) && !m_bHeadSet)
	{
		...
		if (m_bInfo)
		{
			...
			pInfo = _vBmpInfo.at(GetItemData(nRow, colINFO));
			if (pInfo)
			{
				CDC buffDC;
...
// updateX_20090109
	...
	//********************************************************************************
	// NXT 종목만 처리... 나머지 종목들은 종목코드에 알파벳이 있으면.. 모두 회색으로...
	//********************************************************************************
	...
	{	 
		...
		if (sCode.CompareNoCase(_T("N.")) == 0 || sNxt.FindOneOf("FJQ") != -1)
...
//						pDC->SetTextColor(PALETTERGB(71, 76, 80));
				...
				{					
					...
					if (m_bInfo)
					{
						CBitmap* pInfo = _vBmpInfo.at(GetItemData(nRow, colINFO));
						if (pInfo)
						{
							CDC buffDC;
							buffDC.CreateCompatibleDC(pDC);
...
BOOL CintGrid::SetItem(const GVITEM* gvitem)
{
	...
	if (gvitem->mask & GVMK_FLASH)   pCell->flash = gvitem->flash;
...
BOOL CintGrid::GetItem(GVITEM* gvitem)
{
	...
	if (gvitem->mask & GVMK_FLASH)   gvitem->flash = pCell->flash;
...
*/
	...
	if(strPreText.Compare(str) || nCol == colNAME || nCol == colCURR)	
	{
		if (_bReal && nRow >= m_nFixedRows && strPreText.Compare(str) != 0)
			Blink(nRow, nCol == colEXPECT ? colCURR : nCol);
...
void CintGrid::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_memoWnd && m_updateRow != -1 && _bEdit == false)
	{
...
// 						}
				...
				else
				{
					if (IsCellAttribute(current, GVAT_TOOLTIP))
					{
						if (current.col == colSIG)
						{
							timerID->kind = GetTypeInRect(cellRCx, point);
							timerID->row = current.row;
							timerID->col = colSIG;
						...
						else if (current.col == colNAME)
						{
							...
							if (m_memoRGN.PtInRegion(point) && _memoType == 0)
							{
								...
								if (click.row >= GetFixedRowCount() && m_memoAry.GetAt(click.row - 1))
								{
									...
									CRect	rc; rc.SetRect(point.x, point.y, point.x + cx, point.y + cy);
									ClientToScreen(&rc); m_updateRow = click.row; memoWindow(code, name, rc);
									return;
								}
							}
							else 
							{
								if (m_updateRow != -1 && _bEdit == false)
								{
									if (m_memoWnd && m_memoWnd->GetSafeHwnd())
										m_memoWnd->ShowWindow(SW_HIDE);
									m_updateRow = -1;
								}
				...
				if (cell.col == m_sortColumn)
				{
					if (TRUE == m_ascending)
					{
						...
						if (((CMainWnd*)m_pMainWnd)->m_bRemain == TRUE)	
						{
							...
							ptoolWnd->SendMessage(WM_MANAGE, MK_TURNCODE);
...
COLORREF CintGrid::DisplayAttribute(int attr, CString& data, int irow, int icol, CString strCode)
{
	...
	if (attr & (GVAT_CONDITION|GVAT_CONDITIONx))
	{
		...
		if (strCode.GetLength() == 8 && (strCode.GetAt(0) == '4' || strCode.GetAt(0) == 'D') && icol == colCURR)
		{
			if (icol == colCURR)
			{
				data.Replace("+", "");
...
void CintGrid::drawContrast(CDC *pDC, CRect cellRc, CString sText, COLORREF clr)
{
	...
	brush = m_pParent->GetAxBrush(clr); // bkcolor
...
void CintGrid::memoWindow(CString code, CString name, CRect rect)
{
	...
	{
		constexpr int cx = 225, cy = 150;
		CRect rc; rc.SetRect(0, 0, cx, cy);
		m_memoWnd->createMemo(this, m_pViewWnd, m_pMainWnd, &rc);
	}
...
{	
	...
	CPen *pen, *penUP, *penDOWN;
	pen     = m_pParent->GetAxPen(RGB(128, 128, 128));
	penUP   = m_pParent->GetAxPen(RGB(255, 0, 0));
	penDOWN = m_pParent->GetAxPen(RGB(0, 0, 255));
	

	CBrush	*brushUP, *brushDOWN;
	
	brushUP   = m_pParent->GetAxBrush(RGB(255, 0, 0));
	brushDOWN = m_pParent->GetAxBrush(RGB(0, 0, 255));
	...
	CString symbol = rcod.GetAt(0);
	if (symbol.FindOneOf("AMN") == 0)
	{
	...
	if (strCode.GetLength() == 8 && (strCode.GetAt(0) == '4' || strCode.GetAt(0) == 'D'))
	{
		for (int ii = 0; ii < 5; ii++)
		{
			CPoint point;
				...
				{
					if (ii == 0)
						ptBox[ii].x = rect.right;
					if (ii == 1)
							ptBox[ii].x = rect.right;
					if (ii == 2)
						ptBox[ii].x = rect.left;
					if (ii == 3)
						ptBox[ii].x = rect.left;
					if (ii == 4)
						ptBox[ii].x = rect.left;
...
void CintGrid::DrawSig(CDC* pDC, CRect rect, int nRow)
{
	...
	{
		CBitmap* pInfo = _vBmpInfo.at(imgSpecial);
...
// 			DrawSise(pDC, rect);
	...
	CPen*	pen;	
	pen = m_pParent->GetAxPen(RGB(200, 200, 200));
	CPen*	oldpen = pDC->SelectObject(pen);
	pDC->MoveTo(rect.left, rect.bottom);
	//pDC->LineTo(rect.right, rect.top);
	pDC->SelectObject(oldpen);
...
void CintGrid::DrawMarginRatio(CDC* pDC, CRect rect, int nRow)
{
	...
	NewBrush = m_pParent->GetAxBrush(RGB(200, 200, 200));
...
{
	CPen*	pen = m_pParent->GetAxPen(RGB(255, 0, 0));
	CPen*	oldpen = pDC->SelectObject(pen);
...
void CintGrid::ReDrawAll()
{
	...
	if (GetSafeHwnd() && (tick - _lastDrawTick >= 500))
	{
		_avgCount = getAverage(_Count);
//AxStd::_Msg("[%s][%d][%f]  draw count[500]", __FUNCTION__, _idrawCount, max((avgCount), 10.0));	
		if (const int avg = max(_avgCount, 30.0); _idrawCount <= avg) {
			//AxStd::_Msg("[%s][%d][%f] 그림", __FUNCTION__, _idrawCount, max((_avgCount), 30.0));		
			lambdaDraw(tick);
		}
		else
		{
			//AxStd::_Msg("[%s][%d][%f] 안그림", __FUNCTION__, _idrawCount, max((_avgCount), 30.0));		
		}
		_idrawCount = 0;
	}
	else 
