...
void CCodeList::AddItem(CString str)
{
	...
	slog.Format("[codectrl]<%d>[%s] AddItem  str= [%s]  size =[%d]  ", __LINE__, __FUNCTION__, str, m_arrData.GetSize());
...
void CCodeList::AddSFItem(CString str)
{
	...
	slog.Format("[codectrl]<%d>[%s] AddSFItem  str= [%s]   size =[%d] ", __LINE__, __FUNCTION__, str, m_arrData.GetSize());
	OutputDebugString(slog);
}
...
void CCodeList::OnAllDeleteBtn()
{
	if (MessageBox("모두 삭제하시겠습니까??", "IBK투자증권", MB_YESNO) == IDYES)
	{
		...
		//testcode
		m_pParent->DeleteHistoryCodeeArr();
		pCtrl->RemoveHistory(symbol);
		m_arrData.RemoveAll();
		Update();
