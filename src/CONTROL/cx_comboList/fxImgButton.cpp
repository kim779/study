...
void CfxImgButton::OnPaint() 
{
	...
	if (m_bUsePng)
	{
		...
		if ((m_capture && m_bHover) || (m_bCheck && m_bChecked))
			pImg = !m_imgDown.IsNull() ? &m_imgDown : &m_imgNormal;
		else if (m_bHover)
			pImg = !m_imgHover.IsNull() ? &m_imgHover : &m_imgNormal;
		else
			pImg = &m_imgNormal;

		if (pImg && !pImg->IsNull())
			DrawPng(&paintDC, *pImg, btnRc);
	}
	else
	{
...
BOOL CfxImgButton::LoadPng(CString sNormal, CString sDown, CString sHover)
{
	...
	// Normal (필수)
	if (sNormal.IsEmpty()) return FALSE;

	m_imgNormal.Destroy();
	if (FAILED(m_imgNormal.Load(sNormal)))
	{
		TRACE("LoadPng Normal 실패: %s\n", sNormal);
		return FALSE;
	}
	...
	m_imgDown.Destroy();
	if (!sDown.IsEmpty())
		m_imgDown.Load(sDown);

	// Hover (없으면 Normal 사용)
	m_imgHover.Destroy();
	if (!sHover.IsEmpty())
		m_imgHover.Load(sHover);
...
// PNG 그리기 헬퍼
void CfxImgButton::DrawPng(CDC* pDC, CImage& img, CRect rc)
{
	if (img.IsNull()) return;

	// AlphaBlend 로 PNG 알파채널 적용하면서 스트레치
	img.Draw(pDC->m_hDC,
		rc.left, rc.top,
		rc.Width(), rc.Height(),
		0, 0,
