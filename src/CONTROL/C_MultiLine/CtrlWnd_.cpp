...
void CCtrlWnd::OnPaint() 
{
	...
	_ctlINFO* pInfoCTL;
	for (int ii = ctl_ZOOMOUT, jj = 0; ii <= ctl_ZOOMIN; ii++, jj += 3)
	{
...
{
	_ctlINFO* pInfoCTL;
	for (int ii = ctl_ZOOMOUT; ii <= ctl_ZOOMIN; ii++)
	{
		if (!m_MapCont.Lookup(ii, pInfoCTL))
			continue;
...
void CCtrlWnd::DrawBtn(CDC* pDC, int selectID, _ctlINFO* pInfoCTL)
{
	...
	{
		if (pInfoCTL->pushed)
			m_pCtrlImage->Draw(pDC, 9 + selectID+2, imagePt, ILD_NORMAL);	
		else
			m_pCtrlImage->Draw(pDC, 9 + selectID, imagePt, ILD_NORMAL);
	}
	else
	{
