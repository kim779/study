...
#define IMXID_GETCODEINFO		20060				// 종목정보
...
OutputDebugString(m_tmpX);
	...
	else if (sImaxMapNum == "5964") //미국시세신청
	{
...
long CControlWnd::IMAXOnMessageJano(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_sViewNo == "1428") //잔고인경우4
	{
		...
		double sumBuyAmt = 0.0;		// 매입금액 합
		double sumMarketAmt = 0.0;	// 평가금액	합
		double sumProfit = 0.0;   // 평가손익 합
		//double sumRate = 0.0;     // 수익률 합
		int count = 0;            // 잔고 데이터 개수
		CString stmp;
		CString sArr[50]{ "", };
		int idx = 0;
		//arr 7 - 매입금액(원화) , 15 - 평가금액 , 11 - 평가손익, 수익률 = ((평가금액 - 매입금액)/매입금액)*100
		...
		for (int ii = 0; ii < 50; ii++)
		{
			...
			if (sArr[ii].GetLength() > 1)
			{
				...
				//replace를 처리안하면 스페이스값이 들어가서 맵화면에서 작업을 한번 더해줘야함.
				CString sBuyAmt, sMarketAmt, sProfit, sQty, sExRate;
				sBuyAmt = colArr[7];//매수금액(외화) 평균단가
				sBuyAmt.Replace(",", "");
				sMarketAmt = colArr[15];//원화 평가금액
				sMarketAmt.Replace(",", "");
				sProfit = colArr[13];	//원화 평가손익
				sProfit.Replace(",", "");
				...
				sumBuyAmt += BuyAmt* Qty * ExRate;	//원화 매수금액이 없는관계로 평균단가*잔고수량*환율로 계산
				sumMarketAmt += MarketAmt;
				sumProfit += profit;
				//sumRate += rate;
				count++;

				//m_slog.Format("IMAXOnMessageJano()  [%s] 평가손익합=[%f] 수익률합=[%f]  평가손익=[%f]   수익률=[%f]",
		...
		if (count > 0)
		{
			...
			result.Format("IMAXOnMessageJano  평균 평가손익: %.2f, 평균 수익률: %.2f", sumProfit, avgRate);
			OutputDebugString(result);
			send.Format("%s%c%d|%d|%d|%.2f", "SetAvgData", P_TAB, (int)sumBuyAmt, (int)sumMarketAmt, (int)sumProfit, avgRate);	//매입금액, 평가금액, 평가손익, 수익률 원화데이터의 경우 소수점이 필요없는 관계로 int형변환
			IMAXToMapTrigger(send);
		}
		else
		{
			OutputDebugString("IMAXOnMessageJano()  데이터 없음");
		}
...
void CControlWnd::parseIMAXMessageData(int nKind, CString sIMAXInfo)
{
	...
	case IMAX_INFO_CODE:
		...
		sISINCode	= sArr[14];
		sablemedo	= sArr[15];
		m_sCodeAll = sISINCode;      //20230321 종목코드 반환
...
//dprint();
	...
	case IMAX_INFO_ORDERDATA:
		...
		//평균단가, 잔고구분, 잔고수량  등락률  평가손익  수익률       평가금액  원화평가수익  원화평가금액    통화 기준환율  대출일       대출금(원화)
...
m_tmpX = _T("parseIMAXMessageData() PRICE_") + stmp; dprint();		
	...
	if (!sOrderQty.IsEmpty())
	{
		...
		m_slog.Format("]IMAXTRIGGER]<%d>[%d] IMAXJQTY code = %s 가능수량 =%s ", __LINE__,nKind, sCode, sArr[6]);
		OutputDebugString(m_slog);
	}
...
BSTR CControlWnd::_GetIMAXValue(LPCTSTR sValueText) 
{
	...
	if (nKey == IMXID_GETCODE)  //20230321 종목코드 반환
	{
		strResult = m_sCodeAll;
		return strResult.AllocSysString();
	}

	if (nKey == IMXID_GETCODEINFO)  //코드정보
	{
	...
	if (nKey == IMXID_GETCURPRICE)  //20230321 잔고 관련
	{
		::SendMessage(hWnd, WUM_IMAXDATAGET, (WPARAM)nKey, (LPARAM)szBuf);
		strResult = szBuf;
	...
	switch (nIDEvent)
	{
		...
		case DF_TIMER_IMAX_LONGIN:
		{
			...
			/*	if(GetTickCount() % 2 == 0)
				...
				if (bret)  //5500 같이 순차적으로 열리는 종합화면을 위한
...
void CControlWnd::AbroadAuth(bool bSet, bool bfile)
{
	if (m_pMainWnd)
	{
		...
		else
		{
			...
			if ((long)m_pParent->SendMessage(WM_USER, MAKEWPARAM(variantDLL, orderCC), 0L) == 0)  //0 직원
			{
				m_sCustomerAuth = "00000000000000000000000000000000022";
				m_sCustomerNum = "";
			}
