...
//	enum	_showInfo { infoNo = 0, infoSiga, infoPivot, infoCurr,  infoVI} m_showOHLC;	// 시고저종 표시
	enum	_showInfo { infoNo = 0, infoSiga, infoPivot, infoCurr, infoVI, infoMID } m_showOHLC;	// 시고저종 표시
	...
	//m_pBitmapC1 : 설정 , m_pBitmapC2 : 설정클릭 , m_pBitmapP1 : 대비 ,  m_pBitmapP1 : 대비클릭 , m_pBitmapT1 : 통합  m_pBitmapS1 : 통합개별
	...
	int CheckCodeMkType(CString sCode);
...
protected:
	...
	void	draw(CDC* dc, const CRect* pClip);
...
public:
	...
	CString m_strNoDataBitmap{};
	CBitmap* m_pBitmapNoData{};
...
protected:
	...
	int m_iDataType = 0;   //0 통합 ,   1 개별
	CRect	m_rcMarket{}, m_rcMKPOPMenu{};
	BSTR GetsDepthInfo();
	void SetsDepthInfo(BSTR newVal);
};