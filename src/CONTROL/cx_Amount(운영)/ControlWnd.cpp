...
BEGIN_DISPATCH_MAP(CControlWnd, CWnd)
	...
	DISP_PROPERTY_EX(CControlWnd, "Always", GetPopAlways, SetPopAlways, VT_BOOL)
	...
	DISP_FUNCTION(CControlWnd, "SetAlways", SetAlways, VT_EMPTY, VTS_I2)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
	...
	if (m_capture)
	{
		...
		if (rc.PtInRect(point))
		{
			...
			if (!m_pPopup)
			{
				...
				m_pPopup = std::make_unique<CAmountPopup>();
				m_pPopup->m_nStatus = m_kind;
				m_pPopup->m_pWizard = m_pParent;

				if (!m_bAlways)
				{
					if (m_kind == 1 || m_kind == 7 )	//거래소이거나 코넥스일때만
					{	
						rc.left = rc.left - 400;	//2014.07.01 KSJ 주버튼누를때 마우스 기준 왼쪽으로 뜨도록
						rc.SetRect(rc.left, rc.bottom, rc.left + 400 + SPACE_LEFT, rc.bottom + 50 + 80);/*(m_kind == 5?3:(m_kind == 4?1:m_kind))  * 20);*/ //2013.10.08 KSJ
					}
					else
					{
						rc.left = rc.left - 400 + 20;	//2014.07.01 KSJ 주버튼누를때 마우스 기준 왼쪽으로 뜨도록
						rc.SetRect(rc.left, rc.bottom, rc.left + 400, rc.bottom + 50 + 20/*(m_kind == 5?3:(m_kind == 4?1:m_kind))  * 20*/);	//2013.06.13 KSJ 사이즈를 왜 이런식으로 했는지 몰라서 20으로 고장
					}
				}
				else
				{
					rc.left = rc.right + 10;		//2014.07.01 KSJ 주버튼누를때 마우스 기준 왼쪽으로 뜨도록
				...
				m_pPopup->SetPopupAlways(m_bAlways);
...
void CControlWnd::WriteAlertx(struct _alertR* alertR)
{
}
...
void CControlWnd::SetAlways(short flag) 
{
	// TODO: Add your dispatch handler code here
	m_bAlways = flag;
}

BOOL CControlWnd::GetPopAlways() 
{
...
void CControlWnd::SetPopAlways(BOOL flag) 
{
	// TODO: Add your property handler here
	m_bAlways = flag;
}
