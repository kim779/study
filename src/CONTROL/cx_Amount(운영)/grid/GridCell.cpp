...
void CGridCell::SetFont(const LOGFONT* plf)
{
	...
	{
		if (!m_plfFont)
			m_plfFont = std::make_shared<LOGFONT>();
		if (m_plfFont)
			memcpy(m_plfFont.get(), plf, sizeof(LOGFONT)); 
	}
}
...
LOGFONT* CGridCell::GetFont() const
{
	if (m_plfFont == NULL)
	{
		const   CGridDefaultCell *pDefaultCell = (CGridDefaultCell*) GetDefaultCell();
...
CFont* CGridCell::GetFontObject() const
{
	...
	else
	{
		...
		Font.DeleteObject();
		Font.CreateFontIndirect(m_plfFont.get());
		return &Font;
	}
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		m_pEditWnd = std::make_shared<CInPlaceEdit>(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
	}
	return TRUE;
}
...
{
	if (m_pEditWnd)
		((CInPlaceEdit*)m_pEditWnd.get())->EndEdit();
}
