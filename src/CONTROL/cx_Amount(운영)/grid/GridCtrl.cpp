...
#if !defined(GRIDCONTROL_NO_DRAGDROP) || !defined(GRIDCONTROL_NO_CLIPBOARD)
    // BUG FIX - EFW
	const COleDataSource *pSource = COleDataSource::GetClipboardOwner();
	if (pSource)
...
void CGridCtrl::EraseBkgnd(CDC* pDC)
{
	...
	if (ClipRect.left < nFixedColumnWidth && ClipRect.top < nFixedRowHeight)
		pDC->FillRect(CRect(ClipRect.left, ClipRect.top, nFixedColumnWidth, nFixedRowHeight),
			&FixedRowColBack);
	...
	{
		CRect CellRect(__max(nFixedColumnWidth, rect.left), __max(nFixedRowHeight, rect.top),
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
	{
		...
		{
			const CDialog* p= (CDialog*) GetParent();
			if (p) p->NextDlgCtrl();
				return;
...
{
	const	GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	CPen	pen(PS_SOLID, 0, m_crGridLineColour);
...
void CGridCtrl::OnEditPaste()
{
	...
	{
		const	CGridCellBase* pCell = GetCell(cell.row, cell.col);
		ASSERT(pCell);
		if (!pCell) return;
						  ...
						  BOOL bForceRecalculation /*=FALSE*/)
{
	...
	// calc bottom
	int	ii = 0, bottom = GetFixedRowHeight();
...
BOOL CGridCtrl::SetFixedRowCount(int nFixedRows)
{
	...
	if (!GetVirtualMode())
	{
		...
		{
			for (int ii = nFixedRows, jj = 0; ii < m_nFixedRows; ii++)
			{
				for (jj = 0; jj < GetFixedColumnCount(); jj++)
					SetItemState(ii, jj, GetItemState(ii, jj) & ~GVIS_FIXEDROW );

				for (jj = GetFixedColumnCount(); jj < GetColumnCount(); jj++)
				{
					SetItemState(ii, jj, GetItemState(ii, jj) & ~(GVIS_FIXED | GVIS_FIXEDROW) );
					SetItemBkColour(ii, jj, CLR_DEFAULT );
					SetItemFgColour(ii, jj, CLR_DEFAULT );
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	const	CGridCellBase* pCell1 = (CGridCellBase*) lParam1;
	const	CGridCellBase* pCell2 = (CGridCellBase*) lParam2;
	if (!pCell1 || !pCell2)
		return 0;
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
	...
	const	CGridCellBase* pCell = GetCell(pItem->row, pItem->col);
	if (!pCell)
		return FALSE;
...
{
	const	CGridCellBase* pCell = GetCell(nRow, nCol);
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
	...
	*/
	...
	int	iColumnStart = 0, iRowStart = 0;
	...
	{
		SendMessage(WM_HSCROLL, SB_LINERIGHT, 0);
		if (pFocusWnd && !GetCellRect(nRow, nCol, rectCell))
		{
			pFocusWnd->SetFocus(); 
			return;
		}
...
CImageList* CGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
	...
	*pHotSpot = rect.BottomRight(); 
	...
	std::unique_ptr<CImageList> pList = std::make_unique<CImageList>();
