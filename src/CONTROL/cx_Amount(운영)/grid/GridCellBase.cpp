...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	COLORREF TextClr=0, TextBkClr=0;
...
// by LKM
	...
	if (pGrid->GetImageList() && GetImage() >= 0)
	{
		...
		if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
		{
			...
			//  working with xxx::CMemDC directly.  Instead, don't display image
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/)
{
	...
	if (pDC == NULL || szText == NULL)
	{
		...
		if (pDC == NULL || szText == NULL) 
		{
			const CGridDefaultCell* pDefCell = (CGridDefaultCell*) GetDefaultCell();
			ASSERT(pDefCell);
			if (pDefCell)
	...
	*pFont = GetFontObject();
	...
	if (pOldFont)
	{
		...
		pFont->DeleteObject();	//2011.12.15 KSJ
	}
