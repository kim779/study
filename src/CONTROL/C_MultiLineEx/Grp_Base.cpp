...
CGrp_Base::CGrp_Base(CWnd *pView, class CGrpWnd *pGrpWnd, char *pInfo, char* dInfo)
{
	m_pView = pView;
	m_pGrpWnd = pGrpWnd;
	...
	struct _data_info*	pData = (struct _data_info*)dInfo;
	m_digit = pData->digit;
	m_index = pData->index;
	m_indexInRGN = pData->rIndex;
	m_szName.Format("%.*s", sizeof(pData->name), pData->name);
	m_szName.TrimRight();
	m_szName.TrimLeft();

	m_szCode.Format("%.*s", sizeof(pData->code), pData->code);
	m_szCode.TrimRight();
	m_szCode.TrimLeft();

	m_szRsym.Format("%.*s", sizeof(pData->rsym), pData->rsym);
	m_szRsym.TrimRight();
	m_szRsym.TrimLeft();
...
CString CGrp_Base::GetDisplayPosHead(CPoint pt)
{
	...
	const struct _multi_line	*mline =
...
void CGrp_Base::DrawTickHorizon(CDC* pDC, double Max, double Min, int tickinfo)
{
	...
	{
		FormatVariableComma(dispStr, dispValue[ii], m_digit, maxdigit);
