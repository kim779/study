...
CString CGrp_Base::GetExcelHead(int idx)
{
	...
	{
		if (m_dIndex == CDI_MIN || m_dIndex == CDI_TICK)
			dispStr.Format("%02d:%02d:%02d", gVal2->index.time.hh, 
			gVal2->index.time.mm, gVal2->index.time.ss);
		else
			dispStr.Format("%04d/%02d/%02d", gVal2->index.date.yy, 
			gVal2->index.date.mm, gVal2->index.date.dd);
	}
	return dispStr;
}
	...
	if (view > 0)
	{
		...
		for (int ii = 0; ii <= endIdx; ii++)
		{
			...
			case CDI_MIN:	case CDI_TICK:
				if (m_pGrpWnd->m_drawGap > 0)
				{
					if (cgBong->index.time.mm % m_pGrpWnd->m_drawGap && ii != endIdx)
						continue;
					
					dispStr.Format("%02d:%02d", cgBong->index.time.hh, cgBong->index.time.mm);
