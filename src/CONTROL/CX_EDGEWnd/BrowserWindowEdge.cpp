...
#pragma warning (disable : 4996)  //klhs Ãß°¡
...
{
	m_slog.Format("[edge][%s]", __FUNCTION__);
	OutputDebugString(m_slog);

	if (m_webViewEnvironment == nullptr)
	{
		m_slog.Format("[edge][%s] m_webViewEnvironment is nullptr", __FUNCTION__);
		OutputDebugString(m_slog);
	...
	HRESULT hr = m_webViewEnvironment->CreateCoreWebView2Controller(m_hWnd, Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
		...
		if (!SUCCEEDED(result))
		{
			m_slog.Format("[edge][%s]  1 CreateCoreWebView2Controller fail", __FUNCTION__);
			OutputDebugString(m_slog);
			return result;
		}
	...
	if (!SUCCEEDED(hr))
	{
		m_slog.Format("[edge][%s] 2 CreateCoreWebView2Controller fail", __FUNCTION__);
		OutputDebugString(m_slog);
		return false;
	}
...
{	
	m_slog.Format("[edge][%s] [%s]", __FUNCTION__, initialUri);
	OutputDebugString(m_slog);
...
void BrowserWindowEdge::Resize(RECT bounds)
{
	...
	if (m_webViewController) {
		...
		m_webViewController->SetBoundsAndZoomFactor(bounds, m_dZoomFactor);
