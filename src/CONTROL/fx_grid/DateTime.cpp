...
{
	const gsl::not_null<HWND> hWndCtrl = pDX->PrepareCtrl(nIDC);
...
void CDateTime::SetMonthCalFont(const CFont& font, const BOOL bRedraw /*=TRUE*/)
{
...
{
	const SYSTEMTIME systimerange[2]{};
...
BOOL CDateTime::OnDateTimeChangeNotify(NMHDR* phdr, LRESULT* lResult)
//(NMDATETIMECHANGE *dtmh, LRESULT* pResult)
{
	*lResult = 0;    // docs say we must return 0
	const NMDATETIMECHANGE* dtmh = (NMDATETIMECHANGE*)phdr;
...
BOOL CDateTime::OnFormat(NMHDR* phdr, LRESULT* lResult)
//(NMDATETIMEFORMAT* /*lpNMFormat*/, LRESULT* pResult)
{
	*lResult = 0; 
	return FALSE; 
}

BOOL CDateTime::OnFormatQuery(NMHDR* phdr, LRESULT* lResult)
//(NMDATETIMEFORMATQUERY* lpDTFormatQuery, LRESULT* pResult)
{
	*lResult = 0; 
	NMDATETIMEFORMATQUERY* lpDTFormatQuery = (NMDATETIMEFORMATQUERY*)phdr;
	lpDTFormatQuery->szMax = CSize(366,16);
	return FALSE;
}

BOOL CDateTime::OnUserString(NMHDR* phdr, LRESULT* lResult)
//(NMDATETIMESTRING* /*lpDTstring*/, LRESULT* pResult)
{
	*lResult = 0;
	return FALSE;
}
