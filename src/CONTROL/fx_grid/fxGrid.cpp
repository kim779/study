...
/////////////////////////////////////////////////////////////////////////////
// CfxGrid
CfxGrid::CfxGrid(LOGFONT* logfont /*nullptr*/)
{
	RegisterWindowClass();
...
CWnd* CfxGrid::Create(const RECT& rect, CWnd* parent, UINT nID, int NOscroll, int dragdrop, DWORD dwStyle)
{
	...
	m_aryRowHeight.resize(m_nRows, 0);
	m_aryColWidth.resize(m_nCols, 0);

	m_aryRowHeight.resize(m_nRows, m_defHeight);
	m_aryColWidth.resize(m_nCols, m_defWidth);
...
m_syncx.Lock();
	...
	{	
		int col = 0;
		for_each(vitem->begin(), vitem->end(), [this, &col](const auto & cell) {
			CString text = cell->text;
			CString temps;
...
void CfxGrid::OnTimer(UINT nIDEvent)
{
	if (nIDEvent != WM_LBUTTONDOWN)
	{
		...
		if (nIDEvent == TM_TOOLTIP)
		{
			...
			//CString sparam; sparam.Format("%d", current.col);
...
//		fgCol = m_fgSELCOL;
	...
	if ((gvitem.attr & GVAT_CHECK) == GVAT_CHECK)
	{
		const int nWidth = 15, nHeight = 15;
		const int topx = (rect.Height() - nHeight) / 2;
		CRect	checkRC(rect.left, rect.top+topx, rect.left+nWidth, rect.top+topx+nHeight);
		if (gvitem.text.GetLength() <= 0)
		{
...
BOOL CfxGrid::SetRowCount(int nRows)
{
	...
	{
		int ii = nRows;
		for_each(m_aryGridRows.begin() + nRows, m_aryGridRows.end(), [this, &ii](const auto& vitem) {
			for (int jj = 0; jj < GetColumnCount(); jj++)
				SetItemState(ii, jj, 0);
			vitem->clear();
			ii++;
		});
		m_aryGridRows.erase(m_aryGridRows.begin() + nRows, m_aryGridRows.end());
	}
...
xCELL CfxGrid::CreateCell(int nRow, int nCol)
{
...
void CfxGrid::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (!(nFlags & MK_LBUTTON) || (m_mousemode == mouseNOTHING && (nFlags & MK_LBUTTON)))
	{
		...
		{
			m_tooltip.Hide();
			SetTimer(TM_TOOLTIP, 200, nullptr);
			/*
			if (m_ptLast == point)
			{
				m_tooltip.Hide();
...
CImageList* CfxGrid::CreateDragImage(CPoint *pHotSpot)
{
	...
	*pHotSpot = rect.BottomRight(); 
	...
	_imageList = std::make_unique<CImageList>();
	if (!_imageList || !_imageList->Create(rect.Width(), rect.Height(), ILC_MASK, 1, 1))
	{
		_imageList.reset();
		return nullptr;
	}
...
BOOL CfxGrid::PasteTextToGrid(CIdCell cell, COleDataObject* pDataObject)
{
	...
	auto szBuffer = std::make_unique<char[]>(::GlobalSize(hmem));
	if (!szBuffer) return FALSE;

	memFile.Read(szBuffer.get(), ::GlobalSize(hmem));
	::GlobalUnlock(hmem); ::GlobalFree(hmem);

	CString strText = szBuffer.get();
	//	delete szBuffer;
...
BOOL CfxGrid::IsValidDrop(COleDataObject*pDataObject)
{
	...
	auto pData = std::make_unique<char[]>(::GlobalSize(hmem));
	if (!pData) return FALSE;

	memFile.Read(pData.get(), ::GlobalSize(hmem));
	::GlobalUnlock(hmem); ::GlobalFree(hmem);

	CString strText = pData.get();
	//	delete pData;
...
BOOL CfxGrid::MoveColAction(int sCol, int dCol)
{
	...
	//{
	//	auto pRow = m_aryGridRows[nRow];
	//	auto cell = pRow->at(sCol);
	//
	//	pRow->erase(sCol);
	//	pRow->insert(dCol, cell);
	//}
