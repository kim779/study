...
void CfxIGrid::OnPaint() 
{
	...
	CRect clipRc, bkRect;
	gsl::not_null<CDC*>pMemDC = &dc;
	CFont* pOldFont{};
	std::unique_ptr<CMemDC> mdc;

	dc.GetClipBox(&clipRc);

	if (m_bMemDC)
	{
		mdc = std::make_unique<CMemDC>(&dc);
		pMemDC = mdc.get();
	}

	if (gsl::narrow_cast<int>(_vArrData.size() - m_StartPnt.y) * m_lDataHeight + m_lHeadHeight < clipRc.bottom)
	{
		bkRect = clipRc;
		bkRect.top = (_vArrData.size() - m_StartPnt.y) * m_lDataHeight + m_lHeadHeight;
	...
	for (size_t ii = 0; ii < _vArrData.size(); ii++)
...
{
	if (nRow + m_StartPnt.y >= gsl::narrow_cast<int>(_vArrData.size()))
		return FALSE;
	...
	const auto pArData = _vArrData.at(nRow + m_StartPnt.y);
	const long nSize = pArData->GetSize();
	CString sData, sTmp;
...
{
	int nRow = 0;
	while (nRow + m_StartPnt.y < gsl::narrow_cast<int>(_vArrData.size()))
	{
		if (m_lHeadHeight + m_lDataHeight * nRow > visibleRc.bottom)
			return;
		nRow++;
...
void CfxIGrid::DrawAddRow(CDC *pDC, bool bTop)
{
	...
	{
		DrawRow(pDC, gsl::narrow_cast<int>(_vArrData.size() - 1 - m_StartPnt.y), clientRc);
	}
}

void CfxIGrid::SetData(long nRow, long nCol, CString sDat)
{
	if (nRow >= gsl::narrow_cast<int>(_vArrData.size()) || nCol >= m_arColumn.GetSize())
		return;
	const auto pArData = _vArrData.at(nRow);
...
{
	std::shared_ptr<CStringArray> pArData = std::make_shared<CStringArray>();
	CDC *pDC = GetDC();
	CFont	*pOldFont;
	...
	if (bTop)
	{
		...
		if (m_nSelectedRow >= 0)
			m_nSelectedRow++;
		_vArrData.insert(_vArrData.begin(), pArData);
	}
	else
		_vArrData.emplace_back(pArData);
...
{
	if (nRow >= gsl::narrow_cast<int>(_vArrData.size()))
		return;
	_vArrData.at(nRow)->RemoveAll();
	_vArrData.erase(_vArrData.begin() + nRow);
...
void CfxIGrid::adjustScroll(int wBar)
{
	...
	if (wBar == SB_VERT || wBar == SB_BOTH)
	{
		if (m_lHeadHeight + m_lDataHeight * gsl::narrow_cast<int>(_vArrData.size()) > clientRc.Height())
		{
			...
			info.cbSize = sizeof(SCROLLINFO);
			info.nMin = 0; info.nMax = _vArrData.size() - (clientRc.Height() - m_lHeadHeight) / m_lDataHeight;
			info.nPage     = 1;
