...
LRESULT CCtrlWnd::OnCtrl(WPARAM wParam, LPARAM lParam)
{
	...
	case CTRL_ChgScrPos:
		...
		case SB_LINEUP:
			...
			m_strTemp.Format("%d\t%d\t", m_dispPos, m_dispEnd);
			m_pParent->SendMessage(CM_GRP, MAKEWPARAM(GRP_Control, ChgDispPos), long(m_strTemp.operator LPCTSTR()));
		...
		case SB_THUMBTRACK:
			...
			if (pos >= 0)
			{
				...
				m_strTemp.Format("%d\t%d\t", dispPos, dispEnd);
				m_pParent->SendMessage(CM_GRP, 
					MAKEWPARAM(GRP_Control, ChgDispPos), long(m_strTemp.operator LPCTSTR()));
			}
			break;
		}
		break;
