...
//	((CWnd*)GetDlgItem(IDC_EDIT_PATH))->SetWindowText(fileDlg.GetPathName());
	...
	CString temp;
	CString strpath;
	CTime	time = CTime::GetCurrentTime();  //G:\RTMS

	temp.Format("G:\\RTMS");  
//	temp.Format("D:\\capture"); 
//	temp.Format("\\\\172.16.213.33\\정보전략팀\\Public\\37_IT개발팀\\09. HTS\\서버모니터링\\%04d\\Temp", time.GetYear());  
//	temp = "C:\\Users\\IBKS\\Desktop\\capture";
...
void CIBK_CaptureDlg::StartdualCapture()
{
	if(IsToDayHoliDay())
		return;
	...
	int moncnt = GetSystemMetrics(SM_CMONITORS);
		...
		case 9999:
			{
				...
				if(!IsToDayHoliDay())  
				{	
					if(atoi(temp) >= 800 && atoi(temp) <= 1700  )  //1100 -> 1530
					{
						StartdualCapture();
					}
					else
					{
						SetWindowText("IBK_Capture ----   캡쳐중지  ----!!!!!");
					}
				}
...
void CIBK_CaptureDlg::BmpTojpg(CString sfile)
{
	...
	Image* pimg = Image::FromFile((const unsigned short *)(LPCTSTR)sfile);
	pimg->Save((const unsigned short *)(LPCTSTR)sfile, &jpegClsid);
}
