...
- [1. 클래스 계층 구조](#1-클래스-계층-구조)
  - [전체 의존성 다이어그램](#전체-의존성-다이어그램)
  - [클래스 관계도](#클래스-관계도)
- [2. CAxisForm — 화면(폼) 레벨 기본 클래스](#2-caxisform-화면폼-레벨-기본-클래스)
  - [역할](#역할)
  - [주요 멤버 (구조 요약)](#주요-멤버-구조-요약)
  - [핵심 공개 인터페이스](#핵심-공개-인터페이스)
  - [가상함수 (CScreen이 override)](#가상함수-cscreen이-override)
- [3. CfmBase — 컨트롤 기본 클래스](#3-cfmbase-컨트롤-기본-클래스)
  - [역할](#역할-1)
  - [주요 멤버 (구조 요약)](#주요-멤버-구조-요약-1)
  - [핵심 가상함수](#핵심-가상함수)
- [4. 주요 파생 클래스 상세](#4-주요-파생-클래스-상세)
  - [4.1 CfmEdit — 입력 필드](#41-cfmedit-입력-필드)
  - [4.2 CfmOut — 출력 필드](#42-cfmout-출력-필드)
  - [4.3 CfmGrid — 데이터 그리드](#43-cfmgrid-데이터-그리드)
- [5. 모든 파생 클래스 목록 (24개)](#5-모든-파생-클래스-목록-24개)
- [6. 컨트롤 생명주기](#6-컨트롤-생명주기)
  - [생성 단계](#생성-단계)
  - [사용 단계 (개념도)](#사용-단계-개념도)
  - [소멸 단계](#소멸-단계)
- [7. 데이터 흐름](#7-데이터-흐름)
  - [RTM(실시간 시세) 갱신 흐름 — 상세는 `docs/KnowledgeBase.md` 11절, `docs/RealtimeCodeIndex_Investigation.md` 참고](#rtm실시간-시세-갱신-흐름-상세는-docsknowledgebasemd-11절-docsrealtimecodeindex_investigationmd-참고)
  - [사용자 입력 흐름 (CfmEdit 기준)](#사용자-입력-흐름-cfmedit-기준)
- [8. DLL 경계의 위험성 (2026-07-23 실제 크래시 사례)](#8-dll-경계의-위험성-2026-07-23-실제-크래시-사례)
  - [상황: axisform.dll ↔ axWizard.ocx는 서로 다른 두 개의 바이너리](#상황-axisformdll-axwizardocx는-서로-다른-두-개의-바이너리)
  - [실제 발생한 크래시](#실제-발생한-크래시)
  - [해결 원칙: 새 가상함수는 반드시 목록 맨 끝에 추가](#해결-원칙-새-가상함수는-반드시-목록-맨-끝에-추가)
  - [재컴파일 시 유의사항](#재컴파일-시-유의사항)
- [9. Wizard와의 인터페이싱 (연동 지점)](#9-wizard와의-인터페이싱-연동-지점)
- [10. 핵심 헬퍼 클래스 (dll/form/ 내)](#10-핵심-헬퍼-클래스-dllform-내)
- [11. 파일 구성 요약](#11-파일-구성-요약)
- [12. 설계 패턴 관찰](#12-설계-패턴-관찰)
- [13. 향후 분석 후보 (미착수)](#13-향후-분석-후보-미착수)
- [14. 관련 문서](#14-관련-문서)
