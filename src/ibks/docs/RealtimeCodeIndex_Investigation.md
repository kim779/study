...
- [배경 / 목표](#배경-목표)
- [확인된 사실](#확인된-사실)
  - [1. "종목코드 필드"의 식별 방법 — `FA_FLASH` 속성 플래그 (2026-07-20 정정, 실측 확인 완료)](#1-종목코드-필드의-식별-방법-fa_flash-속성-플래그-2026-07-20-정정-실측-확인-완료)
  - [2. 읽기측(매칭) — `CScreen::OnAlert`](#2-읽기측매칭-cscreenonalert)
  - [3. 쓰기측(값이 바뀌는 경로) — 최소 3갈래 확인됨](#3-쓰기측값이-바뀌는-경로-최소-3갈래-확인됨)
- [미해결 질문 → 답변 완료 (2026-07-20)](#미해결-질문-답변-완료-2026-07-20)
- [관련 파일 (이번 조사에서 읽은 곳)](#관련-파일-이번-조사에서-읽은-곳)
- [3단계 설계: 실제 컷오버 (2026-07-20 설계, 미구현)](#3단계-설계-실제-컷오버-2026-07-20-설계-미구현)
  - [설계 원칙 — 매칭 로직은 그대로, "몇 개 화면을 부르느냐"만 바꾼다](#설계-원칙-매칭-로직은-그대로-몇-개-화면을-부르느냐만-바꾼다)
  - [변경 지점 후보](#변경-지점-후보)
  - [안전한 롤아웃 순서 (제안)](#안전한-롤아웃-순서-제안)
  - [①단계 실행 결과 (2026-07-23)](#①단계-실행-결과-2026-07-23)
  - [②단계 실행 결과 (2026-07-23) — 런타임 킬스위치(레지스트리)](#②단계-실행-결과-2026-07-23-런타임-킬스위치레지스트리)
  - [미결 사항 (③단계 구현 전 반드시 확정 필요)](#미결-사항-③단계-구현-전-반드시-확정-필요)
- [관련 문서](#관련-문서)
