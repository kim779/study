...
// for window 7
#include "imm.h"
LRESULT Cmemo::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
	case WM_IME_COMPOSITION:
		...
		{
//{ CString tmpx; tmpx.Format("GCS_RESULTSTR = [%s]\n", m_stIMET); OutputDebugString(tmpx); }
			if (!m_stIMET.IsEmpty())
			{
				m_stIMET.Empty();
			}
	...
	case WM_IME_ENDCOMPOSITION:
		GetWindowText(m_stIMET);
//{ CString tmpx; tmpx.Format("ENDCOMPOSITION = [%s]\n", m_stIMET); OutputDebugString(tmpx); }
		break;

	case WM_KILLFOCUS:
		GetWindowText(m_stCHAR);
//{ CString tmpx; tmpx.Format("WM_KILLFOCUS1 = [%s][%s]\n", m_stIMET, m_stCHAR); OutputDebugString(tmpx); }
		if (!m_stIMET.IsEmpty() && m_stIMET == m_stCHAR)
		{
//{ CString tmpx; tmpx.Format("WM_KILLFOCUS2 = [%s]\n", m_stIMET); OutputDebugString(tmpx); }
			SetWindowText(m_stIMET); m_stIMET.Empty();
		}
		break;
	default:
		break;
