...
CString CLedger::GetLedger(int pos, int length)
{
	...
	CString slog;
	slog.Format("[LEDGER][%s]<%d> pos=[%d] length=[%d] m_text=[%s]", 
		__FUNCTION__, __LINE__, pos, length, m_text.Mid(137,20));
	...
	if (pos == 0)
	{
		...
		slog.Format("[LEDGER][%s]<%d> next=[%s]   data=[%s]", __FUNCTION__, __LINE__, 
			CString(ledgerH->next, 1), CString(ledgerH->nkey, 18));
		OutputDebugString(slog);
	}
...
//		m_text = CString(ledgerH->time_r, sizeof(ledgerH->time_r));	break;
		...
		CString slog;
		slog.Format("[LEDGER][%s]<%d> id=[%d]  m_text=[%s]", __FUNCTION__, __LINE__, id,  m_text.Mid(137,20));
		OutputDebugString(slog);
	}
...
void CLedger::SetLedger(int pos, int length, void* data)
{
	...
	struct	_ledgerH* ledgerH;
	ledgerH = (struct _ledgerH*)m_ledger;
	slog.Format("[LEDGER][%s]<%d> pos=[%d] length=[%d]   연속여부=[%.1s]   연속키 =[%.18s] ", 
		__FUNCTION__, __LINE__, pos, length, ledgerH->next, ledgerH->nkey);
	OutputDebugString(slog);
