...
#ifdef DF_RECORD
		m_wsh->StartRecord(_T("D:\\rtm_replay.bin"));
#endif
#ifdef DF_QUELOG
		m_wsh->StartQueLog(_T("D:\\que_log.txt"));
#endif
#ifdef DF_REPLAY
...
#ifdef DF_REPLAY
void CSockCtrl::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1)
	{
		KillTimer(1);
		AfxMessageBox("파일로드 시작");
		m_wsh->StartReplay(_T("D:\\rtm_replay.bin"), 10);
	}
