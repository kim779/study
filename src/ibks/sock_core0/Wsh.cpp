...
{
#ifdef DF_RECORD
	if (m_recFile.m_hFile != CFile::hFileNull)
	{
		DWORD tick = GetTickCount() - m_recStartTick;
		m_recFile.Write(&tick, sizeof(DWORD));
		m_recFile.Write(&pMQ->m_nBytes, sizeof(int));
		m_recFile.Write(pMQ->m_pBytes, pMQ->m_nBytes);
	}
#endif
	m_sectionRTM.Lock();
	m_queRTM.Add(pMQ);
...
{
#ifdef DF_POSTMSG
	if (!m_pWnd->PostMessage(WFM_EVENT, FEV_ANM, (LPARAM)pMQ))
		delete pMQ;   // 실패 시 직접 해제
#else
...
UINT dispatchRTM(LPVOID lpvoid)
{
	...
	while (pWsh->m_running)
	{
		...
		if (now - queLogTick >= 1000)
		{
			SYSTEMTIME st;
			GetLocalTime(&st);
			int cnt = (int)pWsh->m_queRTM.GetSize();
			CString line;
			line.Format("%02d:%02d:%02d.%03d que=%d\n", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, cnt);
			if (pWsh->m_queLog.m_hFile != CFile::hFileNull)
				pWsh->m_queLog.WriteString(line);
			queLogTick = now;
		}
#endif
	}
...
static DWORD_PTR GetOtherCore()
{
	...
	return lastCoreMask ? lastCoreMask : sysMask;
}
...
void CWsh::BeginRTM()
{
	...
	{
		m_running = false;   // ← AfxBeginThread 실패 시 원복
	}
}
//void CWsh::BeginRTM()
//{
...
{
	if (!m_running || !m_thread)   // ← m_thread NULL 체크 추가
		return;
...
void CWsh::StartQueLog(LPCTSTR path)
{
	m_queLog.Open(path, CFile::modeCreate | CFile::modeWrite | CFile::typeText);
}

void CWsh::StopQueLog()
{
	m_queLog.Close();
}

void CWsh::StartRecord(LPCTSTR path)
{
	m_recStartTick = GetTickCount();
	m_recFile.Open(path, CFile::modeCreate | CFile::modeWrite | CFile::typeBinary);
}

void CWsh::StopRecord()
{
	m_recFile.Close();
}

UINT replayRTM(LPVOID lpvoid)
{
	CWsh* pWsh = (CWsh*)lpvoid;
	CFile f;
	if (!f.Open(pWsh->m_replayPath, CFile::modeRead | CFile::typeBinary))
		return 0;
	...
	while (pWsh->m_running)
	{
		if (f.Read(&tick, sizeof(DWORD)) < sizeof(DWORD)) break;
		if (f.Read(&nBytes, sizeof(int)) < sizeof(int)) break;

		CMQue* pMQ = new CMQue();
		pMQ->m_ssm    = ssM_SM;
		pMQ->m_nBytes = nBytes;
		pMQ->m_pBytes = new char[nBytes];
		f.Read(pMQ->m_pBytes, nBytes);
		...
		DWORD targetMs = (tick - firstTick) / pWsh->m_replaySpeed;
		DWORD elapsedMs = GetTickCount() - realStart;
		if (targetMs > elapsedMs)
			Sleep(targetMs - elapsedMs);

		// Back-pressure: prevent tight-loop when replaying faster than consumption
		while (pWsh->m_running)
		{
			pWsh->m_sectionRTM.Lock();
			int qsize = (int)pWsh->m_queRTM.GetSize();
			pWsh->m_sectionRTM.Unlock();
			if (qsize < 50) break;
			Sleep(5);
		}

		if (!pWsh->m_running) { delete pMQ; break; }

		pWsh->m_sectionRTM.Lock();
		pWsh->m_queRTM.Add(pMQ);
		pWsh->m_sectionRTM.Unlock();
		pWsh->m_eventRTM.SetEvent();
	}
...
void CWsh::StartReplay(LPCTSTR path, DWORD speed)
{
	m_replaySpeed = (speed > 0) ? speed : 1;
	m_replayPath  = path;
	AfxBeginThread((AFX_THREADPROC)replayRTM, (LPVOID)this);
}
