...
m_slog.Format("[axSock][Receive]--------conitnue!!! [%x][%s]  m_que.GetUpperBound()=[%d] m_iUpperbound=[%x][%d]",
	this, __FUNCTION__, m_que.GetUpperBound(), &m_iUpperbound, m_iUpperbound);
		...
		//m_section.Lock();   //이거의 위치가 이상하다고 한다.
		...
		{
			m_iUpperbound = m_que.GetSize();
			m_slog.Format("[axSock][Receive]--------처리 [%x][%s]  m_que.GetUpperBound()=[%d] m_iUpperbound=[%x][%d]",
				this, __FUNCTION__, m_que.GetUpperBound(), &m_iUpperbound, m_iUpperbound);
			if (m_type == WSH)
				OutputDebugString(m_slog);
		}
...
void CWsh::Dispatch(int ssm, char* pBytes, int nBytes)
{
	...
	/*	char* pval = new char[9];
		memset(pval, 0x00, 9);
		memcpy(pval, "pibo", 4);*/
		//if (strncmp(pBytes + 8, pval, strlen(pval)) == 0) {
	...
	if (nBytes > 0)
	{
		...
		{
			m_slog.Format("[axSock][Receive] 특정TR 찾아서 따로 처리 [%x][%s]  extractedString =[%s]",
				this, __FUNCTION__, extractedString);
			OutputDebugString(m_slog);
			//
			m_section.Unlock();
			m_event.SetEvent();
			m_pWnd->SendMessage(WFM_AXIS, (WPARAM)mque->m_nBytes, (LPARAM)mque->m_pBytes);
			return;
		}
	}
	...
	m_que.Add(mque);
	m_iUpperbound = m_que.GetSize();
	m_slog.Format("[axSock][Receive][%x][%s] m_que.GetUpperBound()=[%d] m_iUpperbound=[%x][%d]",
		this, __FUNCTION__,  m_que.GetUpperBound(), &m_iUpperbound, m_iUpperbound);
	//OutputDebugString(m_slog);
	if (ssm == 1) {
		m_slog.Format("[axSock][Receive][%x][%s]  [%.24s]  m_que.GetUpperBound()=[%d] m_iUpperbound=[%x][%d]",
			this, __FUNCTION__, (char*)&mque->m_pBytes[8], m_que.GetUpperBound(), &m_iUpperbound, m_iUpperbound);
	}
#endif
	m_section.Unlock();
	m_event.SetEvent();
...
//	text = CString(pBytes, datL);
		...
		m_slog.Format("[axSock][Receive]정상처리해서 위자드에 메시지  [%s]  >>>>>[%s]<<<<<", __FUNCTION__, sdata);
		if (m_type == WSH)
			OutputDebugString(m_slog);
#endif
...
//#ifndef _DEBUG
//	CString stmp;
//	stmp.Format("[axSock][patch] %s", rsmH->resN);
//	stmp.TrimRight();
//	if (stmp.Find("icss") >= 0)   //icss test 
//	{
//		stmp.Format("[axSock][patch] 1111  [%x]", rsmH->resK);
...
//#endif
...
#ifndef _DEBUG
				m_slog.Format("[axSock][patch]222 [%s]  [%s]", __FUNCTION__, path);
				//OutputDebugString(m_slog);
#endif

#ifndef DF_NORESOURCE_DOWN
				if (MakeUpdateList())
					DoRequest();
				else
#endif
...
#ifndef _DEBUG
	m_slog.Format("[axSock][patch] %s", rsmH->resN);
	m_slog.TrimRight();
	if (m_slog.Find("icss") >= 0)   //icss test 
	{
		m_slog.Format("[axSock][patch]2222  [%x][%x]", rsmH->resK, rsmH->resF);
		//OutputDebugString(stmp);
	}
#endif
	BOOL	update = FALSE;
	...
	{
#ifdef _DEBUG
		m_slog.Format("[axSock][patch][%s][%x][%s]내려받은 파일을 실제 폴더에 복사하는 단계", __FUNCTION__, rsmH->resK, rsmH->resN);
		OutputDebugString(m_slog);
#endif
		...
		else
		{
			DeleteFile(tmps);
#ifndef _DEBUG
		m_slog.Format("[axSock][patch]  [%s][%x]  DeleteFile= [%s]  MoveFile(=[%s]", __FUNCTION__, rsmH->resK, path, tmps);
		OutputDebugString(m_slog);
#endif
	...
	//m_slog.Format("[axSock][patch][%s][%x] rsmH->resF != resF_LAS || rsmH->resF != resF_ONLY", __FUNCTION__, rsmH->resK);
	//OutputDebugString(m_slog);
...
BOOL CWsh::DoRequest(bool info)
{
	...
	//	if (str.Find("icss") >= 0)   //icss test 
		{
			m_slog.Format("[axSock][patch][%s] [%s]", __FUNCTION__, str);
			OutputDebugString(m_slog);
		}
...
OutputDebugString(m_slog);
	...
	LoadFile(path, arr);
#ifndef _DEBUG
	m_slog.Format("[axSock][patch] MakeUpdateList 22222222 arr=[%d]", arr.GetSize());
	//OutputDebugString(m_slog);
#endif
	for (int ii = 0; ii < arr.GetSize(); ii++)
	{
...
//m_slog.Format("[axSock][patch]  [%s]  name= [%s]  vers=[%s]", __FUNCTION__, wb, keys);
//OutputDebugString(m_slog);
			}
		}
...
void CWsh::BeginRTM()
{
	...
	//	DWORD_PTR dwProcessAffinityMask = 0x01 | 0x02 | 0x03 | 0x04 | 0x05 | 0x06 | 0x07 | 0x08 | 0x09 | 0x0a;
		...
		if (SetThreadAffinityMask(m_thread->m_hThread, dwProcessAffinityMask) == 0) {
		    ...
		    slog.Format("[MULTICORE]axissock 스레드 친화도 실패[%d]", ierror);
			OutputDebugString(slog);
		}
		else
		{
			slog.Format("[MULTICORE]  axissock 프로세스 친화도 성공");
			OutputDebugString(slog);
		}
