...
{
CString text;
//text.Format("[axSock][CStream][OnReceive] ");
//OutputDebugString(text);
//text.Format("[axSock][CStream][OnReceive] ");
//OutputDebugString(text);
//text.Format("[axSock][CStream][OnReceive]  m_idx=[%d]----------- [%s] \r\n", getFGubn(m_flow), m_idx);
//OutputDebugString(text);
...
//OutputDebugString(text);
	...
	for (int ii = 0; ii < m_frameL; ii++)
	{
		...
		case _frameD_:			// Data
			...
			{
				m_flow = (fmH->stat & stat_BCC) ? _frameBCC_ : _frameE_;
			//	text.Format("[axSock]m_idx = [%d] m_rcvL =[%d] [%s] \r\n", m_idx, m_rcvL, getFGubn(m_flow));
//OutputDebugString(text);
			}
			break;
...
//text.Format("[axSock]m_idx = [%d] [%s] \r\n", m_idx, getFGubn(m_flow)); _frameE_
...
bool CStream::WriteData(char *data, int ndat, bool wait)
{
	...
	//m_slog.Format("[axSock][%s]<%d> TR=[%.8s]  len =[%d]", __FUNCTION__, __LINE__, (char*)&data[11], ndat);
...
{
	if (AfxGetApp()->GetProfileInt("INFORMATION", "NOSOCKWRITE", 0) == 1)
		return false;
#ifdef	_DEBUG
	...
	for (int ii = 0; ;)
	{
		...
		if (nBytes > 900)
		{
		//	m_slog.Format("[axSock][FDS] nBytes=[%d]  nWritten =[%d]", nBytes, nWritten);
		//	OutputDebugString(m_slog);
		}
