...
void WriteLog(LPCSTR log, ...)
{
	...
	TRY
	{
		...
		modPath += "\\axis.log";
		OutputDebugString("WriteLog = " + modPath);

		FILE* fp;
		fopen_s(&fp, modPath, "a+");
		if (!fp)
		{
...
BOOL CIBKSConnectorCtrl::Initialize() 
{
	...
	const char* const REG_KEY = "IBK_OPENAPI";
...
int CIBKSConnectorCtrl::SendTR( LPCSTR trnm, int tkey, int stat, LPCSTR data, int size, TR_CALLBACK cb)
{
	...
	/*CString stmp, sresult{}, strtmp;
	stmp.Format("%s", trnm);
	if (stmp.Find("pidouini") >= 0)
	{
		stmp.Format("%s", data);
		sresult.Format("[%d][%d]  ", stmp.GetLength(), size);
		for (int ii = 0; ii < stmp.GetLength(); ii++)
		{
			strtmp.Format("%c", stmp.GetAt(ii));
			sresult += strtmp;
		}
...
//	기존 약정계좌인지 체크하는 서비스
			...
			for(int n=0; n<m_acno.size(); ++n)
			{
				...
				SendTR("piboPBxQ", n, US_ENC, (LPCSTR)&data[0], data.size(), &CIBKSConnectorCtrl::C_SACAQ504);  
...
BOOL CIBKSConnectorCtrl::S_PIBOSODR(int key, int mmgb, LPCSTR acno, LPCSTR pswd, int ojno, LPCSTR fcod, int jqty, int jprc, int hogb, int mdgb )
{
	...
	// 주문 편집
	memcpy(mid->gubn, "^C", 2);				//^C 2014.04.21 KSJ 차익/비차익 때문에 추가   //test
	//memcpy(mid->gubn, "11", 2);
	sprintf(mid->nrec, "%04d", 1);			// 반복횟수
	mid->odgb[0] = '1';						// 선물
	mid->mmgb[0] = '0' + mmgb;				// 매매구분
	memcpy(mid->accn, acno, 11);				// 계좌번호
	...
	if (ledger->ecod[0]!='0')
	{
		...
		for(size_t n=0; n<m_acno.size(); ++n)
		{
			if (m_chk_acno[key]==m_acno[n])
			{
				...
				//m_acno.erase(m_acno.begin()+n);  //test  우선주석
				//m_acnm.erase(m_acnm.begin()+n);
				break;
			}
		}
