...
#include "stdafx.h"
#include "fxGrid.h"
#include "grid/excel9.h"

#include "InPlaceEdit.h"
#include "grid/InPlaceCombo.h"
#include "grid/InPlaceDate.h"
#include "math.h"
...
#define	GRIDCTRL_CLASSNAME	_T("fxGrid")
...
CWnd* CfxGrid::Create(const RECT& rect, CWnd* parent, UINT nID, int NOscroll, int dragdrop, DWORD dwStyle)
{
	...
	if (!CWnd::Create(GRIDCTRL_CLASSNAME, _T("fxgrid"), dwStyle, rect, parent, nID))
		return NULL;
...
void CfxGrid::Clear(int sidxRow, int sidxCol)
{
	...
	for (int row = sidxRow; row < m_nRows; row++) 
	{
		...
		{
			SetItemState(row, col, 0); SetItemText (row, col, _T(""));
...
bool CfxGrid::RunExcel_()
{
	...
	const	int alphaC = 24;
	CString	string; string.Format(_T("A1:%c%d"), (char)('A' + m_nCols), m_nFixedRows + m_nRows);
	if (m_nCols >= alphaC)
		string.Format(_T("A1:%c%c%d"), (char)('A' + ((m_nCols/alphaC) -1)),
	...
	for (int jj = 0; m_nFixedRows && jj < m_nCols; jj++)
	{
		...
		string.Format(_T("%c%d:%c%d"), (char)('A' + jj), m_nFixedRows, (char)('A' + jj), m_nRows);
		if (jj >= alphaC)
		{
			string.Format(_T("%c%c%d:%c%c%d"), (char)('A' + ((jj / alphaC)-1)),
	...
	for (int ii = 0; !m_excelS.IsEmpty() && ii < m_nRows; ii++)
	{
		...
		for (int jj = 0; !stringX.IsEmpty() && jj < m_nCols; jj++)
		{
			...
			if (IsCellAttribute(CIdCell(ii, jj), GVAT_COMMA) && ii > 0)
			{
			//	TRACE("[ROW : %d] [COL : %d]\n", ii, jj);
				iPos1 = string.Find(ARR_UP);
				iPos2 = string.Find(ARR_DN);
				iPos3 = string.Find(TRI_UP);
				iPos4 = string.Find(TRI_DN);

				if (iPos1 != -1 || iPos2 != -1 || iPos3 != -1 || iPos4 != -1)
				{
					CString strTemp = string.Left(2);
					string = string.Mid(2);
					string = CommaModify(string);
...
void CfxGrid::OnTimer(UINT nIDEvent)
{
	if (nIDEvent != WM_LBUTTONDOWN)
	{
		...
		COLORREF  bkcol;
		int row = nIDEvent / 10000;
		int col = nIDEvent % 10000;
...
{
//TRACE("onvscroll=m=%d, sb=%d, %x,%x\n", m_noMessage, nSBCode, GetFocus()->GetSafeHwnd(), GetSafeHwnd());
/*#define SB_LINEUP           0
	...
	if ((gvitem.attr & GVAT_HDRMERGE) == GVAT_HDRMERGE)
	{
		...
		if (m_mapMerge.Lookup(nDrawCol, fixMerge))
		{
			...
			for (int ii = 0; ii < maxMERGE; ii++)
			{
				...
				if (fixMerge.cols[ii] < minCol)
				{
					nLWidth += GetColumnWidth(fixMerge.cols[ii]);

				}

				if (fixMerge.cols[ii] > maxCol)
				{
					nRWidth += GetColumnWidth(fixMerge.cols[ii]);
			...
			if (nLWidth > 0)
				fixMerge.rect.left -= nLWidth;

			if (nRWidth > 0)
				fixMerge.rect.right += nRWidth;
...
BOOL CfxGrid::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	else if (m_selectLine && gvitem.state & GVST_SELECTED && !pDC->IsPrinting())
	{
		...
		pDC->FillSolidRect(rect, m_bkSELCOL);
		rect.right--; rect.bottom--;
// updateX_20060109 ReUpdate(20061207) 푸르덴셜 선택셀 폰트 흰색 반전
		fgCol = m_fgSELCOL;
		pDC->SetTextColor(fgCol);
	}
	else
	...
	/* 2009. 3. 23. 일봉 그리기 추가 */
	if ((gvitem.attr & GVAT_DRAWBONG) == GVAT_DRAWBONG)
	{
		CString strDayData = gvitem.text;
		CString strTemp("");
		double value[6];
		...
		{
			iPos = strDayData.Find('\t');
			strTemp = strDayData.Left(iPos);
			strDayData = strDayData.Mid(iPos + 1);

			strTemp.Remove('-');	strTemp.Remove('+');
			value[ii] = (double)_wtof(strTemp);		
		}

		DrawBong(pDC, rect, value, upCOLOR, dnCOLOR);
...
//			temps = ARR_UP; gvitem.fgcol = m_upCOL; break;
		...
		case '+':
		case '2':
			temps = TRI_UP; gvitem.fgcol = m_upCOL; 
//updateX_20090429 색반전 추가
			if (rtmCol == -1 && m_selectLine && (gvitem.state & GVST_SELECTED) && !(gvitem.state & GVST_FOCUSED))
			{
...
//			temps = ARR_DN; gvitem.fgcol = m_dnCOL; break;
		...
		case '-':
		case '5':
			temps = TRI_DN; gvitem.fgcol = m_dnCOL; 
//updateX_20090429 색반전 추가
			if (rtmCol == -1 && m_selectLine && (gvitem.state & GVST_SELECTED) && !(gvitem.state & GVST_FOCUSED))
			{
				gvitem.fgcol = m_fgSELCOL;
...
//				if ((m_nFixedCols != 0 && nCol < m_nFixedCols) || (m_nFixedCols == 0 && nRow == 0 && nCol == 0))
		...
		// 2009. 5. 4. 그리드 마지막 라인 버그처리(Fixed Column 처리)
			CRect visRect;
			CRangeCell visCellRange = GetVisibleNonFixedCellRange(visRect);
			if (visCellRange.GetMaxCol() - 1 <= nCol)
			{
				pDC->MoveTo(visRect.right - 1, rect.top);	pDC->LineTo(visRect.right - 1, rect.bottom);
			}
...
// 수정처리<FixedColumn일 경우>
//	if (m_nFixedCols > 0)
//		rect.right += rect.left;
	if (m_nFixedCols > 0)
	{
		if (rect.left - GetFixedColumnWidth() >= rect.right || rect.top >= rect.bottom)
			return;
	}
	else
	{
...
//TRACE("ResetScrollBars:%d,%d,%d\n", m_Vscroll, m_Hscroll, m_DIscroll);
}
...
//	ResetSelectedRange();
	...
	{
		int val = 0;
		m_aryColWidth.InsertAt(nColumn, val);
		for (int row = 0; row < m_nRows; row++) 
		{
			GRID_ROW* pRow = m_aryGridRows[row];
			if (!pRow) return -1;
...
//	ResetSelectedRange();
	...
	{
		int val = 0;
		m_aryRowHeight.InsertAt(nRow, val);
			...
			case cellVal:
				if (ascending)
				{
					if (string.IsEmpty() && !stringx.IsEmpty())
						swap = true;
					else if (!stringx.IsEmpty() && _wtof(string) > _wtof(stringx))
						swap = true;
				}
				else
				{
	...
	if (!(nFlags & MK_LBUTTON) || (m_mousemode == mouseNOTHING && (nFlags & MK_LBUTTON)))
	{
		...
		if (m_mousemode == mouseNOTHING && m_tooltip && m_enabletip)
		{
				...
				&& IsCellAttribute(current, GVAT_TOOLTIP))
			{
				...
				CString string = GetItemText(current.row, lParam);
				if (!string.IsEmpty())
...
BOOL CfxGrid::PasteTextToGrid(CIdCell cell, COleDataObject* pDataObject)
{
	...
	wchar_t* szBuffer = new wchar_t[::GlobalSize(hmem)];
...
// updateX_20060113
	...
	// Get a tab delimited string to copy to cache
	CCell *pCell; CString string;
	string.Format(_T("%c"), 0x01);
	for (int row = selection.GetMinRow(); row <= selection.GetMaxRow(); row++) 
	{
		for (int col = 0; col < GetColumnCount(); col++)
		{
...
BOOL CfxGrid::IsValidDrop(COleDataObject*pDataObject)
{
	...
	wchar_t* pData = new wchar_t[::GlobalSize(hmem)];
...
BOOL CfxGrid::DropRowAction(COleDataObject* pDataObject)
{
	...
	while (!strLine.IsEmpty())
	{
		...
		if (m_idDrag.row != -1)
			...
			fBool = InsertRow(_T("insert"), dropRow); // + nLine);
...
COLORREF CfxGrid::DisplayAttribute(int attr, CString& data)
{
	...
	{
		if (_wtof(string) == 0) string.Empty();
	}
	...
	case GVAT_FLOAT1:
		if (stringL >= 2)
			stringx.Format(_T("%s.%s"), string.Left(stringL -1), string.Right(1));
		else
			stringx.Format(_T("0.%s"), string);
		break;
	case GVAT_FLOAT2:
		if (stringL >= 3)
			stringx.Format(_T("%s.%s"), string.Left(stringL -2), string.Right(2));
		else
			stringx.Format(_T("0.%s"), string);
		break;
	case GVAT_FLOAT4:
		if (stringL >= 5)
			stringx.Format(_T("%s.%s"), string.Left(stringL -4), string.Right(4));
		else
			stringx.Format(_T("0.%s"), string);
		break;
	}
...
double CfxGrid::TOfabs(CString string)
{
	...
	dval = fabs(_wtof(string));
	return dval;
}


void CfxGrid::OnSetFocus(CWnd* pOldWnd) 
{
	MessageToParent(m_idCurrent.row, m_idCurrent.col, GVNM_SETFOCUS);
	CWnd::OnSetFocus(pOldWnd);	
}


void CfxGrid::DrawBong(CDC *pDC, CRect rect, double value[], COLORREF clrUP, COLORREF clrDN)
{
	double dOpen = value[0];
	double dHigh = value[1];
	double dLow = value[2];
	double dLast = value[3];
	double dHighLimit = value[4];	// 상한가
	double dLowLimit = value[5];	// 하한가
	...
	// Draw 기준선
	CPen penMiddle;
	penMiddle.CreatePen(PS_SOLID, 1, RGB(128, 128, 128));
	CPen* ppenOld = pDC->SelectObject(&penMiddle);
	pDC->MoveTo(rect.CenterPoint().x, rect.top + 1);
	pDC->LineTo(rect.CenterPoint().x, rect.bottom - 1);
	...
	// 영역계산
	double dDiff = dHighLimit - dLowLimit;
	...
	CPoint 	pt[11];	
	pt[0].x = (int)((double)rect.left + (((dLow - dLowLimit) * nWidth) / dDiff));
	pt[0].y = rect.top + (nHeight / 2);

	if (bUp)
	{
		pt[1].x = (int)((double)rect.left + (((dOpen - dLowLimit) * nWidth) / dDiff));
		pt[4].x = (int)((double)rect.left + (((dLast - dLowLimit) * nWidth) / dDiff));
	}
	else
	{
		pt[1].x = (int)((double)rect.left + (((dLast - dLowLimit) * nWidth) / dDiff));
		pt[4].x = (int)((double)rect.left + (((dOpen - dLowLimit) * nWidth) / dDiff));
	}
	...
	pt[5].x = (int)((double)rect.left + (((dHigh - dLowLimit) * nWidth) / dDiff));
	pt[5].y = pt[0].y;
