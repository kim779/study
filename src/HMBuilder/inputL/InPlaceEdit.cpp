					...
					CString sInitText, int limit, UINT nFirstChar, LOGFONT* logfont)
{
	...
	if (!CreateEx(NULL, _T("EDIT"), _T(""), dwEditStyle, rect, parent, nID))
		return;
...
void CInPlaceEdit::OnPaint() 
{
	...
	if (IsWindowEnabled())
	{
		...
		if (m_bSetFocus)
		{
			...
			m_caretPos.y = (rc.Height() - dc.GetTextExtent(_T("A")).cy) / 2;
...
void CInPlaceEdit::drawSelRect(CDC *pDC, CRect rc, CString str)
{
	...
	rc.top = (rc.Height() - pDC->GetTextExtent(_T("A")).cy) / 2 + 1;
	rc.bottom = rc.top + pDC->GetTextExtent(_T("A")).cy;
