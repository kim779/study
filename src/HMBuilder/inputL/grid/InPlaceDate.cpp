...
CInPlaceDate::CInPlaceDate(CWnd* parent, CRect& rect, UINT nid, CString date, int row, int col, CString format)
{
	...
	m_date = new CDateTime(); GetWindowRect(&rect); ScreenToClient(&rect);
	...
	m_date->SetRange((COleDateTime*) NULL, (COleDateTime*) NULL);

	if (!date.IsEmpty() && date.GetLength() >= 8)
	{
		int year = _wtoi(date.Mid(0, 4));
		int mon  = _wtoi(date.Mid(4, 2));
		int day  = _wtoi(date.Mid(6, 2));

		CTime time(year, mon, day, 23, 59, 59);
...
{
	COleDateTime time = m_date->GetOleDateTime();
	CString msg; msg.Format(_T("%d %d %d"), time.GetYear(), time.GetMonth(), time.GetDay());
}
...
void CInPlaceDate::OnUserString(NMHDR* pNotifyStruct, LRESULT* result)
{
	...
	if (!string.CompareNoCase(_T("KILLFOCUS"))
		|| !string.CompareNoCase(_T("VK_TAB"))
		|| !string.CompareNoCase(_T("VK_RETURN")))
	{
		EndDate();
	}
	else if (!string.CompareNoCase(_T("VK_ESCAPE")))
	{
...
{
	COleDateTime time = m_date->GetOleDateTime();
	CString string; string.Format(_T("%04d/%02d/%02d"), time.GetYear(), time.GetMonth(), time.GetDay());
...
CInPlaceTime::CInPlaceTime(CWnd* parent, CRect& rect, UINT nid, CString time, int row, int col, CString format)
{
	...
	m_time = new CDateTime(); GetWindowRect(&rect); ScreenToClient(&rect);
	...
	m_time->SetRange((COleDateTime*) NULL, (COleDateTime*) NULL);

	if (!time.IsEmpty() && time.GetLength() >= 6)
	{
		time.Remove(':');
		int hh = _wtoi(time.Mid(0, 2));
		int mm = _wtoi(time.Mid(2, 2));
		int ss = _wtoi(time.Mid(4, 2));

		CTime date(2001, 2, 11, hh, mm, ss);
...
void CInPlaceTime::OnUserString(NMHDR* pNotifyStruct, LRESULT* result)
{
	...
	else if (!string.CompareNoCase(_T("VK_TAB")) || !string.CompareNoCase(_T("VK_RETURN")))
	{
		EndTime();
	}
}
...
void CInPlaceTime::EndTime()
{
	...
	CString string; string.Format(_T("%02d:%02d:%02d"), time.GetHour(), time.GetMinute(), time.GetSecond());
