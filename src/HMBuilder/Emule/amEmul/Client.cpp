...
//		return false;
	...
	if (!equal)
	{
		...
		m_mapN = maps;
		screen->Parse();	// Lua init
		OnResize(m_rect.Width(), m_rect.Height());
		m_guard->SendAxis(MAKEWPARAM(titleVIEW, m_key), (LPARAM)screen->m_mapH->caption);
...
void CClient::OnWrite(CScreen* screen, CamBase* form, CString text)
{
	CString	string;
...
#ifdef _DEBUG
	m_pbitmap = ::new Bitmap(tmps.AllocSysString());
#else
	m_pbitmap = new Bitmap(tmps.AllocSysString());
#endif
	if (update)
...
void CClient::SetBK(ARGB argb, bool update)
{
	if (argb != m_bkground)
	{
		FreeBK(false);
...
void CClient::SetBK()
{
	...
	if (value <= 0 || text.IsEmpty())
		m_bkground = GRGB(GetSysColor(COLOR_BTNFACE));
	else
	{
		value = atoi(text);
		if (value & 0x02000000)
...
bool CClient::GetFileExist(CString sName)
{
	CFileFind finder;
	CString sRoot, ss;
...
bool CClient::SetObject(CScreen* main, int key, char *mapN)
{
	...
	if (!GetFileExist(mapN))
		return false;

	m_sync.Lock();
	CamBase* form = main->GetAtForm(key);
	...
	float ratio = (float)screen->m_mapH->resolution / 100.0;
	float hr = ((float)main->m_rect.Width() / (float)main->m_mapH->width) * ratio;
	float vr = ((float)main->m_rect.Height() / (float)main->m_mapH->height) * ratio;
...
void CClient::GetKeyObject(CScreen* screen, CamBase*& form)
{
	CScreen* main;
...
void CClient::OnFocus(BOOL focus, HWND hWnd)
{
	...
	if (m_status & S_LOAD)
	{
		...
		if (!focus)
		{
			...
			{
				int	result;
				CamBase* form = screen->GetAtForm(m_mouse->m_hover.m_idx);
...
void CClient::OnFocus(CScreen* screen, int idx, bool event)
{
	...
	if (GetAtScreen(screen, caret.m_key))
	{
		...
		CamBase* form = screen->GetAtForm(caret.m_idx);
...
bool CClient::isVisible(CScreen* screen, CamBase* form)
{
	CScreen* main;
...
void CClient::SaveDomino()
{
	...
	for (idx = 0; idx < m_magic; idx++)
	{
		...
		for (int ii = 0; ii < screen->m_mapH->formN; ii++)
		{
			...
			case FM_GRID:
			case FM_GRIDEX:
//				cell = (struct _precellR *) &screen->m_cellR[form->m_form->vals[0]];
//				nCols = HIWORD(form->m_form->vals[1]);
//				nRows = LOWORD(form->m_form->vals[1]);

				pcell = screen->m_MGRID[form->m_form->name];	//&screen->m_vCELL;
				...
				for (jj = 0; jj < nCols; jj++)
				{
					...
					for (kk = 0; kk < nRows; kk++)
					{
						...
						name += tmps;
						name = name + form->GetSymbolName(jj);//cell[jj].name);
...
//			pcell = &screen->m_vCELL;
			...
			for (ii = 0; ii < nCols; ii++)
			{
				...
				for (kk = 0; kk < nRows; kk++)
				{
					...
					name = form->GetSymbolName();
					name += tmps;
					name = name + form->GetSymbolName(ii);	//(pcell[ii].name);
...
{
	switch (screen->m_vFORM[idx]->kind)
	{
	case FM_EDIT:
	case FM_COMBO:
	//case FM_MEMO:
		switch (screen->m_vFORM[idx]->type)
		{
		case IO_HAN:
			m_status |= S_HAN;
			break;
...
bool CClient::RunMenu(CScreen* screen, CamBase* form, CPoint point)
{
	CScreen* main;
	if (!GetAtScreen(main))
		return false;
		...
		case FM_TABLE:
			...
			if (m_guard->GetString(AS_2XLS, tmps))
			{
				if (form->m_form->kind == FM_TABLE
					|| (form->m_form->kind == FM_GRID /*&& !(form->m_form->attr2 & GO_XXLS)*/ && ((CamGrid *)form)->GetValidRows() > 0))
				{
					if (++index > 0)
					{
						pos++;
...
void CClient::ClearShift()
{
	...
	{
		CamBase* form = screen->GetAtForm(m_cbox.m_idx);
		form->ClearData(true);
	}
}
...
bool CClient::isNext(CScreen* screen, int key)
{
	...
	if (form->m_form->kind != FM_GRID || CString(form->m_form->str).IsEmpty())//form->m_form->vals[2] == NOVALUE)
		return false;

	CString	text = form->m_form->str;	//(char *)form->m_form->vals[2];
...
bool CClient::SetCode(CScreen* screen, CamBase* form, CString& codes, bool force)
{
	int	row, col;
	...
	codes = form->GetSymbolName(col);	//(*pcell)[col]->name);
	codes += '\t';
	codes += text;	codes += '\n';
...
bool CClient::GetDragCell(CScreen* screen, CamBase* form, int& col, bool force)
{
	/*
	CPtrArray*	grid;
...
/*	
	...
	{
		if (grid != NULL)
			cell = (struct _precellR *)grid->GetAt(col);
...
// Declaretion 
void CClient::OnDeclaretion(CScreen* screen)
{
	int	parent = screen->m_key;

	m_vm->OnDeclaretion(screen);
	for (int key = 0; key < m_magic; key++)
	{
		if (GetAtScreen(screen, key) && !screen->isMain() &&
						screen->m_parent == parent)
...
void CClient::MakeFile(CScreen* screen, CamBase* form, CString path, bool head, bool append)
{
	CString	text = _T("");
...
void CClient::MakeXlsFile(CScreen* screen, CamBase* form, bool visible, CString path, bool head)
{
	CString	text;
...
void CClient::Export(CScreen* screen, CamBase* form, bool visible, CString data)
{
	int	offs;
...
void CClient::OnPrint(CScreen* screen, CamBase* form, bool print)
{
	return;
	...
	//		{
	//			if (grid != NULL)
	//				cell = (struct _precellR *)grid->GetAt(kk);
	...
	//	return;
	//}
	//m_guard->OnPrint(m_string + text, form->GetSymbolName(), form->m_form->fName/*screen->GetString(form->m_form->fName)*/, form->m_form->fPoint);
}
...
BOOL CClient::SetFont(int point, bool resize, bool attach)
{
	...
	if (resize && GetAtScreen(screen))
	{
		...
		struct _fontR font;
	//	ZeroMemory(&font, sizeof(struct _fontR));
		//strcpy_s(font.name, fonts.GetLength() + 1, );
		font.name = fonts;
		font.bold = false;
		font.point = point;
		font.italic = false;
				
		m_guard->m_pdraw->SetFont(&font);
		dc->GetOutputTextMetrics(&tm);
		width  = tm.tmAveCharWidth;
		height = tm.tmHeight;

		font.point = screen->m_mapH->fPoint;
		m_guard->m_pdraw->SetFont(&font);
		dc->GetOutputTextMetrics(&tm);
...
int CClient::LookTips(CPoint point, CString& text)
{
	...
	for (POSITION pos = m_tips.GetStartPosition(); pos; )
	{
		...
		case P_OUT:	// OUT
			break;
		case P_IN:	// IN
			if (form->m_form->kind != FM_GRID && form->m_form->kind != FM_GRIDEX && form->m_form->kind != FM_TABLE)
			{
...
void CClient::EditColumns(CScreen* screen, CamBase* form)
{
	//if (form->m_form->vals[0] != NOVALUE)
	{
		int	index, idx;
...
//		pcell = &screen->m_vCELL;
		...
		{
			text += form->GetSymbolName(idx);	//cell[idx].name);
			text += '\t';
			text += (CString((*pcell)[idx]->head).IsEmpty()) ? "HEAD ?" : CString((*pcell)[idx]->head);	//screen->GetString(cell[idx].head);
			text += '\t';

			if ((*pcell)[idx]->properties & PR_VISIBLE && index == 0)
			{
				tmps += form->GetSymbolName(idx);	//(*pcell)[idx]->name);
				tmps += '|';
			}
		}
		...
		for (idx = form->m_form->type; idx < nCols; idx++)
		{
			if (!((*pcell)[idx]->properties & PR_VISIBLE))
				continue;

			defs += form->GetSymbolName(idx);	//cell[idx].name);
			defs += '|';
		}
...
CString CClient::GetFormat(CScreen* screen, CamBase* form)
{
	...
	vector<struct _cellR*>* pvcell;
	struct _cellR* pcell = NULL;
	//struct _precellR	*cell;
	...
	//cell = (struct _precellR *) &screen->m_cellR[form->m_form->vals[0]];
//	pvcell = &screen->m_vCELL;
	pvcell = screen->m_MGRID[form->m_form->name];


	for (int kk = 0; kk < nCols; kk++)//, /*pcell++*/)
	{
		if (grid != NULL)
			pcell = (struct _cellR *)grid->GetAt(kk);
		else
			pcell = (*pvcell)[kk];
...
// wParam = (screen Key, target), lParam = TR msg
void CClient::LuaSend(WPARAM wParam, LPARAM lParam)
{
	// screen Key
	CString screenKey, sData;
	screenKey = (LPCSTR)lParam;	
	sData = screenKey.Mid(screenKey.Find("=") + 1);
	screenKey = screenKey.Left(screenKey.Find("="));

	CScreen* screen;
	if (m_screen.Lookup(atoi(screenKey), screen))
	{
		screen->LuaSend((long)HIWORD(wParam), sData);
	}
}

// wParam = (screen Key, target), lParam = TR msg
/*
void CClient::LuaGetObj(WPARAM wParam, LPARAM lParam)
{
	//luabind::object obj;
	lua_State* L;
	CScreen *screen1, *screen2;
	
	int mKey, sKey;
	int bParent = (int)HIWORD(lParam);
	int currKey = (int)HIWORD(wParam);
	int subKey  = (int)LOWORD(lParam);
	if (bParent)
	{
		mKey = currKey; sKey = currKey-1;
	}
	else
	{
		mKey = currKey; sKey = subKey;
	}

	if (sKey < 0)
		return;
	
	if (m_screen.Lookup(mKey, screen1))
	{	
		if (m_screen.Lookup(sKey, screen2))
		{
			L =  screen2->LuaGetObj();
			screen1->setParentObj(L);
		}	
	}
}
*/
void CClient::LuaGetObj(WPARAM wParam, LPARAM lParam)
{
	...
	int currKey = (int)HIWORD(wParam);
	int subKey = (int)lParam;
	
	if (m_screen.Lookup(currKey, screen1))
	{	
		if (m_screen.Lookup(subKey, screen2))
		{
			L =  screen2->LuaGetObj();
			screen1->setParentObj(L);
		}	
...
// 0: 자신, 1: main
void CClient::LuaGetName(WPARAM wParam, LPARAM lParam)
{	
	int isMain = (int)HIWORD(lParam);	// target
	int currKey = (int)HIWORD(wParam);	// 호출한 곳의 Key
	...
	if (isMain)	// Main : 1 이면
	{
		if (m_screen.Lookup(0, screen))
			formName = screen->m_mapH->mapN;
	}
	
	if (m_screen.Lookup(currKey, screen))
		screen->setFormName(formName);
}

// 0: 자신, 1: main
void CClient::LuaGetMain(WPARAM wParam, LPARAM lParam)
{		
	CScreen *screen, *screen1;
	int currKey = (int)HIWORD(wParam);	// 호출한 곳의 Key
	int subKey = (int)lParam;		// 0이면 Main, other면 SubForm

	if (m_screen.Lookup(currKey, screen))
	{
		if (m_screen.Lookup(subKey, screen1))		
			screen->setFormMain(screen1);
	}
}

void CClient::LuaSystem(WPARAM wParam, LPARAM lParam)
{
	int sKey;
	CString strKey, strData = _T("Data Empty.");
	CScreen *screen;
	...
	case 12:		// Push
		...
		strKey = strData.Left(strData.Find("/"));
		strData = strData.Mid(strData.Find("/") + 1);

		m_mapLuaPushData.SetAt(strKey, strData);
		break;
	case 13:		// Pop
		strKey = (LPCTSTR)lParam;
		sKey = atoi(strKey.Left(strKey.Find("/")));
		strKey = strKey.Mid(strKey.Find("/") + 1);

		if (m_screen.Lookup(sKey, screen))
		{
			int aa = m_mapLuaPushData.GetCount();

			if (m_mapLuaPushData.GetCount())
				m_mapLuaPushData.Lookup(strKey, strData);
			
			screen->SetPopData(strData);
		}
		break;
	}
