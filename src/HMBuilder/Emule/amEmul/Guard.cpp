...
//	AfxGetMainWnd()->GetWindowText(m_keys);
	...
	//AfxGetApp()->SetRegistry(m_keys);
	HKEY hKey;
	DWORD dwBufLen;
			
	CString sKey;
	CString sData;
	sKey.Format("Software\\%s\\Axis Workstation V03.00\\ENVIRONMENT\\", m_keys);

	RegOpenKeyEx( HKEY_CURRENT_USER, sKey, 0, KEY_QUERY_VALUE, &hKey );

	RegQueryValueEx( hKey, "ROOTDIR", NULL, NULL,
		(LPBYTE) sData.GetBuffer(100), &dwBufLen);
...
//	sData.Format("%s\\worksho.ini", sPath);
...
//	m_root = AfxGetApp()->GetProfileString(ENVIRONMENT, ROOTDIR, "");	
	...
	/*
	if (!m_sock->CreateControl("AxisSock.SockCtrl.KTB64", NULL, 0, CRect(0), control, 0))
		return -1;
...
//				(BYTE *) VTS_BSTR VTS_I8 VTS_I4 VTS_BOOL, m_ips, datB, datL, TRUE);
	
	((CMSocktCtrl*)m_sock)->WriteDgram(m_ips, (LONG_PTR)datB, datL, TRUE);  
}
...
void CGuard::PushData(CamBase* form, CString keys, int row, int column)
{
	CString	text;
...
void CGuard::PopData(CamBase* form, CString keys, int row, int column)
{
	CString	text;
...
//		return false;
	...
	GetPrivateProfileString("Environment", "Emul_Path", "", str.GetBuffer(256), 256, sRoot); 
	str.ReleaseBuffer();
	...
	fileH.Read(sFile.GetBuffer(mapL), mapL);
	sFile.ReleaseBuffer();
	sFile.Replace(12, 0x08);
	string ss;
	ss.append(sFile);

	vector<string> sDeque; 
//	boost::regex_split(std::back_inserter(sDeque), ss, boost::regex(cClass.GetBuffer(0)));
	Split(sDeque, ss, mCLASS);

	((struct _mapH*)mapH)->width  = ATOI(sDeque[3]);
	((struct _mapH*)mapH)->height = ATOI(sDeque[2]);
...
CString CGuard::PathMAP(CString mapN, bool force)
{
	...
	if (mapN.Mid(L_SGID+L_SELC).CompareNoCase(_AXUS_))
	{
		if (force)
		{
			pathN.Format("%s\\%s\\%s", m_root, FORMDIR, mapN.Left(L_SGID));
		...
		pathN.Format("%s\\%s\\%s\\%s\\%s", m_root, FORMDIR,
...
bool CGuard::RouteTR(CClient* client)
{
	...
	if (!Write(client->m_stream->m_sndB, client->m_stream->m_sndL, false))
	{
		...
		xTRACE(x_STRs, _T("(ERROR)Mobile Builder does not connected to the Server.\n"));
		return false;
	}
...
BOOL CGuard::Approve(CScreen* screen, int key, CString ip, CString map, char* datB, int datL)
{
	...
	BOOL	retv = TRUE;
//	m_sock->InvokeHelper(DI_DWRITE, DISPATCH_METHOD, VT_BOOL, (void*)&retv,
//				(BYTE *)(VTS_BSTR VTS_I8 VTS_I4 VTS_BOOL), ip, sendB, L_axisH+datL, FALSE);

	((CMSocktCtrl*)m_sock)->WriteDgram(ip, (LONG_PTR)sendB, L_axisH+datL, FALSE);
	delete [] sendB;
	return retv;
}
...
//						(BYTE *)(VTS_I8 VTS_I4), pBytes, nBytes);
	return ((CMSocktCtrl*)m_sock)->WriteStream((LONG_PTR)pBytes, nBytes);
...
int CGuard::OpenView(CWorks* works, CString maps, int type, int pos)
{
	...
	if (find)
	{
		...
		if (screen)
		{
			CamBase* form = screen->GetAtForm(((CClient *)works)->m_mouse->m_mouse.m_idx);
...
void CGuard::SetProfit(CScreen* screen, CamBase* form, CProfit*& profit)
{
	CString	tmps, text;
	char	twb[256];
...
//		cell = (struct _precellR *) &screen->m_cellR[form->m_form->vals[0]];
		...
		{
			if (!text.CompareNoCase(form->GetSymbolName(idx/*cell[idx].name*/)))
			{
				if (sum == '#')
...
void CGuard::xTRACE(int type, char* trace, int size)
{
	...
	::AfxGetMainWnd()->SendMessage(WM_TRACE, MAKEWPARAM(size, type), (LPARAM)trace);
}
...
//						(BYTE*)VTS_I8 VTS_I4 VTS_I4, NULL, NULL, enumQUEUE);
	
	((CMSocktCtrl*)m_sock)->GetEnum(NULL, NULL, enumQUEUE);
	return value;
}
...
void CGuard::LuaSend(WPARAM wParam, LPARAM lParam)
{
	CWorks *pwork;
//	CClient *pclient;
	
	if (m_clients.Lookup((int)LOWORD(wParam), pwork))
	{
		((CClient *)pwork)->LuaSend(wParam, lParam);
	}		
}

// wParam = (client Key, screen Key), lParam = Lua_State
void CGuard::LuaGetObj(WPARAM wParam, LPARAM lParam)
{
	CWorks *pwork;
//	CClient *pclient;
	...
	if (m_clients.Lookup((int)LOWORD(wParam), pwork))
	{
		((CClient *)pwork)->LuaGetObj(wParam, lParam);
	}		
}

// wParam = (client Key, screen Key), lParam = Lua_State
void CGuard::LuaGetName(WPARAM wParam, LPARAM lParam)
{
	CWorks *pwork;
//	CClient *pclient;
	...
	if (m_clients.Lookup((int)LOWORD(wParam), pwork))
	{
		((CClient *)pwork)->LuaGetName(wParam, lParam);
	}		
}

// wParam = (client Key, screen Key), lParam = Lua_State
void CGuard::LuaGetMain(WPARAM wParam, LPARAM lParam)
{
	CWorks *pwork;
//	CClient *pclient;
	...
	if (m_clients.Lookup((int)LOWORD(wParam), pwork))
	{
		((CClient *)pwork)->LuaGetMain(wParam, lParam);
	}		
}

// Lua System
void CGuard::LuaSystem(WPARAM wParam, LPARAM lParam)
{
	CWorks *pwork;
//	CClient *pclient;
	...
	if (m_clients.Lookup((int)LOWORD(wParam), pwork))
	{
		((CClient *)pwork)->LuaSystem(wParam, lParam);
	}
}