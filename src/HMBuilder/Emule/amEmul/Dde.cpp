...
int CDde::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	//	m_guard = ((CWizardApp *)AfxGetApp())->GetGuard();
		...
		GetPrivateProfileString("Informations", "Rate", "", tmpb, sizeof(tmpb), m_profile);
		text = tmpb;
		m_rate.RemoveAll();
		for ( ; !text.IsEmpty(); )
		{
...
void CDde::OnAlert(CString topic, CString item)
{
	...
	m_sync.Lock();
	topic.MakeUpper();
	if (topic.GetLength() == m_guard->m_CODEs+1 && topic.Find('A') == 0)
		topic.Delete(0, 1);
...
void CDde::OnAlert(CString topic, CdataSet* fms)
{
	if (m_dde != startDDE)
		return;
...
void CDde::OnAlert(CString text)
{
	...
	topic = text.Left(idx++);
//
//	"topic" has been checked on the previous phase
//
//	if (topic.GetLength() == m_guard->m_CODEs+1 && topic.Find('A') == 0)
//		topic.Delete(0, 1);
	if (!m_post.Lookup(topic, (CObject*&)post))
...
void CDde::OnTimer(UINT_PTR nIDEvent) 
{
	switch (nIDEvent)
	{
	case TM_DDE:
