...
LRESULT CALLBACK CallProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ERASEBKGND:
		{
			...
			Graphics g(tDC.GetSafeHdc());
			if (client->m_pbitmap)	
			{
				pBitmap = client->m_pbitmap;
#ifdef _DEBUG
				TextureBrush *pBr = ::new TextureBrush(pBitmap, WrapModeTile);
#else
				TextureBrush *pBr = new TextureBrush(pBitmap, WrapModeTile);
#endif
				//rect.InflateRect(1, 1);
				g.FillRectangle(pBr, RECTGA(rect));
			...
			{				
				...
				pbr = new SolidBrush(client->m_bkground);//(client->m_bkground));
				rect.InflateRect(1, 1);
	...
	case WM_USER:
		...
		{
		case getPALETTE:
			return client->m_guard->m_palette->GetPaletteARGB((int)lParam);
		case invokeTRx:
		...
		case getFONT:
			return (LRESULT)client->m_guard->m_pdraw->SetFont((struct _fontR *)lParam);
		case getPEN:
			return (LRESULT)client->m_guard->m_pdraw->SetPen((struct _penR *)lParam);
		case getBRUSH:
			return (LRESULT)client->m_guard->m_pdraw->SetBrush((COLORREF)lParam);
		case getBITMAP:
			return (LRESULT)client->m_guard->m_pdraw->SetImage((char *)lParam);
		case encPASSx:
			text = (char *)lParam;
			if (!text.IsEmpty())
			{
