...
LRESULT CALLBACK DllProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_USER:
		...
		{
		case getPALETTE:
			return dll->m_guard->m_palette->GetPaletteARGB((int)lParam);
		case invokeTRx:
		...
		case getFONT:
			return (LRESULT)dll->m_guard->m_pdraw->SetFont((struct _fontR *)lParam);
		case getPEN:
			return (LRESULT)dll->m_guard->m_pdraw->SetPen((struct _penR *)lParam);
		case getBRUSH:
			return (LRESULT)dll->m_guard->m_pdraw->SetBrush((COLORREF)lParam);
		case getBITMAP:
			return (LRESULT)dll->m_guard->m_pdraw->SetImage((char *)lParam);
		case encPASSx:
			text = (char *)lParam;
			if (!text.IsEmpty())
			{
