...
void CamCombo::SetData(LPCTSTR lpszNewValue) 
{
	...
	Split(vDis, string((LPSTR)lpszNewValue), ',');
//	int istrLen;
	for (int ii = 0; ii < vDis.size(); ii++)
	{
		string ss = vDis[ii];
		if (!ss.empty())
		{
			...
			string data = ss.substr(0, m_form->size);
			ss = ss.substr(m_form->size);
			m_datas.Add(data.data());
			m_displays.Add(ss.data());
		}
	}	
