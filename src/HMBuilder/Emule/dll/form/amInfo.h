...
public:
		...
		: CiBase(pinfo, pscript, psub, pMAP)
	{
		...
		for (ii = mSTR ; ii < m_pInfo->size(); ii++)
		{
			ss = (*m_pInfo)[ii];
			algorithm::trim(ss);
			nLen = ss.length() + 1;
		...
		: CiBase(pinfo, pscript, psub, pMAP)
	{
		...
		//	Msg("%s %s", pch[ii - mSIZE], ss.data());
			...
			case mLINECOLOR:
				...
				break;
			case mRTSSYMBOL:
				m_pForm->rtssy = new char[nLen];
				strcpy_s(m_pForm->rtssy, nLen, ss.data());
				break;
			case mPIXFONT:
				break;
			case mALPHA:
...
{
public:
	CiGrid(vector<string>* pinfo, vector<string>* pscript, vector<string>* psub, map<string, char*>* pMAP, vector<struct _cellR*>* pCell, int kind)
		: CiBase(pinfo, pscript, psub, pMAP)
	{
		m_pCELL = pCell;
		UINT ii = 0;
...
//				m_pForm->spRGB = ATOL(ss);
			...
			case mTIN:
				break;
			case mFROW:
				m_pForm->fixrow = ATOI(ss);
				break;
			case mFCOL:
				m_pForm->type = ATOI(ss);
				break;
			case mHHEIGHT:
				m_pForm->headH = ATOI(ss);
				break;
			case mROWOF:
...
public:
	...
	struct _cellR* MakeSub(vector<string>* pDeque)
	{
		...
		char* pch[] = 
		{
			...
			"cmDNCOL",
			//"cmKIND",
			"cmCOUNT",		// keys;		// COMBO data count
			"cmSTR",		// str[256];		// COMBO data string
			"cmDAT",		// dat[512];		// COMBO display string
			"cmSTR2",		// str2[256];		// CHECK  display string
			"cmTXT",		// txt[256];		// BUTTON string
			"cmIMAGE",		// str3[512];	// image file name
			"cmRTSSYMBOL",
			"cmKIND",
			"cmFNAME",
			"cmFPOINT",
		...
		//	Msg("%s %s", pch[ii-cmNAME], ss.data());
			...
			case cmDNCOL:
				...
				break;
			case cmKIND:	// Cell Kind
				/*
				if (ss.compare("NOMAL") == 0)
					pCell->kind = CK_NORMAL;
				else if (ss.compare("CHECK") == 0)
					pCell->kind = CK_CHECK;
				else if (ss.compare("COMBO") == 0)
					pCell->kind = CK_COMBO;
				else if (ss.compare("BUTTON") == 0)
					pCell->kind = CK_BUTTON;
				else if (ss.compare("CHART") == 0)
					pCell->kind = CK_CHART;
				*/
				pCell->kind = ATOI(ss);
				break;
			case cmCOUNT:		// COMBO data count
				pCell->keys = ATOL(ss);
				break;
			case cmSTR:		// COMBO data string
				strcpy_s(pCell->str, nLen, ss.data());
				break;
			case cmDAT:		// COMBO display string
				strcpy_s(pCell->dat, nLen, ss.data());
				break;
			case cmSTR2:		// CHECK  display string
				strcpy_s(pCell->str2, nLen, ss.data());
				break;
			case cmTXT:		// BUTTON string
				strcpy_s(pCell->txt, nLen, ss.data());
				break;
			case cmIMAGE:		// image file name
				strcpy_s(pCell->str3, nLen, ss.data());
				break;
			case cmRTSSYMBOL:
				strcpy_s(pCell->rtssy, nLen, ss.data());
				break;
			case cmFNAME:
				pCell->fName = new char[nLen];				
				strcpy_s(pCell->fName, nLen, ss.data());
				break;
			case cmFPOINT:
				pCell->fPoint = ATOI(ss);
				break;
			case cmFSTYLE:
				pCell->fStyle = ATOI(ss);
				break;
			default:
				break;
			}
