...
//		dc->FillRect(m_pRc, brush);
	...
	CString str = CString(m_form->name);
	g.FillRectangle(setgBrush(clWHITE), RECTG(m_pRc));

	CFont* oldfont;
	CFont*	font = setFont(dc);
	oldfont = dc->SelectObject(font);
	dc->DrawText(str, m_pRc, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
	//g.DrawString(str.AllocSysString(), -1, setgFont(), RECTF(m_pRc), &sf, setgBrush(GRGB(m_tRGB)));
