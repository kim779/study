...
LPCTSTR CamRadio::_GetGruopName() 
{	
	return m_axform->GetRadioGroupName(GetSymbolName());
}

LPCTSTR CamRadio::GetText() 
{
	CString strResult;
...
//	if (!(m_form->properties & PR_TRANSPARENT))
	...
	if (m_form->properties & (PR_IMAGE) && m_pic && m_pic->GetValid(m_image))
		m_pic->DrawPicture(g, cRc2, m_form->alignImage, m_image);
