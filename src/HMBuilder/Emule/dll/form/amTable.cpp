...
BEGIN_DISPATCH_MAP(CamTable, CamBase)
	...
	DISP_FUNCTION(CamTable, "GetPRgb", _GetPRgb, VT_I4, VTS_I4)
	DISP_FUNCTION(CamTable, "SetPRgb", _SetPRgb, VT_EMPTY, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamTable, "GetTRgb", _GetTRgb, VT_I4, VTS_I4)
	DISP_FUNCTION(CamTable, "SetTRgb", _SetTRgb, VT_EMPTY, VTS_I4 VTS_I4)
...
LPCTSTR CamTable::_GetHead(long row) 
{
	m_strResult = m_pairs.GetAt(row-1)->m_head;
...
LPCTSTR CamTable::_GetCell(long row) 
{	
	ReadData(m_strResult, false, -1, row-1);
	return (LPCTSTR)m_strResult;
}
...
LPCTSTR CamTable::_GetText(long row) 
{
	ReadData(m_strResult, true, -1, row-1);
	return (LPCTSTR)m_strResult;
}

long CamTable::_GetPRgb(long row) 
{
	return GetBkColor(0, row-1);
}

void CamTable::_SetPRgb(long row, long rgb) 
{
	SetBkColor(rgb, 0, row-1);
}

long CamTable::_GetTRgb(long row) 
{
	return GetFgColor(0, row-1);
}

void CamTable::_SetTRgb(long row, long rgb) 
{
	SetFgColor(rgb, 0, row-1);
}
...
void CamTable::SetBkColor(int rgb, int col, int row)
{
	...
	if (row >= m_totRows || 
		(((*m_cellR)[row]->attr & FA_CORGB) && 
		((*m_cellR)[row]->attr2 & SA_CORGB) == SA_PAINT ||
		((*m_cellR)[row]->attr2 & SA_CORGB) == SA_PAINTSUP))
		return;
...
void CamTable::WriteData(CString data, bool redraw, int col, int row)
{
	...
	if ((*m_cellR)[row]->attr & FA_FLOAT)
	{
		...
		{
			text = tmps.Left(pos);
			if ((*m_cellR)[row]->vals/*vals[1]*/ == (*m_cellR)[row]->size)
				tmps = text;
			else
			{
				str  = text.Right((*m_cellR)[row]->size - (*m_cellR)[row]->vals/*vals[1]*/);
				text = tmps.Mid(pos+1);
				text = text.Left((*m_cellR)[row]->vals/*vals[1]*/);
				if (str.IsEmpty())
					str += _T("0");
				str += CString('0', (*m_cellR)[row]->vals/*vals[1]*/ - text.GetLength());
				str += text;
				tmps = str;
			}
		}
		else if (!(tmps.IsEmpty() && ((*m_cellR)[row]->attr & FA_ZEROSUP)))
		{
			...
			*/
			...
			pos = tmps.GetLength() - (*m_cellR)[row]->vals/*vals[1]*/;
			if (pos <= 0)
...
//		tmpS.TrimRight();
	...
	formatData((*m_cellR)[index]->attr, tmpS, ((*m_cellR)[index]->vals/*vals[1]*/ == (*m_cellR)[index]->size) ? -1 : (int)(*m_cellR)[index]->vals/*vals[1]*/);
	return tmpS;
...
void CamTable::makeSignData(CString& data, int row, int col)
{
	...
	if ( !(((*m_cellR)[row]->attr2 & SA_CORGB) & SA_PAINT) ||
	     ...
	     !(((*m_cellR)[row]->attr2 & SA_CORGB) & SA_PAINTSUP) ||
	     !(((*m_cellR)[row]->attr2 & SA_CORGB) & SA_TEXTSUP))
		 return;
...
//		return;
	...
	// add Supress + Background 2014. 01. 29.
	switch ((*m_cellR)[row]->attr2 & SA_CORGB)
	{
	case SA_TEXT:
	case SA_PAINT:
