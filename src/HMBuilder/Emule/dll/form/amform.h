...
/////////////////////////////////////////////////////////////////////////////
// CAmForm command target

class AFX_EXT_CLASS CAmForm : public CCmdTarget
{
	DECLARE_DYNCREATE(CAmForm)

	CAmForm();           // protected constructor used by dynamic creation
	virtual ~CAmForm();
...
protected:
	CMap	<CString, LPCTSTR, int, int&>	m_radio;
...
public:
	...
	vector <struct _formR*>	m_vFORM;
	map	   <string, char*>	m_mSCRIPT;
	
	map	<string, vector <struct _cellR*>*>	m_MTABLE;
	map <string, vector <struct _cellR*>*>	m_MGRID;
	
	map <string, string> m_MTRINFO;
	map <string, string> m_LINKINFO;


//	struct	_precellR*		m_cellR;
//	struct	_pageR*			m_pageR;
	...
	//***************************************
	// GDI+ Ãß°¡ 2013-05-14 kjs
	//***************************************
	class	CAmDraw*	 m_pAmDraw;
	class   CAmPalette*	 m_pPalette;
...
public:
	...
	bool	LoadtrLayout(CString sPath, CString trinfo);
	void	SetRoot(CString sRoot){ m_root = sRoot; }

	CamBase* GetAtForm(int key);
	bool	FindForm(CString symbol, int& key);
	bool	FindForm(CString symbol, CamBase*& form);
	int		FindForm(CamBase* form);
	...
	void	SetRadioGroup(CString symbol, bool checked, int n = -1);
	CString	GetRadioGroupName(CString symbol);
	void	UnCheckRadioGroup(CString symbol);

	void	SetTabPage(int index, CamBase* tab);
	bool	IsFormVisible(CamBase* form);
	...
	virtual	long DoSomething(int type, CamBase* form, WPARAM wParam = 0, LPARAM lParam = 0)	{ return 0; }
	virtual	int  DoComboBox(CamBase* form)	{ return 0; }
	virtual	void DoProc(CString procs, CamBase* form, CString data, int count = -1)	{}
...
protected:
	...
	bool	DoInfo(CString sContorl, vector<string>* pScript, vector<string>* pSub);
