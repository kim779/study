// fmGridx.cpp: implementation of the CamGrid class.
//
//////////////////////////////////////////////////////////////////////
...
int CamGrid::insertRow(int index, CString str, bool dataOnly, BOOL skip)
{
	CString	tmps  = _T("");
	CString	tmps_ = _T("");
	int	offs  = 0;
	...
	if (!dataOnly && (m_form->attr2 & GO_ROW))
	{
		for (int ii = 0; ii < m_totCols; ii++)
		{
			...
			for (int jj = 0; jj < m_totRows; jj++)
			{
				...
				if (((*m_cellR)[col->m_ref]->attr & FA_CORGB) && 
					(((*m_cellR)[col->m_ref]->attr2 & SA_CORGB) == SA_PAINT ||
					((*m_cellR)[col->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP))
				{
...
CString CamGrid::editData(CString str, int col, int row, bool input)
{
	CString tmps, tmps_;
	int	ii,  len,  dcnt = 0;
	DWORD	attr;
...
//		tmps.TrimRight();
	...
	{
	// ???????????
	//	formatData(attr, tmps, ((*m_cellR)[column->m_ref]->vals[0] == (*m_cellR)[column->m_ref]->size) ? -1 : (int)(*m_cellR)[column->m_ref]->vals[1]);
		return tmps;
	}
	*/
	if (attr & FA_COMMA || attr & FA_FLOAT)
	{
		...
		if (attr & FA_FLOAT)
		{
			...
			{
				for (ii = 0; ii < (int)(*m_cellR)[column->m_ref]->vals; ii++)
				{
					if (ii > valS.GetLength()-1)
					{
						tmps += '0';
...
void CamGrid::makeSignData(CString& data, int col, int row)
{
	Ccolumn* column = m_columns.GetAt(col);
	if (!((*m_cellR)[column->m_ref]->attr & FA_CORGB) || data.IsEmpty())
		return;

	// add Supress + Background 2014. 01. 29.
	if (!(((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) & SA_PAINT) ||
		!(((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) & SA_TEXT) ||
		!(((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) & SA_PAINTSUP) ||
		!(((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) & SA_TEXTSUP))
		return;
	...
	// add Supress + Background 2014. 01. 29.
	switch ((*m_cellR)[column->m_ref]->attr2 & SA_CORGB)
	{
	case SA_PAINT:
	case SA_TEXT:
	...
	if ((*m_cellR)[column->m_ref]->onEdit != 0)
	{
		upRGB = HIWORD((*m_cellR)[column->m_ref]->onEdit);
		dnRGB = LOWORD((*m_cellR)[column->m_ref]->onEdit); 
	}
	else
	{
		upRGB = dnRGB = (*m_cellR)[column->m_ref]->tRGB;
	}
	...
	if (row >= 0)
	{
			...
			((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP)
		{
			...
			{
				setFgColor(row, col, getgINDEX(clWHITE));
			...
			else
			{
				...
				if (m_form->attr2 & GO_ROW)
					column->m_pRGBs.SetAt(row, ((row % (m_form->rowof * 2)) < m_form->rowof) ? m_sRGB : m_fRGB);
				else
				{
...
void CamGrid::doColumnEditCORGB(int row, int col, CString& data)
{
	Ccolumn* column = m_columns.GetAt(col);
...
// gdi+ 변경 완료
void CamGrid::drawEdit(CDC* dc, CRect rc, Ccolumn* column, int col, int row, CString data, COLORREF pRGB, COLORREF tRGB)
{
	Graphics g(dc->GetSafeHdc());

	Matrix mat(1, 0, 0, 1, (REAL)-rc.left, (REAL)-rc.top);
	Bitmap bitmap(rc.Width(), rc.Height());
	Graphics mg(&bitmap);
	mg.SetTransform(&mat);

	COLORREF tcf, pcf, bcf;		// Text C, Paint C, Brush C
	StringFormat sf;
...
//	CBrush*	brush;
	...
	if ((index <= column->m_enables.GetUpperBound())
	...
	mg.FillRectangle(setgBrush(GRGB(bcf)), RECTG(rc));
	...
	if (data.GetLength() != 0)
	{
		...
		mg.MeasureString(tmps.AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, &tr);
		size = CSize((int)tr.Width, (int)tr.Height);
		...
		if (size.cx > tRc.Width())
		{
			for (int ii = data.GetLength(); ii >= 0; ii--)
			{
				...
				mg.MeasureString(tmps.Left(ii).AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, &tr);
				size = CSize((int)tr.Width, (int)tr.Height);
				if (size.cx < tRc.Width())
				{
					tmps = data.Left(ii);
...
/*	
	if ((*m_cellR)[column->m_ref]->attr & FA_RIGHT)
	{
		...
		mg.DrawString(tmps.AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, setgBrush(GRGB(tcf)));
...
*/
	{
	mg.MeasureString(tmps.AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, &tr);
	mg.FillRectangle(setgBrush(GRGB(pcf)), tr);
		...
		if (focus_this)
		{
			...
			case esNormal :
				//size = dc->GetOutputTextExtent(tmps, calcCaret(data, column->m_edit));
				g.MeasureString(tmps.AllocSysString(), calcCaret(data, column->m_edit), setgFont(), RECTF(tRc), &tr);
				size = CSize((int)tr.Width, (int)tr.Height);
				break;
			case esNone :
			default :
...
//		else
	...
	g.DrawCachedBitmap(&cache, rc.left, rc.top);
}


void CamGrid::drawCombo(CDC* dc, CRect rc, Ccolumn* column, int col, int row, CString data, COLORREF pRGB, COLORREF tRGB)
{
	Graphics g(dc->GetSafeHdc());

	//Bitmap bitmap(rc.Width(), rc.Height());
	//Graphics mg(&bitmap);
	//mg.TranslateTransform((REAL)-rc.left, (REAL)-rc.top);
...
//	dc->FillRect(rc, brush);
	//RECT rcccc = RECTG(rc);
	g.FillRectangle(setgBrush(GRGB(bcf)), RECTG(rc));
	...
	if (combo < column->m_display.GetSize())
	{
		...
		//g.DrawString(tmps.AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, setgBrush(GRGB(m_tRGB)));
	...
	if (false)
	{
		...
		if (m_pic && m_pic->GetValid(combo))
		{
			...
			if (rBtn.left >= rc.left)
				m_pic->DrawPicture(dc, rBtn, IA_STRETCH, combo);
			else
				rBtn.left = rc.left;
		}
		else
		{
			...
			if (rBtn.left >= rc.left)
			{
				...
				g.FillRectangle(setgBrush(GRGB(bg)), RECTG(tRc));
				...
				g.DrawLine(setgPen(GRGB(bg)), rBtn.left, rBtn.top+1, rBtn.left, rBtn.bottom-1);
				g.DrawLine(setgPen(GRGB(bg)), rBtn.right-1, rBtn.top+1, rBtn.right-1, rBtn.bottom-1);
				...
				pts[0].Y = (REAL)tRc.bottom;
				pts[1].Y = (REAL)tRc.top;
				pts[2].Y = (REAL)tRc.top;
				pts[0].X = (REAL)tRc.left + (REAL)(tRc.Width()/2);
				pts[1].X = (REAL)tRc.right-1;
				pts[2].X = (REAL)tRc.left;
				//pts[0].y = tRc.bottom;
				//pts[1].y = tRc.top;
				//pts[2].y = tRc.top;
				...
				//pts[2].x = tRc.left;
				//dc->Polygon(pts, 3);
				g.DrawPolygon(setgPen(GRGB(fg)), pts, 3);
				g.FillPolygon(setgBrush(GRGB(fg)), pts, 3, FillModeAlternate);
	...
	else
	{
		...
		{	
			...
			Bitmap* pBitmap = DrawFrameControl(dc, rBtn, DFC_SCROLL, style);
			g.DrawImage(pBitmap, rBtn.left, rBtn.top, pBitmap->GetWidth(), pBitmap->GetHeight());
		}
		else
			rBtn.left = rc.left;
	}
...
void CamGrid::drawCheck(CDC* dc, CRect rc, Ccolumn* column, int col, int row, CString data, COLORREF pRGB, COLORREF tRGB)
{
	Graphics g(dc->GetSafeHdc());
...
//	setFont(dc, ((*m_cellR)[column->m_ref]->properties & PR_BOLD) ? 1 : 0);
	...
	//Bitmap* pBitmap = DrawFrameControl(dc, tRc, DFC_BUTTON, style);	
	...
	{
		iRc.left = tRc.right;
		column->m_pic->DrawPicture(g, iRc, 0, PI_DEFAULT);
	...
	if (rc.Height() < rc.Width())
	{
		...
		tmpRc.OffsetRect(max(index, 0), 0);
		dc->DrawText(column->m_edit, tmpRc, DT_SINGLELINE|DT_VCENTER|DT_LEFT|DT_NOPREFIX);
		//sf.SetLineAlignment(StringAlignmentCenter);
		//sf.SetFormatFlags(StringFormatFlagsLineLimit|StringFormatFlagsNoWrap);
		//sf.SetHotkeyPrefix(HotkeyPrefixNone);
		//sf.SetAlignment(StringAlignmentCenter);
		//g.DrawString(column->m_edit.AllocSysString(), -1, setgFont(), RECTF(tmpRc), &sf, setgBrush(GRGB(tcf)));
	}
//	CachedBitmap cache(&bitmap, &g);
//	g.DrawCachedBitmap(&cache, rc.left, rc.top);
}
...
void CamGrid::drawButton(CDC* dc, CRect rc, Ccolumn* column, int col, int row)
{
	Graphics g(dc->GetSafeHdc());

	/*Matrix mat(1, 0, 0, 1, (REAL)-rc.left, (REAL)-rc.top);
	Bitmap bitmap(rc.Width(), rc.Height(), &g);
	Graphics mg(&bitmap);
	mg.SetTransform(&mat);*/
...
//	CBrush*	brush;
	...
	{
		border = false;
		//column->m_pic->DrawPicture(g, rc, (*m_cellR)[column->m_ref]->alignC, PI_DEFAULT);
		column->m_pic->DrawPicture(g, rc, 0, PI_DEFAULT);
	...
	else //if (!((*m_cellR)[column->m_ref]->properties & PR_TRANSPARENT))
	{
		...
		//brush  = setBrush(dc, pRGB);
		//dc->FillRect(rc, brush);
		g.FillRectangle(setgBrush(GRGB(pRGB)), RECTF(rc));
	}
...
//			setPaintColor(dc, pRGB);
			...
			&& !column->m_enables.GetAt(index))
		{
			...
			//mg.DrawString(column->m_edit.AllocSysString(), -1, setgFont(), RECTF(rc), &sf, setgBrush(clWHITE));
			rc.left -= 1;
			rc.top  -= 1;
			setTextColor(dc, getRGB(cXFG));
			dc->SetBkMode(TRANSPARENT);
			dc->DrawText(column->m_edit, rc, format);
			//mg.DrawString(column->m_edit.AllocSysString(), -1, setgFont(), RECTF(rc), &sf, setgBrush(clGRAY));
		}
		else
		{
			setTextColor(dc, tRGB);
			dc->DrawText(column->m_edit, rc, format);
			//mg.DrawString(column->m_edit.AllocSysString(), -1, setgFont(), RECTF(rc), &sf, setgBrush(GRGB(tRGB)));
		}
	}
...
// GDI+ OK!
//void CamGrid::drawHead(CDC* dc, int col, CString data, CRect rc, StringFormat* sf, COLORREF tRGB)
void CamGrid::drawHead(CDC* dc, int col, CString data, CRect rc, UINT format, StringFormat* sf, COLORREF tRGB)
{
	Graphics g(dc->GetSafeHdc());
	...
	if (rc.Width() > size.cx)
	{
		...
		if (sf->GetAlignment() & StringAlignmentNear)
			sRc.right = sRc.left + ssize.cx;
		else if (sf->GetAlignment() & StringAlignmentFar)
		{
		...
		case 1:		// sASCEND
			...
			//dc->Polygon(pt, 3);
			g.FillPolygon(setgBrush(clBASIC), pt, 3, FillModeAlternate);
			...
			//dc->Polygon(pt, 4);
			g.FillPolygon(setgBrush(clSORTD), pt, 4, FillModeAlternate);
			...
			//dc->Polygon(pt, 6);						
			g.FillPolygon(setgBrush(clSORTL), pt, 6, FillModeAlternate);
			break;
		...
		case 2:		// sDESCEND
			...
			pt[5].X = pt[0].X;		pt[5].Y = pt[0].Y;
			g.FillPolygon(setgBrush(clSORTD), pt, 6, FillModeAlternate);
			//setPen(dc, getRGB(cSORTD));
			//setBrush(dc, getRGB(cSORTD));
			//dc->Polygon(pt, 6);

			pt[1] = pt[0];
			g.FillPolygon(setgBrush(clSORTL), pt, 4, FillModeAlternate);
			//setPen(dc, getRGB(cSORTL));
			//setBrush(dc, getRGB(cSORTL));
			//dc->Polygon(pt, 4);
			break;
		...
		tmps  = _T(" ");
		tmps += data;
		//g.DrawString(data.AllocSysString(), -1, setgFont(), RECTF(rc), sf, setgBrush(GRGB(tRGB)));
...
void CamGrid::drawPage(CDC* dc, CRect rc, int type, bool up, bool enable, bool flat)
{
	CPoint	pts[3];
	CBrush*	br;
	CRect	rect, tRc;
	...
	{
		tRc.OffsetRect(1, 1);
		line = m_slRGB < 0 ? _ARGB(255, 160, 160, 160) : getRGB(m_slRGB);
		fg   = m_ssRGB < 0 ? _ARGB(255, 119, 119, 119) : getRGB(m_ssRGB);
	}
...
void CamGrid::drawScroll(CDC* dc, CRect rc, int type, bool enable, bool pushed)
{
	Graphics g(dc->GetSafeHdc());
	...
	g.FillPolygon(setgBrush(GRGB(enable ? fg : line)), pts, 3, FillModeAlternate);	
	//dc->Polygon(pts, 3);

	//setPen(dc, line);
	//dc->SelectStockObject(NULL_BRUSH);
	//dc->Rectangle(rc);
	g.FillRectangle(setgBrush(GRGB(line)), RECTG(rc));
	g.DrawRectangle(setgPen(GRGB(line)), RECTF(rc)); // gdi+ 수정
	
}

void CamGrid::lbutton(bool down, CPoint pt, int& result)
{
	...
	if (down)
	{
		...
		case ckItem:
			...
			for (ii = m_headN*m_multiN; ii < rows + 1; ii++)
			{
				...
				if (m_form->headH != 0 && !(m_form->attr2 & GO_XHEAD))
					tmpRc1.OffsetRect(0, m_HeaderH - m_cy);
			...
			case stButton:
				...
				if ((m_cellpos.m_row-1 <= col->m_enables.GetUpperBound())
...
bool CamGrid::lbuttonEdit(bool down, CPoint pt, int& result)
{
...
bool CamGrid::lbuttonCombo(bool down, CPoint pt, int& result)
{
	CRect	tmpRect;
	CRect	tmpRc1,  tmpRc2;
...
bool CamGrid::lbuttonCheck(bool down, CPoint pt, int& result)
{
	CRect	tRc = m_cell;
	result = RC_NOP;
	if (down || (!down && m_statechange))
...
bool CamGrid::lbuttonButton(bool down, CPoint pt, int& result)
{
...
void CamGrid::rbutton(bool down, CPoint pt, int& result)
{
	result = RC_NOP;
	if (m_totCols <= 0)
		return;
...
void CamGrid::keyDelete(int& result)
{
	result = RC_NOP;
	if (m_state != stEdit)
		return;
...
void CamGrid::keyBackspace(int& result)
{
	result = RC_NOP;
	if (m_state != stEdit)
		return;
...
int CamGrid::keyArrowLeft(int& result)
{
	bool	update;
	CString	data;
	CRect	tRc;
...
void CamGrid::keyArrowRight(int& result)
{
	bool	update;
	CString	data;
	CRect	tRc;
...
void CamGrid::keyArrowUp(int& result)
{
	CRect	pRc;
	int	stRow, state;
	...
	case stEdit:
		...
		if (!column->m_enables.GetAt(m_curpos.m_row-1))
		{
			m_state = stFocus;
			m_edfocusing = true;
			m_edcaret    = false;
...
void CamGrid::keyArrowDown(int& result)
{
	CRect	pRc;
	int	stRow, state;
...
void CamGrid::keyReturn(int& result)
{
...
void CamGrid::keyEscape(int& result)
{
	m_select = false;
	m_move   = mvNone;
	keyReturn(result);
...
void CamGrid::keyPrior(int& result)
{
	result = RC_NOP;
...
void CamGrid::keyNext(int& result)
{
	result = RC_NOP;
	if (!(m_state == stNone || m_state == stFocus))
		return;
...
void CamGrid::keyHome(int& result)
{
	result = RC_NOP;
	if (!(m_state == stNone || m_state == stFocus))
		return;
...
void CamGrid::keyEnd(int& result)
{
	result = RC_NOP;
	if (!(m_state == stNone || m_state == stFocus))
		return;
...
void CamGrid::keyTab(int& result)
{
...
int CamGrid::findColumn(CString name)
{
	name.TrimLeft();
	name.TrimRight();
	for (int ii = 0; ii < m_totCols; ii++)
...
//		if (!name.CompareNoCase(GetSymbolName((*m_cellR)[m_columns.GetAt(ii)->m_ref]->name)))
		if (!name.CompareNoCase((*m_cellR)[m_columns.GetAt(ii)->m_ref]->name))		
			return ii;
	}
...
int CamGrid::findColumn(int ref)
{
	for (int ii = 0; ii < m_totCols; ii++)
	{
...
int CamGrid::findRow(int py, bool isrow)
{
	CRect	tRc;
	int	row, rows;
...
int CamGrid::findRow(int py, CRect& rc, CRect& rcx)
{
	CRect	tRc;
	int	row, rows;
...
int CamGrid::findStartColumn(int col)
{
	/*
...
bool CamGrid::findCell(bool back, int row, int col)
{
	bool	quit = false;
	DWORD	attr;
	Ccolumn* column;
...
bool CamGrid::findCell(CPoint pt, int& row, int& col, CRect& hRc, CRect& vRc)
{
	if (m_form->attr2 & GO_MULTI)
		return false;
...
bool CamGrid::setFgColor(int row, int col, int rgb)
{
	DWORD	color;
	Ccolumn* column;
...
bool CamGrid::setBkColor(int row, int col, int rgb)
{
	Ccolumn* column = m_columns.GetAt(col);
...
void CamGrid::setInformation(CString info)
{
	CString	item;
	int	offs   = 0;
...
void CamGrid::getInformation(CString& info)
{
	int	key;
	...
	info.Format("%02d%04d%d%d%d%-16s%d%04d%-80s",
						...
						(m_sortc < 0) ?
						"" : (*m_cellR)[m_columns.GetAt(m_sortc)->m_ref]->name,
...
void CamGrid::setSortInfo(CString info)
{
	int	sortc;
	CRect	tRc;
...
int CamGrid::validCell(int index, int col, int& ref)
{
	int	href, vref, rc = 0;
	CString	text, string;
...
bool CamGrid::getRect(int row, int col, CRect& rect)
{
...
bool CamGrid::makeListBox(int col, CRect rItem)
{
	if (m_list)
		return false;
...
CRect CamGrid::setRect(CDC* dc, int col, CRect rItem)
{
	int	value, count;
	CSize	size;
	CRect	rect, tRc, wRc;
...
int CamGrid::getStart(int index, int show, int count)
{
	int	start = index;
...
int CamGrid::getIndex(int col, int row, CString data, bool force)
{
	if (col < 0)
...
void CamGrid::setData(int index, int col, int row)
{
	if (col < 0)
		col = max(0, m_curpos.m_col-1);
	if (row < 0)
...
int CamGrid::calcCaret(CString data, CString edit)
{
...
int CamGrid::getRowIndex(CPoint pt)
{
	CRect	tRc;
	int	idx, rows;
...
int CamGrid::getVisibleRows()
{
	return (m_showRows - m_headN*m_multiN)/m_multiN;
}

bool CamGrid::getMovableColumn(CPoint pt)
{
	if (m_form->attr2 & GO_MULTI)
		return false;
...
bool CamGrid::isReverseCell(int row, int col)
{
...
void CamGrid::refreshSel(Cpos pos, int key)
{
...
void CamGrid::changeIME(int row, int col)
{
	DWORD	attr;
	Ccolumn* column;
	bool	han = false;
...
double CamGrid::compare(bool string, CString arg1, CString arg2, int col)
{
	if (string)
	...
	if (!arg1.IsEmpty())
	{
		...
		else if ((*m_cellR)[col]->attr & FA_CORGB)
		{
			switch (arg1[ii])
			{
			case '1':	// 상한
