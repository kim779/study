...
IMPLEMENT_DYNCREATE(CamGrid, CamBase)
...
CamGrid::CamGrid(CAmForm* axform, struct _formR* formR, /*struct _cellR**/vector  <struct _cellR*>* cellR)
	: CamBase(axform, formR)
{
	if (m_form->size == NOVALUE)
		m_form->size = 0;

	m_tRGB          = getgRGB(cBLACK);
	m_stRow         = 0;
	m_totCols       = 0;
	m_totRows       = 0;
	m_showRows      = 0;
	...
	*/
	if (m_form->spRGB != 0)
	{
		m_fRGB = GRGB(getgRGB(m_form->spRGB));
		m_sRGB = GRGB(getgRGB(m_form->stRGB));
	}
	else
	{
		m_fRGB = GRGB(clBASIC);
	...
	if ((m_form->isEdit == FI_RBAR || m_form->isEdit == FI_RREC) && m_form->onEdit != 0)
		m_rRGB = (m_form->onEdit & userCOLOR) ? m_form->onEdit : getgRGB(m_form->onEdit);
	else
		m_rRGB = getgRGB(cDBLUE);

	m_totCols = m_form->col;	//	HIWORD(m_form->vals[1]);
...
//	m_form->isHint = IH_NOP;
	...
	for (ii = 0; ii < m_totCols; ii++)
	{
		...
		if (!sHead.IsEmpty())
		{
			column->m_head = sHead;		//m_axform->GetString((*m_cellR)[ii]->head);
			if (m_form->attr2 & GO_MERGE)
			{
				tmps = column->m_head;
				for ( ; !tmps.IsEmpty(); )
		...
		CString sName = CString((*m_cellR)[ii]->name);
		...
		if (!sName.IsEmpty())
		{
			...
			tmps = sName; //m_axform->GetString((*m_cellR)[ii]->name);
			m_names.SetAt(/*(*m_cellR)[ii]->name*/ii, tmps);
		...
		column->m_pRGB = ((*m_cellR)[ii]->hpRGB & userCOLOR) ? (*m_cellR)[ii]->hpRGB : getgRGB((*m_cellR)[ii]->hpRGB);
		column->m_tRGB = ((*m_cellR)[ii]->htRGB & userCOLOR) ? (*m_cellR)[ii]->htRGB : getgRGB((*m_cellR)[ii]->htRGB);
...
*/
		...
		case CK_CHECK:
			...
			if (!CString((*m_cellR)[ii]->str2).IsEmpty())
				column->m_edit = (char *)(*m_cellR)[ii]->str2;	//&m_axform->m_strR[(*m_cellR)[ii]->vals[0]];
			if (((*m_cellR)[ii]->properties & (PR_IMAGE|PR_IMAGETEXT)) && !CString((*m_cellR)[ii]->str3).IsEmpty())//(*m_cellR)[ii]->vals[1] != NOVALUE)
			{
				tmps.Format("%s\\%s\\%s", m_axform->m_root, IMAGEDIR, (*m_cellR)[ii]->str3);//&m_axform->m_strR[(*m_cellR)[ii]->vals[1]]);
				column->m_pic = new Cimage(m_axform->m_pAmDraw, tmps);
...
//					(*m_cellR)[ii]->properties |= PR_TRANSPARENT;
		...
		case CK_COMBO:
			{
				char*	data;
				char*	disp;
				data = (*m_cellR)[ii]->dat;	//&m_axform->m_strR[(*m_cellR)[ii]->vals[0]];
				disp = (*m_cellR)[ii]->str;	//&m_axform->m_strR[(*m_cellR)[ii]->vals[1]];

				vector<string> vData, vDisp;
				Split(vData, string(data), ',');
				Split(vDisp, string(disp), ',');


				for (jj = 0; jj < (int)(*m_cellR)[ii]->keys; jj++)
				{
					column->m_display.Add(CSTRING(vDisp[jj]));
				//	disp += strlen(disp);
				//	disp++;
					column->m_data.Add(CSTRING(vData[jj]));
				//	data += strlen(data);
				//	data++;
				}
		...
		case CK_BUTTON:
	//		if ((*m_cellR)[ii]->vals[0] != NOVALUE)
			if (!CString((*m_cellR)[ii]->txt).IsEmpty())
				column->m_edit = (char *)(*m_cellR)[ii]->txt;//&m_axform->m_strR[(*m_cellR)[ii]->vals[0]];
			if (((*m_cellR)[ii]->properties & (PR_IMAGE|PR_IMAGETEXT)) && !CString((*m_cellR)[ii]->str3).IsEmpty())//(*m_cellR)[ii]->vals[1] != NOVALUE)
			{
				tmps.Format("%s\\%s\\%s", m_axform->m_root, IMAGEDIR, (*m_cellR)[ii]->str3);//&m_axform->m_strR[(*m_cellR)[ii]->vals[1]]);
				column->m_pic = new Cimage(m_axform->m_pAmDraw, tmps);
...
//					(*m_cellR)[ii]->properties |= PR_TRANSPARENT;
		...
		//	{
		//	case IE_EDIT:
			if (!CString((*m_cellR)[ii]->editS).IsEmpty())	// != NOVALUE)
					column->m_edit = (char *)(*m_cellR)[ii]->editS; //&m_axform->m_strR[(*m_cellR)[ii]->onEdit];
		//		break;
...
//		m_form->checked = flash;
	...
	for (ii = 0; ii < m_totCols; ii++)
	{
		...
		{
			if ((m_form->attr2 & GO_ROW) && (m_form->rowof > 0))
			{
				if ((jj % (m_form->rowof * 2)) < m_form->rowof )
			...
			{
				rgb = ((*m_cellR)[ii]->pRGB  & userCOLOR) ? (*m_cellR)[ii]->pRGB  : getgRGB((*m_cellR)[ii]->pRGB);
			...
			{
				column->m_tRGBs.Add(getgRGB((*m_cellR)[ii]->tRGB));
	...
	m_cy =	(float)m_cellH * ((float)m_axform->m_mapH->resolution / 100.0);
	m_HeaderH = m_cellH * ((float)m_axform->m_mapH->resolution / 100.0);

	if (m_form->headH != 0)
		m_HeaderH = m_form->headH * ((float)m_axform->m_mapH->resolution / 100.0);
...
BEGIN_MESSAGE_MAP(CamGrid, CamBase)
	//{{AFX_MSG_MAP(CamGrid)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

BEGIN_DISPATCH_MAP(CamGrid, CamBase)
	//{{AFX_DISPATCH_MAP(CamGrid)
	/*
	DISP_PROPERTY_EX(CamGrid, "Text", _getText, SetNotSupported, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Data", _getData, _setData, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Cell", _getCell, _setCell, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Visible", _getVisible, _setVisible, VT_BOOL)
	DISP_PROPERTY_EX(CamGrid, "Index", _getIndex, _setIndex, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Row", _getRow, _setRow, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Column", _getColumn, _setColumn, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Head", _getHead, _setHead, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Rows", _getRows, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Columns", _getColumns, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Pos", _getPos, _setPos, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Page", _getPage, _setPage, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Direction", _getDirection, _setDirection, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "SortColumn", _getSortColumn, _setSortColumn, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Action", _getAction, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Info", _getInfo, _setInfo, VT_BSTR)
	DISP_PROPERTY_EX(CamGrid, "Realtm", _getRealTm, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "HitPos", _getHitPos, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX(CamGrid, "Focus", _getFocus, SetNotSupported, VT_BOOL)
	DISP_FUNCTION(CamGrid, "SetFocus", _SetFocus, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamGrid, "Refresh", _Refresh, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamGrid, "GetGridCell", _GetCell, VT_BSTR, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "SetGridCell", _SetCell, VT_EMPTY, VTS_I4 VTS_I4 VTS_BSTR)
	DISP_FUNCTION(CamGrid, "GetGridText", _GetText, VT_BSTR, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "GetPRgb", _GetPRgb, VT_I4, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "SetPRgb", _SetPRgb, VT_EMPTY, VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "GetTRgb", _GetTRgb, VT_I4, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "SetTRgb", _SetTRgb, VT_EMPTY, VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "IsColumnVisible", _IsColumnVisible, VT_BOOL, VTS_I4)
	DISP_FUNCTION(CamGrid, "SetColumnVisible", _SetColumnVisible, VT_EMPTY, VTS_I4 VTS_BOOL)
	DISP_FUNCTION(CamGrid, "IsCellEnable", _IsCellEnable, VT_BOOL, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "SetCellEnable", _SetCellEnable, VT_EMPTY, VTS_I4 VTS_I4 VTS_BOOL)
	DISP_FUNCTION(CamGrid, "GetHead", _GetHead, VT_BSTR, VTS_I4)
	DISP_FUNCTION(CamGrid, "SetHead", _SetHead, VT_EMPTY, VTS_I4 VTS_BSTR)
	DISP_FUNCTION(CamGrid, "IsColumnSortable", _IsColumnSortable, VT_BOOL, VTS_I4)
	DISP_FUNCTION(CamGrid, "GetColumnName", _GetColumnName, VT_BSTR, VTS_I4)
	DISP_FUNCTION(CamGrid, "SetDataPosition", _SetDataPosition, VT_EMPTY, VTS_I4)
	DISP_FUNCTION(CamGrid, "Sort", _Sort, VT_EMPTY, VTS_I4 VTS_BOOL VTS_BOOL)
	DISP_FUNCTION(CamGrid, "InsertRow", _InsertRow, VT_EMPTY, VTS_I4)
	DISP_FUNCTION(CamGrid, "RemoveRow", _RemoveRow, VT_EMPTY, VTS_I4)
	DISP_FUNCTION(CamGrid, "Key", _Key, VT_EMPTY, VTS_I4)
	DISP_FUNCTION(CamGrid, "PushGridCell", _Push, VT_EMPTY, VTS_I4 VTS_I4 VTS_BSTR)
	DISP_FUNCTION(CamGrid, "PopGridCell", _Pop, VT_EMPTY, VTS_I4 VTS_I4 VTS_BSTR)
	DISP_FUNCTION(CamGrid, "Print", _Print, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamGrid, "Clear", _Clear, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamGrid, "SetColumnItems", _SetItems, VT_EMPTY, VTS_I4 VTS_BSTR)
	DISP_FUNCTION(CamGrid, "SetCellFocus", _SetCellFocus, VT_EMPTY, VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "Import", _Import, VT_EMPTY, VTS_BSTR)
	DISP_FUNCTION(CamGrid, "SetCellAttr", _SetCellAttr, VT_EMPTY, VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION(CamGrid, "Excel", _Excel, VT_EMPTY, VTS_BOOL VTS_BOOL VTS_BSTR)
	DISP_FUNCTION(CamGrid, "Export", _Export, VT_EMPTY, VTS_BOOL VTS_BSTR VTS_BOOL)
	DISP_FUNCTION(CamGrid, "Edit", _Edit, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamGrid, "SetSkip", _SetSkip, VT_EMPTY, VTS_BSTR VTS_BOOL)
	DISP_FUNCTION(CamGrid, "ExportEx", _ExportEx, VT_EMPTY, VTS_BOOL VTS_BSTR VTS_BSTR VTS_BSTR)
	DISP_FUNCTION(CamGrid, "SetRows", _SetRows, VT_EMPTY, VTS_I4)
	*/
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
BEGIN_INTERFACE_MAP(CamGrid, CamBase)
	INTERFACE_PART(CamGrid, IID_IfmGrid, Dispatch)
END_INTERFACE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CamGrid message handlers

void CamGrid::_SetFocus() 
{
	m_axform->DoSomething(doFOCUS, this);
}

void CamGrid::_Refresh() 
{
	Refresh();
}

LPCTSTR CamGrid::_GetCell(long row, long column) 
{
	CString strResult;
...
void CamGrid::_SetCell(long row, long column, LPCTSTR cells) 
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
	{
...
LPCTSTR CamGrid::_GetText(long row, long column) 
{
	CString strResult;
...
long CamGrid::_GetPRgb(long row, long column) 
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
...
void CamGrid::_SetPRgb(long row, long column, long rgb) 
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
		SetBkColor(rgb, column-1, row-1);	
}

long CamGrid::_GetTRgb(long row, long column) 
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
...
void CamGrid::_SetTRgb(long row, long column, long rgb) 
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
...
bool CamGrid::_IsColumnVisible(long column) 
{
	return IsColumnVisible(column);
}

void CamGrid::_SetColumnVisible(long column, bool visible) 
{
	SetColumnVisible(column, visible);
}

bool CamGrid::_IsCellEnable(long row, long column) 
{
	return IsCellEnable(row, column);
}

void CamGrid::_SetCellEnable(long row, long column, bool enable) 
{
 ...
 /*
  * <br>- Grid 설정 정보 구조
  * <br>","을 구분자로 각 Column의 정보들이 나누어진다.
  * <br>하나의 Column 정보에는 ":" 구분자로 세 개의 부분으로 나누어지는데,
  * <br>첫 번째 부분은 Grid Property에서 지정한 Index값이 들어가고, 
  * <br>두 번째 부분은 해당 Column의 Head Text가 들어간다. 
  * <br>그리고 세 번째 부분은 Visible 정보가 들어있다. "1"이면 Visible이고, "0"이면 Invisible상태이다.
  * <br>
  * <br>- Example
  * <br>"1:주가:1,0:시간:1,3:대비:0,2:거래량:1" 으로 설정 정보가 들어온다면
  * <br>첫 번째 Column의 경우 이 Column의 원래 index는 1이며, 이 Column의 Head는 "주가"이고, 마지막 값이 "1"이므로 visible 상태가 된다.
  * <br>두 번째 Column의 경우 이 Column의 원래 index는 0이며, 이 Column의 Head는 "시간"이고, 마지막 값이 "1"이므로 visible 상태가 된다.
  * <br>세 번째 Column의 경우 이 Column의 원래 index는 3이며, 이 Column의 Head는 "대비"이고, 마지막 값이 "0"이므로 invisible 상태가 된다.
  * <br>네 번째 Column의 경우 이 Column의 원래 index는 2이며, 이 Column의 Head는 "거래량"이고, 마지막 값이 "1"이므로 visible 상태가 된다.
  */
LPCTSTR CamGrid::_getOrderInfo(bool isInitial)
{
	CString strOrder, tmps;
	for (int ii = 0; ii < m_totCols; ii++)
	{
		Ccolumn* column = m_columns.GetAt(ii);

		tmps.Format("%d:%s:%d", ii, column->m_head, (column->m_visible ? 1 : 0));
		strOrder += tmps;
		if (ii != m_totCols-1)
			strOrder += ",";
	}
	return (LPCTSTR)strOrder;
}

void CamGrid::_setOrderInfo(LPCTSTR info)
{
	CString sTable = _T("");
	CArray	< Ccolumn*, Ccolumn* > columns;
	Ccolumn* column;

	vector<string> sORDER, sCOL, sTABLE;
	int nCount = Split(sORDER, info, ',');
	algorithm::trim(sORDER[0]);

	// Tr_Layout 순서 변경
	int saveNum = 0;
	string sTrlayout;
	vector<string> sTMP, sTRNAME, sTRORDER, sSWAP;
	map <string, string> ::iterator pIter;
	pIter = m_axform->m_LINKINFO.begin();

	// sTrlayout : Grid TR Layour 정보
	for (pIter = m_axform->m_LINKINFO.begin(); pIter != m_axform->m_LINKINFO.end(); pIter++)
	{
		sTrlayout = pIter -> second;
		if (sTrlayout.find(m_form->name))
			break;

		saveNum++;
	}	
	int inCnt = Split(sTMP, sTrlayout, 0x0e);		// Input / Output 분리
	int outCnt = Split(sTRORDER, sTMP[inCnt-1], 0x0c);	// Output 분리
	Split(sTRNAME, sTMP[saveNum], 0x0c);
	
	CAxStringArray	edit;

	for (int ii = 0; ii < m_edit.GetSize(); ii++)
	{
		edit.Add("");
	}
	...
	{
		Split(sCOL, sORDER[ii], ':');

		if (ATOI(sCOL[0]) > m_totCols - 1)
			return;

		column = m_columns.GetAt(ATOI(sCOL[0]));

		if (column->m_head.CompareNoCase(sCOL[1].data()))
			continue;

		column->m_visible = ATOI(sCOL[2]) ? true : false;
		columns.Add(column);
		sTable += sCOL[0].data();
		
		// 실제 데이터 배치 변경
		for (int jj = 0; jj < m_totRows; jj++)
			edit.SetAt(jj*m_totCols+ii, m_edit.GetAt(jj*m_totCols+ATOI(sCOL[0])));

		// TR Layout 변경
		sSWAP.push_back(sTRORDER[ATOI(sCOL[0])]);
	...
	for (int ii = 0; ii < m_totCols; ii++)
	{
		...
		for (int jj = 0; jj < m_totRows; jj++)
			edit.SetAt(jj*m_totCols+(columns.GetCount()-1), m_edit.GetAt(jj*m_totCols+ii));

		// TR Layout 변경 : 위에 저장한 내용 빼고 저장
		sSWAP.push_back(sTRORDER[ii]);
	}

	// TR Layout Input 정보
	for (int ii = 0;  ii < inCnt-1; ii++)
	{
		sTrlayout = sTMP[ii];
		sTrlayout += 0x0e;
	}
	...
	for (int ii = 0; ii < m_totCols; ii++)
	{
		column = columns.GetAt(ii);
		m_columns.SetAt(ii, columns.GetAt(ii));

		for (int jj = 0; jj < m_totRows; jj++)
			m_edit.SetAt(jj*m_totCols+ii, edit.GetAt(jj*m_totCols+ii));

		sTrlayout += sSWAP[ii];
		sTrlayout += 0x0c;
	}

	m_axform->m_LINKINFO[sTRNAME[1]] = sTrlayout;	
	invalidateRect(&m_pRc, false);
}



void CamGrid::_appendView(LPCTSTR mapName, int left, int top, int width, int Height)
{
	AfxMessageBox("appendView : This function isn't supported");
	return;
}

bool CamGrid::_isCellChecked(long row, long col)
{	
	if (m_totCols < col || m_totRows < row)
		return false;
	...
	if ((*m_cellR)[column->m_ref]->kind != CK_CHECK)
		return false;

	CString check = m_edit.GetAt(row * m_totCols + col);	
...
void CamGrid::_setCellChecked(long row, long col, bool checked)
{
	if (m_totCols < col || m_totRows < row)
		return;
	...
	if (checked)
		m_edit.SetAt(row * m_totCols + col, "1");
	else
		m_edit.SetAt(row * m_totCols + col, "0");

	invalidateRect(&m_pRc, false);
}

void CamGrid::_scrollToRow(long row)
{
	AfxMessageBox("scrollToRow : This function isn't supported by Emulator.");
}

LPCTSTR CamGrid::GetHeaderDirection()
{
	AfxMessageBox("headerDirection : This property isn't supported by Emulator.");
	return "";
}

LPCTSTR CamGrid::GetHeaderSortDirection()
{
	AfxMessageBox("headerSortDirection : This property isn't supported by Emulator.");
	return "";
}

LPCTSTR CamGrid::GetHeaderSortColumn()
{
	AfxMessageBox("headerColumn : This property isn't supported by Emulator.");
	return "";
}

LPCTSTR CamGrid::GetHeaderSave()
{
	AfxMessageBox("headerSave : This property isn't supported by Emulator.");
	return "";
}

LPCTSTR CamGrid::GetHeaderPage()
{
	AfxMessageBox("headerPage : This property isn't supported by Emulator.");
	return "";
}

int CamGrid::GetScrollX()
{
	AfxMessageBox("scrollX : This property isn't supported by Emulator.");
	return 0;
}

void CamGrid::SetSctollX(int x)
{
	AfxMessageBox("scrollX : This property isn't supported by Emulator.");
}

int CamGrid::GetScrollY()
{
	AfxMessageBox("scrollY : This property isn't supported by Emulator.");
	return 0;
}

void CamGrid::SetScrollY(int y)
{
	AfxMessageBox("scrollY : This property isn't supported by Emulator.");
}

LPCTSTR CamGrid::_GetHead(long column) 
{
	CString strResult;
...
void CamGrid::_SetHead(long column, LPCTSTR head) 
{
	if (column > 0 && column <= m_totCols)
		WriteData(head, true, column-1, -2);
}

bool CamGrid::_IsColumnSortable(long column) 
{
	return IsColumnSortable(column);
}

LPCTSTR CamGrid::_GetColumnName(long column) 
{
	CString	strResult;
...
void CamGrid::_SetDataPosition(long method) 
{
	SetDataPosition(method);
}

void CamGrid::_Sort(long col, bool ascend, bool string) 
{
	Sort(col, ascend ? true : false, string ? true : false);
}

void CamGrid::_InsertRow(long index, LPCTSTR rowData) 
{
	CString str = (LPCSTR)(LPSTR)rowData;
		
	InsertRow(index - 1, str);
}

void CamGrid::_RemoveRow(long index) 
{
	RemoveRow(index - 1);
}

void CamGrid::_Key(long key) 
{
	m_axform->DoSomething(doKEY, this, (WPARAM)0, (LPARAM)key);
}

void CamGrid::_Push(long row, long column, LPCTSTR name) 
{
	m_axform->DoSomething(doPUSHX, this, MAKEWPARAM(row-1, column-1), (LPARAM)name);
}

void CamGrid::_Pop(long row, long column, LPCTSTR name) 
{
...
void CamGrid::_SetItems(long col, LPCTSTR items) 
{
	SetItems(col, items);
}

void CamGrid::_SetCellFocus(long row, long col)
{
	SetCellFocus(row, col);
}

void CamGrid::_Import(LPCTSTR files) 
{
	Import(files);
}

void CamGrid::_SetCellAttr(long row, long column, long dwAdd, long dwRemove) 
{
	SetCellAttr(row, column, dwAdd, dwRemove);
}

void CamGrid::_Excel(bool run, bool head, LPCTSTR path) 
{
	m_axform->DoSomething(doEXCEL, this, MAKEWPARAM(run ? 1 : 0, head ? 1 : 0), (LPARAM)path);
}

void CamGrid::_Export(bool head, LPCTSTR path, bool append) 
{
	m_axform->DoSomething(doTEXT, this, MAKEWPARAM(append ? 1 : 0, head ? 1 : 0), (LPARAM)path);
}

void CamGrid::_ExportEx(bool run, LPCTSTR top, LPCTSTR bottom, LPCTSTR path) 
{
	CString	data;
...
void CamGrid::_SetSkip(LPCTSTR cname, bool skip) 
{
	CString	name = CString(cname);
	for (int ii = 0; ii < m_totCols; ii++)
	{
//		if (!name.CompareNoCase(GetSymbolName(ii)))///*(*m_cellR)[ii]->name)*/))
...
void CamGrid::_SetRows(long rows) 
{
	if (m_totRows == rows)
		return;
		...
		for (ii = m_totRows; ii < rows; ii++)
		{
			for (jj = 0; jj < m_totCols; jj++)
			{
				...
				if (m_form->attr2 & GO_ROW)
				{
					if (m_form->rowof != 0)
					{
						if ((ii % (m_form->rowof * 2)) < m_form->rowof)
							column->m_pRGBs.Add(m_sRGB);
						else
							column->m_pRGBs.Add(m_fRGB);
					}
					else
						column->m_pRGBs.Add(m_pRGB);
				}
				else
					column->m_pRGBs.Add(((*m_cellR)[column->m_ref]->pRGB  & userCOLOR) ? (*m_cellR)[column->m_ref]->pRGB  : getgRGB((*m_cellR)[column->m_ref]->pRGB));
				...
				{
					column->m_tRGBs.Add(getgRGB((*m_cellR)[column->m_ref]->tRGB));
...
void CamGrid::SetData(LPCTSTR lpszNewValue) 
{
...
void CamGrid::SetCell(LPCTSTR lpszNewValue) 
{
	WriteData(lpszNewValue);
	if (m_axform->DoSomething(doIGNORE, this))
		IsChanged();
...
void CamGrid::SetFG(int color)
{
	SetFgColor(color);
}

bool CamGrid::GetVisible() 
{
	return (m_form->properties & PR_VISIBLE) ? true : false;
}

void CamGrid::SetVisible(bool bNewValue) 
{
	CamBase::SetVisible(bNewValue ? true : false);
}

long CamGrid::GetIndex() 
{
	return GetCurSel();
}

void CamGrid::SetIndex(long nNewValue) 
{
	SetCurSel(nNewValue);
}
...
void CamGrid::SetRow(long nNewValue) 
{
	SetCellRow(nNewValue);
}

long CamGrid::GetColumn() 
{
	return GetCellColumn();
}

void CamGrid::SetColumn(long nNewValue) 
{
	SetCellColumn(nNewValue);
}

LPCTSTR CamGrid::GetHead() 
{
	CString strResult;
...
void CamGrid::SetHead(LPCTSTR lpszNewValue) 
{
	WriteData(lpszNewValue, true, -1, -2);
}

long CamGrid::GetRows() 
{
	return GetValidRows();
}

long CamGrid::GetColumns() 
{
	return m_totCols;
}
...
void CamGrid::SetPos(long nNewValue) 
{
	m_stat = 0;
	switch (nNewValue)
	{
...
void CamGrid::SetPage(LPCTSTR lpszNewValue) 
{
	m_page = lpszNewValue;
}

long CamGrid::GetDirection() 
{
	return m_dir;
}

void CamGrid::SetDirection(long nNewValue) 
{
	m_dir = nNewValue;
}

LPCTSTR CamGrid::GetSortColumn() 
{
	CString strResult;

//	strResult = (m_sortc >= 0) ? GetSymbolName((*m_cellR)[m_columns.GetAt(m_sortc)->m_ref]->name) : _T("");
	strResult = (m_sortc >= 0) ? GetSymbolName(m_columns.GetAt(m_sortc)->m_ref) : _T("");
	return (LPCTSTR)strResult;
}

void CamGrid::SetSortColumn(LPCTSTR lpszNewValue) 
{
	int	value;
...
long CamGrid::GetAction() 
{
	return (m_form->attr2 & GO_TOP) ? 1 : 2;
}

LPCTSTR CamGrid::GetInfo() 
{
	CString strResult;
...
void CamGrid::SetInfo(LPCTSTR lpszNewValue) 
{
	m_inits = lpszNewValue;
}

long CamGrid::GetRealTm() 
{
	return (m_form->attr2 & GO_TOP) ? 1 : 2;
}

long CamGrid::GetHitPos() 
{
	return m_axform->DoSomething(doHITPOS, this);
}

bool CamGrid::GetFocus() 
{
	return (m_focus ? true : false);
}

CString CamGrid::GetSymbolName(int col)
{
	if (col < 0)
...
bool CamGrid::IsValid(int& guide)
{
	if (!(m_form->attr2 & GO_TABLE))
		return true;
	/*
...
void CamGrid::Adjust(CPoint pt, float hR, float vR)
{
	float	tmpF;
	float	tWidth = 0;
	Ccolumn* column;
...
void CamGrid::Resize(float hR, float vR, bool font, bool redraw)
{
	float	tWidth = 0;
	Ccolumn* column;
...
int CamGrid::IsPointInRect(CPoint pt)
{
	int	ret = CamBase::IsPointInRect(pt);
...
void CamGrid::Draw(CDC* dc)
{
	...
	CRect	sRc/*whole Grid*/,  sRc1,  sRc2; // Scroll 
	CRect	tRc/*text field*/,  hRc/*header field*/;
	float	dx, dy;
	int	showRows;
	...
	int nFixColDisc = 0, nFixRowDisc = 0;
	    
	sRc = m_pRc;	// Grid 전체 영역
	...
	// Resolution Apply
	//m_cy = m_cy *((float)m_axform->m_mapH->resolution / 100.0);
	//m_HeaderH = (int)((double)m_HeaderH * ((double)m_axform->m_mapH->resolution / 100.0));
	...
	if (m_totCols != 0 && m_stCol != -1)
	{
		...
		m_cell.SetRectEmpty();	// m_cell : Cell Rect
		m_enCol = m_totCols-1;
		index = 0;
		...
		for (kk = 0; kk < showRows; kk++)
		{
			...
			{ // Fixed Row를 위해 수
				if (m_nfixRows > 0)
				{
					if (kk > (m_nfixRows * m_multiN))
						index = (m_stRow + kk)/m_multiN - m_headN;
					else
						index = kk/m_multiN - m_headN;
				}
				else
					index = (m_stRow + kk)/m_multiN - m_headN;
			}/////////////////////////////////////////////////////
...
TRACE("index = %d\n", index);
			...
			{
				if (m_form->attr2 & GO_MERGE)
					tRc.bottom = tRc.top + (int)m_HeaderH;
				else
					tRc.bottom = tRc.top + m_HeaderH  * m_headN;
			...
			for ( ; jj < colidx + colcnt ; jj++)
			{
				...
				/////////////////////////////////////////
				// Fix_Col && Fix_Row 동시에 세팅될경우
				/*if (m_form->type > 0 && m_nfixRows > 0)
				{
					if (m_form->type > jj && m_nfixRows < index)
						nFixColDisc = m_stRow;
					else
						nFixColDisc = 0;
						
					if (m_form->type < jj && m_nfixRows > index)
						nFixRowDisc = m_stCol - m_form->type;
					else
						nFixRowDisc = 0;
				}*/
				/////////////////////////////////////////
					...
					|| (!(m_form->attr2 & GO_MERGE) && kk >= m_multiN) || m_headN == 0)
				{
	  				if ((m_edit.GetSize()/m_totCols) > index)
						tmps = m_edit.GetAt((index - nFixColDisc) * m_totCols + (jj - nFixRowDisc)); //Fix Row/Col을 위한 수정
					else
				...
				if (((m_form->attr2 & GO_MERGE && kk < m_headN)		// Header Drawing
					|| (!(m_form->attr2 & GO_MERGE) && kk < m_multiN)) && m_headN != 0)
				{
					//brush = setBrush(dc, col->m_pRGB);
					//setgBrush(col->m_pRGB);
					...
					if (draw)
					{
						...
						g.FillRectangle(setgBrush(col->m_pRGB), RECTF(cRc));    // gdi+ 수정
						if (!tmps.IsEmpty())
						{
							// Upper Check
							if ((*m_cellR)[col->m_ref]->attr & FA_UPPER)
								tmps.MakeUpper();
							...
							for (ii = 0; ii < col->m_ihead; ii++)
							{
								...
								{
									//hRc.bottom += 50;
									//drawHead(dc, jj, tmps, hRc, &sf, col->m_tRGB);
									drawHead(dc, jj, tmps, hRc, format, &sf, col->m_tRGB);
								}
								else
								{
									dc->SetBkMode(TRANSPARENT);
									dc->DrawText(tmps, hRc, format);
									//g.DrawString(tmps.AllocSysString(), -1, setgFont(), RECTF(hRc), &sf, setgBrush(col->m_tRGB));
								}
							}
						}

						if (!(m_form->attr2 & GO_X3D))							
							drawEdge(g, cRc, EDGE_RAISED, BF_RECT); // gdi+
						else
						{
							cRc.right--;
							if (jj != colidx)
							...
							if (kk != 0 && cRc.top > m_pRc.top)
								cRc.top--;
							g.DrawRectangle(setgPen(GRGB(m_bRGB)), RECTF(cRc)); // gdi+ 수정

							if (colidx && (m_form->attr2 & GO_XINLINE))
								g.DrawLine(setgPen(GRGB(col->m_pRGB)), cRc.left+1, cRc.top, cRc.right-1, cRc.top);
	...
	//dc->Rectangle(cRc);
				...
				{		// data drawing
					/*if (m_form->kind == FM_GRIDEX)
					{
						CFont* cellFont = setCellFont(dc, m_axform->m_mapH->resolution, (*m_cellR)[col->m_ref]);
			...
			//		dc->FillRect(tRc, brush);
					...
					dc->FillSolidRect(tRc, ARGBtoRGB(GRGB(pRGB)));

					//setgBrush(getRGB(m_form->onEdit))
				
			//		if ((*m_cellR)[col->m_ref]->kind == CK_NORMAL)	
			//		{
			//			g.DrawRectangle(setgPen(GRGB(pRGB)), RECTF(tRc));
			//			g.FillRectangle(setgBrush(GRGB(pRGB)),RECTF(tRc)); // gdi+ 수정
			//		}
					if (!noWrite)
					{
						tRc.left++;
					...
					if ((int)dx <= sRc.Width())
					{
						...
						if (!(m_form->attr2 & GO_XHLINE))
						{
							if ((*m_cellR)[col->m_ref]->iok == EIO_OUTPUT && (*m_cellR)[col->m_ref]->kind == CK_NORMAL
								&& (int)(*m_cellR)[col->m_ref]->vals2 > 0/*vals[2] > 0*/
								&& (m_stRow+kk-skipN)%(int)(*m_cellR)[col->m_ref]->vals2/*vals[2]*/ != (int)(*m_cellR)[col->m_ref]->vals2 - 1)//vals[2]-1)
							{
								//setPen(dc, pRGB);								
							...
							else
							{
								...
								if ((m_form->attr2 & GO_XINLINE) && ((m_stRow + kk) % m_multiN) != (m_multiN-1))
									g.DrawLine(setgPen(GRGB(pRGB)), tRc.left, tRc.bottom, tRc.right, tRc.bottom);
								else
									g.DrawLine(setgPen(GRGB(m_bRGB)), tRc.left, tRc.bottom, tRc.right, tRc.bottom);
							}
						...
						if (!(m_form->attr2 & GO_XVLINE))
						{
							...
							g.DrawLine(setgPen(GRGB(m_bRGB)), tRc.right, tRc.top, tRc.right, tRc.bottom);
						}
					}
					else
					{
			...
			if ((int)dx < sRc.Width())
			{
				...
				//dc->FillRect(tRc, brush);
				g.FillRectangle(setgBrush(getRGB(m_form->tRGB)), RECTF(tRc)); // gdi+ 수정
				g.DrawRectangle(setgPen(getRGB(m_form->tRGB)), RECTF(tRc));
			}
...
//	if (!(m_form->attr2 & GO_X3D))
//		drawEdge(g, sRc, EDGE_SUNKEN, BF_RECT); 		
//	else
	{
		//setPen(dc, m_bRGB);
		//dc->SelectStockObject(NULL_BRUSH);
		//dc->Rectangle(sRc);
		g.DrawRectangle(setgPen(GRGB(m_bRGB)),RECTF(sRc)); // gdi+ 수정
	}
	...
	if ((m_focus || m_form->attr2 & GO_ALWAYS) && (m_form->isEdit == FI_CELL || m_form->isEdit == FI_RBOX))
	{
		...
		//dc->SelectStockObject(NULL_BRUSH);
		//dc->Rectangle(tRc);
		//if (m_form->shape == 1)	// Bar
		//	g.FillRectangle(setgBrush(getRGB(m_form->onEdit)), RECTF(tRc)); // gdi+ 수정			
		//if (m_form->shape == 2)	// Rectangle
			g.DrawRectangle(setgPen(getRGB(m_form->onEdit)), RECTF(tRc)); // gdi+ 수정
	}
	...
	if (m_hscroll)	
	{
		...
		if (m_form->attr2 & GO_X3D)
		{
			...
			g.FillRectangle(setgBrush(GRGB(m_sbRGB < 0 ? _ARGB(255, 250, 250, 250) : getgRGB(m_sbRGB))),RECTF(tRc)); // gdi+ 수정
			sRc = m_hscr;
			...
			g.DrawRectangle(setgPen(GRGB(m_slRGB < 0 ? _ARGB(255, 160, 160, 160) : getgRGB(m_slRGB))), RECTF(m_hthumb)); // gdi+ 수정
		...
		*/
		{
			//brush = setBrush(dc, getRGB(cLGRAY));
             		//dc->FillRect(sRc, brush);
//			g.FillRectangle(setgBrush(clLGRAY),RECTF(tRc)); // gdi+ 수정
			dc->FillSolidRect(sRc, m_sbRGB < 0 ? RGB(255, 255, 255) : ARGBtoRGB(getgRGB(m_sbRGB)));
			sRc = m_hscr;
			if (m_click == ckScrollLeft)
			{
             			//dc->DrawFrameControl(sRc1, DFC_SCROLL, DFCS_SCROLLLEFT|DFCS_PUSHED);
				pBitmap = DrawFrameControl(dc, sRc1, DFC_SCROLL, DFCS_SCROLLLEFT|DFCS_PUSHED);
				g.DrawImage(pBitmap, sRc1.left, sRc1.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			}
			else
			{
             			//dc->DrawFrameControl(sRc1, DFC_SCROLL, DFCS_SCROLLLEFT);
				pBitmap = DrawFrameControl(dc, sRc1, DFC_SCROLL, DFCS_SCROLLLEFT);
				g.DrawImage(pBitmap, sRc1.left, sRc1.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			}
			...
			pBitmap = DrawFrameControl(dc, m_hthumb, DFC_BUTTON, DFCS_BUTTONPUSH);
			g.DrawImage(pBitmap, m_hthumb.left, m_hthumb.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			
			if (m_click == ckScrollRight)
			{
				//dc->DrawFrameControl(sRc2, DFC_SCROLL, DFCS_SCROLLRIGHT | DFCS_PUSHED);
				pBitmap = DrawFrameControl(dc, sRc2, DFC_SCROLL, DFCS_SCROLLRIGHT | DFCS_PUSHED);
				g.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());				
			}
			else
			{
				//dc->DrawFrameControl(sRc2, DFC_SCROLL, DFCS_SCROLLRIGHT);
				pBitmap = DrawFrameControl(dc, sRc2, DFC_SCROLL, DFCS_SCROLLRIGHT);
				g.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			}
		}
	//	invalidateRect(tRc, true);
...
//		sRc.DeflateRect(2, 2);
	...
	if  (m_vscroll)
	{
		...
		if (m_form->attr2 & GO_X3D)
		{
			...
			//dc->FillRect(sRc, brush);
			//g.FillRectangle(setgBrush(GRGB(m_sbRGB < 0 ? _ARGB(255, 255, 255, 255) : getgRGB(m_sbRGB))),RECTF(sRc)); // gdi+ 수정
			...
			else
			{
				...
				setPen(dc, m_slRGB < 0 ? _ARGB(255, 160, 160, 160) : getRGB(m_slRGB));
				setBrush(dc, m_sbRGB < 0 ? _ARGB(255, 255, 255, 255) : getRGB(m_sbRGB));
		...
		else*/
		{
			...
			if (m_enableScroll)
			{
				...
				pBitmap = DrawFrameControl(dc, sRc1, DFC_SCROLL, DFCS_SCROLLUP | DFCS_INACTIVE);
				g.DrawImage(pBitmap, sRc1.left, sRc1.top, pBitmap->GetWidth(), pBitmap->GetHeight());
				pBitmap = DrawFrameControl(dc, sRc2, DFC_SCROLL, DFCS_SCROLLDOWN | DFCS_INACTIVE);
				g.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());
				if (m_form->attr2 & GO_PAGE)
				{
			...
			else
			{
				if (m_click == ckScrollUp)
				{
             	//	dc->DrawFrameControl(sRc1, DFC_SCROLL, DFCS_SCROLLUP | DFCS_PUSHED);			
					pBitmap = DrawFrameControl(dc, sRc1, DFC_SCROLL, DFCS_SCROLLUP | DFCS_PUSHED);
					g.DrawImage(pBitmap, RECTG(sRc1));					
				}
				else
				{
				//	dc->DrawFrameControl(sRc1, DFC_SCROLL, DFCS_SCROLLUP);
					pBitmap = DrawFrameControl(dc, sRc1, DFC_SCROLL, DFCS_SCROLLUP);
					g.DrawImage(pBitmap, RECTG(sRc1));					
				...
				pBitmap = DrawFrameControl(dc, m_vthumb, DFC_BUTTON, DFCS_BUTTONPUSH);
				g.DrawImage(pBitmap, m_vthumb.left, m_vthumb.top, pBitmap->GetWidth(), pBitmap->GetHeight());				
				
				if (m_click == ckScrollDown)
				{
					//dc->DrawFrameControl(sRc2, DFC_SCROLL, DFCS_SCROLLDOWN | DFCS_PUSHED);
					pBitmap = DrawFrameControl(dc, sRc2, DFC_SCROLL, DFCS_SCROLLDOWN | DFCS_PUSHED);
					g.DrawImage(pBitmap, RECTG(sRc2));	
				//	g.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());					
				}
				else
				{
					//dc->DrawFrameControl(sRc2, DFC_SCROLL, DFCS_SCROLLDOWN);
					pBitmap = DrawFrameControl(dc, sRc2, DFC_SCROLL, DFCS_SCROLLDOWN);
					g.DrawImage(pBitmap, RECTG(sRc2));	
				//	g.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());
				}
	...
	else if (m_form->attr2 & GO_PAGE)
	{
		...
		if (m_form->attr2 & GO_X3D)
		{
			//brush = setBrush(dc, m_sbRGB < 0 ? PALETTERGB(255, 255, 255) : getRGB(m_sbRGB));
             		//dc->FillRect(sRc, brush);
			g.FillRectangle(setgBrush(GRGB(m_sbRGB < 0 ? _ARGB(255, 255, 255, 255) : getgRGB(m_sbRGB))), RECTG(sRc));
		...
		else
		{
			//brush = setBrush(dc, getRGB(cLGRAY));
             		//dc->FillRect(sRc, brush);
			g.FillRectangle(setgBrush(clLGRAY), RECTG(sRc));
...
void CamGrid::OnLButton(bool down, CPoint pt, int& result)
{
	result = RC_NOP;
	m_down = false;
...
void CamGrid::OnRButton(bool down, CPoint pt, int& result)
{
	result = RC_NOP;
...
void CamGrid::OnDblClick(CPoint pt, int& result)
{
	result = RC_NOP;
	m_select = false;
	m_move   = mvNone;
	...
	if (m_vscroll && m_enableScroll && m_vscr.PtInRect(pt))
	{
		...
		if (stRow != m_stRow)
		{
			...
			if (m_form->attr2 & GO_MULTI && m_nfixRows > 0)
			{
				tmpRc1.top  += (int)(m_cy * m_headN * (m_multiN * (m_nfixRows+1)));
				tmpRc1.top  += (m_multiN * 2);			//head extra space
			}
			else
...
bool CamGrid::OnRepeat(CPoint pt, int& result)
{
	CRect	tRc;
	float	tWidth = 0;
	int	bN, ii = 0;
	...
	case ckScrollUp:
		...
		if (m_form->attr2 & GO_MULTI && m_nfixRows > 0)
		{
			tRc.top  += (int)(m_cy * m_headN * (m_multiN * (m_nfixRows+1)));
			tRc.top  += (m_multiN * 2);			//head extra space
		}
		else
...
bool CamGrid::OnMouseWheel(int delta, int& result)
{
...
int CamGrid::OnDrag(CPoint pt, int& result)
{
	int	idx;
	CRect	tRc;
...
bool CamGrid::OnDrop(CPoint pt, CString data, CString& key)
{
...
int CamGrid::OnKey(int key, int& result)
{
	Ccolumn* column;
	...
	if (m_curpos.m_col >= 1 && m_curpos.m_row >= 1)
	{
		...
		if ((*m_cellR)[column->m_ref]->kind == CK_COMBO
			&& (*m_cellR)[column->m_ref]->vals2/*vals[2]*/ != NOVALUE && key == (int)(*m_cellR)[column->m_ref]->vals2) //vals[2])
		{
			result = RC_NOP;
...
void CamGrid::InsertRow(int index, CString str)
{
	if (index >= m_totRows)
		return;
...
void CamGrid::InsertRows(int index, CString str)
{
	int	ii, pos;
	CString	text;
		...
		for (ii = 0; ii < m_totCols; ii++)
		{
			...
			for (int jj = 0; jj < m_totRows; jj++)
			{
				if (m_form->rowof != 0)
				{
					...
					col->m_pRGBs.Add(m_pRGB);

				// add Supress + Background 2014. 01. 29.
				if ((((*m_cellR)[col->m_ref]->attr & FA_CORGB) && ((*m_cellR)[col->m_ref]->attr2 & SA_CORGB) == SA_PAINT) ||
					(((*m_cellR)[col->m_ref]->attr & FA_CORGB) && ((*m_cellR)[col->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP))
				{
...
void CamGrid::RemoveRow(int index)
{
	index = max(0, index);
	...
	if (m_form->attr2 & GO_ROW)
	{
		for (int ii = 0; ii < m_totCols; ii++)
		{
			...
			for (int jj = 0; jj < m_totRows; jj++)
			{
				...
				// add Supress + Background 2014. 01. 29.
				if ((((*m_cellR)[column->m_ref]->attr & FA_CORGB) && ((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINT) ||
					(((*m_cellR)[column->m_ref]->attr & FA_CORGB) && ((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP))
				{
					tmps = m_data.GetAt(jj*m_totCols+ii);
...
void CamGrid::WriteAll(CString str, bool redraw)
{
	CString	tmps = _T("");
	int	row, count, offs;
	...
	if (m_save == svExchange)
	{
		for (int ii = 0; ii < m_totCols; ii++)
		{
			...
			for (int jj = 0; jj < m_totRows; jj++)
			{
				...
				{
					COLORREF rgb = ((*m_cellR)[column->m_ref]->pRGB  & userCOLOR) ? (*m_cellR)[column->m_ref]->pRGB : getgRGB((*m_cellR)[column->m_ref]->pRGB);
					column->m_pRGBs.Add(rgb);
				}
			}
		}
...
void CamGrid::WriteData(CString data, bool redraw, int col, int row)
{
	if (col >= m_totCols)
		return;
	...
	if ((*m_cellR)[column->m_ref]->attr & FA_CORGB/* && (*m_cellR)[column->m_ref]->onEdit != 0*/)
	...
	if ((*m_cellR)[column->m_ref]->attr & FA_FLOAT)
	{
		...
		if (floatx >= 0)
		{
			...
			if ((*m_cellR)[column->m_ref]->vals/*vals[1]*/ == (*m_cellR)[column->m_ref]->size)
				tmps = text;
			else
			{
				str  = text.Right((*m_cellR)[column->m_ref]->size - (*m_cellR)[column->m_ref]->vals/*vals[1]*/);
				text = tmps.Mid(floatx+1);
				text = text.Left((*m_cellR)[column->m_ref]->vals/*vals[1]*/);
				if (str.IsEmpty())
				{
					str += _T("0");
					floatx++;
				}
				str += CString('0', (*m_cellR)[column->m_ref]->vals/*vals[1]*/ - tmps.GetLength());
				str += text;
				tmps = str;
			}
		}
		...
		else
		{
			...
			*/
			...
			floatx = tmps.GetLength() - (*m_cellR)[column->m_ref]->vals/*vals[1]*/;
			if (floatx <= 0)
			{
...
void CamGrid::ReadAll(CString& data, bool edit, bool head)
{
	int	rows;
	Ccolumn* col;
	CString	tmps = _T("");
...
void CamGrid::ReadData(CString& data, bool edit, int col, int row)
{
	data = _T("");
...
void CamGrid::ClearData(bool selectOnly)
{
	COLORREF rgb;
	int	ii, stCol;
	...
	if (m_save == svExchange)
	{
		...
		{
			Ccolumn* column = m_columns.GetAt(ii);
			column->m_pRGB = ((*m_cellR)[column->m_ref]->hpRGB & userCOLOR) ? (*m_cellR)[column->m_ref]->hpRGB : getgRGB((*m_cellR)[column->m_ref]->hpRGB);
			column->m_tRGB = ((*m_cellR)[column->m_ref]->htRGB & userCOLOR) ? (*m_cellR)[column->m_ref]->htRGB : getgRGB((*m_cellR)[column->m_ref]->htRGB);
			column->m_pRGBs.RemoveAll();
			column->m_tRGBs.RemoveAll();
			column->m_itRGBs.RemoveAll();
			column->m_valids.RemoveAll();
			...
			for (int jj = 0; jj < m_totRows; jj++)
			{
				...
				{
					rgb = ((*m_cellR)[column->m_ref]->pRGB  & userCOLOR) ? (*m_cellR)[column->m_ref]->pRGB : getgRGB((*m_cellR)[column->m_ref]->pRGB);
					column->m_pRGBs.Add(rgb);
				}
				if ((*m_cellR)[column->m_ref]->tRGB & userCOLOR)
				{
...
void CamGrid::UpdateData(int key, bool moving, int& result)
{
	if (m_curpos.m_col < 1)
		return;
...
//			return;
	...
	if (((*m_cellR)[column->m_ref]->attr & FA_FLOAT)
		&& ((floatx == 0xff && m_edcaretpos >= (*m_cellR)[column->m_ref]->size - (int)(*m_cellR)[column->m_ref]->vals)
		|| (floatx != 0xff && data.GetLength() - floatx >= (int)(*m_cellR)[column->m_ref]->vals))
...
void CamGrid::InsertData(int key, bool moving, int& result)
{
	UpdateData(key, moving, result);
}

void CamGrid::GetEnum(CString& text, int& value, int func)
{
	text.Empty();
...
//			text = (m_sortc >= 0) ? GetSymbolName((*m_cellR)[m_columns.GetAt(m_sortc)->m_ref]->name) : _T("");
			text = (m_sortc >= 0) ? GetSymbolName(m_columns.GetAt(m_sortc)->m_ref) : _T("");
			break;
		case 107:	// sortable
...
//				text = GetSymbolName((*m_cellR)[m_columns.GetAt(m_cellpos.m_col-1)->m_ref]->name);
				text = GetSymbolName(m_columns.GetAt(m_cellpos.m_col-1)->m_ref);
...
void CamGrid::SetEnum(CString text, int value, int func)
{
	int	ii, offs;
	CString	tmps;
	Ccolumn* column;
	...
	if (text.IsEmpty())
	{
		for (ii = m_form->type; ii < m_totCols; ii++)
		{
			offs = findColumn(GetSymbolName(ii));		// GetSymbolName
			if (offs >= 0)
			{
...
//				if (!tmps.CompareNoCase(GetSymbolName((*m_cellR)[arCOLx.GetAt(ii)->m_ref]->name)))
				if (!tmps.CompareNoCase(GetSymbolName(arCOLx.GetAt(ii)->m_ref)))
				{
...
void CamGrid::SetFocus(bool focus)
{
	bool	whole = false;
...
bool CamGrid::IsChanged(bool reset)
{
...
void CamGrid::SetCellRow(int row)
{
	if (row <= m_totRows)
		m_cellpos.m_row = row;
}

void CamGrid::SetCellColumn(int col)
{
	if (col <= m_totCols)
		m_cellpos.m_col = col;
}

int CamGrid::GetCellRow()
{
	return m_cellpos.m_row;
}

int CamGrid::GetCellColumn()
{
	return m_cellpos.m_col;
}

int CamGrid::GetValidRows()
{
	return m_validRows;
}

void CamGrid::SetAttributes(DWORD attr, bool set, int col, int row)
{
...
void CamGrid::_SetVisible(bool visible, int col)
{
	if (col < 0)
	{
		CamBase::SetVisible(visible);
		return;
	}
...
bool CamGrid::IsVisible(int col)
{
	if (col < 0)
		return CamBase::IsVisible(col);
...
void CamGrid::SetEnable(bool enable)
{
	//if (!(m_form->attr2 & GO_TABLE))
	//	return;
...
void CamGrid::SetCurSel(int index, bool redraw)
{
...
int CamGrid::GetCurSel()
{
	return (m_index / m_multiN);
}

void CamGrid::SetFgColor(int rgb, int col, int row)
{
	if (col == -1 && row == -1)
	{
...
void CamGrid::SetBkColor(int rgb, int col, int row)
{
	if (col == -1 && row == -1)
	{
		if (m_cellpos.m_row < 1 || m_cellpos.m_col < 1)
	...
	// add Supress + Background 2014. 01. 29.
	if (row >= 0 && (((*m_cellR)[column->m_ref]->attr & FA_CORGB) && ((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINT ||
			((*m_cellR)[column->m_ref]->attr & FA_CORGB) && ((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP))
		return;
...
int CamGrid::GetFgColor(int col, int row)
{
	if (col == -1 && row == -1)
	{
		if (m_cellpos.m_row < 1 || m_cellpos.m_col < 1)
...
int CamGrid::GetBkColor(int col, int row)
{
	if (col == -1 && row == -1)
	{
		if (m_cellpos.m_row < 1 || m_cellpos.m_col < 1)
...
void CamGrid::SetRowColor(int fRgb, int sRgb)
{
	CString	tmps;
	Ccolumn* column;
	...
	for (int ii = 0; ii < m_totCols; ii++)
	{
		...
		for (int jj = 0; jj < m_totRows; jj++)
		{
			...
			// add Supress + Background 2014. 01. 29.
			if (((*m_cellR)[column->m_ref]->attr & FA_CORGB) && 
				(((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINT ||
				((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINTSUP))
			{
				tmps = m_data.GetAt(jj*m_totCols+ii);
				makeSignData(tmps, ii, jj);
			}
...
void CamGrid::SetDomino(bool redraw)
{
	int	auxS;
	DWORD	rgb;
	Ccolumn* column;
...
void CamGrid::Sort(int col, bool ascend, bool string)
{
	if (col <= 0)
		return;
...
bool CamGrid::IsColumnVisible(long column)
{
	if (column > 0 && column <= m_totCols)
...
void CamGrid::SetColumnVisible(long column, bool visible)
{
	if (column > 0 && column <= m_totCols)
		_SetVisible(visible ? true : false, column-1);
}

bool CamGrid::IsCellEnable(long row, long column)
{
	if (row > 0 && row <= m_totRows && column > 0 && column <= m_totCols)
	{
...
void CamGrid::SetCellEnable(long row, long column, bool enable)
{
	row--; column--;
	if (row >= 0 && row < m_totRows && column >= 0 && column < m_totCols)
	{
...
bool CamGrid::IsColumnSortable(long column)
{
	if (column > 0 && column <= m_totCols)
	{
		Ccolumn* ccol = m_columns.GetAt(column-1);
...
CString CamGrid::GetColumnName(long column)
{
	CString	name;
...
//		name = GetSymbolName((*m_cellR)[ccol->m_ref]->name);
		name = GetSymbolName(ccol->m_ref);	//GetSymbolName(ccol->m_ref);
...
void CamGrid::SetDataPosition(long method)
{
	switch (method)
	{
	case 1:
...
void CamGrid::SetItems(int col, CString items)
{
...
void CamGrid::SetCellFocus(long row, long col) 
{
	CRect	tRc;
	Ccolumn* column;
...
void CamGrid::Import(CString files)
{
	CFile	file;
	DWORD	size;
	char*	wb;
...
void CamGrid::SetCellAttr(long row, long column, long attrAdd, long attrRemove)
{
	if (!(m_form->attr2 & GO_TABLE))
		return;
...
void CamGrid::DoListBox(bool apply, bool focus)
{
...
bool CamGrid::IsFocus(HWND hWnd)
{
