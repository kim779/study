...
IMPLEMENT_DYNCREATE(CamEdit, CfmBase)
...
CamEdit::CamEdit(CAxisForm* axform, struct _formR* formR, bool drawOnly)
...
BEGIN_MESSAGE_MAP(CamEdit, CfmBase)
	//{{AFX_MSG_MAP(CamEdit)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

BEGIN_DISPATCH_MAP(CamEdit, CfmBase)
...
BEGIN_INTERFACE_MAP(CamEdit, CfmBase)
...
void CamEdit::_setData(LPCTSTR lpszNewValue) 
{
...
long CamEdit::_getPRgb() 
{
	return m_pRGB;
}

void CamEdit::_setPRgb(long nNewValue) 
{
	CfmBase::SetBkColor(nNewValue);
}

long CamEdit::_getTRgb() 
{
	return m_tRGB;
}

void CamEdit::_setTRgb(long nNewValue) 
{
	CfmBase::SetFgColor(nNewValue);
}

BOOL CamEdit::_getVisible() 
{
	return (m_form->properties & PR_VISIBLE) ? TRUE : FALSE;
}

void CamEdit::_setVisible(BOOL bNewValue) 
{
	CfmBase::SetVisible(bNewValue ? true : false);
}

BOOL CamEdit::_getEnable() 
{
	return (m_form->properties & PR_ENABLE) ? TRUE : FALSE;
}

void CamEdit::_setEnable(BOOL bNewValue) 
{
	CfmBase::SetEnable(bNewValue ? true : false);
}

BOOL CamEdit::_getSkip() 
{
	return (m_form->attr & FA_SKIP) ? TRUE : FALSE;
}

void CamEdit::_setSkip(BOOL bNewValue) 
{
	if (bNewValue)
		m_form->attr |= FA_SKIP;
	else
...
//	int	ndc = dc->SaveDC();
	...
	*/
	...
	//	dc->FillRect(pRc, brush);
		...
		g.FillRectangle(setgBrush(clDBLUE), RECTG(pRc));
	}
	else
	{
