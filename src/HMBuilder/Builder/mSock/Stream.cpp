...
CStream::CStream(CWsh* rsm, CWsh* wsh, CString home)
{
	m_idx    = 0;
	m_flow   = _frameF_;
...
//	path.Format("%s/axsock.trc", home);

	path.Format("c:\\axsock.trc");
	m_log = new CLog((char *)path.operator LPCTSTR());
#endif
}
...
void CStream::OnReceive(int nErrorCode) 
{
	...
	TRACE("OnReceive\n");
	TRACE("%s\n", CString(m_frameB, m_frameL));
#ifdef	_DEBUG
	m_log->Trace(m_frameB, m_frameL, dirRCV);	// packed data
#endif	
	...
	struct _rsmH* prsm = (struct _rsmH*)(m_frameB + L_fmH);
...
#ifdef	_DEBUG
	m_log->Trace(m_chain, m_chainL, dirRCV);
#endif
...
bool CStream::RunVersion(char* keys, int size)
{
	...
	((struct _rsmH *)sndb)->resC = resC_INFO;
...
bool CStream::WriteData(char *data, int ndat, bool wait)
{
	...
	text.Format("%08d", ndat);
	CopyMemory(fmh->datL, (char *)text.operator LPCSTR(), min(sizeof(fmh->datL), text.GetLength()));
//	sprintf(fmh->datL, "%08d", ndat);


	int	nBytes = ndat;
//	int nBytes = m_zip->compress((BYTE *) data, ndat, (BYTE *) &m_pBytesToData[L_fmH]);
...
void CStream::CloseStream()
{
	...
	{
		char	msg[80];
		sprintf_s(msg, sizeof(msg), "Close(%s, %d)", name, port);
		m_log->Trace(msg, 0, dirSTR);
	}
#endif
	m_alive = false;
