...
void CMDITabs::SetPosition(CRect rect)
{
...
//	else
	{
		...
		if (m_bTop)
			MoveWindow(rect.left, rect.top, m_width, m_height, true);
		else
			MoveWindow(rect.left, rect.bottom - m_height, m_width, m_height, true);
...
void CMDITabs::Update()
{
	...
	TCITEM	item;
	char	text[256];
	int	l_text = sizeof(text);
	item.pszText = text;
	
	for (int ii = GetItemCount(); ii--;)  // for each tab
	{
		item.mask = TCIF_PARAM;
		GetItem(ii, &item);
		...
		{
			item.mask = TCIF_TEXT;
			ZeroMemory(text, l_text);
			pos = ::GetWindowText(*it, text, l_text);
			CopyMemory(&text[pos], "    ", 4);
			if (m_bImages)
...
//	if ((!bShow && IsWindowVisible()) || (bShow && !IsWindowVisible())) 
	{
		static_cast<CMDIFrameWndEx*>(FromHandlePermanent(::GetParent(m_mdiClient)))->RecalcLayout();
	}
...
void CMDITabs::Create(CFrameWnd* pMainFrame, CRect rect, DWORD dwStyle)
{
