...
IMPLEMENT_DYNCREATE(CamBrowser, CamBase)

CamBrowser::CamBrowser()
{
	EnableAutomation();
}

CamBrowser::CamBrowser(CAmForm* axform, struct _formR* formR, bool drawOnly)
...
CamBrowser::~CamBrowser()
{
	if (!m_drawOnly && m_browser)
	{
		...
		//	m_browser->PostMessage(WM_DESTROY);
		}
	}
...
void CamBrowser::OnFinalRelease()
{
...
BEGIN_MESSAGE_MAP(CamBrowser, CamBase)
	//{{AFX_MSG_MAP(CamBrowser)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

BEGIN_DISPATCH_MAP(CamBrowser, CamBase)
	//{{AFX_DISPATCH_MAP(CamBrowser)
	/*
	DISP_PROPERTY_EX(CamBrowser, "Text", _getText, _setText, VT_BSTR)
	DISP_PROPERTY_EX(CamBrowser, "Visible", _getVisible, _setVisible, VT_BOOL)
	DISP_FUNCTION(CamBrowser, "Refresh", _Refresh, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION(CamBrowser, "Navigate", _Navigate, VT_EMPTY, VTS_BSTR)
	*/
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
BEGIN_INTERFACE_MAP(CamBrowser, CamBase)
	INTERFACE_PART(CamBrowser, IID_IfmBrowser, Dispatch)
END_INTERFACE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CamBrowser message handlers

void CamBrowser::_Refresh() 
{
	Refresh();
}

void CamBrowser::_Navigate(LPCTSTR url) 
{
	WriteData(url);
}

LPCTSTR CamBrowser::GetText() 
{
	CString strResult;
...
void CamBrowser::SetText(LPCTSTR lpszNewValue) 
{
	WriteData(lpszNewValue);
}

BOOL CamBrowser::GetVisible() 
{
	return (m_form->properties & PR_VISIBLE) ? TRUE : FALSE;
}

void CamBrowser::SetVisible(BOOL bNewValue) 
{
	_SetVisible(bNewValue ? true : false);
}

void CamBrowser::Adjust(CSize size)
{
	CamBase::Adjust(size);
...
void CamBrowser::Adjust(CPoint pt, float hR, float vR)
{
...
void CamBrowser::Resize(float hR, float vR, bool font, bool redraw)
{
	m_hR = hR;
	m_vR = vR;
...
void CamBrowser::Draw(CDC* dc)
{
	...
	if (m_drawOnly)
	{
		...
		//g.DrawString(m_strR.AllocSysString(), -1, NULL, RECTF(m_pRc), &sf, setgBrush(clBLACK));	// font ºüÁü
	}
	else if (m_hastool)
	{
		...
		for (int ii = 0; ii < TOOL_COUNT; ii++)
		{
			...
			if (ii == m_hot)
			{
				if (m_hImage.GetSafeHandle() && ii < m_hImage.GetImageCount())
				{
					...
					drawEdge(dc, tRc, m_down ? EDGE_SUNKEN : EDGE_RAISED, BF_RECT);
				}
			}
...
void CamBrowser::OnLButton(bool down, CPoint pt, int& result)
{
	CRect	tRc;
	int	hot;
...
int CamBrowser::OnDrag(CPoint pt, int& result)
{
	CRect	tRc;
	int	hot = m_hot;
...
void CamBrowser::WriteAll(CString str, bool redraw)
{
	if (m_file)
	{
		m_file = false;
	...
	if (str.Find("text://") == 0)
	{
		...
		tmps.Format("%s\\%s\\%s%X.html", m_axform->m_root, USRDIR, m_name, GetTickCount());
...
void CamBrowser::WriteData(CString data, bool redraw, int col, int row)
{
	if (m_file)
	{
		m_file = false;
...
void CamBrowser::_SetVisible(bool visible, int col)
{
...
bool CamBrowser::IsFocus(HWND hWnd)
{
...
void CamBrowser::ShowControl(BOOL visible)
{
	if (m_browser)
		m_browser->ShowWindow(visible ? SW_SHOW : SW_HIDE);
}

void CamBrowser::loadBitmap()
{
	CString	path;
	CBitmap	bitmap;
	HBITMAP	hbitmap;
...
int CamBrowser::getIndex(CPoint pt)
{
	CRect	tRc;
	for (int ii = 0; ii < TOOL_COUNT; ii++)
	{
...
CRect CamBrowser::getRect(int index)
{
	CRect	tRc = m_toolRc;
