	...
	*/
	if (attr & FA_COMMA || attr & FA_FLOAT)
	{
		...
		if (attr & FA_FLOAT)
		{
			...
			{
				for (ii = 0; ii < (int)(*m_cellR)[column->m_ref]->digit; ii++)
				{
					if (ii > valS.GetLength()-1)
					{
						tmps += '0';
...
void CamGrid::makeSignData(CString& data, int col, int row)
{
	...
	if (row >= 0)
	{
		if (((*m_cellR)[column->m_ref]->attr2 & SA_CORGB) == SA_PAINT)
		{
			if (state != ct_Flat)	//(state == ct_uLimit || state == ct_lLimit)
			{
...
//	dc->FillRect(rc, brush);
	...
	if (combo < column->m_display.GetSize())
	{
		...
		//mg.DrawString(tmps.AllocSysString(), -1, setgFont(), RECTF(tRc), &sf, setgBrush(GRGB(m_tRGB)));
	...
	if (m_form->attr2 & GO_X3D)
	{
		...
		else
		{
			...
			if (rBtn.left >= rc.left)
			{
				...
				mg.FillRectangle(setgBrush(GRGB(bg)), RECTG(tRc));
				...
				mg.DrawLine(setgPen(GRGB(bg)), rBtn.left, rBtn.top+1, rBtn.left, rBtn.bottom-1);
				mg.DrawLine(setgPen(GRGB(bg)), rBtn.right-1, rBtn.top+1, rBtn.right-1, rBtn.bottom-1);
				...
				//pts[2].x = tRc.left;
				//dc->Polygon(pts, 3);
				mg.DrawPolygon(setgPen(GRGB(fg)), pts, 3);
				mg.FillPolygon(setgBrush(GRGB(fg)), pts, 3, FillModeAlternate);
	...
	else
	{
		...
		{	
			...
			mg.DrawImage(pBitmap, rBtn.left, rBtn.top, pBitmap->GetWidth(), pBitmap->GetHeight());
		}
		else
			rBtn.left = rc.left;
	}
...
//	setFont(dc, ((*m_cellR)[column->m_ref]->properties & PR_BOLD) ? 1 : 0);
	...
	mg.DrawImage(pBitmap, tRc.left, tRc.top, pBitmap->GetWidth(), pBitmap->GetHeight());
	...
	if (rc.Height() < rc.Width())
	{
		...
		sf.SetHotkeyPrefix(HotkeyPrefixNone);
		sf.SetAlignment(StringAlignmentCenter);
		//mg.DrawString(column->m_edit.AllocSysString(), -1, setgFont(), RECTF(tmpRc), &sf, setgBrush(GRGB(tcf)));
	}
	CachedBitmap cache(&bitmap, &g);
	g.DrawCachedBitmap(&cache, rc.left, rc.top);
}
