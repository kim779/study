...
BEGIN_MESSAGE_MAP(CCrystalEditView, CCrystalTextView)
	...
	ON_COMMAND(ID_EDIT_LINECUT, OnRemoveLine)
	//}}AFX_MSG_MAP
...
void CCrystalEditView::OnEditDelete() 
{
	...
	if (ptSelStart == ptSelEnd)
	{
		...
		else
		{
			...
			if (ptSelEnd.x > 0 && IsDBCSLeadByte((BYTE)pszChars[ptSelEnd.x]) && !IsLeadByte(pszChars, ptSelEnd.x))
				ptSelEnd.x ++;
		}
	}
...
void CCrystalEditView::OnEditDeleteBack() 
{
	...
	else										// If Caret Not At SOL
	{
		...
		if (ptCursorPos.x > 0 && (BYTE)pszChars[ptCursorPos.x] >= 0x80)
			ptCursorPos.x --;
		bDeleted = true;							// Set Deleted Flag
	}
...
void CCrystalEditView::OnEditReplace() 
{
	...
	dlg.m_matchCase = pApp->GetProfileInt(REG_REPLACE_SUBKEY, REG_MATCH_CASE, FALSE);
	dlg.m_wholeWord = pApp->GetProfileInt(REG_REPLACE_SUBKEY, REG_WHOLE_WORD, FALSE);
	...
	if (IsSelection())
	{
		...
		dlg.m_scope = 0;	// Replace in current selection
		dlg.m_ptCurrentPos = m_ptSavedSelStart;
		dlg.m_enableScopeSelection = TRUE;
		dlg.m_ptBlockBegin = m_ptSavedSelStart;
		dlg.m_ptBlockEnd = m_ptSavedSelEnd;
	}
	else
	{
		dlg.m_scope = 1;	// Replace in whole text
		dlg.m_ptCurrentPos = GetCursorPos();
		dlg.m_enableScopeSelection = FALSE;
	}
	...
	pApp->WriteProfileInt(REG_REPLACE_SUBKEY, REG_MATCH_CASE, dlg.m_matchCase);
	pApp->WriteProfileInt(REG_REPLACE_SUBKEY, REG_WHOLE_WORD, dlg.m_wholeWord);
...
void CCrystalEditView::OnEditOperation(int nAction, LPCTSTR pszText)
{
	if (m_bAutoIndent)
	{
		...
		if (nAction == CE_ACTION_TYPING && _tcscmp(pszText, _T("\r\n")) == 0 && ! m_bOvrMode)
		{
			...
			int nPos = 0;
			while (nPos < nLength && (BYTE)pszLineChars[nPos] < 0x80 && isspace(pszLineChars[nPos]))
				nPos ++;
			...
			if (nPos > 0)
			{
				...
				_tcsncpy_s(pszInsertStr, sizeof(TCHAR) * (nLength + 1), pszLineChars, nPos);
				pszInsertStr[nPos] = 0;
...
void CCrystalEditView::OnRemoveLine()
{
	if (! QueryEditable())
		return;
	if (m_pTextBuffer == NULL)
