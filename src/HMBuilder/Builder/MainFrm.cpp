...
#include "amDlg/ADBDlg.h"
#include "resource.h"
//#include "amDlg/SearchDlg.h"
#include <direct.h>
...
CMainFrame::CMainFrame()
{
	...
	m_EmulResolution = 1.0;	// Emulation Resolution
	m_mapLanguage.RemoveAll();

	m_pADBDlg = nullptr;
	m_MergePath.Empty();
	m_pSearchDlg = nullptr;
}
...
LRESULT CMainFrame::OnFireEvent(WPARAM wParam, LPARAM lParam)
{
	...
	case FEV_OPEN:
		...
		break;
	case FEV_CLOSE:
		AfxMessageBox("Different User Writing...");
		break;
	case FEV_GUIDE:
		break;
	case FEV_ERROR:
...
BOOL CMainFrame::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN)
	{
		...
		case VK_ESCAPE:			
			...
			if (!child->m_bSubChild || !child->m_bEmulRun)
				break;
...
void CMainFrame::OnClose()
{
	...
	for (ii = 0; ii < m_Line.GetCount(); ii++)
	{
		rc = m_Line.GetAt(ii);
		sKey.Format(_T("%d"), ii);
		sData.Format("%d %d %d %d", rc.left, rc.top, rc.right, rc.bottom);
		WritePrivateProfileString(STANDARDLINE, sKey, sData, m_currentPath + _T("\\workshop.ini"));
	}
	sKey.Format(_T("%d"), ii);
	WritePrivateProfileString(STANDARDLINE, sKey, "", m_currentPath + _T("\\workshop.ini"));
	...
	case ID_APP_SETTING:
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				m_EmulResolution = dlg.m_basePage->m_EmulSize / 100.0;
...
//		break;
	...
	case ID_EDIT_GOTO:
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				else if (child)
				{
					...
					//m_wndScript.GetBookmark(NULL, &lineNum, &idx);
	...
	case ID_VIEW_STANDARDLINE:
		OnViewStandardline();
		break;
	case ID_VIEW_INPUTLANGUAGE:
		OnViewLanguage();
		break;
	case ID_VIEW_LOGCAT:
		ShowADBViewer();
		break;
	case ID_VIEW_FORMSEARCH:
		FormSearch();
		break;
	default:
		break;
...
void CMainFrame::DestroyEmulator()
{
	CChildFrame *childTmp, *childTmp2;
	CChildFrame* child = (CChildFrame *)MDIGetActive();
	if (m_mapEmul.GetCount() > 0)
	{
		for (POSITION pos = m_mapEmul.GetStartPosition(); pos;)
		{
			// 동작중인 에뮬 종료
			m_mapEmul.GetNextAssoc(pos, childTmp, childTmp2);
			if (child == childTmp2 || child == childTmp)
			{
				// 원본 정리
				childTmp->m_bEmulRun = false;
				childTmp->m_bSubChild = false;
				childTmp->m_view.m_bEmulRun = false;
				m_wndScript.m_bEmulRun = false;
				// Disable -> Enable
				m_wndProperties.EnableWindow();
				m_wndScript.m_editScript.EnableWindow();
				// Des파일 제거
//				childTmp2->m_view.Emulate();
				childTmp2->DestroyWindow();
				// Map Remove
				m_mapEmul.RemoveAll();
				return;
			}
...
void CMainFrame::EmulatorRun(CChildFrame *child)
{
	CChildFrame *childTmp, *childTmp2;	

	// Emul의 Child일 경우 - 에뮬 종료
	if (m_mapEmul.GetCount() > 0)
	{
		for (POSITION pos = m_mapEmul.GetStartPosition(); pos;)
		{
	...
	// 현재 Child가 원본 Form인 경우
	// Emul 실행중인지 체크
	if (m_wndScript.m_bEmulRun && m_mapEmul.GetCount() > 0)
	{
		AfxMessageBox("Different Emulator Running...!!!");
		return;
	}
	...
	// Emul 복제
	childTmp2 = createEmulChild(child);
	m_mapEmul.SetAt(child, childTmp2);

	child->m_bEmulRun = true;	
	child->m_view.m_bEmulRun = true;
	childTmp2->m_bEmulRun = true;
	childTmp2->m_bSubChild = true;
	childTmp2->m_view.m_bEmulRun = true;	
	m_wndScript.m_bEmulRun = true;

	// Property Grid Enable/Disable
	m_wndProperties.EnableWindow(FALSE);
	// SCript Wnd Text Enable/Disable
	m_wndScript.m_editScript.EnableWindow(FALSE);
	childTmp2->m_view.Emulate();	
}
...
void CMainFrame::OnDrawCommand(UINT cmdID)
{
	...
	if (child)
	{
		...
		if (cmdID == ID_TOOL_CONTROL)
			text = TrackPopupControl();
		child->DrawForm(cmdID, text);
	}
}

CString CMainFrame::TrackPopupControl()
{
...
void CMainFrame::OnUpdateMainCommand(CCmdUI *pCmdUI)
{
	...
	case ID_COMPILE_COMPILE:
	case ID_COMPILE_MOVE:
		if (child && (child->m_mapH.mapK == MK_NORM || child->m_mapH.mapK == MK_PROCEDURES) && !child->m_view.m_pEmulateWnd && !child->m_bEmulRun)
			enable = TRUE;
		break;
...
LRESULT CMainFrame::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case ID_USRID:
		return (LRESULT)m_userID.operator LPCSTR();
		break;
	case ID_USR_SETTING:
		{
	...
	case ID_USR_REFRESHCTRL:	// no activate
		if (!pActiveChild)
		{
			...
			m_wndScript.SetWindowText(CString("script window"));
			return 0;
		}
		break;
	...
	case ID_USR_TRLAYOUT:
		if (m_trLayout && m_trLayout->IsWindowVisible())
			trlayout();
		break;
	case ID_USR_RTSINFO:
	...
	case ID_USR_REFRESHCTRL:	// from child to propertiesbar refresh control (lParam = bDelete)
		...
		if (!(LOWORD(lParam) & 2))
		{
			...
			m_wndProperties.SelectControl(pActiveChild->GetSelectedForm(), pActiveChild->IsLayoutMode());	// Layout 추가
			m_wndScript.SelectControl(pActiveChild->GetSelectedForm(), true);
	...
	case ID_USR_EMULCLOSE:
		{
			CChildFrame* child;
			CChildFrame* childOld;
			childOld = (CChildFrame *)MDIGetActive();
			if (childOld && (childOld->m_view.m_pEmulateWnd || childOld->m_bEmulRun))
			{
				for (POSITION pos = m_mapEmul.GetStartPosition(); pos;)
				{
					CChildFrame* ptmpchild;
					m_mapEmul.GetNextAssoc(pos, ptmpchild, child);
					if (childOld == child || childOld == ptmpchild)
					{
						m_mapEmul.RemoveKey(child);

						ptmpchild->m_bEmulRun = !ptmpchild->m_bEmulRun;
						ptmpchild->m_view.m_bEmulRun = !ptmpchild->m_view.m_bEmulRun;
						ptmpchild->MDIActivate();
						// Property Grid Enable/Disable
						m_wndProperties.EnableWindow();
						// SCript Wnd Text Enable/Disable
						m_wndScript.m_bEmulRun = false;
	...
	case ID_USR_LAYOUTMODE:
		{
			CString Caption;
			CString mode = pActiveChild->m_formItem->GetCurrLayoutName();
	...
	case ID_USR_STANDARDLINE:
		return (LONG)(&m_Line);
		break;
	case ID_USR_SHOWDIFF:
		{
			CChildFrame* child = (CChildFrame *)MDIGetActive();
			ShowDiff(child->m_mapH.mapN, (int)(lParam));
		}
		break;
...
BOOL CMainFrame::createToolBars()
{
	...
	m_mainTB.SetWindowText(_T("System"));
	...
	m_drawTB.SetWindowText(_T("Object"));
...
int CMainFrame::initDir(CMapStringToString& maps)
{
	...
	m_root = CString(wb, idir);
	m_root = m_root.Left(m_root.ReverseFind('\\'));
	...
	if (!(idir & FILE_ATTRIBUTE_DIRECTORY) || m_root.IsEmpty())
	{
		...
		WritePrivateProfileString(ENVIRONMENT, ROOTDIR, m_root, m_currentPath + _T("\\workshop.ini"));
	...
	m_SourcePath = wb;	
	//////////////////////////////////////////////////////////////////////////////
	GetPrivateProfileString(ENVIRONMENT, "MERGE", m_currentPath, wb, sizeof(wb), m_currentPath + _T("\\workshop.ini"));
	m_MergePath = wb;


	//////////////////////////////////////////////////////////////////////////////
	// Emulation Resolution
	int resolution = GetPrivateProfileInt(ENVIRONMENT, _T("Emul_Resolution"), 100, m_currentPath + _T("\\workshop.ini"));
	m_EmulResolution = resolution / 100.0;
...
void CMainFrame::createNewChild(int mapK)
{
	...
	if (!layout.IsEmpty())
	{
		...
		child->SetWindowPos(NULL, 0, 0, atoi(width) + delta.cx, atoi(height) + delta.cy, SWP_NOMOVE | SWP_SHOWWINDOW);
...
/*
struct _mapH CMainFrame::CopyMAPH(int nCnt, struct _mapH* mapH)
{
	int nLen = 0;
	struct _mapH* src_mapH;
	//src_mapH = *mapH;

	src_mapH = (_mapH *) new char[sizeof(_mapH)];
	ZeroMemory(src_mapH, sizeof(_mapH));
	CopyMemory(src_mapH, mapH, sizeof(_mapH));

	if (mapH->trinfo)
	{
		src_mapH->trinfo = new char[sizeof(mapH->trinfo)];
		ZeroMemory(src_mapH->trinfo, sizeof(mapH->trinfo));
		CopyMemory(src_mapH->trinfo, mapH->trinfo, sizeof(mapH->trinfo));
	}
	if (mapH->rtsinfo)
	{
		src_mapH->rtsinfo = new char[sizeof(mapH->rtsinfo)];
		ZeroMemory(src_mapH->rtsinfo, sizeof(mapH->rtsinfo));
		CopyMemory(src_mapH->rtsinfo, mapH->trinfo, sizeof(mapH->rtsinfo));
	}

	if (nCnt <= 0)
		return *src_mapH;
	
	src_mapH->formR = (_formR *) new char[sizeof(_formR)];
	ZeroMemory(src_mapH->formR, sizeof(_formR));

	struct _formR* formSrc = (_formR *) mapH->formR;
	struct _formR* formDes = (_formR *) src_mapH->formR;
	...
	for (int ii = 0; ii < nCnt; ii++)
	{
		...
		case FM_TABLE:
				if (formSrc->auxR != NULL)
				{
					...
					while (cellSrc != NULL)
					{
						...
						cellSrc = cellSrc->next; cellBak = cellDes; cellDes = NULL;
					}
				}
			break;
		default:
		...
		formSrc = formSrc->next; backDes = formDes; formDes = NULL;
	}
...
struct _mapH CMainFrame::CopyMAPH(int nCnt, struct _mapH* mapH)
{
	...
	src_mapH->trinfo = NULL;
	src_mapH->rtsinfo = NULL;
	src_mapH->formR = NULL;
	src_mapH->pUndoMap = NULL;
	return *src_mapH;
}

CChildFrame* CMainFrame::createEmulChild(CChildFrame* childOld)
{	
	int	formN, repN, grN;
	CString tmps;
	...
	child->m_mapH = CopyMAPH(childOld->m_formItem->GetCount(), &childOld->m_mapH);		
	child->ChangeKind();
	...
	//
	CString strCaption;
	strCaption = CString(child->m_mapH.caption) + _T(" - Running");
	child->SetWindowText(strCaption);
	...
	child->SetWindowPos(NULL, m_childPnt.x, m_childPnt.y, (int)(child->m_mapH.width * m_EmulResolution) + delta.cx, (int)(child->m_mapH.height * m_EmulResolution) + delta.cy, SWP_SHOWWINDOW);
	child->SetNew(false);
...
CChildFrame* CMainFrame::openFile(CString path, bool bDefault)
{
	int	offs;
	...
	// Download Resource not delete...
	if (tmps.CompareNoCase(path))
	{
		DeleteFile(tmps);
		CopyFile(path, tmps, FALSE); // by NMH
	}
...
void CMainFrame::openLocalFile()
{
	...
	CFileDialog fDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_OVERWRITEPROMPT, _T("XML Files (*.XML)|*.XML||"));
...
int CMainFrame::generateMap(bool alarm, CChildFrame *child)
{
	...
	if (m_wndScript.m_bwhole)
		m_wndScript.SetAllScript(&child->m_mapH);
	else
		m_wndScript.WriteScript();
...
//	CMFCToolBar::AddToolBarForImageCollection(IDR_MENU_IMAGES,IDB_MENU_IMAGES_24);
	...
	int nCount = 0, nVal = 0;;
	CRect rcline;
	CString sData, sKey, sRc;
	m_Line.RemoveAll();
	sKey.Format(_T("%d"), nCount);
	GetPrivateProfileString(STANDARDLINE, sKey, _T(""), sData.GetBuffer(256), 256, m_currentPath + _T("\\workshop.ini"));
	sData.ReleaseBuffer();
	...
	{	
		for (int ii = 0; ii < 4; ii++)
		{
			AfxExtractSubString(sRc, sData, ii, ' ');
			nVal = atoi(sRc);
			switch (ii)
			{
			case 0:
...
void CMainFrame::rtsInfo()
{
	...
	if (m_children.GetSize() && pActiveChild)
	{
		...
		for (int ii = 0; ii < pActiveChild->m_formItem->GetCount(); ii++)
		{
			if (!pActiveChild->m_formItem->GetFormStr(ii, _T("name")).IsEmpty())
			{
				symbol = pActiveChild->m_formItem->GetFormStr(ii, _T("rts"));
				...
				case FM_GRID:
					...
					for (int jj = 0; jj < repR->cols; jj++)
					{
						...
						if (cellR->attr & FA_SKIP)
							continue;
						tmps.Format(_T(":%s=%s"), cellR->name, cellR->rtssy);
						tmp += tmps;
					}
				...
				case FM_COMBO:
					tmp += pActiveChild->m_formItem->GetFormStr(ii, _T("name"));
					tmps.Format(_T("=%s=%s%c"), _T("COMBO"), symbol, GB_SYMBOL);
					tmp += tmps;
					break;
				case FM_CONTROL:
					tmp += pActiveChild->m_formItem->GetFormStr(ii, _T("name"));
					tmps.Format(_T("=%s=%s%c"), _T("CONTROL"), symbol, GB_SYMBOL);
					tmp += tmps;
					break;
				case FM_OUT:
					tmp += pActiveChild->m_formItem->GetFormStr(ii, _T("name"));
					tmps.Format(_T("=%s=%s%c"), _T("OUT"), symbol, GB_SYMBOL);
					tmp += tmps;
					break;
				case FM_EDIT:
					tmp += pActiveChild->m_formItem->GetFormStr(ii, _T("name"));
					tmps.Format(_T("=%s=%s%c"), _T("EDIT"), symbol, GB_SYMBOL);
					tmp += tmps;
					break;
				}
			}
...
void CMainFrame::trlayout(CString path, bool breset)
{
	...
	if (m_children.GetSize() && pActiveChild)
	{
		...
		if (m_trLayout == NULL)
		{
			m_trLayout = new CTrLayOut(this);
			m_trLayout->Create(IDD_TRLAYOUT);
			m_trLayout->ShowWindow(SW_SHOW);
		}

		if (path.IsEmpty())// && pActiveChild->m_mapH.trinfo != NULL)
		{
			CString trName, trinfo;
			int	pos;

			if (pActiveChild->m_mapH.trinfo != NULL)
		...
		m_trLayout->readTrList(path);
		if (!path.IsEmpty())
			bfile = true;

		if (!tmp.IsEmpty())
			m_trLayout->formObjectLayout(&pActiveChild->m_mapH, tmp, bfile);	
	}
}
...
void CMainFrame::SettTrRtsDataset(int kind, CString data)
{
	...
	if (pActiveChild)
	{
		...
		case 0:
			if (pActiveChild->m_mapH.trinfo != NULL)
				delete[] pActiveChild->m_mapH.trinfo;
			len = strlen(data);
		...
		case 1:
			if (pActiveChild->m_mapH.rtsinfo != NULL)
				delete[] pActiveChild->m_mapH.rtsinfo;
			len = strlen(data);
...
LRESULT CMainFrame::trMaptree(WPARAM wParam, LPARAM lParam)
{
	...
	CString tmps = CString(rsmH->erno, sizeof(rsmH->erno));
	...
	if (atoi(tmps) > 0)
	{
		...
		tmps.Format(_T("errNo [%s]\n%s\n\nIt does not log-in to Server, it is offline mode."),
			CString(rsmH->erno, sizeof(rsmH->erno)), CString(rsmH->errmsg, sizeof(rsmH->errmsg)).Trim());
		MessageBox(tmps, "Message", MB_OK);
	//	MessageBox("It does not log-in to Server, it is offline mode.", "Message", MB_OK);
		return 0;
	}
	...
	ShowPane(&m_mainTB, TRUE, FALSE, FALSE);
...
void CMainFrame::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (nIDEvent == 500)
	{
		...
		download->Download(m_userID, _T(""), _T(""), tmps, TRGB_FORMALL, 0, 0, false);
		delete download;

		download = new CDownload();
		download->Create(NULL, NULL, WS_CHILD, CRect(0, 0, 0, 0), this, 2000);
		wccSetTempPath(tmps, TR_FILE);
		download->Download(m_userID, _T(""), _T(""), tmps, TRGB_TRLAYOUT, 0, 0, false);
		delete download;
	}
...
void CMainFrame::Login(CString sUserID, CString sPass)
{
	...
	CopyMemory(plog->pswd,  sPass,   min(sizeof(plog->pswd),  sPass.GetLength())   );
...
void CMainFrame::DownMapDlg(CString sName, int mode, int nSel)
{
	CString sRoot;
	sRoot.Format("%s\\exe\\tmp\\%s", m_root, sName);
	CDownload* pDown = new CDownload();
	pDown->Create(NULL, NULL, WS_CHILD, CRect(0, 0, 0, 0), this, 10);
	pDown->Download(m_userID, sName, "", sRoot, TRGB_FORMSOURCE, mode, nSel);
...
void CMainFrame::DownTRlayOut(CString sName, int nSel)
{
	...
	CDownload* pDown = new CDownload();
	pDown->Create(NULL, NULL, WS_CHILD, CRect(0, 0, 0, 0), this, 10);
	pDown->Download(m_userID, sName, "", sRoot, TRGB_FORMSOURCE, 0, nSel);
...
void CMainFrame::moveMobileDevice()
{
	...
	char	wb[512];
	GetPrivateProfileString(_T("MOBILE"), _T("RUNPATH"), _T("com.winix.axMobileApp/com.winix.app.WinixActivity"), wb, sizeof(wb), m_currentPath + _T("\\workshop.ini"));
	CString runAPP = wb;
	GetPrivateProfileString(_T("MOBILE"), _T("FORMPATH"), _T("/sdcard/WinixMobile"), wb, sizeof(wb), m_currentPath + _T("\\workshop.ini"));
	...
	if (strFormPath.Right(1) == '/')
		strFormPath = strFormPath.Left(strFormPath.GetLength()-1);

	if (strFormPath.Find(_T("form")) == -1)
		strFormPath += "/form";

	CString	string;
	string.Format("\"%s\" push \"%s\" %s", adbPath, mapPath, strFormPath);
	...
	{
		TRACE("errno = [%s]\n", GetLastError());
		AfxMessageBox("Form Copy Error");
		return;
	}
	WaitForSingleObject(pi.hProcess, INFINITE);

	string.Format("\"%s\" shell sync", adbPath);
	ZeroMemory(&si, sizeof(STARTUPINFO));
	ZeroMemory(&pi, sizeof(PROCESS_INFORMATION));
	si.cb = sizeof(STARTUPINFO);
	si.dwFlags = STARTF_USESHOWWINDOW;
	...
	if (!bRc)
	{
		AfxMessageBox("Application running error");
		return;
	}
	WaitForSingleObject(pi.hProcess, INFINITE);
}
...
void CMainFrame::OnViewStandardline()
{
	...
	pchild->m_view.m_bLine = !pchild->m_view.m_bLine;
	
	if (pchild->m_view.m_bLine)	Caption.Format("%s - Line Mode", pchild->m_mapH.caption);
	else                            Caption.Format("%s", pchild->m_mapH.caption);
	
	pchild->SetWindowText(Caption);
	m_bLayoutMode = true;
}

void CMainFrame::OnViewLanguage()
{
	HWND hWnd = GetSafeHwnd();
	CString sProgram = FORMAT("%s\\%s\\inputl.EXE %p", m_root, RUNDIR, GetSafeHwnd());

	ExecuteCreateProcess(sProgram, sProgram);
}

void CMainFrame::ExecuteCreateProcess(CString sAppName, CString sCommand/*=_T("")*/)
{
	STARTUPINFO si;
	ZeroMemory(&si, sizeof(STARTUPINFO));
	si.cb = sizeof(STARTUPINFO);
...
//	ping.exe - n 100
	...
	CreateProcess(NULL,									// application name
		sCommand.GetBuffer(),								// command line
		NULL,										// process attribute
		NULL,										// thread attribute
		FALSE,										// is inherit handle
		0,										// creation flags
		NULL,										// environment
		sAppName.Left(sAppName.ReverseFind('\\')),					// current directory
		&si,										// STARTUPINFO
		&pi);
}

bool CMainFrame::loadLanguage()
{
	CFile file;
	CString sPath;
	CStringW sData;
	...
	BOOL bflag = file.Open(sPath, CFile::modeRead | CFile::typeBinary);
	...
	CStringW sHeader, sTxt;
	CArray<CStringW, CStringW> sArrRow;

	sHeader = ParserW(sData, CStringW(_T("\n")));
	if (sHeader.IsEmpty())
		return false;

	while (!sData.IsEmpty())
	{
		sTxt = ParserW(sData, CStringW(_T("\n")));
		sTxt = ParserW(sTxt, CStringW(_T("\t")));
		sTxt.Remove('\r');
		sArrRow.Add(sTxt);
	}
	...
	CString ss;
	for (int ii = 0; ii < sArrRow.GetCount(); ii++)
	{
		ss.Empty();
		sTxt = sArrRow.GetAt(ii);
		int len = sTxt.GetLength() * 2;
		::WideCharToMultiByte(CP_ACP, 0, sTxt, -1, ss.GetBuffer(len), len, NULL, NULL);
		ss.ReleaseBuffer();
		m_mapLanguage.SetAt(ss, ss); 
	}
	return true;
}


CStringW CMainFrame::ParserW(CStringW &srcstr, CStringW substr)
{
	if (srcstr.Find(substr) == -1)
	{
		CStringW temp = srcstr;
		srcstr.Empty();
		return temp;
	}
	else
	{
		CStringW  temp = srcstr.Left(srcstr.Find(substr));
		srcstr = srcstr.Mid(srcstr.Find(substr) + substr.GetLength());
		return temp;
	}
	return CStringW(_T(""));
...
{
	if (m_pADBDlg == nullptr)
	{
		m_pADBDlg = new CADBDlg();
		m_pADBDlg->Create(IDD_DIALOG_ADB);
	}
	m_pADBDlg->CenterWindow();
	m_pADBDlg->ShowWindow(SW_SHOW);
}

void CMainFrame::ShowDiff(CString sName, int nSel)
{
	CString tmp;
	DownDiff(sName, nSel, nSel - 1);
	if (nSel == -1)
		tmp.Format("%s\\exe\\tmp\\%s_tmp %s\\exe\\tmp\\%s.xml", m_root, sName, m_root, sName);
	else
		tmp.Format("%s\\exe\\tmp\\%s_rev%d %s\\exe\\tmp\\%s_rev%d", m_root, sName, nSel, m_root, sName, nSel - 1);
	ShellExecute(NULL, "open", m_strDiffViewer, tmp, NULL, SW_SHOW);
}

bool CMainFrame::DownDiff(CString sName, int nSel, int nPre)
{
	CString sDiffL, sDiffR;
	sDiffL.Format("%s_rev%d", sName, nSel);
	sDiffR.Format("%s_rev%d", sName, nPre);
	DeleteFile(sDiffL);
	DeleteFile(sDiffR);

	if (sName.Find(_T(".XML")) == -1)
		sName += _T(".XML");

	DownMapDlgEx(sName, sDiffL, nSel);
	DownMapDlgEx(sName, sDiffR, nPre);
	
	return true;
}


void CMainFrame::DownMapDlgEx(CString sName, CString ex, int nSel)
{
	CString sRoot;
	sRoot.Format("%s\\exe\\tmp\\%s", m_root, ex);
	CDownload* pDown = new CDownload();
	pDown->Create(NULL, NULL, WS_CHILD, CRect(0, 0, 0, 0), this, 10);
	pDown->Download(m_userID, sName, "", sRoot, TRGB_FORMSOURCE, 0, nSel);
...
void CMainFrame::SetMasterlayout()
{
	...
	if (!child->m_mapH.source)
		return;
	dir.Format("%s\\mtlayout\\%s.xml", m_root, mapName);
...
void CMainFrame::OpenLayOut()
{
	CString sRoot;
	/*
	m_layoutDlg.SetRoot(m_root);
	if (m_layoutDlg.DoModal() == IDOK)
	{
		sRoot = m_layoutDlg.GetLayOutPath();
...
void CMainFrame::FormSearch()
{
	/*
	if (m_pSearchDlg)
		SAFE_DELETE(m_pSearchDlg);
	m_pSearchDlg = new CSearchDlg();
	m_pSearchDlg->SetRoot(m_root);
	m_pSearchDlg->Create(IDD_SEARCH);
