		...
		case CK_COMBO:
			{
				...
				split(vData, string(data), is_any_of(","), token_compress_on);
				split(vDisp, string(disp), is_any_of(","), token_compress_on);
...
//	setgFont();
	...
	if (m_totCols != 0 && m_stCol != -1)
	{
		...
		for (kk = 0; kk < showRows; kk++)
		{
			...
			{				
				if (kk >= 2)
					index = (m_stRow + 2 + kk)/m_multiN - getrow;
				else
			...
			for ( ; jj < colidx + colcnt ; jj++)
			{
					...
					|| (!(m_form->attr2 & GO_MERGE) && kk < m_multiN)) && m_headN != 0)
				{
					...
					if (draw)
					{
						...
						mg.FillRectangle(setgBrush(col->m_pRGB), RECTF(cRc));    // gdi+ 수정
						if (!tmps.IsEmpty())
						{
							if (m_form->attr2 & GO_MERGE)
							{
							...
							for (ii = 0; ii < col->m_ihead; ii++)
							{
									...
									&& (jj == m_sortc) && (m_dir > 0))
								{
									...
									mg.DrawString(tmps.AllocSysString(), -1, setgFont(), RECTF(hRc), &sf, setgBrush(col->m_tRGB));
							}
						}

						if (!(m_form->attr2 & GO_X3D))							
							drawEdge(mg, cRc, EDGE_RAISED, BF_RECT); // gdi+
						else
						{
	//setPen(dc, m_bRGB);
	//dc->SelectStockObject(NULL_BRUSH);
							...
							if (kk != 0 && cRc.top > m_pRc.top)
								cRc.top--;
							mg.DrawRectangle(setgPen(GRGB(m_bRGB)), RECTF(cRc)); // gdi+ 수정
	//dc->Rectangle(cRc);
						}
					}
				}		// head drawing ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			//		{
			//			mg.DrawRectangle(setgPen(GRGB(pRGB)), RECTF(tRc));
			//			mg.FillRectangle(setgBrush(GRGB(pRGB)),RECTF(tRc)); // gdi+ 수정
			//		}
					if (!noWrite)
					{
						tRc.left++;
					...
					if ((int)dx <= sRc.Width())
					{
						...
						if (!(m_form->attr2 & GO_XHLINE))
						{
							...
							{
								//setPen(dc, pRGB);								
								mg.DrawLine(setgPen(GRGB(pRGB)), tRc.left, tRc.bottom, tRc.right, tRc.bottom);
							}
							else
							{
								//dc->LineTo(tRc.right, tRc.bottom);
								//setPen(dc, m_bRGB);
								mg.DrawLine(setgPen(GRGB(m_bRGB)), tRc.left, tRc.bottom, tRc.right, tRc.bottom);
							}
						...
						if (!(m_form->attr2 & GO_XVLINE))
						{
							...
							mg.DrawLine(setgPen(GRGB(m_bRGB)), tRc.right, tRc.top, tRc.right, tRc.bottom+1);
						}
					}
					else
					{
			...
			if ((int)dx < sRc.Width())
			{
				...
				//dc->FillRect(tRc, brush);
				mg.FillRectangle(setgBrush(clBASIC), RECTF(tRc)); // gdi+ 수정
				mg.DrawRectangle(setgPen(clBASIC), RECTF(tRc));
			}
			...
			|| !m_fixRows))
		{
			...
			//brush = setBrush(dc, getRGB(cLGRAY));//cBASIC));
             		//dc->FillRect(tRc, brush);
			mg.FillRectangle(setgBrush(clLGRAY), RECTF(tRc)); // gdi+ 수정
		}
	}
	rects.RemoveAll();
...
//	else
	{
		...
		//dc->SelectStockObject(NULL_BRUSH);
		//dc->Rectangle(sRc);
		mg.DrawRectangle(setgPen(GRGB(m_bRGB)),RECTF(sRc)); // gdi+ 수정
	}
	...
	if ((m_focus || m_form->attr2 & GO_ALWAYS) && (m_form->isEdit == FI_CELL || m_form->isEdit == FI_RBOX))
	{
		...
		//dc->SelectStockObject(NULL_BRUSH);
		//dc->Rectangle(tRc);
		mg.DrawRectangle(setgPen(clBLACK), RECTF(sRc)); // gdi+ 수정
	}
	...
	if (m_hscroll)
	{
		...
		if (m_form->attr2 & GO_X3D)
		{
			...
			mg.FillRectangle(setgBrush(GRGB(m_sbRGB < 0 ? _ARGB(255, 250, 250, 250) : getgRGB(m_sbRGB))),RECTF(tRc)); // gdi+ 수정
			sRc = m_hscr;
			...
			mg.DrawRectangle(setgPen(GRGB(m_slRGB < 0 ? _ARGB(255, 160, 160, 160) : getgRGB(m_slRGB))), RECTF(m_hthumb)); // gdi+ 수정
...
//			mg.FillRectangle(setgBrush(clLGRAY),RECTF(tRc)); // gdi+ 수정
			...
			if (m_click == ckScrollLeft)
			{
				...
				mg.DrawImage(pBitmap, sRc1.left, sRc1.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			}
			else
			{
			...
			mg.DrawImage(pBitmap, m_hthumb.left, m_hthumb.top, pBitmap->GetWidth(), pBitmap->GetHeight());
			...
			if (m_click == ckScrollRight)
			{
				...
				mg.DrawImage(pBitmap, sRc2.left, sRc2.top, pBitmap->GetWidth(), pBitmap->GetHeight());				
			}
			else
			{
...
//		sRc.DeflateRect(2, 2);
			...
			else
			{
				if (m_click == ckScrollUp)
				{
             	//	dc->DrawFrameControl(sRc1, DFC_SCROLL, DFCS_SCROLLUP | DFCS_PUSHED);			
					...
					mg.DrawImage(pBitmap, RECTG(sRc1));					
				}
				else
				{
				...
				mg.DrawImage(pBitmap, m_vthumb.left, m_vthumb.top, pBitmap->GetWidth(), pBitmap->GetHeight());				
				...
				if (m_click == ckScrollDown)
				{
					...
					mg.DrawImage(pBitmap, RECTG(sRc2));	
	...
	else if (m_form->attr2 & GO_PAGE)
	{
		...
		if (m_form->attr2 & GO_X3D)
		{
			//brush = setBrush(dc, m_sbRGB < 0 ? PALETTERGB(255, 255, 255) : getRGB(m_sbRGB));
             		//dc->FillRect(sRc, brush);
			mg.FillRectangle(setgBrush(GRGB(m_sbRGB < 0 ? _ARGB(255, 255, 255, 255) : getgRGB(m_sbRGB))), RECTG(sRc));
		...
		else
		{
			//brush = setBrush(dc, getRGB(cLGRAY));
             		//dc->FillRect(sRc, brush);
			mg.FillRectangle(setgBrush(clLGRAY), RECTG(sRc));
