	...
	: CamBase(axform, formR)
{
	...
	*/
	if (m_form->spRGB != 0)
	{
		m_fRGB = GRGB(getgRGB(m_form->stRGB));
		m_sRGB = GRGB(getgRGB(m_form->spRGB));
	}
	else
	{
		m_fRGB = GRGB(clBASIC);
...
BSTR CamGrid::_GetCell(long row, long column) 
{
	CString strResult;
...
BSTR CamGrid::_GetText(long row, long column) 
{
	CString strResult;
...
BSTR CamGrid::_GetHead(long column) 
{
	CString strResult;
...
BSTR CamGrid::_GetColumnName(long column) 
{
	CString	strResult;
...
void CamGrid::_InsertRow(long index) 
{
	InsertRow(index);
}
...
void CamGrid::Draw(CDC* dc)
{
	...
	//Matrix mat(1, 0, 0, 1, (REAL)-m_pRc.left, (REAL)-m_pRc.top);	
	//Bitmap bitmap(m_pRc.Width(), m_pRc.Height());
	//Graphics mg(&bitmap);
	//mg.SetTransform(&mat);
...
//	int	sMode;
	...
	//dc->Rectangle(cRc);
			...
			//		}
					...
					if ((int)dx <= sRc.Width())
					{
						...
						if (!(m_form->attr2 & GO_XVLINE))
						{
							...
							g.DrawLine(setgPen(GRGB(m_bRGB)), tRc.right, tRc.top, tRc.right, tRc.bottom+1);
						}
					}
					else
					{
			...
			if ((int)dx < sRc.Width())
			{
				...
				//dc->FillRect(tRc, brush);
				g.FillRectangle(setgBrush(clBASIC), RECTF(tRc)); // gdi+ 수정
				g.DrawRectangle(setgPen(clBASIC), RECTF(tRc));
			}
...
//	else
	...
	if ((m_focus || m_form->attr2 & GO_ALWAYS) && (m_form->isEdit == FI_CELL || m_form->isEdit == FI_RBOX))
	{
		...
		//dc->SelectStockObject(NULL_BRUSH);
		//dc->Rectangle(tRc);
		g.DrawRectangle(setgPen(clBLACK), RECTF(tRc)); // gdi+ 수정
	}
...
void CamGrid::WriteData(CString data, bool redraw, int col, int row)
{
	...
	if ((*m_cellR)[column->m_ref]->attr & FA_FLOAT)
	{
		...
		if (floatx >= 0)
		{
			...
			if ((*m_cellR)[column->m_ref]->digit/*vals[1]*/ == (*m_cellR)[column->m_ref]->size)
				tmps = text;
			else
			{
				str  = text.Right((*m_cellR)[column->m_ref]->size - (*m_cellR)[column->m_ref]->digit/*vals[1]*/);
				text = tmps.Mid(floatx+1);
				text = text.Left((*m_cellR)[column->m_ref]->digit/*vals[1]*/);
				if (str.IsEmpty())
				{
					str += _T("0");
					floatx++;
				}
				str += CString('0', (*m_cellR)[column->m_ref]->digit/*vals[1]*/ - tmps.GetLength());
				str += text;
				tmps = str;
			}
		}
		...
		else
		{
			...
			*/
			...
			floatx = tmps.GetLength() - (*m_cellR)[column->m_ref]->digit/*vals[1]*/;
			if (floatx <= 0)
			{
...
//			return;
	...
	if (((*m_cellR)[column->m_ref]->attr & FA_FLOAT)
		&& ((floatx == 0xff && m_edcaretpos >= (*m_cellR)[column->m_ref]->size - (int)(*m_cellR)[column->m_ref]->digit)
		|| (floatx != 0xff && data.GetLength() - floatx >= (int)(*m_cellR)[column->m_ref]->digit))
