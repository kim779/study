...
void CPropGrid::SetProperties(int kind)
{
	...
	for (int ii = 0; ii < arr->GetSize(); ii++)
	{
		...
		case 1:	// edit
			...
			pEdit = new CMFCPropertyGridProperty(prop->m_head, (_variant_t)getValue(prop, ii), prop->m_desc, (DWORD)prop);
		...
		case 2:	// edit + button
			...
			pButton = new CPropertyGridButtonProperty(false, atoi(prop->m_val2), prop->m_head, (_variant_t)getValue(prop, ii), prop->m_desc, (DWORD)prop);
		...
		case 4:	// button
			...
			pfButton = new CPropertyGridButtonProperty(true, atoi(prop->m_val2), prop->m_head, (_variant_t)getValue(prop, ii), prop->m_desc, (DWORD)prop);
...
{
	CString field = _T("00$bool\t01$fstyle\t02$alignment\t");
