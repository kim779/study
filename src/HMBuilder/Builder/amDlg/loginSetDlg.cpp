...
BOOL CLoginSetDlg::OnInitDialog()
{
	...
	GetPrivateProfileString(SERVER, SHOPIPS, _T(""), wb, sizeof(wb), m_path + _T("\\workshop.ini"));
	m_ips = wb;

	m_nPort = GetPrivateProfileInt(SERVER, _T("PROXY"), 0, m_path + _T("\\workshop.ini"));
	m_bFloat = GetPrivateProfileInt(ENVIRONMENT, _T("FLOAT"), 0, m_path + _T("\\workshop.ini"));
...
void CLoginSetDlg::OnOK()
{
	...
	WritePrivateProfileString(SERVER, _T("PROXY"), tmps, m_path + _T("\\workshop.ini"));
