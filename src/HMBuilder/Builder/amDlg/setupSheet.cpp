...
void CSetupSheet::Save()
{
	...
	// Form Size Infomation
	WritePrivateProfileString(ENVIRONMENT, _T("Form_Width"), m_defaultPage->m_FormWidth, path);
	WritePrivateProfileString(ENVIRONMENT, _T("Form_Height"), m_defaultPage->m_FormHeight, path);

	// Emulator Size
	tmps.Format(_T("%d"), m_basePage->m_EmulSize);
	WritePrivateProfileString(ENVIRONMENT, _T("Emul_Resolution"), tmps, path);

	// Merge 
	tmps = m_basePage->m_merge;
	WritePrivateProfileString(ENVIRONMENT, _T("MERGE"), tmps, path);
	...
	AfxGetMainWnd()->PostMessage(WM_USER, ID_USR_SETTING);
