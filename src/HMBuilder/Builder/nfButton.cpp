...
BEGIN_MESSAGE_MAP(CNFButton, CWnd)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDBLCLK()
...
// CNFButton  메시지 처리기입니다.

void CNFButton::OnPaint() 
{
...
void CNFButton::OnLButtonDown(UINT nFlags, CPoint point) 
{
	SetCapture();
	m_capture = true;
	Invalidate();
...
void CNFButton::OnMouseMove(UINT nFlags, CPoint point) 
{
	Invalidate();
	ReleaseCapture();
...
void CNFButton::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	SetCapture();
	m_capture = true;
	Invalidate();
...
void CNFButton::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_capture)
	{
		CRect rc;
...
void CNFButton::SetCheck(bool bCheck)
{
	m_bCheck = bCheck;
	Invalidate();
}

bool CNFButton::GetCheck()
{
	return m_bCheck;
}

int CNFButton::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
LRESULT CNFButton::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	MSG msg;
	msg.hwnd = m_hWnd;
	msg.message = message;
...
CString CNFButton::Parser(CString &srcstr, CString substr)
{
...
LOGFONT CNFButton::setFont(int fsize, CString fname, BOOL bBold, BOOL bItalic)
{
	LOGFONT	lf;
...
LRESULT CNFButton::HandleSetFont(WPARAM wParam, LPARAM lParam)
{
