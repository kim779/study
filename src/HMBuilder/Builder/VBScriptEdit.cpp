...
// updateXX__
				if (nIdentBegin > 1 && (pszChars[nIdentBegin - 1] == _T('.') || pszChars[nIdentBegin - 1] == _T(':')))
				{
					CString reSymbol;
					sSymbol = GetFrontWord(pszChars, nIdentBegin - 2) + _T(".") + sSymbol;
					if (IsInSymbol(sSymbol, _T("."), reSymbol))
					{
						DEFINE_BLOCK(nIdentBegin, COLORINDEX_KEYWORD);
						LPTSTR str = m_buf.GetLineChars(nLineIndex);
						CopyMemory(str + nIdentBegin, reSymbol, ii - nIdentBegin);
					}
...
CString CVBScriptEdit::GetLineString(int nLine)
{
	...
	if (sStr.GetLength() != GetLineLength(nLine))
...
int CVBScriptEdit::IsSymbol(CString &sym)
{
	...
	sSymbol = sym;
	sSymbol.Remove('_');
	if (m_ScriptStrMap.Lookup(sSymbol, rtValue))	// ljh->m_ScriptStrMap
		return (int)rtValue;

	for (int kk = 0; kk < formItem.GetCount(); kk++)
	{
		CString str = formItem.GetFormStr(kk, _T("name"));
		if (atoi(str))
			str = _T("S") + str;
		if (!sym.CompareNoCase(str))
		{
			sym = str;
			m_sControlName.Empty();
			if (formItem.GetForm(kk)->kind == FM_CONTROL)
				m_sControlName = formItem.GetFormStr(kk, _T("str"));

			return MAKELONG(formItem.GetForm(kk)->kind, formItem.GetForm(kk)->type);
		}
	}
...
void CVBScriptEdit::showList(int kind, bool isdot)
{
	CString info;
	CRect	rc;
	...
	if (/*m_AutoListMap.Lookup(kind, info)*/!info.IsEmpty() || !m_sControlName.IsEmpty())
	{
		...
		do
		{
			...
			isProp = atoi(temp) ? true : false;
			if (isdot == isProp && !text.IsEmpty())
			...
			case VK_RETURN:
				{
					...
					if (!m_pAutoDlg->GetSelString().IsEmpty())
					{
						...
						{
// updateXX__
							if (pszChars[pos.x] == _T('.') || pszChars[pos.x] == _T(':'))
								break;
							pos.x--;
						} while(pos.x >= 0);
			...
			default:
				...
				else if (pMsg->wParam >= _T('!'))	// maybe ascii
				{
					...
					txt = txt.Left(pos.x);
					if (txt.ReverseFind(_T('.')) > 0)
						txt = txt.Mid(txt.ReverseFind(_T('.')) + 1);
					else if (txt.ReverseFind(_T(':')))
						txt = txt.Mid(txt.ReverseFind(_T(':')) + 1);
...
// updateX__
		...
		for (int L = nStartLine; L <= nEndLine; L ++)
		{
			...
			int	pszCharsL = GetLineLength(L);

			if (pszCharsL > 0 && !_strnicmp(pszChars, pszText, (int)strlen(pszText)))
				m_pTextBuffer->DeleteText(this, L, 0, L, 2, CE_ACTION_INDENT);
			else
...
BOOL CVBScriptEdit::IsInSymbol(CString sKey, CString sGubn, CString& retStr)
{
	CRect rc;
	CString sSym = Parser(sKey, sGubn), sTipText = _T(""), info = _T("");
	BOOL	bRet = FALSE;
