...
bool CScreen::Parse(bool resize)
{
	...
	if (m_mapH->scriptR != NOVALUE)
	{
		...
		m_vbe->LoadScript(text, m_mapH->pythonMode);
	}
...
void CScreen::SetCells(CfmBase* form, CString text, bool update)
{
	struct	_cellR *cell;
	cell = (struct _cellR *) &m_cellR[form->m_form->vals[0]];
	int nCols = HIWORD(form->m_form->vals[1]);
	...
	while (!text.IsEmpty())
	{
		...
		{
			if (!strcmp(form->GetSymbolName(cell[idx].name), tmps))
				grid->Add(&cell[idx]);
		}
	}
...
void CScreen::GuideLedger()
{
	...
	string = m_guard->GetLedger(m_ledger, getMsg);
	string.TrimRight();
	pos = string.Find('\t');
	if (etype.GetAt(0) == '2')
	{
		rgb = PALETTERGB(0, 0, 0);
		msg.Format("[%s] %s", m_guard->GetLedger(m_ledger, getTimeR), (pos != -1) ? string.Left(pos) : string);
	}
	else
	{
		rgb = PALETTERGB(255, 0, 0);
		msg.Format("[%s] %s %s", m_guard->GetLedger(m_ledger, getTimeR),
					m_guard->GetLedger(m_ledger, getEcode), (pos != -1) ? string.Left(pos) : string);
	}

	msg.TrimRight();
	bool guide = m_client->m_stream->SetGuide(this, msg, rgb);

	if (etype.GetAt(0) == '1' || pos != -1)
		::MessageBox(m_view->GetSafeHwnd(), msg, (pos != -1) ?
			string.Mid(pos+1) : (m_guard->GetString(AS_ERROR, etype) ? etype : _T("ERROR")), MB_OK);
	else if (!guide)
	...
	for (int ii = 0; ii < m_mapH->formN; ii++)
	{
		...
		case FM_GRID:
			...
			for (value = 0; value < HIWORD(form->m_form->vals[1]); value++, cell++)
			{
				...
				if (force)
				{
					for (int jj = 0; jj < ((CfmGrid *)form)->GetValidRows(); jj++)
