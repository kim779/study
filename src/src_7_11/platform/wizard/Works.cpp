...
CWorks::CWorks(CGuard* guard, CWnd* view, int key, int type)
{
	...
	m_drop     = NULL;
	m_callproc = NULL;
	SetWindowLongPtr(m_view->GetSafeHwnd(), GWLP_USERDATA, LONG_PTR(this));
}

CWorks::~CWorks()
{
	if (m_callproc)
		SetWindowLongPtr(m_view->GetSafeHwnd(), GWLP_WNDPROC, (LONG_PTR)m_callproc);
...
CString CWorks::OnNotice(CString text)
{
	return _T("");
}
...
LRESULT CWorks::OnInvoke(int wParam, int lParam)
{
	return 0;
}
...
void CWorks::SetPalette(bool force)
{
}
