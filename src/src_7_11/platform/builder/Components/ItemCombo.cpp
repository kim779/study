// ItemCombo.cpp : implementation file
//
...
CItemCombo::CItemCombo( int nItem, int nSubItem, CStringList *psList )
{
	m_nItem		= nItem;
	m_nSubItem	= nSubItem;
...
BEGIN_MESSAGE_MAP(CItemCombo, CComboBox)
	//{{AFX_MSG_MAP(CItemCombo)
	ON_WM_NCDESTROY()
	ON_WM_CHAR()
	ON_WM_KILLFOCUS()
	ON_CONTROL_REFLECT(CBN_CLOSEUP, OnCloseup)
	ON_CONTROL_REFLECT(CBN_DROPDOWN, OnDropdownComboGroup)
	ON_WM_CREATE()
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CItemCombo::OnSize(UINT nType, int cx, int cy) 
{
...
void CItemCombo::OnDropdownComboGroup()
{
	CDC*    pDC = GetDC();
	CSize   sz;
	int     dx = 0;
