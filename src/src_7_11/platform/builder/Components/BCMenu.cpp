...
int BCMenu::m_iconX = 17;
int BCMenu::m_iconY = 17;
...
BCMenu::BCMenu()
{
	...
	m_bitmapBackground=RGB(0,255,255); //gray
...
BCMenu *BCMenu::FindMenuOption(int nId,int& nLoc)
{
	...
	for(ii = 0; ii < (int)(GetMenuItemCount()); ++ii){
#ifdef _CPPRTTI 
		psubmenu = dynamic_cast<BCMenu *>(GetSubMenu(ii));
#else
		psubmenu = (BCMenu *)GetSubMenu(ii);
#endif
		if (psubmenu){
			pgoodmenu=psubmenu->FindMenuOption(nId,nLoc);
			if(pgoodmenu)return(pgoodmenu);
		}
		else if (nId == (int)GetMenuItemID(ii)){
			nLoc = ii;
			return(this);
		}
	}
