...
void CCrystalTextView::MoveLeft(BOOL bSelect)
{
	...
	else
	{
		...
		else
		{
			...
			if (m_ptCursorPos.x > 0 && (BYTE)pszChars[m_ptCursorPos.x] >= 0x80)
				m_ptCursorPos.x --;
		}
	}
...
void CCrystalTextView::MoveRight(BOOL bSelect)
{
	...
	else
	{
		...
		else
		{
			m_ptCursorPos.x ++;
			LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
			if (m_ptCursorPos.x > 0 && IsDBCSLeadByte((BYTE)pszChars[m_ptCursorPos.x]) && !IsLeadByte(pszChars, m_ptCursorPos.x))
				m_ptCursorPos.x ++;
		}
	}
	m_nIdealCharPos = CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x);
...
void CCrystalTextView::MoveWordLeft(BOOL bSelect)
{
	...
	while (nPos > 0 && (BYTE)pszChars[nPos - 1] < 0x80 && isspace(pszChars[nPos - 1]))
		nPos --;

	if (nPos > 0)
	{
		nPos --;
		if ((BYTE)pszChars[nPos] >= 0x80 || isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
		{
			while (nPos > 0 && ((BYTE)pszChars[nPos - 1] >= 0x80 || isalnum(pszChars[nPos - 1]) || pszChars[nPos - 1] == _T('_')))
				nPos --;
		}
		else
		{
			while (nPos > 0 && (BYTE)pszChars[nPos - 1] < 0x80 && !isalnum(pszChars[nPos - 1])
...
void CCrystalTextView::MoveWordRight(BOOL bSelect)
{
	...
	if ((BYTE)pszChars[nPos] >= 0x80 || isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
	{
		while (nPos < nLength && ((BYTE)pszChars[nPos] >= 0x80 || isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_')))
			nPos ++;
	}
	else
	{
		while (nPos < nLength && (BYTE)pszChars[nPos] < 0x80 && !isalnum(pszChars[nPos])
						&& pszChars[nPos] != _T('_') && ! isspace(pszChars[nPos]))
			nPos ++;
	}

	while (nPos < nLength && (BYTE)pszChars[nPos] < 0x80 && isspace(pszChars[nPos]))
		nPos ++;
...
void CCrystalTextView::MoveHome(BOOL bSelect)
{
	...
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	int nHomePos = 0;
	while (nHomePos < nLength && pszChars[nHomePos] > 0 && isspace(pszChars[nHomePos]))
		nHomePos ++;
...
void CCrystalTextView::AdjustHangulCursor()
{
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	if (m_ptCursorPos.x > 0 && IsDBCSLeadByte((BYTE)pszChars[m_ptCursorPos.x]) && !IsLeadByte(pszChars, m_ptCursorPos.x))
		m_ptCursorPos.x --;
...
CPoint CCrystalTextView::WordToRight(CPoint pt)
{
	...
	while (pt.x < nLength)
	{
		if ((BYTE)pszChars[pt.x] < 0x80 && !isalnum(pszChars[pt.x]) && pszChars[pt.x] != _T('_'))
			break;
		pt.x ++;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
CPoint CCrystalTextView::WordToLeft(CPoint pt)
{
	...
	while (pt.x > 0)
	{
		if ((BYTE)pszChars[pt.x - 1] < 0x80 && !isalnum(pszChars[pt.x - 1]) && pszChars[pt.x - 1] != _T('_'))
			break;
		pt.x --;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
{
	pCmdUI->Enable(TRUE);//m_ptSelStart != m_ptSelEnd);
}
...
void CCrystalTextView::Copy()
{
	...
	{
		if (GetLineLength(m_ptSelStart.y))
		{
			CString text;
			GetText(CPoint(0, m_ptSelStart.y), CPoint(GetLineLength(m_ptSelStart.y), m_ptSelStart.y), text);
			PutToClipboard(text);
		}
		return;
	}
...
BOOL CCrystalTextView::PutToClipboard(LPCTSTR pszText)
{
	...
	if (OpenClipboard())
	{
		...
		if (hData != NULL)
		{
			...
			USES_CONVERSION;
			_tcscpy_s(pszData, (_tcslen(pszText) + 1) * 2, T2A((LPTSTR) pszText));
			GlobalUnlock(hData);
