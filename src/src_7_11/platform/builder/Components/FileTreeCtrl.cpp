...
BOOL CTreeFileCtrl::Open(HTREEITEM hItem)
{
	...
	{
		//Show the "properties" for the selected file
		m_sFilePath = ItemToPath(hItem);
		CFileFind finder;
		if (finder.FindFile(m_sFilePath))
		{
			finder.FindNextFile();
			if (!finder.IsDirectory())
				AfxGetMainWnd()->SendMessage(WM_USER, ID_USR_FILEOPEN, (LPARAM)m_sFilePath.operator LPCTSTR());
			finder.Close();
		}
	}
	return bSuccess;
...
BOOL CTreeFileCtrl::CanDisplayFile(const CFileFind& find)
{
	...
	bool bRet = true;
	if (!m_sFileNameMask.Compare("????????"))
	{
		char buf[31];
		TRY
		{
			CFile file(find.GetFilePath(), CFile::modeRead);
			UINT nBytesRead;
			nBytesRead = file.Read(buf, 30);
			if (nBytesRead)
			{
				buf[nBytesRead] = '\0';
				CString sData = CString(buf, nBytesRead);

				if (sData.Find("GLOBAL DIVISION") == -1)
					bRet = false;
			}
	...
	return (bRet && m_bShowFiles && !find.IsDirectory() && !find.MatchesMask(m_dwFileHideFlags));
}
	...
	case HideExtension:
		{
			...
			if (IsFile(pItem->m_sFQPath))
			{
				...
				_tsplitpath_s(pItem->m_sRelativePath, 
					    szDrive, _MAX_DRIVE, 
					    szDir,   _MAX_DIR,
					    szFname, _MAX_FNAME, 
					    NULL, 0);

				_tmakepath_s(szPath, _MAX_PATH, szDrive, szDir, szFname, NULL);
...
BOOL CTreeFileCtrl::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pDispInfo->item.pszText)
	{
		...
		if (pItem->m_bExtensionHidden)
		{
			...
			_tsplitpath_s(sFrom, NULL, 0, NULL, 0, NULL, 0, szExt, _MAX_EXT);
			sTo += szExt;
			sToRelative += szExt;
		}
...
void CTreeFileCtrl::DisplayPath(const CString& sPath, HTREEITEM hParent, BOOL bUseSetRedraw)
{
	...
	for (int ii=0; ii < nDirectories; ii++)
	{
		CString& sPath = DirectoryPaths.ElementAt(ii);
		TCHAR	 szPath[_MAX_PATH];
		TCHAR	 szFname[_MAX_FNAME];
		TCHAR	 szExt[_MAX_EXT];

		_tsplitpath_s(sPath, NULL, 0, NULL, 0, szFname, _MAX_FNAME, szExt, _MAX_EXT);
		_tmakepath_s(szPath, _MAX_PATH, NULL, NULL, szFname, szExt);
...
BOOL CTreeFileCtrl::OnBeginDrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	//Begin the dragging  
	m_pilDrag->BeginDrag(0, CPoint(0, 0)/*offsetPt*/);
...
void CTreeFileCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	{
		AfxGetMainWnd()->SendMessage(WM_USER, ID_USR_DROPPNT, MAKELPARAM(pt.x, pt.y));
		EndDragging(TRUE);
	}
...
void CTreeFileCtrl::EndDragging(BOOL bCancel)
{
	if (IsDragging())
	{
		...
		if (bCancel)//m_hItemDrop == NULL)
		{
			m_sFilePath = ItemToPath(m_hItemDrag);
			AfxGetMainWnd()->SendMessage(WM_USER, ID_USR_FILEOPEN, (LPARAM)m_sFilePath.operator LPCTSTR());
			return;
		}
