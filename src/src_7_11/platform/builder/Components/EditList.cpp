...
void CEditList::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	{
		if (getType(pNMListView->iSubItem) == '2')
			ComboItem(pNMListView->iItem, pNMListView->iSubItem);
		else if (getType(pNMListView->iSubItem) == '1')
...
#define IDC_COMBOBOXINLISTVIEW 0x1235
	...
	CComboBox *pList = new CItemCombo(nItem, nSubItem, &m_strList);
...
char CEditList::getType(int col)
{
	if (m_sType.GetLength() <= col)
		return ' ';
	return m_sType.GetAt(col);
}
