...
int CUpDlg::makeData()
{
	...
	CString	resN = _T("");
	CButton* pExport = (CButton*) GetDlgItem(IDC_EXPORT);
	...
	CopyMemory(&m_workIO.mgub[0], tmps.operator LPCTSTR(), 3);
	strcpy_s(&m_workIO.mapd[0], 20+1, &m_mapH->mapdesc[0]);
	tmps = _T("");
...
LONG CUpDlg::OnOpenedSock(WPARAM wParam, LPARAM lParam)
{
	if (!lParam)
		OnCancel();
	return 0;
