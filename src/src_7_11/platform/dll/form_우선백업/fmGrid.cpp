...
Msg("row[%d]", row);
	...
	{
		Msg("row[%d] > column[%d]", row, column->m_valids.GetUpperBound());
		return;
	}
	column->m_valids.SetAt(row, 1);
...
Msg("index(%d), text(%s), skipInput(%d)", index, text, skipInput);
...
//Msg("nRow[%d]", nn);
	...
	tRc.top += (int)((m_cy+m_slit) * m_multiN * m_headN);
	if (m_vscroll && !m_vscr.IsRectEmpty())
		tRc.right = m_vscr.left-1;
	if ((m_form->attr2 & GO_PAGE) && !m_pscr.IsRectEmpty())
		tRc.right = m_pscr.left-1;
