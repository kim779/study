...
CfmAvi::CfmAvi(CAxisForm* axform, struct _formR* formR)
	: CfmBase(axform, formR)
{
	...
	mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE | MCI_ANIM_OPEN_WS, (DWORD_PTR)&m_mciOpen);
	m_aviDev = m_mciOpen.wDeviceID;
	mciSendCommand(m_aviDev, MCI_WHERE, MCI_ANIM_WHERE_SOURCE | MCI_ANIM_RECT, (DWORD_PTR)&m_mciRect);
	m_mciRect.rc = m_pRc;
	mciSendCommand(m_aviDev, MCI_PUT, MCI_ANIM_PUT_DESTINATION|MCI_ANIM_RECT, (DWORD_PTR)&m_mciRect);
	m_mciWindow.hWnd = m_axform->m_view->m_hWnd;
	mciSendCommand(m_aviDev, MCI_WINDOW, MCI_ANIM_WINDOW_HWND, (DWORD_PTR)&m_mciWindow);
}
...
bool CfmAvi::Play()
{
	...
	mciSendCommand(m_aviDev, MCI_PAUSE, MCI_NOTIFY, (DWORD_PTR)&m_mciPlay);
	mciSendCommand(m_aviDev, MCI_WHERE, MCI_ANIM_WHERE_SOURCE | MCI_ANIM_RECT, (DWORD_PTR)&m_mciRect);
	...
	mciSendCommand(m_aviDev, MCI_PUT, MCI_ANIM_PUT_DESTINATION | MCI_ANIM_RECT, (DWORD_PTR)&m_mciRect);
	m_mciPlay.dwCallback = (DWORD_PTR)m_axform->m_view->m_hWnd;
	mciSendCommand(m_aviDev, MCI_PLAY, MCI_NOTIFY, (DWORD_PTR)&m_mciPlay);
	return true;
}
