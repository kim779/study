...
CfmButton::CfmButton(CAxisForm* axform, struct _formR* formR)
...
void CfmButton::WriteData(CString data, bool redraw, int col, int row)
{
	...
	{
		data.MakeLower();
		if (data.Find(".bmp") != -1 || data.Find(".gif") != -1)
			image = true;
	}
...
void CfmButton::ReadData(CString& data, bool edit, int col, int row)
{
