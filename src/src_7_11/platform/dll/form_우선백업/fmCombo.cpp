...
CfmCombo::CfmCombo(CAxisForm* axform, struct _formR* formR)
...
//	drawEdge(dc, m_pRc, EDGE_SUNKEN, BF_RECT);
	...
	if (m_pic && m_pic->GetValid(image))
	{
		...
		m_pic->DrawPicture(dc, m_rBtn, AL_STRETCH, image);
	}
	else
	{
		COLORREF bg, fg;
		...
		dc->MoveTo(m_rBtn.left, m_rBtn.top+1);
		dc->LineTo(m_rBtn.left, m_rBtn.bottom-1);
		dc->MoveTo(m_rBtn.right-1, m_rBtn.top+1);
		dc->LineTo(m_rBtn.right-1, m_rBtn.bottom-1);
...
void CfmCombo::ReadData(CString& data, bool edit, int col, int row)
{
	if (m_editMode)
	{
		int	len;
