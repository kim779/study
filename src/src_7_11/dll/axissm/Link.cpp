...
CLink::CLink(class CTMenu* menu, CString home, CString mapN, CWnd* pParent /*=NULL*/)
	: CExDialog(CLink::IDD, pParent)
{
	...
	m_name = app->GetProfileString(WORKSTATION, SIGNONID);
	int pos = m_name.Find('|');
	if (pos < 0)	
		m_name = _T("Anonymous");
	else		
		m_name = m_name.Mid(pos+1);
...
void CLink::OnClickGlist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
	...
	DWORD dw = GetPrivateProfileString("SNAME", m_smapN, "", name, sizeof(name), file);
	if (dw <= 0)	
...
void CLink::LoadInfo()
{
	...
	m_gList.DeleteAllItems();
	file.Format("%s\\%s\\%s\\%s", m_home, USRDIR, m_name, m_mapN.Left(4));
	DWORD	dw = GetPrivateProfileSection(m_mapN, buf, sizeof(buf), file);
	if (dw > 0)
	{
		for (int ii = 0; ii < (int) dw; ii++)
		{
			...
			str = str.Mid(pos);
			int	row = m_gList.GetItemCount();
			m_gList.InsertItem(row, m_menu->GetDispN(mapN));
	...
	{
		file.Format("%s\\%s\\%s", m_home, MTBLDIR, m_mapN.Left(4));
		dw = GetPrivateProfileSection(m_mapN, buf, sizeof(buf), file);
		if (dw > 0)
		{
			for (int ii = 0; ii < (int) dw; ii++)
			{
	...
	if (!GetPrivateProfileInt("SHOWBAR", m_mapN, 1, file))
		((CButton *) GetDlgItem(IDC_USELINKBAR))->SetCheck(1);

	file.Format("%s\\%s\\linkname.ini", m_home, TABDIR);
	if (!GetPrivateProfileInt("SNAME", "DISPN", 1, file))
		((CButton *) GetDlgItem(IDC_DISPSNAME))->SetCheck(1);
}
...
void CLink::SaveInfo()
{
	...
	file.Format("%s\\%s\\%s\\%s", m_home, USRDIR, m_name, m_mapN.Left(4));
	WritePrivateProfileSection(m_mapN, "", file);
	for (int ii = 0; ii < tot; ii++)
	{
		...
		WritePrivateProfileString(m_mapN, keys, value, file);
	}

	file.Format("%s\\%s\\linkbar.ini", m_home, TABDIR);
	if (((CButton *) GetDlgItem(IDC_USELINKBAR))->GetCheck() == BST_CHECKED)
		value = "0";
	else	value = "1";
	WritePrivateProfileString("SHOWBAR", m_mapN, value, file);

	file.Format("%s\\%s\\linkname.ini", m_home, TABDIR);
	if (((CButton *) GetDlgItem(IDC_DISPSNAME))->GetCheck() == BST_CHECKED)
		value = "0";
	else	value = "1";
	WritePrivateProfileString("SNAME", "DISPN", value, file);
}
...
void CLink::OnSaveSName() 
{
	...
	WritePrivateProfileString("SNAME", m_smapN, sName, file);
}
