...
void	CGlobal::Init(CString strpath)
{
	if(m_bInit)
		return;
	...
	SetRGB(IDX_RGB_MASK, RGB(255,0,255));
	SetRGB(IDX_RGB_BACKGROUND, RGB(255,255,255));

	SetRGB(IDX_RGB_FONT, RGB(51,51,51));
	SetRGB(IDX_RGB_FONT_CAPTION, RGB(0,0,0));
	SetRGB(IDX_RGB_FONT_CAPTION_INACTIVE, RGB(178,178,178));
	SetRGB(IDX_RGB_CONTROL_BORDER, RGB(240,240,240));

	SetFont(IDX_FONT_SMALL, _T("±¼¸²Ã¼"), 8);
	SetFont(IDX_FONT_MEDIUM, _T("±¼¸²Ã¼"), 9);
	SetFont(IDX_FONT_LARGE, _T("±¼¸²Ã¼"), 11, TRUE);
...
CString CGlobal::GetBitmapFileName(UINT nBitmapID)
{
	...
	case IDX_BMP_LIST_HEADER_DIVISION		:	
		return _T("skin\\list_header_division.bmp");		break;
	}
	return _T("");
}
...
void CGlobal::LoadIcons()
{
	...
	for(UINT i=0; i<IDX_ICON_MAX; i++)
	{
		...
		strFileName = sModulePath + strFileName;
		//(HICON) LoadImage(NULL, iconN, IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
		//m_hIcon[i][0] = (HICON)::LoadImage(::AfxGetInstanceHandle(), strFileName, IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
		m_hIcon[i][0] = (HICON) LoadImage(NULL, strFileName, IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
		//m_hIcon[i][1] = (HICON)::LoadImage(::AfxGetInstanceHandle(), strFileName, IMAGE_ICON, 24, 24, LR_LOADFROMFILE);
		m_hIcon[i][1] = (HICON) LoadImage(NULL, strFileName, IMAGE_ICON, 24, 24, LR_LOADFROMFILE);
		//m_hIcon[i][2] = (HICON)::LoadImage(::AfxGetInstanceHandle(), strFileName, IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
		m_hIcon[i][2] = (HICON) LoadImage(NULL, strFileName, IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
	}
}
