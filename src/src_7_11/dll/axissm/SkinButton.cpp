...
//pDC->StretchBlt(rect.left, rect.bottom-size.cy, rect.Width(), size.cy, &tmpDC, 0, 0, size.cx, size.cy, SRCCOPY);
	//::StretchBlt(pDC->m_hDC, rpItem->left, rpItem->top, rpItem->right, rpItem->bottom, hdcBmpMem, 0, 0,  rpItem->right - rpItem->left,  rpItem->bottom - rpItem->top, SRCCOPY);

	BITMAP	csBitmapSize;
	::GetObject(m_csBitmaps[0].hBitmap, sizeof(csBitmapSize), &csBitmapSize);

	::StretchBlt(pDC->m_hDC, rpItem->left, rpItem->top, rpItem->right, rpItem->bottom, hdcBmpMem, 0, 0,  csBitmapSize.bmWidth,  csBitmapSize.bmHeight, SRCCOPY);
