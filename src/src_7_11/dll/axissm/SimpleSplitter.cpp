	...
	m_nBarThickness(nBarThickness)
{
	...
	int i, total;
	for (i = 0, total = 0; i < m_nPanes - 1; i++)			// default, set equal size to all panes
	{
...
void CSimpleSplitter::InvertTracker()
{
	...
	if (m_nOrientation == SSP_HORZ)
	{
		int	x = m_nTracker - m_nBarThickness + 1;
		int	y = 1;
		int	cx = m_nBarThickness;
		int	cy = m_nTrackerLength - 2;
...
void CSimpleSplitter::drawBackGround(CDC* pDC)
{
	...
	{
		pDC->FillSolidRect(cRc, m_barColor);
	}
}
