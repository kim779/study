...
int CXListCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetExtendedStyle(dwExStyle |  LVS_EX_BORDERSELECT);
	SetOutlineColor(RGB(255,0,0));
	*/
...
void CXListCtrl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	// TODO: Add your message handler code here and/or call default
	if(!m_bShowHscroll)
...
void	CXListCtrl::SetTextFont(CString strFname, int isize, BOOL bBold) //{ m_pTextFont = pTextFont; RedrawWindow(); }
{
	...
	m_fontname =  _T("굴림체");
	LOGFONT	lf;
	m_pTextFont->CreatePointFont(isize*10, strFname);
	m_pTextFont->GetLogFont(&lf);
	m_pTextFont->DeleteObject();

	m_pTextFont->CreateFont(	lf.lfHeight,			// nHeight
				lf.lfWidth,			// nWidth
				lf.lfEscapement,		// nEscapement
				lf.lfOrientation,		// nOrientation
				0 ? FW_BOLD : FW_NORMAL,	// nWeight
				lf.lfItalic,			// bItalic
				...
				(char *) strFname.operator LPCTSTR());        // lpszFacename//
}
