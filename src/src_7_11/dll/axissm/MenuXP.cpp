...
const int CXBUTTONMARGIN = 3;	// num pixels wider button is than bitmap
const int CYBUTTONMARGIN = 3;	// ditto for height


// DrawText flags
const UINT DT_MYSTANDARD = DT_SINGLELINE|DT_LEFT|DT_VCENTER|DT_NOPREFIX;
//#define BACK_COLOR		RGB(239, 239, 239)
#define BACK_COLOR		RGB(76,81,90)
#define SELECTBAR_COLOR		RGB(57, 62, 68) //드랍다운된 메뉴들중 하나에 마우스가 오버됐을때
//#define DISABLEBAR_COLOR	RGB(237, 237, 237)
#define DISABLEBAR_COLOR	RGB(237, 0, 0)

//#define BORDER_COLOR		RGB(  0,   0, 128)
#define BORDER_COLOR		RGB(  76,   81, 90)

#define CHECK_COLOR		RGB(216, 216, 235) //GB(255, 176, 0)
//#define SELECTCHECK_COLOR	RGB(140, 140, 197)
#define SELECTCHECK_COLOR	RGB(255, 176, 0)

#define ICONAREA_COLOR		RGB(204, 204, 204)

#define SELBORDER_CO		RGB(76,81,90)
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////
#define BORDER_COLOR	SELBORDER_CO
...
//	VERIFY(m_fontMenu.CreateFontIndirect(&info.lfMenuFont));	// vista
	CString	fontName;
	fontName.LoadString(STSM_FONTNAME_DEFAULT);
	m_fontMenu.CreatePointFont(90, fontName);

	//initialize colors with system default
	//m_clrBackGround		= COLOR_FRAME;//BACK_COLOR;
	m_clrBackGround		= RGB(76,81,90);
	m_clrSelectedBar	= SELECTBAR_COLOR;
	m_clrSelectedText	= RGB(255,175,0); //
	//m_clrText			= RGB(45,54,139); //::GetSysColor(COLOR_MENUTEXT);
	m_clrText			= RGB(255,255,255); //::GetSysColor(COLOR_MENUTEXT);
	...
	m_Style			= STYLE_STARTMENU;//STYLE_XP;
...
CString tmpS;
tmpS.Format(_T("%d %d %d %d [%d] \n"), rcItem.left, rcItem.top, rcItem.right, rcItem.bottom, pItem->m_bSeparator);
TRACE(tmpS);
	...
	else if (pItem->m_bSeparator) 
	{
		...
		rc.bottom -= 1;
		dc.FillSolidRect(&rc,RGB(38,38,44));
	...
	else
	{
		...
		{ 
		//	DrawButton(&dc, rcButton, bSelected, bDisabled, bChecked);  //메뉴중 체크형태일경우 체크박스를 안그림
		}
...
void CMenuXP::DrawBackGround(CDC *pDC, CRect rect, BOOL bSelected, BOOL bDisabled)
{
	...
	else if (bSelected)
	{
		...
		if (bDisabled)
			FillRect(pDC, rect, DISABLEBAR_COLOR);
		else	
...
void CMenuXP::DrawSideBar(CDC *pDC, CRect rect, HICON hIcon, CString strText)
{
	...
	HBITMAP	bmpBar = CreateGradientBMP(pDC->m_hDC, m_clrSideBarStart, m_clrSideBarEnd,
		rect.Width(), rect.Height(), 0, 256);
	if (bmpBar)
	{
		CDC memDC;
...
void CMenuXP::DrawCheckMark(CDC *pDC, CRect rect, BOOL bSelected)
{
	...
	int xOff = (rect.Width()-nCheckDots)/2 + rect.left + 1;
...
BOOL CMenuXP::InsertMenuX(UINT nPosition, UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem)
{
	if (nFlags & MF_SEPARATOR)	return AppendSeparator();
	CMenuXPItem*	item = new CMenuXPText(nIDNewItem, lpszNewItem);
	ASSERT(item);
...
BOOL CMenuXP::AppendMenuX(UINT nFlags, UINT nIDNewItem, LPCTSTR lpszNewItem)
{
	if (nFlags & MF_SEPARATOR)	return AppendSeparator();
	CMenuXPItem*	item = new CMenuXPText(nIDNewItem, lpszNewItem);
	ASSERT(item);
