...
int CReportCtrl::MoveTo(int nItem, int nNewPosition)
{
	...
	const DWORD DATA = (const DWORD)CListCtrl::GetItemData(nItem);
	CArray<int, int> aImages;
	CStringArray aTexts;
...
BOOL CReportCtrl::SwapItems(int nItem1, int nItem2)
{
	...
	const DWORD DATA1 = (const DWORD)CListCtrl::GetItemData(nItem1);
	const DWORD DATA2 = (const DWORD)CListCtrl::GetItemData(nItem2);
...
void CReportCtrl::_AllocItemMemory(int nItem)
{
	...
	pData->dwData = (DWORD)CListCtrl::GetItemData(nItem);
...
BOOL CReportCtrl::_PartialSort(int nStart, int nEnd)
{
	...
	for (i = 0; i < COUNT; i++)
	{
		...
		aRows[i].dwData = (DWORD)CListCtrl::GetItemData(aIndices[i]);
