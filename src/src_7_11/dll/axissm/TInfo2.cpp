...
CTInfo2::CTInfo2(bool (*callback)(int, WPARAM, LPARAM), CString home, CString userN)
{
	m_axiscall	= callback;
	m_home		= home;
	m_userN		= userN;
...
void CTInfo2::OnDestroy() 
{
	...
	WritePrivateProfileString("GENERAL", "BAR01SIZE", value, file);
...
void CTInfo2::ResizeBase()
{
	...
	if (m_base && m_base->GetSafeHwnd())
	{
		...
		m_setup->MoveWindow(cRc);

		cRc.left = cRc.right;
		cRc.right = cRc.left + BUTTONWIDTH2;
		m_sdi->MoveWindow(cRc);
	}
}
...
void CTInfo2::GetPaneSize(int configN, CArray <int, int> &ary)
{
	...
	file.Format("%s\\user\\%s\\%s", m_home, m_userN, TICKSETUP);
	GetPrivateProfileString("GENERAL", "BAR01SIZE", "10", buf, sizeof(buf), file);
...
void CTInfo2::ProcessRTS(CString symbol, CString dat)
{
	CPane*	pane;
	for (POSITION pos = m_arPane.GetHeadPosition(); pos; )
	{
...
void CTInfo2::ConclusionNotice(CString str)
{
	CPane*	pane;
	for (POSITION pos = m_arPane.GetHeadPosition(); pos; )
	{
...
void CTInfo2::createButton()
{
	...
	m_setup = new CNButton(CTRL_TICKSETUP);
	if (!m_setup->Create(NULL, "티커설정", dwStyle, CRect(0, 0, 0, 0), this, -1))
	{
		delete m_setup;
		m_setup = NULL;
	}
	...
	{
		m_setup->set_ToolTipText("티커설정");
		imgC.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_TickerSet");
		imgO.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_TickerSet_en");
		imgD.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_TickerSet_dn");
		m_setup->set_Image(imgC, imgO, imgD);
	}

	m_sdi = new CNButton(CTRL_SDI);
	if (!m_sdi->Create(NULL, "독립창", dwStyle, CRect(0, 0, 0, 0), this, -1))
	{
		delete m_sdi;
		m_sdi = NULL;
	}
	...
	{
		m_sdi->set_ToolTipText("MDI/SDI 전환");
		imgC.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_sdi");
		imgO.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_sdi_en");
		imgD.Format("%s\\%s\\%s.bmp", m_home, "image", "mb_sdi_dn");
		m_sdi->set_Image(imgC, imgO, imgD);
	}
}

LRESULT CTInfo2::OnXMSG(WPARAM wParam, LPARAM lParam)
{
	CString imgC, imgO, imgD;
	switch (LOWORD(wParam))
	{
	case CTRL_TICKSETUP:	(*m_axiscall)(AXI_TICKSETUP, wParam, lParam);	break;
	case CTRL_SDI:		(*m_axiscall)(AXI_APPSWITCH, wParam, lParam);	break;
	case CTRL_HIDE:		(*m_axiscall)(AXI_HIDETICKER, 0, 2);	break;
	}
	return 0;
}
