...
void CScreenList::fit()
{
	if (m_listctrl.GetSafeHwnd())
	{
		...
		int fitN  = m_listctrl.GetCountPerPage();
		int W0, W1;
		if (count > fitN)
		{
...
void CScreenList::add_History(CString dat)
{
	...
	if (m_history.GetSize() > HMAX)	
		m_history.RemoveAt(HMAX);
	for (int ii = (int)m_history.GetSize() -1; ii >= 1; ii--)
	{
...
bool CScreenList::updateCtrl(CStringArray& array, int type, bool bParse)
{
