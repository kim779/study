...
//	SetURL(NULL);
	...
	m_Font.CreatePointFont(9*10, "µ¸¿ò");
...
//DEL BOOL CButtonST::PreTranslateMessage(MSG* pMsg) 
//DEL {
//DEL //	InitToolTip();
//DEL 	m_ToolTip.RelayEvent(pMsg);
//DEL 	
//DEL //	if (pMsg->message == WM_LBUTTONDBLCLK)
//DEL //		pMsg->message = WM_LBUTTONDOWN;
//DEL 
//DEL 	return CButton::PreTranslateMessage(pMsg);
//DEL } // End of PreTranslateMessage
...
LRESULT CButtonST::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	if (m_pWnd)
		((CJongmukDlg*)m_pWnd)->SetListCtrl();
	return 0;
} // End of OnMouseLeave
...
//	m_crColors[byColorIndex] = RGB(byRed + shOffset, byGreen + shOffset, byBlue + shOffset);
	...
	m_crColors[byColorIndex] = RGB(shOffset, shOffset, shOffset);
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsFlat == FALSE)
	{
		...
		//	pDC->SelectObject(br);
		//	pRect->InflateRect(1,1);
		//	pDC->RoundRect(pRect, point);
	...
	if (m_bMouseOnButton || m_bIsPressed)
	{
		...
		CBrush		brBackground(crColor), *poldbrush;	
	...
	else
	{
		...
		else
		{
			...
			/*	
				if (m_bIsDisabled)
				{
					CBrush		disable(USER_BACKCOLOR);
					CPen		pen(PS_SOLID, 1, USER_BACKCOLOR);
					pDC->SelectObject(disable);
...
void CButtonST::SetBold(BOOL flag)
{	
//	m_Font.SetBold(flag);
	m_bBold = TRUE;
	Invalidate();
...
void CButtonST::SetFont(int nheight, CString sztext)
{
//	m_Font.SetMyFont(nheight, sztext);
}

void CButtonST::SetWnd(CWnd *pWnd)
{
	m_pWnd = pWnd;
}
