...
public:
	CMyFont();//Default Constructor
	CMyFont(CString facename, int height = -12);//Font name constructor
	CMyFont(LOGFONT& logfont);//LogFont constructor
	CMyFont(CFont font);//Constructs font based on existing font
	~CMyFont();//Destructor

	void SetMyFont(int height, CString szFaceName);
	...
	//f: CFont or CMyFont object to initialize the dlg with.
