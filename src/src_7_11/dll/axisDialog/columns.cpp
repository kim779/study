...
void CColumns::InitControl()
{
	...
	m_BtnInit->Create("기본설정으로 되돌리기\n기본설정으로 되돌리기", CRect(10, 50, 160, 72), this, IDC_INIT, false, true);
	m_BtnInit->SetImgBitmap(HbmInit, HbmInit_dn, HbmInit_en);
	m_BtnInit->SetFont(m_pFont);
	m_BtnInit->SetTextColor(RGB(255,255,255));
...
void CColumns::SetBitmaps()
{
	...
	sPath.Format("%s\\%s", m_sImageDir, TOP);		bmTop = CreateBmp(sPath);	HbmTop = bmTop->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, TOP_DN);		bmTop_dn = CreateBmp(sPath);	HbmTop_dn = bmTop_dn->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, TOP_EN);		bmTop_en = CreateBmp(sPath);	HbmTop_en = bmTop_en->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, UP);		bmUp = CreateBmp(sPath);	HbmUp = bmUp->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, UP_DN);		bmUp_dn = CreateBmp(sPath);	HbmUp_dn = bmUp_dn->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, UP_EN);		bmUp_en = CreateBmp(sPath);	HbmUp_en = bmUp_en->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, DOWN);		bmDown = CreateBmp(sPath);	HbmDown = bmDown->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, DOWN_DN);		bmDown_dn = CreateBmp(sPath);	HbmDown_dn = bmDown_dn->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, DOWN_EN);		bmDown_en = CreateBmp(sPath);	HbmDown_en = bmDown_en->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, BOTTOM);		bmBottom = CreateBmp(sPath);	HbmBottom = bmBottom->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, BOTTOM_DN);		bmBottom_dn = CreateBmp(sPath);	HbmBottom_dn = bmBottom_dn->operator HBITMAP();
	sPath.Format("%s\\%s", m_sImageDir, BOTTOM_EN);		bmBottom_en = CreateBmp(sPath);	HbmBottom_en = bmBottom_en->operator HBITMAP();
