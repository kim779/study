...
BEGIN_EVENTSINK_MAP(CTransDlg, CDialog)
	ON_EVENT(CTransDlg, -1, 1, OnFire, VTS_I4 VTS_I4 VTS_I4)
END_EVENTSINK_MAP()
...
BOOL CTransDlg::OnInitDialog() 
{
	...
	m_socket = new CWnd();
	if (m_socket->CreateControl("AxisSock.SockCtrl.AX2010", NULL, WS_CHILD, CRect(0,0,0,0), this, -1))
		m_state = sCREATE;
	...
	case FEV_OPEN:
		if (!pBytes)
		{
			...
			m_socket->InvokeHelper(DI_AXIS, DISPATCH_METHOD, VT_BOOL, (void *)&rc,
							(BYTE *)(VTS_I4 VTS_I4), NULL, modeWS);  //modeWM  modeWS
			if (rc)
			{
				m_state = sOPEN;
				OnTransmit();
...
bool CTransDlg::sendData()
{
	...
	//				(BYTE *)(VTS_I8 VTS_I4), m_sendB, sendL+L_rsmH);
...
void CTransDlg::close()
{
	enableSend(true);
	if (m_state == sOPEN)
		m_socket->InvokeHelper(DI_CLOSE, DISPATCH_METHOD, VT_EMPTY, (void *)NULL, VTS_NONE);
