...
#define	msgK_ARMx	0x9c		// AUX real-time msg (on FEP)
...
#define	winK_RMN	0xfd		// remain dll window
...
struct	_signR	{
	...
	unsigned char	absS;			// absolute security
	unsigned char	incS[4];		// inclusive security
	unsigned char	excS[4];		// exclusive security
	...
	char	sign[30];			// sign identification			 userid[16]
	char	name[30];			// sign name				 name[30]
	char	menu[12];			// menu

	char	trx[3];				// transaction timeout(sec.)
	char	usage[3];			// usage time (min.)
	char	idle[3];			// idle timeout (min.)
	char	guide[200];			// guide msg				error message
	//char	service[10];			// service#

	char	info[64];			// information				날짜 시간
						// server time  '\t' + ...   
	/*	
	char	auth[7];			// user authority
	char	dept[5];			// department	
	char	pwch[1];			// change password enable(1: 변경필요)
	char	dupc[1];			// Simultaneous connection enable(동시접속여부)
	char	tmpuse[1];			// temporary service enable(임시사용자 여부)
	char	userst[1];			// user restriction enable (사용자제한 여부)
	char	email[1];			// email active enable
	*/
