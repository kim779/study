...
BOOL CfxGrid::GetCellSize(int fromRow, int endRow, int& colW, int& rowH)
{
	...
	for (ii = fromRow; ii < endRow; ii++)
		rowH += GetRowHeight(ii);
	for (ii = 0; ii < GetColumnCount(); ii++)
...
//	ResetSelectedRange();
	...
	{
		int a = 0;
		m_aryColWidth.InsertAt(nColumn, a);
		for (int row = 0; row < m_nRows; row++) 
		{
			GRID_ROW* pRow = m_aryGridRows[row];
			if (!pRow) return -1;
...
//	ResetSelectedRange();
	...
	{
		int a = 0;
		m_aryRowHeight.InsertAt(nRow, a);
...
BOOL CfxGrid::PasteTextToGrid(CIdCell cell, COleDataObject* pDataObject)
{
	...
	CMemFile memFile((BYTE*) ::GlobalLock(hmem), (UINT)::GlobalSize(hmem));
	...
	memFile.Read(szBuffer, (UINT)::GlobalSize(hmem));
...
BOOL CfxGrid::IsValidDrop(COleDataObject*pDataObject)
{
	...
	memFile.Read(pData, (UINT)::GlobalSize(hmem));
	::GlobalUnlock(hmem); ::GlobalFree(hmem);
