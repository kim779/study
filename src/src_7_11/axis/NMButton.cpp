...
NMButton::NMButton(UINT id, COLORREF color, int mode, bool defaultColor)
{
	m_id = id;
	m_over = false;
	m_push = false;
...
BEGIN_MESSAGE_MAP(NMButton, CWnd)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_WM_DESTROY()
...
void NMButton::OnLButtonDown(UINT nFlags, CPoint point)
{
	ReleaseCapture();
	m_push = true;
	Invalidate(FALSE);
...
void NMButton::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT NMButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	m_over = false;
	Invalidate(FALSE);
	return 0;
...
// NMButton 메시지 처리기입니다.
LRESULT NMButton::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_LBUTTONDOWN:
...
void NMButton::OnSize(UINT nType, int cx, int cy)
{
...
void NMButton::set_ToolTipText(CString text, BOOL bActivate)
{
...
void NMButton::change_img(CString cool, CString over, CString down)
{
	set_Image(cool, over, down);
	Invalidate();
}

void NMButton::set_Image(UINT nID_cool, UINT nID_over, UINT nID_down)
{
	m_imgMode = true;
	if (m_Icool.GetSafeHandle())	m_Icool.DeleteObject();
	if (m_Iover.GetSafeHandle())	m_Iover.DeleteObject();
...
void NMButton::set_Image(CString img_cool, CString img_over, CString img_down)
{
	m_imgMode = true;
	if (m_Icool.GetSafeHandle())	m_Icool.DeleteObject();
	if (m_Iover.GetSafeHandle())	m_Iover.DeleteObject();
...
void NMButton::set_BkMaskSize(CRect size)
{
	m_rcMasking = size;
	m_bMasking = true;
}

bool NMButton::DrawImg(CDC* pDC)
{
	CRect	bRC;
	GetClientRect(bRC);
...
void NMButton::SetMaskDraw(bool bmask, COLORREF backColr)
{
	m_bMaskDraw = bmask;
	m_bkMask = backColr;
}

int NMButton::DrawBitmapByMask(CDC* pDC,int maskcolor)
{
	CRect	bRC;
	GetClientRect(bRC);
...
void NMButton::transparentBlt(CDC *pDC, CBitmap *pBitmap, int x, int y, int cx, int cy)
{
...
void NMButton::OnLButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
	m_push = false;
	Invalidate(FALSE);
