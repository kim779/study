...
void CAxBitmap::DrawTilte(CDC *pDC, RECT dest)
{
	CDC MemDC;
	CBitmap *poldbitmap;
	if(m_hObject == NULL)
	...
	CPen pen(PS_SOLID, 1, RGB(84, 91, 105));
	CPen pen2(PS_SOLID, 1, RGB(36, 38, 41));

	/*
	pDC->SelectObject(&pen); 
	pDC->MoveTo(dest.right - 70, 0);
	pDC->LineTo(dest.right - 70,  dest.bottom - 3);

	pDC->SelectObject(&pen2); 
	pDC->MoveTo(dest.right - 69, 0);
	pDC->LineTo(dest.right - 69,  dest.bottom - 3);

	pDC->SelectObject(&pen); 
	pDC->MoveTo(dest.right - 68, 0);
	pDC->LineTo(dest.right - 68,  dest.bottom - 3);
