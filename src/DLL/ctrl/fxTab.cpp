...
LOGFONT CfxTab::getLogFont(int fsize, DWORD weight)
{
	...
	strcpy_s(lf.lfFaceName, 32, "굴림체");    //vc2019
...
CBitmap* CfxTab::CreateBmp( LPCTSTR sBMPFile/*, CPalette *pPal */)
{
	...
	//if (file.ReadHuge((LPSTR)hDIB, nPackedDIBLen) != nPackedDIBLen )  //vc2019
...
void CfxTab::drawNormal(CDC *pDC)
{
	...
	for (int ii = 0; ii < GetItemCount(); ii++)
	{
		...
		const COLORREF color = pDC->SetTextColor(m_selectedfColor);
		if (bSelect)
		{
