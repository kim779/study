...
void CXListCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	else if (pLVCD->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
	{
		...
		const int nSubItem = pLVCD->iSubItem;
...
void CXListCtrl::DrawCheckbox(int nItem, int nSubItem, CDC *pDC, COLORREF crText,
				COLORREF crBkgnd, const CRect& rect, XLISTCTRLDATA *pXLCD)
{
	ASSERT(pDC);
	ASSERT(pXLCD);
	...
	if (pImageList)
	{
		...
		if (nImage >= 0)
		{
			if (rect.Width() > 0)
			{
				...
				const COLORREF rgb = pImageList->GetBkColor();
	...
	if (nItem != -1)
	{
		...
		if (nSubItem != -1)
		{
			if (pXLCD[nSubItem].nCheckedState >= 0)
			{
				...
				{
					const int nCheckedCount = CountCheckedItems(nSubItem);
...
int CXListCtrl::InsertItem(const LVITEM* pItem)
{
	...
	const int index = CListCtrl::InsertItem(pItem);
...
int CXListCtrl::InsertItem(int nItem, LPCTSTR lpszItem, COLORREF crText, COLORREF crBackground)
{
	...
	const int index = CListCtrl::InsertItem(nItem, lpszItem);
...
BOOL CXListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText)
{
	...
	const BOOL rc = CListCtrl::SetItemText(nItem, nSubItem, lpszText);
...
int CXListCtrl::GetCheckbox(int nItem, int nSubItem)
{
	...
	const XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(nItem);
	if (!pXLCD)
	{
		return -1;
	}
