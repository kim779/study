...
CTSetup::CTSetup(class CTMenu* menu, CWnd* pParent /*=NULL*/)
...
BOOL CTSetup::PreTranslateMessage(MSG* pMsg) 
{
	...
	{
		const CEdit* edit = m_usrtree.GetEditControl();
...
{
	const NM_TREEVIEW* pTView = (NM_TREEVIEW*)pNMHDR;
	*pResult = 0;
...
void CTSetup::redrawCombo()
{
	POSITION pos = NULL;
	CString key = _T("");
	const _Group*	pGroup = (_Group *)NULL;
...
void CTSetup::OnDelgroup() 
{
	...
	const int idx = m_Combo.GetCurSel();
	if (idx < 0)
	{
...
{
	const int tot = m_Combo.GetCount();
...
void CTSetup::AddTreeItem(CString mapN, HTREEITEM sItem)
{
	...
	const _Group*	pGroup = GetCurGroup();
...
void CTSetup::AddTreeGroup()
{
	...
	for (int ii = 0; ii <= m_subGroup.GetCount(); ii++)
	{
		gKeys.Format("#%03d", ii);
...
LONG CTSetup::OnUSER(WPARAM wParam, LPARAM lParam)
{
...
{
	const TV_DISPINFO* pTVDispInfo = (TV_DISPINFO*)pNMHDR;
	*pResult = 0;
...
{
	CString file;
	file.Format("%s\\%s\\usertool.ini", Axis::home, MTBLDIR);
...
{
	const int chk = ((CButton *)GetDlgItem(IDC_TOOLVIEW))->GetCheck();
	m_tool2 = chk ? 2 : 1;
}

void CTSetup::draw_groupBox(CPaintDC *pdc, int ids)
{
