...
void CLink::OnClickGlist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	*pResult = 0;
	...
	sName = profile.GetString("SNAME", m_smapN);
	Trim(sName);
...
void CLink::LoadInfo()
{
	CString	buffer, str, mapN;

	m_gList.DeleteAllItems();
	CProfile profile(Format("%s\\%s\\%s\\%s", Axis::home, USRDIR, Axis::user, m_mapN.Left(4)));

	buffer = profile.GetSectionLF(m_mapN);
	DWORD	dw = buffer.GetLength();
	if (dw > 0)
	{
	...
	((CButton*)GetDlgItem(IDC_USELINKBAR))->SetCheck(profile.GetInt(szShowBar, m_mapN, 1));

	profile.SetFileName(pkLinkName);
	((CButton*)GetDlgItem(IDC_DISPSNAME))->SetCheck(profile.GetInt("SNAME", "DISPN", 1));
}
...
void CLink::SaveInfo()
{
	...
	profile.SetFileName(pkLinkBar);
	profile.Write(szShowBar, m_mapN, Format("%d", !((CButton*)GetDlgItem(IDC_USELINKBAR))->GetCheck()));

	profile.SetFileName(pkLinkName);
	profile.Write("SNAME", "DISPN", Format("%d", !((CButton*)GetDlgItem(IDC_DISPSNAME))->GetCheck()));
}
...
void CLink::setTree()
{
	...
	fopen_s(&fp, GetProfileFileName(pkAxisMenu), "r");
	if (!fp)	return;
...
void CLink::OnDefaultlink() 
{
	...
	CString buffer(profile.GetSectionLF(m_mapN));
...
void CLink::OnSaveSName() 
{
	...
	CProfile profile(pkLinkName);
	profile.Write("SNAME", m_smapN, sName);
}
