...
BOOL CTotalSetup::OnInitDialog() 
{
	...
	m_dlgs.emplace_back(std::make_shared< CMNG_NXT>());
	...
	{
		wnd = getanyFind(item);
		if (i < MAX_MAIN_NODE_CNT)
			hItem = m_tree.InsertItem(wnd->m_caption, i, i, 0);

		if (wnd->m_caption.Find("장운영정보") >= 0)
		{
			m_tree.SetItemData(hItem, i);

			InsertChildTreeNode(hItem, "KRX 장운영", i, 9);
			InsertChildTreeNode(hItem, "NXT 장운영", xNODE_NXT, 10);
...
void CTotalSetup::InsertChildTreeNode(HTREEITEM hItem, CString sNode, int gubn, int iImg)
{
	TVINSERTSTRUCT tvis = {};
	tvis.hParent = hItem;                 // 부모 노드
	tvis.item.mask = TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM;

	tvis.item.pszText = (LPSTR)(LPCTSTR)sNode; // 노드 텍스트
	tvis.item.iImage = iImg;                  // 일반 상태 아이콘 (ImageList 인덱스)
	tvis.item.iSelectedImage = iImg;          // 선택 상태 아이콘
	tvis.item.lParam = gubn;               // 노드 데이터 (예: ID)
	HTREEITEM h = m_tree.InsertItem(&tvis);

	m_slog.Format("[MngSetup][%s]<%d> h=[%d]", __FUNCTION__, __LINE__, h);
...
CSetupDialog* CTotalSetup::getanyFind(std::any& item)
{
	...
	else if (item.type() == typeid(std::shared_ptr<CMNG_NXT>))
		wnd = std::any_cast<std::shared_ptr< CMNG_NXT>>(item).get();
	else
		wnd = nullptr;
...
void CTotalSetup::OnSelchangedTreemenu(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	m_slog.Format("[MngSetup][%s]<%d> m_index=[%d]", __FUNCTION__, __LINE__, m_index);
	OutputDebugString(m_slog);

	if (m_dlgs.size() > m_index)
	{
	...
	if (m_index > 9)
	{
		...
		case xNODE_NXT:
		{
			any item = m_dlgs.at(m_dlgs.size() - 1);
			CSetupDialog* wnd = std::any_cast<std::shared_ptr< CMNG_NXT>>(item).get();
	...
	else
	{
		...
		getanyFind(m_dlgs.at(m_index))->Loadsetup();
	}
...
void CTotalSetup::OnApply() 
{
	...
	switch (m_index)
	{
		case xNODE_NXT:
		{
			auto wnd = getanyFind(m_dlgs[m_index - 1]);
			wnd->ApplySetup();
			AfxGetMainWnd()->SendMessage(WM_USER, 3, IDD_MNGSETUP);
		}
		break;
	}
}
