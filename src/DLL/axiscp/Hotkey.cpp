...
CHotkey::CHotkey(class CTMenu* menu, CWnd* pParent /*=NULL*/)
	: CSetupDialog(CHotkey::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CHotkey, CSetupDialog)
	//{{AFX_MSG_MAP(CHotkey)
...
void CHotkey::loadKey()
{
	...
	for (int ii = 0; ii < m_hlist.GetSize(); ii++)
	{
		...
		str = profile.GetString(szHKey, key);
	...
	((CButton*)GetDlgItem(IDC_USEKEY))->SetCheck(profile.GetInt(szHKey, "state"));
	((CButton*)GetDlgItem(IDC_HOME))->SetCheck(profile.GetInt(szHKey, "home", 1));
	((CButton*)GetDlgItem(IDC_END))->SetCheck(profile.GetInt(szHKey, "end", 0));
}
...
{
	*pResult = 0;
	const UINT index = m_keyList.GetNextItem(-1, LVNI_SELECTED);
	changeState(index);
}
...
void CHotkey::changeState(int index)
{
	...
	switch (index)
	{
	case 32:	nRes = IDC_SQ;	break;	
	case 31:	nRes = IDC_DAT;		break;
	case 30:	nRes = IDC_RB;	break;
	case 29:	nRes = IDC_SLASH;		break;
	case 28:	nRes = IDC_SC;		break;
	case 27:	nRes = IDC_LB;		break;
	case 26:	nRes = IDC_COMMA;		break;
...
void CHotkey::writeKey()
{
	...
	dat.Format("%d", ((CButton *) GetDlgItem(IDC_HOME))->GetCheck());
	WritePrivateProfileString("HKEY", "HOME", dat, file);

	dat.Format("%d", ((CButton *) GetDlgItem(IDC_END))->GetCheck());
	WritePrivateProfileString("HKEY", "END", dat, file);
...
void CHotkey::setTree()
{
	...
	file.Format("%s\\%s\\%s", Axis::home, TABDIR, AXISMENU);

	FILE* fp;   //vc2019 fopen
	fopen_s(&fp, file, "r");
	if (!fp)	return;
...
void CHotkey::DefaultSetup()
{
	char	buf[256]{};
	CString	file, key, tmpS, str;
	file.Format("%s\\%s\\hkey.ini", Axis::home, MTBLDIR);
	...
	for (int ii = 0; ii < m_hlist.GetSize(); ii++)
	{
		key.Format("%c", m_hlist.GetAt(ii));
		m_keyList.InsertItem(ii, key);
		const DWORD dw = GetPrivateProfileString("HKEY", key, "", buf, sizeof(buf), file);
//		if (dw <= 0)	continue;
		str = buf;
		int	pos = str.Find(',');
//		if (pos == -1)	continue;
	...
	const int state = GetPrivateProfileInt("HKEY", "state", 0, file);
	((CButton *) GetDlgItem(IDC_USEKEY))->SetCheck(state);

	const int home = GetPrivateProfileInt("HKEY", "home", 1, file);
	((CButton *) GetDlgItem(IDC_HOME))->SetCheck(home);

	const int end = GetPrivateProfileInt("HKEY", "end", 1, file);
	((CButton *) GetDlgItem(IDC_END))->SetCheck(end);
}

void CHotkey::draw_groupBox(CPaintDC *pdc, int ids)
{
...
void CHotkey::OnPaint() 
{
	CPaintDC dc(this); // device context for painting
	
	// TODO: Add your message handler code here
	draw_groupBox(&dc, IDC_STATIC_22);
	draw_groupBox(&dc, IDC_STATIC_23);
