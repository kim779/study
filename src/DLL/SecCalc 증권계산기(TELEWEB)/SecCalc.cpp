...
void CSecCalcApp::LoadInfo()
{
	...
	CString spath, stmp;
	stmp = "CUSTOMERCALC.EXE\\";
	spath.Format("%s\\%s", chFileName, "calc.ini");
	spath.TrimRight();
...
//	m_sPath.Format("%s\\calc.ini", pPath);  //test  
//	m_sPath.Format("%s\\calc.ini", "C:\\Program Files\\IBKS_STAFF\\IBK hot Trading(Á÷¿ø¿ë)\\exe");
