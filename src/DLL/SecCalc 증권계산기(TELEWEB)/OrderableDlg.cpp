...
void COrderableDlg::OnRadioStock() {	OnMkRadio(mk_Stock);	}
void COrderableDlg::OnRadioFuture() {	OnMkRadio(mk_Future);	}
void COrderableDlg::OnRadioOption() {	OnMkRadio(mk_Option);	}
...
//		m_ctrlCheckSingle.ShowWindow(SW_SHOW); //test
...
void COrderableDlg::CalcOrderable(double dMoney, double dPrice)
{
	...
	double	dFeeRate = atof(m_pApp->m_pFeeRate[m_CurMarket])*0.01;
	...
	dFee = int(dFee / 10)*10+atoi(m_pApp->m_pCharFee[m_CurMarket]);
	str.Format("%.f", dFee);
