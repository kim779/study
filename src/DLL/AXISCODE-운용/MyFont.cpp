...
{
	const BYTE b=lf.lfOutPrecision;
...
{
	const BYTE b=lf.lfClipPrecision;
...
{
	const BYTE b=lf.lfPitchAndFamily;
...
{
	const BOOL b=(BOOL)lf.lfUnderline;
...
{
	const BOOL b=(BOOL)lf.lfStrikeOut;
...
void CMyFont::SetLogFont(const LOGFONT& logfont)
{
	lf=logfont;
	DeleteObject();
	CreateFontIndirect(&lf);
...
void CMyFont::ExtractFont(CString& str)
{
	...
	strcpy_s(lf.lfFaceName, (LPCTSTR)GetToken(str, ","));
