...
public:
	...
	void SetState(BOOL bstat = FALSE){ m_bState = bstat; }
	void SetSelected(BOOL bselect = FALSE){	m_bSelected = bselect;	}
	void SetFont(int nheight, CString sztext);
	void SetBold(BOOL flag);
	void SetSize(BOOL flag){ m_Bool = flag; }	
...
//	DWORD SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut = nullptr, int nCxDesiredOut = 0, int nCyDesiredOut = 0);
//	DWORD SetIcon(int nIconIn, int nIconOut = nullptr);
//	DWORD SetIcon(HICON hIconIn, HICON hIconOut = nullptr);

//	DWORD SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut = nullptr, COLORREF crTransColorOut = 0);
