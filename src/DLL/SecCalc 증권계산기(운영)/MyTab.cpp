...
const int bmp_selL = 0, bmp_selM = 1, bmp_selR = 2, bmp_norL = 3, bmp_norM = 4, bmp_norR = 5;
...
CMyTab::CMyTab()
{
	...
	m_bmpSelL = m_bmpSelM = m_bmpSelR = m_bmpL = m_bmpM = m_bmpR = NULL;
	for (int ii = 0; ii <= bmp_norR; ii++)
		m_pics[ii] = NULL;
...
BOOL CMyTab::Create(CRect rect, CWnd* parent, UINT nID)
{
	DWORD dwStyle = WS_CHILD|WS_VISIBLE;
	return CTabCtrl::Create(dwStyle, rect, parent, nID);
}

LOGFONT CMyTab::getLogFont(int fsize, DWORD weight)
{
	LOGFONT	lf;
...
void CMyTab::SetFont(CFont* pFont, bool redraw)
{
	m_pFont = pFont;
	if (redraw) RedrawWindow();
}

void CMyTab::SetColor(COLORREF fcolor, COLORREF bcolor, COLORREF sfcolor, COLORREF sbcolor, COLORREF backColor)
{
	m_frColor = fcolor;
	m_bkColor = bcolor;
...
void CMyTab::SetBorderColor(COLORREF bcolor)
{
	m_bLine = TRUE;
	m_linecolor = bcolor;
}

void CMyTab::SetStyle(DWORD style, bool redraw)
{
	SetMinTabWidth(5);
...
void CMyTab::OnDraw(CDC *pDC)
{
	drawImage(pDC);
}


void CMyTab::draw3dRoundRect(CDC *dc, CRect rc, int align, int depth, COLORREF borderColor)
{
	CRect tRc = rc;
...
void CMyTab::draw2Rect(CDC *dc, CRect rc, COLORREF color1, COLORREF color2)
{
	if (rc.Width() == 2)
	{
		CRect tRc = rc;
...
void CMyTab::SetImage(HBITMAP hselectL, HBITMAP hSelectM, HBITMAP hSelectR, HBITMAP hL, HBITMAP hM, HBITMAP hR)
{
...
void CMyTab::SetImage(CString imageDir, CString selectL, CString selectM, CString selectR, CString left, CString middle, CString right)
{
	ClearImage();
	...
	SetImage(m_bmpSelL->operator HBITMAP(), m_bmpSelM->operator HBITMAP(), m_bmpSelR->operator HBITMAP(), 
		m_bmpL->operator HBITMAP(), m_bmpM->operator HBITMAP(), m_bmpR->operator HBITMAP());	
}

void CMyTab::LoadTabImage(CString rootDir, bool bTopTab)
{
	CString path, prefix;
	CString selL, selM, selR, left, middle, right;
	...
	SetImage(path, selL, selM, selR, left, middle, right);
}

CBitmap* CMyTab::CreateBmp( LPCTSTR sBMPFile/*, CPalette *pPal */)
{
	CFile file;
...
int CMyTab::drawBmp(CDC *dc, CDC *memDC, CRect rc, int bmpidx, int posX, int posTo)
{
	HBITMAP pbmp = NULL;
	BITMAP	bm;
	...
	if (posTo == -1)
	{
		...
		dc->StretchBlt(posX, 0, bm.bmWidth, rc.Height(), //bm.bmHeight, 
...
int CMyTab::getBmpWidth(int bmpidx)
{
	BITMAP bm;
	GetObject(m_pics[bmpidx], sizeof(bm), &bm);
	return bm.bmWidth;
...
int CMyTab::getBmpHeight(int bmpidx)
{
	BITMAP bm;
	GetObject(m_pics[bmpidx], sizeof(bm), &bm);
	return bm.bmHeight;
}
void CMyTab::ClearImage()
{
...
void CMyTab::drawNormal(CDC *pDC)
{
	char        szTabText[100];
	TC_ITEM     tci;
	CRect rt, tRc1, tBRc;
...
void CMyTab::drawImage(CDC* pDC)
{
	...
	memDC.FillSolidRect(&rt, m_backColor);
	...
	for (int ii = 0; ii < GetItemCount(); ii++)
	{
		...
		if (bSelect)
		{
			...
			pDC->SetTextColor(m_selectedfColor);
			ileft = posX;
			ih = rtItem.bottom;//getBmpHeight(bmp_selR);
		} 
		else
		{

			posX = drawBmp(pDC, &memDC, rtItem, bmp_norL, posX);
			posX = drawBmp(pDC, &memDC, rtItem, bmp_norM, posX, rtItem.right - getBmpWidth(bmp_norR));
	...
	if(m_bLine && ileft > 0)
	{
		...
		pDC->MoveTo(ileft, ih-igap);
		pDC->LineTo(rt.right , ih-igap);
...
{
	int	index = GetCurSel();
	GetParent()->SendMessage(WM_CHILD, TAB_SELCHANGE, index);
	*pResult = 0;
}
