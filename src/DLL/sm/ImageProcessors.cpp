...
CSize CImageRotator::CalcDestSize(CSize sizeSrc)
{
	...
	const double dCosA = cos(m_dRadians);
	const double dSinA = sin(m_dRadians);
	...
	const int nMaxY = max(abs(ptTopLeft.y), max(abs(ptTopRight.y), max(abs(ptBottomLeft.y), abs(ptBottomRight.y))));
	const int nMaxX = max(abs(ptTopLeft.x), max(abs(ptTopRight.x), max(abs(ptBottomLeft.x), abs(ptBottomRight.x))));
...
BOOL CImageRotator::ProcessPixels(RGBX* pSrcPixels, CSize sizeSrc, RGBX* pDestPixels, CSize sizeDest)
{
	...
	else
	{
		...
		const CSize sizeDestOffset(sizeDest.cx / 2 + sizeDest.cx % 2, sizeDest.cy / 2 + sizeDest.cy % 2);
		const CSize sizeSrcOffset(sizeSrc.cx / 2 + sizeSrc.cx % 2, sizeSrc.cy / 2 + sizeSrc.cy % 2);
		...
		for (int nY = 0; nY < sizeDest.cy; nY++)
		{
			...
			const double dCosYComponent = (nY - sizeDestOffset.cy) * dCosA;
			const double dSinYComponent = (nY - sizeDestOffset.cy) * dSinA;
			...
			for (int nX = 0; nX < sizeDest.cx; nX++)
			{
				...
				if (rSrc.PtInRect(ptSrc))
				{
					if (!m_bWeightingEnabled)
					{
						...
						const RGBX* pRGBSrc = &pSrcPixels[ptSrc.y * sizeSrc.cx + ptSrc.x];
	...
	else
	{
		...
		for (int nX = 0; nX < sizeDest.cx; nX++)
		{
			...
			for (int nY = 0; nY < sizeDest.cy; nY++)
			{
				...
				const double dSrcX = nX - dXOffset;
				const double dSrcY = nY - dYOffset;

				if ((int)dSrcX >= 0 && (int)dSrcX < sizeSrc.cx && (int)dSrcY >= 0 && (int)dSrcY < sizeSrc.cy)
				{
					if (!m_bWeightingEnabled)
					{
						const RGBX* pRGBSrc = &pSrcPixels[(int)dSrcY * sizeSrc.cx + (int)dSrcX];
			...
			for (int nY = 0; nY < sizeSrc.cy; nY++)
			{
				...
				for (int nSubX = nX - 1; nSubX <= nX + 1; nSubX++)
				{
					for (int nSubY = nY - 1; nSubY <= nY + 1; nSubY++)
					{
						...
						{
							const RGBX* pRGBSub = &pSrcPixels[nSubY * sizeSrc.cx + nSubX];
...
BOOL CImageSharpener::ProcessPixels(RGBX* pSrcPixels, CSize sizeSrc, RGBX* pDestPixels, CSize sizeDest)
{
	...
	else
	{
		...
		const double dMinMaxRatio = (double)1 / (1 + (10 - m_nAmount) * 5);

		const double dMaxFactor = 1 / (4 * (1 + dMinMaxRatio));
		const double dMinFactor = dMaxFactor * dMinMaxRatio;

		const double dMask[9] = { -dMinFactor, -dMaxFactor, -dMinFactor, -dMaxFactor,  2, -dMaxFactor, -dMinFactor, -dMaxFactor, -dMinFactor };
...
BOOL CImageResizer::Enlarge(RGBX* pSrcPixels, CSize sizeSrc, RGBX* pDestPixels, CSize sizeDest)
{
	...
	const double dFactor = 1 / m_dFactor;
	double dXSrc = 0;
	...
	for (int nX = 0; nX < sizeDest.cx; nX++)
	{
		...
		for (int nY = 0; nY < sizeDest.cy; nY++)
		{
			...
			for (int nXSub = nXStart; nXSub <= nXEnd; nXSub++)
			{
				...
				{
					const RGBX* pRGBSrc = &pSrcPixels[nYSub * sizeSrc.cx + nXSub];
...
BOOL CImageNegator::ProcessPixels(RGBX* pSrcPixels, CSize sizeSrc, RGBX* pDestPixels, CSize sizeDest)
{
	...
	for (int nX = 0; nX < sizeSrc.cx; nX++)
	{
		for (int nY = 0; nY < sizeSrc.cy; nY++)
		{
			const RGBX* pRGBSrc = &pSrcPixels[nY * sizeSrc.cx + nX];
			RGBX* pRGBDest = &pDestPixels[nY * sizeSrc.cx + nX];
