...
CRegCode::CRegCode(CWnd* pParent /*=NULL*/)
...
void CRegCode::loadInfo()
{
	...
	while (file.Read(&regCode, sizeof(regCode)))
	{
		...
		const int row = m_selectList.GetItemCount();
...
void CRegCode::saveInfo()
{
	...
	path.Format("%s\\%s\\%s\\regCode", Axis::home, USRDIR, Axis::user);
	path_.Format("%s\\%s\\%s\\regCode_", Axis::home, USRDIR, Axis::user);
...
void CRegCode::LoadSCODE()
{
	...
	file.Format("%s\\%s\\%s", Axis::home, TABDIR, SCODE_FN);
...
void CRegCode::LoadCFCODE()
{
	...
	file.Format("%s\\%s\\%s", Axis::home, TABDIR, CFCODE_FN);
  	if (!fileH.Open(file, CFile::modeRead|CFile::typeBinary))
	{
		::MessageBox(m_hWnd, "Don't read cfcode file.", "Error!", MB_ICONASTERISK);
		return;
...
void CRegCode::LoadOTHER(char kind)
{
	...
	{
	case M_GROUP:
		file.Format("%s\\%s\\group.ini", Axis::home, TABDIR);
		value.LoadString(ST_REGCODE_GROUP);
		break;
	case M_THEME:
		file.Format("%s\\%s\\theme.ini", Axis::home, TABDIR);
		value.LoadString(ST_REGCODE_THEME);
		break;
	case M_INGRE:
		file.Format("%s\\%s\\ingre.ini", Axis::home, TABDIR);
	...
	{
		keys.Format("%02d", ii);
		const DWORD dw = GetPrivateProfileString("GROUP", keys, "", buf, sizeof(buf), file);
		if (dw <= 0)	break;
...
void CRegCode::setCode(char* dat, DWORD dw)
{
	...
	case M_OTC:
		{
		const int	Catagory = atoi(&dat[1]);
			for (int ii = 0; ii < m_arCode.GetSize(); ii++)
			{
				const _stockcode* stockcode = m_arCode.GetAt(ii);
	...
	{
		const int	itemCnt = m_codeList.GetItemCount();
...
void CRegCode::OnSelchangedCategorytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	setCode((char*)(const char *)dat, dw);
}
