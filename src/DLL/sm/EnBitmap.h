...
public:
	...
	RGBX Gray() 
	{ 
		const BYTE btGray = ((int)btBlue + (int)btGreen * 6 + (int)btRed * 3) / 10;
...
class AFX_EXT_CLASS C32BitImageProcessor
{
public:
...
//////////////////////////////////////////////////////////////////////////////////////////
class AFX_EXT_CLASS CEnBitmap : public CBitmap  
{
public:
...
inline RGBX C32BitImageProcessor::CalcWeightedColor(RGBX* pPixels, CSize size, double dX, double dY)
{
	...
	const RGBX* pRGB = &pPixels[nY * size.cx + nX]; // current

	const double dXFraction = dX - nX;
	const double dX1MinusFraction = 1 - dXFraction;
	
	const double dYFraction = dY - nY;
	const double dY1MinusFraction = 1 - dYFraction;
	
	const RGBX* pRGBXP = &pPixels[nY * size.cx + min(nX + 1, size.cx - 1)]; // x + 1
	const RGBX* pRGBYP = &pPixels[min(nY + 1, size.cy - 1) * size.cx + nX]; // y + 1
	
	const int nRed = (int)((dX1MinusFraction * pRGB->btRed +
						dXFraction * pRGBXP->btRed +
						dY1MinusFraction * pRGB->btRed +
						dYFraction * pRGBYP->btRed) / 2);
	
	const int nGreen = (int)((dX1MinusFraction * pRGB->btGreen +
						dXFraction * pRGBXP->btGreen +
						dY1MinusFraction * pRGB->btGreen +
						dYFraction * pRGBYP->btGreen) / 2);
	
	const int nBlue = (int)((dX1MinusFraction * pRGB->btBlue +
