...
CRunX::CRunX(CWnd* parent, COLORREF bk)
{
	m_parent = parent;
	m_bk = bk;
	m_code = true;
...
void CRunX::OnPaint() 
{
	...
	const COLORREF color = dc.SetBkColor(m_bk);
...
bool CRunX::make_FB()
{
	const DWORD	style = WS_CHILD|WS_CHILD|WS_VISIBLE|BS_GROUPBOX|BS_LEFT|BS_RIGHT;
	if (!m_group1.Create("", style, CRect(0,0,0,0), this, 1))
		return false;
	if (!m_group2.Create("", style, CRect(0,0,0,0), this, 2))
		return false;
	DWORD dwStyle = WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_TABSTOP
			|WS_BORDER|ES_NUMBER|ES_CENTER|ES_WANTRETURN;
	if (!m_edit.CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), "3", dwStyle, CRect(0,0,0,0), this, 200))
		return false;
	if (!m_spin.Create(WS_CHILD|WS_VISIBLE|WS_BORDER, CRect(0,0,0,0), this, EDIT_SPIN))
		return false;
...
#ifdef DF_USESTL_RUNX
	CString	tipS;
	dwStyle = WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
	m_play = std::make_unique<CNButton>(CTRL_PLAY);
	if (!m_play->Create(NULL, "start", dwStyle, CRect(0, 0, 0, 0), this, CTRL_PLAY))
		m_play = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXPLAY);
		m_play->set_ToolTipText(tipS);
		m_play->set_Image(CAxButtonName("codeplay"));
	}

	m_pause = std::make_unique<CNButton>(CTRL_PAUSE);
	if (!m_pause->Create(NULL, "pause", dwStyle, CRect(0, 0, 0, 0), this, CTRL_PAUSE))
		m_pause = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXPAUSE);
		m_pause->set_ToolTipText(tipS);
		m_pause->set_Image(CAxButtonName("codepause"));
	}

	m_continue = std::make_unique<CNButton>(CTRL_CONTINUE);
	if (!m_continue->Create(NULL, "continue", dwStyle, CRect(0, 0, 0, 0), this, CTRL_CONTINUE))
		m_continue = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXCONTINUE);
		m_continue->set_ToolTipText(tipS);
		m_continue->set_Image(CAxButtonName("codecontinue"));
	}

	m_stop = std::make_unique<CNButton>(CTRL_STOP);
	if (!m_stop->Create(NULL, "stop", dwStyle, CRect(0, 0, 0, 0), this, CTRL_STOP))
		m_stop = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXSTOP);
		m_stop->set_ToolTipText(tipS);
		m_stop->set_Image(CAxButtonName("codestop"));
	}

	m_prev = std::make_unique<CNButton>(CTRL_PREV);
	if (!m_prev->Create(NULL, "prev", dwStyle, CRect(0, 0, 0, 0), this, CTRL_PREV))
		m_prev = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXPREV);
		m_prev->set_ToolTipText(tipS);
		m_prev->set_Image(CAxButtonName("codeprev"));
	}

	m_next = std::make_unique<CNButton>(CTRL_NEXT);
	if (!m_next->Create(NULL, "next", dwStyle, CRect(0, 0, 0, 0), this, CTRL_NEXT))
		m_next = NULL;
	else
	{
		tipS.LoadString(ST_TEXT_RUNXNEXT);
		m_next->set_ToolTipText(tipS);
		m_next->set_Image(CAxButtonName("codenext"));
	}
#else
	CString	tipS;
	dwStyle = WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
...
void CRunX::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_play)
	{
		CRect	rc;
		GetClientRect(cRc);
		const CSize size = m_play->get_ImgSize();
		
		const int cy = gRc.bottom + (cRc.bottom - gRc.bottom - size.cy)/2;
		const int offset = size.cx + 5;
		const int move = (cRc.Width() - offset*4)/2;
...
LONG CRunX::OnXMSG(WPARAM wParam, LPARAM lParam)
{
