...
void CPPDrawManager::GetSizeOfBitmap(HBITMAP hBitmap, LPSIZE pSize) const
{
	...
	if (hBitmap != NULL)
	{
		BITMAP	csBitmapSize;
		// Get bitmap size
		const int nRetValue = ::GetObject(hBitmap, sizeof(csBitmapSize), &csBitmapSize);
		if (nRetValue)
		{
...
void CPPDrawManager::AlphaBitBlt(HDC hDestDC, int nDestX, int nDestY, DWORD dwWidth, DWORD dwHeight, HDC hSrcDC, int nSrcX, int nSrcY, int percent /* = 100 */)
{
	...
	if ((NULL != hSrcDib) && (NULL != pSrcBits))
	{
		...
		if ((NULL != hDestDib) && (NULL != pDestBits))
		{
			...
			const double src_darken = (double)percent / 100.0;
			const double dest_darken = 1.0 - src_darken;
...
{
	const BYTE nGrayColor = (BYTE)((GetBValue(clrColor) * 0.299) + (GetGValue(clrColor) * 0.587) + (GetRValue(clrColor) * 0.114));
...
{
	const BYTE nGrayColor = (BYTE)((GetRValue(clrColor) * 0.299) + (GetGValue(clrColor) * 0.587) + (GetBValue(clrColor) * 0.114));
...
HICON CPPDrawManager::StretchIcon(HICON hIcon, DWORD dwWidth, DWORD dwHeight)
{
	...
	{
		const DWORD	dwWidthOrg = csOriginal.xHotspot * 2;
		const DWORD	dwHeightOrg = csOriginal.yHotspot * 2;
								...
								BOOL bHorz/* = TRUE*/)
{
	...
	for (int i = 0; i < nSteps; i++)
    {
		...
		const BYTE bR = (BYTE) ((GetRValue(colorStart) * (nSteps - i) +
                   GetRValue(colorFinish) * i) >> nShift);
		const BYTE bG = (BYTE) ((GetGValue(colorStart) * (nSteps - i) +
                   GetGValue(colorFinish) * i) >> nShift);
		const BYTE bB = (BYTE) ((GetBValue(colorStart) * (nSteps - i) +
						  ...
						  BYTE coloring /* = 0 */, COLORREF hicr /* = 0 */, COLORREF midcr /* = 0 */, COLORREF locr /* = 0 */)
{
	...
	const int grainx2 = RAND_MAX/(max(1,2*granularity));
										...
										HDC hSrcDC, int nSrcX, int nSrcY, DWORD dwSrcWidth, DWORD dwSrcHeight)
{
	// Horizontal copying
	int right{}, bottom{};
	const int nDestRight = (int)(nDestX + dwDestWidth);
	const int nDestBottom = (int)(nDestY + dwDestHeight);
...
//		::SelectObject(hSrcDC, hBitmap);
	...
	else
	{
		...
		// original color
		//-------------------------------------------------------------------
		const COLORREF crOldColor = ::SetTextColor(hDC, RGB(0, 0, 0));
		//Merge the image mask with background
		::SelectObject(hSrcDC, csOriginal.hbmMask);
		::BitBlt(hDC, x, y, dwWidth, dwHeight, hSrcDC, 0, 0, SRCAND);
		//Draw the image
					...
					COLORREF clrShadow /*= PPDRAWMANAGER_SHADOW_COLOR*/)
{
	...
	const int nMaxCol = sz.cx / cx;
	const int nMaxRow = sz.cy / cy;
	const int nMaxImages = nMaxCol * nMaxRow;
								   ...
								   int nSize /* = 1 */)
{
	...
	const int nDoubleLineOffset = nSize * 2;
	switch (nStyle)
	{
