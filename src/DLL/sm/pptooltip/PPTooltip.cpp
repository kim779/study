...
BOOL CPPToolTip::IsCursorOverTooltip() const
{
	...
	const CPPToolTip * pWnd = (CPPToolTip*)WindowFromPoint(pt);
...
CString CPPToolTip::GetDebugInfoTool(LPPOINT lpPoint)
{
	...
	const HWND hwndTool = FindTool(lpPoint, ti);
...
BOOL CPPToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
	...
	const mapIter item = m_ToolMap.find(hWnd);
	...
	const CScrollView * pScroll = (CScrollView*)CScrollView::FromHandle(hWnd);
	if (pScroll->IsKindOf(RUNTIME_CLASS(CScrollView))) 
	{
		...
		const CPoint ptScroll = pScroll->GetScrollPosition();
...
void CPPToolTip::AddTool(CWnd * pWnd, const PPTOOLTIP_INFO & ti)
{
	...
	else if (nItemID && hSubMenu) 
	{
		...
		if (NULL != hwndMenu)
		{
			...
			for(int nItem = 0; nItem < count; nItem++) 
			{
				...
				{
					const UINT nState = GetMenuState(hSubMenu, nItemID, MF_BYCOMMAND);
					CString str;
...
////////////////////////////////////////////////////////////////////
void CPPToolTip::SetImageList(UINT nIdBitmap, int cx, int cy, int nCount, COLORREF crMask /* = Axis::maskColor */)
{
	m_drawer.SetImageList(nIdBitmap, cx, cy, nCount, crMask);
} //End of SetImageList

void CPPToolTip::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = Axis::maskColor */)
{
