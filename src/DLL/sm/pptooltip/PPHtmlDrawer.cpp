...
{
	const iter_mapStyles iter = m_mapSpecChars.find(lpszAlias);
...
void CPPHtmlDrawer::AddTagToList(LPCTSTR lpszName, DWORD dwTagIndex, LPCTSTR lpszFullName)
{
	...
	const iterMapTags iterMap = m_mapTags.find(lpszName);
...
DWORD CPPHtmlDrawer::GetTagFromList(CPPString sTagName, CPPString & strFullName, BOOL & bCloseTag)
{
	...
	const iterMapTags iterMap = m_mapTags.find(sTagName);
...
{
	const iterMapColors iterMap = m_mapColors.find(lpszColorName);
...
{
	const STRUCT_HYPERLINK & link = m_arrLinks [nLink];
...
//	TRACE (_T("CPPHtmlDrawer::OnLButtonDown()\n"));
	...
	const int nLink = PtInHyperlink(lpClient);
	if (nLink >= 0)
	{
...
{
    HKEY hkey;
	const LONG retval = RegOpenKeyEx(key, subkey, 0, KEY_QUERY_VALUE, &hkey);
...
//		m_arrTableSizes.clear();
	...
	while (nIndex < m_csHtmlText.GetLength())
	{
		...
		nBegin = nIndex;
		const BOOL bFoundTable = SearchTag(m_csHtmlText, nIndex, _T("table"));
...
//		sc.nWidth = 0;
	...
	const int nIndexTable = m_nCurTable;
	...
	if (MODE_FIRSTPASS != m_nNumPass)
	{
		...
		const int nRealWidth = lpRect->right - lpRect->left;
...
//
///////////////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::DrawHtmlTableRow(const CPPString & sRow, LPCRECT lpRect, STRUCT_TABLE & st, int nRow)
{
...
//				sc.szCell.cy += m_defStyle.nPadding + m_defStyle.nPadding;
				...
				const int nColSpan = sc.nColSpan + nCol;
				const int nRowSpan = sc.nRowSpan + nRow;
...
//
///////////////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::DrawHtmlTableCell(const CPPString & sCell, LPCRECT lpRect, STRUCT_CELL & sc)
{
	if (MODE_DRAW != m_nNumPass)
	{
		sc.szText.cx = 0;
	...
	const SIZE szSpan = AnalyseCellParam(sProperties, m_defStyle, FALSE);
...
//		sc.szText.cy = m_defStyle.nCellHeight;
	...
	while(nIndex < sCell.GetLength())
	{
		...
		const int nNearlyTag = min(nEndCell, nNewTable);
		SIZE szTemp = {0, 0};
...
SIZE CPPHtmlDrawer::DrawHtmlString (CPPString & sHtml, LPCRECT lpRect)
{
	...
	const COLORREF clrShadow = m_bIsEnable ? m_crShadow : GetColorByName("");
	...
	const int nTextWrapWidth = m_rcOutput.right - m_rcOutput.left;
...
//	} //if
	...
	const int nFirstLine = m_nCurLine;
	...
	while (i < sHtml.GetLength())
	{
		...
		if (!sText.IsEmpty())
		{
			...
			while (!sText.IsEmpty())
			{
				...
				const int nMaxSize = nTextWrapWidth - ptOutput.x + m_rcOutput.left;
		...
		if (!sTag.IsEmpty())
		{
			...
			const DWORD dwTag = GetTagFromList(sTag, m_defStyle.strTag, bCloseTag);
						...
						case TAG_ICON:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								if (NULL != hIcon)
								{
									//ENG: Image with shadow or not?
									//RUS: Изображени??тень?ил?не?
									const BOOL bShadow = IsImageWithShadow(si);
...
int CPPHtmlDrawer::InitNewLine(int x)
{
	...
	else if (MODE_DRAW == m_nNumPass)
	{
		...
		const int nRealWidth = m_rcOutput.right - m_rcOutput.left;
...
void CPPHtmlDrawer::Tag_Tabulation(LPPOINT lpPoint, int nNum)
{
	...
	if (!nNum)
		nNum = 1;
	const int nWidth = (lpPoint->x - m_rcOutput.left) % m_nTabSize;
	if (nWidth)
	{
		//aligns with tab
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::SetImageList(UINT nIdBitmap, int cx, int cy, int nCount, COLORREF crMask /* = Axis::maskColor */)
{
	// Load bitmap
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = Axis::maskColor */)
{
...
BOOL CPPHtmlDrawer::IsImageWithShadow(const _STRUCT_IMAGE & si)
{
	const DWORD dwStyles = si.nStyles | si.nHotStyles;
...
LPCTSTR CPPHtmlDrawer::GetTextStyle(LPCTSTR lpszStyleName)
{
	...
	name.MakeLower();
	const iter_mapStyles iterMap = m_mapStyles.find(name);
...
BOOL CPPHtmlDrawer::GetStyleFontStyle(const CPPString & str, BOOL bDefault)
{
...
int CPPHtmlDrawer::GetStyleFontWeight(const CPPString & str, int nDefault)
{
	if ((str == _T("normal")) || str.IsEmpty())
	{
		nDefault = FW_NORMAL;
...
int CPPHtmlDrawer::GetStyleHorzAlign(const CPPString & str, int nDefault)
{
...
int CPPHtmlDrawer::GetStyleVertAlign(const CPPString & str, int nDefault)
{
...
int CPPHtmlDrawer::GetStyleTextTransform(const CPPString & str, int nDefault)
{
...
COLORREF CPPHtmlDrawer::GetStyleColor(const CPPString & str, COLORREF crDefault)
{
//	if (!m_bIsEnable)
...
int CPPHtmlDrawer::GetLengthUnit(CPPString & str, int nDefault, BOOL bFont /* = FALSE */)
{
	...
	else
	{
		...
		else
		{
			...
			{
				const int nSize [] = {8, 10, 12, 14, 18, 24, 36};
...
int CPPHtmlDrawer::StyleBorder(const CPPString & str, int nDefault)
{
...
/////////////////////////////////////////////////////////////////
CPPString CPPHtmlDrawer::SearchNextTag(const CPPString & str, CPPString & strTag, int & nIndex)
{
	int nBegin{};
	CPPString sText = _T("");
	strTag.Empty();
...
//	A tag's name .
/////////////////////////////////////////////////////////////////
CPPString CPPHtmlDrawer::GetTagBody(const CPPString & str, int & nIndex)
{
...
CPPString CPPHtmlDrawer::SplitTag(CPPString & sTag)
{
	CPPString sParam(_T(""));
	int nIndex = 0;
	const TCHAR tch = GetIndexNextChars(sTag, nIndex, _T(" ="));
	if (tch != _T('\0'))
	{
...
CPPString CPPHtmlDrawer::GetNextProperty(const CPPString & str, int & nIndex, CPPString & sProp)
{
	CPPString sValue(_T(""));
	sProp.Empty();
...
// Return: A property's string 
/////////////////////////////////////////////////////////////////
CPPString CPPHtmlDrawer::SearchPropertyOfTag(const CPPString & str, int & nIndex)
{
	CPPString sText = _T("");
	...
	{
		//The begin of the property was found
		const int nBegin = nIndex;
		//Searching end of the property
		const TCHAR chFound = GetIndexNextChars(str, nIndex, _T(" ="));
		//Gets a property's string
		sText = str.Mid(nBegin, nIndex - nBegin);
	} //if
	return sText;
...
/////////////////////////////////////////////////////////////////
BOOL CPPHtmlDrawer::SearchTag(const CPPString & str, int & nIndex, CPPString strTag)
{
	strTag = _T("<") + strTag;
	while (nIndex < str.GetLength())
	{
...
/////////////////////////////////////////////////////////////////
BOOL CPPHtmlDrawer::GetIndexNextAlphaNum(const CPPString & str, int & nIndex, BOOL bArithmetic /* = FALSE */)
{
	TCHAR ch{};
...
/////////////////////////////////////////////////////////////////
TCHAR CPPHtmlDrawer::GetIndexNextChars(const CPPString & str, int & nIndex, CPPString strChars)
{
	int i{};
	for (; nIndex < str.GetLength(); nIndex++)
	{
...
/////////////////////////////////////////////////////////////////
TCHAR CPPHtmlDrawer::GetIndexNextNoChars(const CPPString & str, int & nIndex, CPPString strChars)
{
	int i{};
	BOOL bFound{};
	for (; nIndex < str.GetLength(); nIndex++)
...
/////////////////////////////////////////////////////////////////
BOOL CPPHtmlDrawer::GetBeginParameter(const CPPString & str, int & nIndex, TCHAR chSeparator /* = _T(':') */)
{
	TCHAR ch{};
	for (; nIndex < str.GetLength(); nIndex++) 
	{
...
/////////////////////////////////////////////////////////////////
CPPString CPPHtmlDrawer::GetParameterString(const CPPString & str, int & nIndex, TCHAR chBeginParam /* = _T(':') */, CPPString strSeparators /* = _T(";") */)
{
	if (GetBeginParameter(str, nIndex, chBeginParam))
	{
		//Parameter for the current property was found
		const TCHAR ch = GetIndexNextNoChars(str, nIndex, strSeparators + _T(" "));
		if (0 != ch)
		{
			const int nBegin = nIndex;
			...
			{
				nIndex++;
				const TCHAR ch = GetIndexNextChars(str, nIndex, _T("\""));
				if (_T('"') == ch)
				{
					nIndex ++;
...
/////////////////////////////////////////////////////////////////
CPPString CPPHtmlDrawer::GetNameOfTag(const CPPString & str, int & nIndex)
{
...
/////////////////////////////////////////////////////
SIZE CPPHtmlDrawer::GetTableDimensions(const CPPString & sTable)
{
		...
		if (SearchTag(sTable, nIndex, _T("tr")))
		{
			...
			do 
			{
				...
				if (nNewCell < nEndRow)
				{
					...
					const SIZE szSpan = AnalyseCellParam(sProperties, style, TRUE);
...
/////////////////////////////////////////////////////
void CPPHtmlDrawer::SearchEndOfTable(const CPPString & str, int & nIndex)
{
...
/////////////////////////////////////////////////////
void CPPHtmlDrawer::SearchEndOfRow(const CPPString & str, int & nIndex)
{
	nIndex += 4;
...
/////////////////////////////////////////////////////
void CPPHtmlDrawer::SearchEndOfCell(const CPPString & str, int & nIndex)
{
	nIndex += 4;
...
///////////////////////////////////////////////////////////////////////
SIZE CPPHtmlDrawer::AnalyseCellParam(const CPPString & sProperties, _STRUCT_CHANGESTYLE & cs, BOOL bTable)
{
	SIZE szSpan = {1, 1};
	if (sProperties.IsEmpty())
		return szSpan;
...
///////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::AnalyseImageParam(const CPPString & sProperties, _STRUCT_IMAGE & si)
{
	if (sProperties.IsEmpty())
		return;
...
//       [o] - overline
///////////////////////////////////////////////////////////////////////
void CPPHtmlDrawer::GetStyleFontShortForm(const CPPString & str)
{
	if (!str.IsEmpty())
	{
		BOOL bSetValue = TRUE;
...
UINT CPPHtmlDrawer::GetStyleImageShortForm(const CPPString & str)
{
...
BOOL CPPHtmlDrawer::IsPercentableValue(const CPPString & str)
{
	if (!str.IsEmpty())
	{
...
int CPPHtmlDrawer::GetTableWidth(const CPPString & str, int nClientWidth, int nMinWidth, BOOL bSet /* = FALSE */)
{
	if (!str.IsEmpty())
	{
		int i = 0;
		...
		while (i < str.GetLength())
		{
			...
			if (strProperty == _T("width"))
			{
				...
				{
					const int nWidth = GetLengthUnit(strParameter, 100);
					if (bSet)
					{
						if (nWidth <= 100)
...
void CPPHtmlDrawer::DrawBackgroundImage(HDC hDC, int nDestX, int nDestY, int nWidth, int nHeight, CPPString strNameImage)
{
	...
	if (GetIndexNextAlphaNum(strNameImage, nIndex))
	{
		const int nBegin = nIndex;
		//Searching end of the style name
		const TCHAR chSymbol = GetIndexNextChars(strNameImage, nIndex, _T(" :"));
		if (0 != chSymbol)
		{
			//Gets a property's name
			...
			if (strName == _T("idres"))
			{
				const UINT nID = (UINT)GetLengthUnit(strParameter, 0);
								  ...
								  BOOL bGradient /* = TRUE */, int nDepthX /* = 7 */, int nDepthY /* = 7 */)
{
	...
	const BYTE nColor = ::MulDiv(255, 100 - m_nDarkenShadow, 100);
