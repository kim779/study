...
public:
	...
	void SetImageList(UINT nIdBitmap, int cx, int cy, int nCount, COLORREF crMask = Axis::maskColor);
	void SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask = Axis::maskColor);
...
//	SIZE DrawHtmlTableRow(CPPString & sRow, LPCRECT lpRect, vecCol & row); //Draws the HTML row of the table
	void DrawHtmlTableRow(const CPPString & sRow, LPCRECT lpRect, STRUCT_TABLE & st, int nRow);
	void DrawHtmlTableCell(const CPPString & sCell, LPCRECT lpRect, STRUCT_CELL & sc); //Draws the HTML cell of the table
...
//public:
	...
	CPPString SearchNextTag(const CPPString & str, CPPString & strTag, int & nIndex); //Search next tag
	BOOL SearchTag(const CPPString & str, int & nIndex, CPPString strTag); //Search begin of the specified tag

	CPPString GetTagBody(const CPPString & str, int & nIndex); //Gets a name of tag and the parameters of tag
	CPPString SplitTag(CPPString & sTag); //Split a tag to the tag's name and parameters
	CPPString GetNextProperty(const CPPString & str, int & nIndex, CPPString & sProp); //Gets next property


	CPPString SearchPropertyOfTag(const CPPString & str, int & nIndex); //Search a name or a property of a tag
	SIZE  AnalyseCellParam(const CPPString & sTag, _STRUCT_CHANGESTYLE & cs, BOOL bTable);
	void  AnalyseImageParam(const CPPString & strTag, _STRUCT_IMAGE & si);
	BOOL  IsImageWithShadow(const _STRUCT_IMAGE & si);
	...
	SIZE  GetTableDimensions(const CPPString & sTable); //Gets dimensions of the table
	void  SearchEndOfTable(const CPPString & str, int & nIndex); //Searching end of the table
	void  SearchEndOfRow(const CPPString & str, int & nIndex); //Searching end of the row
	void  SearchEndOfCell(const CPPString & str, int & nIndex); //Searching end of the cell
	...
	BOOL GetIndexNextAlphaNum(const CPPString & str, int & nIndex, BOOL bArithmetic = FALSE);
	BOOL GetBeginParameter(const CPPString & str, int & nIndex, TCHAR chSeparator = _T(':'));
	TCHAR GetIndexNextChars(const CPPString & str, int & nIndex, CPPString strChars);
	TCHAR GetIndexNextNoChars(const CPPString & str, int & nIndex, CPPString strChars);
	CPPString GetParameterString(const CPPString & str, int & nIndex, TCHAR chBeginParam = _T(':'), CPPString strSeparators = _T(";"));
	CPPString GetNameOfTag(const CPPString & str, int & nIndex);
	...
	BOOL GetStyleFontStyle(const CPPString & str, BOOL bDefault);
	int  GetStyleFontWeight(const CPPString & str, int nDefault);
	int  GetStyleHorzAlign(const CPPString & str, int nDefault);
	int  GetStyleVertAlign(const CPPString & str, int nDefault);
	COLORREF GetStyleColor(const CPPString & str, COLORREF crDefault);
	int  GetStyleTextTransform(const CPPString & str, int nDefault);
	CPPString GetStyleString(CPPString str, CPPString strDefault);
	void GetStyleFontShortForm(const CPPString & str);
	UINT GetStyleImageShortForm(const CPPString & str);
	...
	int StyleBorder(const CPPString & str, int nDefault);

	//Get
	int GetLengthUnit(CPPString & str, int nDefault, BOOL bFont = FALSE);
	BOOL IsPercentableValue(const CPPString & str);
	int GetTableWidth(const CPPString & str, int nClientWidth, int nMinWidth, BOOL bSet = FALSE);
