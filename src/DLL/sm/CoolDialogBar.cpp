...
CCoolDialogBar::CCoolDialogBar(CWnd* parent, bool (*callback)(int, WPARAM, LPARAM), CWnd* wizard) :
...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd/*, CDialog *pDialog, CString &pTitle*/, UINT nID, DWORD dwStyle) 
{
	...
	if (!CWnd::Create(wndclass, "돌려보기", dwStyle|WS_CLIPCHILDREN,
		CRect(0,0,m_cMinWidth,m_cMinHeight), pParentWnd, 0xE807))  //vc2019 돌려보기가 안보일때
		return FALSE;

	// create the child dialog
#ifdef DF_USESTL_CATEGORY
	m_cDialog = std::make_unique<CBasebar0>(m_parent, m_axiscall);
#else
	m_cDialog = new CBasebar0(m_parent, m_axiscall);
#endif
...
LRESULT CCoolDialogBar::OnNcHitTest(CPoint point)  //vc2019 UINT->LRESULT
{
	if (IsFloating())
...
void CCoolDialogBar::ParingGroupList(char* data, int len)
{
	CString stmp, seq, sgname, sRet;
	char* pdata = data;
...
#ifdef DF_USESTL_CATEGORY
	m_cDialog.get()->m_categoryPopup.get()->InsertGroupList(sRet);
#endif
...
void CCoolDialogBar::ParingGroupCode(char* data, int len)
{
	char* pdata = data;
...
#ifdef DF_USESTL_CATEGORY
	m_cDialog.get()->m_categoryPopup.get()->InsertCodeInGroup(sRet);
#endif
