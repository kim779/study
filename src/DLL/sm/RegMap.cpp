...
CRegMap::CRegMap(class CTMenu* menu, CWnd* pParent /*=NULL*/)
...
void CRegMap::SaveInfo()
{
	...
	CProfile profile(pkScreenReg);
	profile.WriteSection(szScreen, "");
	for (int ii = 0; ii < tot; ii++)
	{
		keys.Format("%04d", ii);
		//value.Format("%s%s00", head, m_gList.GetItemText(ii, 0));
		profile.Write(szScreen, keys, m_menu->GetMap(m_gList.GetItemText(ii, 0)));
	}
}
...
void CRegMap::setTree()
{
	...
	char	buf[1024]{};
	CString	file(GetProfileFileName(pkAxisMenu)), dat;
