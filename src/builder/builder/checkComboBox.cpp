...
extern "C" LRESULT FAR PASCAL ComboBoxListBoxProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		if (wParam == VK_SPACE)
		{
			...
			m_pComboBox->GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetWindowLongPtr(m_pComboBox->m_hWnd, GWLP_ID), CBN_SELCHANGE), (LPARAM)m_pComboBox->m_hWnd);
			return 0;
		}
		break;
...
LRESULT CCheckComboBox::OnCtlColorListBox(WPARAM wParam, LPARAM lParam)
{
	if (m_listBox == 0)
	{
		...
		if (hWnd != 0 && hWnd != m_hWnd)
		{
			m_listBox = hWnd;
			// Do the subclassing
			m_pWndProc = (WNDPROC)GetWindowLongPtr(m_listBox, GWLP_WNDPROC);
			SetWindowLongPtr(m_listBox, GWLP_WNDPROC, (LONG_PTR)ComboBoxListBoxProc);
		}
	}
