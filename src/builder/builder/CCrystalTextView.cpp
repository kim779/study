...
#define LINENUM_GAP		10	// Line Number. add. KSI
...
BEGIN_MESSAGE_MAP(CCrystalTextView, CView)
	...
	ON_WM_MOUSEACTIVATE()
	ON_COMMAND(ID_EDIT_LINENUMBER, OnSetLinNumber)
END_MESSAGE_MAP()
...
int CCrystalTextView::GetLineActualLength(int nLineIndex)
{
	...
	m_pnActualLineLength[nLineIndex] = (int)nActualLength;
	return (int)nActualLength;
}
...
void CCrystalTextView::DrawSingleLine(CDC *pdc, const CRect &rc, int nLineIndex)
{
	...
	CString sLine = LINESTRING;
	sLine.Remove('\n');
	if (strcmp(CString(pszChars, strlen(sLine)), sLine) == 0)
	{
		CPen	Pen, *pOldPen;
		Pen.CreatePen(PS_SOLID, 1, GetSysColor(COLORINDEX_GRAYLINE));
...
void CCrystalTextView::DrawMargin(CDC *pdc, const CRect &rect, int nLineIndex)
{
	...
	if (nImageIndex >= 0)
	{
		...
		{ 
			...
			VERIFY(m_pIcons->Create(IDB_MARGIN_ICONS, 14, 14, RGB(255, 255, 255)));
		}
...
// add by KSI
void CCrystalTextView::DrawLineNumber(CDC *pdc, const CRect &rect, int nLineIndex)
{
	CRect rc, rcNum, rcLine;

	CString str;
	CPen	Pen, *pOldPen;
	CFont *pOldFont = pdc->SelectObject(GetFont());
	COLORREF crOldText = pdc->SetTextColor(GetColor(COLORINDEX_LINENUMBER));

	rc = rcNum= rcLine = rect; 
	rcNum.right -= LINENUM_GAP;
	rcLine.left = rcLine.right - LINENUM_GAP;
	...
	pdc->FillSolidRect(rect, GetColor(COLORINDEX_WHITESPACE));
	pdc->DrawText(str, rcNum, DT_VCENTER|DT_SINGLELINE|DT_RIGHT);
	pdc->MoveTo(rcLine.left + (LINENUM_GAP/2), rcLine.top);
	pdc->LineTo(rcLine.left + (LINENUM_GAP/2), rcLine.bottom);
...
void CCrystalTextView::OnDraw(CDC *pdc)
{
	...
	CRect rcCacheNumber, rcCacheLine;
	if (m_bLineNum)
	{
		rcCacheNumber.SetRect(GetMarginWidth(), 0, (int)(GetMarginWidth() * 3), nLineHeight);
		rcCacheLine.SetRect((int)(GetMarginWidth() * 3), 0, rcLine.Width(), nLineHeight);
	}
	else
	...
	while (rcLine.top < rcClient.bottom)
	{
		if (nCurrentLine < nLineCount)
		{
			...
			if (m_bLineNum)
				DrawLineNumber(&cacheDC, rcCacheNumber, nCurrentLine);
...
void CCrystalTextView::CalcLineCharDim()
{
	...
	m_nLineHeight = szCharExt.cy + m_nHeightGap;
	if (m_nLineHeight < 1)
		m_nLineHeight = 1;
...
void CCrystalTextView::RecalcPageLayouts(CDC *pdc, CPrintInfo *pInfo)
{
	...
	m_nPrintLineHeight = pdc->GetTextExtent(_T("X")).cy + m_nHeightGap;
...
CPoint CCrystalTextView::ClientToText(const CPoint &point)
{
	...
	// add Line Number
	int szLineNumMargin = 0;
	if (m_bLineNum)
		szLineNumMargin = GetMarginWidth() * 2;

	int nPos = m_nOffsetChar + (point.x - GetMarginWidth() - szLineNumMargin) / GetCharWidth();
	if (nPos < 0)
		nPos = 0;
	...
	if (nIndex > 0 && IsDBCSLeadByte((BYTE)pszLine[nIndex]) && !IsLeadByte(pszLine, nIndex))
		nIndex--;
...
bool CCrystalTextView::IsLeadByte(LPCTSTR pChar, int nLen)
{
	int	ii;

	for (ii = 0; ii <= nLen; ii++)
	{
		if (IsDBCSLeadByte((BYTE)pChar[ii]))
		{
			if (ii == nLen)
				return true;
			ii++;
...
CPoint CCrystalTextView::TextToClient(const CPoint &point)
{
	...
	pt.x = (pt.x - m_nOffsetChar) * GetCharWidth() + GetMarginWidth() + szLineNumMargin;
	return pt;
}
...
void CCrystalTextView::OnEditFind() 
{
	...
	if (m_bLastSearch)
	{
		...
		dlg.m_matchCase = (m_dwLastSearchFlags & FIND_MATCH_CASE) != 0;
		dlg.m_wholeWord = (m_dwLastSearchFlags & FIND_WHOLE_WORD) != 0;
		dlg.m_direction = (m_dwLastSearchFlags & FIND_DIRECTION_UP) != 0 ? 0 : 1;
		if (m_pszLastFindWhat != NULL)
	...
	else
	{
		...
		dlg.m_matchCase = pApp->GetProfileInt(REG_FIND_SUBKEY, REG_MATCH_CASE, FALSE);
		dlg.m_wholeWord = pApp->GetProfileInt(REG_FIND_SUBKEY, REG_WHOLE_WORD, FALSE);
		dlg.m_direction = 1;		// Search down
	...
	// Execute Find dialog
	dlg.m_currentPt = m_ptCursorPos;		// Search from cursor position
	...
	pApp->WriteProfileInt(REG_FIND_SUBKEY, REG_MATCH_CASE, dlg.m_matchCase);
	pApp->WriteProfileInt(REG_FIND_SUBKEY, REG_WHOLE_WORD, dlg.m_wholeWord);
...
void CCrystalTextView::OnEditRepeat() 
{
	...
	{
		CString sText = SelCursorWord();
		
		m_bLastSearch = TRUE;
		if (m_pszLastFindWhat != NULL)
			free(m_pszLastFindWhat);
		#ifdef _UNICODE
				m_pszLastFindWhat = _wcsdup(sText);
		#else
				m_pszLastFindWhat = _strdup(sText);
		#endif
	}
...
void CCrystalTextView::OnUpdateEditRepeat(CCmdUI* pCmdUI) 
{
	if (::GetKeyState(VK_CONTROL) & 0x800)
		...
		pCmdUI->Enable(m_bLastSearch || IsSelection());
}
...
// [LWJ]
CString CCrystalTextView::SelCursorWord()
{
	CString sText = "";
	if (!IsSelection())
	{
	...
	if (IsSelection())
	{
		...
		if (ptSelStart.y == ptSelEnd.y)
		{
			LPCTSTR pszChars = GetLineChars(ptSelStart.y);
			int nChars = ptSelEnd.x - ptSelStart.x;
			lstrcpyn(sText.GetBuffer(nChars + 1), pszChars + ptSelStart.x, nChars + 1);
			sText.ReleaseBuffer();
		}
...
int CCrystalTextView::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	...
	if (pParentFrame == pDesktopWnd)	// docked
