...
void CTrListctrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_FocusCol != hitinfo.iSubItem || m_FocusRow != hitinfo.iItem)
	{
		// Mouse moved over a new cell
		m_FocusCol = hitinfo.iSubItem;
		m_FocusRow = hitinfo.iItem;

		if (m_FocusRow != -1 && m_FocusCol == 8)
		{
			//CString str = GetItemText(m_FocusRow, m_FocusCol);
			//TRACE("str:%s\n",str);
			//////////////////
			InitToolTip();
			m_ToolTip.Activate(FALSE);
			CString str = m_ArryMdsc.GetAt(m_FocusRow);
			CDC* pDC = GetDC();
			CSize szText = pDC->GetTextExtent(str);
			ReleaseDC(pDC);
			m_ToolTip.AddTool(this, str, NULL, 0);
			m_ToolTip.SetMaxTipWidth(szText.cx);
			m_ToolTip.SetDelayTime(TTDT_AUTOPOP, 360000);
...
BOOL CTrListctrl::PreTranslateMessage(MSG* pMsg)
{
