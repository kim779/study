...
bool CFileTree::checkMapOnly(CString path)
{
	...
	if (file.Open(path, CFile::modeRead))
	{
		...
		if (bRead)
		{
			if (strReadLine.Find(_T("<GLOBAL_DIVISION>")) != -1)
			{
				file.Close();
				return true;
			}
...
bool CFileTree::checkTrxOnly(CString path)
{
	CString strReadLine;
	CStdioFile file;
...
void CFileTree::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (hitem)
	{
		...
		else if (GetItemPathName(tmps, hitem) && checkTrxOnly(tmps))
