...
void CResultEditCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bMode)
		m_pRMenu->ModifyMenu(5, MF_BYCOMMAND, 5, _T("Change TEXT Mode"));
	else
		m_pRMenu->ModifyMenu(5, MF_BYCOMMAND, 5, _T("Change BIN Mode"));
...
LRESULT CResultEditCtrl::OnMessage(WPARAM wParam, LPARAM lParam)
{
	CHARRANGE cr;
	...
	if (cr.cpMin == cr.cpMax)
	{
		...
		m_sRtn = GetSelText();
	return (LONG_PTR)m_sRtn.operator LPCTSTR();
}


void CResultEditCtrl::OnContextMenu(CWnd* pWnd, CPoint point)
{
	int	idx = -1;
	long	start, end;
