...
void CCrystalTextBuffer::SUndoRecord::SetText(LPCTSTR pszText)
{
	...
	if (pszText != NULL && pszText[0] != _T('\0'))
	{
		...
		if (nLength > 1)
		{
			int	size = (nLength + 1) * sizeof(TCHAR);
			m_pszText = new TCHAR[size];
			ZeroMemory(m_pszText, size);
			strcpy_s(m_pszText, size, pszText);
		}
		else
		{
...
{
	if (HIWORD((DWORD_PTR)m_pszText) != 0)
	{
		delete[] m_pszText;
		m_pszText = NULL;
	}
...
#else
		_splitpath_s(pszFileName, drive, _MAX_PATH, dir, _MAX_PATH, name, _MAX_PATH, ext, _MAX_PATH);
#endif
...
void CCrystalTextBuffer::WriteText(LPCTSTR pszText, int nCrlfStyle)
{
	FreeAll();
	ASSERT(! m_bInit);
	ASSERT(m_aLines.GetSize() == 0);
	...
	char *pcBuf = (char *) _alloca(strlen(pszText));
	DWORD dwCurSize;
	memcpy(pcBuf, pszText, strlen(pszText));
	dwCurSize = (int)strlen(pszText);
...
CString CCrystalTextBuffer::ReadText(LPCTSTR pszCRLF)
{
	CString text;
	...
	int nStartLine = 0, nEndLine = GetLineCount() - 1;
	int nBufSize = 0;
	for (int L = nStartLine; L <= nEndLine; L ++)
	{
		nBufSize += m_aLines[L].m_nLength;
	...
	if (nStartLine < nEndLine)
	{
		int nCount = m_aLines[nStartLine].m_nLength;
		if (nCount > 0)
		{
			memcpy(pszBuf, m_aLines[nStartLine].m_pcLine, sizeof(TCHAR) * nCount);
			pszBuf += nCount;
		}
		memcpy(pszBuf, pszCRLF, sizeof(TCHAR) * nCRLFLength);
		pszBuf += nCRLFLength;
		...
		memcpy(pszBuf, m_aLines[nEndLine].m_pcLine, sizeof(TCHAR) * m_aLines[nEndLine].m_nLength);
		pszBuf += m_aLines[nEndLine].m_nLength;
	}
	else
	{
		memcpy(pszBuf, m_aLines[nStartLine].m_pcLine, sizeof(TCHAR) * m_aLines[nEndLine].m_nLength);
		pszBuf += m_aLines[nEndLine].m_nLength;
	}
	pszBuf[0] = 0;
	text.ReleaseBuffer();
