...
// CTmplView
#define	ID_TMPL_TREE		1
#define	ID_TMPL_LIST		2
#define	ID_TMPL_COMBO		3
#define	ID_TMPL_EDIT		4

IMPLEMENT_DYNAMIC(CTmplView, CDockablePane)
...
BEGIN_MESSAGE_MAP(CTmplView, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
	...
	ON_CBN_SELENDOK(ID_TMPL_COMBO, OnSelChange)
	ON_UPDATE_COMMAND_UI_RANGE(ID_REFRESH, ID_LIST, OnUpdateCommand)
	ON_MESSAGE(WM_USER+100, &CTmplView::OnMessage)
END_MESSAGE_MAP()


// CTmplView 메시지 처리기입니다.


int CTmplView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_tree.Create(dwViewStyle, rectDummy, this, ID_TMPL_TREE))
	{
		TRACE0(_T("서버템플릿 트리뷰를 만들지 못했습니다.\n"));
		return -1;
	}
	m_tree.Initialize(m_dir, true);

	// listview mode
	// group combo
	m_group.Create(WS_CHILD | WS_BORDER | WS_VSCROLL | CBS_DROPDOWNLIST | CBS_AUTOHSCROLL | CBS_HASSTRINGS, rectDummy, this, ID_TMPL_COMBO);

	// list ctrl
	if (!m_list.Create(WS_CHILD| LVS_REPORT | LVS_ALIGNTOP | LVS_NOCOLUMNHEADER | LVS_SHOWSELALWAYS | LVS_SORTASCENDING |  WS_CLIPSIBLINGS | WS_CLIPCHILDREN, rectDummy, this, ID_TMPL_LIST))
	{
		TRACE0(_T("서버템플릿 리스트뷰를 만들지 못했습니다.\n"));
		return -1;
	}
	...
	if (!m_edit.Create(WS_CHILD | WS_BORDER | WS_TABSTOP | ES_AUTOHSCROLL | ES_UPPERCASE, rectDummy, this, ID_TMPL_EDIT))
	{
...
void CTmplView::OnSize(UINT nType, int cx, int cy)
{
...
void CTmplView::adjustLayout()
{
	...
	else
	{
		...
		m_group.SetWindowPos(NULL, rect.left, rect.top, rect.Width(), 100, SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOZORDER);
...
void CTmplView::OnRefresh()
{
	...
	m_tree.RemoveAll();
	m_section.Lock();
	wccSetTempPath(tmps, _T("tmplres.ini"));
	...
	if (!dn->Download(UserID, _T(""), _T(""), tmps, 3))
	{
		delete dn;
		m_section.Unlock();
		return;
...
void CTmplView::OnTree()
{
	m_isTree = false;
	adjustLayout();
}

void CTmplView::OnOpen()
{
	if (m_isTree)
		m_tree.OpenMap();
}

void CTmplView::OnTwoDepth()
{
	m_isTwoDepth = !m_isTwoDepth;
	m_tree.SetDepth(m_isTwoDepth ? L_SELC : L_TGID);
}

void CTmplView::OnList()
{
	m_isTree = true;
	adjustLayout();
}

void CTmplView::OnSelChange()
{
	struct	_mapTree	mt;
...
void CTmplView::OnUpdateCommand(CCmdUI *pCmdUI)
{
	switch (pCmdUI->m_nID)
	{
	case ID_LIST:
...
afx_msg LRESULT CTmplView::OnMessage(WPARAM wp, LPARAM lp)
{
	switch (LOWORD(wp))
	{
	...
	case 2: // download
		{
			...
			for (POSITION pos = files->GetStartPosition(); pos; )
			{
				...
				if (!dn->Download(UserID, _T(""), fulls, tmps, is_template))
				{
					delete dn;
					m_section.Unlock();
					return 0;
...
int CTmplView::GetFiles(CStringArray& files)
{
	if (m_isTree)
...
void CTmplView::OpenMap(CString maps)
{
	m_tree.OpenMap(maps);
}

int CTmplView::IsTwoDepth()
{
	return m_isTwoDepth ? 1 : 0;
}

void CTmplView::SetTwoDepth(bool twoDepth)
{
	m_isTwoDepth = twoDepth;
	m_tree.SetDepth(m_isTwoDepth ? L_SELC : L_TGID, false);
}

void CTmplView::setFont()
{
...
BOOL CTmplView::PreTranslateMessage(MSG* pMsg)
{
	CRect	rc;
