...
public:
	CEditOrder(CWnd* pParent, class mapForm* mapForm, int nKind = 0);   // 표준 생성자입니다.
	virtual ~CEditOrder();
...
protected:
	...
	CString	getCellIO(class CformProp* Prop, int idx = -1);
	int	cellSize(class CformProp* Prop, int idx = -1);
	CString	getIO(int ioType);
	CString	getDataKind(int charType);
