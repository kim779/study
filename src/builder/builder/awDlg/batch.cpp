...
BOOL CBatch::OnInitDialog()
{
	...
	m_download->Download(UserID, _T(""), _T(""), tmps, 2);
	delete m_download;
	loadFile();
...
void CBatch::OnBatch()
{
	...
	for (int ii = 0; ii < m_downArray.GetSize(); ii++)
	{
		...
		m_download->Download(UserID, _T(""), svrMap, _T(""), 0);
		delete m_download;
		...
		if (compile(mapName))
		{
			...
			m_upload->Upload(svrMap, 1/*W_BIN*/, false, &m_mapH);
			delete m_upload;
...
bool CBatch::isValidTabOrder(class CFormItem* formItem, int idx)
{
	...
	//case fmOBJECT:
	//	if (formItem->GetForm(idx)->iok == ioINPUT || formItem->GetForm(idx)->iok == ioBOTH)
	//		return true;
	//	break;
	//}
	return false;
...
bool CBatch::isValidIOOrder(class CFormItem* formItem, int idx)
{
	...
	//case fmBROWSER:
	//	if (formItem->GetForm(idx)->iok == ioOUTPUT)
	//		return true;
	//	break;
	//case fmRADIO:
	//case fmCHECK:
	//	if(formItem->GetForm(idx)->iok != ioFREE)
	//		return true;
...
void CBatch::sortIoOrder()
{
	...
	//			if (kind == fmTABLE)
	//			{
	//				if (!(formItem.GetForm(ii)->attr2 & gaMULTI) && !(formItem.GetForm(ii)->attr2 & gaXHEAD))
	//				{
