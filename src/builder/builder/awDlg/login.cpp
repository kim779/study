...
BOOL CLogin::OnInitDialog()
{
	...
	if (GetPrivateProfileInt(ENVIRONMENT, "OFFLINE", 0, m_currentPath + _T("\\workshop.ini")))
		m_offline = FALSE;
	else
		m_offline = TRUE;
	...
	m_setBN.SetWindowPos(NULL, m_bitmapSz.cx - 155, m_bitmapSz.cy - 81, 51, 24, SWP_NOZORDER);
	m_connectBN.SetWindowPos(NULL, m_bitmapSz.cx - 213, m_bitmapSz.cy - 81, 59, 24, SWP_NOZORDER);
	m_cancelBN.SetWindowPos(NULL, m_bitmapSz.cx - 20, 7, 10, 10, SWP_NOZORDER);

	GetDlgItem(IDC_DOMAIN)->SetWindowPos(NULL, m_bitmapSz.cx - 209, m_bitmapSz.cy - 100, 100, 14, SWP_NOZORDER);
...
{
	GetDlgItem(IDC_DOMAIN)->EnableWindow(bEnable);
