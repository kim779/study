	...
	: CPropertyPage(CBaseInfoPage::IDD)
{
	...
	m_current = (char *)AfxGetMainWnd()->SendMessage(WM_USER, ID_USR_GETCURRENTPATH);

	path = m_current + _T("\\workshop.ini");
	m_gap     = GetPrivateProfileInt(ENVIRONMENT, _T("gap"), 5, path);
	
	GetPrivateProfileString(ENVIRONMENT, SHOPIPS, _T(""), wb, sizeof(wb), path);
	m_ips = wb;
