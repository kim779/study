...
Cimage::Cimage(CiForm* ifm, CString fileN, bool only)
{
	m_draw = ifm;
	...
	if (only)
	{
		bitmap = (CBitmap*)m_draw->SetImage((char *)fileN.operator LPCTSTR());
		m_images.Add(bitmap);
	...
	else
	{
		for (int ii = 0; ii < 4; ii++)
		{
			...
			bitmap = (CBitmap*)m_draw->SetImage((char *)tmps.operator LPCTSTR());
			m_images.Add(bitmap);
			m_valids.Add(bitmap ? 1 : 0);
		}
	}
