...
CfmSheet::CfmSheet(CAxisForm* axform, struct _formR* formR)
...
void CfmSheet::ReadAll(CString& data, bool edit, bool head)
{
	...
	int	index = min((int)m_data.GetUpperBound(), max(0, m_item));
...
void CfmSheet::ReadData(CString& data, bool edit, int col, int row)
{
	data.Empty();
