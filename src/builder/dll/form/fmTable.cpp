...
CfmTable::CfmTable(CAxisForm* axform, struct _formR* formR, struct _cellR* cellR)
	: CfmBase(axform, formR)
{
	...
	for (int ii = 0; ii < m_totRows; ii++)
	{
		...
		if (m_cellR[ii].head != NOVALUE)
			pair->m_head = m_axform->GetString(m_cellR[ii].head);
		else
			pair->m_head = _T("");
...
CString CfmTable::GetSymbolName(int col)
{
	if (col < 0)
...
void CfmTable::ReadData(CString& data, bool edit, int col, int row)
{
	data = _T("");
	if (row >= m_totRows)
		return;
	...
	if (m_cellR[row].attr & FA_FLOAT)
	{
		...
		if (pos >= 0)
		{
			...
			else
			{
				...
				if (str.IsEmpty())
					str += _T("0");
				str += CString('0', m_cellR[row].vals[1] - text.GetLength());
				str += text;
				tmps = str;
			}
		}
...
CString CfmTable::editData(CString str, int index)
{
	...
	formatData(m_cellR[index].attr, tmpS, (m_cellR[index].vals[1] == m_cellR[index].size) ? -1 : (int)m_cellR[index].vals[1]);
	return tmpS;
	...
	if (m_cellR[index].attr & FA_COMMA || m_cellR[index].attr & FA_FLOAT)
	{
		...
		{
			for (ii = 0; ii < (int)m_cellR[index].vals[1]; ii++)
			{
