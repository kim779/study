...
void CAxisForm::LoadForm(CRect mRect, bool drawOnly)
{
	...
	if (m_mapH->fName != NOVALUE)
		m_fonts = (char *)&m_strR[m_mapH->fName];
	else
		m_fonts = _T("");
	...
	for (int ii = 0; ii < m_mapH->formN; ii++)
	{
		...
		case FM_SYS:
			CfmSysm* fmSysm;
			fmSysm = new CfmSysm(this, &m_formR[ii]);
			ASSERT_VALID(fmSysm);
			m_fmObs.Add(fmSysm);
			break;
		...
		case FM_LABEL:
			CfmLabel* fmLabel;
			fmLabel = new CfmLabel(this, &m_formR[ii]);
		...
		case FM_BOX:
			CfmBox* fmBox;
			fmBox = new CfmBox(this, &m_formR[ii]);
			ASSERT_VALID(fmBox);
			m_fmObs.Add(fmBox);
			break;
		...
		case FM_GROUP:
			CfmGroup* fmGroup;
			fmGroup = new CfmGroup(this, &m_formR[ii]);
		...
		case FM_PANEL:
			CfmPanel* fmPanel;
			fmPanel = new CfmPanel(this, &m_formR[ii]);
		...
		case FM_BUTTON:
			...
			case BTN_RADIO:
				CfmRadio* fmRadio;
				fmRadio = new CfmRadio(this, &m_formR[ii]);
				ASSERT_VALID(fmRadio);
				m_fmObs.Add(fmRadio);
				SetRadioGroup(fmRadio->GetSymbolName(), m_formR[ii].properties & PR_CHECKED ? true : false);
				break;

			case BTN_CHECK:
				CfmCheck* fmCheck;
				fmCheck = new CfmCheck(this, &m_formR[ii]);
			...
			default:
				CfmButton* fmButton;
				fmButton = new CfmButton(this, &m_formR[ii]);
		...
		case FM_EDIT:
			...
			{
				CfmEditEx* fmEditEx;
				fmEditEx = new CfmEditEx(this, &m_formR[ii]);
			...
			CfmEdit* fmEdit;
			fmEdit = new CfmEdit(this, &m_formR[ii], m_drawOnly);
			ASSERT_VALID(fmEdit);
			m_fmObs.Add(fmEdit);
			break;
		...
		case FM_COMBO:
			CfmCombo* fmCombo;
			fmCombo = new CfmCombo(this, &m_formR[ii]);
		...
		case FM_OUT:
			CfmOut* fmOut;
			fmOut = new CfmOut(this, &m_formR[ii]);
			ASSERT_VALID(fmOut);
			m_fmObs.Add(fmOut);
			break;
		...
		case FM_MEMO:
			CfmMemo* fmMemo;
			fmMemo = new CfmMemo(this, &m_formR[ii], m_drawOnly);
			ASSERT_VALID(fmMemo);
			m_fmObs.Add(fmMemo);
			break;
		...
		case FM_GRID:
			...
			if (m_formR[ii].vals[0] != NOVALUE)
				fmGrid = new CfmGrid(this, &m_formR[ii], &m_cellR[m_formR[ii].vals[0]]);
			else
				fmGrid = new CfmGrid(this, &m_formR[ii]);
			ASSERT_VALID(fmGrid);
			m_fmObs.Add(fmGrid);
			break;
		...
		case FM_TABLE:
			CfmTable* fmTable;
			if (m_formR[ii].vals[0] != NOVALUE)
				fmTable = new CfmTable(this, &m_formR[ii], &m_cellR[m_formR[ii].vals[0]]);
			else
				fmTable = new CfmTable(this, &m_formR[ii]);
		...
		case FM_OBJECT:
			CfmObject* fmObject;
			fmObject = new CfmObject(this, &m_formR[ii]);
		...
		case FM_TAB:
			CfmTab* fmTab;
			fmTab = new CfmTab(this, &m_formR[ii]);
			ASSERT_VALID(fmTab);
			m_fmObs.Add(fmTab);
			break;
		...
		case FM_UTAB:
			...
			fmUserTab = new CfmUserTab(this, &m_formR[ii]);
		...
		case FM_SHEET:
			CfmSheet* fmSheet;
			fmSheet = new CfmSheet(this, &m_formR[ii]);
		...
		case FM_BROWSER:
			...
			fmBrowser = new CfmBrowser(this, &m_formR[ii], m_drawOnly);
		...
		case FM_CONTROL:
			CfmCtrl* fmCtrl;
			fmCtrl = new CfmCtrl(this, &m_formR[ii], m_drawOnly);
			ASSERT_VALID(fmCtrl);
			m_fmObs.Add(fmCtrl);
			break;
		...
		default:
			CfmBase* fmBase;
			fmBase = new CfmBase(this, &m_formR[ii]);
			ASSERT_VALID(fmBase);
			m_fmObs.Add(fmBase);
			break;
		}
...
int CAxisForm::FindForm(CfmBase* form)
{
	for (int ii = 0; ii < m_mapH->formN; ii++)
	{
		if (form == (CfmBase *)m_fmObs.GetAt(ii))
			return ii;
	}
	return -1;
}
...
void CAxisForm::ResizeForm(float hRatio, float vRatio, bool redraw)
{
	m_hRatio = hRatio;
	m_vRatio = vRatio;
...
CString CAxisForm::GetString(DWORD offset)
{
	if (offset != NOVALUE)
