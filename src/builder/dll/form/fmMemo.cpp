...
IMPLEMENT_DYNCREATE(CfmMemo, CfmBase)
...
CfmMemo::CfmMemo(CAxisForm* axform, struct _formR* formR, bool drawOnly)
	: CfmEdit(axform, formR, false)
{
	m_memo = (Cmemo *) NULL;
	m_pWnd = (CWnd *) NULL;
	m_drawOnly = drawOnly;
...
BEGIN_DISPATCH_MAP(CfmMemo, CfmBase)
	...
	DISP_FUNCTION(CfmMemo, "LimitText", _LimitText, VT_EMPTY, VTS_I4)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
void CfmMemo::_LimitText(long nChar) 
{
...
void CfmMemo::ReadData(CString& data, bool edit, int col, int row)
{
	ReadAll(data, edit);
}
