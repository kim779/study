...
struct	_rsmH {
	unsigned char	dirF;		/* dirf_INB   0x01      */
					/* dirf_OUTB  0x02      */
	unsigned char	resK;		/* */
	unsigned char	resC;		/* */
	unsigned char	resF;		/* */
	char	usid[32];		/* user ID          */		
	char	resN[128 + 1];		/* resource name         */
	char	mode;		/* Mode : R / W / T  */		
	char	rcod;		/* return code if response   */		
				/* 0: OK, other: ERROR   */		
	char	rsvB[4];		/* reserved        */
	char	datL[5];		/* data length (rcmhdr 길이 제외)*/
};
...
#define resk_TRX	0x06          /* TRAN Layout file        */		
...
#define resk_REQT	0x84            /* Transaction Layout List       */	

#define	resK_ERR	0x90		// resource error indicator
#define	resK_ERR2	0x99		// error ... terminate

////////////////////////////////////////////////////////////////////
#define resk_LOGIN	0x8a            /* login struct                 */							
#define resk_CHKOUT	0x8b            /* check out resource struct        */							
#define resk_HISTORY	0x8c            /* load resource history struct */	
#define resk_SEARCH	0x8d										
