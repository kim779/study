	...
	: CDialog(CScreenDlg::IDD, pParent)
{
	...
	const int pos = m_user.Find('|');
	if (pos < 0)	m_user = _T("Anonymous");
	else		m_user = m_user.Mid(pos+1);
	if (m_user.IsEmpty())	m_user = _T("Anonymous");
...
void CScreenDlg::OnADD() 
{
	...
	for (ii = 0; ii < m_list.GetItemCount(); ii++)
	{
		...
		if (!mapN.CompareNoCase(tmps))
		{
			...
			if (Axis::MessageBox(msg, MB_YESNO) == IDNO)
			{
	...
	if (m_parent->SendMessage(WM_AXIS, MAKEWPARAM(axSCREEN, 0), (LPARAM)(const char *)tmps))
	{
		...
		if (m_list.GetItemCount() > 8)
		{
			...
			col.fmt = LVCFMT_LEFT;
			col.pszText = (char *)(const char*)m_colName;
			col.iSubItem = 0;
			m_list.GetClientRect(rc);
			col.cx = rc.Width();
...
void CScreenDlg::OnDelete() 
{
	...
	if (m_parent->SendMessage(WM_AXIS, MAKEWPARAM(axSCREEN, 1), (LPARAM)(const char *)tmps))
	{
...
{
	const NM_LISTVIEW* pListView = (NM_LISTVIEW *)pNMHDR;
	*pResult = 0;
