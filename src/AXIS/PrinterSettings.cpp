...
BOOL CPrinterSettings::SetPrinterDevice(LPCTSTR pszDeviceName) const
{
	...
	const int dwsize = sizeof(*p2->pDevMode);
...
CString CPrinterSettings::DevmodePrinterName()
{
	...
	{
		const DEVMODE* pDevMode = (DEVMODE*)GlobalLock(m_hDevMode);
...
int CPrinterSettings::Save(LPCTSTR strFileName)
{
	...
	const BOOL ret = CreateDirectory(m_strdirname,NULL);
	if(!ret)
	{
		dret = GetLastError();
...
{
	const int nBytes = min( pDest->dmSize + pDest->dmDriverExtra, pSrc->dmSize + pSrc->dmDriverExtra);
