...
// 프로그래머가 디버깅 할 수 있도록 합니다.
	...
	char *pStack = (LPSTR)GetFirstStackTraceString(GSTSO_MODULE|GSTSO_SYMBOL|GSTSO_SRCLINE|GSTSO_PARAMS, pExPtrs);
	while (pStack)
	{
		strReason += CString(pStack) + "<br>";
		pStack = (LPSTR)GetNextStackTraceString(GSTSO_MODULE|GSTSO_SYMBOL|GSTSO_SRCLINE|GSTSO_PARAMS, pExPtrs);
	}

	if (strReason.GetLength() > 500)
		strReason = strReason.Left(500);
